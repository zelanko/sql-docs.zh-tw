---
title: SQL Server 升級的資料庫測試助理中捕捉追蹤
description: 在資料庫測試助理中捕捉追蹤
ms.custom: ''
ms.date: 10/22/2018
ms.prod: sql
ms.prod_service: dea
...
1. 在 [**新增 Capture**] 底下, 輸入或選取下列資訊:
    ...
    - **SQL Server 實例名稱**:輸入您要用來捕捉伺服器追蹤之 SQL Server 的電腦名稱稱。
    ...
    - **持續時間 (分鐘)** :選取您想要執行追蹤捕捉的時間長度 (以分鐘為單位)。
    - **儲存輸出追蹤檔案的路徑**:選取追蹤檔案的目的地路徑。 
...
是, 在追蹤收集期間, 效能會有最小的影響。 在我們的測試中, 我們發現大約有 3% 的記憶體壓力。
...
- 確認追蹤名稱不符合標準變換慣例 (Capture\_1)。 相反地, 請嘗試追蹤名稱\_, 例如 Capture 1a 或 Capture1。
