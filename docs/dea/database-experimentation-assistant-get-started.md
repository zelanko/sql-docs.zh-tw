---
title: 工作負載比較程式的總覽
description: 資料庫測試助理 (DEA) 是 A/B 測試解決方案，用於 SQL Server 環境中的變更，例如升級或新的索引。
ms.date: 12/12/2019
ms.prod: sql
ms.prod_service: dea
ms.suite: sql
...
ms.locfileid: "87951213"
---
# <a name="overview-of-the-workload-comparison-process"></a>工作負載比較程式的總覽

資料庫測試助理 (DEA) 可協助您評估您目前環境中來源伺服器 (上的工作負載，) 將在新環境中執行。 DEA 會藉由完成三個階段，引導您執行 A/B 測試：
...
- 若要重新執行工作負載追蹤，您的電腦必須設定為以 Distributed Replay (DReplay) 追蹤來執行。
...
- 如果遺失一或多個元件，當您嘗試產生新的分析報表時，會出現包含下載連結的必要條件頁面， () 所需的網際網路連接。
