...
1. 若要執行 Docker 容器映像，您可以使用下列命令從 bash 殼層 (Linux/macOS) 或提高權限的 PowerShell 命令提示字元。
 ...
   | **-p 1401:1433** | 將主機環境上的 TCP 通訊埠對應至 （第一個值） 與容器中的 TCP 連接埠 （第二個值）。 在此範例中，SQL Server 在容器中接聽 TCP 1433 這連接埠並在主機上公開 1401 連接埠。 |
   | **-名稱 sql1** | 指定容器名稱，而不是一個隨機產生的自訂名稱。 如果您執行多個容器，您無法重複使用這個相同的名稱。 |
...
1. 如果**STATUS**資料行會顯示狀態為**UP**、 然後 SQL Server 正在執行中容器和接聽的通訊埠中指定**連接埠**資料行。 如果**STATUS**資料行的 SQL Server 容器為**Exited**，請參閱[疑難排解 > 一節的設定指南](sql-server-linux-configure-docker.md#troubleshooting)。
...
您也可以連接到 SQL Server 執行個體在 Docker 電腦上從任何外部 Linux、 Windows 或 macOS 可支援 SQL 連線的工具。

下列步驟會使用您容器之外的**sqlcmd**連接到容器內中執行的 SQL Server。 這些步驟假設您已經安裝您的容器之外的 SQL Server 命令列工具。 使用其他工具 時，適用相同的主體，而每項工具的連接是唯一的程序。

1. 尋找主控您的容器之電腦的 IP 位址。 在 Linux 上使用**ifconfig**或**ip 位址**。在 Windows 中，使用**ipconfig**。

1. 執行指定的 IP 位址和連接埠對應到您的容器中的通訊埠 1433 埠的 sqlcmd。 在此範例中，這是在主機上的連接埠 1401 埠。
...
> [!WARNING]
> 停止並移除容器，永久刪除容器中的任何 SQL Server 資料。 如果您要保留您的資料，[建立並將備份檔案複製容器之外](tutorial-restore-backup-in-sql-server-container.md)或使用[容器資料持續性技術](sql-server-linux-configure-docker.md#persist)。
