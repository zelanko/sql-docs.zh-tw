...
本主題將介紹[SQL Server Management Studio (SSMS)](/sql-docs/docs/ssms/sql-server-management-studio-ssms)和會逐步引導您透過幾個常見工作。 SSMS 是 Windows 應用程式，因此使用 SSMS 時可以連線到遠端的 SQL Server 執行個體，在 Linux 上的 Windows 電腦。

[SQL Server Management Studio (SSMS)](/sql-docs/docs/ssms/sql-server-management-studio-ssms)是一套的 Microsoft 提供免費的開發和管理的需求不需要支付費用的 SQL 工具的一部分。 SSMS 是一個整合式的環境，可存取、 設定、 管理、 管理和開發 SQL Server 的執行在內部部署或雲端，在 Linux、 Windows 或 macOS 和 Azure SQL Database 上的 Docker 和 Azure SQL 資料倉儲中的所有元件。 SSMS 利用許多豐富的指令碼編輯器，以提供開發人員和所有技能等級的系統管理員存取 SQL Server 合併了非常廣泛的圖形工具。
...
在您的伺服器已成功建立新的資料庫。 如果您想要建立新的資料庫使用 T-SQL，則請參閱[CREATE DATABASE (SQL Server TRANSACT-SQL&AMP;)](/sql-docs/docs/t-sql/statements/create-database-sql-server-transact-sql)。
...
從您的伺服器，順利卸除資料庫。 如果您想要卸除資料庫，使用 T-SQL，則請參閱[DROP DATABASE (SQL Server TRANSACT-SQL&AMP;)](/sql-docs/docs/t-sql/statements/drop-database-transact-sql)。

## <a name="use-activity-monitor-to-see-information-about-sql-server-activity"></a>使用 活動監視器來查看 SQL Server 活動相關資訊

[活動監視器](/sql-docs/docs/relational-databases/performance-monitor/activity-monitor)工具是內建到 SQL Server Management Studio (SSMS)，並顯示 SQL Server 處理序以及這些處理序如何影響目前的 SQL Server 執行個體的相關資訊。
...
- [教學課程：撰寫國際性通用的 Transact-SQL 陳述式](/sql-docs/docs/t-sql/tutorial-writing-transact-sql-statements)
- [伺服器效能與活動監視](/sql-docs/docs/relational-databases/performance/server-performance-and-activity-monitoring)
