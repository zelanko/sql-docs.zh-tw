...
3. **Mssql** 擴充功能需要花費一分鐘來安裝。 等候提示，告訴您已成功安裝。
   ...
   > [!NOTE]
   > MacOS，您必須安裝 OpenSSL。 這是適用於.Net 的必要條件 mssql 擴充功能所使用的核心。 請遵循**安裝必要條件**中的步驟[.Net 核心指示]。 或者，您可以執行下列命令，在您 macOS 終端機。
   >
   >   ```bash
   >   brew update
