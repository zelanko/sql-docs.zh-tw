...
- 必須使用 AT_KEYEXCHANGE KeySpec 選項來建立憑證。 通常，憑證的金鑰使用方法屬性 (KEY_USAGE) 也包括金鑰編密 (CERT_KEY_ENCIPHERMENT_KEY_USAGE)。
- 憑證的 Subject 屬性必須指出一般名稱 (CN)，做為主機名稱或伺服器電腦的完整的網域名稱 (FQDN) 會相同。 附註： 支援萬用字元憑證。 

## <a name="overview"></a>概觀
TLS 用來加密從用戶端應用程式的連線[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]。 如果設定正確，TLS 提供隱私權與資料完整性，以用戶端與伺服器之間的通訊。  TLS 連線可以是起始的用戶端或伺服器起始。 


## <a name="client-initiated-encryption"></a>用戶端起始加密 
- **產生憑證**（/CN 應符合您的 SQL Server 主機完整的網域名稱）
...
- **註冊用戶端電腦 （Windows、 Linux 或 macOS） 上的憑證**
    ...
    -   如果您使用自我簽署的憑證，只要將.pem 檔案複製到下列資料夾發佈到個別並執行命令，讓它們 
