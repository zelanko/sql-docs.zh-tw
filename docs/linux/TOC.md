...
### [建立及執行作業](sql-server-linux-run-sql-server-agent-job.md)
...
### [從 Windows 還原](sql-server-linux-migrate-restore-database.md)
...
### [容錯移轉叢集執行個體](sql-server-linux-shared-disk-cluster-configure.md)
...
### [Active Directory 驗證](sql-server-linux-active-directory-authentication.md)

## [將效能最佳化](sql-server-linux-performance-get-started.md)
