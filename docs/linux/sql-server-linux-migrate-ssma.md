...
本主題將介紹[SQL Server 移轉小幫手 (SSMA)](http://msdn.microsoft.com/library/mt613434.aspx) ，以協助您輕鬆地將資料庫從 Microsoft Access、 DB2、 MySQL、 Oracle 及 Sybase 移轉至在 Linux 上的 SQL Server 2017 RC2。 SSMA 是 Windows 應用程式，因此使用 SSMA 時可以連線到遠端的 SQL Server 執行個體，在 Linux 上的 Windows 電腦。 

SSMA 支援各種不同的來源資料庫包括 Oracle、 MySQL、 Sybase、 DB2 和 Microsoft Access 在 Linux 上的 SQL Server 2017 RC2，並有助於自動化移轉工作，例如：
- 評估您的來源資料庫
...
若要開始使用，請下載 SQL Server 移轉小幫手 (SSMA) 為您的來源資料庫，請從下列清單：
