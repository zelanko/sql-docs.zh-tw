...
## <a name="arguments"></a>引數  
 ...
 [ @runinscaleout =] *runinscaleout*  
 表示是否在向外執行。 在向外執行封裝中使用值為 1。 使用 0 的值來執行封裝而不向外。 這個參數是選擇性的。 它會設定為 DEFAULT_EXECUTION_MODE [SSISDB] 中。[目錄]。[catalog_properties]，如果未指定。 *Runinscaleout*是**元**。 
 
 [ @useanyworker =] *useanyworker*  
  指出是否允許任何標尺出背景工作執行時，不要。 您可以使用 1 的值來執行具有任何標尺出背景工作的封裝。 使用值為 0 表示並非所有標尺出背景工作可執行封裝。 這個參數是選擇性的。 它是設定為 1，如果未指定。 *Useanyworker*是**元**。 
