...
ms.topic: "article"
helpviewer_keywords: 
  - "變數 [Integration Services], 在封裝之間傳送"
  - "使用者自訂變數 [Integration Services]"
  - "範圍 [Integration Services]"
  - "系統變數 [Integration Services]"
  - "變數 [Integration Services]"
  - "變數 [Integration Services], 關於變數"
  - "值 [Integration Services]"
...
## 系統及使用者定義變數  
 ...
 所有的變數 (系統變數和使用者自訂變數) 都可在「執行 SQL」工作用來將變數對應至 SQL 陳述式之參數的參數繫結中使用。 如需詳細資訊，請參閱[執行 SQL 工作](../integration-services/control-flow/execute-sql-task.md)和[執行 SQL 工作中的參數和傳回碼](../Topic/Parameters%20and%20Return%20Codes%20in%20the%20Execute%20SQL%20Task.md)。  
...
-   指定變數的值和資料類型。  
 ...
 如需變數之實際使用狀況的詳細資訊，請參閱[在封裝中使用變數](../Topic/Use%20Variables%20in%20Packages.md)。  
...
## 設定變數  
 ...
 如需可以在 [[!INCLUDE[ssIS](../includes/ssis-md.md)] 設計師] 中設定之屬性的詳細資訊，請參閱[變數視窗](../integration-services/variables-window.md)。  
  
 若要了解變數屬性，以及以程式設計方式設定這些屬性的詳細資訊，請參閱<xref:Microsoft.SqlServer.Dts.Runtime.Variable>。  
  
## 相關工作  
 [加入、刪除、變更封裝中使用者定義變數的範圍](../Topic/Add,%20Delete,%20Change%20Scope%20of%20User-Defined%20Variable%20in%20a%20Package.md)  
  
 [設定使用者定義變數的屬性](../Topic/Set%20the%20Properties%20of%20a%20User-Defined%20Variable.md)  
  
 [在子封裝中使用變數和參數的值](../integration-services/packages/use-the-values-of-variables-and-parameters-in-a-child-package.md)  
