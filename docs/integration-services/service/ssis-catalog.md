...
> [!NOTE] 您無法為 **SSISDB** 資料庫重新命名。  
  
> [!NOTE] 如果附加 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SSISDB **資料庫的** 執行個體停止或沒有回應，ISServerExec.exe 處理序便會結束。 會在 Windows 事件記錄檔中寫入一則訊息。  
>   
...
-   [目錄物件識別碼](../../integration-services/service/ssis-catalog.md#CatalogObjectIdentifiers)  
  
-   [目錄組態](../../integration-services/service/ssis-catalog.md#Configuration)  
  
-   [權限](../../integration-services/service/ssis-catalog.md#Permissions)  
  
-   [資料夾](../../integration-services/service/ssis-catalog.md#Folders)  
  
-   [專案和套件](../../integration-services/service/ssis-catalog.md#ProjectsAndPackages)  
  
-   [參數](../../integration-services/service/ssis-catalog.md#Parameters)  
  
-   [伺服器環境、伺服器變數和伺服器環境參考](../../integration-services/service/ssis-catalog.md#ServerEnvironments)  
  
-   [執行和驗證](../../integration-services/service/ssis-catalog.md#Executions)  
  
-   [AlwaysOn 支援](../../integration-services/service/ssis-catalog.md#AlwaysOn)  
  
-   [相關工作](../../integration-services/service/ssis-catalog.md#RelatedTasks)  
  
-   [相關內容](../../integration-services/service/ssis-catalog.md#RelatedContent)  
  
##  <a name="a-namecatalogobjectidentifiersa-catalog-object-identifiers"></a><a name="CatalogObjectIdentifiers"></a> 目錄物件識別碼  
...
###  <a name="a-namefoldera-folder-project-environment"></a><a name="Folder"></a> 資料夾、專案、環境  
...
-   不允許使用 @ 作為第一個字元，但隨後的字元可以使用 @.。  
  
-   名稱的長度必須大於 0 且小於或等於 128。  
  
###  <a name="a-nameparametera-parameter"></a><a name="Parameter"></a> 參數  
 在命名參數時，請考慮以下規則。  
...
###  <a name="a-nameenvironmentvariablea-environment-variable"></a><a name="EnvironmentVariable"></a> 環境變數  
 在命名環境變數時，請考慮以下規則。  
...
##  <a name="a-nameconfigurationa-catalog-configuration"></a><a name="Configuration"></a> 目錄組態  
...
###  <a name="a-namecleanupa-operations-and-project-version-cleanup"></a><a name="Cleanup"></a> 作業和專案版本清除  
...
###  <a name="a-nameencryptiona-encryption-algorithm"></a><a name="Encryption"></a> 加密演算法  
...
-   DES  
 ...
 資料庫主要金鑰會用於加密。 當您建立目錄時會建立此金鑰。 如需詳細資訊，請參閱 [建立 SSIS 目錄](../../integration-services/service/create-the-ssis-catalog.md)。  
...
##  <a name="a-namepermissionsa-permissions"></a><a name="Permissions"></a> 權限  
...
-   為了讓環境，使用 **中的 權限** 頁面 [NIB︰ 環境屬性 對話方塊](http://msdn.microsoft.com/zh-tw/6a91a8d4-0006-4cfd-9759-3e4295ae452b)。  
...
##  <a name="a-namefoldersa-folders"></a><a name="Folders"></a> 資料夾  
...
##  <a name="a-nameprojectsandpackagesa-projects-and-packages"></a><a name="ProjectsAndPackages"></a> 專案和套件  
 每一個專案都可包含多個封裝。 專案和封裝都可以包含參數及環境的參考。 您可以使用 [Configure Dialog Box](../../integration-services/service/configure-dialog-box.md)來存取參數和環境參考。  
...
##  <a name="a-nameparametersa-parameters"></a><a name="Parameters"></a> 參數  
...
##  <a name="a-nameserverenvironmentsa-server-environments-server-variables-and-server-environment-references"></a><a name="ServerEnvironments"></a> 伺服器環境、伺服器變數和伺服器環境參考  
...
##  <a name="a-nameexecutionsa-executions-and-validations"></a><a name="Executions"></a> 執行和驗證  
...
##  <a name="a-namealwaysona-alwayson-support"></a><a name="AlwaysOn"></a> AlwaysOn 支援  
 AlwaysOn 可用性群組功能是一個高可用性與災害復原解決方案，可以提供資料庫鏡像的企業級替代方案。 可用性群組支援一組可一起容錯移轉之離散化使用者資料庫的容錯移轉環境，也就是所謂的可用性資料庫。 如需詳細資訊，請參閱 [AlwaysOn 可用性群組](https://msdn.microsoft.com/library/hh510230.aspx)。  
  
 在 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]中，SQL Server Integration Services (SSIS) 會引進新功能，讓您輕鬆部署到集中式 SSIS 目錄 (也就是 SSISDB 使用者資料庫)。 為了提供 SSISDB 資料庫及其內容 (專案、封裝、執行記錄等) 的高可用性，您可以將 SSISDB 資料庫 (就像其他任何使用者資料庫) 加入 AlwaysOn 可用性群組。 發生容錯移轉時，其中一個次要節點會自動變成新的主要節點。  
...
##  <a name="a-namerelatedtasksa-related-tasks"></a><a name="RelatedTasks"></a> 相關工作  
  
-   [建立 SSIS 目錄](../../integration-services/service/create-the-ssis-catalog.md)  
  
-   [備份、還原和移動 SSIS 目錄](../../integration-services/service/backup-restore-and-move-the-ssis-catalog.md)  
  
##  <a name="a-namerelatedcontenta-related-content"></a><a name="RelatedContent"></a> 相關內容  
