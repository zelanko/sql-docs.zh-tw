---
title: "針對 SSIS 服務的存取設定 Windows 防火牆 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "安全性 [Integration Services], 防火牆"
  - "Windows 防火牆 [Integration Services]"
  - "未經授權的存取 [Integration Services]"
  - "Integration Services 服務, 防火牆"
  - "防火牆系統 [Integration Services]"
  - "SQL Server Integration Services, 防火牆"
  - "SSIS, 防火牆"
...
>  若要管理儲存在遠端伺服器上的封裝，您不必連接到該遠端伺服器上的 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 服務執行個體， 而是要編輯 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 服務的組態檔，好讓 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 顯示儲存在遠端伺服器上的封裝。 如需詳細資訊，請參閱[設定 Integration Services 服務 &#40;SSIS 服務&#41;](../../integration-services/service/configuring-the-integration-services-service-ssis-service.md)。  
 ...
 如需預設 Windows 防火牆設定的詳細資訊，以及影響 Database Engine、Analysis Services、Reporting Services 和 Integration Services 之 TCP 通訊埠的描述，請參閱[設定 Windows 防火牆以允許 SQL Server 存取](../../sql-server/install/configure-the-windows-firewall-to-allow-sql-server-access.md)。  
  
## 設定 Windows 防火牆  
 您可以使用下列命令來開啟 TCP 通訊埠 135，將 MsDtsSrvr.exe 加入至例外狀況清單，並指定防火牆的不封鎖範圍。  
  
#### 若要使用命令提示字元視窗設定 Windows 防火牆  
