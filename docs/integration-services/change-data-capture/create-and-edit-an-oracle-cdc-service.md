...
-   **SQL Server 驗證**：如果您選取這個選項，您必須針對您要使用的 **登入來輸入** [使用者名稱] **和** [密碼] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 。 Oracle CDC 服務會在連接到目標 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體時使用這些認證。  
 ...
 Oracle CDC 服務所使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 登入只要求它必須是公用固定伺服器角色的成員，不需要其他權限。 一旦加入新的 Oracle CDC 執行個體之後，該登入會取得關聯 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] CDC 資料庫的 **db_owner** 存取權。  
