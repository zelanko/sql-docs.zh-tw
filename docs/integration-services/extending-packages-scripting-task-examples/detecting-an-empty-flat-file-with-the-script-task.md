---
title: "偵測空的一般檔案 with the Script Task |Microsoft 文件"
ms.custom: 
ms.date: 03/14/2017
...
## <a name="description"></a>Description  
 下列範例會使用從方法**System.IO**來測試一般檔案連接管理員來判斷檔案是空的還是它是否僅包含預期的非資料列，例如資料行中指定的一般檔案的命名空間標頭或空白的行。 此指令碼會先檢查檔案的大小。如果大小為零個位元組，表示檔案是空白的。 如果檔案大小大於零，此指令碼就會從檔案中讀取各行，直到沒有其他行為止，或者直到行數超過預期的非資料列數目為止。 如果檔案中的行數小於或等於預期的非資料列數目，此檔案就會被視為空白。 結果會以布林值的形式傳入使用者變數中，而這個變數的值可用於在封裝的控制流程中分支。 **FireInformation**方法也會顯示在結果**輸出**視窗[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] Tools for Applications (VSTA)。  
...
4.  新增`FFNonDataRows`變數加入指令碼工作的**[readonlyvariables]**屬性。  
  
5.  新增`FFIsEmpty`變數加入指令碼工作的**[readwritevariables]**屬性。  
