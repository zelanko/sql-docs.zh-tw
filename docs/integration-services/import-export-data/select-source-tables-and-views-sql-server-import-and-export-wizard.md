...
**Excel 活頁簿中的資料類型**。 Excel 不是一般的資料庫。 其資料行沒有固定的資料類型。 精靈只會辨識有限的 Excel 資料類型集合：數字、貨幣、布林值，日期/時間、字串 (255 個或更少的字元) 以及註解 (255 個以上字元)。 精靈會在現有 Excel 資料來源中抽樣特定數目的資料列 (預設是前八個資料列)，以猜測各資料行的資料類型。
...
-   Unicode Excel 字串資料行與使用特定字碼頁之非 Unicode 字串資料行間的轉換。

### <a name="special-considerations-for-excel-sources"></a>Excel 來源的特殊考量
**匯入的資料中有 Null 或遺漏值**。 Excel 資料行似乎在精靈所取樣的前八個資料列中包含混合的資料類型時 (例如混合使用數值和文字值)，精靈會挑選多數的資料類型作為該資料行的資料類型，而包含其他類型資料的資料格則傳回 Null 值。 沒有任何方法可以變更精靈的此種行為。

**匯入的資料中有截斷的字串**。 當精靈判斷出某個 Excel 資料行包含文字資料時，精靈將會根據其前八個資料列取樣的最長值，來挑選字串或備忘的資料行資料類型。 如果精靈未在其取樣的資料列中發現任何長度超過 255 個字元的值，則會將該資料行視為 255 個字元的字串資料行，而非備忘資料行，並截斷超過 255 個字元的值。 若要從備忘資料行中匯入資料卻不截斷，您必須確保備忘資料行在精靈所取樣的前八個資料列中至少包含一個超過 255 個字元的值。

### <a name="special-considerations-for-excel-destinations"></a>Excel 目的地的特殊考量
**指定現有的範圍**。 當您將現有範圍指定為目的地時，如果範圍的「資料行」數目比來源資料還要少，則會發生錯誤。 不過，如果您所指定範圍的「資料列」數目比來源資料還要少，則精靈會繼續寫入資料列，並延伸範圍定義，以符合新的資料列數目。

**正在儲存備忘 (ntext) 資料**。 在 Excel 資料行中成功儲存長於 255 個字元的字串之前，精靈必須能將目的地資料行的資料類型辨識為 **備忘** ，而不是 **字串**。
-   如果目的地資料表已包含幾列的資料，則精靈所取樣的前八個資料列必須在備忘資料行中至少包含一個值超過 255 個字元的資料列。
-   如果目的地資料表是由精靈建立，則 **CREATE TABLE** 陳述式必須使用 **LONGTEXT** (或其同義字之一) 作為備忘資料行的資料類型。 按一下 [資料行對應] 頁面的 [建立目的地資料表] 選項旁邊的 [編輯 SQL]，檢查並修訂 **CREATE TABLE** 陳述式。
