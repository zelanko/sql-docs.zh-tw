...
-   **如果您有適用於資料來源的 ODBC 驅動程式**，請選取 .NET Framework Data Provider for ODBC。 然後輸入驅動程式的特定資訊。 ODBC 驅動程式並未列在下拉式資料來源清單中。 .Net Framework Data Provider for ODBC 作用為 ODBC 驅動程式的包裝函式。 如需詳細資訊，請參閱[連線至 ODBC 資料來源](../../integration-services/import-export-data/connect-to-an-odbc-data-source-sql-server-import-and-export-wizard.md)。
