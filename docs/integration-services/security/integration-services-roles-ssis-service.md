---
description: Integration Services 角色 (SSIS 服務)
...
[!INCLUDE[sqlserver-ssis](../../includes/applies-to-version/sqlserver-ssis.md)]
  ...
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 提供某些固定資料庫層級角色，以協助保護對儲存在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 中套件的存取。 可用的角色根據您將封裝儲存在 SSIS 目錄資料庫 (SSISDB) 或 msdb 資料庫而有所不同。  
...
## <a name="roles-in-the-msdb-database"></a>msdb 資料庫中的角色  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 包含以下三個固定資料庫層級角色：**db_ssisadmin**、**db_ssisltduser** 和 **db_ssisoperator**，可用於控制針對儲存至 **msdb** 資料庫的套件進行存取。 您可以使用 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]將角色指派給封裝。 角色指派會儲存到 **msdb** 資料庫。  
...
## <a name="assign-a-reader-and-writer-role-to-a-package"></a><a name="assign"></a> 指派讀取器和寫入器角色給封裝
  您可將讀取器和寫入器角色指派給每個封裝。  
...
## <a name="create-a-user-defined-role"></a><a name="create"></a> 建立使用者定義角色
...
## <a name="package-roles-dialog-box-ui-reference"></a><a name="roles_dialog"></a> 套件角色對話方塊 UI 參考
