...
- **Windows 驗證**。 若連接字串使用 Windows 驗證，驗證就會產生警告。 Windows 驗證需要其他設定步驟。 如需詳細資訊，請參閱[使用 Windows 驗證連線到資料和檔案共用](ssis-azure-connect-with-windows-auth.md)。
