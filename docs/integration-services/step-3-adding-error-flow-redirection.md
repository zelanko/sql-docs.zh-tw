...
9. 在 **Name** 屬性中，輸入 **ErrorDescription**，然後將 **DataType** 屬性設為 [Unicode 字串 [DT_WSTR]]。  
  
10. 在 [指令碼] 頁面上，確認 [LocaleID] 屬性是設為 [英文 (美國)]。  
  
11. 按一下 [編輯指令碼] 開啟 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Tools for Applications (VSTA)。 在 **Input0_ProcessInputRow** 方法中，輸入或貼上下列程式碼。  
