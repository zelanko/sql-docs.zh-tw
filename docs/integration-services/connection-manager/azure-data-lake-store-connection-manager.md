...
---
# Azure Data Lake Store 連線管理員
  **Azure Data Lake Store 連線管理員**可讓 SSIS 套件透過兩種驗證類型連接到 Azure Data Lake Store 服務︰Azure AD 使用者識別及 Azure AD 服務識別。  
  
 **Azure Data Lake Store 連線管理員**是 SQL Server 2016 的 SQL Server Integration Services (SSIS) Feature Pack for Azure 的元件。 請在 [這裡](http://go.microsoft.com/fwlink/?LinkID=626967)。  
...
3.  選擇對應的驗證類型，以存取 Azure Data Lake Store 資料。
    ...
    1.  如已選取 [Azure AD 使用者識別] 驗證選項，請執行下列動作︰
        ...
        >   [!NOTE] [Azure AD 使用者識別] 驗證選項「不」支援多重要素驗證和 Microsoft 帳戶。
