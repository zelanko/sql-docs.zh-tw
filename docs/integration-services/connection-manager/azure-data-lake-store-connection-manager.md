...
2.  在 [Azure Data Lake Store Connection Manager Editor] (Azure Data Lake Store 連線管理員編輯器) 對話方塊的 [ADLS Host] (ADLS 主機) 欄位中，輸入 Azure Data Lake Store 的主機 URL。 例如：`https://test.azuredatalakestore.net` 或 `test.azuredatalakestore.net`。
  
3.  在 [驗證] 欄位中，選擇適當的驗證類型來存取 Azure Data Lake Store 中的資料。
