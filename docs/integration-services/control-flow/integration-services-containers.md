...
helpviewer_keywords: 
  - "SSIS 容器"
  - "容器 [Integration Services]"
  - "容器 [Integration Services], 關於容器"
  - "控制流程 [Integration Services], 容器"
  - "SQL Server Integration Services 容器"
...
|**ID**|在建立封裝時所指派的容器 GUID。 此屬性是唯讀的。<br /><br /> 如需詳細資訊，請參閱 <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.ID%2A>。|  
