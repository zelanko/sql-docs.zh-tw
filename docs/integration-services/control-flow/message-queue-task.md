...
helpviewer_keywords: 
  - "訊息佇列工作 [Integration Services]"
  - "接收訊息"
  - "訊息 [Integration Services]"
  - "傳送訊息"
...
-   將檔案散發至整個企業。 例如，封裝可使用「訊息佇列」工作將封裝檔案傳送至另一台電腦。 接著，執行於目的電腦的封裝使用「訊息佇列」工作，以在本機擷取與儲存封裝。  
 ...
 工作使用 MSMQ 連接管理員以連接到訊息佇列。 如需詳細資訊，請參閱 [MSMQ 連線管理員](../../integration-services/connection-manager/msmq-connection-manager.md)。 如需有關 Message Queuing 的詳細資訊，請參閱 [MSDN Library](http://go.microsoft.com/fwlink/?LinkId=7022)。  
...
## 訊息佇列工作上可用的自訂記錄訊息  
 下表列出「訊息佇列」工作的自訂記錄項目。 如需詳細資訊，請參閱 [Integration Services &#40;SSIS&#41; 記錄](../../integration-services/performance/integration-services-ssis-logging.md)和[自訂訊息以進行記錄](../../integration-services/performance/custom-messages-for-logging.md)。  
...
## 訊息佇列工作的組態  
 您可以透過「 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 設計師」或以程式設計方式設定屬性。 如需有關可以在「 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 設計師」中設定之屬性的詳細資訊，請按下列其中一個主題：  
