...
# 將專案部署至 Integration Services 伺服器
 ...
 如需環境的詳細資訊，請參閱[建立和對應伺服器環境](../../integration-services/packages/create-and-map-a-server-environment.md)。  
...
1.  建立 SSISDB 目錄 (若無)。 如需詳細資訊，請參閱[建立 SSIS 目錄](../../integration-services/service/create-the-ssis-catalog.md)。  
...
2.  請執行 [Integration Services 專案轉換精靈] 將專案轉換為專案部署模型。 如需詳細資訊，請參閱底下指示：[將專案轉換為專案部署模型](#convert)  
    ...
    -   若是在舊版 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 中建立專案，在您於 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 中開啟專案檔案後，請將專案轉換為專案部署模型。  
        ...
        > [!NOTE]  
        >  如果專案包含一個或多個資料來源，則在完成專案轉換時，會移除資料來源。 若要建立專案中的封裝可以共用的資料來源連接，請在專案層級加入連接管理員。 如需詳細資訊，請參閱[加入、刪除或共用封裝中的連線管理員](../Topic/Add,%20Delete,%20or%20Share%20a%20Connection%20Manager%20in%20a%20Package.md)。  
...
4.  (選擇性) 建立部署專案的環境。 如需詳細資訊，請參閱[建立和對應伺服器環境](../../integration-services/packages/create-and-map-a-server-environment.md)。  
  
##  <a name="convert"></a> 若要將專案轉換為專案部署模型  
...
2.  完成精靈。 如需相關資訊，請參閱 [Integration Services Project Conversion Wizard](../../integration-services/packages/integration-services-project-conversion-wizard.md)。  
  
##  <a name="deploy"></a> 若要將專案部署至 Integration Services 伺服器  
...
3.  完成精靈。 如需詳細資訊，請參閱 [Integration Services 部署精靈](../../integration-services/packages/integration-services-deployment-wizard.md)。  
