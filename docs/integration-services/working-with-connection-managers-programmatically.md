---
title: "以程式設計方式使用連接管理員 |Microsoft 文件"
ms.custom: 
ms.date: 03/03/2017
...
# <a name="working-with-connection-managers-programmatically"></a>以程式設計方式使用連接管理員
  在[!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)]，AcquireConnection 方法的相關聯的連接管理員類別是您正在使用連接管理員，在 managed 程式碼時最常呼叫的方法。 當您撰寫 managed 程式碼時，您必須在管理員呼叫 AcquireConnection 方法，若要使用之連接的功能。 不論是在指令碼工作、指令碼元件、自訂物件或是自訂應用程式中撰寫 Managed 程式碼，都必須呼叫這個方法。  
  
 若要順利呼叫 AcquireConnection 方法，您必須知道下列問題的答案：  
  
-   **哪一個連接管理員從 AcquireConnection 方法傳回 managed 的物件？**  
  
     許多連接管理員會傳回 unmanaged 的 COM 物件 (System.__ComObject) 以及這些物件無法輕鬆地使用從 managed 程式碼。 這些連接管理員的清單包括最常使用的 OLE DB 連接管理員。  
  
-   **對於傳回 managed 的物件之連接管理員，哪些物件其 AcquireConnection 方法傳回？**  
  
     傳回值轉換成適當的型別，您必須知道何種 AcquireConnection 方法傳回的物件。 比方說，說明 AcquireConnection 方法[!INCLUDE[vstecado](../includes/vstecado-md.md)]當您使用 SqlClient 提供者時，連接管理員會傳回開啟的 SqlConnection 物件。 不過，檔案連接管理員的 AcquireConnection 方法只會傳回字串。  
...
## <a name="connection-managers-that-do-not-return-a-managed-object"></a>不會傳回 Managed 物件的連接管理員  
 下表列出從 AcquireConnection 方法傳回原生 COM 物件 (System.__ComObject) 的連接管理員。 這些 Unmanaged 物件無法輕鬆地從 Managed 程式碼中加以使用。  
...
|OLEDB|OLE DB 連接管理員|  
 ...
 一般而言，您可以使用[!INCLUDE[vstecado](../includes/vstecado-md.md)]從 managed 程式碼連接至 ADO、 Excel、 ODBC 或 OLE DB 資料來源的連接管理員。  
  
## <a name="return-values-from-the-acquireconnection-method"></a>從 AcquireConnection 方法傳回值。  
 下表列出從 AcquireConnection 方法傳回 managed 的物件的連接管理員。 這些 Managed 物件可以輕鬆地從 Managed 程式碼中加以使用。  
...
|SMOServer|SMO 連接管理員|**Microsoft.SqlServer.Management.Smo.Server**||  
