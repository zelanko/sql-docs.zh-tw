---
title: "使用快取連接管理員以完整快取模式實作查閱轉換 | Microsoft Docs"
ms.custom: ""
ms.date: "12/09/2016"
...
### 若要使用快取連接管理員和資料流程中的資料來源在單一封裝中以完整快取模式實作查閱轉換  
...
5.  按一下 [快取連線管理員編輯器] 對話方塊的 [資料行] 索引標籤，然後使用 [索引位置] 選項，指定哪些資料行是索引資料行。  
    ...
    > [!NOTE]  
    >  當查閱轉換是設定為使用快取連接管理員，則只有參考資料集中的索引資料行可以對應到輸入資料行。 而且，所有的索引資料行都必須進行對應。 如需詳細資訊，請參閱[快取連線管理員編輯器](../../../integration-services/data-flow/transformations/cache-connection-manager-editor.md)。  
...
### 使用快取連接管理員和資料流程中的資料來源在兩個封裝中以完整快取模式實作查閱轉換  
...
### 使用快取連接管理員和現有的快取檔案以完整快取模式實作查閱轉換  
