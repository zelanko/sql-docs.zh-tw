...
|RetainNulls|布林值|一個值，指定當「資料轉換管線」引擎處理資料時，是否要將來源檔案的 Null 值保留成 Null 值。 此屬性的預設值為 **False**。|  
...
|Overwrite|布林值|一個值，指定要覆寫或附加至具有相同名稱的現有目的地檔案。 這個屬性的預設值為 **True**。|  
