...
Python 或 R 指令碼可以使用的資源是由 SQL Server 管理。 建議您使用 SQL Server DMV 來監視資源使用狀況，或是查看 Windows 工作物件相關的效能計數器，並隨之調整伺服器記憶體的使用。 如果您使用 SQL Server Enterprise Edition，則可以設定[外部資源集區](how-to-create-a-resource-pool.md)，以配置用於執行外部指令碼的資源。
