...
3. 在 [特徵選取]  頁面上，選取下列選項：
    ...
    -   **Python**勾選此選項可取得之 Python 3.5 可執行檔，然後選取從 Anaconda 發佈的文件庫。 安裝每個執行個體只能有一個語言。
        ...
        > [!NOTE]
        > 未選取的選項**共用功能**如**Microsoft R Server （獨立）**。 如果您需要新增機器學習使用 R 開發的不同電腦的元件，請在個別的安裝中使用此選項。 比方說，這可能是適用於資料科學家的膝上型電腦。
        
        ![安裝程式選項的 Python](media/ml-svcs-features-python-highlight.png "設定 Python 的選項")
...
5. 在**安裝準備就緒**頁面上，確認這些選取項目會包含在內，然後選取**安裝**。
    ...
    這些選取項目代表使用 Python 與 SQL Server 所需的最小設定。
...
3.  若要啟用外部指令碼的功能支援 Python，執行下列陳述式：
    ...
    ```
    這是完全相同的程序，因為基礎的擴充功能支援這兩種語言用來啟用 R。
...
2. 開啟**服務**面板或 SQL Server 組態管理員，並確認您的執行個體的啟動控制板服務正在執行。 如果未執行 [啟動列]，重新啟動服務。
    ...
    不過，如果您安裝 R，並將 Python 的單一執行個體時，只能有一個 [啟動列] 將會安裝。 獨立的語言特有啟動器 DLL 會加入每一種語言。 如需詳細資訊，請參閱[元件來支援 Python 整合](new-components-in-sql-server-to-support-python-integration.md)。 
...
> [!IMPORTANT]
> 背景工作群組名為 SQLRUserGroup，不論您執行的指令碼類型為何。 沒有單一的群組，每個執行個體。

如果您需要執行 R 指令碼從遠端資料科學用戶端，而且您使用 Windows 驗證，還有其他考量。 這些工作者帳戶必須獲得權限才能登入[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]代替您執行個體。
...
6. 根據預設，在登入時會指派給**公用**角色，而且具有連接到 database engine 的權限。
7. 選取 [確定]。

> [!NOTE]
> 如果您使用的 SQL 登入 SQL Server 計算內容中執行指令碼，就不需要這個額外步驟。
...
不過，大多數使用者而言不需要這類較高的權限。 例如，使用者在組織中的使用 SQL 登入來存取資料庫通常沒有提高權限。 因此，每個使用者正在使用 Python，您必須授與使用者的機器學習服務正 Python 每個資料庫中執行外部指令碼的權限。 方法：
...
當使用者正在執行指令碼時，可能需要的使用者帳戶或 SQL 登入從其他資料庫讀取資料。 使用者帳戶或 SQL 登入可能也需要建立新的資料表來儲存結果，並將資料寫入資料表。

每個使用者帳戶或 SQL 登入執行 R 或 Python 指令碼，確認它是否`db_datareader`， `db_datawriter`，或`db_ddladmin`特定資料庫的權限。

例如，下列[!INCLUDE[tsql](../../includes/tsql-md.md)]陳述式可讓 SQL 登入*MySQLLogin*執行 T-SQL 查詢的權限*ML_Samples*資料庫。 若要執行此陳述式，SQL 登入必須存在於伺服器的安全性內容。
...
您可以建立的機器學習資料科學用戶端電腦上的方案。 如果您需要使用 SQL Server 電腦當成計算內容中執行程式碼，您會有兩個選項。 您可以使用 SQL 登入或整合式的 Windows 驗證。

+ SQL 登入： 請確認登入具有適當的權限的資料庫讀取資料。 您可以藉由新增*連接到*和*選取*權限，或加入登入`db_datareader`角色。 建立物件的登入需要`DDL_admin`權限。 必須將資料儲存至資料表的登入應該加入至`db_datawriter`角色。
...
如果您預期許多使用者會同時執行的指令碼時，您可以增加背景工作帳戶指派給 Launchpad 服務數目。 如需詳細資訊，請參閱[修改使用者帳戶集區的 SQL Server R Services](../r/modify-the-user-account-pool-for-sql-server-r-services.md)。
