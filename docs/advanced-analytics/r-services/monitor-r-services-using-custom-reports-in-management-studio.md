...
---
# 使用 Management Studio 中的自訂報表監視 R Services
...
1. 開啟包含所有 SQL Server 產品範例 (包括範例資料庫) 的 GitHub 存放庫。 
...
> [!IMPORTANT] 某些電腦無法使用這些報表，例如顯示裝置解析度高於 1080p 或為高 DPI 的電腦，或處於某些遠端桌面工作階段中的電腦。 SSMS 的報表檢視器控制項中有一個 Bug，會損毀報表。  
...
+ **R Services - 組態**
  ...
  > [!NOTE] 如需有關這些欄位的詳細資訊，請參閱 Hadley Wickam 執筆的 [Package metadata](http://r-pkgs.had.co.nz/description.html) (套件中繼資料)。 例如，推出 R 執行階段的 *Nickname* 欄位，以協助區分版本。 
