...
description: 使用 sp_rxPredict 產生預測，並根據 SQL Server 上以 R 撰寫的預先定型模型來評分資料輸入。
ms.prod: sql
...
即時評分會使用 [sp_rxPredict](https://docs.microsoft.com//sql/relational-databases/system-stored-procedures/sp-rxpredict-transact-sql) 系統預存程序和 SQL Server 中的 CLR 擴充功能，在預測工作負載上獲得高效能的預測或分數。 即時評分沒有語言限制，即使沒有相依性，也可在 R 或 Python 執行階段上執行。 假設模型是使用 Microsoft 函式來建立並定型，然後在 SQL Server 中序列化為二進位格式，您可以在未安裝 R 或 Python 附加元件的 SQL Server 執行個體上，使用即時評分來針對新的資料輸入產生預測輸出。
...
即時評分不會使用解譯器；因此，在評分步驟執行期間，任何可能需要解譯器的功能皆不支援。  這些情況可能包括：
  ...
  + 即時評分中不支援模型使用轉換函式或包含轉換的公式 (例如 <code>A ~ log(B)</code>)。 若要使用此類型的模型，我們建議您先對輸入資料執行轉換，然後再將資料傳遞至即時評分。


## <a name="example-sp_rxpredict"></a>範例：sp_rxpredict

本節會說明設定**即時**預測所需的步驟，並提供如何從 T-SQL 呼叫函式的 R 範例。
...
> [!NOTE]
> 
> SQL Server 2017 中有額外的安全性措施，可避免 CLR 整合發生問題。 這些量值也會對此預存程序的用法施加額外限制。 

### <a name="step-2-prepare-and-save-the-model"></a>步驟 2： 準備及儲存模型
...
### <a name="step-3-call-sp_rxpredict"></a>步驟 3： 呼叫 sp_rxPredict

您可以像呼叫任何其他預存程序一樣地呼叫 sp\_rxPredict。 在目前的版本中，預存程序只會採用兩個參數： _\@model_ 代表二進位格式的模型，而 _\@inputData_ 代表用於評分的資料，並定義為有效的 SQL 查詢。
...
> [!NOTE]
> 
> 如果用於評分的輸入資料不包含符合模型需求的資料行，則呼叫 sp\_rxPredict 的作業就會失敗。 目前僅支援下列 .NET 資料類型：double、float、short、ushort、long、ulong 和 string。
> 
...
如需 SQL Server 中有關評分的詳細背景資料，請參閱[如何在 SQL Server 機器學習中產生預測](r/how-to-do-realtime-scoring.md)。
