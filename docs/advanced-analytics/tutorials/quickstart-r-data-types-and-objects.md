---
title: 有關 R 與 SQL 資料類型和物件-SQL Server Machine Learning 的快速入門
...
當您的指令碼將結果從 R 傳回到 SQL Server 時，它必須傳回資料做為 **data.frame**。 任何其他類型的清單、 因數、 向量或二進位資料-不論您的指令碼-產生的物件必須轉換成資料框架，如果您想要輸出做為預存程序結果的一部分。 幸好有多個 R 函數可用來支援將其他物件變更為資料框架。 您甚至可將二進位模型序列化，並在資料框架中傳回它，您將在本教學課程稍後執行此動作。

首先，讓我們使用一些 R 的基本 R 物件-向量、 矩陣和清單-實驗，並查看轉換成資料框架如何變更傳遞至 SQL Server 的輸出。
...
如您所見，稍微變化一下 R 語法，就會對結果的結構描述產生極大的影響。 我們不會詳述原因，但 R 資料類型的差異會說明詳細*資料結構*一節[Hadley Wickham 所著的 《 進階 R"](http://adv-r.had.co.nz)。
...
> [快速入門：使用 R 函式與 SQL Server 資料](quickstart-r-functions.md)
