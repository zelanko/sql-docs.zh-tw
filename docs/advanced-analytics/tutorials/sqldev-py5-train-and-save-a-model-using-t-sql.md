...
您可以載入模組，並呼叫所需的函式，以使用 SQL Server 預存程式來建立和定型模型。 此模型需要您在先前課程中設計的資料功能。 最後，您會將定型的模型儲存至 @no__t 0 資料表。
...
為了讓您更輕鬆地針對新資料重新定型模型，您可以將 sp_execute_external_script 的呼叫包裝在另一個預存程式中，並以參數的形式傳入新的定型資料。 本節將逐步引導您完成該程式。
...
2. 執行下列 SQL 語句，將定型的模型插入資料表 nyc @ no__t-0taxi_models 中。
    ...
    處理資料並調整模型，可能需要幾分鐘的時間。 將輸送到 Python **stdout**資料流程的訊息會顯示在 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 的 [**訊息**] 視窗中。 例如:

    *來自外部腳本的 STDOUT 訊息：* 
  *C:\Program Files\Microsoft SQL Server\MSSQL14。MSSQLSERVER\PYTHON_SERVICES\lib\site-packages\revoscalepy*

3. 開啟資料表*nyc @ no__t-1taxi_models*。 您可以看到當中已新增一個新的資料列，其 _model_資料行中包含序列化的模型。
...
1. 在 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 中，開啟新的**查詢**視窗，並執行下列語句來建立預存程式_TrainTipPredictionModelRxPy_。  因為預存程式已經包含輸入資料的定義，所以您不需要提供輸入查詢。
    ...
    - SELECT 查詢會套用自訂純量函數_fnCalculateDistance_ ，以計算挑選和捨棄位置之間的直線距離。 查詢的結果會儲存在預設的 Python 輸入變數中，`InputDataset`。
...
2. 執行預存程式，如下所示，將定型的**revoscalepy**模型插入資料表*nyc_taxi_models*中。
    ...
    處理資料並對模型進行調整可能需要一段時間。 將輸送到 Python **stdout**資料流程的訊息會顯示在 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 的 [**訊息**] 視窗中。 例如:
