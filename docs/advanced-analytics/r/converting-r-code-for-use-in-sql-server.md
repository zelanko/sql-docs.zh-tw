...
+ 您想要在[sp_execute_external_script](../../relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql.md)的 *\@script*參數中執行程式碼，並參數化預存程式。
...
+ 建立可能資料類型問題的檢查清單。
    ...
    所有 R 資料類型都受到 SQL Server 機器學習服務的支援。 不過，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 支援比 R 更多的資料類型。因此，將 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料傳送至 R 時，會執行某些隱含的資料類型轉換，反之亦然。 您可能需要明確地轉換或轉換某些資料。 

    支援 NULL 值。 不過，R 會使用 `na` 資料結構來代表遺漏值，這類似于 null。
...
+ 在預存程式中執行 R 時，您可以**傳遞多個**純量輸入。 針對您想要在輸出中使用的任何參數，加入**output**關鍵字。 

    例如，下列純量輸入 `@model_name` 包含模型名稱，這也會在結果中自己的資料行中輸出：
