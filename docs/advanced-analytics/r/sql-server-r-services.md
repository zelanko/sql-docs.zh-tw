...
ms.locfileid: "43240094"
---
# <a name="r-services-in-sql-server-2016"></a>SQL Server 2016 中的 R Services
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md-winonly](../../includes/appliesto-ss-xxxx-xxxx-xxx-md-winonly.md)]

SQL Server 2016 R Services 是 database engine 執行個體，用於 SQL Server 上執行 R 程式碼和函式的附加元件。 與核心引擎處理序隔離，但預存程序、 T-SQL 指令碼包含 R 陳述式，或包含 T-SQL 的 R 程式碼的關聯式資料完全可供使用的擴充性架構中，執行程式碼。 
...
在資料庫內分析的最常見方法是使用[sp_execute_external_script](../../relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql.md)，傳遞 Rscript 做為輸入參數。
