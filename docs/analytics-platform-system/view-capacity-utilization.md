...
## <a name="to-view-capacity-utilization-by-using-admin-console"></a>若要使用管理主控台中檢視容量使用率  
若要檢視使用的空間，開啟 系統管理 主控台，然後按一下**儲存體** 索引標籤。沒有**儲存體**PDW 區域的索引標籤。  
...
若要了解是否節點空間不足，SQL Server PDW 的健全狀況監視系統已監視每個節點內的所有磁碟區的可用空間。  
  
如果是磁碟區中的可用空間低於 30%時，就會產生 SQL Server PDW**警告**中的警示[sys.dm_pdw_component_health_active_alerts](../relational-databases/system-dynamic-management-views/sys-dm-pdw-component-health-active-alerts-transact-sql.md)。  警示保留，直到可用空間為止。  
  
如果是磁碟區中的可用空間降至 10%，就會產生 SQL Server PDW**重要**警示。 這會視為重大，因為查詢可能會失敗，如果它們會導致要展開的資料庫。  
...
若要擷取的資料庫所使用的應用裝置節點之間的空間，請參閱下列的範例。  
