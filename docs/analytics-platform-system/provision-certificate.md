---
title: 憑證布建
description: Analytics Platform System 中的憑證布建。
author: mzaman1
ms.prod: sql
...
---
# <a name="certificate-provisioning-in-analytics-platform-system"></a>Analytics Platform System 中的憑證布建
Analytics Platform System 的 [ **PDW 憑證** 布建] 頁面**Configuration Manager** 匯入或移除 PDW 所使用的憑證。 

使用加密連線的憑證，可以透過 SQL Server 用戶端、使用 SQL Server PDW 驅動程式的工具、 [管理主控台](monitor-the-appliance-by-using-the-admin-console.md)和 Integration Services 載入來協助保護與控制節點的通訊。 
...
如需詳細資訊，請參閱 [啟動 Configuration Manager &#40;Analytics Platform System&#41;](launch-the-configuration-manager.md)。  
...
![DWConfig 應用裝置 PDW 憑證](media/dwconfig-appl-pdw-cert.png "Dwconfig 應用設備 PDW 憑證")  
