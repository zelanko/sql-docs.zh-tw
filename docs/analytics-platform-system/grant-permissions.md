---
title: 授與 T-sql 許可權
description: 針對平行資料倉儲中的資料庫作業，授與 T-sql 許可權。
author: mzaman1
ms.prod: sql
...
ms.locfileid: "92257471"
---
# <a name="grant-t-sql-permissions-for-parallel-data-warehouse"></a>授與平行處理資料倉儲的 T-sql 許可權
針對平行資料倉儲中的資料庫作業，授與 T-sql 許可權。

## <a name="grant-permissions-to-submit-database-queries"></a>授與許可權以提交資料庫查詢
本節說明如何將許可權授與資料庫角色和使用者，以查詢 SQL Server PDW 設備上的資料。  
  
用來授與許可權給查詢資料的語句取決於所需的存取範圍。 下列 SQL 語句會建立名為 KimAbercrombie 的登入，該登入可以存取設備、在 **AdventureWorksPDW2012** 資料庫中建立名為 KimAbercrombie 的資料庫使用者、建立名為 PDWQueryData 的資料庫角色、將 use KimAbercrombie 新增至 PDWQueryData 角色，然後根據存取是否授與資料庫層級，顯示授權查詢存取的選項。  
...
## <a name="grant-permissions-to-use-the-admin-console"></a>授與許可權以使用管理主控台
本節說明如何將許可權授與登入，以使用管理主控台。  
  
**使用管理主控台**  
  
若要使用管理主控台，登入需要伺服器層級 **VIEW SERVER STATE** 許可權。 下列 SQL 語句會將 **VIEW SERVER STATE** 許可權授與登入， `KimAbercrombie` 讓 Kim 可以使用管理主控台來監視 SQL Server PDW 設備。  
...
若要將終止會話的許可權授與登入，請授與 **ALTER ANY CONNECTION** 許可權，如下所示：  
...
本節說明如何授與資料庫角色和資料庫使用者的許可權，以將資料載入 SQL Server PDWappliance 上。  
...
## <a name="grant-permissions-to-copy-data-off-the-appliance"></a>授與許可權以從設備資料複製
本節說明如何將許可權授與使用者或資料庫角色，以將資料複製到 SQL Server PDW 設備之外。  
  
若要將資料移到另一個位置，需要包含要移動之資料之資料表的 **SELECT** 許可權。  
...
在某些情況下，公司會為資料庫指派管理員。 管理員會控制其他登入對資料庫的存取權，以及資料庫中的資料和物件。 若要管理資料庫中的所有物件、角色和使用者，請授與使用者資料庫的 **CONTROL** 許可權。 下列語句會將**AdventureWorksPDW2012**資料庫的**CONTROL**許可權授與使用者 `KimAbercrombie` 。  
...
若要授與使用者控制設備上所有資料庫的許可權，請在 master 資料庫中授與 **ALTER ANY database** 許可權。  
...
下列 SQL 語句會建立名為 KimAbercrombie 的登入，此登入可以使用 [CREATE login](../t-sql/statements/create-login-transact-sql.md) 語句來建立新的登入，並使用 [alter login](../t-sql/statements/alter-login-transact-sql.md) 語句來改變現有的登入。  
  
**ALTER ANY LOGIN**許可權可授與建立新登入的能力，並捨棄現有的登入。 一旦登入存在，登入就可以使用 **ALTER ANY login** 許可權或該登入的 **alter** 許可權來管理登入。 登入可以變更自己登入的密碼和預設資料庫。  
...
建立和卸載資料庫使用者需要 **ALTER ANY USER** 許可權。 管理現有的使用者需要 **ALTER ANY user** 許可權或該使用者的 **alter** 許可權。 下列範例會使用稍 `KimAbercrombie` 早建立的登入。  
...
建立和卸載使用者定義資料庫角色需要 **ALTER ANY ROLE** 許可權。 下列範例會使用 `KimAbercrombie` 先前建立的登入和使用。  
...
下列圖表可能會令人困惑，但會顯示更多的杠杆許可權 (例如控制項) 包含可個別授與的更細微許可權 (例如 ALTER) 。 最佳做法是一律授與最少量的許可權，讓某人完成所需的工作。 若要這樣做，請授與更特定的許可權，而不是最上層許可權。  
...
您可以使用管理主控台或 SQL Server PDW 系統檢視來監視 SQL Server PDW 設備。 登入需要伺服器層級 **VIEW SERVER STATE** 許可權才能監視設備。 登入需要 **ALTER ANY CONNECTION** 許可權，才能使用管理主控台或 **KILL** 命令來終止連接。 如需使用管理主控台所需許可權的相關資訊，請參閱 [授與使用管理主控台 &#40;SQL Server PDW&#41;的許可權 ](#grant-permissions-to-use-the-admin-console)。  
  
### <a name="grant-permission-to-monitor-the-appliance-by-using-system-views"></a><a name="PermsAdminConsole"></a>使用系統檢視來授與監視設備的許可權  
下列 SQL 語句會建立名為的登入 `monitor_login` ，並將 **VIEW SERVER STATE** 許可權授與 `monitor_login` 登入。  
...
### <a name="grant-permission-to-monitor-the-appliance-by-using-system-views-and-to-terminate-connections"></a>授與許可權，以使用系統檢視和終止連線來監視設備  
下列 SQL 語句會建立名為的登入 `monitor_and_terminate_login` ，並將 **VIEW SERVER STATE** 和 **ALTER ANY CONNECTION** 許可權授與 `monitor_and_terminate_login` 登入。  
