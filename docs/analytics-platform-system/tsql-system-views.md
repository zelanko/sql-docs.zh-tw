---
title: 系統檢視表
description: 分析平臺系統 (AP) SQL Server 平行資料倉儲 (PDW) 的系統檢視。
author: mzaman1
ms.prod: sql
...
## <a name="parallel-data-warehouse-dynamic-management-views-dmvs"></a> (Dmv 的平行處理資料倉儲動態管理檢視) 
...
## <a name="sql-server-dmvs-available-in-parallel-data-warehouse"></a>平行處理資料倉儲中可用的 SQL Server Dmv
平行處理資料倉儲會 (Dmv) 公開許多 SQL Server 動態管理檢視。 在平行處理資料倉儲中查詢這些視圖時，會報告散發上執行的 SQL Server 資料庫的狀態。

每個 DMV 都有一個稱為 pdw_node_id 的特定資料行。 這是計算節點的識別碼。 

> [!NOTE]
> 若要使用這些視圖，請在名稱中插入 ' pdw_nodes_ '，如下表所示。
> 
> 
...
## <a name="sql-server-2016-polybase-dmvs-available-in-parallel-data-warehouse"></a>在平行處理資料倉儲中可用的 SQL Server 2016 PolyBase Dmv
