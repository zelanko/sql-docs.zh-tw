...
升級至 CU 7.6 之後，客戶將能夠重新命名使用者建立之資料表的資料行。 如需語法、範例、限制和詳細資訊，請參閱[RENAME （transact-sql）](https://docs.microsoft.com/sql/t-sql/statements/rename-transact-sql) 。

### <a name="alter-view"></a>改變視圖
客戶現在將能夠改變 views。 如需詳細資訊，請參閱[ALTER VIEW （transact-sql）](https://docs.microsoft.com/sql/t-sql/statements/alter-view-transact-sql) 。
