---
title: "使用格式檔案略過資料欄位 (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "09/19/2016"
...
如需詳細資訊，請參閱[非 XML 格式檔案 (SQL Server)](../../relational-databases/import-export/non-xml-format-files-sql-server.md)。 下列命令將使用 [bcp 公用程式](../../tools/bcp-utility.md)，根據 `myTestSkipField` 的結構描述產生非 XML 格式檔案 `myTestSkipField.fmt`。  另外還會使用限定詞 `c` 來指定字元資料、使用 `t,` 來指定逗號作為欄位結束字元，並使用 `T` 來指定使用整合式安全性的信任連接。  請在命令提示字元之下，輸入下列命令：
...
如需詳細資訊，請參閱 [XML 格式檔案 (SQL Server)](../../relational-databases/import-export/xml-format-files-sql-server.md)。  下列命令將使用 [bcp 公用程式](../../tools/bcp-utility.md)，根據 `myTestSkipField` 的結構描述建立 XML 格式檔案 `myTestSkipField.xml`。  另外還會使用限定詞 `c` 來指定字元資料、使用 `t,` 來指定逗號作為欄位結束字元，並使用 `T` 來指定使用整合式安全性的信任連接。  必須使用 `x` 限定詞來產生 XML 格式檔案。  請在命令提示字元之下，輸入下列命令：
