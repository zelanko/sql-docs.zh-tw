...
ms.topic: "article"
helpviewer_keywords: 
  - "bcp 公用程式 [SQL Server], 前置長度"
  - "前置長度 [SQL Server]"
  - "長度 [SQL Server], 前置詞字元"
  - "資料格式 [SQL Server], 前置長度"
...
|**sql_variant**|8|8|8|8|  
 ...
 \*[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的未來版本將會移除 **ntext**、**text** 及 **image** 資料類型。 請避免在新的開發工作中使用這些資料類型，並規劃修改目前在使用這些資料類型的應用程式。 請改用 **nvarchar(max)**、**varchar(max)** 和 **varbinary(max)**。  
