---
title: "使用格式檔案將資料表資料行對應至資料檔欄位 (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "09/19/2016"
...
ms.topic: "article"
helpviewer_keywords: 
  - "在匯入期間將資料行對應到欄位 [SQL Server]"
  - "格式檔案 [SQL Server], 將資料行對應到欄位"
...
下面的資料會依照 `myRemap` 資料表所示，以相反順序顯示 `FirstName` 和 `LastName`。  使用 [記事本] 建立空白檔案 `D:\BCP\myRemap.bcp`，並插入下列資料：
```
1,Grosse,Anthony,M
2,Fatnowna,Alica,F
...
如需詳細資訊，請參閱[非 XML 格式檔案 (SQL Server)](../../relational-databases/import-export/non-xml-format-files-sql-server.md)。 下列命令將使用 [bcp 公用程式](../../tools/bcp-utility.md)，根據 `myRemap` 的結構描述產生非 XML 格式檔案 `myRemap.fmt`。  另外還會使用限定詞 `c` 來指定字元資料，使用 `t,` 來指定逗號作為欄位結束字元，並使用 `T` 來指定使用整合式安全性的信任連接。  請在命令提示字元之下，輸入下列命令：
```
...
如需詳細資訊，請參閱 [XML 格式檔案 (SQL Server)](../../relational-databases/import-export/xml-format-files-sql-server.md)。  下列命令將使用 [bcp 公用程式](../../tools/bcp-utility.md)，根據 `myRemap` 的結構描述建立 XML 格式檔案 `myRemap.xml`。  另外還會使用限定詞 `c` 來指定字元資料，使用 `t,` 來指定逗號作為欄位結束字元，並使用 `T` 來指定使用整合式安全性的信任連接。  必須使用 `x` 限定詞來產生 XML 格式檔案。  請在命令提示字元之下，輸入下列命令：
```
