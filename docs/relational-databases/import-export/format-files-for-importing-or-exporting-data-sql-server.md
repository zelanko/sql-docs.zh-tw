...
-   [格式檔案的範例](#ExamplesOfFFs)  
  
-   [何時需要格式檔案？](#WhenFFrequired)  
...
##  <a name="ExamplesOfFFs"></a> 格式檔案的範例  
 下列範例顯示非 XML 格式檔案及 XML 格式檔案的配置。 這些格式檔案會對應到 [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] 範例資料庫中的 `HumanResources.myTeam` 資料表。 這個資料表包含四個資料行：`EmployeeID`、`Name`、`Title` 和 `ModifiedDate`。  
...
### A. 使用非 XML 格式檔案  
 下列非 XML 格式檔案使用 `HumanResources.myTeam` 資料表的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 原生資料格式。 這個格式檔案是使用下列 `bcp` 命令所建立。  
...
### B. 使用 XML 格式檔案  
 下列 XML 格式檔案使用 `HumanResources.myTeam` 資料表的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 原生資料格式。 這個格式檔案是使用下列 `bcp` 命令所建立。  
