...
<a name="when-you-install-the-includessdenoversionincludesssdenoversion-mdmd-the-tools-that-are-installed-depend-upon-the-edition-and-your-setup-choices-this-lesson-reviews-the-principal-tools-and-shows-you-how-to-connect-and-perform-a-basic-function-authorizing-more-users"></a>當您安裝 [!INCLUDE[ssDEnoversion](../includes/ssdenoversion-md.md)]時，所安裝的工具視版本和安裝選項而定。 這一課檢閱主要工具，顯示您如何連接及執行基本功能 (授權更多使用者)。  
 -  
 <a name="-this-lesson-contains-the-following-tasks"></a>-本課程包含下列工作：  
 -  
 <a name="-----tools-for-getting-startedtools"></a>--   [使用者入門的工具](#tools)  
 -  
 <a name="-----connecting-with-management-studioconnect"></a>--   [與 Management Studio 連線](#connect)  
 -  
 <a name="-----authorizing-additional-connectionsadditional"></a>--   [授權其他連線](#additional)  
 -  
 ...
 -  
 --   [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 組態管理員會隨著 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 及用戶端工具一起安裝。 它可讓您啟用伺服器通訊協定、設定通訊協定選項 (例如 TCP 通訊埠)、設定伺服器服務自動啟動，以及設定用戶端電腦以您偏好的方式連接。 這個工具會設定更進階的連接元素，但是不會啟用功能。  
 -  
 -### 範例資料庫  
 -[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 未隨附範例資料庫及範例。 《 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 線上叢書》中所描述的大多數範例都是使用 [!INCLUDE[ssSampleDBobject](../includes/sssampledbobject-md.md)] 範例資料庫。  
 -  
 ...
 -  
 -> [!NOTE]  
 -> 本主題討論如何連線至內部部署 SQL Server。 若要連接到 Azure SQL Database，請參閱 [使用 SQL Server Management Studio 連接到 SQL Database 並執行範例 T-SQL 查詢](https://azure.microsoft.com/documentation/articles/sql-database-connect-query-ssms/)。  
 -  
 -##### 判斷資料庫引擎執行個體的名稱  
 -  
 ...
 -##### 連線至資料庫引擎  
 -
 -安裝 [!INCLUDE[ssNoVersion_md](../includes/ssnoversion-md.md)] 時，選取至少一個 Administrator 帳戶。 以系統管理員身分登入 Windows 時，請執行下列步驟。
 -  
 ...
 -
 -> [!NOTE]
 -> 本教學課程會假設您不熟悉 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 而且沒有特殊連線問題。 這應該適用於大部分的人，並且保持本教學課程的簡單性。 如需詳細疑難排解步驟，請參閱 [針對 SQL Server Database Engine 的連接進行疑難排解](../database-engine/configure-windows/troubleshoot-connecting-to-the-sql-server-database-engine.md)。 
 -  
 -## <a name="additional"></a>授權其他連線  
 -以系統管理員的身分連線至 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 後，您的首要工作之一就是授權其他使用者連線。 您可以建立登入，並授權該登入以使用者身分存取資料庫，來達成此目的。 登入可以是使用 Windows 認證的 Windows 驗證登入，或是 SQL Server 驗證登入，這種登入會將驗證資訊儲存在 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 中，而且與 Windows 認證無關。 可能的話，請盡量使用 Windows 驗證。
 -
 -> [!TIP]
