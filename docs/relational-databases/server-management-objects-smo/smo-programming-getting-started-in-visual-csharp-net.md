---
title: 'Visual c # .NET 中的消費者入門 |Microsoft Docs'
ms.custom: ''
ms.date: 08/02/2016
ms.prod: sql
ms.technology: ''
ms.topic: quickstart
helpviewer_keywords:
- Visual C# [SMO]
ms.assetid: 37a22721-546c-453d-bc6f-238c13d7505d
author: markingmyname
ms.author: maghan
monikerRange: =azuresqldb-current||=azure-sqldw-latest||>=sql-server-2016||=sqlallproducts-allversions||>=sql-server-linux-2017||=azuresqldb-mi-current
ms.openlocfilehash: 088b0991d3549cfa7d12a0d0365d40d3ededbfba
ms.sourcegitcommit: b87d36c46b39af8b929ad94ec707dee8800950f5
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 02/08/2020
ms.locfileid: "70911169"
---
# <a name="smo-programming---getting-started-in-visual-c-net"></a>SMO 程式設計 - Visual C# .NET 使用者入門
[!INCLUDE[appliesto-ss-asdb-asdw-xxx-md](../../includes/appliesto-ss-asdb-asdw-xxx-md.md)]

  [!INCLUDE[csprcs](../../includes/csprcs-md.md)].NET 是 .Net 中[!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] common language RUNTIME （CLR）所支援的其中一種 managed 程式設計語言。 許多如需有關 c # 語言的詳細資訊， [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)]請參閱 .net 檔。 您可以使用 [!INCLUDE[vsprvslong](../../includes/vsprvslong-md.md)] 和 [!INCLUDE[vsOrcas](../../includes/vsorcas-md.md)] 來開發 SMO 應用程式。  
  
## <a name="references-and-imports"></a>參考和匯入  
 您在啟動 [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] 專案之後，必須加入 SMO 組件的參考。 然後再匯入必要的命名空間，使您的程式可以辨識 SMO 類型。  
  
 在[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]中，SMO 元件位於 C:\PROGRAM Files\Microsoft SQL Server\130\SDK\Assemblies\ 資料夾中。  
  
## <a name="see-also"></a>另請參閱  
 [在 Visual Studio .NET 中建立 Visual C&#35; SMO 專案](../../relational-databases/server-management-objects-smo/how-to-create-a-visual-csharp-smo-project-in-visual-studio-net.md)  
  
  
