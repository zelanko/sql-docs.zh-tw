...
[!INCLUDE[appliesto-ss-asdb-asdw-xxx-md](../../../includes/appliesto-ss-asdb-asdw-xxx-md.md)]
 ...
 若要在 SMO 中建立外部索引鍵，您必須在 <xref:Microsoft.SqlServer.Management.Smo.ForeignKey> 物件的建構函式中，指定會在其上定義外部索引鍵的資料表。 您必須從該資料表至少選取一個資料行做為外部索引鍵。 若要進行這項作業，請建立 <xref:Microsoft.SqlServer.Management.Smo.ForeignKeyColumn> 物件變數，並指定做為外部索引鍵之資料行的名稱。 然後，指定參考的資料表和參考的資料行。 使用方法, 即可將資料行加入至 Columns 物件屬性。 <xref:Microsoft.SqlServer.Management.Smo.ForeignKeyColumnCollection.Add%2A>  
...
## <a name="sample-foreign-keys-primary-keys-and-unique-constraint-columns"></a>範例：外鍵、主鍵和唯一條件約束資料行  
 此範例示範：  
