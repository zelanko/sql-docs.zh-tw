...
|方法|語法|描述|  
|------------|------------|-----------------|  
|**初始**|`public void Init();`|查詢處理器使用這個方法將彙總計算初始化。 這個方法針對查詢處理器正在彙總的每個群組叫用一次。 查詢處理器為了計算多個群組彙總，可能選擇重複使用彙總類別之相同執行個體。 **Init**方法應視需要在此實例的先前使用中執行任何清除，並讓它重新開機新的匯總計算。|  
