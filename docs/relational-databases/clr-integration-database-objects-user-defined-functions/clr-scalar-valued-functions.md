...
## <a name="requirements-for-clr-scalar-valued-functions"></a>CLR 純量值函式的需求  
 ...
 在.NET Framework 語言實作.NET Framework SVF 時**SqlFunction**可以指定自訂屬性，以包括其他函數的相關資訊。 **SqlFunction**屬性會指出是否可以存取或修改資料，如果是具決定性，而且函式如果涉及浮點運算。  
...
```  
 ...
 程式碼參考的第一行**Microsoft.SqlServer.Server**來存取屬性和**System.Data.SqlClient**存取 ADO.NET 命名空間。 (這個命名空間包含**SqlClient**，.NET Framework Data Provider for [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。)  
  
 接下來，在函數收到**SqlFunction**中找到的自訂屬性**Microsoft.SqlServer.Server**命名空間。 此自訂屬性會指出使用者定義函數 (UDF) 是否會使用同處理序提供者來讀取伺服器上的資料。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 不允許 UDF 更新、插入或刪除資料。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 可以讓不使用同處理序提供者之 UDF 的執行最佳化。 這藉由設定表示**DataAccessKind**至**DataAccessKind.None**。 在下一行中，目標方法為 public static (Visual Basic .NET 中則為 shared)。  
  
 **SqlContext**類別，位於**Microsoft.SqlServer.Server**命名空間，然後可以存取**SqlCommand**具有連接物件[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]已設定的執行個體。 雖然未使用此處，但目前的交易內容也會提供透過**System.Transactions**應用程式開發介面 (API)。  
  
 大部分的函式主體中的程式碼行看起來應該熟悉的開發人員撰寫用戶端應用程式中找到的類型使用**System.Data.SqlClient**命名空間。  
...
```  
 ...
 指定適當的命令文字初始化**SqlCommand**物件。 上述範例會計算資料表中的資料列數目**SalesOrderHeader**。 下一步 **ExecuteScalar**方法**cmd**稱為物件。 這會傳回型別的值**int**查詢為基礎。 最後，訂單計數會傳回到呼叫端。  
...
> [!NOTE]  
>  使用 visual c + + 資料庫物件編譯 **/clr: pure**上不支援執行[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 例如，這類資料庫物件包括純量值函式。  
