...
```  
 ...
 在此機碼下會列出機碼清單，其中每個機碼各代表電腦上已安裝的每個 LocalDB 版本。 這些索引鍵的每一個都是以格式的 LocalDB 版本號碼來命名 *\<major-version>* 。*\<minor-version>* （例如，的索引鍵 [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 名為13.0）。 在每個版本機碼下會列出 `InstanceAPIPath` 名稱/值組，定義隨該版本安裝之 SQLUserInstance.dll 檔案的完整路徑。 下列範例顯示已安裝 LocalDB 版本11.0 和13.0 之電腦的登錄專案：  
