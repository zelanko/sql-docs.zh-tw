...
建議使用者在建立節點資料表時，于資料`$node_id`行上建立唯一的條件約束或索引，但如果未建立，則會自動建立預設的唯一非叢集索引。 不過，在基礎內部資料行上會建立圖形虛擬資料行上的任何索引。 也就是在資料`$node_id`行上建立的索引，將會出現在`graph_id_<hex_string>`內部資料行上。   
...
|資料行名稱    |描述  |
|---   |---  |
|`$edge_id`   |可唯一識別資料庫中的指定邊緣。 這是產生的資料行，而值是邊緣資料表的 object_id 與內部產生的 Bigint 值的組合。 不過，選取資料`$edge_id`行時，會顯示 JSON 字串形式的計算值。 `$edge_id`是虛擬資料行，其對應至具有十六進位字串的內部名稱。 當您從`$edge_id`資料表中選取時，資料行名稱將會`$edge_id_\<hex_string>`顯示為。 在查詢中使用虛擬資料行名稱是查詢內部`$edge_id`資料行的建議方式，而且應該避免使用具有十六進位字串的內部名稱。 |
|`$from_id`   |儲存來自`$node_id`邊緣來源之節點的。  |
...
與資料`$node_id`行類似，建議使用者在建立邊緣資料表時，于資料`$edge_id`行上建立唯一的索引或條件約束，但如果未建立，則會自動在此資料行上建立預設的唯一非叢集索引。 不過，在基礎內部資料行上會建立圖形虛擬資料行上的任何索引。 也就是在資料`$edge_id`行上建立的索引，將會出現在`graph_id_<hex_string>`內部資料行上。 也建議在 OLTP 案例中，使用者會在（`$from_id`， `$to_id`）資料行上建立索引，以加速在邊緣方向進行查閱。  
...
|EDGE_ID_FROM_PARTS |從`edge_id` `object_id`和身分識別結構  |
