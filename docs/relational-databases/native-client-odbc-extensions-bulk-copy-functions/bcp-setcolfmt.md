...
ms.locfileid: "37411227"
---
# <a name="bcpsetcolfmt"></a>bcp_setcolfmt
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
[!INCLUDE[SNAC_Deprecated](../../includes/snac-deprecated.md)]

  **Bcp_setcolfmt**函數會取代[bcp_colfmt](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-colfmt.md)。 在指定資料行定序**bcp_setcolfmt**函式必須使用。 [bcp_setbulkmode](../../relational-databases/native-client-odbc-extensions-bulk-copy-functions/bcp-setbulkmode.md)可用來指定多個資料行格式。  
  
 此函數會提供彈性的方法來指定大量複製作業中的資料行格式。 它會用來設定個別的資料行格式屬性。 每次呼叫**bcp_setcolfmt**設定一個資料行格式屬性。  
  
 **Bcp_setcolfmt**函式會指定使用者檔案中的資料來源或目標格式。 當做來源格式使用時**bcp_setcolfmt**指定現有的資料檔案做為資料來源中的資料表中大量複製資料的格式[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 資料檔案當做目標格式使用時，會建立使用指定的資料行格式**bcp_setcolfmt**。  
...
## <a name="remarks"></a>備註  
 這個函數會取代**bcp_colfmt**函式。 所有的功能**bcp_colfmt**中提供**bcp_setcolfmt**函式。 此外，也提供資料行定序的支援。 建議使用以下提供的順序設定下列資料行格式屬性：  
 ...
 **Bcp_columns**必須呼叫函式，然後再呼叫**bcp_setcolfmt**。  
 ...
 您必須呼叫**bcp_setcolfmt**一次，每個屬性的使用者檔案中的每個資料行。  
...
## <a name="bcpsetcolfmt-support-for-enhanced-date-and-time-features"></a>bcp_setcolfmt 對於增強型日期和時間功能的支援  
 BCP_FMT_TYPE 屬性使用用於日期/時間類型的類型是在指定[增強型日期和時間類型的大量複製變更&#40;OLE DB 和 ODBC&#41;](../../relational-databases/native-client-odbc-date-time/bulk-copy-changes-for-enhanced-date-and-time-types-ole-db-and-odbc.md)。  
