 ...
 *EUserDataType*參數是由 sqlncli 中的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料類型 token 所列舉，而不是 ODBC C 資料類型枚舉器。 例如，您可以使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 專屬類型 SQLCHARACTER 來指定字元字串 ODBC type SQL_C_CHAR。  
...
## <a name="bcp_colfmt-support-for-enhanced-date-and-time-features"></a>bcp_colfmt 支援增強的日期和時間功能  
 如需有關搭配日期/時間類型的*eUserDataType*參數使用之類型的詳細資訊，請參閱[OLE DB 和&#40;ODBC&#41;的增強型日期和時間類型的大量複製變更](../../relational-databases/native-client-odbc-date-time/bulk-copy-changes-for-enhanced-date-and-time-types-ole-db-and-odbc.md)。  
