...
|**長**|**Bigint**|[Systypes &#40;transact-sql&#41;](../../relational-databases/system-compatibility-views/sys-systypes-transact-sql.md)的最大實體儲存體長度。|  
...
|**投影**|**int**|這個資料行出現在其中的資料列內位移。|  
|**collationid**|**int**|資料行定序的識別碼。 以非字元為基礎的資料行是 NULL。|  
|**語言**|**int**|資料行的語言識別碼。|  
|**狀態**|**int**|用來描述資料行或參數屬性的點陣圖：<br /><br /> **0x08** = 資料行允許 null 值。<br /><br /> **0x10** = 當加入**Varchar**或**Varbinary**資料行時，ANSI 填補已生效。 會保留**Varchar**的尾端空白，並保留**Varbinary**資料行的尾端零。<br /><br /> **0x40** = 參數是輸出參數。<br /><br /> **0x80** = 資料行是識別欄位。|  
