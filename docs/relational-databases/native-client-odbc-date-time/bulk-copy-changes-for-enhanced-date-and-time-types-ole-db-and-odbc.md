...
## <a name="character-data-files"></a>字元資料類型  
 在字元資料檔中，日期和時間值會以 odbc[日期和時間改善的資料類型支援](../../relational-databases/native-client-odbc-date-time/data-type-support-for-odbc-date-and-time-improvements.md)的「資料格式：字串和常值」一節中的描述來表示，或[OLE DB 日期和時間改善的資料類型支援](../../relational-databases/native-client-ole-db-date-time/data-type-support-for-ole-db-date-and-time-improvements.md)OLE DB。  
  
 在原生資料檔中，四個新類型的日期和時間值會表示為其 TDS 標記法，其小數位數為7（因為這是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 所支援的最大值，而且 bcp 資料檔案不會儲存這些資料行的小數位數）。 現有 **datetime** 和 **smalldatetime** 類型的儲存或其表格式資料流 (TDS) 表示法沒有變更。  
