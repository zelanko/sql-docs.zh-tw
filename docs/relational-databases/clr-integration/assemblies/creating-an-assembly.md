...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
  預存程序或觸發程序之類的 Managed 資料庫物件會經過編譯，然後再稱為組件的單元中進行部署。 Managed DLL 元件必須先在中[!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]註冊，然後才能使用元件提供的功能。 若要在 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 資料庫中註冊組件，請使用 CREATE ASSEMBLY 陳述式。 本主題討論如何在資料庫中使用 CREATE ASSEMBLY 陳述式註冊組件，以及如何指定組件的安全性設定。  
 ...
 **UNSAFE**程式碼許可權適用于元件不是可驗證的安全或需要額外存取受限制資源（例如[!INCLUDE[msCoName](../../../includes/msconame-md.md)] WIN32 API）的情況。  
 ...
 若要在中[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]建立**EXTERNAL_ACCESS**或**UNSAFE**元件，必須符合下列兩個條件之一：  
  
1.  組件是用強式名稱簽署，或用包含憑證的 Authenticode 簽署。 這個強式名稱（或憑證）會在內[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]建立為非對稱金鑰（或憑證），並具有對應的登入，具有**外部存取元件**許可權（適用于外部存取元件）或**unsafe 元件**許可權（適用于 UNSAFE 元件）。  
