...
description: 裝載 CLR 的 Microsoft SQL Server 稱為 CLR 整合，預設為停用。 使用 sp_configure 預存程式來啟用 CLR 整合。
ms.custom: ''
ms.date: 09/17/2019
ms.prod: sql
ms.reviewer: ''
...
ms.locfileid: "85727616"
---
# <a name="clr-integration---enabling"></a>CLR 整合 - 啟用
[!INCLUDE [SQL Server SQL MI](../../includes/applies-to-version/sql-asdbmi.md)]
  預設會停用 Common Language Runtime (CLR) 整合功能，且為了使用 CLR 整合所實作的物件，必須啟用這個功能。 若要啟用 CLR 整合，請使用中**sp_configure**預存程式的**CLR enabled**選項 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] ：  
...
```  
 ...
 您可以將**clr enabled**選項設定為0，以停用 clr 整合。 當您停用 CLR 整合時，會 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 停止執行所有使用者定義的 CLR 常式，並卸載所有應用程式域。 依賴 CLR 的功能（例如**hierarchyid**資料類型、函式 `FORMAT` 、複寫和以原則為基礎的管理）不會受到這項設定的影響，而且將會繼續運作。
