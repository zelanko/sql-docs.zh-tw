...
description: 您可以使用 Microsoft Visual Studio 開發 SQL Server 的 CLR 資料庫物件、將它們部署至測試伺服器，然後將它們散發至實際伺服器。
ms.custom: ''
ms.date: 03/16/2017
ms.prod: sql
ms.reviewer: ''
...
> [!NOTE]  
>  您必須在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體上啟用 TCP/IP 網路通訊協定，才能針對遠端開發、偵錯和開發使用 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Visual Studio。 如需在伺服器上啟用 TCP/IP 通訊協定的詳細資訊，請參閱 [設定用戶端通訊](../../database-engine/configure-windows/configure-client-protocols.md)協定。  
  
#### <a name="to-deploy-the-assembly-using-visual-studio"></a>使用 Visual Studio 來部署組件  
  
1.  **Build** \<project name> 從 [**組建**] 功能表選取 [組建] 以建立專案。  
  
2.  解決所有建立錯誤和警告，然後再將組件部署至測試伺服器。  
  
3.  從 [**組建**] 功能表中選取 [**部署**]。 然後，系統就會在第一次於 Visual Studio 中建立 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 專案時指定的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體和資料庫中註冊此組件。  
...
1.  然後，您必須在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的執行個體中建立程序、函數、彙總、使用者定義型別或觸發程序。 如果**HelloWorld** **元件在 procedure 類別中**包含名為**HelloWorld**的方法，則可以將下列程式 [!INCLUDE[tsql](../../includes/tsql-md.md)] 新增至查詢，以建立名為**hello** in 的程式 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 。  
 ...
 如需在中建立不同類型的 managed 資料庫物件的詳細資訊 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，請參閱 [clr User-Defined](../../relational-databases/clr-integration-database-objects-user-defined-functions/clr-user-defined-functions.md)函式、 [clr User-Defined 匯總](../../relational-databases/clr-integration-database-objects-user-defined-functions/clr-user-defined-aggregates.md)、 [Clr User-Defined 類型](../../relational-databases/clr-integration-database-objects-user-defined-types/clr-user-defined-types.md)、 [clr 預存程式](/dotnet/framework/data/adonet/sql/clr-stored-procedures)和 [clr 觸發](/dotnet/framework/data/adonet/sql/clr-triggers)程式。  
  
## <a name="deploying-the-assembly-to-production-servers"></a>將組件部署至實際伺服器  
 一旦 CLR 資料庫物件已經在測試伺服器上測試並驗證之後，您就可以將它們散發至實際伺服器。 如需有關偵錯工具管理資料庫物件的詳細資訊，請參閱偵測 [CLR 資料庫物件](../../relational-databases/clr-integration/debugging-clr-database-objects.md)。  
...
2.  在 **物件總管**中，展開 [ **\<server name>** 和 **資料庫** ] 樹狀目錄。 以滑鼠右鍵按一下已註冊管理資料庫物件的資料庫 **，選取 [** 工作]，然後選取 [ **產生腳本**]。 指令碼精靈隨即開啟。  
...
6.  針對 [**選擇物件類型**] 窗格中選取的每個物件類型，會顯示** \<type> [選擇**] 窗格。 在這個窗格中，您可以從該資料庫物件類型 (已在指定的資料庫中註冊) 的所有執行個體中選擇。 選取一或多個物件，然後按 **[下一步]**。  
  
7.  當選取所有所需的資料庫物件類型時，就會出現 [ **輸出選項** ] 窗格。 選取 [ **編寫腳本至** 檔案]，並指定腳本的檔案路徑。 選取 [下一步] 。 檢查您的選擇，然後按一下 **[完成]**。 此時，部署指令碼就會儲存至指定的檔案路徑。  
  
## <a name="post-deployment-scripts"></a>部署後指令碼  
 您可以執行部署後指令碼。  
  
 若要加入部署後指令碼，請在您的 Visual Studio 專案目錄中加入稱為 postdeployscript.sql 的檔案。 例如，在 **方案總管** 中，以滑鼠右鍵按一下您的專案，然後選取 [ **加入現有專案**]。 將此檔案加入專案的根目錄中，而非加入 Test Scripts 資料夾中。  
