...
# 登入觸發程序
 ...
 您可以使用登入觸發程序稽核和控制伺服器工作階段，例如追蹤登入活動、限制登入 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]，或限制特定登入的工作階段數。 例如，在下列程式碼中，如果登入 *login_test* 已經建立三個使用者工作階段，登入觸發程序就會拒絕該登入對 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 所起始的登入嘗試。  
