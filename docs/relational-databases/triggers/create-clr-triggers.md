...
-   以 .NET Framework 支援的語言，將觸發程序定義為類別。 如需如何在 CLR 中設計觸發程序的程式的詳細資訊，請參閱 [CLR 觸發程序](../Topic/CLR%20Triggers.md)。 然後，使用適當的語言編譯器編譯類別，在 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 中建立組件。  
...
> [!NOTE]  
>  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行 CLR 程式碼的功能預設為關閉。 您可以建立、修改和卸除參考 Managed 程式碼模組的物件，但是除非使用 [sp_configure (Transact-SQL)](../../relational-databases/system-stored-procedures/sp-configure-transact-sql.md) 來啟用 [clr enabled 選項](../../database-engine/configure-windows/clr-enabled-server-configuration-option.md)，否則在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 中將無法執行這些參考。  
