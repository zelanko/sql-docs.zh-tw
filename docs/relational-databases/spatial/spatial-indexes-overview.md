...
> [!IMPORTANT]  
>   如需 [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]導入的空間功能 (包括影響空間索引的功能) 的詳細描述和範例，請下載技術白皮書： [SQL Server 2012 中的新空間功能](https://go.microsoft.com/fwlink/?LinkId=226407)。  
...
>  空間索引的空間分解與應用程式資料使用的度量單位無關。  
 ...
 ![將多邊形和線條放入 4x4 層級 1 方格內](../../relational-databases/spatial/media/spndx-level-1-objects.gif "將多邊形和線條放入 4x4 層級 1 方格內")  
...
#### <a name="geometry-grid-tessellation-scheme"></a>幾何方格鑲嵌式配置  
 GEOMETRY_AUTO_GRID 鑲嵌是 **和更新版本之** geometry [!INCLUDE[ssNoVersion](../../includes/sssql11-md.md)] 資料類型的預設鑲嵌式配置。  GEOMETRY_GRID 鑲嵌是唯一適用於 [!INCLUDE[ssNoVersion](../../includes/sskatmai-md.md)]中 geometry 資料類型的鑲嵌式配置。 本章節討論與處理空間索引相關之幾何方格鑲嵌的層面：支援的方法及週框方塊。  
...
3.  聯結扁平化的金字塔，以形成非 Euclidean 平面。  
 ...
 下圖顯示三個步驟之分解程序的圖解檢視。 在金字塔中，點狀的線條代表每一個金字塔之四個 Facet 的界限。 步驟 1 和 2 會使用綠色水平線代表赤道緯度線，以及一系列綠色垂直線代表多條經度線來繪製測地橢圓體。 步驟 1 顯示投射到兩個半球上的金字塔。 步驟 2 顯示扁平化的金字塔。 步驟 3 說明扁平化的金字塔在經過組合之後會形成一個平面，顯示投射的經線數目。 請注意，這些投射的線條會變直，而且長度會視其落於金字塔上的位置而有所不同。  
