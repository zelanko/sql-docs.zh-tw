...
description: 了解如何使用 SQL Server Management Studio 或 Transact-SQL，在 SQL Server 中啟用資料表或索引的壓縮。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
5.  在 **[選取壓縮類型]** 頁面上，選取要套用至您想要壓縮之資料表或索引中每個資料分割的壓縮類型。 完成後，請按一下 [下一步]。  
...
在 SQL Server 中執行 `sp_estimate_data_compression_savings`，然後啟用資料表或索引的壓縮。 請參閱下列各節。 
...
### <a name="on-azure-sql-database"></a>在 Azure SQL Database 上

Azure SQL Database 不支援 `sp_estimate_data_compression`。 下列指令碼會啟用壓縮，而不會估計壓縮量。 
...
3.  複製下列範例並將其貼到查詢視窗中，然後按一下 **[執行]** 。 此範例會針對指定資料表中的所有資料分割啟用 ROW 壓縮。  
...
3.  複製下列範例並將其貼到查詢視窗中，然後按一下 **[執行]** 。 此範例會先查詢 `sys.indexes` 目錄檢視以傳回 `index_id` 資料表上每個索引的名稱和 `Production.TransactionHistory` 。 最後，此範例會重建索引識別碼 2 (`IX_TransactionHistory_ProductID`)，並指定 PAGE 壓縮。  
