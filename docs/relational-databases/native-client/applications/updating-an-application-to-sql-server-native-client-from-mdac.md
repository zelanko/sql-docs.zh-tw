...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
 ...
 本主題的資訊可幫助您更新 MDAC (或 Windows DAC) 應用程式，使其保持為 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 中隨附的最新 [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] Native Client 版本。 為了説明您使此應用程式與在[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)][!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)]中 附帶的本機客戶端的版本保持最新,請參閱從 SQL Server [2005 本機用戶端更新應用程式](../../../relational-databases/native-client/applications/updating-an-application-from-sql-server-2005-native-client.md)。  
...
-   [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 不包含 XML 整合。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本機客戶端支援 SELECT ...對於 XML 查詢,但不支援任何其他 XML 功能。 但是,[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本機客戶端**xml**確實支援[!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)]在中引入的 xml 數據類型。  
...
-   [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 與 odbcbcp.dll 不相容。 必須重新生成同時使用 ODBC 和**bcp** API 的應用程式才能與 sqlncli11.lib[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]連結才能使用本機用戶端。  
...
-   MDAC 連接字串允許針對 **Trusted_Connection** 關鍵字使用布林值 (**true**)。 這個機客戶端連接字串必須使用**是**或否**no**[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]。  
...
-   [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 的錯誤檢查要比 MDAC 嚴格，這表示未嚴謹符合 ODBC 和 OLE DB 規格的某些應用程式可能會有不同的行為。 例如,SQLOLEDB 提供程式沒有強制實施參數名稱必須以\@'' 開頭的結果參數的[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]規則,但 本機用戶端 OLE DB 提供程式強制執行。  
...
-   [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本機客戶端實現**IDBDataSourceAdmin**介面,該介面是以前未實現的可選 OLE DB 介面,但僅實現此可選介面的**CreateDataSource**方法。 [!INCLUDE[ssNoteDepFutureAvoid](../../../includes/ssnotedepfutureavoid-md.md)]  
...
-   數據類型[!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)]**udt**varchar(max)、nvarchar(max)、varbinary(max)、xml、udt 或其他大型對象類型的返回值不能返回到早於的**varchar(max)****nvarchar(max)****varbinary(max)****xml**用戶端版本。 如果您想要使用這些類型當做傳回值，則必須使用 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client。  
...
-   當 MDAC 應用程式連接到 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 時，[!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] 中導入的資料類型將會以 [!INCLUDE[ssVersion2000](../../../includes/ssversion2000-md.md)] 相容的資料類型形式出現，如下表所示。  
    ...
    |**xml**|**ntext**|  
     ...
     此類型對應會影響資料行中繼資料傳回的值。 例如,**文本**列的最大大小[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]為 2,147,483,647,但本機用戶端 ODBC 將**varchar(最大)** 列的最大大小[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]報告為SQL_SS_LENGTH_UNLIMITED,本機 用戶端 OLE DB 報告**varchar(最大)** 列的最大大小為 2,147,483,647 或 -1,具體取決於平臺。  
...
-   如果您使用 ODBC 或 OLE DB 呼叫來開始交易，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 與 MDAC 之間會有行為上的差異；使用 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 時會立即開始交易，但是使用 MDAC 時會在初次存取資料庫之後開始交易。 這可能會影響存儲過程和批處理的行為,因為[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]要求在\@\@批處理或存儲過程完成執行後,TRANCOUNT 與批處理或存儲過程啟動時相同。  
  
-   使用[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]本機用戶端時,I事務本地:開始事務將導致立即啟動事務。 當使用 MDAC 時，交易會延遲到應用程式執行陳述式之後才開始，這需要交易處於隱含交易模式中。 有關詳細資訊,請參閱[set IMPLICIT_TRANSACTIONS&#40;交易-SQL&#41;](../../../t-sql/statements/set-implicit-transactions-transact-sql.md)。  
  
-   在將本機客戶端驅動程式[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]與 System.Data.Odbc 一[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]起使用伺服器 電腦時可能會遇到錯誤,[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]該伺服器電腦公開了新的特定資料類型或功能。 System.Data.Odbc 提供了通用 ODBC 實現,隨後不公開供應商特定的功能或擴展。 (本機[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]客戶端驅動程式已更新為本機[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]支援最新 功能。要解決此問題,可以還原到 MDAC,也可以遷移到 System.Data.SqlClient。  
