...
> 如需資訊，請參閱[OLE DB Driver for SQL Server 搭配使用連接字串關鍵字](../../../connect/oledb/applications/using-connection-string-keywords-with-oledb-driver-for-sql-server.md)。
...
## <a name="odbc-driver-connection-string-keywords"></a>ODBC 驅動程式連接字串關鍵字  
 ODBC 應用程式會將連接字串做為參數[SQLDriverConnect](../../../relational-databases/native-client-odbc-api/sqldriverconnect.md)並[SQLBrowseConnect](../../../relational-databases/native-client-odbc-api/sqlbrowseconnect.md)函式。  
...
|**驅動程式**|所傳回的驅動程式名稱[SQLDrivers](../../../relational-databases/native-client-odbc-api/sqldrivers.md)。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驅動程式的關鍵字值是 "{SQL Server Native Client 11.0}"。 **伺服器**關鍵字是必要的如果**驅動程式**指定並**DriverCompletion**設定為 SQL_DRIVER_NOPROMPT。<br /><br /> 如需驅動程式名稱的詳細資訊，請參閱[使用 SQL Server Native Client 標頭和程式庫檔案](../../../relational-databases/native-client/applications/using-the-sql-server-native-client-header-and-library-files.md)。|  
|**DSN**|現有 ODBC 使用者或系統資料來源的名稱。 這個關鍵字會覆寫任何可能在中指定的值**伺服器**，**網路**，並**位址**關鍵字。|  
...
|**TrustServerCertificate**|當搭配**Encrypt**，可讓使用自我簽署的伺服器憑證加密。|  
...
|**WSID**|工作站識別碼。 一般而言，這是應用程式所在之電腦的網路名稱 (選擇性)。 如果指定，這個值會儲存在**master.dbo.sysprocesses**資料行**hostname** ，傳回[sp_who](../../../relational-databases/system-stored-procedures/sp-who-transact-sql.md)而[HOST_NAME](../../../t-sql/functions/host-name-transact-sql.md)函式。|  
  
> **注意：** 地區轉換設定適用於貨幣、數值、日期和時間資料類型。 此轉換設定只適用於輸出轉換，而且只有當貨幣、數值、日期或時間值轉換為字元字串時，才能看到。  
...
|**Server**|DBPROP_INIT_DATASOURCE|組織中 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 執行個體的名稱。<br /><br /> 如果沒有指定，就會連接至本機電腦上的預設執行個體。<br /><br /> 如需有關有效位址語法的詳細資訊，請參閱說明**Server** ODBC 關鍵字，本主題中的。|  
