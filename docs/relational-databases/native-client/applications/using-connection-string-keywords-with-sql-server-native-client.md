...
|**[資料庫備份]**|用於連接的預設 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 資料庫名稱。 如果**資料庫**未指定，會使用登入所定義的預設資料庫。 ODBC 資料來源中的預設資料庫會覆寫針對此登入所定義的預設資料庫。 資料庫必須是現有的資料庫，除非**AttachDBFileName**同時指定。 如果**AttachDBFileName**同時指定，會附加它所指向的主要檔案，並指定所指定的資料庫名稱**資料庫**。|  
...
|**資料來源名稱**|現有 ODBC 使用者或系統資料來源的名稱。 這個關鍵字會覆寫任何可能在中指定的值**伺服器**，**網路**，和**位址**關鍵字。|  
...
|**封包**|SSPROP_INIT_PACKETSIZE|網路封包大小。 預設值為 4096。|  
