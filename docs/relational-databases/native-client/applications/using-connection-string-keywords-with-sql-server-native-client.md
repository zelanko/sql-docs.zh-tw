---
title: 使用連接字串關鍵字
description: 某些 SQL Server Native Client Api 會使用連接字串來指定連接屬性。 連接字串是關鍵字/值配對。
ms.custom: ''
ms.date: 08/01/2016
ms.prod: sql
ms.reviewer: ''
...
|**APP**|呼叫[SQLDriverConnect](../../../relational-databases/native-client-odbc-api/sqldriverconnect.md)的應用程式名稱（選擇性）。 如果指定的話，這個值會儲存在 [ **master.dbo.sys處理**程式] 資料行中**program_name**並由[sp_who](../../../relational-databases/system-stored-procedures/sp-who-transact-sql.md)和[APP_NAME](../../../t-sql/functions/app-name-transact-sql.md)函數傳回。|  
...
|**語言**|[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 語言名稱 (選擇性)。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]可以在**sysmessages**中儲存多種語言的訊息。 如果使用 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 多種語言連接到，**語言**會指定要用於連接的訊息集。|  
|**MARS_Connection**|啟用或停用連接上的 Multiple Active Result Sets (MARS)。 可辨識的值為 "yes" 和 "no"。 預設值是 "no"。|  
|**MultiSubnetFailover**|當連接到可用性群組的可用性群組接聽程式或容錯移轉叢集實例時，一律指定**multiSubnetFailover = Yes** [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 。 **multiSubnetFailover = Yes**會設定 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client，以提供對（目前）使用中伺服器更快速的偵測與連接。 可能的值為 **[是] 和 [** **否**]。 預設值為 [**否**]。 例如：<br /><br /> `MultiSubnetFailover=Yes`<br /><br /> 如需有關 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 支援的詳細資訊 [!INCLUDE[ssHADR](../../../includes/sshadr-md.md)] ，請參閱[高可用性和嚴重損壞修復 SQL Server Native Client 支援](../../../relational-databases/native-client/features/sql-server-native-client-support-for-high-availability-disaster-recovery.md)。|  
|**Net**|"Network" 的同義字。|  
|**Network**|有效值為**dbnmpntw** （具名管道）和**dbmssocn** （tcp/ip）。<br /><br /> 指定**Network**關鍵字的值和**Server**關鍵字的通訊協定前置詞是錯誤的。|  
|**PWD**|指定於 UID 參數中之 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 登入帳戶的密碼。 **PWD**如果登入具有 Null 密碼或使用 Windows 驗證（），則不需要指定 PWD `Trusted_Connection = yes` 。|  
...
|**WSID**|工作站識別碼。 一般而言，這是應用程式所在之電腦的網路名稱 (選擇性)。 如果指定的話，這個值會儲存在**master.dbo.sys處理**資料行**主機名稱**，並由[sp_who](../../../relational-databases/system-stored-procedures/sp-who-transact-sql.md)和[HOST_NAME](../../../t-sql/functions/host-name-transact-sql.md)函數傳回。|  
