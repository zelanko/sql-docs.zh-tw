...
您也可以從 SQL Server 2012 功能套件網頁取得 sqlncli.msi。 若要下載最新版本的 SQL Server Native Client，請移至[Microsoft® SQL Server® 2012 Feature Pack](https://www.microsoft.com/download/confirmation.aspx?id=29065)。 如果電腦上也安裝早于 SQL Server 2012 的舊版 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 11.0 將與舊版並存安裝。  
...
>  [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者和 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驅動程式的所有適當的登錄設定，都會在安裝程序時進行。  
 ...
 當您叫用 sqlncli.msi 時，依預設會安裝用戶端元件。 用戶端元件是支援執行使用 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 所開發之應用程式的檔案。 如果也要安裝 SDK 元件，請在命令列上指定 `ADDLOCAL=All`。 例如：  
...
## <a name="uninstalling-sql-server-native-client"></a>解除安裝 SQL Server Native Client  
 因為 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] server 和 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 工具之類的應用程式相依于 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client，所以在卸載所有相依的應用程式之前，請務必不要卸載 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client。 若要提供應用程式相依于 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 的警告給使用者，請使用 MSI 中的 APPGUID 安裝選項，如下所示：  
