---
title: 用戶端連接中的服務主要名稱 (SPN) 支援 |Microsoft Docs
ms.custom: ''
ms.date: 08/08/2016
ms.prod: sql
ms.reviewer: ''
...
ms.locfileid: "37411467"
---
# <a name="service-principal-name-spn-support-in-client-connections"></a>用戶端連接中的服務主要名稱 (SPN) 支援
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
[!INCLUDE[SNAC_Deprecated](../../../includes/snac-deprecated.md)]

  開頭為[!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)]，服務主體名稱 (Spn) 的支援已經過擴充，以跨所有通訊協定進行相互驗證。 在舊版[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]，支援 Spn 的 Kerberos 透過 TCP 時的預設 SPN[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]與 Active Directory 註冊任何執行個體。  
  
 驗證通訊協定會使用 Spn 來判斷帳戶[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]執行個體執行。 如果已知執行個體帳戶，可以透過用戶端和伺服器使用 Kerberos 驗證提供相互驗證。 如果未知執行個體帳戶，會使用僅透過伺服器提供用戶端驗證的 NTLM 驗證。 目前，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]原生用戶端會執行驗證查閱，從執行個體名稱和網路連接屬性衍生 SPN。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 執行個體將會嘗試在啟動時，註冊 Spn，或者也可以手動將它註冊。 不過，如果對於嘗試註冊 SPN 的帳戶沒有足夠的存取權限，註冊將會失敗。  
  
 網域和電腦帳戶會自動在 Active Directory 中註冊。 這些帳戶可以當做 SPN 使用，或者系統管理員可以定義自己的 SPN。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 使安全驗證更容易管理而且更可靠，以允許用戶端直接指定要使用的 SPN。  
...
-   [Windows 的 Kerberos 技術資訊補充](http://go.microsoft.com/fwlink/?LinkId=101449)  
...
## <a name="linked-servers-and-delegation"></a>連結的伺服器與委派  
 建立連結的伺服器，當**@provstr**參數[sp_addlinkedserver](../../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md)可用來指定伺服器和容錯移轉夥伴 Spn。 這麼做的優點與在用戶端連接字串中指定 SPN 相同：建立使用 Kerberos 驗證的連接更簡單，而且更可靠。  
...
-   [服務主體名稱&#40;Spn&#41;用戶端連接中&#40;OLE DB&#41;](../../../relational-databases/native-client/ole-db/service-principal-names-spns-in-client-connections-ole-db.md)  
