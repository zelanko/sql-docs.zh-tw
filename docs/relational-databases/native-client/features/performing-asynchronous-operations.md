...
## <a name="opening-and-closing-a-database-connection"></a>開啟及關閉資料庫連接  
 ...
 您可以呼叫**IDBAsynchStatus：： abort**或[ISSAsynchStatus：： abort](../../../relational-databases/native-client-ole-db-interfaces/issasynchstatus-abort-ole-db.md)來取消非同步**初始化**呼叫。 取用者必須明確地要求非同步資料來源初始化。 否則，要等到資料來源物件完全初始化之後，**IDBInitialize::Initialize** 才會傳回。  
