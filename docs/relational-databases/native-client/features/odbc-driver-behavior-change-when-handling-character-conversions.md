...
|[!INCLUDE[ssSQL11](../../../includes/sssql11-md.md)] Native Client (11.0.2100.60 版) 或更新版本|-4 (SQL_NO_TOTAL)|驅動程式不會再假設從 CHAR 轉換成 WCHAR 或 WCHAR 至 CHAR 的是（相乘） \*2 或（除法）/2 動作。<br /><br /> 呼叫**SQLGetData**不再傳回預期轉換的長度。 驅動程式將偵測出這是 CHAR 與 WCHAR 之間的相互轉換，並且傳回 (-4) SQL_NO_TOTAL 而不至於會有 *2 或 /2 的錯誤行為。|  
