...
>  大數值資料類型的最大大小可介於 1 和 8 KB 之間，或者也可以指定為無限制。  
 ...
 先前，只有[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]等資料類型**文字**， **ntext**並**映像**無法達到此種長度。 **Max**規範**varchar**， **nvarchar**並**varbinary**使這些資料類型的備援。 不過，因為 long 資料類型仍可使用，所以大部分 OLE DB 和 ODBC 資料存取元件的介面都仍保持原樣。 為了與先前的版本保持回溯相容性，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者中的 DBCOLUMNFLAGS_ISLONG 旗標以及 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驅動程式中的 SQL_LONGVARCHAR 仍會繼續使用。 當新類型設定為無限制的最大長度時，針對 [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] 和更新版本所撰寫的提供者和驅動程式仍會為其繼續使用這些詞彙。  
  
> [!NOTE]  
>  您也可以指定**varchar （max)**， **nvarchar （max)**，並**varbinary （max)** 做為預存程序、 函式的輸入和輸出參數類型的資料類型傳回型別或在[CAST 和 CONVERT](../../../t-sql/functions/cast-and-convert-transact-sql.md)函式。  
  
> [!NOTE]  
>  如果複寫資料，則您可能需要設定[max text repl size 伺服器組態選項](../../../database-engine/configure-windows/configure-the-max-text-repl-size-server-configuration-option.md)為-1。  
...
## <a name="sql-server-native-client-ole-db-provider"></a>SQL Server Native Client OLE DB 提供者  
 ...
 資料型別**varchar （max)**， **varbinary （max)**，和**nvarchar （max)** 中的資料行**max**大小設定為無限制表示為 ISLONG 核心 OLE DB 結構描述資料列和傳回資料行資料類型的介面。  
  
 命令物件的**IAccessor**實作已變更為允許繫結為 DBTYPE_IUNKNOWN。 如果取用者指定 DBTYPE_IUNKNOWN，並設定*pObject*為 null，提供者會傳回**ISequentialStream**介面，讓取用者可以資料流取用者**varchar (max)**， **nvarchar （max)**，或**varbinary （max)** 傳出輸出變數的資料。  
  
 以資料流傳輸的輸出參數值會在任何結果資料列之後傳回。 如果應用程式會嘗試繼續到下一個結果集藉由呼叫**imultipleresults:: Getresult**而不取用所有傳回的輸出參數值，就會傳回 DB_E_OBJECTOPEN。  
  
 為了支援資料流， [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者需要循序存取可變長度的參數。 這表示，必須要 DBPROPVAL_AO_SEQUENTIALSTORAGEOBJECTS 或 DBPROPVAL_AO_SEQUENTIAL 設定 DBPROP_ACCESSORDER 每當**varchar （max)**， **nvarchchar （max)**，或**varbinary （max)** 資料行或輸出參數繫結至 DBTYPE_IUNKNOWN。 若要呼叫**irowset:: Getdata**如果不符合這項存取順序限制將會失敗且傳回 DBSTATUS_E_UNAVAILABLE。 當沒有任何使用 DBTYPE_IUNKNOWN 的輸出繫結時，這項限制就不適用。  
  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者也支援繫結的輸出參數針對大數值資料類型為 dbtype_iunknown，預存程序傳回作為傳回值公開為 DBTYPE_IUNKNOWN 的大數值類型用戶端。  
...
-   繫結為 DBTYPE_IUNKNOWN 並使用資料流。  
 ...
 報告的資料行的大小上限時[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]Native Client OLE DB 提供者會報告：  
  
-   比方說，即 2000年的定義的大小上限**varchar (** 2000年 **)** 資料行，或  
  
-   「 無限制 」 的值如果是這**varchar （max)** 資料行則等於 ~ 0。 這個值會針對 DBCOLUMN_COLUMNSIZE 中繼資料屬性設定。  
  
 標準轉換規則會套用至**varchar （max)** 資料行，代表任何有效的轉換**varchar (** 2000年 **)** 資料行也會適用於**varchar （max)** 資料行。 這也適用於**nvarchar （max)** 並**varbinary （max)** 資料行。  
...
## <a name="sql-server-native-client-odbc-driver"></a>SQL Server Native Client ODBC 驅動程式  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驅動程式會公開**varchar （max)**， **varbinary （max)** 並**nvarchar （max)** 為 SQL_VARCHAR、 SQL_VARBINARY 和 SQL_ 類型WVARCHAR 接受或傳回 ODBC SQL 資料類型的 ODBC API 函式中。  
  
 在報告資料行的最大大小時，驅動程式會報告下列其中一項：  
  
-   比方說，即 2000年的定義的大小上限**varchar(2000)** 資料行，或  
  
-   「 無限制 」 的值如果是這**varchar （max)** 資料行則等於 0。  
  
 標準轉換規則適用於**varchar （max)** 資料行，代表任何有效的轉換**varchar (** 2000年 **)** 資料行也會適用於**varchar （max)** 資料行。 這也適用於**nvarchar （max)** 並**varbinary （max)** 資料行。  
