...
|DBTYPE_VARIANT (VT_BSTR)|支援<sup>3、6</sup>|N/A<sup>2</sup>|N/A|N/A<sup>2</sup>|  
 ...
 <sup>1</sup>如果使用**ICommandWithParameters：： SetParameterInfo**指定了 DBTYPE_UDT 以外的伺服器類型，且存取子類型為 DBTYPE_UDT，則在執行語句（DB_E_ERRORSOCCURRED; 參數狀態為 DBSTATUS_E_BADACCESSOR）時，就會發生錯誤。 否則資料會傳給伺服器，但是伺服器會傳回一則錯誤，指示從 UDT 到參數的資料類型之間沒有隱含轉換。  
  
 <sup>2</sup>超出本主題的範圍。  
  
 <sup>3</sup>從十六進位字串轉換為二進位資料的資料。  
  
 <sup>4</sup>發生從二進位資料轉換為十六進位字串的資料。  
  
 <sup>5</sup>在建立存取子時間或提取時，會進行驗證，錯誤是 DB_E_ERRORSOCCURRED，系結狀態設定為 DBBINDSTATUS_UNSUPPORTEDCONVERSION。  
