...
### <a name="ole-db-user-interface-password-expiration"></a>OLE DB 使用者介面密碼逾期  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者會透過對 [ **SQL Server 登**入] 對話方塊所做的變更，支援密碼到期。 如果 DBPROP_INIT_PROMPT 的值設定為 DBPROMPT_NOPROMPT，則密碼到期時，初始連接嘗試將會失敗。  
...
### <a name="ole-db-programmatic-password-expiration"></a>OLE DB 程式設計密碼逾期  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者透過新增已加入至 DBPROPSET_SQLSERVERDBINIT 屬性集的 SSPROP_AUTH_OLD_PASSWORD （類型 VT_BSTR）屬性，來支援密碼到期。  
...
### <a name="odbc-user-interface-password-expiration"></a>ODBC 使用者介面密碼逾期  
 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client ODBC 驅動程式透過對**SQL Server 登**入對話方塊所做的變更，支援密碼到期。  
 ...
 如果使用者按一下 [確定]，而且密碼已過期，[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 會使用 [**變更 SQL Server 密碼**] 對話方塊提示輸入並確認新密碼。  
