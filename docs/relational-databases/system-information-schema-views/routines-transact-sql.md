...
|DATA_TYPE|**Nvarchar （** 128 **）**|函數傳回值的資料類型。 如果資料表值函式，則傳回**資料表**。|  
|CHARACTER_MAXIMUM_LENGTH|**int**|傳回類型是字元類型時的最大長度 (以字元為單位)。<br /><br /> -1 適用于**xml**和大數數值型別資料。|  
|CHARACTER_OCTET_LENGTH|**int**|傳回類型是字元類型時的最大長度 (以位元組為單位)。<br /><br /> -1 適用于**xml**和大數數值型別資料。|  
