...
```  
 ...
 WQL 查詢的 `FROM` 子句中的引數 (`DDL_DATABASE_LEVEL_EVENTS`) 可以是能在其上建立事件通知的任何有效事件。 `SELECT` 和 `WHERE` 子句中的引數可以指定與事件或其上層事件相關的任何事件屬性。{3} 如需有效事件和事件屬性的清單，請參閱[事件通知（資料庫引擎）](https://technet.microsoft.com/library/ms182602.aspx)。  
...
```  
 ...
 如果 `SchemaName='X' AND ObjectType='Y' AND ObjectName='Z'` 全都在 `WHERE` 子句中指定，就會嘗試直接在架構 `X`的物件 `Z` 上註冊事件通知。 如果用戶端擁有權限，註冊就會成功。 請注意，目前只有佇列才支持對象層級事件，而且僅適用于 QUEUE_ACTI加值稅ION *event_type*。  
...
## <a name="see-also"></a>另請參閱  
 ...
 [事件通知（資料庫引擎）](https://technet.microsoft.com/library/ms182602.aspx)  
