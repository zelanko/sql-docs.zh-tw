...
description: 藉由發出 WMI 查詢語言語句，瞭解管理應用程式如何使用伺服器事件的 WMI 提供者來存取 SQL Server 事件。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
 ...
 *event_property*  
 這是事件的屬性。 範例包括 **PostTime**、 **SPID**和 **LoginName**。 查閱 [WMI 提供者中針對伺服器事件類別和屬性](../../relational-databases/wmi-provider-server-events/wmi-provider-for-server-events-classes-and-properties.md) 所列的每個事件，以判斷其所保留的屬性。 例如，DDL_DATABASE_LEVEL_EVENTS 事件會保存 **DatabaseName** **和使用者名稱屬性** 。 它也會從其父事件繼承 **SQLInstance**、 **LoginName**、 **PostTime**、 **SPID**和 **ComputerName** 屬性。  
  
 **,** *...n*  
 指出 *event_property* 可以查詢多次，並以逗號分隔。  
 ...
 *where_condition*  
 這是 WHERE 子句查詢述詞，由 *event_property* 名稱和邏輯和比較運算子組成。 *Where_condition*會決定在目標資料庫中註冊對應事件通知的範圍。 它也可以做為篩選準則，將目標設為要查詢 event_type 的特定架構或物件 *。* 如需詳細資訊，請參閱本主題稍後的＜備註＞一節。  
  
 只有 `=` 運算元可以與 **DatabaseName**、 **SchemaName**和 **ObjectName**一起使用。 其他運算式無法搭配這些事件屬性使用。  
  
## <a name="remarks"></a>備註  
 「伺服器事件的 WMI 提供者」語法的 *where_condition* 會決定下列各項：  
  
-   提供者嘗試取得指定之 *event_type*的範圍：伺服器層級、資料庫層級或物件層級 (目前唯一支援的物件是佇列) 。 這個範圍最終會判斷在目標資料庫中所建立之事件通知的類型。 這個程序稱為事件通知註冊。  
 ...
 *Where_condition*也可以做為篩選準則，以進一步限制特定資料庫、架構或物件的查詢。 例如，請看下列的 WQL 查詢：  
...
```  
 ...
 如果 `SchemaName='X' AND ObjectType='Y' AND ObjectName='Z'` 全部都在子句中指定 `WHERE` ，就會嘗試直接在架構的物件上註冊事件通知 `Z` `X` 。 如果用戶端擁有權限，註冊就會成功。 請注意，目前物件層級的事件僅在佇列上受到支援，而且僅適用于 QUEUE_ACTI加值稅ION *event_type*。  
