...
description: 了解針對與 SQL Server 中的磁碟資料表具有不同模式的記憶體內部 OLTP 記憶體耗用量進行監視及疑難排解。
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
...
-   [監視記憶體使用狀況](../../relational-databases/in-memory-oltp/monitor-and-troubleshoot-memory-usage.md#bkmk_Monitoring)  
    ...
    -   [使用 DMVs](../../relational-databases/in-memory-oltp/monitor-and-troubleshoot-memory-usage.md#bkmk_UsingDMVs)  
  
-   [管理記憶體最佳化物件耗用的記憶體](../../relational-databases/in-memory-oltp/monitor-and-troubleshoot-memory-usage.md#bkmk_MemOptObjects)  
...
3.  在操作功能表中，依序選取 [報表] -> [標準報表] -> [記憶體最佳化物件的記憶體使用量]  
 ...
 ![顯示 [物件總管] 的螢幕擷取畫面，其中已選取[報表] > [標準報表] > [依記憶體最佳化物件的記憶體使用量]。](../../relational-databases/in-memory-oltp/media/hk-mm-ssms-stdrpt-memuse.gif "HK_MM_SSMS")  
  
 此報表會顯示上面所建立資料庫的記憶體耗用量。  
  
 ![[依記憶體最佳化物件的總記憶體使用量] 報表的螢幕擷取畫面。](../../relational-databases/in-memory-oltp/media/hk-mm-ssms-stdrpt-memuserpt.gif "HK_MM_SSMS")  
