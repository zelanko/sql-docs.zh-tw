...
|**行**|**char (20) **|資料表現有的資料列數。 如果指定的物件是一個 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 佇列，這個資料行會指出佇列中的訊息數目。|  
 ...
 **database_size**通常會大於**保留**的  +  **未配置空間**的總和，因為它包含記錄檔的大小，但**保留**和**unallocated_space**只考慮資料頁面。 在 Azure Synapse Analytics 的某些情況下，此陳述可能不會是 true。 
 ...
 當指定 *updateusage* 時，會 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)] 掃描資料庫中的資料頁，並對 **sys. allocation_units** 和 **sys.** 分割區目錄的每個資料表所使用的儲存空間，進行任何必要的更正。 例如，在某些狀況下，在卸除索引之後，資料表的空間資訊可能不是目前的資訊。 *updateusage* 可能需要一些時間才能在大型資料表或資料庫上執行。 只有當您懷疑會傳回不正確的值，以及處理常式對資料庫中的其他使用者或進程不會造成負面影響時，才使用 *updateusage* 。 如果願意的話，您可以個別執行 DBCC UPDATEUSAGE。  
