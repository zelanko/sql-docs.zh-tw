...
ms.locfileid: "85720281"
---
# <a name="sp_execute_remote-azure-sql-database"></a>sp_execute_remote (Azure SQL Database)
[!INCLUDE[Azure SQL Database Azure SQL Managed Instance](../../includes/applies-to-version/asdb-asdbmi.md)]

  在 [!INCLUDE[tsql](../../includes/tsql-md.md)] 水準資料分割配置中，于單一遠端 Azure SQL Database 或一組資料庫上執行語句，做為分區。  
...
>  \@stmt 可以包含具有與變數名稱相同形式的參數，例如：`N'SELECT * FROM HumanResources.Employee WHERE EmployeeID = @IDParameter'`  
 ...
 [ \@ params =] N ' \@*parameter_name * * data_type* [,.。。*n* ] '  
 這是一個字串，其中包含已內嵌在 stmt 中之所有參數的定義 \@ 。此字串必須是 Unicode 常數或 Unicode 變數。 每個參數定義都由參數名稱和資料類型組成。 *n*是指出其他參數定義的預留位置。 Stmtmust 中指定的每個參數 \@ 都是在 params 中定義 \@ 。 如果 [!INCLUDE[tsql](../../includes/tsql-md.md)] stmt 中的語句或批次不 \@ 包含參數， \@ 則不需要 params。 這個參數的預設值是 NULL。  
...
在使用者資料庫中建立資料庫範圍認證，並指定 master 資料庫的系統管理員認證。 建立指向 master 資料庫的外部資料源，並指定資料庫範圍認證。 接著，在使用者資料庫中的範例會執行 `sp_set_firewall_rule` master 資料庫中的程式。 程式 `sp_set_firewall_rule` 需要3個參數，且 `@name` 參數必須為 Unicode。
