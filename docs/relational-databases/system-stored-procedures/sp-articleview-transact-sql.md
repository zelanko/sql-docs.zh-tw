...
`[ @force_reinit_subscription = ] _force_reinit_subscription_`認可這個預存程式所採取的動作可能需要重新初始化現有的訂閱。 *force_reinit_subscription*是**位**，預設值是**0**。  
...
## <a name="remarks"></a>備註  
 **sp_articleview**會建立定義已發佈發行項的視圖，並在[sysarticles &#40;transact-sql&#41;](../../relational-databases/system-tables/sysarticles-transact-sql.md)資料表的 [ **sync_objid** ] 資料行中插入此視圖的識別碼，然後在 [ **filter_clause** ] 資料行中插入限制子句的文字。 如果所有資料行都已複寫，而且沒有**filter_clause**，則[sysarticles &#40;transact-sql&#41;](../../relational-databases/system-tables/sysarticles-transact-sql.md)資料表中的**sync_objid**會設定為基表的識別碼，而且不需要使用**sp_articleview** 。  
  
 若要發行垂直篩選的資料表（也就是篩選資料行），請先執行不含*sync_object*參數的**sp_addarticle** ，執行[sp_articlecolumn &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articlecolumn-transact-sql.md)一次，以複寫每個資料行（定義垂直篩選），然後執行**sp_articleview**以建立定義已發行之發行項的視圖。  
  
 若要發行水準篩選的資料表（也就是篩選資料列），請執行 sp_addarticle，&#40;不含*篩選*參數的[transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md) 。 [Sp_articlefilter &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articlefilter-transact-sql.md)執行，並提供包含*filter_clause*在內的所有參數。 然後執行**sp_articleview**，並提供所有參數，包括相同的*filter_clause*。  
  
 若要發行垂直和水準篩選的資料表，請執行[sp_addarticle &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md) ，而不使用*sync_object*或*filter*參數。 針對要複寫的每個資料行執行[sp_articlecolumn &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articlecolumn-transact-sql.md)一次，然後執行[sp_articlefilter &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-articlefilter-transact-sql.md)和**sp_articleview**。  
  
 如果文章已經有定義已發行之文章的視圖， **sp_articleview**會卸載現有的視圖，並自動建立一個新的。 如果已手動建立視圖（在 sysarticles 中**輸入**， [&#40;transact-sql&#41;](../../relational-databases/system-tables/sysarticles-transact-sql.md)為**5**），則不會卸載現有的視圖。  
  
 如果您建立自訂篩選預存程式，以及手動定義已發行文章的視圖，請勿執行**sp_articleview**。 相反地，請提供這些做為*篩選準則*，並*sync_object*參數來[sp_addarticle &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md)以及適當的*類型*值。  
