...
若要成功`sp_enclave_send_keys`叫用，您必須連接到已啟用資料庫連接之 Always Encrypted 和記憶體保護區計算的資料庫。 您也必須能夠存取資料行主要金鑰，保護您要傳送的資料行加密金鑰，而且您需要存取資料庫中 Always Encrypted 金鑰中繼資料的許可權。 
...
## <a name="permissions"></a>權限
 ...
 需要資料庫`VIEW ANY COLUMN ENCRYPTION KEY DEFINITION`中`VIEW ANY COLUMN MASTER KEY DEFINITION`的和許可權。  
