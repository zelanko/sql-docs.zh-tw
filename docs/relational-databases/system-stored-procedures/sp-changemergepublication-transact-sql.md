...
## <a name="arguments"></a>引數  
 ...
 [  **@property=**] **'***屬性***'**  
 給定發行集要變更的屬性。 *屬性*是**sysname**，它可以其中一個值列出在後續的表格。  
  
 [  **@value=**] **'***值***'**  
 指定之屬性的新值。 *值*是**nvarchar （255)**，它可以其中一個值列出在後續的表格。  
...
|**conflict_retention**||**Int**所指定的保留期限，衝突保留的天數。 設定*conflict_retention*至**0**表示不需要清除任何衝突。|  
|**描述**||發行集的描述。|  
...
|NULL (預設值)||傳回支援的值清單*屬性*。|  
 ...
 [  **@force_invalidate_snapshot =** ] *force_invalidate_snapshot*  
 認可這個預存程序所採取的動作可能會使現有的快照集失效。 *force_invalidate_snapshot*是**元**，預設值是**0**。  
 ...
 **1**指定變更發行集，會導致現有的訂閱重新初始化，並提供發生訂閱重新初始化的權限。  
 ...
 **sp_changemergepublication**用於合併式複寫中。  
 ...
 變更下列屬性需要產生新的快照集。 您您必須指定值為**1**如*force_invalidate_snapshot*參數。  
...
-   **publication_compatibility_level** (至**80SP3**只)  
...
-   **use_partition_groups**  
 ...
 變更下列屬性需要重新初始化現有的訂閱。 您必須指定值為**1**如*force_reinit_subscription*參數。  
  
-   **dynamic_filters**  
  
-   **validate_subscriber_info**  
  
 清單發行集物件到 Active Directory 使用*publish_to_active_directory*、[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]必須已在 Active Directory 中建立物件。  
