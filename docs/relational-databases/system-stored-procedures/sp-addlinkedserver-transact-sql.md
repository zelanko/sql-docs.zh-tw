...
> [!NOTE]
> 使用 SQLNCLI 會將 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 重新導向至 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者的最新版本。 OLE DB 提供者預期會登錄在登錄中指定的 PROGID。
...
[@provstr =] *\'provider_string\'*           
 ...
 針對 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者建立連結的伺服器時，可以使用 SERVER =*servername*\\*instancename*來指定實例，以指定 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]的特定實例。 *servername*是執行 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 之電腦的名稱，而*instancename*是使用者將連接之特定 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 實例的名稱。  
  
> [!NOTE]
> 若要存取鏡像資料庫，連接字串必須包含資料庫名稱。 這個名稱是讓資料存取提供者進行容錯移轉嘗試所需的名稱。 您可以在 **\@provstr**或 **\@目錄**參數中指定資料庫。 此外，連接字串也可以提供容錯移轉夥伴名稱。  
  
[@catalog =] *\'目錄\'*        
 這是連接 OLE DB 提供者時所用的目錄。 *目錄*是**sysname**，預設值是 Null。 *目錄*會當做 DBPROP_INIT_CATALOG 屬性傳遞，以初始化 OLE DB 提供者。 當您對 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的執行個體定義連結伺服器時，目錄會參考連結伺服器所對應的預設資料庫。  
...
## <a name="remarks"></a>Remarks  
 下表所顯示的，是針對可以透過 OLE DB 來存取的資料來源，設定連結伺服器的方法。 您可以對一個特定的資料來源，用一個以上的方法來設定連結伺服器；一個資料來源類型可以有一個以上的資料列。 下表也會顯示用於設定連結伺服器的**sp_addlinkedserver**參數值。  
...
|Oracle 第 8 版和更新的版本|Oracle OLE DB 提供者|任意|**Oraoledb.oracle Oracle**|Oracle 資料庫的別名||||  
|Access/Jet|Microsoft OLE DB Provider for Jet|任意|**Microsoft Jet. v4。0**|Jet 資料庫檔案的完整路徑||||  
...
|[!INCLUDE[msCoName](../../includes/msconame-md.md)] Excel 試算表|[!INCLUDE[msCoName](../../includes/msconame-md.md)] OLE DB Provider for Jet|任意|**Microsoft Jet. v4。0**|Excel 檔的完整路徑||Excel 5。0||  
|IBM DB2 資料庫|[!INCLUDE[msCoName](../../includes/msconame-md.md)] OLE DB Provider for DB2|任意|**DB2OLEDB**|||請參閱 [!INCLUDE[msCoName](../../includes/msconame-md.md)] OLE DB Provider for DB2 檔。|DB2 資料庫的目錄名稱|  
  
 <sup>1</sup>這種設定連結伺服器的方式會強制連結伺服器的名稱與 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]遠端實例的網路名稱相同。 使用*data_source*來指定伺服器。  
  
 <sup>2</sup> "Any" 表示產品名稱可以是任何專案。  
  
 如果未指定提供者名稱，或將 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 指定為產品名稱，則 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者就是與 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 搭配使用的提供者。 即使您指定較早的提供者名稱 SQLOLEDB，它也會在保存到目錄時，改為 SQLNCLI。  
  
 [ *Data_source*]、[*位置*]、[ *provider_string*] 和 [*目錄*] 參數會識別連結伺服器指向的資料庫或資料庫。 如果這些參數有任何一個是 NULL，就不會設定對應的 OLE DB 初始化屬性。  
  
 在群集環境中，當您指定讓檔名指向 OLE DB 資料來源時，請使用通用命名慣例名稱 (UNC) 或共用磁碟機來指定位置。  
  
 **sp_addlinkedserver**不能在使用者自訂交易內執行。  
  
> [!IMPORTANT]
> 使用**sp_addlinkedserver**建立連結的伺服器時，會為所有本機登入新增預設的自我對應。 若為非 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 提供者，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 驗證的登入可能可以在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 服務帳戶下取得提供者的存取權。 管理員應該考慮使用 `sp_droplinkedsrvlogin <linkedserver_name>, NULL` 來移除全域對應。  
  
## <a name="permissions"></a>Permissions  
 `sp_addlinkedserver` 語句需要 `ALTER ANY LINKED SERVER` 許可權。 （[[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]**新增連結伺服器**] 對話方塊會以需要 `sysadmin` 固定伺服器角色成員資格的方式來執行）。  
...
```  
 ...
 下列範例會使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者，在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的實例上建立連結的伺服器 `S1_instance1`。  
...
> [!NOTE]  
> 這個範例假設已安裝 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 存取和範例**northwind**資料庫，而且**northwind**資料庫位於 C:\Msoffice\Access\Samples。  
...
### <a name="g-add-a-includesssdsfullincludessssdsfull-mdmd-as-a-linked-server-for-use-with-distributed-queries-on-cloud-and-on-premises-databases"></a>G. 新增 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] 做為連結的伺服器，以便與雲端和內部部署資料庫上的分散式查詢搭配使用  
 ...
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] box 產品包含分散式查詢功能，可讓您撰寫查詢，以結合來自本機資料來源的資料和遠端來源的資料（包括來自訂為連結伺服器之非 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料來源的資料）。 您將能加入每個 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] (虛擬 master 除外) 成為個別的連結伺服器，然後直接供您的資料庫應用程式使用，與任何其他資料庫無異。  
  
 使用 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]的好處包括管理能力、高可用性、延展性、有您熟悉的開發模型和關聯式資料模型。 您的資料庫應用程式的需求決定了該如何在雲端使用 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]。 您可以一次將所有資料移到 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]，或是逐漸搬移部分資料並將其餘資料留在內部部署環境。 針對這類混合式資料庫應用程式，現在可以將 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] 新增為連結的伺服器，而且資料庫應用程式可以發出分散式查詢，以結合來自 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] 與內部部署資料來源的資料。  
  
 以下是一個簡單的範例，說明如何使用分散式查詢連接到 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]：  
