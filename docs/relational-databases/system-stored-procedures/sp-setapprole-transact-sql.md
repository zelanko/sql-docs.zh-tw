...
## <a name="remarks"></a>備註
 ...
 使用 **sp_setapprole**啟動應用程式角色之後，角色會保持作用中，直到使用者中斷與伺服器的連線或執行 **sp_unsetapprole**為止。 **sp_setapprole** 只能由直接 [!INCLUDE[tsql](../../includes/tsql-md.md)] 語句執行。 **sp_setapprole** 不能在另一個預存程式內執行，或在使用者定義的交易內執行。  
