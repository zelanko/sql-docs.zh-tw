...
## <a name="arguments"></a>引數  
 ...
 [  **@article=**] **'***文章***'**  
 這是發行集的發行項名稱。 *發行項*已**sysname**，預設值是**%**。 如果*文章*是未提供，會傳回指定之發行集的所有發行項的相關資訊。  
  
 [  **@returnfilter=**] *returnfilter*  
 指定是否應該傳回篩選子句。 *returnfilter*已**位元**，預設值是**1**，它會傳回篩選子句。  
  
 [ **@publisher**=] **'***發行者***'**  
 指定非[!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]發行者。 *發行者*已**sysname**，預設值是 NULL。  
  
> [!NOTE]  
>  *發行者*不應指定當要求資訊的發行項上發佈的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]發行者。  
...
|**status**|**tinyint**|可以是[& (Bitwise AND)](../../t-sql/language-elements/bitwise-and-transact-sql.md)一或多個這些發行項屬性的結果：<br /><br /> **0x00** = [!INCLUDE[ssInternalOnly](../../includes/ssinternalonly-md.md)]<br /><br /> **0x01** = 發行項在使用。<br /><br /> **0x08** = 包含 insert 陳述式中的資料行名稱。<br /><br /> **0x16** = 使用參數化陳述式。<br /><br /> **0x32** = 使用參數化陳述式和 insert 陳述式中包含的資料行名稱。|  
...
|**schema_option**|**binary(8)**|給定發行項的結構描述產生選項點陣圖。 如需完整的清單**schema_option**值，請參閱[sp_addarticle &#40;-&#41;](../../relational-databases/system-stored-procedures/sp-addarticle-transact-sql.md)。|  
...
|**publisher_identity_range**|**int**|如果發行項的範圍在發行者端之識別範圍的大小*identityrangemanagementoption*設為**自動**或是**auto_identity_range**設為**true**。|  
|**identity_range**|**bigint**|如果發行項有範圍的訂閱者端的識別範圍大小*identityrangemanagementoption*設為**自動**或是**auto_identity_range**設為**true**。|  
|**threshold**|**bigint**|這是一個百分比值，指出散發代理程式指派新識別範圍的時機。|  
|**identityrangemanagementoption**|**int**|指出處理發行項的識別範圍管理。|  
|**fire_triggers_on_snapshot**|**bit**|這是指在套用初始快照集時，是否執行複寫的使用者觸發程序。<br /><br /> **1** = 執行觸發程序的使用者。<br /><br /> **0** = 不會執行觸發程序的使用者。|  
...
## <a name="permissions"></a>Permissions  
 只有成員**sysadmin**固定伺服器角色**db_owner**固定的資料庫角色或目前發行集之發行集存取清單能夠執行**sp_helparticle**.  
