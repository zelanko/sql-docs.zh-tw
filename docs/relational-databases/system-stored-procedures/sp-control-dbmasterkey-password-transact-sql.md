 ...
 **可能的回溯相容性問題：** 目前預存程序不會檢查主要金鑰是否存在。 這可允許回溯相容性，但是會顯示警告。 這個行為已被取代。 在未來版本中，主要金鑰必須存在和預存程序中使用的密碼**sp_control_dbmasterkey_password**必須是相同的密碼，做為其中一個用來加密資料庫主要金鑰的密碼。  
