...
`[ @name = ] 'name'`這是您想要重新命名的原則類別目錄名稱。 *名稱*是**sysname**，而且如果*policy_category_id*是 Null，就必須指定。  
  
`[ @policy_category_id = ] policy_category_id`這是您想要重新命名之原則類別目錄的識別碼。 *policy_category_id*是**int**，而且如果*name*為 Null，就必須指定。  
  
`[ @new_name = ] 'new_name'`這是原則類別目錄的新名稱。 *new_name*是**sysname**，而且是必要的。 不得為 NULL 或空字串。  
