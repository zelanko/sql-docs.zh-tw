...
`[ @disconnect_users = ] 'disconnect_users'` 如果設定為1，則在執行還原作業時，使用者會與次要資料庫中斷連線。 預設值 = 0。 *中斷* 使用者的連接是 **bit** ，不能是 Null。  
...
`[ @threshold_alert = ] 'threshold_alert'` 這是超出備份臨界值時要引發的警示。 *threshold_alert* 是 **int**，預設值是14420。  
  
`[ @threshold_alert_enabled = ] 'threshold_alert_enabled'` 指定當超出 *backup_threshold* 時是否引發警示。 預設值 1 表示產生警示。 *threshold_alert_enabled* 是 **bit**。  
...
1.  **sp_add_log_shipping_secondary_primary** 應該在此預存程式之前呼叫，以初始化次要伺服器上的主要記錄傳送資料庫資訊。  
  
2.  使用提供的引數，在 **log_shipping_secondary_databases** 中新增次要資料庫的專案。  
  
3.  使用提供的引數，在次要伺服器的 **log_shipping_monitor_secondary** 中加入本機監視器記錄。  
  
4.  如果監視伺服器不同于次要伺服器，請使用提供的引數，在監視伺服器的 **log_shipping_monitor_secondary** 中加入監視記錄。  
  
## <a name="permissions"></a>權限  
 只有 **系統管理員（sysadmin** ）固定伺服器角色的成員，才能夠執行這個程式。  
  
## <a name="examples"></a>範例  
 此範例將說明如何使用 **sp_add_log_shipping_secondary_database** 預存程式，將資料庫 **LogShipAdventureWorks** 新增為記錄傳送設定中的次要資料庫，以及主資料庫 [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] 位於主伺服器 tribeca 有上的主資料庫。  
