...
`[ @file_retention_period = ] 'file_retention_period'` 這是將保留記錄的時間長度（以分鐘為單位）。 *history_retention_period* 是 **int**，預設值是 Null。 若未指定，則使用 14420。  
...
2.  如果監視伺服器不同于次要伺服器，請視需要使用提供的引數，變更監視伺服器上 **log_shipping_monitor_secondary** 的監視器記錄。  
