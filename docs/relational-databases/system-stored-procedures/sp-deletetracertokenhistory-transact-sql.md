...
這是追蹤 Token 插入其中之發行集的名稱。 資料類型為**sysname**。 此參數為必要。

`[ @tracer_id= ] tracer_id`  
這是要刪除之追蹤 Token 的識別碼。 資料類型為**int**。預設值是 *null*。 如果是*null*, 則會刪除屬於發行集的所有追蹤 token。

`[ @cutoff_date= ] cutoff_date`  
在刪除此日期之前, 插入發行集的追蹤 token。 資料類型為**datetime**。 預設值是 *null*。

`[ @publisher= ] 'publisher'`  
這是發行者的名稱。 資料類型為**sysname**。 預設值是 *null*。
...
如果您同時指定*tracer_id*和*cutoff_date*參數, 就會發生錯誤。

如果您未執行**sp_deletetracertokenhistory**來刪除追蹤 token 中繼資料, 則會在定期排程的記錄清除發生時刪除資訊。
...
只有下列人員具有執行**sp_deletetracertokenhistory**的許可權:
