...
`[ @rowcount_only = ] type_of_check_requested`指定是否只傳回資料表的資料列計數。 *type_of_check_requested*是**Smallint**, 預設值是**1**。  
 ...
 如果是**0**, 則執行資料列[!INCLUDE[msCoName](../../includes/msconame-md.md)]計數和7.0 相容的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]總和檢查碼。  
  
 如果是**1**, 則只執行資料列計數檢查。  
  
 如果為**2**, 則執行資料列計數和二進位總和檢查碼。  
  
`[ @full_or_fast = ] full_or_fast`這是用來計算資料列計數的方法。 *full_or_fast*是**Tinyint**, 它可以是下列其中一個值。  
...
`[ @subscription_level = ] subscription_level`指定是否由一組訂閱者挑選驗證。 *subscription_level*是**bit**, 預設值是**0**。 如果是**0**, 則會將驗證套用至所有訂閱者。 如果是**1**, 驗證只會套用至目前開啟的交易中呼叫**sp_marksubscriptionvalidation**所指定之訂閱者的子集。  
...
## <a name="permissions"></a>Permissions  
 只有在要驗證之發行項的來源資料表上具有 [全選] 許可權的使用者可以執行**sp_article_validation**。  
