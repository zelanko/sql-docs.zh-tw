...
[!INCLUDE[tsql-appliesto-xxxxxx-xxxx-asdw-pdw-md](../../includes/tsql-appliesto-xxxxxx-xxxx-asdw-pdw-md.md)]
  ...
  這會將網路認證儲存在 [!INCLUDE[ssSDW](../../includes/sssdw-md.md)] 中，並將它們與伺服器產生關聯。 例如，您可以使用這個預存程式，授與 [!INCLUDE[ssSDW](../../includes/sssdw-md.md)] 適當的讀取/寫入權限，以便在目標伺服器上執行資料庫備份和還原作業，或建立用於 TDE 之憑證的備份。  
...
## <a name="general-remarks"></a>一般備註  
 這個預存程式會將網路認證新增到 [!INCLUDE[ssSDW](../../includes/sssdw-md.md)]的 NetworkService 帳戶。 NetworkService 帳戶會在控制節點和計算節點上執行 SMP [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的每個實例。 例如，當備份作業執行時，控制節點和每個計算節點都會使用 NetworkService 帳號憑證來取得目標伺服器的讀取和寫入權限。  
