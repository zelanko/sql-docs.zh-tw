...
`[ @optname = ] 'option_name'` 這是要為指定的伺服器設定的選項。 *option_name* 是 **Varchar (** 35 **) **，沒有預設值。 *option_name* 可以是下列值之一。  
...
|**使用遠端定序**|決定將使用遠端資料行或本機伺服器的定序。<br /><br /> 若為 **true**，則會使用遠端資料行的定序作為 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料來源，而定 **序名稱** 中指定的定序會用於非 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料來源。<br /><br /> 若 **為 false**，分散式查詢一律會使用本機伺服器的預設定序，而會忽略定 **序名稱** 和遠端資料行的定序。 預設值為 **false**。  (**false** 值與7.0 中使用的定序語義相容 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ) 。|  
...
`[ @optvalue = ] 'option_value'` 指定 *option_name* 是否應啟用 (**TRUE** **或) 或** 停用 (**FALSE** 或 **off**) 。 *option_value* 是 **Varchar (** 10 **) **，沒有預設值。  
  
 *option_value* 可能是 **連接逾時** 和 **查詢超時** 選項的非負整數。 若為 [定 **序名稱]** 選項， *option_value* 可以是定序名稱或 Null。  
  
## <a name="return-code-values"></a>傳回碼值  
 0 (成功) 或 1 (失敗)  
  
## <a name="remarks"></a>備註  
 如果 [定 **序相容]** 選項設定為 [TRUE]，定 **序名稱** 會自動設為 Null。 如果定 **序名稱** 設定為非 null 值，則 **與自動相容** 的定序將設定為 FALSE。  
