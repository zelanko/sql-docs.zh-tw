 ...
 **Bounding_Box_xmin**  
 **Geometry**類型空間索引的 X 最小周框方塊屬性。 **Geography**類型的這個屬性值是 Null。 值為 **float**。  
  
 **Bounding_Box_ymin**  
 **幾何**類型空間索引的 Y 最小周框方塊屬性。 **Geography**類型的這個屬性值是 Null。 值為 **float**。  
  
 **Bounding_Box_xmax**  
 **Geometry**類型空間索引的 X-最大周框方塊屬性。 **Geography**類型的這個屬性值是 Null。 值為 **float**。  
  
 **Bounding_Box_ymax**  
 **幾何**類型空間索引的 Y 軸最大周框方塊屬性。 **Geography**類型的這個屬性值是 Null。 值為 **float**。  
 ...
 **Total_Number_Of_ObjectCells_In_Level0_For_QuerySample**  
 指出代表性查詢範例是否落在 **幾何** 索引的周框方塊之外，以及 (層級0資料格) 的根儲存格。 這可能是 0 (不在層級 0 資料格中) 或 1。 如果是在層級 0 資料格中，則調查的索引不是查詢範例的適當索引。 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_ObjectCells_In_Level0_In_Index**  
 鑲嵌于層級 0 (根儲存格的索引物件的資料格實例數目，位於 **幾何**) 的周框方塊之外。 這是核心屬性。 值為 **Bigint**。  
  
 針對 **幾何** 索引，如果索引的周框方塊小於資料網域，就會發生這種情況。 如果查詢視窗落在周框方塊外部，而且會降低索引效能 (例如， **Total_Number_Of_ObjectCells_In_Level0_For_QuerySample** 為 1) ，則層級0中的大量物件可能會需要次要篩選。 如果查詢視窗落在週框方塊內，則層級 0 的物件數目如果很多，反而可能會改善索引的效能。  
  
 NULL 和空白的執行個體會計算層級 0 的數目，但不會影響效能。 層級 0 的資料格會與基底資料表的 NULL 和空白執行個體一樣多。 針對 **地理位置** 索引，層級0的資料格數目會是 Null 和空實例 + 1 個數據格，因為查詢範例會計算為1。  
  
 **Total_Number_Of_ObjectCells_In_Level1_In_Index**  
 以層級1精確度鑲嵌的索引物件的資料格實例數目。 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_ObjectCells_In_Level2_In_Index**  
 以層級2精確度鑲嵌的索引物件的資料格實例數目。 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_ObjectCells_In_Level3_In_Index**  
 以層級3有效位數鑲嵌的索引物件的資料格實例數目。 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_ObjectCells_In_Level4_In_Index**  
 鑲嵌為層級 4 精度的索引物件的資料格執行個體數目。 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_interior_ObjectCells_In_Level1_In_Index**  
 在鑲嵌層級1的物件完全涵蓋的資料格數目，因此是物件的內部。  (Cell_attributevalue 為2。 ) 這是核心屬性。 值為 **Bigint**。  
 ...
 **Total_Number_Of_intersecting_ObjectCells_In_Level1_In_Index**  
 在鑲嵌層級1以物件交集的資料格數目。  (Cell_attribute 值為1。 ) 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_intersecting_ObjectCells_In_Level2_In_Index**  
 在鑲嵌層級2的物件交集的資料格數目。  (Cell_attribute 值為1。 ) 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_intersecting_ObjectCells_In_Level3_In_Index**  
 在鑲嵌層級3以物件交集的資料格數目。  (Cell_attribute 值為1。 ) 這是核心屬性。 值為 **Bigint**。  
 ...
 **Total_Number_Of_Border_ObjectCells_In_Level1_In_Index**  
 在鑲嵌層級1上接觸方格資料格界限的物件儲存格數目。  (Cell_attribute 值為0。 ) 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_Border_ObjectCells_In_Level2_In_Index**  
 在鑲嵌式層級2觸控方格資料格界限的物件儲存格數目。  (Cell_attribute 值為0。 ) 這是核心屬性。 值為 **Bigint**。  
  
 **Total_Number_Of_Border_ObjectCells_In_Level3_In_Index**  
 在鑲嵌層級3觸控方格資料格界限的物件儲存格數目。  (Cell_attribute 值為0。 ) 這是核心屬性。 值為 **Bigint**。  
 ...
 **Intersecting_To_Total_Cells_Normalized_To_Leaf_Grid_Percentage**  
 與 **Interior_To_Total_Cells_Normalized_To_Leaf_Grid_Percentage**相同，不同之處在于這些是部分涵蓋的儲存格。 值為 **float**。  
 ...
 **Primary_Filter_Efficiency**  
 如果主要篩選選取了 P 個數據列，而 O 是輸出的資料列數目，則這個 returnsO/P 會以百分比表示。 主要篩選的效率愈高，次要篩選必須處理的誤判愈少。 這是核心屬性。 值為 **float。**  
