 ...
 *cursor*  
 SQL Server 產生的資料指標識別碼。 資料*指標*是一個必須在所有牽涉到資料指標的後續程式上提供的*控制碼*值，例如 sp_cursorfetch。 *cursor*是具有**int**傳回值的必要參數。  
  
 *cursor*允許單一資料庫連接上有多個資料指標為使用中狀態。  
  
 *把*  
 這是定義資料指標結果集的必要參數。 任何字串類型的任何有效查詢字串（語法和系結，不論 Unicode、大小等等為何）都可以做為有效的*stmt*數值型別。  
...
|0x100000|FAST_FORWARD_ACCEPTABLE|  
 ...
 由於要求的值不適合*stmt*所定義的資料指標，因此這個參數會同時做為輸入和輸出。 在這類情況下，SQL Server 會指派適合的值。  
...
|當做輸入值|當做傳回值|  
|--------------------|---------------------|  
|當指定 AUTO_FETCH *scrollopt*值時， *rowcount*代表要放入提取緩衝區的資料列數目。<br /><br /> 注意：當指定 AUTO_FETCH 時，>0 是有效的值，但會被忽略。|表示結果集中的資料列數目，但在指定*scrollopt* AUTO_FETCH 值時除外。|  
...
## <a name="return-code-values"></a>傳回碼值  
 ...
 -n  
 當非同步擴展在作用中時傳回。 表示在指定*scrollopt* AUTO_FETCH 值時，放入提取緩衝區中的資料列數目。  
...
> [!NOTE]  
>  如果 sp_cursoropen 程式成功執行，則會傳送具有 TDS 資料行格式資訊（0xa0 & 0xa1 訊息）的 RPC 傳回參數和結果集。 如果不成功，則會傳送一個或多個 TDS 錯誤訊息。 不論是哪一種情況，都不會傳回任何資料列資料，而*完成*的訊息計數將會是零。 如果您正在使用早於 7.0 的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 版本，則會傳回 0xa0、0xa1 (SELECT 陳述式的標準) 連同 0xa5 和 0xa4 Token 資料流。 如果您正在使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 7.0，則會傳回 0x81 (SELECT 陳述式的標準) 連同 0xa5 和 0xa4 Token 資料流。  
  
## <a name="remarks"></a>備註  
  
## <a name="stmt-parameter"></a>stmt 參數  
 如果*stmt*指定預存程式的執行，則輸入參數可能會定義為包含在*stmt*字串中的常數，或指定為*boundparam*引數。 宣告的變數可依照這種方式傳遞為繫結參數。  
...
## <a name="scrollopt-parameter"></a>scrollopt 參數  
 前五個*scrollopt*值（KEYSEY、DYNAMIC、FORWARD_ONLY、STATIC 和 FAST_FORWARD）是互斥的。  
...
## <a name="ccopt-parameter"></a>ccopt 參數  
 前四個*ccopt*值（READ_ONLY、SCROLL_LOCKS 和兩個開放式值）是互斥的。  
  
> [!NOTE]  
>  選擇前四個*ccopt*值的其中一個，會指出資料指標是否為唯讀，或是否使用鎖定或開放式方法來防止遺失更新。 如果未指定*ccopt*值，則預設值為開放式。  
 ...
 如果 CHECK_ACCEPTED_TYPES 是 ON，則最後四個*ccopt*值（READ_ONLY_ACCEPTABLE、SCROLL_LOCKS_ACCEPTABLE 和其中一個 OPTIMISTIC_ACCEPTABLE 值）中至少必須有一個。  
  
 定位的 UPDATE 和 DELETE 函式只能在提取緩衝區內執行，而且只有在*ccopt*值等於 SCROLL_LOCKS 或開放式時才會執行。 如果 SCROLL_LOCKS 是指定的值，則保證此作業一定成功。 如果 OPTIMISTIC 是指定的值，則當資料列在上一次提取之後已經變更時，作業將會失敗。  
...
## <a name="bound_param-parameter"></a>bound_param 參數  
 根據程式碼中的錯誤訊息，指定 PARAMETERIZED_STMT 時，應該*paramdef*參數名稱。 如果未指定 PARAMETERIZED_STMT，錯誤訊息中不會指定任何名稱。  
