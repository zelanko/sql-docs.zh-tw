...
`[ @frequency_interval = ] frequency_interval` 這是套用至 *frequency_type*所設定之頻率的值。 *frequency_interval* 是 **int**，預設值是 **1**。  
...
## <a name="permissions"></a>權限  
 只有 **系統管理員（sysadmin** ）固定伺服器角色的成員，才可以執行 **sp_changesubscriber_schedule**。  
