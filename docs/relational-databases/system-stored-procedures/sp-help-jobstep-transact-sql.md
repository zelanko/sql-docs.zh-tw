...
`[ @suffix = ] suffix` 旗標，指出文字描述是否附加至輸出中的 **旗標** 資料行。 *尾碼*是 **bit**，預設值是 **0**。 如果 *尾碼* 為 **1**，則會附加描述。  
...
|**output_file_name**|**nvarchar(200)**|應 ([!INCLUDE[tsql](../../includes/tsql-md.md)] 、 **CmdExec**和 **PowerShell** 步驟寫入命令輸出的檔案) 。|  
 ...
 **SQLAgentUserRole**的成員只能看到他們所擁有之作業的作業步驟。  
