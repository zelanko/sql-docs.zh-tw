...
`[ @continuous ] = continuous_` 指出 capture 作業是連續執行（1），還是只執行一次（0）。 「*連續*」是**bit** ，預設值是 Null，表示沒有此參數的變更。  
 ...
 如果 **\@連續** 從1變更為0， **\@pollinginterval**會自動設定為0。 已忽略為0以外的 **\@pollinginterval**所指定的值。  
  
 如果 **\@連續** 省略或明確設定為 Null，且 **\@pollinginterval**明確設定為大於0的值， **\@連續**會自動設為1。  
  
 「*連續*」只對「捕獲」作業有效。  
  
在記錄檔掃描迴圈之間 `[ @pollinginterval ] = polling_interval_` 秒數。 *polling_interval*是**Bigint** ，預設值是 Null，表示沒有此參數的變更。  
...
### <a name="a-changing-a-capture-job"></a>A. 變更擷取作業  
 下列範例會更新 `AdventureWorks2012` 資料庫中 capture 作業的 `@job_type`、`@maxscans`和 `@maxtrans` 參數。 擷取作業其他有效的參數 (`@continuous` 和 `@pollinginterval`) 被省略，而且其值不會進行修改。  
...
### <a name="b-changing-a-cleanup-job"></a>b. 變更清除作業  
 下列範例會在 `AdventureWorks2012` 資料庫中更新清除作業。 系統會指定此作業類型的所有有效參數（ **\@臨界值**除外）。 不會修改 **\@閾值**的值。  
