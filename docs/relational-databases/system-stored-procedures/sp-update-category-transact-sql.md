...
`[ @class = ] 'class'` 要更新的類別目錄類別。 *類別*已**varchar(8)**，沒有預設值，它可以是下列值之一。  
...
`[ @name = ] 'old_name'` 類別目錄的目前名稱。 *old_name&lt*已**sysname**，沒有預設值。  
  
`[ @new_name = ] 'new_name'` 類別的的新名稱。 *new_name*已**sysname**，沒有預設值。  
