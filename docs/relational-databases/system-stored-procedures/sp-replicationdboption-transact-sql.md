 ...
 **sp_replicationdboption**用於快照式複寫、異動複寫和合併式複寫中。  
 ...
 隨著給定的選項而不同，這個程序會建立或卸除特定複寫系統資料表、安全性帳戶等。 在**master 資料庫**系統資料表中設定對應的**is_published** （transacational 或快照式複寫）、 **is_merge_published** （合併式複寫）或**is_distributor**位，並建立必要的系統多.  
