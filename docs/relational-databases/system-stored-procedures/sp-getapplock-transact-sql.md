...
## <a name="remarks"></a>Remarks  
 資源所設定的鎖定與目前交易或目前工作階段相關聯。 當認可或回復交易時，便會釋放與目前交易相關聯的鎖定。 當會話已登出時，就會釋放與會話相關聯的鎖定。當伺服器因任何原因而關閉時，就會釋放所有鎖定。  
...
-   @Resource 參數中指定的鎖定名稱。  
 ...
 您可以利用 sp_releaseapplock 來明確釋放鎖定。 當應用程式針對相同鎖定資源來重複呼叫 sp_getapplock 時，也必須呼叫 sp_releaseapplock 相同次數，以便釋出鎖定。  當使用 `Transaction` 鎖定擁有者開啟鎖定時，會在認可或回復交易時釋放該鎖定。
