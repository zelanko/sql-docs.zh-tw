 ...
 *propertyvalue* **輸出**  
 ...
 如果屬性傳回 OLE 物件，則 *propertyvalue* 必須是資料類型 **int**的本機變數。物件標記會儲存在本機變數中，而這個物件標記可以與其他 OLE Automation 預存程式一起使用。  
  
 如果屬性傳回單一值，請指定 *propertyvalue*的本機變數，這會在本機變數中傳回屬性值;或不指定 *propertyvalue*，這會將屬性值以單一資料行、單一資料列結果集的形式傳回給用戶端。  
  
 當屬性傳回陣列時，如果指定了 *propertyvalue* ，則會將它設定為 Null。  
...
### <a name="b-using-a-result-set"></a>B. 使用結果集  
 下列範例 `HostName` 會取得先前建立之 **SQLServer** 物件的屬性 () ，並將它以結果集的形式傳回至用戶端。  
