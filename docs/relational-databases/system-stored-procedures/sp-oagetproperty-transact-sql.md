---
title: sp_OAGetProperty (TRANSACT-SQL) |Microsoft 文件
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
ms.author: edmaca
manager: craigg
ms.openlocfilehash: 9597d72448c710b47b055f524b31f3f84a02e274
 ...
 *propertyvalue* **輸出**  
 ...
 如果屬性傳回 OLE 物件， *propertyvalue*必須是資料類型的本機變數**int**。物件 Token 儲存在本機變數中，這個物件 Token 可以搭配其他 OLE Automation 預存程序來使用。  
  
 如果屬性傳回單一值，也可以指定一個本機變數*propertyvalue*，它會傳回屬性值在本機變數; 或不指定*propertyvalue*，它會傳回用戶端，做為單一資料行、 單一資料列結果集的屬性值。  
  
 當屬性傳回陣列，如果*propertyvalue*指定，則設定為 NULL。  
  
 如果*propertyvalue*指定，但屬性未傳回值，就會發生錯誤。 如果屬性傳回含有多於二個維度的陣列，就會發生錯誤。  
...
### <a name="b-using-a-result-set"></a>B. 使用結果集  
 下列範例會取得`HostName`屬性 (先前建立**SQLServer**物件)，並傳回用戶端，當作結果集。  
