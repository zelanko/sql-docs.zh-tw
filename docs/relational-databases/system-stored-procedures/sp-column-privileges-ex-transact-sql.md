...
|**授權**|**sysname**|已將此**COLUMN_NAME**的許可權授**與列出之**被授與者的資料庫使用者名稱。 在中 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，這個資料行一律與**TABLE_OWNER**相同。 這個欄位一律會傳回值。<br /><br /> 「**授與者」資料行可以**是資料庫擁有者（**TABLE_OWNER**），或資料庫擁有者利用 GRANT 語句中之 WITH GRANT OPTION 子句來授與許可權的使用者。|  
