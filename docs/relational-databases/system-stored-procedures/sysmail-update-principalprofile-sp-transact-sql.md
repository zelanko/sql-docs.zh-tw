...
`[ @is_default = ] 'is_default'` 這是此設定檔是否為資料庫使用者的預設設定檔。 資料庫使用者只能有一個預設設定檔。 *is_default* 是 **bit**，沒有預設值。  
...
## <a name="remarks"></a>備註  
 ...
 當關聯的主體名稱為 **public** 或關聯的主體識別碼為 **0**時，此預存程式就會變更公用設定檔。 只能有一個預設公用設定檔。  
  
 當** \@ is_default**為 '**1**'，且主體與一個以上的設定檔相關聯時，指定的設定檔會成為主體的預設設定檔。 先前是預設設定檔的設定檔仍會關聯於這個主體，但已不再是預設設定檔。  
  
 預存程式 **sysmail_update_principalprofile_sp** 位於 **msdb** 資料庫中，而且是由 **dbo** 架構所擁有。 如果目前的資料庫不是 **msdb**，就必須以三部分名稱執行程式。  
