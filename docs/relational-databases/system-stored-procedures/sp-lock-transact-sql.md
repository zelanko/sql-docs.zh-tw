...
> [!IMPORTANT]  
> [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] 若要取得 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]中鎖定的相關資訊，請使用[sys.databases dm_tran_locks](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md)動態管理檢視。  
...
`[ @spid1 = ] 'session ID1'` 是來自**sys. dm_exec_sessions**的 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 會話識別碼，使用者想要鎖定資訊。 *SESSION ID1*是**int** ，預設值是 Null。 執行**sp_who**來取得會話的相關處理資訊。 如果未指定*SESSION ID1* ，則會顯示所有鎖定的相關資訊。  
  
`[ @spid2 = ] 'session ID2'` 是**sys. dm_exec_sessions**中的另一個 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 會話識別碼，可能會有與*會話 ID1*相同的時間，以及使用者也想要的資訊。 *SESSION ID2*是**int** ，預設值是 Null。  
  
## <a name="return-code-values"></a>傳回碼值  
 0 (成功)  
  
## <a name="result-sets"></a>結果集  
 **Sp_lock**的結果集會針對 **\@spid1**和 **\@spid2**參數中指定的會話所持有的每個鎖定，各包含一個資料列。 如果未指定 **\@spid1**或 **\@spid2** ，則結果集會報告 [!INCLUDE[ssDE](../../includes/ssde-md.md)]實例中目前作用中的所有會話的鎖定。  
