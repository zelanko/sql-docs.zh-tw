---
title: sp_lock & Amp;#40;transact-SQL&AMP;#41; |Microsoft Docs
ms.custom: ''
ms.date: 03/17/2017
ms.prod: sql
...
## <a name="arguments"></a>引數  
 [  **@spid1 =** ] **'***session ID1***'**  
 已[!INCLUDE[ssDE](../../includes/ssde-md.md)]工作階段識別碼，從**sys.dm_exec_sessions** ，使用者想要鎖定資訊。 *session ID1*已**int**預設值是 NULL。 執行**sp_who**來取得工作階段的程序資訊。 如果*session ID1*未指定，會顯示所有鎖定的相關資訊。  
  
 [  **@spid2 =** ] **'***工作階段 ID2***'**  
 這是另一個[!INCLUDE[ssDE](../../includes/ssde-md.md)]工作階段識別碼，從**sys.dm_exec_sessions**可能會與同時具有鎖定*session ID1*和使用者也需要其相關資訊。 *工作階段 ID2*已**int**預設值是 NULL。  
...
|**資源**|**nchar(32)**|用來識別鎖定資源的值。 值的格式取決於中所識別之資源的類型**型別**資料行：<br /><br /> **型別**值：**資源**值<br /><br /> RID：格式為 fileid:pagenumber:rid 的識別碼，fileid 用來識別包含頁面的檔案，pagenumber 用來識別包含資料列的頁面，rid 用來識別頁面的特定資料列。 fileid 符合**file_id**中的資料行**sys.database_files**目錄檢視。<br /><br /> KEY：[!INCLUDE[ssDE](../../includes/ssde-md.md)] 在內部使用的十六進位號碼。<br /><br /> PAG：格式為 fileid:pagenumber 的號碼，其中 fileid 用來識別包含頁面的檔案，pagenumber 用來識別頁面。<br /><br /> EXT：用來識別範圍內第一頁的號碼。 這個號碼的格式為 fileid:pagenumber。<br /><br /> 索引標籤： 無資訊，因為資料表已經識別**ObjId**資料行。<br /><br /> DB： 無資訊，因為在已識別的資料庫**dbid**資料行。<br /><br /> FIL： 檔案的識別碼，用來比對**file_id**中的資料行**sys.database_files**目錄檢視。<br /><br /> APP：鎖定之應用程式資源的專屬識別碼。 格式 DbPrincipleId:\<前兩個至 16 個字元的資源字串 >\<雜湊值 >。<br /><br /> MD：隨資源類型而不同。 如需詳細資訊，請參閱說明**resource_description**中的資料行[sys.dm_tran_locks &#40;-&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-tran-locks-transact-sql.md)。<br /><br /> HBT：未提供任何資訊。 使用**sys.dm_tran_locks**動態管理檢視。<br /><br /> AU：未提供任何資訊。 使用**sys.dm_tran_locks**動態管理檢視。|  
...
## <a name="see-also"></a>另請參閱  
 ...
 [sys.dm_os_threads &#40;-SQL&AMP;#41;&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-os-threads-transact-sql.md)  
