...
|**update_languages**|NULL|更新使用全文檢索搜尋所註冊的語言和篩選清單。 這些語言是在設定索引和進行全文檢索查詢時所指定。 篩選用來擷取文字資訊從對應的檔案格式，例如.docx 儲存的資料類型，篩選背景程式主機**varbinary**， **varbinary （max)**，**映像**，或**xml**，全文檢索索引。<br /><br /> 如需詳細資訊，請參閱 [View or Change Registered Filters and Word Breakers](../../relational-databases/search/view-or-change-registered-filters-and-word-breakers.md)。|  
