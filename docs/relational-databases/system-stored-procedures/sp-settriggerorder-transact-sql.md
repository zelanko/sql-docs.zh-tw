...
`[ @triggername = ] '[ _triggerschema.] _triggername'` 是觸發程式的名稱，以及它所屬的架構（如果適用的話），其順序是要設定或變更。 [_triggerschema_ **。** ]*triggername*為**sysname**。 如果名稱未對應於觸發程序，或名稱對應於 INSTEAD OF 觸發程序，程序會傳回錯誤。 無法為 DDL 或登入觸發程式指定*triggerschema* 。  
  
`[ @order = ] 'value'` 是觸發程式新順序的設定。 *值*為**Varchar （10）** ，它可以是下列任何一個值。  
...
`[ @stmttype = ] 'statement_type'` 指定引發觸發程式的 SQL 語句。 *statement_type*為**Varchar （50）** ，而且可以是 INSERT、UPDATE、DELETE、LOGON，或[DDL 事件](../../relational-databases/triggers/ddl-events.md)中列出的任何 [!INCLUDE[tsql](../../includes/tsql-md.md)] 語句事件。 您不能指定事件群組。  
  
 只有在該觸發程式已定義為該語句類型的觸發程式之後，才可以將觸發程式指定為語句類型的**第一個**或**最後一個**觸發程式。 例如，如果將**TR1**定義為 insert 觸發程式，就可以**先**針對資料表**T1**上的 insert 指定觸發程式**TR1** 。 如果**TR1**（已定義為 INSERT 觸發程式）已設定為 UPDATE 語句的**第一個**或**最後**一個觸發程式，則 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 會傳回錯誤。 如需詳細資訊，請參閱＜備註＞一節。  
  
 **\@命名空間 =** { **' 資料庫 '**  |  **' 伺服器 '** |Null  
 當*triggername*是 DDL 觸發程式時， **\@命名空間**會指定是否使用資料庫範圍或伺服器範圍來建立*triggername* 。 如果*triggername*是登入觸發程式，則必須指定伺服器。 如需 DDL 觸發程式範圍的詳細資訊，請參閱[Ddl 觸發](../../relational-databases/triggers/ddl-triggers.md)程式。 如果未指定，或指定了 Null， *triggername*就是 DML 觸發程式。  
...
## <a name="dml-triggers"></a>DML 觸發程序  
 ...
 如果資料表、資料庫或伺服器上已定義了**第一個**觸發程式，您就不能針對相同的 statement_type **，為相同**的資料表、資料庫或伺服器指定新的觸發程式。 這種限制也會套用**最後**的觸發程式。  
