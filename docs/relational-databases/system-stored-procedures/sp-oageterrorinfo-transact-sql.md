 ...
 *objecttoken*  
 物件 token，或是使用先前建立的 OLE 物件的**sp_OACreate**或它便是 NULL。 如果*objecttoken*已指定，會傳回該物件的錯誤資訊。 如果指定了 NULL，就會傳回整個批次的錯誤資訊。  
...
|**說明檔**|**nvarchar(nn)**|來源的說明檔。|  
|**HelpID**|**int**|說明來源檔案中的說明內容識別碼。|  
  
## <a name="remarks"></a>備註  
 每次呼叫 OLE Automation 預存程序 (除了**sp_OAGetErrorInfo**) 重設錯誤資訊; 因此， **sp_OAGetErrorInfo**取得只會針對最近的 OLE 資訊時發生錯誤Automation 預存程序呼叫。 請注意，因為**sp_OAGetErrorInfo**不重設錯誤資訊，它可以多次呼叫以取得相同的錯誤資訊。  
