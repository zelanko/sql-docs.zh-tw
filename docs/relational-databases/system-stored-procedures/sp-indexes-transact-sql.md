...
## <a name="arguments"></a>引數  
 ...
 [ @is_unique =] '*is_unique*'  
 這是要傳回資訊的索引類型。 *is_unique* 是 **bit**，預設值是 Null，而且可以是下列其中一個值。  
...
|INDEX_QUALIFER|**sysname**|索引擁有者的名稱。 部分 DBMS 產品允許資料表擁有者以外的使用者建立索引。 在中 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，這個資料行一律與 **TABLE_NAME**相同。|  
