...
description: 卸載發行者上發行項、發行集或訂閱的訂閱。 這個預存程式會在發行集資料庫的「發行者」端執行。
ms.custom: ''
ms.date: 03/03/2017
ms.prod: sql
...
`[ @publication = ] 'publication'` 這是關聯發行集的名稱。 *發行* 集是 **sysname**，預設值是 Null。 如果是 **all**，指定訂閱者的所有發行集之所有訂閱都會取消。 *發行* 集是必要參數。  
  
`[ @article = ] 'article'` 這是發行項的名稱。 *文章* 是 **sysname**，預設值是 Null。 如果是 **all**，就會卸載每個指定發行集和訂閱者之所有發行項的訂閱。 針對允許立即更新的發行集使用 [ **全部** ]。  
  
`[ @subscriber = ] 'subscriber'` 這是將卸載訂閱的訂閱者名稱。 *訂閱者* 是 **sysname**，沒有預設值。 如果是 **all**，就會卸載所有訂閱者的所有訂閱。  
