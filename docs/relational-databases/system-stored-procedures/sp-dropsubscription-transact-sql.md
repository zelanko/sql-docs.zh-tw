...
`[ @publication = ] 'publication'` 是相關聯發行集的名稱。 *發行*集是**sysname**，預設值是 Null。 如果是**all**，就會取消指定之訂閱者之所有發行集的所有訂閱。 *發行*集是必要參數。  
  
`[ @article = ] 'article'` 是發行項的名稱。 發行項是**sysname**，預設*值是 Null* 。 如果是**all**，就會卸載每個指定發行集和訂閱者之所有發行項的訂閱。 對於允許立即更新的發行集，請使用 [**全部**]。  
  
`[ @subscriber = ] 'subscriber'` 是將卸載其訂閱的訂閱者名稱。 *訂閱者*是**sysname**，沒有預設值。 如果是**all**，就會卸載所有訂閱者的所有訂閱。  
  
`[ @destination_db = ] 'destination_db'` 是目的地資料庫的名稱。 *destination_db*是**sysname**，預設值是 Null。 如果是 NULL，就會卸除這個訂閱者的所有訂閱。  
