...
`[ @dynamic_snapshot_jobname = ] 'dynamic_snapshot_jobname'`這是要變更之快照集作業的名稱。 *dynamic_snapshot_jobname*是**sysname**, 預設值是 N '% '。 如果指定了*dynamic_snapshot_jobid* , 您就必須使用*dynamic_snapshot_jobname*的預設值。  
  
`[ @dynamic_snapshot_jobid = ] 'dynamic_snapshot_jobid'`這是要變更之快照集作業的識別碼。 *dynamic_snapshot_jobid*是**uniqueidentifier**, 預設值是 Null。 如果指定了*dynamic_snapshot_jobname*, 您就必須使用*dynamic_snapshot_jobid*的預設值。  
  
`[ @frequency_type = ] frequency_type`這是排程代理程式的頻率。 *frequency_type*是**int**, 它可以是下列值之一。  
...
`[ @frequency_interval = ] frequency_interval`代理程式的執行天數。 *frequency_interval*是**int**, 它可以是下列值之一。  
...
`[ @frequency_relative_interval = ] frequency_relative_interval`這是合併代理程式執行的日期。 當*frequency_type*設定為**32** (每月相對) 時, 會使用這個參數。 *frequency_relative_interval*是**int**, 它可以是下列值之一。  
...
`[ @job_login = ] 'job_login'`這是使用參數化資料列篩選器來產生訂閱的快照集時, 用來執行快照集代理程式的Windows帳戶。[!INCLUDE[msCoName](../../includes/msconame-md.md)] *job_login*是**Nvarchar (257)** , 預設值是 Null。  
