...
ms.locfileid: "97466909"
---
# <a name="sp_describe_first_result_set-transact-sql"></a>sp_describe_first_result_set (Transact-SQL)
[!INCLUDE [sql-asdb-asdbmi-asa-pdw](../../includes/applies-to-version/sql-asdb-asdbmi-asa-pdw.md)]

  傳回批次第一個可能結果集的中繼資料 [!INCLUDE[tsql](../../includes/tsql-md.md)] 。 如果批次沒有傳回任何結果，就會傳回空的結果集。 如果 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 無法判斷將透過執行靜態分析來執行之第一個查詢的中繼資料，則會引發錯誤。 動態管理檢視 [sys.dm_exec_describe_first_result_set &#40;transact-sql&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-exec-describe-first-result-set-transact-sql.md) 會傳回相同的資訊。  
...
`[ \@tsql = ] 'Transact-SQL_batch'` 一或多個 [!INCLUDE[tsql](../../includes/tsql-md.md)] 語句。 *交易 SQL_batch* 可能是 **Nvarchar (** _n_ *_)_* 或 **Nvarchar (max)**。  
  
`[ \@params = ] N'parameters'`\@params 提供批次參數的宣告字串 [!INCLUDE[tsql](../../includes/tsql-md.md)] ，類似于 sp_executesql。 參數可以是 **Nvarchar (n)** 或 **Nvarchar (max)**。  
 ...
 **sp_describe_first_result_set** 一律會在成功時傳回零的狀態。 如果程式擲回錯誤，並以 RPC 的形式呼叫程式，則會以 sys.dm_exec_describe_first_result_set 的 error_type 資料行中所述的錯誤類型來填入傳回狀態。 如果程序是從 [!INCLUDE[tsql](../../includes/tsql-md.md)] 所呼叫，即使發生錯誤，傳回值也永遠是零。  
...
|**system_type_id**|**int NOT NULL**|包含 sys. 類型中所指定資料行之資料類型的 system_type_id。 針對 CLR 類型，即使 system_type_name 資料行將傳回 NULL，這個資料行將會傳回值 240。|  
...
|**max_length**|**Smallint 非 Null**|資料行的最大長度 (以位元組為單位)。<br /><br /> -1 = 資料行資料類型是 **Varchar (max)**、 **Nvarchar (max)**、 **Varbinary (max)** 或 **xml**。<br /><br /> 若為 **文字** 資料行， **max_length** 值會是16或 **sp_tableoption ' text in row '** 所設定的值。|  
|**有效位數**|**Tinyint 非 Null**|如果以數值為基礎，就是資料行的有效位數。 否則傳回 0。|  
|**scale**|**Tinyint 非 Null**|如果是以數值為基礎，便是資料行的小數位數。 否則傳回 0。|  
...
|**source_server**|**sysname**|在這個結果中的資料行所傳回的原始伺服器名稱 (如果它來自遠端伺服器)。 系統會在 sys. 伺服器中提供名稱。 如果資料行來自本機伺服器，或是如果無法判別其原始伺服器，則傳回 NULL。 只會在要求瀏覽資訊時填入。|  
...
|**order_by_list_length**|**Smallint Null**|ORDER BY 清單的長度。 如果沒有 ORDER BY 清單，或如果無法唯一判別 ORDER BY 清單，則傳回 NULL。 請注意，sp_describe_first_result_set 所傳回的所有資料列，這個值都是一樣的 **。**|  
...
## <a name="remarks"></a>備註  
 **sp_describe_first_result_set** 保證如果程式傳回 (假設) 批次的第一個結果集中繼資料，而且後續執行該批次 () ，則批次將 (1) 引發優化時錯誤， (2) 會引發執行階段錯誤， (3) 不會傳回任何結果集，或 (4) 會傳回第一個結果集，其中包含 **sp_describe_first_result_set** 所描述的相同中繼資料。  
  
 名稱、Null 屬性和資料類型可以不同。 如果 **sp_describe_first_result_set** 傳回空的結果集，則保證批次執行將不會傳回任何結果集。  
  
 這保證會假設伺服器上沒有任何相關的架構變更。 伺服器上的相關架構變更不包括在呼叫 **sp_describe_first_result_set** 的時間和執行期間傳回結果集的時間之間，于批次中建立臨時表或資料表變數，包括批次 B 所做的架構變更。  
  
 **sp_describe_first_result_set** 在下列任何情況下會傳回錯誤。  
...
-   查詢包含建立隨後要查詢的永久資料表。  
 ...
 如果其他所有檢查已成功，就會將輸入批次內所有可能的控制流程路徑列入考量。 這會將所有控制流程語句納入考慮， (移至 IF/ELSE、WHILE 和 [!INCLUDE[tsql](../../includes/tsql-md.md)] TRY/CATCH 區塊) 以及 [!INCLUDE[tsql](../../includes/tsql-md.md)] 由 EXEC 語句從輸入批次叫用的任何程式、動態批次或觸發程式、引發 ddl 觸發程式的 ddl 語句，或導致在目標資料表上引發觸發程式的 DML 語句，或是因為外鍵條件約束的串聯動作而修改的資料表。 在具有許多可能的控制路徑之情況下，有時候，演算將會停止。  
  
 針對每個控制流程路徑，如果傳回結果集的任何) 是由 **sp_describe_first_result_set** 決定，則第一個語句 (。  
...
-   如果資料類型不同，將會擲回錯誤，且不會傳回任何結果，除非遇到下列情況：  
    ...
    -   **Varchar ()** 至 **Varchar (' )** where a ' >。  
  
    -   **Varchar ()** 至 **Varchar (max)**  
  
    -   **Nvarchar ()** 至 **Nvarchar (' )** where a ' >。  
  
    -   **Nvarchar ()** 至 **Nvarchar (max)**  
  
    -   **Varbinary ()** 至 **Varbinary (' )** where a ' >。  
  
    -   **Varbinary ()** 至 **Varbinary (max)**  
...
```  
 ...
 結果：錯誤、不相符的類型 (**Varchar (10)** 與 **Nvarchar (10)**) 。  
...
#### <a name="error-caused-by-a-ambiguous-result-set"></a>因位結果集模稜兩可而發生錯誤  
 此範例假設名為 user1 的另一個使用者在預設架構 s1 中有一個名為 t1 的資料表，而資料行 (**INT NOT Null**) 。  
...
```  
 ...
 結果：錯誤。 t1 可以是 dbo. t1 或 s1，每個都有不同的資料行數目。  
...
```  
 ...
 結果： **int 是 Null** ，因為 dbo. a 和 s1. t1 都具有類型 **int** 和不同的 null 屬性。  
