...
`[ \@tsql = ] 'Transact-SQL_batch'` 一或多個 [!INCLUDE[tsql](../../includes/tsql-md.md)] 語句。 *交易 SQL_batch* 可能是 **Nvarchar (***n***) ** 或 **Nvarchar (max) **。  
 ...
 **sp_describe_first_result_set** 一律會在成功時傳回零的狀態。 如果程式擲回錯誤，並以 RPC 的形式呼叫程式，則傳回狀態會填入 sys. dm_exec_describe_first_result_set 的 error_type 資料行中所述的錯誤類型。 如果程序是從 [!INCLUDE[tsql](../../includes/tsql-md.md)] 所呼叫，即使發生錯誤，傳回值也永遠是零。  
