...
`[ @distributor = ] 'distributor'`這是散發伺服器名稱。 散發者是**sysname**, 沒有預設值。 只有在設定遠端散發者時，才使用這個參數。 它會在 msdb 中加入散發者屬性的專案 **。MSdistributor**資料表。  
