...
+ 在 RevoScaleR 中使用 `rxGlm` 或 `rxNaiveBayes` 演算法的模型
+ R 中的 PMML 模型
+ 使用其他協力廠商程式庫建立的模型 
...
除了是有效的 SQL 查詢， *\@inputData*中的輸入資料也必須包含與預存模型中的資料行相容的資料行。
