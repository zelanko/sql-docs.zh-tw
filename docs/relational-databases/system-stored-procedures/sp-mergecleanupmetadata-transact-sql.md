...
ms.locfileid: "70123088"
---
# <a name="sp_mergecleanupmetadata-transact-sql"></a>sp_mergecleanupmetadata (Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]

  只能用於包含執行[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)] Service Pack 1 之前版本之[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]伺服器的複寫拓撲中。**sp_mergecleanupmetadata**可讓管理員清除**MSmerge_genhistory**、 **MSmerge_contents**和**MSmerge_tombstone**系統資料表中的中繼資料。 這個預存程序執行於發行集資料庫的發行者端。  
...
`[ @publication = ] 'publication'`這是發行集的名稱。 *發行*集是**sysname**, 預設值 **%** 是, 這會清除所有發行集的中繼資料。 如果明確指定的話，發行集必須已存在。  
...
> [!CAUTION]
>  在執行**sp_mergecleanupmetadata**之後, 根據預設, 在**MSmerge_genhistory**、 **MSmerge_contents**和**MSmerge_tombstone**中儲存中繼資料之發行集訂閱者的所有訂閱都會標示為重新初始化, 訂閱者端的任何暫止變更都會遺失, 而目前的快照集會標示為過時。  
> 
> [!NOTE]
>  如果資料庫上有多個發行集, 而且其中任何一個發行集使用無限期的發行集保留=期限 ( **\@保留** **0**), 則執行**sp_mergecleanupmetadata**並不會清除資料庫的合併式複寫變更追蹤中繼資料。 因此，在使用無限期的發行期限時，一定要特別小心。  
  
 執行此預存程式時, 您可以將 **\@reinitialize_subscriber**參數設定為**TRUE** (預設值) 或**FALSE**, 以選擇是否要重新初始化訂閱者。 如果執行sp_mergecleanupmetadata 時, 將**reinitialize_subscriber參數設定為TRUE,即使在沒有初始快照集的情況下建立訂閱,也會在訂閱者端重新套用快照集(適用于\@** 例如, 如果快照集資料和架構已手動套用, 或已存在於訂閱者端)。 將參數設定為**FALSE**應謹慎使用, 因為如果發行集不是重新初始化, 您必須確定「發行者」和「訂閱者」端的資料已同步處理。  
  
 不論 **\@reinitialize_subscriber**的值為何, 如果有正在進行的合併程式嘗試將變更上傳至發行者或重新發行的訂閱者 (在儲存的時間), **sp_mergecleanupmetadata**就會失敗程式已叫用。  
  
 **執行具有 reinitialize_subscriber \@= TRUE 的 sp_mergecleanupmetadata:**  
...
## <a name="permissions"></a>Permissions  
 ...
 若要使用這個預存程序，發行者必須執行 [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)]。 訂閱者必須[!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)]執行或[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 7.0 Service Pack 2。  
