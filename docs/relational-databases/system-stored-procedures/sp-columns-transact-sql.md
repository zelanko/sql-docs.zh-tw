 ...
 **Sp_columns**目錄預存程序相當於**SQLColumns** ODBC 中。 傳回的結果會依照**TABLE_QUALIFIER**， **TABLE_OWNER**，和**TABLE_NAME**。  
...
|**IS_NULLABLE**|**varchar(254)**|物件中資料行的 Null 屬性。 遵照 ISO 規則來決定 Null 屬性。 ISO SQL 標準 DBMS 無法傳回空字串。<br /><br /> YES = 資料行可以包括 NULLS。<br /><br /> NO = 資料行不能包括 NULLS。<br /><br /> 如果 Null 屬性不明，這個資料行會傳回長度為零的字串。<br /><br /> 傳回的值，這個資料行傳回的值不同的**NULLABLE**資料行。|  
...
## <a name="permissions"></a>Permissions  
 需要結構描述上的 SELECT 和 VIEW DEFINITION 權限。  
