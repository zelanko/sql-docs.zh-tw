...
ms.locfileid: "97478909"
---
# <a name="sp_help-transact-sql"></a>sp_help (Transact-SQL)
[!INCLUDE [SQL Server SQL Database](../../includes/applies-to-version/sql-asdb.md)]

   (**sys.sys物件** 相容性檢視) 、使用者定義資料類型或資料類型中所列的任何物件，報告資料庫物件的相關資訊。  
...
`[ @objname = ] 'name'` 這是 **sysobjects** 中的任何物件名稱，或 **systypes** 資料表中任何使用者定義的資料類型。 *名稱* 是 **Nvarchar (** 776 **)**，預設值是 Null。 不接受資料庫名稱。  兩個或三個部分的名稱必須加以分隔，例如 'Person.AddressType' 或 [Person.AddressType]。   
...
## <a name="result-sets"></a>結果集  
 傳回的結果集會根據指定的 *名稱* 、指定的時間，以及它的資料庫物件而定。  
  
1.  如果執行的 **sp_help** 沒有任何引數，則會傳回目前資料庫中所有類型之物件的摘要資訊。  
...
2.  如果 *name* 是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料類型或使用者定義資料類型， **sp_help** 會傳回這個結果集。  
...
3.  如果 *name* 是資料類型以外的任何資料庫物件， **sp_help** 會根據指定的物件類型，傳回這個結果集以及其他的結果集。  
    ...
    |**Created_datetime**|**datetime**|資料表的建立日期|  
     ...
     如果 *name* 是系統資料表、使用者資料表或視圖， **sp_help** 會傳回下列結果集。 不過，不會傳回描述資料檔在檔案群組中之位置的結果集。  
...
## <a name="permissions"></a>權限  
 需要 **public** 角色的成員資格。 使用者至少必須有一個 *objname* 的許可權。 若要檢視資料行條件約束索引鍵、預設值或規則，您必須具有此資料表的 VIEW DEFINITION 權限。  
