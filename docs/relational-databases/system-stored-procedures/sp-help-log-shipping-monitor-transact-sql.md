---
title: sp_help_log_shipping_monitor & Amp;#40;transact-SQL&AMP;#41; |Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
ms.prod: sql
...
|資料行名稱|資料類型|描述|  
|-----------------|---------------|-----------------|  
|**status**|**bit**|記錄傳送資料庫的代理程式整體狀態：<br /><br /> **0** = 狀況良好且沒有代理程式失敗。<br /><br /> **1** = 其他。|  
|**is_primary**|**bit**|指出這個資料列是否為主要資料庫的資料列：<br /><br /> **1** = 資料列是主要資料庫。<br /><br /> **0** = 資料列是次要資料庫。|  
...
|**backup_threshold**|**int**|前次備份之後到產生 threshold_alert 錯誤之前的時間長度 (以分鐘為單位)。 **backup_threshold**已**int**，預設值是**60 分鐘**。<br /><br /> NULL = 資訊無法取得或不相干。<br /><br /> 這個值可以使用來變更[sp_add_log_shipping_primary_database &#40;TRANSACT-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-log-shipping-primary-database-transact-sql.md)。|  
|**is_backup_alert_enabled**|**bit**|指定是否可以警示時引發**backup_threshold**超過。 其中一個值 (**1**)，預設值，表示將會引發警示。<br /><br /> NULL = 資訊無法取得或不相干。<br /><br /> 這個值可以使用來變更[sp_add_log_shipping_primary_database &#40;TRANSACT-SQL&#41;](../../relational-databases/system-stored-procedures/sp-add-log-shipping-primary-database-transact-sql.md)。|  
