...
`[ @article = ] 'article'`這是要變更其屬性的發行項名稱。 發行*項是* **sysname**，預設值是 Null。  
...
> [!NOTE]  
>  變更*schema_option*的值時，系統不會執行位更新。 這表示當您使用**sp_changearticle**設定*schema_option*時，可能會關閉現有的位設定。 若要保留現有的設定，您應該執行[|（位 OR）](../../t-sql/language-elements/bitwise-or-transact-sql.md)在您要設定的值和目前的*schema_option*值之間，可以藉由執行[sp_helparticle](../../relational-databases/system-stored-procedures/sp-helparticle-transact-sql.md)來判斷。  
