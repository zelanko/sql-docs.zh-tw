...
[!INCLUDE[appliesto-ss-asdbmi-xxxx-xxx-md](../../includes/appliesto-ss-asdbmi-xxxx-xxx-md.md)]
  ...
  以可讀取格式傳回已標記為要進行複寫的交易命令。 只有在用戶端連接 (包括目前的連接) 並未從記錄檔讀取複寫的交易時, 才能執行**sp_replshowcmds** 。 這個預存程序執行於發行集資料庫的發行者端。  
 ...
 **sp_replshowcmds**用於異動複寫中。  
 ...
 您可以使用**sp_replshowcmds**來查看目前未散發的交易 (交易記錄檔中剩餘但尚未傳送到「散發者」的交易)。  
  
 在相同資料庫內執行**sp_replshowcmds**和**sp_replcmds**的用戶端會收到錯誤18752。  
  
 若要避免這個錯誤, 第一個用戶端必須中斷連接, 或用戶端的角色, 因為記錄讀取器必須藉由執行**sp_replflush**來發行。 當所有用戶端都已與記錄讀取器中斷連線之後, **sp_replshowcmds**就可以順利執行。  
  
> [!NOTE]  
>  **sp_replshowcmds**應僅執行以疑難排解複寫的問題。  
