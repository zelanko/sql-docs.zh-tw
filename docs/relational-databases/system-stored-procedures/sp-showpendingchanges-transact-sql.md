...
`[ @publication = ] 'publication'` 這是發行集的名稱。 *發行* 集是 **sysname**，預設值是 Null。 當指定了 *發行* 集時，結果只會限制為指定的發行集。  
  
`[ @article = ] 'article'` 這是發行項的名稱。 *文章* 是 **sysname**，預設值是 Null。 當 *指定發行項時，* 結果只會限制為指定的發行項。  
  
`[ @show_rows = ] 'show_rows'` 指定結果集是否包含有關暫止變更的更特定資訊，預設值為 **0**。 如果指定值為 **1** ，結果集會包含 is_delete 和 rowguid 的資料行。  
...
|is_dest_subscriber|**bit**|指出正要將變更複寫到訂閱者。 值為 **1** 時，表示正在將變更複寫至「訂閱者」。 **0** 表示正在將變更複寫到發行者。|  
...
## <a name="remarks"></a>備註  
 ...
 當針對發行項指定 *的發行* 項不屬於發行集所指定的發行集時 *，* 會傳回 pending_deletes 的計數0和 pending_ins_and_upd。  
