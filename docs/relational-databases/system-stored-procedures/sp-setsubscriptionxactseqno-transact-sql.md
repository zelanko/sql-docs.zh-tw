...
ms.locfileid: "89547876"
---
# <a name="sp_setsubscriptionxactseqno-transact-sql"></a>sp_setsubscriptionxactseqno (Transact-SQL)
[!INCLUDE [SQL Server](../../includes/applies-to-version/sqlserver.md)]

  在疑難排解期間使用 (LSN) 的記錄序號指定最後傳遞的交易，讓散發代理程式開始在下一次交易時傳遞。 重新開機之後，散發代理程式會從散發資料庫快取 (msrepl_commands) 傳回大於此水位線的交易 (LSN) 。 這個預存程序執行於訂閱資料庫的訂閱者端。 不支援非 SQL Server 訂閱者使用這個項目。  
...
`[ @publisher_db = ] 'publisher_db'` 這是發行集資料庫的名稱。 *publisher_db* 是 **sysname**，沒有預設值。 若為非 SQL Server 發行者， *publisher_db* 是散發資料庫的名稱。  
  
`[ @publication = ] 'publication'` 這是發行集的名稱。 *發行* 集是 **sysname**，沒有預設值。 當一個以上的發行集共用散發代理程式時，您必須為 *發行*集指定 ALL 的值。  
  
`[ @xact_seqno = ] xact_seqno` 這是散發者端要在訂閱者端套用下一個交易的 LSN。 *xact_seqno* 是 **Varbinary (16) **，沒有預設值。  
 ...
 **sp_setsubscriptionxactseqno** 可以用來略過在訂閱者端套用時，會導致錯誤的特定交易。 當發生失敗，而且在散發代理程式停止之後，請在散發者端呼叫 [sp_helpsubscriptionerrors &#40;transact-sql&#41;](../../relational-databases/system-stored-procedures/sp-helpsubscriptionerrors-transact-sql.md) 以抓取失敗交易的 xact_seqno 值，然後呼叫 **sp_setsubscriptionxactseqno**，傳遞此值以供 *xact_seqno*。 這可以確保只會處理在這個 LSN 之後的命令。  
  
 針對*xact_seqno*指定**0**的值，將散發資料庫中的所有暫止命令傳遞給訂閱者。  
  
 如果散發代理程式使用多個訂用帳戶資料流程， **sp_setsubscriptionxactseqno**可能會失敗。  
