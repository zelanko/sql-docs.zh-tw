...
`[ @distributor = ] 'distributor'`這是散發者的名稱。 散發者是**sysname**, 預設值是*發行者*所指定的值。  
...
`[ @dts_package_name = ] 'dts_package_name'`指定 DTS 封裝的名稱。 *dts_package_name*是**sysname** , 預設值是 Null。 例如，若要指定 `DTSPub_Package` 封裝，這個參數便是 `@dts_package_name = N'DTSPub_Package'`。  
...
`[ @dts_package_location = ] 'dts_package_location'`指定封裝位置。 *dts_package_location*是**Nvarchar (12)** , 預設值是「**訂閱者**」。 封裝的位置可以是「散發者」或「**訂閱者**」。  
