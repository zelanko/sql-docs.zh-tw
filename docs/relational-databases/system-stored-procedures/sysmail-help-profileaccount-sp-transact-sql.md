...
## <a name="remarks"></a>備註  
 未指定 *profile_id* 或 *profile_name* 時，此預存程式會傳回實例中每個設定檔的資訊。  
  
 預存程式 **sysmail_help_profileaccount_sp** 位於 **msdb** 資料庫中，而且是由 **dbo** 架構所擁有。 如果目前的資料庫不是 **msdb**，就必須以三部分名稱執行程式。  
