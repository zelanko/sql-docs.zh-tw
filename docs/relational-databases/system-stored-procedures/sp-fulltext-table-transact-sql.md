...
> [!IMPORTANT]  
>  [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)]請改用[CREATE 全文檢索索引](../../t-sql/statements/create-fulltext-index-transact-sql.md)、 [ALTER 全文檢索索引](../../t-sql/statements/alter-fulltext-index-transact-sql.md)和[DROP 全文檢索索引](../../t-sql/statements/drop-fulltext-index-transact-sql.md)。  
...
|**啟用**|啟用全文檢索索引資料在停用之後，針對*qualified_table_name*收集的功能。 至少必須有一個資料行參與全文檢索索引，您才能啟動它。<br /><br /> 只要加入了第一個索引資料行，全文檢索索引就會自動成為使用中 (以便擴展)。 如果從索引中卸除最後一個資料行，索引便會成為非使用中。 如果變更追蹤是開啟的，啟動非作用中的索引會啟動新的擴展。<br /><br /> 請注意，這並不會實際填入全文檢索索引，而只會在檔案系統的全文檢索目錄中註冊資料表，以便在下一個全文檢索索引擴展期間，可以抓取*qualified_table_name*的資料列。|  
|**取消**|停用*qualified_table_name*的全文檢索索引，如此就無法再收集*qualified_table_name*的全文檢索索引資料。 仍會保留全文檢索索引中繼資料，且可以重新啟動資料表。<br /><br /> 如果變更追蹤是開啟的，停用使用中的索引會凍結索引狀態：任何進行中的擴展都會停止，不會再將任何其他變更傳播給索引。|  
|**start_change_tracking**|啟動全文檢索索引的累加擴展。 如果資料表沒有時間戳記，便啟動全文檢索索引的完整擴展。 啟動資料表的變更追蹤。<br /><br /> 全文檢索變更追蹤並不會追蹤**image**、 **text**或**Ntext**類型之全文檢索索引資料行上執行的任何 WRITETEXT 或 UPDATETEXT 作業。|  
...
`[ @ftcat = ] 'fulltext_catalog_name'`這是**create**動作的有效現有全文檢索目錄名稱。 所有其他動作的這個參數都必須是 NULL。 *fulltext_catalog_name*是**sysname**，預設值是 Null。  
  
`[ @keyname = ] 'unique_index_name'`這是**create**動作*qualified_table_name*的有效單一索引鍵資料行、唯一的不可為 null 索引。 所有其他動作的這個參數都必須是 NULL。 *unique_index_name*是**sysname**，預設值是 Null。  
...
## <a name="remarks"></a>備註  
 ...
 定義全文檢索索引的資料表之後，將全文檢索唯一索引鍵資料行從某個資料類型切換到另一個，方法是變更該資料行的資料類型，或將全文檢索唯一索引鍵從某個資料行變更為另一個，而不需要完整的替換，可能會導致後續查詢期間發生失敗，並傳回錯誤訊息：「全文檢索搜尋索引鍵值*key_value*的轉換為類型*data_type*失敗。」 若要避免這種情況，請使用**sp_fulltext_table**的**drop**動作來卸載此資料表的全文檢索定義，然後使用**sp_fulltext_table**和**sp_fulltext_column**重新定義它。  
...
### <a name="a-enabling-a-table-for-full-text-indexing"></a>A. 啟用資料表的全文檢索索引  
 下列範例會針對 `Document` 資料庫的 `AdventureWorks` 資料表建立全文檢索索引中繼資料。 
  `Cat_Desc` 是一個全文檢索目錄。 
  `PK_Document_DocumentID` 是 `Document` 的唯一單一資料行索引。  
