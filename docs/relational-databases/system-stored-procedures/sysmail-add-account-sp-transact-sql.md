...
`[ @use_default_credentials = ] use_default_credentials` 指定是否使用 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]的認證將郵件傳送至 SMTP 伺服器。 **use_default_credentials**是 bit，預設值是0。 當此參數是 1 時，Database Mail 會使用 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 的認證。 當此參數為0時，Database Mail 會**傳送\@使用者名稱**和 **\@密碼**參數（如果有的話），否則會傳送不含\@使用者**名稱**和 **\@密碼**參數的郵件。  
