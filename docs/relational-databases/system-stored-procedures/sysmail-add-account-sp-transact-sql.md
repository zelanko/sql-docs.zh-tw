...
`[ @email_address = ] 'email_address'` 用來傳送訊息的電子郵件地址。 這個地址必須是網際網路電子郵件地址。 *email_address*是**Nvarchar （128）** ，沒有預設值。 例如，@no__t 0 代理程式的帳戶可能會從位址 **SqlAgent@Adventure-Works.com** 傳送電子郵件。  
  
`[ @display_name = ] 'display_name'` 此帳戶的電子郵件訊息所用的顯示名稱。 *display_name*是**Nvarchar （128）** ，預設值是 Null。 例如，@no__t 0 代理程式的帳戶可能會在電子郵件訊息上顯示**SQL Server Agent 自動郵件**的名稱。  
  
`[ @replyto_address = ] 'replyto_address'` 此帳戶的訊息回應的位址會傳送至。 *replyto_address*是**Nvarchar （128）** ，預設值是 Null。 例如，回復 @no__t 0 代理程式的帳戶，可能會前往資料庫管理員， **danw@Adventure-Works.com** 。  
...
`[ @port = ] port_number`：電子郵件伺服器的埠號碼。 *port_number*是**int**，預設值是25。  
...
## <a name="remarks"></a>備註  
 Database Mail 提供 **@no__t 1email_address**、 **@no__t 3display_name**和 **@no__t 5replyto_address**的個別參數。 **@No__t 1email_address**參數是傳送訊息的來源位址。 **@No__t 1display_name**參數是電子郵件訊息的 [**寄件者：** ] 欄位中顯示的名稱。 **@No__t 1replyto_address**參數是將傳送電子郵件訊息的位址。 例如，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 所用的帳戶，可以從只供 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 使用的電子郵件地址傳送電子郵件訊息。 這個地址所送出的訊息應該會顯示易記名稱，使收件者能夠輕鬆判斷是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 送出訊息。 如果收件者回應訊息，這個回應應該會送給資料庫管理員，而不是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 所用的地址。 針對此案例，帳戶會使用 **SqlAgent@Adventure-Works.com** 做為電子郵件地址。 [顯示名稱] 設定為 [ **SQL Server Agent 自動郵件**傳送]。 帳戶會使用 **danw@Adventure-Works.com** 做為回復位址，因此，從這個帳戶傳送之訊息的回復會移至資料庫管理員，而不是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 代理程式的電子郵件地址。 Database Mail 分別提供這三個參數的獨立設定，因此，您可以依照需要來設定訊息。  
  
 **@No__t 1mailserver_type**參數支援 **' SMTP '** 值。  
  
 當 **@no__t 1use_default_credentials**為1時，會使用 [!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)] 的認證將郵件傳送至 SMTP 伺服器。 當 **\@use_default_credentials**為0，且為帳戶指定了 **@no__t 3username**和 **@no__t 5password**時，該帳戶會使用 SMTP 驗證。 **@No__t 1username**和 **@no__t 3password**是帳戶用於 SMTP 伺服器的認證，而不是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 或電腦所在網路的認證。  
