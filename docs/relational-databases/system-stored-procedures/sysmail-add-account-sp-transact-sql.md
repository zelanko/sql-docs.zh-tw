...
`[ @email_address = ] 'email_address'`要傳送訊息的來源電子郵件地址。 這個地址必須是網際網路電子郵件地址。 *email_address*是**Nvarchar （128）**，沒有預設值。 例如， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 的帳戶可能會從 address **SqlAgent\@Adventure-Works.com**傳送電子郵件。  
...
`[ @replyto_address = ] 'replyto_address'`此帳戶中的訊息回應的位址會傳送至。 *replyto_address*是**Nvarchar （128）**，預設值是 Null。 例如，回復[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 的帳戶可能會前往資料庫管理員**danw\@Adventure-Works.com**。  
...
`[ @use_default_credentials = ] use_default_credentials`指定是否使用的認證將郵件傳送至 SMTP 伺服器[!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]。 **use_default_credentials**是 bit，預設值是0。 當此參數是 1 時，Database Mail 會使用 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 的認證。 當此參數為0時，Database Mail 會傳送使用者** \@名稱**和** \@密碼**參數（如果有的話），否則會傳送不含** \@使用者名稱**和** \@密碼**參數的郵件。  
