...
`[ @force_invalidate_snapshot = ] force_invalidate_snapshot` 確認此預存程式所採取的動作可能會使現有的快照集失效。 *force_invalidate_snapshot*是**bit**，預設值是**0**。  
...
`[ @force_reinit_subscription = ] force_reinit_subscription` 確認此預存程式所採取的動作可能需要重新初始化現有的訂閱。 *force_reinit_subscription*是**位**，預設值是**0**。  
