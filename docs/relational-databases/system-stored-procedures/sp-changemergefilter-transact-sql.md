...
`[ @filtername = ] 'filtername'` 這是篩選的目前名稱。 *filtername* 是 **sysname**，沒有預設值。  
  
`[ @property = ] 'property'` 這是要變更的屬性名稱。 *屬性* 是 **sysname**，沒有預設值。  
  
`[ @value = ] 'value'` 這是指定之屬性的新值。 *值*為 **Nvarchar (1000) **，沒有預設值。  
 ...
 **1** 表示合併發行項的變更可能使快照集失效，如果有現有的訂閱需要新的快照集，則會將現有快照集的許可權標示為已淘汰，並產生新的快照集。  
 ...
 **1** 表示合併發行項的變更會使現有的訂閱重新初始化，且會提供將發生之訂閱重新初始化的許可權。  
