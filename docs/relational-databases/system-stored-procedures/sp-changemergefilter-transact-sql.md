...
`[ @article = ] 'article'` 是發行項的名稱。 *文章*是**sysname**，沒有預設值。  
  
`[ @filtername = ] 'filtername'` 是篩選準則的目前名稱。 *filtername*是**sysname**，沒有預設值。  
  
`[ @property = ] 'property'` 是要變更的屬性名稱。 *屬性*是**sysname**，沒有預設值。  
  
`[ @value = ] 'value'` 是指定之屬性的新值。 *value*是**Nvarchar （1000）** ，沒有預設值。  
...
`[ @force_invalidate_snapshot = ] force_invalidate_snapshot` 會認可此預存程式所採取的動作可能會使現有的快照集失效。 *force_invalidate_snapshot*是**bit**，預設值是**0**。  
...
`[ @force_reinit_subscription = ] force_reinit_subscription` 會認可此預存程式所採取的動作可能需要重新初始化現有的訂閱。 *force_reinit_subscription*是**bit** ，預設值是**0**。  
