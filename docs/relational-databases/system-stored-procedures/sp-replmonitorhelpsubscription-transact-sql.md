...
`[ @publisher = ] 'publisher'` 這是正在監視之狀態的發行者名稱。 *publisher* 是 **sysname**，預設值是 Null。 如果是 **null**，則會傳回所有使用散發者之發行者的資訊。  
  
`[ @publisher_db = ] 'publisher_db'` 這是已發行的資料庫名稱。 *publisher_db* 是 **sysname**，預設值是 Null。 如果是 NULL，就會傳回發行者端所有已發行資料庫的資訊。  
  
`[ @publication = ] 'publication'` 這是要監視之發行集的名稱。 *發行* 集是 **sysname**，預設值是 Null。  
...
`[ @mode = ] mode` 這是傳回訂閱監視資訊時使用的篩選模式。 *模式* 是 **int**，它可以是下列值之一。  
...
`[ @topnum = ] topnum` 將結果集限制為只有所傳回資料頂端指定數目的訂閱。 *t* 是 **int**，沒有預設值。  
  
`[ @exclude_anonymous = ] exclude_anonymous` 這是指是否從結果集排除匿名提取訂閱。 *exclude_anonymous* 是 **bit**，預設值是 **0**; **1** 值表示排除匿名訂閱， **0** 值則表示包含這些訂閱。  
...
|**subtype**|**int**|這是訂閱類型，它可以是下列其中一個值：<br /><br /> **0** = 推送<br /><br /> **1** = 提取<br /><br /> **2** = 匿名|  
...
|**monitorranking**|**int**|這是對結果集裡的訂閱進行排序所用的次序值，它可以是下列值之一：<br /><br /> 對於交易式發行集：<br /><br /> **60** = 錯誤<br /><br /> **56** = 警告：效能嚴重不足<br /><br /> **52** = 警告：即將過期或已過期<br /><br /> **50** = 警告：訂用帳戶未初始化<br /><br /> **40** = 正在重試失敗的命令<br /><br /> **30** = 未執行 (成功) <br /><br /> **20** = 正在執行 (啟動中、執行中或閒置) <br /><br /> 對於合併式發行集：<br /><br /> **60** = 錯誤<br /><br /> **56** = 警告：效能嚴重不足<br /><br /> **54** = 警告：長時間執行的合併<br /><br /> **52** = 警告：即將過期<br /><br /> **50** = 警告：訂用帳戶未初始化<br /><br /> **40** = 正在重試失敗的命令<br /><br /> **30** = 正在執行 (啟動中、執行中或閒置) <br /><br /> **20** = 未執行 (成功) |  
...
## <a name="permissions"></a>權限  
 只有散發資料庫上的 **db_owner** 或 **replmonitor** 固定資料庫角色的成員，才能夠執行 **sp_replmonitorhelpsubscription**。  
