...
`[ @article = ] 'article'` 這是發行項的名稱。 *文章* 是 **sysname**，預設值是 all。 若為立即更新的發行集 *，則* 必須 **全部**為：否則，預存程式會略過發行集並報告錯誤。  
...
`[ @for_schema_change = ] 'for_schema_change'` 指出是否因發行集資料庫的架構變更而發生重新初始化。 *for_schema_change* 是 **bit**，預設值是0。 如果是 **0**，只要重新初始化整個發行集，而不只是部分發行項，就會重新開機允許立即更新之發行集的有效訂閱。 這表示會因結構描述變更而重新初始化。 如果是 **1**，則在快照集代理程式執行之前，不會重新開機使用中的訂閱。  
  
`[ @publisher = ] 'publisher'` 指定非 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 發行者。 *publisher* 是 **sysname**，預設值是 Null。  
  
> [!NOTE]  
>  *發行者* 不能用於發行者 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 。  
 ...
 **sp_reinitsubscription** 用於異動複寫中。  
 ...
 點對點異動複寫不支援**sp_reinitsubscription** 。  
  
 如果是會自動套用初始快照集且發行集不允許可更新的訂閱之訂閱，就必須在執行這個預存程序之後執行快照集代理程式，以便準備結構描述和大量複製程式，然後散發代理程式就能夠重新同步處理訂閱。  
  
 如果是會自動套用初始快照集且發行集允許可更新的訂閱之訂閱，散發代理程式會利用快照集代理程式先前所建立的大量複製程式和最新的結構描述，來重新同步處理訂閱。 散發代理程式在使用者執行 **sp_reinitsubscription**之後，如果散發代理程式不忙碌，則會立即重新同步訂用帳戶;否則，可能會在 (由散發代理程式命令提示字元參數： **MessageInterval**) 指定的訊息間隔之後進行同步處理。  
  
 **sp_reinitsubscription** 不會影響以手動方式套用初始快照集的訂閱。  
  
 若要重新同步處理發行集的匿名訂閱，請以*訂閱者*的形式傳遞**all**或 Null。  
...
## <a name="permissions"></a>權限  
 只有 **系統管理員（sysadmin** ）固定伺服器角色的成員、 **db_owner** 固定資料庫角色的成員，或訂閱的建立者才能執行 **sp_reinitsubscription**。  
