...
|**proc schema only**|僅限結構描述的預存程序。|  
|**僅限同義字架構**|僅限結構描述的同義字。|  
|**view schema only**|只含結構描述的檢視。|  
...
`[ @status = ] 'status'`這是發行項的狀態。 *狀態*是**Nvarchar （10）**，預設值是未**同步**處理。 如果**使用**中，則會執行發行資料表的初始處理腳本。 如果未**同步**處理，下次執行快照集代理程式時，就會執行發行資料表的初始處理腳本。  
