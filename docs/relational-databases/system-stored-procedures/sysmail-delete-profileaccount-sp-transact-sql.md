...
`[ @profile_id = ] profile_id` 要刪除之設定檔的設定檔識別碼。 *profile_id* 是 **int**，預設值是 Null。 您可以指定 *profile_id* 或 *profile_name* 。  
  
`[ @profile_name = ] 'profile_name'` 要刪除之設定檔的設定檔名稱。 *profile_name* 是 **sysname**，預設值是 Null。 您可以指定 *profile_id* 或 *profile_name* 。  
  
`[ @account_id = ] account_id` 要刪除的帳戶識別碼。 *account_id* 是 **int**，預設值是 Null。 您可以指定 *account_id* 或 *account_name* 。  
  
`[ @account_name = ] 'account_name'` 要刪除的帳戶名稱。 *account_name* 是 **sysname**，預設值是 Null。 您可以指定 *account_id* 或 *account_name* 。  
...
## <a name="remarks"></a>備註  
 ...
 預存程式 **sysmail_delete_profileaccount_sp** 位於 **msdb** 資料庫中，而且是由 **dbo** 架構所擁有。 如果目前的資料庫不是 **msdb**，就必須以三部分名稱執行程式。  
