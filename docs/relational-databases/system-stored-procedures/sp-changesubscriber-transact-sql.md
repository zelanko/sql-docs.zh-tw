...
`[ @type = ] type`這是訂閱者類型。 *類型*是**Tinyint**, 預設值是 Null。 **0** [!INCLUDE[msCoName](../../includes/msconame-md.md)] 表示[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]訂閱者。 **1**指定非[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]或其他 ODBC 資料來源伺服器訂閱者。  
  
`[ @login = ] 'login'`這是[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]驗證登入識別碼。 *login* 是預設值為 NULL 的 **sysname**。  
  
`[ @password = ] 'password'`這是[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]驗證密碼。 *password*是**sysname**, 預設值 **%** 是。 **%** 表示密碼屬性沒有任何變更。  
...
`[ @frequency_type = ] frequency_type`這是散發工作的排程頻率。 *frequency_type*是**int**, 而且可以是下列其中一個值。  
...
`[ @frequency_recurrence_factor = ] frequency_recurrence_factor`這是在定義的*frequency_type*期間, 散發工作應該重複發生的頻率。 *frequency_recurrence_factor*是**int**, 預設值是 Null。  
...
`[ @description = ] 'description'`這是選擇性的文字描述。 *description*是**Nvarchar (255)** , 預設值是 Null。  
...
## <a name="permissions"></a>Permissions  
 只有**系統管理員 (sysadmin** ) 固定伺服器角色的成員, 才能夠執行**sp_changesubscriber**。  
