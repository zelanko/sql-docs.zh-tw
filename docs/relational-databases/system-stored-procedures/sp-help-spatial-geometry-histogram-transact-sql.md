...
`[ @sample = ] 'sample'` 這是使用的資料表百分比。 有效的值是從0到100。 *範例* 為 **浮點數**。 預設值為 100。  
...
|**細胞**|**幾何**|這是表示每個資料格的矩形多邊形。 資料格形狀與空間索引所使用的資料格形狀相同。|  
...
## <a name="remarks"></a>備註  
 ...
 結果集內可能會加入額外的資料列，以保留週框方塊之外或觸及週框方塊框線的物件數目。 此資料列的 **cellid** 為0，而此資料列的資料 **格** 包含表示周框方塊的 **LineString** 。 這個資料列代表週框方塊外的整個空間。  
