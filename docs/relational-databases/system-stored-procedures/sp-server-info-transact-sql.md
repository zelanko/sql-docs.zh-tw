...
|**達**|COLLATION_SEQ<br /><br /> 指定這部伺服器的字元集排序。|charset=iso_1 sort_order=dictionary_iso charset_num=1 sort_order_num=51|  
|**診斷**|SAVEPOINT_SUPPORT<br /><br /> 指定基礎 DBMS 是否支援具名儲存點。|Y|  
|**20**|MULTI_RESULT_SETS<br /><br /> 指定基礎資料庫或閘道本身是否支援多個結果集 (可以透過閘道傳送多個陳述式，將多個結果集傳回用戶端)。|Y|  
|**22**|ACCESSIBLE_TABLES<br /><br /> 指定是否在 **sp_tables**中，閘道只會傳回目前使用者可存取的資料表、視圖等等， (也就是至少具有資料表) 之 SELECT 許可權的使用者。|Y|  
...
|**104**|ACCESSIBLE_SPROC<br /><br /> 指定是否在 **sp_stored_procedures**中，閘道只會傳回目前使用者可執行檔預存程式。|Y|  
...
## <a name="remarks"></a>備註  
 **sp_server_info** 傳回 ODBC 中 **SQLGetInfo** 所提供的資訊子集。  
