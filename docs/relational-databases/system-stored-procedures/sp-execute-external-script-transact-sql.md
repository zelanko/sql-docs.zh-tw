...
`[ @input_data_1_name = N'input_data_1_name' ]` 指定用來表示 @input_data_1 所定義之查詢的變數名稱。 外部腳本中變數的資料類型取決於語言。 如果是 R，則輸入變數是資料框架。 在 Python 的案例中，輸入必須是表格式。 *input_data_1_name*是**sysname**。  預設值為*InputDataSet*。  
...
`[ @output_data_1_name =  N'output_data_1_name' ]` 指定外部腳本中的變數名稱，其中包含完成預存程序呼叫時，要傳回給 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的資料。 外部腳本中變數的資料類型取決於語言。 針對 R，輸出必須是資料框架。 針對 Python，輸出必須是 pandas 的資料框架。 *output_data_1_name*是**sysname**。  預設值為*OutputDataSet*。  
...
`[ @parallel = 0 | 1 ]` 會藉由將 `@parallel` 參數設定為1，以啟用 R 腳本的平行執行。 這個參數的預設值為0（沒有平行處理原則）。 如果 `@parallel = 1`，而且輸出是直接串流處理到用戶端電腦，則需要 `WITH RESULT SETS` 子句，而且必須指定輸出架構。  
 ...
 + 對於使用 RevoScaleR 函數的 R 腳本，平行處理會自動處理，您不應該指定**sp_execute_external_script**呼叫的 `@parallel = 1`。  
...
除了傳回結果集之外，您還可以使用 OUTPUT 參數，將純量值傳回給 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 
...
串流處理可讓 Python 或 R 腳本使用比記憶體中容納更多的資料。 若要控制在串流處理期間傳遞的資料列數目，請在 `@params` 集合中為參數指定整數值，`@r_rowsPerRead`。  例如，如果您要針對使用非常廣泛資料的模型進行定型，您可以調整此值以讀取較少的資料列，以確保所有資料列都可以在一區塊資料中傳送。 您也可以使用此參數來管理一次讀取和處理的資料列數目，以減輕伺服器效能問題。 
  
串流處理的 `@r_rowsPerRead` 參數和 `@parallel` 引數都應該視為提示。 若要套用提示，必須產生包含平行處理的 SQL 查詢計劃。 如果無法這麼做，就無法啟用平行處理。  
...
下列範例會建立一個預存程式，使用**sp_execute_external_script**將 R 所包含的鳶尾花資料集傳回給 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。  
...
下列範例會建立一個預存程式，使用**sp_execute_external_script**來產生鳶尾花模型，並將模型傳回給 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。  
...
+ [SQL Server 機器學習服務](../../advanced-analytics/index.yml)
