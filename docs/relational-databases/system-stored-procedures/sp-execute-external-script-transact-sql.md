...
**Sp_execute_external_script**預存程式會執行提供做為程式之輸入引數的腳本，並搭配 SQL Server 2017 上的[Machine Learning Services](../../machine-learning/sql-server-machine-learning-services.md)使用。
...
**Sp_execute_external_script**預存程式會執行提供做為程式之輸入引數的腳本，並搭配 SQL Server 2016 上的[R 服務](../../machine-learning/r/sql-server-r-services.md)使用。
...
**Sp_execute_external_script**預存程式會執行提供做為程式之輸入引數的腳本，並用於[Azure SQL 受控執行個體中的 Machine Learning 服務](/azure/azure-sql/managed-instance/machine-learning-services-overview)。
...
::: moniker-end
::: moniker range="=sql-server-2016||=sqlallproducts-allversions"
## <a name="syntax-for-sql-server-2017-and-earlier"></a>SQL Server 2017 和更早版本的語法
...
`[ @parallel = 0 | 1 ]`藉由將參數設定為1，以啟用 R 腳本的平行執行 `@parallel` 。 這個參數的預設值為0（沒有平行處理原則）。 如果 `@parallel = 1` 和輸出是直接串流處理到用戶端電腦，則 `WITH RESULT SETS` 需要子句，而且必須指定輸出架構。  
 ...
 + 對於使用 RevoScaleR 函式的 R 腳本，平行處理會自動處理，您不應該將指定 `@parallel = 1` 為**sp_execute_external_script**呼叫。  
...
::: moniker-end
::: moniker range="=azuresqldb-mi-current||=sqlallproducts-allversions"
使用**sp_execute_external_script**執行以支援的語言撰寫的腳本。 支援的語言為 Azure SQL 受控執行個體 Machine Learning 服務中的**Python**和**R** 。
::: moniker-end

根據預設，這個預存程式所傳回的結果集是具有未命名資料行的輸出。 腳本內使用的資料行名稱在腳本環境中是本機的，而且不會反映在輸出結果集中。 若要命名結果集資料行，請使用的 `WITH RESULT SET` 子句 [`EXECUTE`](../../t-sql/language-elements/execute-transact-sql.md) 。

除了傳回結果集之外，您還可以將純量值傳回給使用 OUTPUT 參數。

::: moniker range=">=sql-server-2016||>=sql-server-linux-ver15||=sqlallproducts-allversions"
您可以藉由設定外部資源集區來控制外部腳本所使用的資源。 如需詳細資訊，請參閱[&#40;transact-sql&#41;建立外部資源集](../../t-sql/statements/create-external-resource-pool-transact-sql.md)區。 您可以從資源管理員目錄檢視、DMV 和計數器取得工作負載的相關資訊。 如需詳細資訊，請參閱[Resource Governor 目錄檢視 &#40;transact-sql&#41;](../../relational-databases/system-catalog-views/resource-governor-catalog-views-transact-sql.md)、 [Resource Governor 相關的動態管理檢視 &#40;transact-sql&#41;](../../relational-databases/system-dynamic-management-views/resource-governor-related-dynamic-management-views-transact-sql.md)和[SQL Server，外部腳本物件](../../relational-databases/performance-monitor/sql-server-external-scripts-object.md)。  
::: moniker-end
...
這兩個參數**input_data_1_partition_by_columns**和**input_data_1_order_by_columns**，其中第二個參數是用來排序結果集。 參數會當做輸入傳遞給，而 `sp_execute_external_script` 外部腳本會針對每個分割區執行一次。 如需詳細資訊和範例，請參閱[教學課程：建立以資料分割為基礎的模型](https://docs.microsoft.com/sql/machine-learning/tutorials/r-tutorial-create-models-per-partition)。
...
串流處理可讓 Python 或 R 腳本使用比記憶體中容納更多的資料。 若要控制在串流處理期間傳遞的資料列數目，請在集合中為參數指定整數值 `@r_rowsPerRead` `@params` 。  例如，如果您要針對使用非常廣泛資料的模型進行定型，您可以調整此值以讀取較少的資料列，以確保所有資料列都可以在一區塊資料中傳送。 您也可以使用此參數來管理一次讀取和處理的資料列數目，以減輕伺服器效能問題。 
  
`@r_rowsPerRead`串流處理的參數和 `@parallel` 引數都應該視為提示。 若要套用提示，必須產生包含平行處理的 SQL 查詢計劃。 如果無法這麼做，就無法啟用平行處理。  
...
因應措施是將資料行或值**轉換**成中支援的類型， [!INCLUDE[tsql](../../includes/tsql-md.md)] 然後再將它傳送至外部腳本。  
...
一般而言，任何無法對應至資料類型的結果集 [!INCLUDE[tsql](../../includes/tsql-md.md)] 都會輸出為 Null。  
...
下列範例會建立一個預存程式，使用**sp_execute_external_script**來傳回 R 所包含的鳶尾花資料集。  
...
這個範例說明如何使用 `sp_execute_external_script` ，在簡單的 Python 模型上產生分數。
...
下列範例會建立一個預存程式，使用**sp_execute_external_script**來產生鳶尾花模型並傳回模型。  

> [!NOTE]
> 這個範例需要預先安裝 e1071 套件。 如需詳細資訊，請參閱[在 SQL Server 上安裝其他 R 套件](../../machine-learning/package-management/install-additional-r-packages-on-sql-server.md)。
