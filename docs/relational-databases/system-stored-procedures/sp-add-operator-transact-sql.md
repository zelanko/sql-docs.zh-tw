...
`[ @weekday_pager_end_time = ] weekday_pager_end_time`這段時間之後, **SQLServerAgent**服務就不會再從星期一到星期五的工作日, 將傳呼通知傳送給指定的操作員。 *weekday_pager_end_time*是**int**, 預設值是 180000, 表示 6:00 P.M。 必須用 HHMMSS 格式來輸入。  
  
`[ @saturday_pager_start_time = ] saturday_pager_start_time`這段時間之後, **SQLServerAgent**服務會在星期六將頁面通知傳送給指定的操作員。 *saturday_pager_start_time*是**int**, 預設值是 090000, 表示 9:00 A.M。 必須用 HHMMSS 格式來輸入。  
