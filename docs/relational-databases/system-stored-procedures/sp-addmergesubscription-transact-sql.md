...
`[ @subscription_type = ] 'subscription_type'` 這是訂閱的類型。 *subscription_type*是 **Nvarchar (15) **，預設值是 PUSH。 如果發送，則會加入發送 **訂閱，並**在散發者端新增合併代理程式。 如果 **提取**，則會新增提取訂閱，而不在散發者端新增合併代理程式。  
  
> [!NOTE]  
>  匿名訂閱不需要使用這個預存程序。  
  
`[ @subscriber_type = ] 'subscriber_type'` 這是訂閱者的類型。 *subscriber_type*是 **Nvarchar (15) **，它可以是下列值之一。  
...
`[ @frequency_type = ] frequency_type` 這是一個值，指出合併代理程式將在何時執行。 *frequency_type* 是 **int**，而且可以是下列其中一個值。  
...
`[ @frequency_relative_interval = ] frequency_relative_interval` 這是每個月排程的頻率間隔合併出現次數。 *frequency_relative_interval* 是 **int**，而且可以是下列值之一。  
...
`[ @frequency_subday_interval = ] frequency_subday_interval` 這是在每個合併之間進行 *frequency_subday* 的頻率。 *frequency_subday_interval* 是 **int**，預設值是 Null。  
...
`[ @description = ] 'description'` 這是此合併訂閱的簡短描述。 *描述*是 **Nvarchar (255) **，預設值是 Null。 複寫監視器會在 [ **易記名稱** ] 資料行中顯示這個值，可用來排序受監視發行集的訂閱。  
  
`[ @enabled_for_syncmgr = ] 'enabled_for_syncmgr'` 指定訂閱是否可以透過 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Windows 同步處理管理員進行同步處理。 *enabled_for_syncmgr* 是 **Nvarchar (5) **，預設值是 FALSE。 如果 **為 false**，則表示訂閱未向同步處理管理員註冊。 若 **為 true**，則會向同步處理管理員註冊訂閱，而且可以在不啟動的情況下同步處理 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 。  
...
`[ @offloadserver = ] 'remote_agent_server_name'` 指定要用於遠端代理程式啟用之伺服器的網路名稱。 *remote_agent_server_name*是 **sysname**，預設值是 Null。  
  
`[ @use_interactive_resolver = ] 'use_interactive_resolver'` 允許以互動方式解決允許互動式解決之所有發行項的衝突。 *use_interactive_resolver* 是 **Nvarchar (5) **，預設值是 FALSE。  
  
`[ @merge_job_name = ] 'merge_job_name'`* \@ Merge_job_name*的參數已被取代，無法設定。 *merge_job_name* 是 **sysname**，預設值是 Null。  
 ...
 **sp_addmergesubscription** 用於合併式複寫中。  
 ...
 當**系統管理員（sysadmin** ）固定伺服器角色的成員執行**sp_addmergesubscription**來建立發送訂閱時，會以隱含方式建立合併代理程式作業，並在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 服務帳戶下執行。 建議您執行[sp_addmergepushsubscription_agent](../../relational-databases/system-stored-procedures/sp-addmergepushsubscription-agent-transact-sql.md) ，並為** \@ job_login**和** \@ Job_password**指定不同的代理程式特定 Windows 帳戶的認證。 如需詳細資訊，請參閱 [複寫代理程式安全性模型](../../relational-databases/replication/security/replication-agent-security-model.md)。  
...
## <a name="see-also"></a>另請參閱  
 ...
 [互動式衝突解決](../../relational-databases/replication/merge/advanced-merge-replication-conflict-interactive-resolution.md)   
