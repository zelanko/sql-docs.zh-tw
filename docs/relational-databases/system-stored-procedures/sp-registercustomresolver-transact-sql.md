...
`[ @is_dotnet_assembly = ] 'is_dotnet_assembly'`指定要註冊之自訂商務邏輯的類型。 *is_dotnet_assembly*是**Nvarchar （50）**，預設值是 FALSE。 **true**表示要註冊的自訂商務邏輯是商務邏輯處理常式元件;**false**表示它是 COM 元件。  
