...
> [!NOTE]  
>  有些額外的系統預存程式只會在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 實例或透過用戶端 Api 使用，而非供一般客戶使用。 這些程式列于[系統預存程式（transact-sql）](https://msdn.microsoft.com/library/ms187961.aspx)。 這些程式可能會變更，而且不保證相容性。 清單中的所有程式都無法在 [!INCLUDE[ssSDW](../../includes/sssdw-md.md)]中使用。  
  
## <a name="see-also"></a>另請參閱  
 [系統預存&#40;函數 transact-sql&#41; ](~/relational-databases/system-functions/system-functions-category-transact-sql.md)   
