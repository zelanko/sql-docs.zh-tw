...
## <a name="arguments"></a>引數  
 下表示范 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)]中支援的引數和選項。  
...
## <a name="remarks"></a>Remarks  
 ...
 當您新增伺服器層級防火牆設定，其中的開始和結束 IP 位址等於 `0.0.0.0`時，您可以從 Azure 啟用 [!INCLUDE[ssSDS](../../includes/sssds-md.md)] 伺服器的存取權。 提供*name*參數的值，可協助您記住伺服器層級防火牆設定的用途。  
...
## <a name="examples"></a>範例  
 下列程式碼會建立名為 `Allow Azure` 的伺服器層級防火牆設定，以啟用 Azure 的存取。 在虛擬 master 資料庫中執行下列動作。  
...
```  
 ...
 以下程式碼只會針對 IP 位址 `Example setting 1` 建立名為 `0.0.0.2` 的伺服器層級防火牆設定。 然後，再次呼叫 `sp_set_firewall_rule` 預存程式，將該防火牆設定中的結束 IP 位址更新為 `0.0.0.4`。 這會建立一個範圍，允許 `0.0.0.2`、`0.0.0.3`和 `0.0.0.4` 的 IP 位址存取伺服器。  
