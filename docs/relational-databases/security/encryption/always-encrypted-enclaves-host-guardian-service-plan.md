---
title: 規劃主機守護者服務證明 | Microsoft Docs
ms.custom: ''
ms.date: 10/12/2019
ms.prod: sql
ms.reviewer: vanto
...
SQL 用戶端和 [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)] 都必須能夠透過 HTTP 與 HGS 通訊。 您可以選擇性地使用 TLS 憑證來設定 HGS，以支援加密的 HTTPS 連線。
...
TPM 證明需要安全開機，以確保 UEFI 載入 Microsoft 簽署的合法開機載入器，且不會有 Rootkit 或 Bootkit 攔截 Hypervisor 開機程序。
...
| RAM | 8 GB (最小值) |
| NIC | 建議使用 2 個 NIC (1 個用於叢集流量，1 個用於 HGS 服務) |
...
- 虛擬化技術的 CPU 支援：
  ...
  - 如果您是在 VM 中執行 [!INCLUDE [ssnoversion-md](../../../includes/ssnoversion-md.md)]，則 Hypervisor 和實體 CPU 必須提供巢狀虛擬化功能。 如需在 VM 中執行 VBS 記憶體保護區時的保證資訊，請參閱[＜信任模型＞](#trust-model)一節。
    - 在 Hyper-V 2016 或更新版本中，[於 VM 處理器上啟用巢狀虛擬化延伸模組](https://docs.microsoft.com/virtualization/hyper-v-on-windows/user-guide/nested-virtualization#configure-nested-virtualization)。
