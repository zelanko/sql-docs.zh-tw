---
title: "Azure SQL Database 的透明資料加密 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
  - "MSDN content"
  - "MSDN - SQL DB"
...
[!INCLUDE[tsql-appliesto-xxxxxx-asdb-asdw-xxx_md](../../../includes/tsql-appliesto-xxxxxx-asdb-asdw-xxx-md.md)]
 ...
 [!INCLUDE[ssSDSFull](../../../includes/sssdsfull-md.md)] 不支援 Azure 金鑰保存庫與 TDE 整合。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 可以使用金鑰保存庫中的非對稱金鑰。 如需詳細資訊，請參閱[使用 Azure 金鑰保存庫進行可延伸金鑰管理 &#40;SQL Server&#41;](../../../relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server.md)。  
...
6.  在 [資料加密] 刀鋒視窗中，將 [資料加密] 按鈕移至 [開啟]，然後按一下 [儲存] (位於頁面頂端)，以套用此設定。 [加密狀態]  會顯示透明資料加密的概略進度。  
     ...
     您也可以使用查詢工具 (例如 [!INCLUDE[ssSDS](../../../includes/sssds-md.md)] )，以具有 [!INCLUDE[ssManStudioFull](../../../includes/ssmanstudiofull-md.md)] VIEW DATABASE STATE **權限的資料庫使用者身分，連接到** 監視加密進度。 查詢 [sys.dm_database_encryption_keys](../../../relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql.md) 檢視的 **encryption_state** 資料行。  
...
3.  若要監視 [!INCLUDE[ssSDS](../../../includes/sssds-md.md)] 的加密進度，具有 **VIEW DATABASE STATE** 權限的資料庫使用者可以查詢 [sys.dm_database_encryption_keys](../../../relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql.md) 檢視的 **encryption_state** 資料行。  
