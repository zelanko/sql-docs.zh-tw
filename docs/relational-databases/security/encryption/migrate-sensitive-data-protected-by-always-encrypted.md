---
title: "移轉透過永遠加密保護的機密資料 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "11/04/2015"
...
---
# 移轉透過永遠加密保護的機密資料
  若要在大量複製作業期間載入加密的資料而不需在伺服器上執行中繼資料檢查，請使用 **ALLOW_ENCRYPTED_VALUE_MODIFICATIONS** 選項來建立使用者。 此選項適用於比 [!INCLUDE[ssSQL15](../../../includes/sssql15-md.md)] 還舊之 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 版本的舊版工具 (例如 bcp.exe)，或者利用無法使用「永遠加密」的協力廠商擷取-轉換-載入 (ETL) 工作流程來使用。 這可讓使用者將加密的資料從某一組資料表 (包含加密的資料行) 安全地移至另一組具有加密資料行 (位於相同或不同的資料庫) 的資料表。  
