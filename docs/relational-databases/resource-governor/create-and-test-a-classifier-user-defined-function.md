...
4.  建立使用時間函數和值 (可針對查閱資料表中的時間評估) 的分類函數。 如需在分類函式使用中使用查閱資料表的詳細資訊，請參閱本主題中的＜在分類函式中使用查閱資料表的最佳做法＞。  
    ...
    > [!NOTE]  
    >  [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 導入了一組擴充的日期與時間資料類型和函數。 如需詳細資訊，請參閱[日期和時間資料類型與函數 &#40;Transact-SQL&#41;](../../t-sql/functions/date-and-time-data-types-and-functions-transact-sql.md)。  
...
1.  除非絕對必要，否則不要使用查閱資料表。 如果您需要使用查閱資料表，則查閱資料表可能以硬式編碼的方式將本身寫入函式中；不過查閱資料表需要與分類函數的複雜度和動態變更相互平衡。  
  
2.  限制針對查閱資料庫執行的 I/O。  
  
    1.  使用 `TOP 1` 以僅傳回一個資料列。  
...
3.  避免在查閱資料表上進行封鎖。  
    ...
    1.  使用 `NOLOCK` 提示避免鎖定，或在函式中使用最大值 1000 毫秒的 `SET LOCK_TIMEOUT`。  
  
    2.  資料表必須在 master 資料庫中 (master 資料庫是在用戶端電腦嘗試連線時，唯一保證可以復原的資料庫)。  
  
    3.  一律以結構描述來完整限定資料表名稱。 資料庫名稱並非必要，因為一定要是 master 資料庫。  
  
    4.  資料表上沒有觸發程序。  
  
    5.  如果您要更新資料表內容，則務必在分類函式中使用快照隔離等級交易，以避免寫入器封鎖讀取器。 請注意，使用 `NOLOCK` 提示也可以防止這種情況發生。  
