...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx-md](../../includes/applies-to-version/sqlserver.md)]
 ...
 當來源資料表啟用異動資料擷取時，就會建立此列舉函數。 函式名稱是衍生的，並且使用**cdc. 的格式 fn_cdc_get_all_changes_**_capture_instance_其中*capture_instance*是當來源資料表啟用變更資料捕獲時，為 capture 實例指定的值。  
 ...
 *from_lsn*  
 LSN 值，代表要包含在結果集之 LSN 範圍的低端點。 *from_lsn*為**binary （10）**。  
 ...
 *to_lsn*  
 LSN 值，代表要包含在結果集之 LSN 範圍的高端點。 *to_lsn*為**binary （10）**。  
...
## <a name="permissions"></a>權限  
 需要**系統管理員（sysadmin** ）固定伺服器角色或**db_owner**固定資料庫角色中的成員資格。 若為所有其他使用者，則需要來源資料表中所有擷取資料行的 SELECT 權限，而且如果定義了擷取執行個體的控制角色，便需要該資料庫角色的成員資格。 當呼叫端沒有查看來源資料的許可權時，此函數會傳回錯誤229（「物件 ' fn_cdc_get_all_changes_ ... ' 的 SELECT 許可權遭到拒絕，資料庫 ' \<DatabaseName> '，架構 ' cdc '。」）。  
  
## <a name="remarks"></a>備註  
 如果指定的 LSN 範圍不在擷取執行個體的變更追蹤時間表內，此函數會傳回錯誤 208 (「提供給程序或函數 cdc.fn_cdc_get_all_changes 的引數數量不足」)。  
  
 當 **__ $ operation** = 1 或 **__ $ operation** = 3 時，資料類型為**image**、 **text**和**Ntext**的資料行一律會被指派 Null 值。 除非資料行在更新期間變更，否則當 **__ $ operation** = 3 時，會指派 Null 值給**Varbinary （max）**、 **Varchar （max）** 或**Nvarchar （max）** 資料類型的資料行。 當 **__ $ operation** = 1 時，這些資料行會在刪除時指派其值。 包含在擷取執行個體中的計算資料行，一律使用 NULL 值。  
  
## <a name="examples"></a>範例  
 有數個 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 可用來示範如何使用變更資料捕獲查詢函數的範本。 這些範本可在的 [ **View** ] 功能表中取得 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 。 如需詳細資訊，請參閱[範本 Explorer](../../ssms/template/template-explorer.md)。  
  
 此範例示範 `Enumerate All Changes for Valid Range Template`。 它會使用函數 `cdc.fn_cdc_get_all_changes_HR_Department` 來報告 `HR_Department` 針對資料庫中的來源資料表 HumanResources 定義的 capture 實例所有目前可用的變更 [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] 。  
