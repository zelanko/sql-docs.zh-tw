...
## <a name="permissions"></a>權限  
 需要**系統管理員（sysadmin** ）固定伺服器角色或**db_owner**固定資料庫角色中的成員資格。 若為所有其他使用者，則需要來源資料表中所有擷取資料行的 SELECT 權限，而且如果定義了擷取執行個體的控制角色，便需要該資料庫角色的成員資格。 當呼叫端沒有查看來源資料的許可權時，此函數會傳回錯誤229（「物件 ' fn_cdc_get_all_changes_ ... ' 的 SELECT 許可權被拒，資料庫 '\<DatabaseName> '，架構 ' cdc '。」）。  
...
## <a name="examples"></a>範例  
 有[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]數個可用來示範如何使用變更資料捕獲查詢函數的範本。 這些範本可在的 [ **View** ] 功能表[!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]中取得。 如需詳細資訊，請參閱[範本 Explorer](../../ssms/template/template-explorer.md)。  
  
 此範例示範 `Enumerate All Changes for Valid Range Template`。 它會使用函數`cdc.fn_cdc_get_all_changes_HR_Department`來報告針對[!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)]資料庫中的來源資料表 HumanResources 定義的`HR_Department`capture 實例所有目前可用的變更。  
