 ...
 **Datetime**值，表示要包含在結果集內之變更資料表專案範圍的低端點。  
 ...
 只有 < capture_instance 中的資料列 > _CT 變更資料表的關聯認可時間大於*start_time* ，才會包含在結果集中。  
 ...
 **Datetime**值，表示要包含在結果集內之變更資料表專案範圍的高階點。  
 ...
 這個參數可能會根據針對 @closed_high_end_point 所選擇的值，來接受兩個可能意義的其中一個，這是在呼叫 sp_cdc_generate_wrapper_function 來產生包裝函式的 create 腳本時：  
  
-   @closed_high_end_point = 1  
  
     只有位於 cdc. < capture_instance > _CT 變更資料表的資料列才會包含在結果集中，而這些資料集具有小於或等於 end_time 的相關聯認可時間。  
...
|@column_list的 \<資料行 >|**差異**|當呼叫它來產生建立包裝函式的腳本時，sp_cdc_generate_wrapper_function 的*column_list*引數中所識別的資料行。|  
...
2.  執行指令碼來實際建立包裝函數。  
 ...
 包裝函式可讓使用者有系統地查詢在以**datetime**值（而不是 LSN 值）限制的間隔內發生的變更。 包裝函式會在提供的**datetime**值與內部所需的 LSN 值之間執行所有必要的轉換，做為查詢函數的引數。 當包裝函式使用順序來處理變更資料的資料流程時，它們會確保不會遺失或重復資料，但前提是遵循下列慣例：與一個呼叫相關聯之間隔的 @end_time 值，會當做與後續呼叫相關聯之間隔的 @start_time 值提供。  
  
 在建立指令碼時使用 @closed_high_end_point 參數，您便可以產生包裝函式來支援指定之查詢視窗上的封閉上限或開放上限。 也就是說，您可以決定具有認可時間的項目是否等於要包含在間隔內之擷取間隔的上限。 預設會包含上限。  
  
 「**所有變更**」包裝函式所傳回的結果集會傳回 __ $ start_lsn，而且 \_\_變更資料表 $seqval 資料行分別 \__CDC_STARTLSN 和 \__CDC_SEQVAL。 只有在產生包裝函式時，出現在 *\@column_list*參數中的追蹤資料行才會遵循這些步驟。 如果 *\@column_list*為 Null，則會傳回所有追蹤的來源資料行。 來源資料行後面接著 operation 資料行，\__CDC_OPERATION，這是可識別作業的一或兩個字元資料行。  
