...
-   **變更時封鎖** - 如果為 **True**，當資料庫結構描述不同於舊版 DAC 中所定義的資料庫結構描述時，就會結束升級。 如果為 **False**，即使偵測到變更，升級仍會繼續。 預設值是 **False**。  
  
-   **失敗時回復** - 如果為 **True**，升級將會包含在交易中，而如果發生錯誤，則會嘗試回復。 如果為 **False**，將會認可所有變更，而如果發生錯誤，您可能必須還原資料庫的先前備份。 預設值是 **False**。  
  
-   **略過原則驗證** - 如果為 **True**，則不會評估 DAC 伺服器選取原則。 如果為 **False**，則會評估原則，而如果發生驗證錯誤，則會結束升級。 預設值是 **False**。  
