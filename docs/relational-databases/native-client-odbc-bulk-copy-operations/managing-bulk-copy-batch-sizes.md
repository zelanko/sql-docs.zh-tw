...
-   如果沒有指定 TABLOCK，則將批次大小限制為小於 1,000 資料列。  
 ...
 除了指定交易大小外，批次也會影響資料列何時會透過網路傳送到伺服器。 大量複製函數通常會快取**bcp_sendrow**中的資料列，直到網路封包填滿為止，然後將完整封包傳送至伺服器。 不過，當應用程式呼叫**bcp_batch**時，不論是否已填入，都會將目前的封包傳送至伺服器。 使用很小的批次大小如果造成傳送許多部分填滿的封包到伺服器，則可能會使效能降低。 例如，在每個**bcp_sendrow**之後呼叫**bcp_batch**會導致個別封包中傳送每個資料列，而除非資料列很大，否則會浪費每個封包中的空間。 SQL Server 的網路封包的預設大小是 4 KB，雖然應用程式可以藉由呼叫指定 SQL_ATTR_PACKET_SIZE 屬性的[SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md)來變更大小。  
  
 批次的另一個副作用是，每個批次都會被視為未完成的結果集，直到使用**bcp_batch**完成為止。 當批次未完成時，如果在連接控制碼上嘗試任何其他作業，則 @no__t 0 Native Client ODBC 驅動程式會發出錯誤，其中包含 SQLState = "HY000"，且錯誤訊息字串為：  
