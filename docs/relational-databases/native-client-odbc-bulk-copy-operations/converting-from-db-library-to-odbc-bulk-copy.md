...
-   DB-LIBRARY 應用程式會在連接之前呼叫 **BCP_SETL** ，以啟用 DBPROCESS 上的大量複製作業。 ODBC 應用程式會在連接之前呼叫 [SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) ，以在連接控制碼上啟用大量作業：  
...
-   **Bcp_control**選項 BCPMAXERRS 指定大量複製作業失敗之前，有多少個別的資料列可能有錯誤。 BCPMAXERRS 的預設值為 0 (第一次發生錯誤時失敗) **bcp_control** 的 db-library 版本中為，而在 ODBC 版本中則為10。 相依于預設值0以終止大量複製作業的 DB-LIBRARY 應用程式必須變更為呼叫 ODBC **bcp_control** ，以便將 BCPMAXERRS 設定為0。  
  
-   ODBC **bcp_control** 函數支援下列 **bcp_control**的 db-library 版本不支援的選項：  
...
-   在 ODBC 版本的大量複製函式中，以字元字串處理 **datetime** 和 **Smalldatetime** 值的格式是 yyyy-mm-dd hh： MM： ss 的 odbc 格式。 **Smalldatetime** 值使用格式為 yyyy-mm-dd hh： mm： SS 的 ODBC 格式。  
     ...
     在使用數種格式的字元字串中，大量複製函數的 DB-LIBRARY 版本會接受 **datetime** 和 **Smalldatetime** 值：  
  
    -   預設格式為 *mmm dd yyyy hh： mmxx* ，其中 *xx* 是 AM 或 PM。  
  
    -   **datetime** 和 **Smalldatetime** 字元字串，以 db-library **dbconvert** 函數支援的任何格式。  
...
-   以字元格式輸出 **money** 值時，ODBC 大量複製函式提供四位數的有效位數，而且沒有逗號分隔符號;DB-LIBRARY 版本只提供兩位數的精確度，並包含逗號分隔符號。  
