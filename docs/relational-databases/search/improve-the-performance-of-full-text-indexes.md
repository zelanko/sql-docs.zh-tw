...
ms.topic: "article"
helpviewer_keywords: 
  - "效能 [SQL Server], 全文檢索搜尋"
  - "全文檢索查詢 [SQL Server], 效能"
  - "搜耙 [全文檢索搜尋]"
  - "全文檢索索引 [SQL Server], 效能"
  - "全文檢索搜尋 [SQL Server], 效能"
  - "批次 [SQL Server], 全文檢索搜尋"
...
-   請將 [sp_configure](../../relational-databases/system-stored-procedures/sp-configure-transact-sql.md)‘**max full-text crawl ranges**’ 設為系統上的 CPU 數目，以使用所有處理器或核心來發揮最大效能。 此組態選項的相關資訊，請參閱[全文檢索搜耙最大範圍伺服器組態選項](../../database-engine/configure-windows/max-full-text-crawl-range-server-configuration-option.md)。  
...
-   如果您想要改善累加母體擴展的效能，請在 [時間戳記] 資料行上建立次要索引。  
...
##  <a name="full"></a> 疑難排解：完整母體擴展的效能問題  
 若要診斷效能問題，請查看全文檢索搜耙記錄檔。 如需搜耙記錄檔的相關資訊，請參閱[擴展全文檢索索引](../../relational-databases/search/populate-full-text-indexes.md)。  
...
> [!NOTE]  
>  請參閱 [sys.dm_fts_memory_buffers &#40;Transact-SQL&#41;](../../relational-databases/system-dynamic-management-views/sys-dm-fts-memory-buffers-transact-sql.md)，以取得全文檢索搜耙所使用的記憶體緩衝區的相關資訊。  
...
|x64|*F* **=** *Number of crawl ranges* **\*** 10 **\*** 8|*M* **=** *T* **–** *F* **–** 500|  
 ...
 (2) 500 MB 是系統中其他處理序所需記憶體的估計。 如果系統正在進行其他工作，請據此增加這個值。  
 ...
 **範例：估計 fdhost.exe 的記憶體需求**  
 ...
 下一個計算會取得 [最大伺服器記憶體] 的最佳值—*M*。 此系統上可用的實體記憶體總計 (以 MB 為單位)—*T*—是`8192`。  
  
 `M = 8192-640-500=7052`  
  
 **範例：設定 max server memory**  
  
 此範例使用 [sp_configure](../../relational-databases/system-stored-procedures/sp-configure-transact-sql.md) 及 [RECONFIGURE](../../t-sql/language-elements/reconfigure-transact-sql.md)[!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式，將 [最大伺服器記憶體] 設為上述範例中計算的 *M* 值，`7052`：  
...
##  <a name="filters"></a> 疑難排解：索引效能因篩選而變慢  
 ...
 若要解決這個問題，請將容器文件 (在此範例中是 Word) 的篩選標示成單一執行緒篩選。 您可以變更篩選登錄值，以便將給定的篩選標示成單一執行緒篩選。 您必須將篩選的 **ThreadingModel** 登錄值設為 **Apartment Threaded**，以將此篩選標示成單一執行緒篩選。 如需有關單一執行緒 Apartment 的詳細資訊，請參閱 [Understanding and Using COM Threading Models](http://go.microsoft.com/fwlink/?LinkId=209159) (了解與使用 COM 執行緒模型) 技術白皮書。  
