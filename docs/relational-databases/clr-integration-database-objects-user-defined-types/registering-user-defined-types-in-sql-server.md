---
title: 在 SQL 伺服器註冊使用者定義類型 :微軟文件
description: 在 SQL 伺服器中安裝 UDT 之前,必須註冊 UDT。 您必須註冊程式集,並在資料庫中創建要使用的類型。
ms.custom: ''
ms.date: 03/16/2017
ms.prod: sql
ms.reviewer: ''
...
ms.locfileid: "81486912"
---
# <a name="registering-user-defined-types-in-sql-server"></a>在 SQL Server 中註冊使用者定義型別
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
  為了在 中[!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]使用 使用者定義的類型 (UDT), 必須註冊它。 註冊 UDT 包括註冊組件，以及在要使用該型別的資料庫中建立它。 UDT 的使用範圍為單一資料庫，而且除非已經向每個資料庫註冊相同的組件及 UDT，否則無法在多個資料庫中使用。 一旦註冊 UDT 組件並建立此型別之後，您便可在 [!INCLUDE[tsql](../../includes/tsql-md.md)] 及用戶端程式碼中使用該 UDT。 如需詳細資訊，請參閱 [CLR 使用者定義型別](../../relational-databases/clr-integration-database-objects-user-defined-types/clr-user-defined-types.md)。  
...
1.  在**視覺化基本**或**可視化 C++** 語言節點中創建新的**資料庫**專案。  
...
5.  在 **"生成"** 選單中,選擇 **「部署**」 。 如此將會在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料庫中註冊組件並建立型別。  

## <a name="using-transact-sql-to-deploy-udts"></a>使用 Transact-SQL 部署 UDT  
 您可以使用 [!INCLUDE[tsql](../../includes/tsql-md.md)] CREATE ASSEMBLY 語法，於要使用 UDT 的資料庫中註冊組件。 它會儲存於資料庫系統資料表內部，而非檔案系統外部。 如果 UDT 與外部組件相關，也必須將它們載入資料庫。 CREATE TYPE 陳述式可在要使用 UDT 的資料庫內建立 UDT。 有關詳細資訊,請參閱[創建"轉換-SQL&#41;&#40;和](../../t-sql/statements/create-assembly-transact-sql.md)[創建類型&#40;轉算-SQL&#41;。](../../t-sql/statements/create-type-transact-sql.md)  
...
#### <a name="example"></a>範例  
 以下[!INCLUDE[tsql](../../includes/tsql-md.md)]語句使用 SAFE[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]許可權集在**AdventureWorks**資料庫中註冊 Point 程式集。 如果省略 WITH PERMISSION_SET 子句，將會使用 SAFE 權限集合註冊該組件。  
...
```  
 ...
 以下[!INCLUDE[tsql](../../includes/tsql-md.md)]語句使用 FROM 子句中 *<assembly_bits>* 參數來註冊程式集。 此**varbinary**值將檔表示為位元組流。  
...
> [!NOTE]  
>  CREATE TYPE 語法還用於建立[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]本機 別名資料類型,旨在替換**sp_addtype**作為創建別名數據類型的方法。 CREATE TYPE 語法中的某些選擇性引數會參考建立 UDT，且不適用於建立別名資料類型 (如基底類型)。  
  
 有關詳細資訊,請參閱[創建類型&#40;交易-SQL&#41;](../../t-sql/statements/create-type-transact-sql.md)。  
  
#### <a name="example"></a>範例  
 以下[!INCLUDE[tsql](../../includes/tsql-md.md)]語句創建**點**類型。 外部名稱是使用*程式集名稱*的兩部分命名語法指定的。*UDT 名稱*。  
...
### <a name="example"></a>範例  
 下列 [!INCLUDE[tsql](../../includes/tsql-md.md)] 必須以下列順序執行。 首先,必須刪除引用**點**UDT 的表,然後刪除類型,最後刪除程式集。  
...
### <a name="finding-udt-dependencies"></a>尋找 UDT 相依性  
 ...
 以下[!INCLUDE[tsql](../../includes/tsql-md.md)]查詢定位**在 AdventureWorks**資料庫中使用 UDT 的所有列和參數。  
...
### <a name="example"></a>範例  
 當您變更並重新編譯 UDT 組件中的原始程式碼之後，可以使用 ALTER ASSEMBLY 陳述式。 它會將此 .dll 檔案複製到伺服器，並重新繫結至新的組件。 有關完整的語法,請參閱[ALTER ASSEMBLY &#40;转算-SQL&#41;](../../t-sql/statements/alter-assembly-transact-sql.md)。  
...
### <a name="using-alter-assembly-to-add-source-code"></a>使用 ALTER ASSEMBLY 加入原始程式碼  
 ...
 以下[!INCLUDE[tsql](../../includes/tsql-md.md)]ALTER ASSEMBLY 語句添加**點**UDT 的Point.cs類原始程式碼。 這會複製 Point.cs 檔案中包含的文字，並將它儲存在名為 PointSource 的資料庫中。  
 ...
 **file_id**  
 標識每個物件的數位,第一個與給定**assembly_id**關聯的物件被給定值為 1。 如果有多個物件與同一**個assembly_id,** 則每個後續**file_id**值將遞增 1。  
  
 **內容**  
 組件或檔案的十六進位表示法。  
  
 您可以使用 CAST 或 CONVERT 函數將**內容**欄的內容轉換為可讀文字。 下列查詢會將 Point.cs 檔案的內容轉換為可讀取的文字，並在 WHERE 子句中使用此名稱，以將結果集限制為單一資料列。  
 ...
 **AdventureWorks**範例資料庫中**的貨幣**UDT 提供了一個有用的範例,用於建議如何建構 UDT 及其相關功能。 **貨幣**UDT 用於根據特定區域性的貨幣系統處理貨幣,並允許存儲不同的貨幣類型,如美元、歐元等。 UDT 類別將區域性名稱公開為字串,並將金額公開為**小數資料類型**。 所有必要的序列化方法都包含在定義類別的組件中。 實現從一個區域性到另一個區域性的貨幣轉換的函數作為名為**ConvertCurrency**的外部函數實現,並且此函數位於單獨的程式集中。 **ConvertCurrency**函數通過從**AdventureWorks**資料庫中的表檢索轉換率來執行其工作。 如果轉換速率的來源應該更改,或者如果對現有代碼應進行任何其他更改,則可以輕鬆地修改程式集,而不會影響**貨幣**UDT。  
  
 可以通過安裝通用語言執行時 (CLR) 範例找到**貨幣**UDT 和**ConvertCurrency**函數的代碼清單。  
...
-   從某個資料庫資料表 UDT 資料行選取 UDT 資料，並將其插入具有相同 UDT 資料行的第二個資料庫中。  
 ...
 請注意,在[!INCLUDE[ssDEnoversion](../../includes/ssdenoversion-md.md)]**tempdb**系統資料庫中創建工作台時,不需要執行任何使用 UDT 的操作。 這包括處理游標、表變數和使用者定義的表值函數,這些函數包括 UDT 並透明地使用**tempdb**。 但是,如果在**tempdb**中顯示式創建定義 UDT 列的暫時,則必須以與用戶資料庫相同的方式在**tempdb**中註冊 UDT。  
