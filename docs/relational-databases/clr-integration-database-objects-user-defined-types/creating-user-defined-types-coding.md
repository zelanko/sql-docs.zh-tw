...
[!INCLUDE[appliesto-ss-xxxx-xxxx-xxx-md](../../includes/appliesto-ss-xxxx-xxxx-xxx-md.md)]
 ...
 在本節中的範例說明如何實作**點**將 UDT 當做**結構**(或**結構**Visual Basic 中)。 **點**UDT 包含 X 和 Y 座標會實作為屬性程序。  
 ...
 **Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute**設定的儲存體格式**點**UDT 到**原生**。 **IsByteOrdered**設定為 **，則為 true**，可保證的比較結果是 SQL Server 中的相同，如果相同的比較中發生的 managed 程式碼。 UDT 會實作**System.Data.SqlTypes.INullable**介面，好讓 UDT null 注意。  
...
### <a name="is-null-vs-isnull"></a>IS NULL 與IsNull  
 包含結構描述點 (id int，location Point)，其中的資料表，請考慮**點**是 CLR UDT 及下列查詢：  
...
```  
 ...
 這兩個查詢傳回的點與非識別碼**Null**位置。 查詢 1 會使用正常 null 處理，而且不需要還原序列化 UDT。 查詢 2，相反地，具有要還原序列化每一個非**Null**物件，並呼叫 CLR，若要取得的值**IsNull**屬性。 很明顯地，使用**IS NULL**會展現較佳的效能，而且永遠不應該繼續閱讀**IsNull**從 UDT 屬性[!INCLUDE[tsql](../../includes/tsql-md.md)]程式碼。  
  
 那麼，為何使用**IsNull**屬性？ 首先，需要用它來判斷值是否**Null**來自 CLR 程式碼。 第二，伺服器需要一個方法來測試例項是否為**Null**，因此伺服器會使用這個屬性。 它會判斷它後就**Null**，它就可以使用其原生的 null 處理來處理它。  
  
## <a name="implementing-the-parse-method"></a>實作 Parse 方法  
 **剖析**並**ToString**方法允許與 UDT 的字串表示的轉換。 **剖析**方法允許將字串轉換成 UDT。 必須宣告為**靜態**(或**Shared** Visual Basic 中)，並採用參數的型別**System.Data.SqlTypes.SqlString**。  
  
 下列程式碼會實作**剖析**方法**點**UDT，這樣會分隔 X 和 Y 座標。 **剖析**方法具有單一引數的型別**System.Data.SqlTypes.SqlString**，並假設 X 和 Y 值會提供以逗號分隔的字串。 設定**Microsoft.SqlServer.Server.SqlMethodAttribute.OnNullCall**屬性設定為**false**可防止**剖析**的 null 執行個體從呼叫的方法點。  
 ...
 **ToString**方法將**點**UDT 的字串值。 在此情況下，會傳回字串"NULL"的 Null 執行個體**點**型別。 **ToString**方法反轉**剖析**使用的方法**System.Text.StringBuilder**要傳回以逗號分隔**System.String**包含 X 和 Y 座標值。 因為**InvokeIfReceiverIsNull**預設值為 false，則為 null 的執行個體的核取**點**並非必要。  
 ...
 **Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute.ValidationMethodName**屬性**Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute**可讓您提供名稱伺服器執行時的驗證方法是指派給 UDT 或轉換成 UDT 資料。 **ValidationMethodName**也稱為執行 bcp 公用程式、 BULK INSERT、 DBCC CHECKDB、 DBCC CHECKFILEGROUP、 DBCC CHECKTABLE、 分散式的查詢及表格式資料流 (TDS) 遠端程序呼叫 (RPC) 作業期間。 預設值**ValidationMethodName**是 null，表示沒有驗證方法。  
  
### <a name="example"></a>範例  
 下列程式碼片段顯示的宣告**點**類別，指定**ValidationMethodName**的**ValidatePoint**。  
...
```  
 ...
 驗證方法可以有任何範圍，並應該傳回 **，則為 true**的值是否有效，並**false**否則。 如果此方法會傳回**false**或擲回例外狀況，值會被視為無效，並發生錯誤，就會引發。  
...
### <a name="validation-method-limitations"></a>驗證方法限制  
 ...
 您必須明確從屬性 setter 呼叫的驗證方法和**剖析**方法，如果您想要在所有情況下執行的驗證方法。 這不是硬性規定，在某些情況下可能也不適當。  
  
### <a name="parse-validation-example"></a>Parse 驗證範例  
 若要確保**ValidatePoint**方法中叫用**點**類別，您必須呼叫從**剖析**方法以及從設定 X 和 Y 屬性程序座標值。 下列程式碼片段示範如何呼叫**ValidatePoint**驗證方法，從**剖析**函式。  
...
## <a name="coding-udt-methods"></a>編碼 UDT 方法  
 當編碼 UDT 方法時，請考慮使用的演算法是否可能隨著時間而改變。 如果會的話，您可能會想要考慮針對 UDT 使用的方法建立個別類別。 如果演算法變更，您可在不影響 UDT 的情況下，以新的程式碼重新編譯該類別，並將組件載入 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 在許多情況下，您可使用 [!INCLUDE[tsql](../../includes/tsql-md.md)] ALTER ASSEMBLY 陳述式重新載入 UDT，但是這樣做可能會導致現有資料發生問題。 例如，**貨幣**UDT 隨附**AdventureWorks**範例資料庫會使用**ConvertCurrency**函數來轉換貨幣值，實作在個別的類別。 轉換演算法在未來有可能以非預期的方式變更，或者可能需要新的功能。 區隔**ConvertCurrency**函式**貨幣**UDT 實作規劃未來的變更時，提供更大的彈性。  
  
### <a name="example"></a>範例  
 **點**類別包含三個簡單的方法，來計算距離：**距離**， **DistanceFrom**並**DistanceFromXY**。 每一個都會傳回**雙**計算距離**點**為零，以指定的點之間的距離**點**，且距離指定 X 和 Y 座標若要**點**。 **距離**並**DistanceFrom**每個呼叫**DistanceFromXY**，並示範如何使用不同的引數，每個方法。  
...
>  **SqlMethodAttribute**類別繼承自**SqlFunctionAttribute**類別，因此**SqlMethodAttribute**繼承**FillRowMethodName**並**TableDefinition**欄位從**SqlFunctionAttribute**。 這意味著撰寫資料表值方法是可行的，不過情況不是這樣。 方法會進行編譯和組件進行部署，但是發生錯誤的相關**IEnumerable**傳回型別就會引發在執行階段，並出現下列訊息：「 方法、 屬性或欄位 '\<名稱 >' 中類別\<類別 >' 的組件中'\<組件 >' 有無效的傳回類型。 」  
 ...
 OnNullCall  
 指出當指定 Null 參考輸入引數時，是否呼叫此方法。 預設值是 **，則為 true**。  
...
> [!NOTE]  
>  查詢中不允許使用 Mutator 方法。 它們只能在指派陳述式或資料修改陳述式中呼叫。 如果方法標示為 mutator 不會傳回**void** (或不**Sub** Visual Basic 中)，CREATE TYPE 失敗並發生錯誤。  
  
 下列陳述式假設存在**三角形**具有 UDT**旋轉**方法。 下列[!INCLUDE[tsql](../../includes/tsql-md.md)]update 陳述式會叫用**旋轉**方法：  
  
```  
UPDATE Triangles SET t.RotateY(0.6) WHERE id=5  
```  
  
 **旋轉**方法以裝飾**SqlMethod**屬性設定**IsMutator**至**true**以便[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]可以將標記方法是使用 mutator 方法。 程式碼也會設定**OnNullCall**要**false**，這會向伺服器指出，方法會傳回 null 參考 (**Nothing** Visual Basic 中) 如果有任何輸入參數是 null 參考。  
...
## <a name="implementing-a-udt-with-a-user-defined-format"></a>以使用者定義的格式實作 UDT  
 當以實作 UDT 的使用者定義的格式，您必須實作**讀取**並**撰寫**處理序列化將 Microsoft.SqlServer.Server.IBinarySerialize 介面的方法和還原序列化 UDT 資料。 您也必須指定**MaxByteSize**屬性**Microsoft.SqlServer.Server.SqlUserDefinedTypeAttribute**。  
  
### <a name="the-currency-udt"></a>Currency UDT  
 **貨幣**UDT 會包含可以使用安裝 CLR 範例[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]開始[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]。  
  
 **貨幣**UDT 支援的特定文化特性的貨幣系統中的處理金額。 您必須定義兩個欄位：**字串**如**CultureInfo**，指定發行貨幣 (en-us-us，例如) 和**十進位**如**CurrencyValue**，金額。  
  
 它不是由伺服器來執行比較，雖然**貨幣**UDT 會實作**System.IComparable**介面，以便公開單一方法， **System.IComparable.CompareTo**。 這會在用戶端上使用，此時需要正確地比較或排序文化特性中的貨幣值。  
  
 在 CLR 上執行的程式碼會對貨幣值與文化特性分別進行比較。 如果是 [!INCLUDE[tsql](../../includes/tsql-md.md)] 程式碼，下列動作會決定比較：  
  
1.  設定**IsByteOrdered**屬性設為 true，告知[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]使用磁碟上保存的二進位表示法，進行比較。  
...
## <a name="creating-read-and-write-methods-with-ibinaryserialize"></a>使用 IBinarySerialize 建立 Read 和 Write 方法  
 當您選擇**UserDefined**序列化格式，您也必須實作**IBinarySerialize**介面，並建立您自己**讀取**並**寫入**方法。 下列程序，從**貨幣**UDT 使用**System.IO.BinaryReader**並**System.IO.BinaryWriter**來讀取並寫入 UDT。  
