...
[!INCLUDE [SQL Server](../../../includes/applies-to-version/sql-asdb-asdbmi-asa-pdw.md)]
 ...
 目標為 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 的 ODBC 應用程式僅能使用直接執行來呼叫預存程序。 連接至舊版時 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] ， [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] NATIVE Client ODBC 驅動程式會藉由建立暫存預存程序來實 [SQLPrepare](https://go.microsoft.com/fwlink/?LinkId=59360) 函式，然後在 **SQLExecute**上呼叫該預存程式。 這會增加額外負荷，讓 **SQLPrepare** 建立僅呼叫目標預存程式，而不是直接執行目標預存程式的暫存預存程序。 即使在連接到 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 執行個體時，跨網路準備呼叫都需要額外的往返，而且需要建立只呼叫預存程序執行計畫的執行計畫。  
