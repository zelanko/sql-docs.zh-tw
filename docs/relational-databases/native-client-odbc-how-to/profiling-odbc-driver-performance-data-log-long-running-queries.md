...
1.  在**控制面板**中,按兩下**管理工具**,然後按兩下**資料來源 (ODBC)。** (或者，您也可以從命令提示字元執行 odbcad32.exe)。  
...
5.  Microsoft SQL 伺服器設定 DSN 匯入時,使用**儲存長時間的查詢瀏覽到紀錄檔**。  
  
6.  選擇**此選項, 會將執行時執行的查詢儲存到紀錄檔**。 在方塊中，放置應該記錄其長時間執行查詢之檔案的名稱。 可選,按下 **「流覽」** 以瀏覽查詢日誌的檔案系統。  
  
7.  在 **「長查詢時間(毫秒)」** 框中設置查詢超時間隔(以毫秒為單位)。  

### <a name="to-log-long-running-queries-data-programmatically"></a>以程式設計方式記錄長時間執行的查詢資料  
  
1.  使用SQL_COPT_SS_PERF_QUERY_LOG以及長時間運行的查詢日誌檔的完整路徑和檔名調用[SQLSetConnectAttr。](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md) 例如：  
  
    ```  
    C:\\Odbcqry.log  
    ```  
  
2.  使用SQL_COPT_SS_PERF_QUERY_INTERVAL調用[SQLSetConnectAttr,](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md)並設置為超時間隔(以毫秒為單位)。  
  
3.  使用SQL_COPT_SS_PERF_QUERY和SQL_PERF_START調用[SQLSetConnectAttr,](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md)以開始記錄長時間運行的查詢。  
  
4.  使用SQL_COPT_SS_PERF_QUERY和SQL_PERF_STOP調用[SQLSetConnectAttr](../../relational-databases/native-client-odbc-api/sqlsetconnectattr.md)以停止記錄長時間運行的查詢。  
