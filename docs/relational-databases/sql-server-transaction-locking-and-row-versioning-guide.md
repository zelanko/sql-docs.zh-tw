...
此外，從 [!INCLUDE[ssSQL11](../includes/sssql11-md.md)] 開始，當發生死結時，**_system\_health_* _ 工作階段已經會擷取所有包含死結圖表的 `xml_deadlock_report` xEvent。 由於根據預設會啟用 _system\_health* 工作階段，因此無須設定個別的 xEvent 工作階段來擷取死結資訊。 
