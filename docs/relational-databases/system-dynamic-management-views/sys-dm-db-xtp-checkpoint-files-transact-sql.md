...
> **注意：** 對於尚未關閉的目前檢查點，s 的 [狀態] 欄 `ys.dm_db_xtp_checkpoint_files` 將會在 [新檔案的結構] 下進行。 當最後一個檢查點之後有足夠的交易記錄成長，或如果您發出 `CHECKPOINT` 命令（[Checkpoint &#40;transact-sql&#41;](../../t-sql/language-elements/checkpoint-transact-sql.md)）時，檢查點會自動關閉。  
  
 記憶體優化的檔案群組會在內部使用僅附加檔案，來儲存記憶體中資料表的插入和刪除資料列。 有兩種檔案類型： 當差異檔案包含已刪除資料列的參考時，資料檔案會包含插入的資料列。 
...
##  <a name="sssql15-and-later"></a><a name="bkmk_2016"></a> [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 及更新版的   
...
|relative_file_path|**nvarchar(256)**|相對於其所對應容器的檔案路徑。|  
|file_type|**smallint**|-1 代表免費<br /><br /> 0表示資料檔案。<br /><br /> 1代表差異檔案。<br /><br /> 2根檔案<br /><br /> 3適用于大型資料檔案|  
|file_type_desc|**nvarchar(60)**|免費-維護為免費的所有檔案都可供配置。 視系統的預期需求而定，可用檔案的大小會有所不同。 大小上限為 1 GB。<br /><br /> 資料檔案包含已插入記憶體優化資料表中的資料列。<br /><br /> 差異差異檔案包含已刪除之資料檔案中的資料列參考。<br /><br /> 根根檔案包含記憶體優化和原生編譯物件的系統中繼資料。<br /><br /> 大型資料-大型資料檔案包含插入（n） Varchar （max）和 Varbinary （max）資料行中的值，以及屬於記憶體優化資料表之資料行存放區索引一部分的資料行區段。|  
|internal_storage_slot|**int**|內部儲存體陣列中之檔案的索引。 根的 Null 或1以外的狀態。|  
|checkpoint_pair_file_id|**uniqueidentifier**|對應的資料檔案或差異檔案。 根的 Null。|  
...
|encryption_status_desc|**nvarchar(60)**|0 => UNENCRTPTED<br /><br /> 1 => 以金鑰1加密<br /><br /> 2 = 以金鑰2加密>。 僅對作用中的檔案有效。|  
...
|file_type_desc|**nvarchar(60)**|檔案類型： DATA_FILE、DELTA_FILE，或如果 state 資料行設定為7，則為 Null。|  
