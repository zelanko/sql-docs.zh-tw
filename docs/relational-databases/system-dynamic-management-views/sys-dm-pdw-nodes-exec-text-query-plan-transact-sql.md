---
title: _pdw_nodes_exec_text_query_plan （Transact-sql） |Microsoft Docs
...
|**query_plan**|**nvarchar(max)**|包含使用*plan_handle*指定之查詢執行計畫的編譯時間顯示計畫標記法。 顯示計畫是文字格式。 每個包含諸如特定 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式、預存程序呼叫和使用者自訂函數呼叫的批次，都會產生一份計畫。<br /><br /> 資料行可為 Null。|  

## <a name="remarks"></a>Remarks  
適用于[_exec_text_query_plan](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-text-query-plan-transact-sql?view=sql-server-ver15)的相同備註。  
