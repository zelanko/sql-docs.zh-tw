---
title: _pdw_nodes_exec_query_profiles （Transact-sql） |Microsoft Docs
...
|request_id|**整數**|識別目標要求。 參考 dm_exec_sessions.request_id。|  
...
|node_id|**整數**|識別查詢樹狀結構中的運算子節點。|  
|thread_id|**整數**|區分屬於相同查詢運算子節點的執行緒 (針對平行查詢)。|  
...
|object_id|**整數**|正在上面執行讀取和寫入之物件的識別碼。 參考 sys.objects.object_id。|  
|index_id|**整數**|資料列集開啟所依據的索引 (如果有的話)。|  
...
|segment_read_count|**整數**|目前為止的區段預先讀取數目。|  
|segment_skip_count|**整數**|目前為止略過的區段數目。| 
...
適用于[_exec_query_profiles](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-query-profiles-transact-sql?view=sql-server-ver15)的相同備註。  
