...
內建函式`DB_ID`可以指定。 當使用`DB_ID`如果不指定資料庫名稱，在目前資料庫的相容性層級必須是 90 或更高。
...
|log_min_lsn    |**nvarchar(24)**   |   目前啟動的交易記錄中的 LSN。 |  
|log_end_lsn    |**nvarchar(24)**   |   交易記錄檔中的最後一筆記錄的 LSN。 |  
|current_vlf_sequence_number    |**bigint** |   目前 VLF 序號時執行。 |  
|current_vlf_size_mb    |**float**  |   目前 VLF 大小 （mb）。 |   
|total_vlf_count    |**bigint** |   交易記錄檔中的 Vlf 總次數。 |  
|total_log_size_mb  |**float**  |   交易記錄總大小 （mb）。 |  
|active_vlf_count   |**bigint** |   使用中交易記錄檔中的 Vlf 的總數。 |  
...
|log_backup_lsn |**nvarchar(24)**   |   上次備份交易記錄的 LSN。 |   
|log_since_last_log_backup_mb   |**float**  |   記錄檔大小 （mb） 自上次備份交易記錄的 LSN。 |  
|log_checkpoint_lsn |**nvarchar(24)**   |   最後一個檢查點 LSN。 |  
|log_since_last_checkpoint_mb   |**float**  |   記錄檔大小 （mb） 自最後一個檢查點 LSN。 |  
|log_recovery_lsn   |**nvarchar(24)**   |   復原 LSN 的資料庫。 如果`log_recovery_lsn`發生前的檢查點 LSN，`log_recovery_lsn`否則是最舊的使用中交易的 LSN，`log_recovery_lsn`是檢查點 LSN。 |  
|log_recovery_size_mb   |**float**  |   記錄大小以 mb 為單位，因為記錄檔的復原 LSN。 |  
|recovery_vlf_count |**bigint** |   若要復原，如果發生容錯移轉或伺服器重新啟動的 Vlf 的總數。 |  
