---
title: _pdw_nodes_exec_query_statistics_xml （Transact-sql） |Microsoft Docs
...
|[狀態]|資料類型|[描述]|  
|-----------------|---------------|-----------------|
|node_id|**整數**|與節點相關聯的唯一數值識別碼。|
|session_id|**smallint**|工作階段的識別碼。 不可為 Null。|
|request_id|**整數**|要求的識別碼。 不可為 Null。|
|sql_handle|**varbinary(64)**|這是一個標記，可唯一識別查詢所屬的批次或預存程式。 可為 Null。|
|plan_handle|**varbinary(64)**|這是一個標記，可唯一識別目前執行之批次的查詢執行計畫。 可為 Null。|
|query_plan|**xml**|包含以包含部分統計資料的*plan_handle*指定之查詢執行計畫的執行時間顯示計畫標記法。 顯示計畫是 XML 格式。 每個包含諸如特定 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式、預存程序呼叫和使用者自訂函數呼叫的批次，都會產生一份計畫。 可為 Null。|

## <a name="remarks"></a>Remarks
適用于[_exec_query_statistics_xml](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-query-statistics-xml-transact-sql?view=sql-server-ver15)的相同備註。   
