...
|**client_thread_id**|**varbinary(8)**|顯示源自要求之用戶端應用程式的執行緒識別碼。<br /><br /> **\*\* 警告\* \*** 這是用戶端應用程式與 SQL Server 的同一部電腦上執行時才有意義。 當遠端電腦上，執行用戶端應用程式時**client_thread_id**顯示代表遠端用戶端的運作方式有些系統處理序的執行緒識別碼。<br /><br /> 可為 Null。|  
