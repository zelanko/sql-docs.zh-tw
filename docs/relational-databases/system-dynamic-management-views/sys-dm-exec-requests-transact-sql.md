---
title: sys.dm_exec_requests (TRANSACT-SQL) |Microsoft 文件
ms.custom: ''
ms.date: 08/25/2017
ms.prod: sql
...
ms.openlocfilehash: bbdb24d243a4dd93bc10dce76b3bb81501edc0c9
...
|quoted_identifier|**bit**|1 = 這項要求的 QUOTED_IDENTIFIER 是 ON。 否則，它可以是 0。<br /><br /> 不可為 Null。|  
|arithabort|**bit**|1 = 這項要求的 ARITHABORT 設定是 ON。 否則，它可以是 0。<br /><br /> 不可為 Null。|  
|ansi_null_dflt_on|**bit**|1 = 這項要求的 ANSI_NULL_DFLT_ON 設定是 ON。 否則，它可以是 0。<br /><br /> 不可為 Null。|  
|ansi_defaults|**bit**|1 = 這項要求的 ANSI_DEFAULTS 設定是 ON。 否則，它可以是 0。<br /><br /> 不可為 Null。|  
|ansi_warnings|**bit**|1 = 這項要求的 ANSI_WARNINGS 設定是 ON。 否則，它可以是 0。<br /><br /> 不可為 Null。|  
|ansi_padding|**bit**|1 = 這項要求的 ANSI_PADDING 設定是 ON。<br /><br /> 否則，它可以是 0。<br /><br /> 不可為 Null。|  
|ansi_nulls|**bit**|1 = 這項要求的 ANSI_NULLS 設定是 ON。 否則，它可以是 0。<br /><br /> 不可為 Null。|  
|concat_null_yields_null|**bit**|1 = 這項要求的 CONCAT_NULL_YIELDS_NULL 設定是 ON。 否則，它可以是 0。<br /><br /> 不可為 Null。|  
...
|dop |**int** |**適用於**： [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 查詢之平行處理原則程度。 |  
|parallel_worker_count |**int** |**適用於**： [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 保留平行背景工作如果這是平行查詢數目。  |  
|external_script_request_id |**uniqueidentifier** |**適用於**： [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 與目前的要求相關聯的外部指令碼要求識別碼。 |  
|is_resumable |**bit** |**適用於**： [!INCLUDE[sssqlv14-md](../../includes/sssqlv14-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 指出要求是否為可繼續索引作業。 |  
  
## <a name="permissions"></a>Permissions  
 如果使用者具有`VIEW SERVER STATE`伺服器上的權限，使用者會看到所有執行的工作階段的執行個體上[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]; 否則使用者會看到目前工作階段。 `VIEW SERVER STATE` 無法授與在[!INCLUDE[ssSDS_md](../../includes/sssds-md.md)]讓`sys.dm_exec_requests`會受限於目前的連接。 
...
### <a name="b-finding-all-locks-that-a-running-batch-is-holding"></a>B. 尋找執行中批次持有的所有鎖定  
 下列範例會查詢**sys.dm_exec_requests**來尋找有趣的批次並複製其`transaction_id`從輸出。  
