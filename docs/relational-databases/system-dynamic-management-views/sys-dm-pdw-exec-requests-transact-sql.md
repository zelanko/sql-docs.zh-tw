...
|importance|**nvarchar(128)**|要求執行的重要性設定。  這是此工作負載群組中的要求與共用資源的工作負載群組之間的相對重要性。  分類器中指定的重要性會覆寫工作負載群組的重要性設定。</br>適用對象：Azure SQL 資料倉儲|NULL</br>低</br>below_normal</br>normal （預設值）</br>above_normal</br>高|
