...
> **注意：** 若要呼叫這個屬性從[!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)]或是[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]，使用名稱**sys.dm_pdw_nodes_os_sys_info**。  
...
|**committed_target_kb**|**int**|**適用於：** [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 代表可由 SQL Server 記憶體管理員耗用的記憶體數量 (KB)。 目標數量是透過各種輸入來計算，例如：<br /><br /> -包含其負載系統的目前狀態<br /><br /> -目前的處理序所要求的記憶體<br /><br /> -電腦上安裝的記憶體數量<br /><br /> -設定參數<br /><br /> 如果**committed_target_kb&lt**大於**committed_kb&lt**，記憶體管理員會嘗試取得其他記憶體。 如果**committed_target_kb&lt**小於**committed_kb&lt**，記憶體管理員會嘗試壓縮已認可的記憶體數量。 **Committed_target_kb&lt**永遠會包含遭奪取和保留的記憶體。 不可為 Null。|  
...
|**visible_target_kb**|**int**|**適用於：** [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 等同於**committed_target_kb&lt**。 不可為 Null。|  
|**stack_size_in_bytes**|**int**|指定 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 建立之每一個執行緒的呼叫堆疊大小。 不可為 Null。|  
|**os_quantum**|**bigint**|代表非先佔式工作的配量 (以毫秒為單位)。 配量 （以秒為單位） = **os_quantum&lt** / CPU 時脈速度。 不可為 Null。|  
...
|**sqlserver_start_time_ms_ticks**|**bigint**|代表**ms_tick**數字時[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]上次啟動。 與目前的 ms_ticks 資料行比較。 不可為 Null。|  
...
|**softnuma_configuration**|**int**|**適用於：** [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> 指定已設定的方式 NUMA 節點。 不可為 Null。<br /><br /> 0 = OFF 表示硬體預設<br /><br /> 1 = 自動軟體 NUMA<br /><br /> 2 = 透過登錄的手動軟體 NUMA|  
|**softnuma_configuration_desc**|**nvarchar(60)**|**適用於：** [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br /> OFF = NUMA 功能關閉<br /><br /> 在 =[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]軟體式 NUMA 會自動決定的 NUMA 節點大小<br /><br /> MANUAL = 手動設定的軟體 NUMA|
|**process_physical_affinity**|**nvarchar(3072)** |**適用於：** 從開始[!INCLUDE[ssSQL17](../../includes/sssql17-md.md)]。<br /><br />尚未要取得的資訊。 |
...
|**sql_memory_model_desc**|**nvarchar(120)**|**適用於：** [!INCLUDE[sssql11](../../includes/sssql11-md.md)] SP4 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP1 透過[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br />指定所使用的記憶體模型[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]配置記憶體。 不可為 Null。<br /><br />**傳統** =  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]配置的記憶體使用傳統記憶體模式。 這是預設 sql 記憶體內部模型時[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]服務帳戶並沒有鎖定的分頁記憶體權限在啟動期間。<br />**LOCK_PAGES**  =  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]使用記憶體中的鎖定分頁配置記憶體。 當 SQL Server 服務帳戶擁有權限中鎖定分頁，在 SQL Server 啟動期間，這是預設的 sql 記憶體管理員。<br /> **LARGE_PAGES**  =  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]使用記憶體中的大型分頁配置記憶體。 SQL Server 會使用大型分頁配置器配置只具備 Enterprise edition 的記憶體，當 SQL Server 服務帳戶會擁有權限中鎖定分頁 在伺服器啟動期間，並開啟追蹤旗標 834。|
...
|**cores_per_socket** |**int** | **適用於：** [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br />在系統上指定每個通訊端可用的處理器的數目。 |  
|**numa_node_count** |**int** | **適用於：** [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] SP2 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。<br /><br />指定系統上的可用的 numa 節點數目。 此資料行包含實體 numa 節點，以及軟體式 numa 節點。 |  
