...
-   粗線代表上限值 (*range_high_key*) 而發生的次數 (*equal_rows*)  
  
-   左邊的實線區域*range_high_key*代表資料行值與每個資料行值出現的次數平均的數字的範圍 (*average_range_rows*)。 *Average_range_rows*對於第一個長條圖步驟一律是 0。  
  
-   虛線代表用來評估的範圍中的相異值總數的取樣的值 (*distinct_range_rows*) 和範圍中值的總數 (*range_rows*)。 查詢最佳化工具使用*range_rows*和*distinct_range_rows*來計算*average_range_rows*並不會儲存取樣的值。  
