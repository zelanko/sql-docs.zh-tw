---
title: _db_page_info (Transact-sql) |Microsoft Docs
ms.custom: ''
ms.date: 09/18/2018
ms.prod: sql
ms.reviewer: ''
...
|page_header_version |ssNoversion |頁首版本 |
...
|page_level |ssNoversion |索引中的頁面層級 (分葉 = 0) |
|object_id |ssNoversion |擁有該頁面之物件的識別碼 |
|index_id |ssNoversion |索引的識別碼 (若為堆積資料頁面, 則為 0) |
|partition_id |BIGINT |資料分割的識別碼 |
|alloc_unit_id |BIGINT |配置單位的識別碼 |
|is_encrypted |bit |指出頁面是否已加密的位 |
|has_checksum |bit |位, 指出頁面是否有總和檢查碼值 |
|總和檢查碼 |ssNoversion |儲存用來偵測資料損毀的總和檢查碼值 |
...
|free_data_offset |ssNoversion |資料區域結尾的可用空間位移 |
|reserved_bytes |SMALLINT |所有交易保留的可用位元組數 (如果堆積) <br> 幻影資料列數 (如果是索引分葉) |
|reserved_bytes_by_xdes_id |SMALLINT |M_xdesID 至 m_reservedCnt 的空間貢獻 <br> 僅供偵錯工具之用 |
|xdes_id |nvarchar(64) |M_reserved 貢獻的最新交易 <br> 僅供偵錯工具之用 |
||||

## <a name="remarks"></a>備註
`file_id` `page_id` `object_id` `index_id`動態管理函數會傳回頁面標頭中出現的頁面資訊, 例如、、等。 `sys.dm_db_page_info` 這種資訊有助於疑難排解和偵測各種效能 (鎖定和閂鎖爭用) 和損毀問題。
...
新`sys.fn_PageResCracker`的函式`page_resource`會採用做為輸入, 並輸出包含`database_id`、 `file_id`和`page_id`的單一資料列。  然後, 可以使用此函式來加速或`sys.dm_exec_requests` `sys.sysprocesses`與`sys.dm_db_page_info`之間的聯結。
...
下列查詢會傳回一個資料列, 其中包含給定`database_id` `file_id` `page_id`之的所有頁面資訊、與預設模式 (「有限」) 的組合
...
### <a name="b-using-sysdm_db_page_info-with-other-dmvs"></a>B. 搭配其他 Dmv 使用 sys.databases _db_page_info 
