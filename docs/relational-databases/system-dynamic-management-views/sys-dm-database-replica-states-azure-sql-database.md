...
|**is_primary_replica**|**bit**|如果複本是主要的，則傳回 1; 如果是資料庫所屬可用性群組中的次要複本，則傳回0。 這不是指分散式可用性群組或作用中異地複寫關聯中的主要或次要資料庫。<br /><br />**適用于：** [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 和更新版本。|  
...
|**last_hardened_lsn**|**numeric(25,0)**|記錄檔區塊的開頭，其中包含次要資料庫之上次強行寫入 LSN 的記錄檔記錄。<br /><br /> 在非同步認可主要資料庫或目前原則為「延遲」的同步認可資料庫上，此值是 NULL。 若為其他同步認可主資料庫， **last_hardened_lsn**表示所有次要資料庫的強化 lsn 的最小值。<br /><br /> **注意： last_hardened_lsn**會反映填滿零的記錄檔區塊識別碼。 這不是實際的記錄序號。|  
