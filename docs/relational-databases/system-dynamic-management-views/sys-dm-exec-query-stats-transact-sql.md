...
|**statement_end_offset**|**int**|表示資料列於其批次或保存物件的文字中所描述之查詢的結束位置 (由 0 開始並以位元組為單位)。 針對 [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]之前的版本，-1 的值表示批次的結尾。 已不再包含尾端批註。|  
|**plan_generation_num**|**bigint**|可用於重新編譯之後區分計畫執行個體的序號。|  
|**plan_handle**|**varbinary(64)**|這是一個標記，可唯一識別已執行之批次的查詢執行計畫，且其計畫位於計畫快取中，或目前正在執行。 這個值可以傳遞給 sys.databases， [dm_exec_query_plan](../../relational-databases/system-dynamic-management-views/sys-dm-exec-query-plan-transact-sql.md)動態管理函數來取得查詢計劃。<br /><br /> 當原生編譯的預存程序查詢記憶體最佳化的資料表時，一律為 0x000。|  
...
|**total_clr_time**|**bigint**|這個計畫在編譯後執行所耗用的時間（以毫秒為單位來報告，但是精確度只到毫秒），在 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] common language runtime （CLR）物件。 CLR 物件可以是預存程序、函數、觸發程序、類型和彙總。|  
...
|**pdw_node_id**|**int**|此散發所在節點的識別碼。<br /><br /> **適用于**： [!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)]、[!INCLUDE[ssPDW](../../includes/sspdw-md.md)]| 
...
[執行相關的&#40; &#41;動態管理檢視和函數 transact-sql](../../relational-databases/system-dynamic-management-views/execution-related-dynamic-management-views-and-functions-transact-sql.md)    
