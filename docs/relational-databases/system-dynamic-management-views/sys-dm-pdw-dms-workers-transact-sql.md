...
|類型|**nvarchar(32)**|此專案所代表的 DMS 工作者執行緒類型。|[DIRECT_CONVERTER]、[DIRECT_READER]、[FILE_READER]、[HASH_CONVERTER]、[HASH_READER]、[ROUNDROBIN_CONVERTER]、[EXPORT_READER]、[EXTERNAL_READER]、[EXTERNAL_WRITER]、[PARALLEL_COPY_READER]、[REJECT_WRITER]、[WRITER]|  
...
|total_elapsed_time|**int**|花費在執行的總時間（以毫秒為單位）。|大於或等於0。<br /><br /> 從系統啟動或重新開機以來經過的總時間。 如果 total_elapsed_time 超過整數的最大值 (24.8 天（以毫秒為單位）) ，則會造成具體化失敗，因為溢位。<br /><br /> 最大值（以毫秒為單位）相當於24.8 天。|  
