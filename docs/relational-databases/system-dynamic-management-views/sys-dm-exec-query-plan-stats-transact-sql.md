...
## <a name="remarks"></a>備註
這是一項選擇性功能。 若要在伺服器層級啟用，請使用 [追蹤旗](../../t-sql/database-console-commands/dbcc-traceon-trace-flags-transact-sql.md) 標2451。 若要在資料庫層級啟用，請使用 ALTER DATABASE 範圍設定中的 LAST_QUERY_PLAN_STATS 選項 [&#40;transact-sql&#41;](../../t-sql/statements/alter-database-scoped-configuration-transact-sql.md)。

這個系統函數可在 **輕量** 查詢執行統計資料分析基礎結構下運作。 如需詳細資訊，請參閱[查詢分析基礎結構](../../relational-databases/performance/query-profiling-infrastructure.md)。  

的顯示計畫輸出 `sys.dm_exec_query_plan_stats` 包含下列資訊：
-  快取計畫中所找到的所有編譯時間資訊
-  執行時間資訊，例如每個運算子的實際資料列數、查詢 CPU 時間和執行時間總計、溢出警告、實際 DOP、使用的記憶體上限和授與的記憶體

在下列情況下，會在所傳回資料表的**query_plan**資料行中，傳回**相當於實際執行計畫**的顯示計畫輸出 `sys.dm_exec_query_plan_stats` ：  
...
在下列情況下，會在所傳回資料表的**query_plan**資料行中傳回**簡化的<sup>1</sup> **個顯示計畫輸出 `sys.dm_exec_query_plan_stats` ：  

-   您可以在 [sys. dm_exec_cached_plans](../../relational-databases/system-dynamic-management-views/sys-dm-exec-cached-plans-transact-sql.md)中找到此計畫。     
    **AND**    
-   此查詢夠簡單，通常會分類為 OLTP 工作負載的一部分。

<sup>1</sup> 指的是僅包含根節點運算子 (選取) 的執行程式表。

在下列情況下， **不會傳回任何輸出** `sys.dm_exec_query_plan_stats` ：

-   使用指定的查詢計劃已經 `plan_handle` 從計畫快取中收回。     
    **OR**    
...
> [!NOTE] 
> 由於 **xml** 資料類型所允許的嵌套層級數目有限制，所以無法傳回 `sys.dm_exec_query_plan` 符合或超過128層級之嵌套專案的查詢計劃。 在舊版的中 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，這種情況會使查詢計劃無法傳回並產生 [錯誤 6335](../../relational-databases/errors-events/database-engine-events-and-errors.md#errors-6000-to-6999)。 在 [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] Service Pack 2 和更新版本中，資料 `query_plan` 行傳回 Null。  
