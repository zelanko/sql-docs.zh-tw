---
title: sys.dm_io_backup_tapes & Amp;#40;transact-SQL&AMP;#41; |Microsoft Docs
ms.custom: ''
ms.date: 06/10/2016
ms.prod: sql
ms.reviewer: ''
...
|**logical_device_name**|**nvarchar(256)**|使用者指定磁碟機的名稱 (從**sys.backup_devices**)。 如果沒有使用者指定名稱可用，便是 NULL。 可為 Null。|  
|**status**|**int**|磁帶的狀態：<br /><br /> 1 = 已開啟，可供使用<br /><br /> 2 = 掛載暫止<br /><br /> 3 = 使用中<br /><br /> 4 = 載入中<br /><br /> **注意：** 正在載入磁帶時 (**狀態 = 4**)，尚未讀取媒體標籤。 這類複製媒體標籤值的資料行**media_sequence_number**，顯示預期的值可能不同於在磁帶上的實際值。 已讀取標籤之後，**狀態**變更為**3** （使用中），以及媒體標籤資料行則會反映實際載入的磁帶。<br /><br /> 不可為 Null。|  
...
|**mount_request_type**|**int**|掛載要求的類型：<br /><br /> 1 = 特定磁帶。 識別的磁帶**media_\*** 欄位是必要。<br /><br /> 2 = 下一個媒體家族。 要求下一個尚未還原的媒體家族。 當還原的來源裝置比媒體家族少時，便使用這個項目。<br /><br /> 3 = 接續磁帶。 將延伸媒體家族，並要求接續磁帶。<br /><br /> 可為 Null。|  
