...
[!INCLUDE[SQL Server Azure SQL Database Synapse Analytics PDW ](../../includes/applies-to-version/sql-asdb-asdbmi-asdw-pdw.md)]
 ...
 注意：對於具有 unixODBC 2.3.1 至2.3.4 的 Linux 和 Mac ODBC 驅動程式，使用 SQL_CA_SS_TYPE_NAME 描述元欄位設定 TVP 名稱到 SQLSetDescField 時，unixODBC 不會在 ANSI 和 Unicode 字串之間自動轉換，視名為的確切函式（SQLSetDescFieldA/SQLSetDescFieldW）而定。 必須一律使用 SQLBindParameter 或 SQLSetDescFieldW 搭配 Unicode （UTF-16）字串來設定 TVP 名稱。
