...
description: 了解如何在目標執行個體離線或無法啟動時，從本機或遠端 SQL Server 執行個體檢視 SQL Server 記錄檔。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
...
-   包含錯誤記錄檔之資料夾的讀取權限。 根據預設，錯誤記錄檔會位於下列路徑 (其中 \<*Drive>* 表示安裝 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的磁碟機，而 \<*InstanceName*> 則是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的執行個體名稱)：  
