...
-   **目前的資料表：**  
    ...
    -   當 **SYSTEM_VERSIONING** 為 **ON** 時，**SWITCH IN** 目前的資料表可用來加速資料載入和查詢  
  
    -   **SYSTEM_VERSIONING** 為 **ON** 時不允許 **SWITCH OUT**  
  
-   **記錄資料表：**  
  
    -   當 **SYSTEM_VERSIONING** 為 **ON** 時，可執行從記錄資料表 **SWITCH OUT**，來清除不再相關的歷程記錄資料部分。  
  
    -   當 **SYSTEM_VERSIONING** 為 **ON** 時，不允許 **SWITCH IN**，因為它可能會使暫存資料不一致。  
