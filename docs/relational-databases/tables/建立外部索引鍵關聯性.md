...
3.  在 [外部索引鍵關聯性] 對話方塊中，按一下 [加入]。    
     ...
     關聯性會出現在 [選取的關聯性] 清單中，並顯示系統提供的名稱，格式為 FK_\<資料表名稱>_\<資料表名稱>，其中 資料表名稱為外部索引鍵資料表的名稱。    
