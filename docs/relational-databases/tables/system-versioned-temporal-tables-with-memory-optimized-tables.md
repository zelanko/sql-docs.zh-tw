...
## <a name="the-data-flush-task"></a>資料排清工作
 ...
 您可以叫用 [sp_xtp_flush_temporal_history](../../relational-databases/system-stored-procedures/temporal-table-sp-xtp-flush-temporal-history.md)，並指定結構描述與資料表名稱 **sys.sp_xtp_flush_temporal_history  @schema_name, @object_name** ，來強制執行資料排清。 利用這個使用者執行的命令，當資料排清工作由系統依據內部排程叫用時，會叫用相同的資料移動程序。  
