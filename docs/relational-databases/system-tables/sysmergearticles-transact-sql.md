...
|-----------------|---------------|-----------------|  
|**name**|**sysname**|發行項的名稱。|  
|**型別**|**tinyint**|指出發行項類型，它可以是下列項目之一：<br /><br /> **10** = 資料表。<br /><br /> **32** = 預存程式（僅限架構）。<br /><br /> **64** = view 或 indexed view （僅限架構）。<br /><br /> **128** = 使用者定義函數（僅限架構）。<br /><br /> **160** = 同義字（僅限架構）。|  
|**objid**|**int**|物件識別碼。|  
...
|**before_image_objid**|**int**|追蹤資料表物件識別碼。 當建立發行集時，追蹤資料表包含特定索引鍵資料行值， *\@keep_partition_changes* = **true**。|  
