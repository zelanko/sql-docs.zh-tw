...
|**reason_code**|**int**|可為內容相關的錯誤碼。 在更新-更新和更新-刪除衝突的情況下，用於此資料行的值與**conflict_type**相同。 但是，對於無法變更衝突，原因代碼將為錯誤，以防止「合併代理程式」套用變更。 例如，如果合併代理程式因為主鍵違規而無法在訂閱者端套用插入，它會記錄6的**conflict_type** （「下載插入失敗」）和**reason_code** 2627，這是 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 主鍵違規的內部錯誤訊息：「違反% ls 條件約束 '%. * ls '。 無法在物件 '% 中插入重複的索引鍵。 \*ls '. "|  
