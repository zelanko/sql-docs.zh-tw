...
ms.openlocfilehash: 697ea3565aef1381bfaaf10605c38fd28e3c07e0
ms.sourcegitcommit: 4cd008a77f456b35204989bbdd31db352716bbe6
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/06/2018
ms.locfileid: "39566072"
---
...
[!INCLUDE[SNAC_Deprecated](../../includes/snac-deprecated.md)]
 ...
 第一個選項根據[SQLBindCol](../../relational-databases/native-client-odbc-api/sqlbindcol.md)。 應用程式提取結果集之前，請使用**SQLBindCol**中結果集至程式變數繫結每個資料行。 驅動程式會將目前的資料列，為變數的資料繫結至結果集資料行每個資料行都已繫結之後，時間應用程式會呼叫**SQLFetch**或是[SQLFetchScroll](../../relational-databases/native-client-odbc-api/sqlfetchscroll.md)。 如果結果集資料行和程式變數的資料類型不同，驅動程式會處理資料轉換。 如果應用程式將 sql_attr_row_array_size 設定為大於 1，它可以繫結結果資料行至變數，在每次呼叫會先填入陣列**SQLFetchScroll**。  
  
 第二個選項根據[SQLGetData](../../relational-databases/native-client-odbc-api/sqlgetdata.md)。 應用程式不會使用**SQLBindCol**繫結結果集資料行至程式變數。 在每次呼叫之後**SQLFetch**，應用程式會呼叫**SQLGetData**一次的結果中的每個資料行設定。 **SQLGetData**指示將從特定的結果集資料行的資料傳送到特定的程式變數的驅動程式，並指定資料行和變數的資料類型。 如果結果資料行和程式變數的資料類型不同，這會讓驅動程式轉換資料。 **文字**， **ntext**，以及**映像**資料行通常太大而無法容納到程式變數，但仍可使用擷取**SQLGetData**。 如果**文字**， **ntext**，或**映像**結果資料行中的資料大於程式變數， **SQLGetData**傳回 SQL_SUCCESS_WITH_INFO 和 SQLSTATE 01004 （字串資料，右邊已截斷）。 後續呼叫**SQLGetData**會傳回連續區塊**文字**或是**映像**資料。 當到達資料的結尾時， **SQLGetData**都會傳回 SQL_SUCCESS。 如果 SQL_ATTR_ROW_ARRAY_SIZE 大於 1，每次提取都會傳回一組資料列或資料列集。 使用之前**SQLGetData**，您必須先使用**SQLSetPos**指定為目前的資料列的特定資料列集中的資料列。  
 ...
 過度使用**SQLBindCol**繫結結果集資料行至程式變數是昂貴因為**SQLBindCol**會導致配置記憶體的 ODBC 驅動程式。 當您將結果資料行繫結至變數時，該繫結會持續有效，直到您呼叫[SQLFreeHandle](../../relational-databases/native-client-odbc-api/sqlfreehandle.md)來釋放陳述式控制代碼或呼叫[SQLFreeStmt](../../relational-databases/native-client-odbc-api/sqlfreestmt.md)使用*fOption*設定為 SQL_UNBIND。 當陳述式完成時，不會自動復原繫結。  
 ...
 ODBC 函式，來處理，例如將資料移入或移出程式變數**SQLGetData**， **SQLBindCol**，並[SQLBindParameter](../../relational-databases/native-client-odbc-api/sqlbindparameter.md)，支援隱含的資料類型轉換。 例如，如果應用程式將整數資料行繫結至字元字串程式變數，驅動程式會先自動將資料從整數轉換為字元，然後再將其放入程式變數中。  
