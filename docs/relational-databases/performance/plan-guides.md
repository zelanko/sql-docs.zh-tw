...
description: 了解計畫指南，這可供將查詢的效能最佳化，而不需要在 SQL Server 中直接變更查詢的文字。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.reviewer: ''
...
```  
另舉一例，請考慮使用 [sp_executesql](../../relational-databases/system-stored-procedures/sp-executesql-transact-sql.md) 提交的下列 SQL 陳述式。
