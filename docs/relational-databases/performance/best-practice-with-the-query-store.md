...
##  <a name="Insight"></a> 在 Azure SQL 資料庫中使用查詢效能深入解析  
 如果您在 Azure [!INCLUDE[ssSDS](../../includes/sssds-md.md)] 中執行查詢存放區，您可以使用 [查詢效能見解]  來分析一段時間內的 DTU 耗用量。 雖然您可以使用 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 來取得所有查詢的資源耗用量詳細資訊 (例如 CPU、記憶體和 I/O)，但 [查詢效能見解] 能夠為您提供一個快速且有效率方式來判斷資源耗用量對資料庫整體 DTU 耗用量的影響。 如需詳細資訊，請參閱 [Azure SQL 資料庫查詢效能深入解析](https://azure.microsoft.com/documentation/articles/sql-database-query-performance/)。
