...
|BCP_OUT_NATIVE_TEXT_MODE|指定非字元類型的原生類型和字元類型的 Unicode。<br /><br /> 對應至 BCP 中的-N 選項。EXE 和 IBCPSession：： BCPColFmt with *eUserDataType*屬性設為**BCP_TYPE_SQLNCHAR**如果資料行類型是字串，或**BCP_TYPE_DEFAULT** （如果不是字串）。|  
