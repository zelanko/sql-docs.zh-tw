...
ms.locfileid: "81301071"
---
# <a name="data-type-support-for-ole-db-date-and-time-improvements"></a>對 OLE DB 日期和時間改善的資料類型支援
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

  本主題提供有關支援[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)][!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]日期/時間數據類型的 OLE 資料庫(本機用戶端)類型的資訊。  
...
```  
 ...
 如果基礎架構更新[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]為使用**datetime2**而不是**日期時間**,則必須更新遷移到使用**sql_variant**並依賴於**有限日期時間**精度的本機客戶端的應用程式。  
...
|----------------------------------|-----------------------------------------|-----------|  
|DBTYPE_DBDATE|date||  
|DBTYPE_DBTIMESTAMP|**datetime2**(p)|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]本機用戶端 OLE 資料庫提供程式檢查 DBCOLUMDESC *bScale*成員以確定小數秒精度。|  
|DBTYPE_DBTIME2|**time**(p)|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]本機用戶端 OLE 資料庫提供程式檢查 DBCOLUMDESC *bScale*成員以確定小數秒精度。|  
|DBTYPE_DBTIMESTAMPOFFSET|**datetimeoffset**(p)|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]本機用戶端 OLE 資料庫提供程式檢查 DBCOLUMDESC *bScale*成員以確定小數秒精度。|  
  
 當應用程式在 *wType* 中指定 DBTYPE_DBTIMESTAMP 時，其可在 *pwszTypeName* 中提供一個類型名稱，藉以將對應覆寫到 **datetime2**。 如果指定了 **datetime**，*bScale* 必須是 3。 如果指定了 **smalldatetime**，*bScale* 必須是 0。 如果*bScale*與*wType*和*pwszTypeName*不一致,則返回DB_E_BADSCALE。  
