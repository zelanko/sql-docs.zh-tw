...
|**filename**|**nvarchar(260)**|資料庫主要檔案的作業系統路徑和名稱。<br /><br /> **檔名**看見**dbcreator**， **sysadmin**、 CREATE ANY DATABASE 權限，或擁有下列權限的被授與者的資料庫擁有者：改變任何資料庫建立任何資料庫、 檢視任何定義。 若要傳回的路徑和檔案名稱，請查詢[sys.sysfiles](../../relational-databases/system-compatibility-views/sys-sysfiles-transact-sql.md)相容性 檢視中，或有[sys.database_files](../../relational-databases/system-catalog-views/sys-database-files-transact-sql.md)檢視。|  
