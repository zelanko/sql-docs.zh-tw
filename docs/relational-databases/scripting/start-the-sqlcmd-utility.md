...
### 啟動 sqlcmd 公用程式和連接 SQL Server 的預設執行個體  
...
### 啟動 sqlcmd 公用程式並連接到 SQL Server 的具名執行個體  
  
1.  開啟 [命令提示字元] 視窗，然後輸入 **sqlcmd -S***myServer\instanceName*。 以要連接的電腦名稱和 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的執行個體來取代 *myServer\instanceName*。  
