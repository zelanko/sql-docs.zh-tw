...
helpviewer_keywords: 
  ...
  - "備份 [SQL Server], 復原模式"
  - "復原模式 [SQL Server]"
  - "復原模式 [SQL Server], 交易記錄管理"
  - "資料庫還原 [SQL Server], 復原模式"
  - "交易記錄檔還原 [SQL Server]"
  - "記錄檔 [SQL Server], 復原模式"
  - "還原資料庫 [SQL Server], 復原模式"
  - "完整復原模式 [SQL Server]"
  - "備份交易記錄 [SQL Server], 復原模式"
...
|--------------------|-----------------|------------------------|-------------------------------|  
|**Simple**|無記錄備份。<br /><br /> 自動收回記錄空間，使空間需求保持在最低，實際消弭管理交易記錄空間的需求。 如需簡單復原模式下之資料庫備份的相關資訊，請參閱[完整資料庫備份 &#40;SQL Server&#41;](../../relational-databases/backup-restore/full-database-backups-sql-server.md)。<br /><br /> 簡單復原模式不支援需要交易記錄備份的作業。 在簡單復原模式中，不能使用下列功能：<br /><br /> -記錄傳送<br /><br /> -AlwaysOn 或資料庫鏡像<br /><br /> -無資料遺失的媒體復原<br /><br /> -時間點還原|最近一次備份之後所做的變更並未受到保護。 如果發生損毀事件，則必須重做這些變更。|只能復原至備份結束時。 如需詳細資訊，請參閱[完整資料庫還原 &#40;簡單復原模式&#41;](../../relational-databases/backup-restore/complete-database-restores-simple-recovery-model.md)。 <br><br> 如需簡單復原模式的更深入說明，請參閱 [MSSQLTips!](https://www.mssqltips.com) 人員所提供的 [SQL Server Simple Recovery Model](https://www.mssqltips.com/sqlservertutorial/4/sql-server-simple-recovery-model/) (SQL Server 簡單復原模式)。|  
