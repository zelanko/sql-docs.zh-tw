...
ms.topic: "article"
helpviewer_keywords: 
  - "交易記錄備份 [SQL Server], SQL Server Management Studio"
  - "備份 [SQL Server], 建立"
  - "備份交易記錄 [SQL Server], SQL Server Management Studio"
...
-   如果資料庫使用完整或大量記錄[復原模式](https://msdn.microsoft.com/library/ms189275.aspx)，您必須定期備份交易記錄，使其足以保護您的資料，並讓[交易記錄不要被寫滿](https://msdn.microsoft.com/library/ms175495.aspx)。 這會截斷記錄並支援將資料庫還原到特定時間點。 並不恰當！
...
7.  或者，您也可以選取 [只複製備份] 來建立只複製備份。 「只複製備份」是與傳統 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 備份順序無關的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 備份。 如需詳細資訊，請參閱[只複製備份 &#40;SQL Server&#41;](../../relational-databases/backup-restore/copy-only-backups-sql-server.md)。  
  
    >** 注意！** 選取 [差異] 選項時，您無法建立只複製備份。  
...
15. 在 **[交易記錄]** 區段中：  
    ...
    -   若要備份記錄的結尾 (亦即，使用中的記錄)，請核取 [Back up the tail of the log, and leave database in the restoring state (備份記錄檔的結尾，並讓資料庫保持在還原狀態)]。  
