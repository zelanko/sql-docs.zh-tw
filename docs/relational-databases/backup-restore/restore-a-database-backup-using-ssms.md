...
helpviewer_keywords: 
  - "完整備份 [SQL Server]"
  - "資料庫還原 [SQL Server], 完整備份"
  - "備份資料庫 [SQL Server], 完整備份"
  - "資料庫備份 [SQL Server], 完整備份"
  - "還原資料庫 [SQL Server], 完整備份"
...
### 重要！    
您可能必須先備份使用中的交易記錄 (也稱為[記錄結尾](https://msdn.microsoft.com/library/ms179314.aspx))，才能在完整或大量記錄復原模式下還原資料庫。 如需詳細資訊，請參閱[備份交易記錄 &#40;SQL Server&#41;](../../relational-databases/backup-restore/back-up-a-transaction-log-sql-server.md)。  
...
### **C.  在原始資料庫仍存在的情況下，使用新的資料庫名稱還原先前的磁碟備份**
...
9. [!INCLUDE[clickOK](../../includes/clickok-md.md)] 
    ...
    > [!NOTE] 
    > 如果您收到下列錯誤訊息：「System.Data.SqlClient.SqlError: 資料庫 `Sales` 的記錄結尾尚未備份。 若您不想遺失其中的內容，請使用 BACKUP LOG WITH NORECOVERY 備份記錄。 亦可使用 RESTORE 陳述式的 WITH REPLACE 或 WITH STOPAT 子句，覆寫記錄的內容。 (Microsoft.SqlServer.SmoExtended)」。  
...
6.  按一下 [加入]，[選取備份檔案位置] 對話方塊隨即開啟。
    ...
    #### **E1.   以覆蓋現有資料庫的方式還原等量備份，而且存在共用存取簽章。**
...
#### **F.   將本機備份還原至 Microsoft Azure 儲存體 (URL)**
