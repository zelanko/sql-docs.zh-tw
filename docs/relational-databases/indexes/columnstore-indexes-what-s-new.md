...
|資料行存放區索引功能|[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]|[!INCLUDE[ssSQL14](../../includes/sssql14-md.md)]|[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]|[!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] Premium Edition|[!INCLUDE[ssSDW](../../includes/sssdw-md.md)]|  
|-------------------------------|---------------------------|---------------------------|---------------------------|--------------------------------------------|-------------------------|  
|批次執行多執行緒查詢|是|是|是|是|是|  
|批次執行單一執行緒查詢|||是|是|是|  
|封存壓縮選項。||是|是|是|是|  
|快照集隔離和讀取認可快照集隔離|||是|是|是|  
|建立資料表時指定資料行存放區索引。|||是|是|是|  
|AlwaysOn 支援資料行存放區索引。|是|是|是|是|是|  
|AlwaysOn 可讀取次要支援唯讀的非叢集資料行存放區索引。|是|是|是|是|是|  
|AlwaysOn 可讀取次要支援可更新的資料行存放區索引。|||是|||  
|堆積或 btree 的唯讀非叢集資料行存放區索引。|是|是|是*|是*|是*|  
|堆積或 btree 的可更新非叢集資料行存放區索引。|||是|是|是|  
|有非叢集資料行存放區索引的堆積或 btree 允許其他的 btree 索引。|是|是|是|是|是|  
|可更新的叢集資料行存放區索引。||是|是|是|是|  
|叢集資料行存放區索引的 btree 索引。|||是|是|是|  
|記憶體最佳化資料表的資料行存放區索引。|||是|是|是|  
|非叢集資料行存放區索引定義支援使用篩選的條件。|||是|是|是|  
|CREATE TABLE 和 ALTER TABLE 中的資料行存放區索引壓縮延遲選項。|||是|是|是|   
  
 *若要建立可讀取的非叢集資料行存放區索引，請將索引儲存在唯讀的檔案群組中。  
  
## [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]  
 [!INCLUDE[ssSQL15](../../includes/sssql15-md.md)] 會加入索引鍵增強功能，以改善資料行存放區索引的效能和彈性。 這可以強化資料倉儲案例，並進行即時作業分析。  
  
### <a name="functional"></a>功能  
  
-   資料列存放區資料表可以有一個可更新的非叢集資料行存放區索引。 非叢集資料行存放區索引以前是唯讀的。  
  
-   非叢集資料行存放區索引定義支援使用篩選的條件。 使用這項功能，只建立操作式工作負載冷資料的非叢集資料行存放區索引。 如此一來，資料行存放區索引對 OLTP 資料表的效能影響會降至最低。  
...
-   資料行存放區索引的壓縮延遲選項，可將交易式工作負載對即時作業分析的影響降到最低。  這個選項允許經常變更的資料列在穩定後，再壓縮到資料行存放區。 如需詳細資訊，請參閱 [CREATE COLUMNSTORE INDEX &#40;Transact-SQL&#41;](../../t-sql/statements/create-columnstore-index-transact-sql.md) 和[開始使用資料行存放區進行即時作業分析](../../relational-databases/indexes/get-started-with-columnstore-for-real-time-operational-analytics.md)。  
