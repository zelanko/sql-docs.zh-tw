...
|索引類型|描述|其他資訊|  
|----------------|-----------------|----------------------------|  
|雜湊|有了雜湊索引，便會透過記憶體中的雜湊表來存取資料。 雜湊索引會耗用固定數量的記憶體，也就是值區計數的函數。|[使用記憶體最佳化資料表索引的方針](http://msdn.microsoft.com/library/16ef63a4-367a-46ac-917d-9eebc81ab29b)|  
|記憶體最佳化的非叢集索引|對於已完成記憶體最佳化的非叢集索引，記憶體耗用量是資料列計數和索引鍵資料行大小的函數|[使用記憶體最佳化資料表索引的方針](http://msdn.microsoft.com/library/16ef63a4-367a-46ac-917d-9eebc81ab29b)|  
