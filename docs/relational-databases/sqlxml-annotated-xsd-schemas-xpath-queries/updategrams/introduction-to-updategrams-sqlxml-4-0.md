...
### <a name="explicit-mapping"></a>明確的對應  
 ...
 如果 updategram 執行複雜的更新 （例如，根據對應結構描述中指定的父子式關聯性的多個資料表中插入記錄），您必須使用明確提供對應結構描述**對應結構描述**updategram 執行屬性。  
  
 由於 Updategram 是一個範本，因此，在 Updategram 中針對對應結構描述所指定的路徑是範本檔位置的相對路徑 (相對於儲存 Updategram 的位置)。 如需詳細資訊，請參閱 <<c0> [ 在 Updategram 中指定註解式對應結構描述&#40;SQLXML 4.0&#41;](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/updategrams/specifying-an-annotated-mapping-schema-in-an-updategram-sqlxml-4-0.md)。</c0>  
