...
description: 瞭解如何在 SQLXML 4.0 中使用 XML updategram 刪除資料。
ms.date: 03/17/2017
ms.prod: sql
...
ms.locfileid: "97462879"
---
# <a name="deleting-data-using-xml-updategrams-sqlxml-40"></a>使用 XML Updategram 刪除資料 (SQLXML 4.0)
[!INCLUDE [SQL Server Azure SQL Database](../../../includes/applies-to-version/sql-asdb.md)]
  當記錄實例出現在區塊中 **\<before>** ，但區塊中沒有對應的記錄時，updategram 會指出刪除作業 **\<after>** 。 在此情況下，updategram 會從資料庫中刪除區塊中的記錄 **\<before>** 。  
 ...
 **\<after>** 如果 updategram 只會執行刪除作業，您可以省略標記。 如果您未指定選擇性的 **對應架構** 屬性，則 **\<ElementName>** updategram 中指定的會對應到資料庫資料表，而子項目或屬性則會對應到資料表中的資料行。  
...
### <a name="a-deleting-a-record-by-using-an-updategram"></a>A. 使用 Updategram 刪除記錄  
 ...
 第一個 updategram 是以屬性為中心，並識別區塊中的兩個班 (Day 和夜間) **\<before>** 。 因為區塊中沒有對應的記錄 **\<after>** ，所以這是一項刪除作業。  
...
1.  完成範例 B ( 「使用 updategram 插入多筆記錄」 ) 在 [使用 XML Updategram 插入資料時 &#40;SQLXML 4.0&#41;](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/updategrams/inserting-data-using-xml-updategrams-sqlxml-4-0.md)。  
  
2.  將上面的 updategram 複製到 [記事本]，並將它儲存為與用來完成 ( 「使用 updategram 插入多筆記錄」 ) 的相同資料夾中的 Updategram-RemoveShifts.xml [&#40;SQLXML 4.0&#41;](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/updategrams/inserting-data-using-xml-updategrams-sqlxml-4-0.md)。  
