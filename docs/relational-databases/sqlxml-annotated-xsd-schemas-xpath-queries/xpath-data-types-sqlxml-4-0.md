...
|time<br /><br /> time.tz|字串|SUBSTRING(CONVERT(nvarchar(4000), EmployeeID, 126), 1 + CHARINDEX(N'T', CONVERT(nvarchar(4000), EmployeeID, 126)), 24)|  
 ...
 日期和時間轉換的設計目的，是要使用[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **datetime**資料類型或**字串**，將值儲存在資料庫中。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]請注意， **datetime**資料類型不會使用**時區**，而且有效位數會比 XML **time**資料類型小。 若要包含**時區**資料類型或其他有效位數，請使用[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] **字串**類型將資料儲存在中。  
...
```  
 ...
 如果@m是固定的**14.4** XDR 資料類型，則會使用下列來完成從 XDR 資料類型轉換為 XPath 資料類型的作業：  
  
```  
CONVERT(money, m)  
```  
  
 在這種轉換中， `m`節點會從**固定的 14.4**轉換成**money**。 不過，加入 3 這個值需要其他轉換：  
...
### <a name="b-perform-several-data-type-conversions-in-an-xpath-query"></a>B. 在 XPath 查詢中執行數個資料類型轉換  
 ...
 這個 XPath 查詢會傳回滿足述詞的`@UnitPrice * @OrderQty > 98` ** \<所有 OrderDetail>** 元素。 如果在批註式架構中使用**固定的 14.4**資料類型來標注**單價**，則此述詞相當於 SQL 運算式：  
...
> [!NOTE]  
>  如果在結構描述中使用的 XSD 資料類型與資料庫中的基礎 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料類型不相容，或者如果執行不可能的 XPath 資料類型轉換，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 可能會傳回錯誤。 例如，如果 [**員工**識別碼] 屬性是以**id 前置**詞注釋標注，則`Employee[@EmployeeID=1]` XPath 會產生錯誤，因為「**員工**id」具有**識別碼首碼**注釋，而且無法轉換為**數位**。  
