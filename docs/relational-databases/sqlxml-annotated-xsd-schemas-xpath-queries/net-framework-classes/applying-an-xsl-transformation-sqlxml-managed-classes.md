...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]在此範例中，針對 AdventureWorks 資料庫執行 SQL 查詢。 XSL 轉換會套用到查詢結果以產生員工名字和姓氏之兩個資料行的資料表。  
...
1.  將 XSL 樣式表儲存在檔案 (MyXSL.xsl) 中。  
  
2.  將此範例中提供的 C# 程式碼 (DocSample.cs) 儲存到儲存樣式表的相同資料夾中。  
