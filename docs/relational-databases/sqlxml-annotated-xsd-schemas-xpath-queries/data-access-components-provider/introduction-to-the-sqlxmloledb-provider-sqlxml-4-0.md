...
|連線<br /><br /> 屬性|Default<br /><br /> (如果有的話)|描述|  
|-----------------------------|----------------------------|-----------------|  
...
|Command<br /><br /> 屬性|Default<br /><br /> (如果有的話)|描述|  
|--------------------------|----------------------------|-----------------|  
...
|對應結構描述|NULL|如果用戶端應用程式針對對應結構描述 (XDR 或 XSD) 執行 XPath 查詢，這個屬性會用來指定對應結構描述的名稱。<br /><br /> 指定的路徑可以是相對路徑 (xyz/abc/MySchema.xml) 或絕對路徑 (C:\MyFolder\abc\MySchema.xml)。<br /><br /> 如果指定相對路徑，則會使用 [基底路徑] 屬性所指定的基底路徑來解析相對路徑。 如果未在 [基底路徑] 屬性中指定路徑，相對路徑會相對於目前的目錄。<br /><br /> 在指定 [對應架構] 屬性的值時，您可以指定本機目錄路徑或 URL （HTTPs://...）。如果您指定 URL，您必須將 WinHTTP 設定為透過 proxy 伺服器存取 HTTP 和 HTTPS 伺服器。 您可以執行 Proxycfg.exe 公用程式來進行這項處理。 如需詳細資訊，請參閱 MSDN Library 中的＜使用 WinHTTP Proxy 組態公用程式＞(英文)。<br /><br /> 如需使用此屬性的範例，請參閱[&#40;SQLXMLOLEDB 提供者&#41;執行 XPath 查詢](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/data-access-components-provider/executing-xpath-queries-sqlxmloledb-provider.md)。|  
|namespaces||這個屬性會讓使用命名空間的 XPath 查詢得以執行。 如需使用此屬性的範例，請參閱[使用命名空間執行 XPath 查詢 &#40;SQLXMLOLEDB 提供者&#41;](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/data-access-components-provider/executing-xpath-queries-with-namespaces-sqlxmloledb-provider.md)。|  
...
|xml root||這個屬性是用來定義產生之 XML 的根標記。 例如，如果您針對資料庫執行 SQL 查詢，而且產生的 XML 文件沒有單一根元素，此屬性的值可用來將單一根元素加入到文件中。<br /><br /> 如需使用此屬性的範例，請參閱[&#40;SQLXMLOLEDB 提供者&#41;執行 SQL 查詢](../../../relational-databases/sqlxml-annotated-xsd-schemas-xpath-queries/data-access-components-provider/executing-sql-queries-sqlxmloledb-provider.md)。|  
