...
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]
  ...
  本機用戶端 OLE 資料庫[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]提供程式 中的**IRow**介面實現得到簡化,以提高性能。 **IRow** 允許直接存取單一資料列物件的資料行。 如果您事先知道命令執行的結果只會產生單一資料列，**IRow** 就會擷取該資料列的資料行。 如果結果集包含多個資料列，**IRow** 就只會公開第一個資料列。  
