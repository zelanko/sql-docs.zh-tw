...
|EventSubClass|**nvarchar**|事件子類別的類型，針對每個事件類別提供更詳細的相關資訊。 這個資料行可包含下列值：<br /><br /> Template created：指出已經在資料庫中建立查詢通知範本。<br /><br /> Template matched：指出已重複使用查詢通知範本。<br /><br /> Template dropped：指出已從資料庫中移除查詢通知範本。|21|是|  
