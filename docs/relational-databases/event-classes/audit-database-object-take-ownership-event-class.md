...
|**ObjectType**|**整數**|代表參與事件之物件類型的值。 這個資料行傳回的值是 **sys.objects** 目錄檢視之 **type** 資料行中對應值與 [ObjectType 追蹤事件資料行](../../relational-databases/event-classes/objecttype-trace-event-column.md)中列出值的組合。 例如，如果傳回 8277-U，則物件類型是使用者自訂資料表。|28|是|  
