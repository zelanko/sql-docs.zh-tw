...
|**ObjectType**|**整數**|代表參與事件之物件類型的值。 此值對應至 **sysobjects** 資料表中的類型資料行。 針對各值，請參閱 [ObjectType 追蹤事件資料行](../../relational-databases/event-classes/objecttype-trace-event-column.md)。|28|是|  
