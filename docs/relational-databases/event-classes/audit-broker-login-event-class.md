---
description: Audit Broker 登入事件類別
...
|6|授權失敗|Authorization Failure 事件是用以報告 Service Broker 拒絕連接的授權。 對於 Windows 驗證，此事件是用以報告連接的安全性識別碼與資料庫使用者不相符。 對於以憑證為基礎的驗證，此事件是用以報告訊息中所傳遞的公開金鑰與資料庫中的憑證不一致。|  
