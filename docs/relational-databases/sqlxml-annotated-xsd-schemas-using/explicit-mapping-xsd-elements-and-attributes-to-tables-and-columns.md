...
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)]當使用 XSD 結構描述提供關聯式資料庫的 XML 檢視，元素和屬性的結構描述必須對應至資料表和資料行的資料庫。 資料庫資料表/檢視表中的資料列將會對應到 XML 文件中的元素。 資料庫中的資料行值會對應到屬性或元素。  
