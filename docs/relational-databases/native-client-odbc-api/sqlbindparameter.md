...
>  將**char**和**wchar**類型的資料插入至影像資料行時，會使用傳入的資料大小，而不是轉換成二進位格式之後的資料大小。  
 ...
 [](../../relational-databases/native-client-odbc-api/sqlrowcount.md)如果系結參數陣列[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]的陣列元素導致語句執行發生錯誤，則 SQLROWCOUNT 與 Native Client ODBC 驅動程式不可靠。 ODBC 陳述式屬性 SQL_ATTR_PARAMS_PROCESSED_PTR 會報告錯誤發生前處理的資料列數目。 接著，如果需要，此應用程式可以周遊其參數狀態陣列以探索成功執行的陳述式數目。  
