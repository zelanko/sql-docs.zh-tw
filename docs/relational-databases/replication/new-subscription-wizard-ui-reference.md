---
title: "新增訂閱精靈 (UI 參考) | Microsoft Docs"
ms.custom: 
ms.date: 03/01/2017
ms.prod: sql-non-specified
ms.prod_service: database-engine, sql-database
ms.service: 
ms.component: replication
ms.reviewer: 
ms.suite: sql
ms.technology: replication
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords: New Subscription Wizard
ms.assetid: fd201921-7c96-4fcc-85fe-61d91396429b
caps.latest.revision: "18"
author: BYHAM
ms.author: rickbyh
manager: jhubbard
ms.workload: Inactive
ms.openlocfilehash: f896ec3aef62e23f25ba58c34c54e1fe447e613a
ms.sourcegitcommit: 2713f8e7b504101f9298a0706bacd84bf2eaa174
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 11/18/2017
---
# <a name="new-subscription-wizard-ui-reference"></a>新增訂閱精靈 (UI 參考)
[!INCLUDE[appliesto-ss-asdb-xxxx-xxx-md](../../includes/appliesto-ss-asdb-xxxx-xxx-md.md)] 本節提供有關 [新增訂閱精靈] 之下列頁面的資訊：  
  
-   [&#60;AgentName&#62; 代理程式位置](../../relational-databases/replication/agentname-agent-location.md)  
  
-   [訂閱者](../../relational-databases/replication/subscribers.md)  
  
-   [新增非 SQL Server 訂閱者](../../relational-databases/replication/add-non-sql-server-subscriber.md)  
  
-   [&#60;AgentName&#62; 代理程式安全性](../../relational-databases/replication/agentname-agent-security.md)  
  
-   [可更新的訂閱](../../relational-databases/replication/updatable-subscriptions.md)  
  
-   [可更新訂閱的登入](../../relational-databases/replication/login-for-updatable-subscriptions.md)  
  
-   [初始化訂閱](../../relational-databases/replication/initialize-subscriptions.md)  
  
-   [Web 伺服器資訊](../../relational-databases/replication/web-server-information.md)  
  
-   [訂閱類型](../../relational-databases/replication/subscription-type.md)  
  
-   [HOST_NAME 值](../../relational-databases/replication/host-name-values.md)  
  
## <a name="see-also"></a>另請參閱  
 [建立提取訂閱](../../relational-databases/replication/create-a-pull-subscription.md)   
 [Create a Push Subscription](../../relational-databases/replication/create-a-push-subscription.md)   
 [訂閱發行集](../../relational-databases/replication/subscribe-to-publications.md)   
 [屬性參考 &#40;複寫&#41;](../../relational-databases/replication/properties-reference-replication.md)  
  
  
