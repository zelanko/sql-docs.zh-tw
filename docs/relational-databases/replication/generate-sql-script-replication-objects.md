...
-   **產生 SQL 指令碼**, ，這是可以從內容功能表的 **複寫** 資料夾及所有子資料夾中的 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]。 這個對話方塊可讓您編寫指令碼的執行個體上的所有複寫物件 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。  
  
-   **產生 SQL 指令碼 \< o b j>**, ，從發行集和訂閱的內容功能表。 此對話方塊可讓您編寫個別物件的指令碼。  
...
## 產生 SQL 指令碼 \< o b j> 選項  
 **若要建立或啟用元件** 和 **卸除或停用元件**  
 指定指令碼是否應包含建立或卸除複寫物件的命令。 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 建議您使用對話方塊中，建立一組啟用和停用元件的指令碼。  
  
 **複寫作業**  
 這個選項就只能從 **產生 SQL 指令碼** 對話方塊。  
