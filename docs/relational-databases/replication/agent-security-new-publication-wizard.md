...
-   允許可更新訂閱的交易式發行集之佇列讀取器代理程式。  [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 如果您指定，會建立此代理程式的代理程式作業 **具有可更新訂閱的交易式發行集** 上 **發行集類型** ] 頁面上，不論您使用的可更新訂閱類型為何。 如需有關可更新訂閱的詳細資訊，請參閱 [交易式複寫的可更新訂閱](../../relational-databases/replication/transactional/updatable-subscriptions-for-transactional-replication.md)。  
 ...
 **佇列讀取器代理程式**  
 允許可更新訂閱的交易式發行集都會顯示。 按一下 **[安全性設定]** ，即可在 **[佇列讀取器代理程式安全性]** 對話方塊中指定安全性設定。 此精靈完成時就會建立佇列讀取器代理程式作業；與您是否建立任何佇列更新訂閱無關。 如果您不要建立任何佇列更新訂閱，就可以停用此作業。 以滑鼠右鍵按一下作業 (命名格式︰ *[\< 發行者>]。 \< 整數>*。) 在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 代理程式 **工作** 資料夾，然後再按一下 **停用**。  
