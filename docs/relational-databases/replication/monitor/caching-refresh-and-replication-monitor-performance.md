...
---
# 快取、重新整理和複寫監視器效能
  [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 複寫監視器 」 設計成能有效地監視的電腦數量龐大，生產系統中。 會定期快取和重新整理「複寫監視器」用作執行計算和收集資料的查詢。 快取可減少您在「複寫監視器」中檢視不同頁面時的查詢和計算次數，並可使監視範圍擴大到多個使用者。  
  
 快取重新整理由 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 代理程式作業 **複寫監視的發佈重新整理器**。 此作業連續執行，但快取重新整理排程需要在上一次重新整理後等候一段時間：  
...
-   所有 windows 可以手動重新都整理，按下 F5，或在複寫監視器樹狀目錄中的節點上按一下滑鼠右鍵，然後按一下 **重新整理**。 手動重新整理會強制重新整理快取。  
  
 如需詳細資訊，請參閱 [重新整理複寫監視器 」 中的資料](../../../relational-databases/replication/monitor/refresh-data-in-replication-monitor.md)。  
