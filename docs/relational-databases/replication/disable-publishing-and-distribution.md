...
-   若要停用發行和散發，所有散發和發行集資料庫都必須在線上。 如果存在散發或發行集資料庫的任何 *「資料庫快照集」* ，則必須先卸除這些快照集，然後才能停用發行和散發。 資料庫快照集是資料庫的唯讀離線副本，與複寫快照集無關聯。 如需詳細資訊，請參閱 [資料庫快照集 & #40。SQL Server & #41;](../../relational-databases/databases/database-snapshots-sql-server.md)。  
...
2.  以滑鼠右鍵按一下 **複寫** 資料夾，然後再按一下 **停用發行與散發**。  
...
2.  在訂閱資料庫上的每個訂閱者，執行 [sp_removedbreplication](../../relational-databases/system-stored-procedures/sp-removedbreplication-transact-sql.md) 從資料庫移除複寫物件。 這個預存程序將不會移除散發者上的複寫作業。  
  
3.  在發行集資料庫的發行者，執行 [sp_removedbreplication](../../relational-databases/system-stored-procedures/sp-removedbreplication-transact-sql.md) 從資料庫移除複寫物件。  
  
4.  如果 「 發行者 」 使用遠端散發者，執行 [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md)。  
  
5.  在散發者 」 執行 [sp_dropdistpublisher](../../relational-databases/system-stored-procedures/sp-dropdistpublisher-transact-sql.md)。 應該針對散發者上註冊的每一個發行者執行此預存程序一次。  
  
6.  在散發者 」 執行 [sp_dropdistributiondb](../../relational-databases/system-stored-procedures/sp-dropdistributiondb-transact-sql.md) 刪除散發資料庫。 應該針對散發者上的每一個散發資料庫執行此預存程序一次。 這樣也會移除與散發資料庫有關的任何佇列讀取器代理程式作業。  
  
7.  在散發者 」 執行 [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md) 從伺服器移除散發者的指定。  
  
    > [!NOTE]  
    >  如果所有的複寫發行和散發物件不會卸除然後再執行 [sp_dropdistpublisher](../../relational-databases/system-stored-procedures/sp-dropdistpublisher-transact-sql.md) 和 [sp_dropdistributor](../../relational-databases/system-stored-procedures/sp-dropdistributor-transact-sql.md), ，這些程序會傳回錯誤。 若要卸除所有與複寫相關的物件時卸除發行者或散發者， **@no_checks** 參數必須設定為 **1**。 如果發行者或散發者已離線或無法存取， **@ignore_distributor** 參數可以設定為 **1** ，以便它們可以卸除; 不過，任何發行和散發物件留必須手動移除。  
...
3.  建立連接到散發者 」 使用 <xref:Microsoft.SqlServer.Management.Common.ServerConnection> 類別。  
  
4.  建立的執行個體 <xref:Microsoft.SqlServer.Replication.DistributionPublisher> 類別。 指定 <xref:Microsoft.SqlServer.Replication.DistributionPublisher.Name%2A> 屬性，並傳遞 <xref:Microsoft.SqlServer.Management.Common.ServerConnection> 步驟 3 中的物件。  
  
5.  （選擇性）呼叫 <xref:Microsoft.SqlServer.Replication.ReplicationObject.LoadProperties%2A> 方法，以取得物件的屬性，並確認發行者確實存在。 如果此方法傳回 **false**，則表示步驟 4 中設定的發行者名稱不正確，或是此散發者並未使用此發行者。  
...
7.  建立的執行個體 <xref:Microsoft.SqlServer.Replication.ReplicationServer> 類別。 傳遞 <xref:Microsoft.SqlServer.Management.Common.ServerConnection> 步驟 3 中的物件。  
