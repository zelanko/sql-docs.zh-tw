...
helpviewer_keywords: 
  ...
  - "記錄讀取器代理程式, 管理"
  - "佇列讀取器代理程式, 管理"
  - "共用代理程式 [SQL Server 複寫]"
  - "合併代理程式, 管理"
  - "散發代理程式, 管理"
  - "代理程式 [SQL Server 複寫], 管理"
  - "複寫清除作業 [SQL Server]"
  - "管理複寫, 代理程式"
  - "複寫 [SQL Server], 管理"
  - "獨立代理程式 [SQL Server 複寫]"
...
---
# 複寫代理程式管理
  複寫代理程式可執行許多有關複寫的工作，包含建立結構描述和資料的副本、偵測「發行者」或「訂閱者」端的更新，以及在伺服器之間傳播變更。 根據預設，複寫代理程式執行 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 代理程式作業步驟。 此代理程式只不過是可執行檔，所以也可以從命令列和批次指令碼直接呼叫。 每個複寫代理程式都支援一組用於控制其執行方式的執行時期參數；這些參數在代理程式設定檔或命令列中指定。  
...
-   [!INCLUDE[ssManStudioFull](../../../includes/ssmanstudiofull-md.md)] 和 「 複寫監視器︰ [啟動和停止複寫代理程式 & #40。SQL Server Management Studio & #41;](../../../relational-databases/replication/agents/start-and-stop-a-replication-agent-sql-server-management-studio.md)  
...
## 代理程式設定檔  
 ...
 直接指定命令列參數的相關資訊，請參閱 [複寫代理程式可執行檔概念](../../../relational-databases/replication/concepts/replication-agent-executables-concepts.md)。  
...
-   下列代理程式與複寫監視器中的發行集相關聯：  
    ...
    -   佇列讀取器代理程式  
     ...
     存取資訊和透過這些代理程式相關聯的工作 **代理程式** ] 索引標籤。 如需詳細資訊，請參閱 [檢視資訊並執行工作的代理程式相關聯的發行集與 #40。複寫監視器 & #41;](../../../relational-databases/replication/monitor/view information and perform tasks for publication agents.md)。  
...
|複寫代理程式檢查|偵測並未動態記錄歷程的複寫代理程式。 它會寫入 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] Windows 事件記錄檔作業步驟失敗時。|每十分鐘執行一次。|  
