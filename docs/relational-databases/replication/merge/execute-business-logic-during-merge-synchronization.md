...
ms.topic: "article"
helpviewer_keywords: 
  - "自訂錯誤解決 [SQL Server 複寫]"
  - "自訂變更處理 [SQL Server 複寫]"
  - "錯誤 [SQL Server 複寫], 商務邏輯處理常式"
...
-   應用程式發展介面 (API) 參考︰ <xref:Microsoft.SqlServer.Replication.BusinessLogicSupport>  
  
-   指示如何實作商務邏輯處理常式︰ [為合併發行項實作商務邏輯處理常式](../../../relational-databases/replication/implement-a-business-logic-handler-for-a-merge-article.md)  
...
-   套用自訂資料  
     ...
     這對於需要覆寫特定資料值或作業的應用程式非常有用。 應用程式，例如無法將刪除的資料列轉換成特殊更新，設定 **狀態** 「 刪除 」 的值的資料列資料行，然後追蹤執行刪除用戶端的身分識別。 可用於稽核或工作流程。  
  
### 自訂衝突解決方案  
 合併式複寫提供了衝突偵測和解決，可讓您接受預設解決策略或選取自訂的衝突解決方案。 如需詳細資訊，請參閱 [Advanced Merge Replication Conflict Detection and Resolution](../../../relational-databases/replication/merge/advanced-merge-replication-conflict-detection-and-resolution.md)。 商務邏輯處理常式可以在處理衝突的資料變更期間叫用，並可執行下列兩個動作之一：  
