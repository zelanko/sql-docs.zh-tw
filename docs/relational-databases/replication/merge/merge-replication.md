...
description: 合併式複寫會使用發行集資料庫物件和資料的快照集，然後使用觸發程序追蹤發行者和訂閱者位置的修改。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
