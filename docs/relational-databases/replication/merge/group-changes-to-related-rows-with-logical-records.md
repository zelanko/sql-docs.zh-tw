...
> [!NOTE]  
>  邏輯記錄功能可以獨立使用，也可以與聯結篩選結合使用。 如需聯結篩選的詳細資訊，請參閱 [聯結篩選](../../../relational-databases/replication/merge/join-filters.md)。 若要使用邏輯記錄，發行集的相容性層級必須至少為 90RTM。  
...
### 將變更當做一個單位  
 如果合併處理中斷 (例如出現連接被卸除)，則在使用了邏輯記錄的情況下，部分完成的那組相關複寫變更會回復。 例如，假設訂閱者要將加入一個新訂單 **OrderID** = 6 和兩個新資料列的 **OrderItems** 資料表具有 **OrderItemID** = 10 和 **OrderItemID** = 11 **OrderID** = 6。  
  
 ![具有值的三個資料表邏輯記錄](../../../relational-databases/replication/merge/media/logical-records-04.gif "具有值的三個資料表邏輯記錄")  
  
 如果複寫程序中斷之後 **訂單** 資料列 **OrderID** = 6 之前已完成， **OrderItems** 10 和 11 完成時，並不會使用邏輯記錄， **OrderTotal** 值 **OrderID** = 6 會不一致的加總 **OrderAmount** 值 **OrderItems** 資料列。 如果使用了邏輯記錄， **訂單** 資料列 **OrderID** = 6 之前，不認可相關 **OrderItems** 複寫變更。  
  
 另一個實例中，如果使用了邏輯記錄，且當合併處理套用變更時，尚有人在查詢資料表，則使用者將看不到僅部分複寫的變更，直至它們全部完成。 例如，複寫程序已上傳的訂單資料列 **OrderID** = 6，但使用者查詢的資料表有複寫的複寫程序之前 **OrderItems** 資料列， **OrderTotal** 值不是相同的總和 **OrderAmount** 值。 如果使用了邏輯記錄，則在 **OrderItems** 資料列的處理完成，且交易做為一個單位得到認可之前，使用者將看不到 **Orders** 資料列。  
...
-   第二個「訂閱者」端的使用者將 **OrderItemID** 6 的 **OrderAmount** 從 25 變更到 125，將 **OrderID** 3 的 **OrderTotal** 從 200 變更到 300。  
 ...
 如果這些變更在未使用邏輯記錄的情況下進行了複寫，則不同的 **OrderTotal** 值會造成衝突，且僅會複寫它們中的一個。 中的非衝突變更，但 **OrderItems** 資料表會複寫而不致發生衝突，使最終 **OrderTotal** 值不一致的狀態以 **OrderItems** 資料列。 如果在此實例中使用了邏輯記錄，與失敗的 **Orders** 資料表變更相關聯的 **OrderItems** 變更也會被回復，且最終的 **OrderTotal** 值會是 **OrderItems** 資料列的精確總和。  
  
 如需有關衝突偵測和解決邏輯記錄的相關選項的詳細資訊，請參閱 [偵測及解決邏輯記錄衝突](../../../relational-databases/replication/merge/detecting-and-resolving-conflicts-in-logical-records.md)。  
...
-   已發行資料表中的外部索引鍵關聯性無法使用 CASCADE 選項進行定義。 如需詳細資訊，請參閱 [CREATE TABLE & #40。TRANSACT-SQL & #41;](../../../t-sql/statements/create-table-transact-sql.md) 和 [ALTER 資料表 & #40。TRANSACT-SQL & #41;](../../../t-sql/statements/alter-table-transact-sql.md)。  
...
-   發行集的相容性層級必須為 90RTM 或更高。 如需詳細資訊，請參閱 「 發行集相容性層級 」 一節 [複寫回溯相容性](../../../relational-databases/replication/replication-backward-compatibility.md)。  
...
-   為了在篩選的發行集中使用邏輯記錄：  
    ...
    -   必須同時使用預先計算資料分割。 對預先計算資料分割的要求也同樣適用於邏輯記錄。 如需詳細資訊，請參閱 [最佳化 Parameterized Filter Performance with Precomputed Partitions](../../../relational-databases/replication/merge/optimize-parameterized-filter-performance-with-precomputed-partitions.md)。  
  
    -   無法使用不重疊的參數化篩選。 如需詳細資訊，請參閱＜ [Parameterized Row Filters](../../../relational-databases/replication/merge/parameterized-row-filters.md)＞的「設定資料分割選項」一節。  
