---
title: "發行集資訊，警告 (合併式發行集，SQL Server 2005 和更新的版本) | Microsoft Docs"
ms.custom: ""
ms.date: "03/04/2017"
...
---
# 發行集資訊，警告 (合併式發行集，SQL Server 2005 和更新的版本)
...
-   超過指定的同步處理時間。  
     ...
     如果達到或超過指定的臨界值，訂閱狀態會顯示為 **長時間執行的合併** （除非需要顯示優先順序較高的問題）。  
...
-   在給定時間內處理的資料列數達不到指定數目。  
     ...
     如果達到或超過指定的臨界值，訂閱狀態會顯示為 **效能嚴重不足** （除非需要顯示優先順序較高的問題）。  
