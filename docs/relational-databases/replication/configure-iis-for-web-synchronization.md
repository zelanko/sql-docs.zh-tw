...
-   [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]:[操作說明：設定訂閱來使用 Web 同步處理 \(SQL Server Management Studio\)](https://msdn.microsoft.com/library/ms345214.aspx)  
  
-   複寫 [!INCLUDE[tsql](../../includes/tsql-md.md)] 程式設計：[操作說明：設定訂閱來使用 Web 同步處理 (複寫 Transact-SQL 程式設計)](https://msdn.microsoft.com/library/ms345206.aspx)  
  
-   RMO：[操作說明：設定訂閱來使用 Web 同步處理 (RMO 程式設計)](https://msdn.microsoft.com/library/ms345207.aspx)  
...
> [!IMPORTANT]  
>  確定您的應用程式只使用 [!INCLUDE[dnprdnlong](../../includes/dnprdnlong-md.md)] 或更新版本，而且 IIS 伺服器上未安裝較早版本的 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 。 較早版本的 [!INCLUDE[dnprdnshort](../../includes/dnprdnshort-md.md)] 可能導致錯誤。 這些選項包括：「Web 同步處理期間的訊息格式無效。 請確認已在 Web 伺服器正確地設定複寫元件」。  
...
6.  在 **[虛擬目錄資訊]** 頁面中：  
    ...
    2.  在 **[路徑]** 方塊中輸入虛擬目錄的路徑。 例如，如果您在 [別名]  方塊中輸入 **websync1** ，請在 [路徑]  方塊中輸入 **C:\Inetpub\wwwroot\websync1** 。 按 [下一步]  。  
