---
title: "含參數化篩選之合併式發行集的快照集 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
---
# 含參數化篩選之合併式發行集的快照集
  在合併發行集內使用參數化資料列篩選器時，覆寫會以兩段式的快照集來初始化每一個訂閱。 首先建立包含複寫所需之所有物件以及已發行物件之結構描述的結構描述快照集，但不含資料。 然後使用包含結構描述快照集中物件與結構描述以及訂閱之資料分割所屬資料的快照集，來初始化每個訂閱。 如果有多個訂閱收到給定的資料分割 (即收到相同的結構描述和資料)，該資料分割的快照集只會建立一次，多個訂閱均從同一快照集初始化。 如需參數化資料列篩選器的詳細資訊，請參閱 [參數化資料列篩選](../../relational-databases/replication/merge/parameterized-row-filters.md)。  
...
-   允許訂閱者在首次執行同步處理時，要求快照集產生和應用程式。 使用此選項可讓新「訂閱者」執行同步處理，而無需管理員介入 (必須在「發行者」端執行 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent，才允許產生快照集)。  
    ...
    > [!NOTE]  
    >  如果發行集內的一或多個發行項的篩選產生對每個訂閱而言是唯一的非重疊資料分割，則只要合併代理程式一執行，就會清除中繼資料。 這表示分割快照集會更快過期。 使用這個選項時，您應該考慮允許訂閱者初始化快照集的產生與傳遞。 如需有關篩選選項的詳細資訊，請參閱 [參數化資料列篩選](../../relational-databases/replication/merge/parameterized-row-filters.md)。  
...
-   使用「快照集代理程式」為每個「訂閱者」手動產生快照集。 然後，「訂閱者」必須為「合併代理程式」提供快照集位置，才能擷取和套用正確的快照集。  
 ...
 若要使用參數化篩選建立發行集的快照集，請參閱 [建立合併式發行集使用參數化篩選的快照集](../../relational-databases/replication/create-a-snapshot-for-a-merge-publication-with-parameterized-filters.md)。  
  
## 快照集代理程式的安全性設定  
 「快照集代理程式」會為每個資料分割建立快照集。 預先產生快照集和訂閱者所要求的快照集，代理程式執行和連接建立發行集的快照集代理程式作業時所指定的認證 (在工作建立新的發行集精靈 」 或 **sp_addpublication_snapshot**)。 若要變更的認證，使用 **sp_changedynamicsnapshot_job**。 如需詳細資訊，請參閱 [sp_changedynamicsnapshot_job & #40。TRANSACT-SQL & #41;](../../relational-databases/system-stored-procedures/sp-changedynamicsnapshot-job-transact-sql.md)。  
