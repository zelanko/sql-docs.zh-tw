---
title: "指定交易式發行項變更的傳播方式 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
-   指定不在任何「訂閱者」端執行此動作。 不會複寫該類型的交易。  
 ...
 若要設定交易式發行項的資料變更傳播方法，請參閱 [之交易式發行項的資料變更設定傳播方法](../../../relational-databases/replication/publish/set-the-propagation-method-for-data-changes-to-transactional-articles.md)。  
  
## 預設與自訂預存程序  
 依預設，複寫為每個資料表發行項建立的三個程序為：  
  
-   **sp_MSins_ \<** *tablename* **>**, ，處理插入。  
  
-   **sp_MSupd_ \<** *tablename* **>**, ，以處理更新。  
  
-   **sp_MSdel_ \<** *tablename* **>**, ，處理刪除。  
  
  **\<***Tablename***>** 中使用此程序取決於發行項新增至發行集的方式，以及訂閱資料庫是否包含具有不同擁有者具有相同名稱的資料表。  
...
-   「訂閱者」端的結構描述通常與「發行者」端的結構描述相同，但如果使用的是資料行篩選，則也可能是「發行者」結構描述的子集。 不過，如果您要轉換為結構描述資料移動，「 訂閱者 」 上的結構描述不是發行者上結構描述的子集 [!INCLUDE[ssISCurrent](../../../includes/ssiscurrent-md.md)] 是建議的解決方案。 如需詳細資訊，請參閱 [SQL Server Integration Services](../../../integration-services/sql-server-integration-services.md)。  
  
-   如果對發行的資料表進行了結構描述變更，則必須重新產生自訂程序。 如需詳細資訊，請參閱 [重新產生自訂交易程序以反映結構描述變更](../../../relational-databases/replication/transactional/regenerate-custom-transactional-procedures-to-reflect-schema-changes.md)。  
  
-   如果您使用的值大於 1 的 **-SubscriptionStreams** 散發代理程式參數，您必須確定主索引鍵資料行的更新會成功。 例如：  
...
#### SCALL 語法  
 UPDATE 預存程序  
 處理 UPDATE 陳述式的預存程序傳遞更新的值僅針對資料行有變更，接著是主索引鍵資料行的原始值，再接著位元遮罩 (**binary （n)**) 參數，指出已變更的資料行。 在下列範例中，資料行 2 (c2) 未變更：  
...
#### MCALL 語法  
 UPDATE 預存程序  
 處理 UPDATE 陳述式的預存程序傳遞的後面接著主要的索引鍵資料行，後面接著位元遮罩的原始值的文件中定義的所有資料行之更新的值 (**binary （n)**) 參數，指出已變更的資料行︰  
...
> [!NOTE]  
>  使用 XCALL 時，之前的映像值 **文字** 和 **映像** 資料行應該是 NULL。  
  
## 範例  
 下列程序是為 [!INCLUDE[ssSampleDBCoShort](../../../includes/sssampledbcoshort-md.md)] 範例資料庫中的`Vendor Table`建立的預設程序。  
