...
4.  在「 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 散發者」端將 Oracle 資料庫設為「發行者」。  
 ...
 如需可以從 Oracle 資料庫複寫的物件，請參閱 [設計考量和限制的 Oracle 發行者](../../../relational-databases/replication/non-sql/design-considerations-and-limitations-for-oracle-publishers.md)。  
  
> [!NOTE]  
>  您必須是屬於 **sysadmin** 固定的伺服器角色，以啟用 「 發行者 」 或 「 散發者 」，並建立 Oracle 發行集或訂閱從 Oracle 發行集。  
  
## 在 Oracle 資料庫中建立複寫管理的使用者結構描述  
...
> [!NOTE]  
>  卸除 **MSSQLSERVERDISTRIBUTOR** 公用同義字和設定的 Oracle 複寫使用者 **CASCADE** 選項會從 「 Oracle 發行者 」 移除所有複寫物件。  
  
 在 Oracle 複寫使用者結構描述的安裝程式中會提供範例指令碼進行輔助。 指令碼位於下列目錄安裝後 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]: *\< 磁碟機>*:\\files\microsoft SQL Server\\*\< e>*\MSSQL\Install\oracleadmin.sql。 它也包含在＜ [Script to Grant Oracle Permissions](../../../relational-databases/replication/non-sql/script-to-grant-oracle-permissions.md)＞主題中。  
  
 使用具有 DBA 權限的帳戶連接到 Oracle 資料庫並執行指令碼。 此指令碼會提示輸入複寫管理使用者結構描述的使用者與密碼，以及要在其中建立物件的預設資料表空間 (資料表空間必須已經存在於 Oracle 資料庫中)。 指定其他資料表空間物件的相關資訊，請參閱 [管理 Oracle 資料表空間](../../../relational-databases/replication/non-sql/manage-oracle-tablespaces.md)。 可以選擇任何使用者名稱和增強式密碼，但是需將兩者都記下來，因為稍後當您將 Oracle 資料庫設為「發行者」時會提示需要此資訊。 建議僅將結構描僅用於複寫所需的物件；不要建立資料表來發行於此結構描述中。  
...
## 在 SQL Server 散發者上安裝和設定 Oracle 用戶端網路軟體  
...
### 測試 SQL Server 散發者和 Oracle 發行者之間的連接  
...
5.  如果您遇到無法連線到 Oracle 資料庫，請參閱 「 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 散發者無法連接到 Oracle 資料庫執行個體 」 在 [Oracle 發行者疑難排解](../../../relational-databases/replication/non-sql/troubleshooting-oracle-publishers.md)。  
...
## 在 SQL Server 散發者端將 Oracle 資料庫設為發行者  
...
>  Oracle 發行者的名稱不能與其 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 散發者相同，也不能與使用同一散發者的任何 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 發行者相同。  
 ...
 在「 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 散發者」端識別「Oracle 發行者」後，複寫會建立名稱與 Oracle 資料庫之 TNS 服務名稱相同的連結伺服器。 此連結伺服器只能由複寫使用。 如果您需要透過連結的伺服器連線連接到 Oracle 發行者，建立另一個 TNS 服務名稱，然後使用此名稱呼叫時 [sp_addlinkedserver & #40。TRANSACT-SQL & #41;](../../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md)。  
