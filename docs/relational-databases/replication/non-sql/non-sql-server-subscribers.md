...
-   依預設，對於非「[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 訂閱者」，「散發代理程式」為訂閱資料庫 (「散發代理程式」的 **-SubscriberDB** 參數) 使用 [(預設目的地)] 的值：  
    ...
    -   對於 IBM DB2，在 DB2 連接字串中指定資料庫。 如需詳細資訊，請參閱＜ [Create a Subscription for a Non-SQL Server Subscriber](../../../relational-databases/replication/create-a-subscription-for-a-non-sql-server-subscriber.md)＞。  
