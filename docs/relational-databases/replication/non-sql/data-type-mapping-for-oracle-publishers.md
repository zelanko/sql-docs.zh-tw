...
---
# Oracle 發行者的資料類型對應
  Oracle 資料型別和 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 資料類型不一定完全相符。 若有可能，將在發行 Oracle 資料表時自動選取相符的資料類型。 如果單一資料類型對應不清楚，將提供替代的資料類型對應。 如需有關如何選取替代對應的詳細資訊，請參閱本主題稍後的「指定替代資料類型對應」一節。  
...
### 大型物件類型  
 ...
 多個大型物件類型的詳細資訊，請參閱 「 大型物件考量 」 一節，在 [設計考量和限制的 Oracle 發行者](../../../relational-databases/replication/non-sql/design-considerations-and-limitations-for-oracle-publishers.md)。  
...
-   使用預存程序對所有未來發行項的預設值進行全域變更 (不變更現有發行項的預設值)。  
 ...
 若要指定替代資料類型對應，請參閱 [Oracle 發行者指定資料型別對應](../../../relational-databases/replication/publish/specify-data-type-mappings-for-an-oracle-publisher.md)。  
