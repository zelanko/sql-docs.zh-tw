...
---
# Microsoft 複寫衝突檢視器 (異動複寫)
  「複寫衝突檢視器」可讓您檢視在點對點異動複寫和具有佇列更新訂閱之異動複寫的同步處理期間發生的衝突。 如需詳細資訊，請參閱 [適用於交易式發行集和 #40; 檢視資料衝突SQL Server Management Studio & #41;](../../relational-databases/replication/view-data-conflicts-for-transactional-publications-sql-server-management-studio.md)。  
...
## 選項  
 ...
 下方區段中的衝突資料會顯示在兩個對應資料行 (**衝突成功者** 和 **衝突失敗者**)。 如果衝突是發生在已更新和已刪除的資料之間，那麼衝突中已刪除的一方可能沒有資料可以顯示。 在此情況下，複寫衝突檢視器會在其中一個資料行裡顯示訊息，這表示資料列在一處已遭刪除，而在另一處已被更新。 它也會指出建議的解決方式。  
 ...
 **記錄衝突的詳細資料**  
 選取此方塊即可將衝突的詳細資料記錄到檔案。 若要指定檔案的位置，請指向 **[檢視]** 功能表，然後按一下 **[選項]**。 輸入值，或按一下瀏覽 (**...**)，並瀏覽至適當的檔案。 按一下 **[確定]** 即可結束 **[選項]** 對話方塊。  
