...
ms.topic: "article"
helpviewer_keywords: 
  - "內嵌資料驗證 [SQL Server 複寫]"
  - "管理複寫, 驗證資料"
  - "複寫 [SQL Server 複寫], 驗證資料"
  - "異動複寫, 驗證資料"
  - "驗證資料"
  - "合併式複寫資料驗證 [SQL Server 複寫]"
  - "合併式複寫資料驗證 [SQL Server 複寫], 關於資料驗證"
  - "驗證複寫的資料"
...
-   設定複寫警示，名為 **複寫︰ 訂閱者資料驗證失敗** ，因此您會收到通知的失敗。 如需詳細資訊，請參閱 [設定預先定義的複寫警示 & #40。SQL Server Management Studio & #41;](../../relational-databases/replication/administration/configure-predefined-replication-alerts-sql-server-management-studio.md)。  
  
-   您的應用程式是否有驗證失敗的問題？ 如果有驗證失敗的問題，請手動更新資料以便對其進行同步處理，或重新初始化訂閱：  
  
    -   資料可以使用更新 [tablediff 公用程式](../../tools/tablediff-utility.md)。 如需有關如何使用此公用程式的詳細資訊，請參閱 [比較複寫資料表差異 & #40。複寫程式設計 & #41;](../../relational-databases/replication/administration/compare-replicated-tables-for-differences-replication-programming.md)。  
  
    -   如需重新初始化的詳細資訊，請參閱 [重新初始化訂閱](../../relational-databases/replication/reinitialize-subscriptions.md)。  
...
-   複寫驗證會使用 **總和檢查碼** 和 **binary_checksum** 函式。 其行為的相關資訊，請參閱  [總和檢查碼 & #40。TRANSACT-SQL & #41;](../../t-sql/functions/checksum-transact-sql.md) 和 [BINARY_CHECKSUM & #40。TRANSACT-SQL & #41;](../../t-sql/functions/binary-checksum-transact-sql.md)。  
...
## 資料驗證如何運作  
 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 所計算的資料列計數或總和檢查碼，在 「 發行者 」，然後比較這些值的資料列計數或總和檢查碼計算 「 訂閱者 」 來驗證其資料。 整個發行集資料表計算一個值，而且整個訂閱資料表，但資料中的會計算一個值 **文字**, ，**ntext**, ，或 **映像** 計算中不包含資料行。  
