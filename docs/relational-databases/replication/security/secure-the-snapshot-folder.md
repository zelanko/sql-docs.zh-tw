...
> [!NOTE]  
>  使用者帳戶控制 (UAC) 可協助系統管理員管理其提高權限的使用者權限 (有時稱為 *權限*)。 在已啟用 UAC 的作業系統上執行時，管理員不會使用其管理權限。 反而會以標準 (非管理員) 使用者的身分執行大部分的動作，只有在必要時才會採用其管理權限。 UAC 可以防止以管理員權限存取快照共用。 因此，您必須針對快照集代理程式、散發代理程式和合併代理程式所使用的 Windows 帳戶，明確地授與快照集共用權限。 即使 Windows 帳戶是管理員群組的成員，也必須這麼做。  
  
 設定時透過 「 設定散發精靈 」 或 「 新增發行集精靈 」 的 「 散發者 」，快照集資料夾預設為本機路徑︰ X:\Program Files\Microsoft SQL Server\\*\< 執行個體>*\MSSQL\ReplData。 如果您使用遠端散發者或提取訂閱，您必須指定 UNC 網路共用 (例如 \\\\<*電腦名稱 >*\snapshot) 而不是本機路徑。  
...
## 透過 FTP 傳遞快照集  
 ...
 若要傳遞透過 FTP 快照集，請參閱 [Deliver a Snapshot Through FTP](../../../relational-databases/replication/publish/deliver-a-snapshot-through-ftp.md)。  
