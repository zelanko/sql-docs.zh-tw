...
[!INCLUDE[tsql-appliesto-ss2008-xxxx-xxxx-xxx_md](../../includes/tsql-appliesto-ss2008-xxxx-xxxx-xxx-md.md)]
  ...
  本主題中的程序將引導您完成手動設定的程序 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 網際網路資訊服務 (IIS) 版本 7 和更新版本的合併式複寫使用 Web 同步處理。 
  
 設定 IIS 7 或更新版本是啟用 Web 同步處理所需的三個步驟的第一個。  
  
 如需整個設定程序的概觀，請參閱 [設定 Web 同步處理](../../relational-databases/replication/configure-web-synchronization.md)。  
...
## 安裝 SQL Server Replication Listener  

IIS 5.0 版起支援 web 同步處理。 設定 Web 同步處理精靈的 IIS 5 和 6 版不提供 IIS 7.0 版或更高版本。 **從 SQL Server 2012、 IIS 伺服器上使用 web 同步處理元件應安裝 SQL Server 複寫。 這可以是免費的 SQL Server Express edition。**
  
#### 若要安裝和設定 SQL Server Replication Listener  
  
1.  IIS 電腦上安裝 SQL Server 複寫。

2. 在執行 IIS 的電腦上，針對 replisapi.dll 建立新的檔案目錄。 您可以建立該目錄位置，但我們建議您先建立目錄下的 \<*機*>: \Inetpub 目錄。 例如，建立目錄 \<*機*>: \Inetpub\SQLReplication\\。  
  
3.  將 replisapi.dll 從目錄 [!INCLUDE[ssInstallPathVar](../../includes/ssinstallpathvar-md.md)]com\ 複製到您在步驟 1 中建立的檔案目錄。  
...
6.  在 IIS 中建立虛擬目錄。 虛擬目錄應該在步驟 4 所建立的網站之下建立，而且應該對應至步驟 1 所建立的目錄。 在指派此目錄的權限時愈嚴格愈好。 您至少必須選取 **[讀取]** 和 **[執行]** 權限。  
    ...
    1.  在 **網際網路資訊服務 (IIS) 管理員**, 中 **連線** ] 窗格中，以滑鼠右鍵按一下 **預設的網站**, ，然後選取 **新增虛擬目錄**。  
  
    2.  針對 **[別名]**，輸入 **SQLReplication**。  
  
    3.  如 **實體路徑**, ，輸入 **\< 磁碟機>: \Inetpub\SQLReplication\\**, ，然後按一下 [ **確定**。  
  
7.  設定 IIS，使 replisapi.dll 能夠執行。  
  
    1.  在 **網際網路資訊服務 (IIS) 管理員**, ，按一下 [ **預設的網站**。  
    ...
    5.  從 **模組** 下拉式清單中，選取 **IsapiModule**。  
  
    6.  如 **可執行檔**, ，輸入 **\< 磁碟機>: \Inetpub\SQLReplication\replisapi.dll**。  
    ...
    10. 確認 Replisapi.dll 是否列於 **[已啟用]** 處理常式對應底下。 如果是在 **已停用** 清單中，以滑鼠右鍵按一下 Replisapi 項目，然後按一下 [ **編輯功能權限**。 核取 **[執行]** 方塊，然後按一下 **[確定]**。  
...
1.  在 **網際網路資訊服務 (IIS) 管理員**, ，展開 [本機伺服器] 節點，然後按一下 **Default Web Site** （或您的同步處理網站是否不同於預設的網站）。  
...
1.  在 **網際網路資訊服務 (IIS) 管理員**, ，按一下 [本機伺服器] 節點，然後在中央窗格中，按兩下 **伺服器憑證**。  
...
1.  在 **連線** ] 窗格中，按一下 [ **Default Web Site** （或您的同步處理網站，如果不同於預設的網站）。  
  
2.  在 **[動作]** 窗格中，按一下 **[繫結]**，然後按一下 **[新增]**。 **[新增站台繫結]** 對話方塊將會出現。  
  
3.  從 **類型** 下拉式清單中，選取 **https**。 保留 **[IP 位址]** 和 **[連接埠]**的預設設定。  
  
4.  從 **SSL 憑證** 按一下下拉式清單中，選取憑證建立在 「 建立自我簽署的憑證來進行測試，」 **確定**, ，然後按一下 [ **關閉**。  
  
###### 若要測試憑證  
  
1.  在 **網際網路資訊服務 (IIS) 管理員**, ，按一下 [ **預設的網站。**  
  
2.  從 **動作** ] 窗格中，按一下 [ **瀏覽 \*: 443(https)**。  
  
3.  Internet Explorer 將開啟並顯示一則訊息：「此網站的安全性憑證有問題」。 這則警告是要告知您，相關聯的憑證是由無法辨識的 CA 所核發，而且可能不值得信任。 這是預期的警告，所以按 **繼續瀏覽此網站 （不建議）**。  
...
1.  在執行 IIS 的電腦上建立本機帳戶：  
    ...
    1.  開啟 **[伺服器管理員]**。 從 [開始] 功能表中，以滑鼠右鍵按一下 **電腦**, ，然後按一下 [ **管理**。  
...
2.  將此帳戶加入至 IIS_IUSRS 群組：  
    ...
    2.  以滑鼠右鍵按一下 **IIS_IUSRS**, ，然後按一下 [ **新增到群組**。  
  
    3.  在 **IIS_IUSRS 內容** 對話方塊中，按一下 [ **新增**。  
  
    4.  在 **[選取使用者、電腦或群組]** 對話方塊中，加入在步驟 1 建立的帳戶。  
  
    5.  確認 **從這個位置** 顯示本機電腦 （而非網域） 的名稱。 如果此欄位沒有顯示本機電腦名稱，請按一下 **[位置]**。 在 **[位置]** 對話方塊中，選取本機電腦，然後按一下 **[確定]**。  
  
    6.  在 **選取使用者** 對話方塊和 **IIS_IUSRS 內容** 對話方塊中，按一下 [**確定**。  
  
3.  將包含 replisapi.dll 之資料夾的最小權限授與帳戶：  
  
    1.  在 Windows 檔案總管] 中以滑鼠右鍵按一下您先前為 replisapi.dll，建立的資料夾，然後按一下 [ **屬性**。  
  
    2.  在 **[安全性]** 索引標籤上，按一下 **[編輯]**。  
  
    3.  在 **權限 \< foldername>** 對話方塊中，按一下 [ **新增** 新增您在步驟 1 中建立的帳戶。  
  
    4.  確認 **從這個位置** 顯示本機電腦 （而非網域） 的名稱。 如果此欄位沒有顯示本機電腦名稱，請按一下 **[位置]**。 在 **[位置]** 對話方塊中，選取本機電腦，然後按一下 **[確定]**。  
  
    5.  確認帳戶已授與僅 **讀取**, ，**讀取及執行**, ，和 **列出資料夾內容** 權限。  
  
    6.  選取任何不需要存取目錄的使用者或群組、按一下 **[移除]**，然後按一下 **[確定]**。  
  
4.  建立應用程式集區中的 **網際網路資訊服務 (IIS) 管理員**:  
  
    1.  在 **網際網路資訊服務 (IIS) 管理員**, 中 **連線** ] 窗格中，展開本機伺服器節點。  
    ...
    > [!NOTE]  
    >  如果您預期有兩個以上的並行同步處理用戶端，可能會想要建立 Web 處理序區。 如需詳細資訊，請參閱 「 建立 Web 處理序區 」 中的 [設定 Web 同步處理](../../relational-databases/replication/configure-web-synchronization.md)。  
  
5.  將帳戶與應用程式集區相關聯：  
  
    1.  在 **網際網路資訊服務 (IIS) 管理員**, ，展開本機伺服器] 節點，然後按一下 [ **應用程式集區**。  
  
    2.  以滑鼠右鍵按一下您所建立，然後按一下 [應用程式集區 **設定應用程式集區預設值**。  
    ...
    7.  按一下 [確定]  關閉 [應用程式集區識別]  對話方塊，然後再按一次 [確定]  關閉 [應用程式集區預設值] 對話方塊。  
  
6.  讓應用程式集區與複寫網站產生關聯：  
  
    1.  在 **網際網路資訊服務 (IIS) 管理員**, ，展開本機伺服器] 節點，然後按一下 [ **Default Web Site** （或您的同步處理網站是否不同於預設的網站）。  
...
2.  在訂閱者端的 Internet Explorer 中，將 `?diag` 附加至 replisapi.dll 的位址，以便在診斷模式下連接伺服器。 例如︰ **https://server.domain.com/directory/replisapi.dll?diag**。  
    ...
    > [!NOTE]  
    >  在上述範例 **server.domain.com** 應該以完全取代 **發給** 底下列出 **伺服器憑證** 區段在 IIS 管理員中。  
