...
> [AZURE.IMPORTANT]不建議且不支援將系統資料庫儲存在 Azure Blob 儲存體中。 
 ...
 如需如何使用此新功能的實際操作體驗，請參閱[教學課程：搭配使用 Microsoft Azure Blob 儲存體服務和 SQL Server 2016 資料庫](https://msdn.microsoft.com/library/dn466438.aspx)。  
  
## 為何要在 Microsoft Azure 中使用 SQL Server 資料檔案？ 
...
1.  *建立資料庫時發生錯誤*   
    解決方式：請檢閱[教學課程：搭配使用 Microsoft Azure Blob 儲存體服務和 SQL Server 2016 資料庫](https://msdn.microsoft.com/library/dn466431.aspx)中第 4 課所提供的指示。  
  
2.  *執行 Alter 陳述式時發生錯誤*   
    解決方式：請務必在資料庫上線時執行 Alter Database 陳述式。 將資料檔案複製到 Azure 儲存體時，一定要建立分頁 Blob 而非區塊 Blob。 否則，ALTER Database 將會失敗。 請檢閱[教學課程：搭配使用 Microsoft Azure Blob 儲存體服務和 SQL Server 2016 資料庫](https://msdn.microsoft.com/library/dn466438.aspx)中第 7 課所提供的指示。  
