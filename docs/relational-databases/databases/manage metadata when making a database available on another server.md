---
title: "在另一個伺服器執行個體上提供可用的資料庫時，管理中繼資料 (SQL Server) | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
helpviewer_keywords: 
  - "跨資料庫查詢 [SQL Server]"
  - "登入 [SQL Server], 在另一個伺服器執行個體上重新建立"
  - "觸發程序 [SQL Server], DLL"
  - "使用者自訂錯誤訊息 [SQL Server]"
  - "權限 [SQL Server], 中繼資料存取"
  - "全文檢索引擎 [SQL Server]"
  - "中繼資料 [SQL Server], 其他執行個體可用的資料庫"
  - "工作 [SQL Server Agent], 在另一個伺服器執行個體上重新建立"
  - "容錯移轉 [SQL Server], 管理中繼資料"
  - "事件通知 [SQL Server], 中繼資料"
  - "資料庫鏡像 [SQL Server], 中繼資料"
  - "啟動選項 [SQL Server]"
  - "還原 [SQL Server], 至另一個伺服器執行個體"
  - "連結的伺服器 [SQL Server], 中繼資料"
  - "伺服器事件的 WMI 提供者, 中繼資料"
  - "附加資料庫 [SQL Server]"
  - "記錄傳送 [SQL Server], 中繼資料"
  - "加密 [SQL Server], 中繼資料"
  - "伺服器組態 [SQL Server]"
  - "分散式查詢 [SQL Server], 中繼資料"
  - "擴充預存程序 [SQL Server], 中繼資料"
  - "認證 [SQL Server], 中繼資料"
  - "複製資料庫"
...
##  <a name="server_configuration_settings"></a> 伺服器組態設定  
...
##  <a name="user_defined_error_messages"></a> 使用者定義錯誤訊息  
 使用者定義錯誤訊息位於 [sys.messages](../Topic/sys.messages%20\(Transact-SQL\).md) 目錄檢視中。 此目錄檢視會存放在 **master**內。 如果資料庫應用程式仰賴使用者定義錯誤訊息，而且此資料庫可在另一個伺服器執行個體上使用時，請使用 [sp_addmessage](../../relational-databases/system-stored-procedures/sp-addmessage-transact-sql.md)，在目的地伺服器執行個體上加入這些使用者定義訊息。  
...
##  <a name="extended_stored_procedures"></a> 擴充預存程序  
  
> **重要！** [!INCLUDE[ssNoteDepFutureAvoid](../../includes/ssnotedepfutureavoid-md.md)] 請改用 [CLR 整合](../Topic/Common%20Language%20Runtime%20\(CLR\)%20Integration%20Overview.md)。  
...
##  <a name="ifts_service_properties"></a> Full-Text Engine for SQL Server 屬性  
...
##  <a name="startup_procedures"></a> 啟動程序  
