...
description: 更加了解資料庫識別碼。 了解其定序、各種類別、分隔需求與命名規則。
ms.custom: ''
ms.date: 03/16/2017
ms.prod: sql
...
-  一般識別碼    
   符合識別碼格式的規則。 在 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式中使用一般識別碼時不以符號分隔。
...
5.  不允許[補充字元](../../relational-databases/collations/collation-and-unicode-support.md#Supplementary_Characters)。
