---
title: "檢視儲存的 XML 結構描述集合 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "結構描述集合 [SQL Server], 檢視"
  - "XML 結構描述 [SQL Server], 檢視"
  - "CREATE XML SCHEMA COLLECTION 陳述式"
  - "xml_schema_namespace 函數"
  - "XML 結構描述集合 [SQL Server], 檢視"
  - "顯示 XML 結構描述集合"
  - "檢視 XML 結構描述集合"
...
---
# 檢視儲存的 XML 結構描述集合
  使用 [CREATE XML SCHEMA COLLECTION](../../t-sql/statements/create-xml-schema-collection-transact-sql.md) 匯入 XML 結構描述集合之後，即會將結構描述元件儲存在中繼資料中。 您可以使用 [xml_schema_namespace](../Topic/xml_schema_namespace%20\(Transact-SQL\).md) 內建函數以重建 XML 結構描述集合。 這個函數會傳回 **xml** 資料類型執行個體。  
  
 例如，下列查詢會從 [!INCLUDE[ssSampleDBobject](../../includes/sssampledbobject-md.md)] 資料庫的生產關聯式結構描述擷取 XML 結構描述集合 (`ProductDescriptionSchemaCollection`)。  
...
```  
 ...
 如果您只想查看 XML 結構描述集合的一個結構描述，則可以針對 `xml_schema_namespace` 傳回的 **xml** 類型結果指定 XQuery。  
