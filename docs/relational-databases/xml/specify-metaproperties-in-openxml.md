...
helpviewer_keywords: 
  ...
  - "中繼屬性 [SQL Server 中的 XML]"
  - "未耗用的資料"
  - "擷取 XML 節點資訊 [SQL Server]"
  - "OPENXML 陳述式, 中繼屬性"
...
### C. 指定 xmltext 中繼屬性來擷取資料行中未耗用的資料  
 此範例使用 OPENXML 來建立範例 XML 文件的資料列集檢視。 範例中將說明如何將 **xmltext** 中繼屬性的屬性對應到 OPENXML 中的資料列集資料行，以擷取未消耗的 XML 資料。  
  
 **comment** 資料行是藉由對應到 **@mp:xmltext** 中繼屬性，而識別為溢位資料行。 *flags* 參數是設為 **9** (XML_ATTRIBUTE 和 XML_NOCOPY)。 這表示**屬性中心**對應，並表示只有未耗用的資料才要複製到溢位資料行。  
