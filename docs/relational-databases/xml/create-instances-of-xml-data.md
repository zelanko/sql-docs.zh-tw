...
### <a name="type-casting-clr-user-defined-types"></a>類型轉換 CLR 使用者定義型別  
 如果 CLR 使用者定義型別具有 XML 序列化，即可將該類型的執行個體明確轉換成 XML 資料類型。 如需有關 CLR 使用者定義型別之 XML 序列化的詳細資料，請參閱 [從 CLR 資料庫物件進行 XML 序列化](/dotnet/standard/serialization/introducing-xml-serialization)。  
