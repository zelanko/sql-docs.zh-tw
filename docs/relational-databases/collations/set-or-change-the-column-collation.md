...
-   [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]。 如需詳細資訊，請參閱[定序與 Unicode 支援](../../relational-databases/collations/collation-and-unicode-support.md)。  
  
-   使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 管理物件 (SMO) 中的 **Column.Collation** 屬性。  
...
-   FOREIGN KEY 條件約束  
 ...
 當您使用 **tempdb** 時，[COLLATE](../Topic/COLLATE%20\(Transact-SQL\).md) 子句會包含 *database_default* 選項，將暫存資料表中的資料行指定為使用連線的目前使用者資料庫預設定序，而非 **tempdb** 的定序。  
