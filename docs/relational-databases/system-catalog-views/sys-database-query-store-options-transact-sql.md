...
|**max_storage_size_mb**|**Bigint**|查詢存放區的最大磁片大小（以 mb 為單位）。 預設值為**100** MB。<br />針對[!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] Premium 版本，預設值為 1 GB， [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)]而 Basic 版本的預設值為 10 MB。<br /><br /> 使用`ALTER DATABASE <database> SET QUERY_STORE (MAX_STORAGE_SIZE_MB = <size>)`語句來變更。|  
|**stale_query_threshold_days**|**Bigint**|不含原則設定的查詢會保留在查詢存放區中的天數。 預設值為 **30**。 設定為0以停用保留原則。<br />
  [!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)] Basic 版的預設值為 7 天。<br /><br /> 使用`ALTER DATABASE <database> SET QUERY_STORE ( CLEANUP_POLICY = ( STALE_QUERY_THRESHOLD_DAYS = <value> ) )`語句來變更。|  
...
|**query_capture_mode_desc**|**Nvarchar （60）**|查詢存放區實際捕捉模式的文字描述：<br /><br /> 全部（預設為[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]）<br /><br /> **AUTO** （的[!INCLUDE[sqldbesa](../../includes/sqldbesa-md.md)]預設值）<br /><br /> 無|  
