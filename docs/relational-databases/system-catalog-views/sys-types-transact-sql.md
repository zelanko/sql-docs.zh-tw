---
title: "sys.types (TRANSACT-SQL) |Microsoft 文件"
ms.custom: 
ms.date: 03/14/2017
...
|**max_length**|**smallint**|類型的最大長度 (以位元組為單位)。<br /><br /> -1 = 資料行資料類型是**varchar （max)**， **nvarchar （max)**， **varbinary （max)**，或**xml**。<br /><br /> 如**文字**資料行， **max_length**值將會是 16。|  
|**有效位數**|**tinyint**|如果是以數值為基礎，便是類型的最大有效位數；否則，便是 0。|  
|**小數位數**|**tinyint**|如果是以數值為基礎，便是類型的最大小數位數；否則，便是 0。|  
|**sys.databases**|**sysname**|如果是以字元為基礎，便是類型的定序名稱；否則，便是 NULL。|  
