...
|**參數**|**xml**|說明與收集項相關聯之收集器型別的參數化。 這個收集項的 XML 架構會使用儲存在**parameter_schema**中的 xml 架構（XSD）來進行驗證，以取得特定的收集器型別。 可為 Null。 如需詳細資訊，請參閱[syscollector_collector_types &#40;transact-sql&#41;](../../relational-databases/system-catalog-views/syscollector-collector-types-transact-sql.md)。|  
