...
ms.locfileid: "73982120"
---
# <a name="sysobjects-transact-sql"></a>sys.objects (Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-all-md](../../includes/tsql-appliesto-ss2008-all-md.md)]

  針對在資料庫內建立的每個使用者定義、架構範圍的物件（包括原生編譯的純量使用者定義函數），各包含一個資料列。  
  
 如需詳細資訊，請參閱[記憶體內部 OLTP 的純量使用者定義函式](../../relational-databases/in-memory-oltp/scalar-user-defined-functions-for-in-memory-oltp.md)。  
  
> [!NOTE]  
>  sys.objects 不顯示 DDL 觸發程序，因為它們不是以結構描述為範圍。 所有的觸發程式（DML 和 DDL）都會在[sys.databases](../../relational-databases/system-catalog-views/sys-triggers-transact-sql.md)中找到。 sys.triggers 支援各種觸發程序種類的混合名稱範圍規則。  
...
## <a name="see-also"></a>另請參閱  
 ...
 [&#40;Transact-sql&#41;的觸發程式](../../relational-databases/system-catalog-views/sys-triggers-transact-sql.md)   
