...
|**failure_condition_level**|**int**|使用者定義的失敗狀況層級，在此情況下，必須觸發自動容錯移轉，這是此資料表正下方之表格中所顯示的其中一個整數值。<br /><br /> 失敗狀況層級 (1-5) 的範圍從最低限制 (層級 1) 到最高限制 (層級 5)。 給定的狀況層級包含所有較少限制的層級。 因此，最嚴格的狀況層級 5 包含四個較少限制的狀況層級 (1-4)，層級 4 則包含層級 1-3，依此類推。<br /><br /> 若要變更此值，請使用[ALTER AVAILABILITY GROUP](../../t-sql/statements/alter-availability-group-transact-sql.md)語句的 FAILURE_CONDITION_LEVEL 選項 [!INCLUDE[tsql](../../includes/tsql-md.md)] 。|  
|**health_check_timeout**|**int**|在假設伺服器實例變慢或沒有回應之前， [sp_server_diagnostics](../../relational-databases/system-stored-procedures/sp-server-diagnostics-transact-sql.md)系統預存程式傳回伺服器健全狀況資訊的等候時間 (（以毫秒為單位）) 。 預設值為 30000 毫秒 (30 秒)。<br /><br /> 若要變更此值，請使用[ALTER AVAILABILITY GROUP](../../t-sql/statements/alter-availability-group-transact-sql.md)語句的 HEALTH_CHECK_TIMEOUT 選項 [!INCLUDE[tsql](../../includes/tsql-md.md)] 。|  
...
|**cluster_type**|**tinyint**|0： Windows Server 容錯移轉叢集 <br/><br/>1：外部叢集 (例如，Linux Pacemaker) <br/><br/>2：無|
|**cluster_type_desc**|**nvarchar(60)**|叢集類型的文字描述|
|**required_synchronized_secondaries_to_commit**|**int**| 必須處於已同步處理狀態，才能完成認可的次要複本數目|
|**sequence_number**|**bigint**|識別可用性群組設定順序。 每次可用性群組主要複本更新群組的設定時，增量增加。|
|**is_contained**|**bit**|1：設定高可用性的 Big data cluster 主要實例。 <br/><br/> 0：所有其他。|
...
|2|指定在發生以下任何情況時應該起始自動容錯移轉：<br /><br /> <br /><br /> -的實例 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 未連接到叢集，而且已超出使用者指定的可用性群組**health_check_timeout**臨界值。<br /><br /> -可用性複本處於失敗狀態。|  
