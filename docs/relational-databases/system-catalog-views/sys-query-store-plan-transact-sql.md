...
|**is_online_index_plan**|**一些**|在線上索引建立期間使用了計畫。 <br/>**注意：** Azure SQL 資料倉儲一律會傳回零（0）。|  
|**is_trivial_plan**|**一些**|計畫是一個簡單的計畫（查詢最佳化工具的第0階段輸出）。 <br/>**注意：** Azure SQL 資料倉儲一律會傳回零（0）。|  
|**is_parallel_plan**|**一些**|方案為平行。 <br/>**注意：** Azure SQL 資料倉儲一律會傳回一（1）。|  
|**is_forced_plan**|**一些**|當使用者執行預存程式**sys. sp_query_store_force_plan**時，計畫會標示為強制。 強制機制*並不保證*會將此計畫用於**query_id**所參考的查詢。 強制執行計畫會再次編譯查詢，而且通常會針對**plan_id**所參考的計畫，產生完全相同或類似的計畫。 如果強制執行計畫不成功， **force_failure_count**會遞增，而**last_force_failure_reason**會填入失敗原因。 <br/>**注意：** Azure SQL 資料倉儲一律會傳回零（0）。|  
|**is_natively_compiled**|**一些**|方案包含原生編譯的記憶體優化程式。 （0 = FALSE，1 = TRUE）。 <br/>**注意：** Azure SQL 資料倉儲一律會傳回零（0）。|  
