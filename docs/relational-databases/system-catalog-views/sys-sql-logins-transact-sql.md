...
|**password_hash**|**varbinary(256)**|SQL 登入密碼的雜湊。 從 [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)] 開始，預存密碼資訊會使用加料式 (Salted) 密碼的 SHA-512 加以計算。|  
 ...
 如需此視圖所繼承之資料行的清單，請參閱 [sys. server_principals &#40;transact-sql&#41;](../../relational-databases/system-catalog-views/sys-server-principals-transact-sql.md)。 資料行 `owning_principal_id` 和 `is_fixed_role` 不會繼承自 sys. server_principals。
