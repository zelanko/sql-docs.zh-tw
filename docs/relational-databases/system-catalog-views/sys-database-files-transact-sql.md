...
|**file_guid**|**uniqueidentifier**|檔案的 GUID。<br /><br /> Null = 資料庫已從舊版 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] （適用于 SQL Server 2005 和更早版本）升級。|  
...
|**physical_name**|**nvarchar(260)**|作業系統檔案名稱。 如果資料庫是由 AlwaysOn[可讀取的次要複本](../../database-engine/availability-groups/windows/active-secondaries-readable-secondary-replicas-always-on-availability-groups.md)所主控，則**physical_name**會指出主要複本資料庫的檔案位置。 如需可讀取次要資料庫的正確檔案位置，請查詢[sysaltfiles](../../relational-databases/system-compatibility-views/sys-sysaltfiles-transact-sql.md)。|  
...
|**max_size**|**int**|最大檔案大小 (以 8 KB 頁面為單位)：<br /><br /> 0 = 不允許任何成長。<br /><br /> -1 = 檔案會成長到磁碟已滿。<br /><br /> 268435456 = 記錄檔可以成長到最大 2 TB 的大小。<br /><br /> 針對 FILESTREAM 檔案群組容器，max_size 會反映容器的大小上限。<br /><br /> 請注意，使用無限制記錄檔大小進行升級的資料庫，會報告-1，表示記錄檔的大小上限。|  
