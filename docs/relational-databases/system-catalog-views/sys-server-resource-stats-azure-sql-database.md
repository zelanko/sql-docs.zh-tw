...
ms.locfileid: "87864489"
---
# <a name="sysserver_resource_stats-azure-sql-database"></a>server_resource_stats (Azure SQL Database) 
[!INCLUDE[Azure SQL Database Azure SQL Managed Instance](../../includes/applies-to-version/asdb-asdbmi.md)]

傳回 Azure SQL 受控執行個體的 CPU 使用量、IO 和儲存體資料。 於五分鐘間隔內收集及彙總資料。 每隔15秒報告就會有一個資料列。 傳回的資料包括 CPU 使用量、儲存體大小、IO 使用率和 SKU。 歷程記錄資料大約會保留 14 天。

根據與資料庫相關聯的 Azure SQL 受控執行個體版本而定，[ **server_resource_stats** ] 視圖會有不同的定義。 請考慮這些差異，以及升級到新伺服器版本時應用程式所需的任何修改。
...
## <a name="remarks"></a>備註  
 **Server_resource_stats sys.databases**所傳回的資料會表示為在 avg_cpu 以外的資料行) 名稱中所述的位元組或 mb (中所使用的總計，這會以您所執行之服務層級/效能等級的最大允許限制百分比表示。  
