...
|file_guid|**uniqueidentifier**|檔案的唯一識別碼。<br /><br /> Null = 資料庫從舊版升級[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] （適用于 SQL Server 2005 和更早版本）。|  
...
|size|**int**|目前檔案大小 (以 8 KB 頁面為單位)。 如果是資料庫快照集，size 會反映快照集可以使用的最大檔案空間。<br /><br /> 注意： FILESTREAM 容器的此欄位會填入零。 查詢*sys.databases 的 database_files*目錄檢視，以取得實際的 FILESTREAM 容器大小。|  
...
|create_lsn|**數值（25，0）**|建立檔案的記錄序號 (LSN)。|  
|drop_lsn|**數值（25，0）**|卸除檔案的 LSN。|  
|read_only_lsn|**數值（25，0）**|從讀寫改成唯讀 (最近的變更) 的檔案所在之檔案群組的 LSN。|  
|read_write_lsn|**數值（25，0）**|從唯讀改成讀寫 (最近的變更) 的檔案所在之檔案群組的 LSN。|  
|differential_base_lsn|**數值（25，0）**|差異備份的基底。 在這個 LSN 之後變更的資料範圍會併入差異備份中。|  
|differential_base_guid|**uniqueidentifier**|差異備份基礎所在之基底備份的唯一識別碼。|  
|differential_base_time|**datetime**|對應於 differential_base_lsn 的時間。|  
|redo_start_lsn|**數值（25，0）**|必須啟動下一次向前復原的 LSN。<br /><br /> 除非 state = RESTORING 或 state = RECOVERY_PENDING，否則，便是 NULL。|  
|redo_start_fork_guid|**uniqueidentifier**|復原分岔的唯一識別碼。 下一個還原的記錄備份之 first_fork_guid 必須符合這個值。 這代表容器目前的狀態。|  
|redo_target_lsn|**數值（25，0）**|能夠停止這個檔案的線上向前復原的 LSN。<br /><br /> 除非 state = RESTORING 或 state = RECOVERY_PENDING，否則，便是 NULL。|  
|redo_target_fork_guid|**uniqueidentifier**|能夠復原容器的復原分岔。 與 redo_target_lsn 形成一組。|  
|backup_lsn|**數值（25，0）**|檔案最近的資料或差異備份的 LSN。|  
|credential_id|**int**|用來儲存檔案的`credential_id` `sys.credentials` 例如，當在[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Azure 虛擬機器上執行，且資料庫檔案儲存在 azure blob 儲存體時，會使用儲存位置的存取認證來設定認證。|  
