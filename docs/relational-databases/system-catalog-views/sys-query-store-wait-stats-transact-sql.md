...
|**wait_category**|**Tinyint**|使用下表來分類等候類型，然後在這些等候類別中匯總等候時間。 不同的等候類別需要不同的後續追蹤分析來解決問題，但來自相同類別的等候類型會導致類似的疑難排解體驗，並提供受影響的查詢，除了等待時間以外，還會遺漏完成的部分大部分的這類調查都成功。|
...
|-----------------|---------------|-----------------|  
|**0**|**未知**|Unknown |  
|**sha-1**|**使用率**|SOS_SCHEDULER_YIELD|
|**2**|**背景工作執行緒**|THREADPOOL|
|**第**|**狀**|LCK_M_%|
|**4gb**|**插銷**|LATCH_%|
|**第**|**緩衝區閂鎖**|PAGELATCH_%|
|**6**|**緩衝區 IO**|PAGEIOLATCH_%|
|**utf-7**|**編譯***|RESOURCE_SEMAPHORE_QUERY_COMPILE|
|**8**|**SQL CLR**|CLR%、SQLCLR%|
|**9**|**鏡像**|DBMIRROR%|
|**十大**|**事務**|交易%、DTC%、TRAN_MARKLATCH_%、MSQL_XACT_%、TRANSACTION_MUTEX|
|**11**|**閒置**|SLEEP_%、LAZYWRITER_SLEEP、SQLTRACE_BUFFER_FLUSH、SQLTRACE_INCREMENTAL_FLUSH_SLEEP、SQLTRACE_WAIT_ENTRIES、FT_IFTS_SCHEDULER_IDLE_WAIT、XE_DISPATCHER_WAIT、REQUEST_FOR_DEADLOCK_SEARCH、LOGMGR_QUEUE、ONDEMAND_TASK_QUEUE、CHECKPOINT_佇列，XE_TIMER_EVENT|
...
|**次**|**網路 IO**|ASYNC_NETWORK_IO、NET_WAITFOR_PACKET、PROXY_NETWORK_IO、EXTERNAL_SCRIPT_NETWORK_IOF|
|**1600**|**平行處理原則**|CXPACKET，EXCHANGE，HT%，BMP%，BP%|
|**17**|**快閃記憶體**|RESOURCE_SEMAPHORE、CMEMTHREAD、CMEMPARTITIONED、EE_PMOLOCK、MEMORY_ALLOCATION_EXT、RESERVED_MEMORY_ALLOCATION_EXT、MEMORY_GRANT_UPDATE|
...
|**上午**|**記錄檔速率管理員**|LOG_RATE_GOVERNOR、POOL_LOG_RATE_GOVERNOR、HADR_THROTTLE_LOG_RATE_GOVERNOR、INSTANCE_LOG_RATE_GOVERNOR|
