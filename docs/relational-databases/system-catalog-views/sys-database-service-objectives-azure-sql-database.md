...
|elastic_pool_name|sysname|資料庫所屬的[彈性集](https://azure.microsoft.com/documentation/articles/sql-database-elastic-pool/)區名稱。 如果資料庫是單一資料庫或資料倉儲，則會傳回**Null** 。|  
  
## <a name="permissions"></a>權限  
 需要 master 資料庫的**dbManager**許可權。  在資料庫層級，使用者必須是「建立者」或「擁有者」。  
  
## <a name="examples"></a>範例  
 這個範例可以在 master 資料庫或 Azure SQL Database 使用者資料庫上執行。 此查詢會傳回資料庫 (s) 的名稱、服務和效能層級資訊。  
