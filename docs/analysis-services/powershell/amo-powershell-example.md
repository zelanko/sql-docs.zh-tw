---
title: "AMO PowerShell 範例 | Microsoft Docs"
ms.custom: ""
ms.date: "03/01/2017"
...
---
# AMO PowerShell 範例
  下列 PowerShell 指令碼範例會呼叫 Analysis Services 管理物件 (AMO) 類型，以根據 SQL Server 關聯式資料庫來建立表格式資料庫 (相容性層級 1200)。  
  
 此範例僅供示範使用。 它不會定義資料表關聯性，也不會涉及資料匯入。  因此，此指令碼比較像是程式碼片段，而不是可執行的程式碼。 若要讓它可以執行，則需要其他 Helper 函數和更多命令來彌補差距。  
  
## DatabaseGenerate.ps1 範例  
...
## 請參閱＜  
 ...
 [Analysis Services PowerShell 參考](../../analysis-services/powershell/analysis-services-powershell-reference.md)   
 [Analysis Services 管理物件 &#40;AMO&#41;](../Topic/Analysis%20Services%20Management%20Objects%20\(AMO\).md)   
