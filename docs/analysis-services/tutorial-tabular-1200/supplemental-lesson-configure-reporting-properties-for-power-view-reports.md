...
在此補充課程中，您將設定報告 AW 網際網路銷售專案的屬性。 報表屬性，讓使用者更輕鬆地選取，然後在 Power View 中顯示模型資料。 您也會設定屬性來隱藏某些資料行和資料表，並建立新的資料供圖表使用。   
...
當製作表格式模型時，有您可以設定個別資料行和資料表，以增強使用者報表體驗 Power View 中的特定屬性。 此外，您也可以建立其他模型資料來支援資料視覺效果以及報表用戶端的其他特有功能。 在範例 Adventure Works Internet Sales Model 中，以下是您將進行的部分變更：  
...
-   **視需要設定群組** - [保留唯一資料列]  屬性會決定資料行中的值是否應該依據另一個欄位 (如識別碼欄位) 中的值來分組。 包含重複的值，例如客戶名稱 （例如，有多位客戶命名為 John Smith） 的資料行，務必上分組 （保留唯一資料列）**資料列識別碼**以提供您的終端使用者使用的欄位正確的結果。  
  
-   **設定資料類型和資料格式** - 根據預設，Power View 會根據資料行資料類型套用規則，以判斷此欄位是否可以當做量值使用。 因為 Power View 中的每一個資料視覺效果也有地方可以放置量值和非量值相關的規則，務必在模型中，設定的資料類型，或覆寫預設值，以達到您想要為您的使用者的行為。  
...
如需預設欄位集的詳細資訊，請參閱[設定預設欄位集 Power View 報表的](../tabular-models/power-view-configure-default-field-set-for-reports.md)。  
...
3.  在 [預設欄位集]  對話方塊的 [資料表中的欄位]  清單方塊內按 Ctrl 鍵，並選取以下欄位，然後按一下 [新增]  ：  
    ...
    **生日**， **Customer Alternate ID**，**名字**，**姓氏**。  
...
如需資料表行為屬性的詳細資訊，請參閱[Power View 報表的設定資料表行為屬性](../tabular-models/power-view-configure-table-behavior-properties-for-reports.md)。  
...
3.  在 **資料表行為**對話方塊中，於**資料列識別碼**下拉式清單方塊中，選取**客戶識別碼**資料行。  
...
6.  重複這些步驟**地理位置**資料表中，選取**地理位置 ID**資料行做為資料列識別碼，而**縣 （市)** 中的資料行**保留唯一資料列**清單方塊。 您不需要為這個資料表設定預設標籤。  
  
7.  針對重複這些步驟中，**產品**資料表中，選取**產品識別碼**資料行當做資料列識別碼，而**產品名稱**中的資料行**保持唯一資料列**清單方塊。 針對**預設標籤**，選取**Product Alternate ID**。  
...
您只會在這裡設定幾個不同的資料行屬性，但是還有許多其他屬性。 如需詳細資料行報表屬性的相關資訊，請參閱[資料行屬性](../tabular-models/column-properties-ssas-tabular.md)。  
  
#### <a name="to-set-properties-for-columns"></a>若要設定資料行的屬性  
  
1.  在模型設計師中，按一下 [Customer]  資料表 (索引標籤)。  
  
2.  按一下  **Customer ID**資料行中顯示屬性的資料行**屬性**視窗。  
  
3.  在 [屬性]  視窗中，將 [Hidden]  屬性設定為 True。 **客戶識別碼**資料行則會變成灰色在模型設計師中。  
...
您現在可以使用 Excel，以視覺化方式檢視模型中的資料。 請確定 SharePoint 網站上的 Analysis Services 和 Reporting Services 帳戶具有模型部署所在之 Analysis Services 執行個體的讀取權限。  
