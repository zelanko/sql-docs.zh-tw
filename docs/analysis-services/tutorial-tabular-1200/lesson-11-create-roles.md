...
在這一課，您將建立角色。 角色會藉由僅限身為角色成員的 Windows 使用者存取的方式，提供模型資料庫物件和資料安全性。 每個角色定義單一權限：無、 讀取、 讀取和處理程序、 處理序或系統管理員。 可以使用角色管理員的模型製作期間定義角色。 部署模型之後，您可以使用 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]管理角色。 若要進一步了解，請參閱[角色](../tabular-models/roles-ssas-tabular.md)。  
...
本主題是表格式模型教學課程的一部分，必須依序完成。 執行工作之前在這一課，您應已完成上一課：[第 10 課：建立分割區](lesson-10-create-partitions.md)。  
...
請移至下一課：[第 12 課：在 Excel 中分析](lesson-12-analyze-in-excel.md)。
