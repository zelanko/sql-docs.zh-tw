...
ms.topic: "article"
helpviewer_keywords: 
  - "翻譯 [Analysis Services], 用戶端應用程式"
  - "日期比較"
  - "星期幾比較 [Analysis Services]"
  - "時間 [Analysis Services]"
  - "月份比較 [Analysis Services]"
...
-   開啟 Excel | [資料] | [現有連接]。 將清單篩選到只剩下這部電腦上的連接檔案。 尋找 Adventure Works 的連接 (請仔細查看名稱；您可能會有一個以上的連接)。 開啟連接。  
     ...
     ![使用法文翻譯的 Excel 樞紐分析表](../analysis-services/media/ssas-localetest-excel.png "使用法文翻譯的 Excel 樞紐分析表")  
...
-   在 [物件總管] 中，選取 [連接] | [Analysis Services] | [選項]，然後按一下 [其他連接參數] 索引標籤。  
  
-   輸入 `Local Identifier=1036` ，然後按一下 [連接] 。  
  
-   對 Adventure Works 資料庫執行 MDX 查詢。 查詢結果應該是法文翻譯。  
  
     ![SSMS 中使用法文翻譯的 MDX 查詢](../analysis-services/media/ssas-localetest-ssms.png "SSMS 中使用法文翻譯的 MDX 查詢")  
