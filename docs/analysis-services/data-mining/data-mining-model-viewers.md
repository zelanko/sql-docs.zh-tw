...
helpviewer_keywords: 
  - "顯示資料採礦模型"
  - "採礦模型 [Analysis Services], 檢視"
  - "資料採礦 [Analysis Services], 模型"
  - "檢視資料採礦模型"
  - "採礦模型內容"
  - "支援 [資料採礦]"
  - "探索資料採礦模型 [Analysis Services]"
...
## Microsoft 一般內容樹狀檢視器詳細資料  
 您也可以使用 [Microsoft 一般內容樹狀檢視器 &#40;資料採礦&#41;](../Topic/Microsoft%20Generic%20Content%20Tree%20Viewer%20\(Data%20Mining\).md) 來檢視每個模型。 此檢視器會根據標準 HTML 表格格式來呈現採礦模型的內容。 但是，節點的排列和每一個節點的內容將會因為用來產生結果的演算法而有很大的差異。  
