 ...
 **MODEL_EXISTENCE_ONLY**  
 指出資料行，將會視為擁有兩個狀態：**遺漏**並**現有**。 若值為 **NULL**，則會視為 Missing。 MODEL_EXISTENCE_ONLY 旗標會套用到可預測的屬性，而且受到大多數的演算法所支援。  
  
 作用中，將 MODEL_EXISTENCE_ONLY 旗標設定為 **，則為 True**變更值的表示，因此有兩個狀態：**遺漏**並**現有**。 所有非遺漏狀態都會結合到單一 **Existing** 值中。  
