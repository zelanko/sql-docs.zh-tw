...
4.  檢閱索引鍵和 Null 索引鍵錯誤組態屬性，但不做任何變更。  
    ...
    ![ErrorConfiguration 屬性集合](../media/l4-productdimensionerrorconfig-1.gif "ErrorConfiguration 屬性集合")  
...
6.  依序展開 [元件]  和 [模型名稱]  層級的未標記成員。  
    ...
    ![用來建立其他元件的組件元件](../media/l4-productdimensionerrorconfig-2.gif "用來建立其他元件的組件元件")  
...
4.  按一下 SELECT DISTINCT 查詢，然後按一下 [檢視詳細資料]  。  
    ...
    ![顯示 WHERE 子句的 SELECT DISTINCT 子句](../media/l4-productnametraceline-1.gif "顯示 WHERE 子句的 SELECT DISTINCT 子句")  
...
4.  展開 [類別目錄]  層級的 [組件元件]  成員，然後展開 [子類別目錄]  層級的 [組件元件]  成員。  
    ...
    ![顯示組件元件的產品名稱層級](../media/l4-assemblycomponents-1.gif "顯示組件元件的產品名稱層級")  
