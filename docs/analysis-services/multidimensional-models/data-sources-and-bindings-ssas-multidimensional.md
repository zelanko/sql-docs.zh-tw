...
ms.topic: "article"
helpviewer_keywords: 
  - "資料來源檢視 [Analysis Services], 繫結"
  - "DSO, 繫結"
  - "Analysis Services 指令碼語言, 資料來源"
  - "cube [Analysis Services], 繫結"
  - "OLAP 採礦模型 [Analysis Services 指令碼語言]"
  - "繫結 [Analysis Services 指令碼語言]"
  - "重新繫結 [Analysis Services 指令碼語言]"
  - "ASSL, 繫結"
  - "關聯式採礦模型 [ASSL]"
  - "資料來源 [Analysis Services 指令碼語言]"
  - "ASSL, 資料來源"
  - "維度 [Analysis Services], 繫結"
  - "量值 [Analysis Services], 繫結"
  - "關聯式資料來源 [Analysis Services Scripting Language]"
  - "Analysis Services 指令碼語言, 繫結"
  - "分段資料列集"
  - "資料粒度"
  - "採礦模型 [Analysis Services], 資料來源"
  - "正規繫結 [ASSL]"
  - "非正規 (Out-of-Line) 繫結"
  - "量值群組 [Analysis Services], 繫結"
  - "資料分割 [Analysis Services], 繫結"
...
### 非正規繫結類型的定義  
 ...
 指定繫結的每一個物件都會以 \<*物件*>ID (例如 **DimensionID**) 格式的 XML 元素來識別。 在您已經盡量明確地識別 \<*物件*>ID 格式的物件之後，您會識別已指定繫結的元素，通常是 **Source**。 值得注意的常見情況是哪一個 **Source** 是 **DataItem** 上的屬性，這是屬性中資料行繫結的情況。 在此情況下，您不會指定 **DataItem** 標記，而只會指定 **Source** 屬性，就像它直接位於要繫結的資料行上一樣。  
