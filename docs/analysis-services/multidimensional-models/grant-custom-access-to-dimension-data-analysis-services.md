...
# 授與維度資料的自訂存取權 (Analysis Services)
 ...
 ![具有及不具有維度成員的樞紐分析表](../../analysis-services/multidimensional-models/media/ssas-permsdimdenied.png "具有及不具有維度成員的樞紐分析表")  
...
> [!NOTE]  
>  下列指示假設用戶端連接使用 MDX 發出查詢。 如果用戶端使用 DAX (例如 Power BI 中的 Power View)，則查詢結果中不會顯示維度安全性。 如需詳細資訊，請參閱[了解適用於多維度模型的 Power View](https://msdn.microsoft.com/library/jj969574.aspx)。
...
1.  在 SQL Server Management Studio 中，以滑鼠右鍵按一下 Cube，然後選取 [編寫 Cube 的指令碼為] | [ALTER 至] | [新增查詢編輯器視窗]。  
...
1.  在 [維度資料] | [基本] 上，向下捲動 [維度] 清單，直到您到達 Cube 維度為止，然後選取 [量值維度]。  
...
## 進階維度安全性  
 ...
 您可以使用 MDX 產生器來撰寫 MDX 陳述式。 如需詳細資訊，請參閱 [MDX 產生器 &#40;Analysis Services - 多維度資料&#41;](../Topic/MDX%20Builder%20\(Analysis%20Services%20-%20Multidimensional%20Data\).md)。 [進階] 索引標籤具有下列選項：  
...
-   如果資料庫角色沒有定義屬性的預設成員，[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 就會使用該屬性本身定義的預設成員。 除非您另外指定，否則屬性的預設成員就是 **All** 成員 (除非該屬性定義為非彙總)。  
 ...
 例如，假設資料庫角色指定 **Male** 作為 **Gender** 屬性的預設成員。 除非查詢明確包含 **Gender** 屬性並為此屬性指定不同成員，否則 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 就會傳回只包含男性客戶的資料集。 如需設定預設成員的詳細資訊，請參閱[定義預設成員](../../analysis-services/multidimensional-models/define-a-default-member.md)。  
