...
## <a name="prerequisites"></a>先決條件  
 ...
 在來源和目的地伺服器必須是相同的版本和 service pack。 因為模型中繼資料也會同步處理，以確保相容性建置這兩部伺服器的數目應該相同。 每個安裝的版本都必須支援資料庫同步處理。 在 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]中，Enterprise、Developer 和 Business Intelligence 版都支援資料庫同步處理。 如需每版功能的詳細資訊，請參閱[版本及支援的功能，適用於 SQL Server 2016](../../sql-server/editions-and-supported-features-for-sql-server-2016.md)。  
