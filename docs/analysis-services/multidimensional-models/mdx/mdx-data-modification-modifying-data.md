...
[!INCLUDE[ssas-appliesto-sqlas](../../../includes/ssas-appliesto-sqlas.md)]除了使用多維度運算式 (MDX) 擷取及處理維度和 cube 的資料，您可以使用 MDX 來更新或*回寫*維度與 cube 資料。 這些更新可能是暫時性 (如同推測性或 "what if" 分析 ) 或永久性 (例如，必須根據資料分析來變更時)。  
