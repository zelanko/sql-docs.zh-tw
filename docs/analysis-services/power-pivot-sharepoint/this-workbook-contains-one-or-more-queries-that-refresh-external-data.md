...
|||  
|-|-|  
|產品名稱|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] for SharePoint|  
...
## 說明  
 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 活頁簿包含內嵌的資料連接字串，Excel 會使用這些連接字串與載入和計算資料的外部 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 伺服器進行通訊。 如果啟用資料重新整理時警告，Excel Services 會偵測此連接字串，並據此警告使用者。  
  
 若要篩選與配量活頁簿中的 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 資料，必須啟用查詢。 請務必僅針對您信任的 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 活頁簿啟用查詢。  
