---
title: "為 Analysis Services 執行個體註冊 SPN | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
---
# 為 Analysis Services 執行個體註冊 SPN
...
##  <a name="bkmk_SPNSyntax"></a> Analysis Services 的 SPN 格式  
 使用 **setspn** 註冊 SPN。 在新版作業系統上， **setspn** 是以系統公用程式的形式安裝。 如需詳細資訊，請參閱＜ [SetSPN](http://technet.microsoft.com/library/cc731241\(WS.10\).aspx)＞。  
...
|服務帳戶|這是 **MSSQLServerOLAPService** Windows 服務的啟動帳戶。 可以是 Windows 網域使用者帳戶、虛擬帳戶、受管理的服務帳戶 (MSA) 或內建帳戶 (如個別服務 SID、NetworkService 或 LocalSystem)。 Windows 網域使用者帳戶，格式可為「網域\使用者」或「使用者@網域」。|  
  
##  <a name="bkmk_virtual"></a> 虛擬帳戶的 SPN 註冊  
 虛擬帳戶是 SQL Server 服務的預設帳戶類型。 預設執行個體的虛擬帳戶為 **NT Service\MSOLAPService**，而具名執行個體的虛擬帳戶則為 **NT Service\MSOLAP$**\<執行個體名稱>。  
 ...
 **以 NT Service\MSOLAP$\<instance-name> 身分執行之具名執行個體的範例語法**  
  
 這個範例會針對在預設虛擬帳戶之下執行的具名執行個體顯示 **setspn** 語法。 在此範例中，電腦主機名稱為 **AW-SRV02**，執行個體名稱則為 **AW-FINANCE**。 同樣地，這是為 SPN 指定的電腦帳戶，而不是虛擬帳戶 **NT Service\MSOLAP$**\<執行個體名稱>。  
...
> [!TIP]  
>  請藉由執行 `Setspn -L <domain account>` 或 `Setspn -L <machinename>`(根據如何註冊 SPN) 來確認是否已針對 Analysis Services 伺服器建立 SPN。 您應該會在清單中看到 MSOLAPSVC.3/\<主機名稱>。  
...
##  <a name="bkmk_spnNamed"></a> 為具名執行個體註冊 SPN  
 Analysis Services 的具名執行個體會使用 SQL Server Browser 服務所偵測到的動態通訊埠指定。 當使用具名執行個體時，請同時為 SQL Server Browser 服務和 Analysis Services 具名執行個體註冊 SPN。 如需詳細資訊，請參閱 [建立連接至 SQL Server Analysis Services 或 SQL Server 的具名執行個體時，需要 SQL Server Browser 服務的 SPN](http://support.microsoft.com/kb/950599)(機器翻譯)。  
...
##  <a name="bkmk_spnCluster"></a> 為 SSAS 叢集註冊 SPN  
 如果是 Analysis Services 容錯移轉叢集，主機名稱必須是指派給叢集的虛擬名稱。 這是當您安裝在現有的 WSFC 上安裝 Analysis Services 時，SQL Server 安裝程式所指定的 SQL Server 網路名稱。 您可以在 Active Directory 中找到此名稱。 您也可以在 [容錯移轉叢集管理員] | [角色] | 資源] 索引標籤中尋找。 [資源] 索引標籤上的伺服器名稱是在 SPN 命令中，要用為「虛擬名稱」的名稱。  
...
## 請參閱＜  
 ...
 [如何將 SQL Server 2008 Analysis Services 與 SQL Server 2005 Analysis Services 設定為使用 Kerberos 驗證](http://support.microsoft.com/kb/917409)   
