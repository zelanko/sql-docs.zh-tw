...
[!INCLUDE[ssas-appliesto-sqlas](../../includes/ssas-appliesto-sqlas.md)]
  Analysis Services 執行個體將會記錄伺服器通知、 錯誤和警告至 msmdsrv.log 檔案-一個用於您所安裝的每個執行個體。 管理員可以參考此記錄檔，獲得例行和異常等事件的深入見解。 最新版本的記錄功能已經過增強，可以加入更多資訊。 記錄檔記錄現在包含產品版本和版本資訊，以及處理器、記憶體、連接及封鎖事件。 您可以在 [記錄改進](http://support.microsoft.com/kb/2965035)檢閱整個變更清單。  
...
##  <a name="bkmk_general"></a> 記錄檔組態設定的一般資訊  
 ...
 ![顯示記錄檔設定的組態檔區段](../../analysis-services/instances/media/ssas-logfilesettings.png "顯示記錄檔設定的組態檔區段")  
...
-   **MaxNumberFiles** 指定保留較舊的記錄檔。 預設值是 0 (停用)。 您可以將它變更為正整數，以保留記錄檔的版本。 達到 **MaxNumberFiles** 時，Analysis Services 會刪除其名稱中具有最舊的時間戳記的檔案。  
...
##  <a name="bkmk_querylog"></a> 查詢記錄  
 ...
 針對查詢監視和疑難排解，許多開發人員和管理員會使用社群工具 **ASTrace**來監視查詢。 您也可以使用 SQL Server Profiler、xEvents 或 Analysis Services 追蹤。
...
4.  如果您想要以不同的速率對查詢取樣，或使用不同的資料表名稱，請選擇性地變更 **QueryLogSampling** 或 **QueryLogTableName** 。  
 ...
 ![查詢記錄檔設定，在 Management Studio](../../analysis-services/instances/media/ssas-querylogsettings.png "在 Management Studio 中的查詢記錄設定")  
 ...
 請參閱 [設定 Analysis Services 查詢記錄](http://technet.microsoft.com/library/Cc917676) ，以深入了解查詢記錄組態。 雖然本文件相當老舊，但最新版本中並未變更查詢記錄組態，因此其內含的資訊仍適用。  
 ...
 **設定當機報告**  
 ...
 除非 Microsoft 支援另有指示，否則大部分的管理員均會使用預設設定。 這個較舊的知識庫文件仍然用來提供有關如何設定傾印檔案的指示：[如何設定 Analysis Services 以產生記憶體傾印檔案](http://support.microsoft.com/kb/919711)。  
...
-   [想要使用小型傾印更深入了解 SQL Server](http://blogs.msdn.com/b/sqlcat/archive/2009/09/11/looking-deeper-into-sql-server-using-minidumps.aspx)  
  
-   [如何建立使用者模式傾印檔案](http://support.microsoft.com/kb/931673)  
  
-   [如何使用 Sqldumper.exe 公用程式，在 SQL Server 產生傾印檔案](http://support.microsoft.com/kb/917825)  
