---
title: "Database Consistency Checker (DBCC) for Analysis Services 表格式和多維度資料庫 | Microsoft Docs"
ms.custom: 
  - "SQL2016_New_Updated"
ms.date: "03/04/2017"
...
manager: "erikre"
---
# Database Consistency Checker (DBCC) for Analysis Services 表格式和多維度資料庫
...
-   相容性層級 1200 之新表格式模型資料庫的中繼資料由 **TableName** 和 **PartitionName**之類的描述元所組成。  
...
## 表格式 1200 資料庫的命令語法  
 1200 相容性層級的表格式資料庫使用表格式中繼資料的物件定義。 下列範例說明在 SQL Server 2016 功能層級建立之表格式資料庫的完整 DBCC 語法。  
  
 這兩種語法之間的主要差異包括較新的 XMLA 命名空間、無 \<物件> 元素，以及無 \<模型> 元素 (每個資料庫仍然只有一個模型)。  
...
### 在 Management Studio 中執行 DBCC 命令  
...
### 在 SQL Server Profiler 2016 中追蹤 DBCC 輸出  
...
4.  在 SQL Server Profiler 中，DBCC 活動係透過擁有 DBCC 事件子類別的 **Command** 事件來表示︰  
     ...
     狀態訊息的開頭為「正在檢查 \<物件> 的一致性」、「已開始檢查 \<物件>」或「已完成檢查 \<物件>」。  
  
    > [!NOTE]  
    >  在 CTP 3.0 中，物件是由內部名稱來加以識別。 例如，類別目錄階層採用 H$Categories-\<物件識別碼> 的表示方式。 在後續的 CTP 中，內部名稱應取代為使用者易記名稱。  
  
     錯誤訊息如下所示。  
  
### SSMS 中 xEvent 工作階段的追蹤 DBCC 輸出  
...
|DBCC 命令|DBCC 命令在透過 DBCC 作業進行時，將會回報多個狀態訊息。  它會在啟動之前回報狀態訊息，包括資料庫、資料表或物件的資料行名稱，然後在每個物件的檢查完成後再回報一次。|正在檢查 \<物件名稱> \<物件類型> 的一致性。 階段︰前置檢查。<br /><br /> 正在檢查 \<物件名稱> \<物件類型> 的一致性。 階段︰後置檢查。|  
...
|錯誤|問題|解決方案|  
|-----------|-----------|----------------|  
|**中繼資料管理員出現錯誤**<br /><br /> 物件參考 '\<物件識別碼>' 無效。 它不符合中繼資料類別階層的結構。|格式不正確的命令|檢查命令語法。 最有可能的情況是您納入了較低層級的物件，卻未指定該物件的一或多個父物件。|  
|**中繼資料管理員出現錯誤**<br /><br /> 識別碼為 '\<物件識別碼>' 的 \<物件> 可能不在識別碼為 '\<父物件識別碼>' 的 \<父物件> 中，或使用者沒有存取該物件的權限。|索引損毀 (多維度)|重新處理物件和任何相依的物件。|  
|**在分割區的一致性檢查期間發生錯誤**<br /><br /> 為 \<database-name> 資料庫的 \<cube-name> Cube 檢查 \<measure-group-name> 量值群組的 \<partition-name> 分割區一致性時發生錯誤。 請重新處理分割區或索引，以修正損毀。|索引損毀 (多維度)|重新處理物件和任何相依的物件。|  
...
|**資料表中繼資料損毀**<br /><br /> 資料表 \<table-name> 中繼資料檔案損毀。 在 DataFileList 節點下找不到主要資料表。|中繼資料損毀 (僅限表格式)|刪除和重新部署專案，或從備份還原後再重新處理。<br /><br /> 如需相關指示，請參閱[如何處理 Analysis Services 資料庫中的損毀 (部落格)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)。|  
|**儲存層發生毀損**<br /><br /> 儲存層發生損毀︰\<parent-name> \<parent-type> 中的 \<type-name> 集合已毀損。|中繼資料損毀 (僅限表格式)|刪除和重新部署專案，或從備份還原後再重新處理。<br /><br /> 如需相關指示，請參閱[如何處理 Analysis Services 資料庫中的損毀 (部落格)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)。|  
|**遺漏系統資料表**<br /><br /> 遺漏系統資料表 \<table-name>。|物件損毀 (僅限表格式)|重新處理物件和任何相依的物件|  
|**資料表統計資料損毀**<br /><br /> 遺漏系統資料表 \<table-name> 的資料表統計資料。|中繼資料損毀 (僅限表格式)|刪除和重新部署專案，或從備份還原後再重新處理。<br /><br /> 如需相關指示，請參閱[如何處理 Analysis Services 資料庫中的損毀 (部落格)](http://blogs.msdn.com/b/karang/archive/2010/08/11/how-to-deal-with-corruption-in-analysis-services.aspx)。|  
  
## 透過 msmdsrv.ini 組態檔停用資料庫載入作業的自動一致性檢查  
