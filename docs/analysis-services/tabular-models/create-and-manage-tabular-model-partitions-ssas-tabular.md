---
title: 建立及管理表格式模型資料分割 |Microsoft Docs
ms.date: 05/07/2018
ms.prod: sql
...
3.  在 **查詢陳述式**中，輸入或貼上資料行和任何您想要包含在資料分割中查詢視窗中的子句會定義 SQL 或 M 查詢陳述式。  
...
> [!IMPORTANT]  
>  合併資料分割並不會更新資料分割中繼資料。 您必須編輯產生的資料分割，藉此確定處理作業會處理合併的資料分割中的所有資料的 SQL 或 M 陳述式。  
