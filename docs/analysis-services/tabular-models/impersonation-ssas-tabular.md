...
##  <a name="bkmk_imp_newmodel"></a> 匯入模型模擬  
 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 不像可以使用數個不同模擬模式支援跨處理序資料收集的表格式模型，前者僅使用一個模式，也就是 ImpersonateCurrentUser。 由於 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 一律執行同處理序，因此它會使用目前登入之使用者的認證，連接到資料來源。 使用表格式模型時，目前登入之使用者的認證僅能搭配 [資料表匯入精靈] 中的 [預覽和篩選] 功能，並在檢視 [資料表屬性] 時使用。 將資料匯入或處理到工作空間資料庫時，或將資料匯入或處理到已部署的模型時，會使用模擬認證。  
  
 匯入現有的 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 活頁簿來建立新模型時，模型設計師預設會設定模擬使用服務帳戶 (ImpersonateServiceAccount)。 建議您變更模型上從 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 匯入到 Windows 使用者帳戶的模擬認證。 匯入 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 活頁簿並在模型設計師中建立新模型後，您可以使用 [現有連接] 對話方塊來變更認證。  
