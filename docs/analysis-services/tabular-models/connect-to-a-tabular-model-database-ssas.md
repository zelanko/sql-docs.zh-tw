...
3.  在 [伺服器名稱] 中，指定裝載資料庫的 Analysis Services 執行個體。 伺服器名稱通常是執行伺服器軟體之電腦的名稱。 如果將伺服器當做具名執行個體安裝，您必須使用下列格式指定名稱：\<伺服器名稱>\\<執行個體名稱\>。  
...
5.  選取資料庫。 有效的選項將會針對資料庫顯示一個 [模型] Cube。 按一下 [下一步]，然後按一下 [完成]。  
 ...
 建立連接之後，您可以使用資料來建立樞紐分析表或樞紐分析圖。 如需詳細資訊，請參閱[在 Excel 中進行分析 &#40;SSAS 表格式&#41;](../../analysis-services/tabular-models/analyze-in-excel-ssas-tabular.md)。  
 ...
 **嘗試執行此作業時發生錯誤。 重新載入活頁簿，然後嘗試再次執行此作業。**  
