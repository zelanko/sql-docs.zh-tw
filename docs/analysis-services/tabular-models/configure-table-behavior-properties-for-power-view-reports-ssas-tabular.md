...
manager: "erikre"
---
# 設定 Power View 報表的資料表行為屬性 (SSAS 表格式)
...
-   從矩陣報表中移除不必要的小計。 欄位層級的預設群組會針對每個欄位建立小計。 如果您只要在資料列層級計算的單一小計，設定 [資料列識別碼] 會產生此結果。  
 ...
 已標記為日期資料表的資料表不能設定資料列識別碼。 日期資料表的資料列識別碼是在標記為日期資料表時指定。 如需詳細資訊，請參閱[標記為日期資料表對話方塊 &#40;SSAS&#41;](../Topic/Mark%20as%20Date%20Table%20Dialog%20Box%20\(SSAS\).md)。  
...
### 遺漏影像  
 ...
 ![影像 URL 在報表中顯示為文字](../../analysis-services/tabular-models/media/ssas-rptprop-noimageurl.gif "影像 URL 在報表中顯示為文字")  
...
### 資料表遺漏一個或多個資料列  
 ...
 ![預設群組會將 2 合併為 1](../../analysis-services/tabular-models/media/ssas-jonyang-norowid.gif "預設群組會將 2 合併為 1")  
 ...
 ![包含依據資料列識別碼之重複項目的資料列資料](../../analysis-services/tabular-models/media/ssas-jonyang.gif "包含依據資料列識別碼之重複項目的資料列資料")  
 ...
 **之前：預設群組行為根據矩陣中的欄位**  
 ...
 ![以資料列識別碼群組為依據的矩陣配置](../../analysis-services/tabular-models/media/ssas-rptprop-matrixrowid.gif "以資料列識別碼群組為依據的矩陣配置")  
 ...
 **之前：預設群組行為根據圖表中的欄位**  
 ...
 ![位於欄位層級中以預設群組為依據的圖表](../../analysis-services/tabular-models/media/ssas-rptprop-chartfieldgroup.gif "位於欄位層級中以預設群組為依據的圖表")  
  
 **之後：群組行為根據資料列識別碼 (資料列識別碼變成軸)**  
  
 ![以資料列識別碼群組為依據的圖表](../../analysis-services/tabular-models/media/ssas-rptprop-chartrowid.gif "以資料列識別碼群組為依據的圖表")  
