...
在這一課，您可以建立角色。 角色會提供模型資料庫物件和資料安全性，方法是限制只有在為角色成員的使用者存取。 每個角色定義單一權限：無、 讀取、 讀取和處理程序、 處理序或系統管理員。 可以使用角色管理員的模型製作期間定義角色。 在部署模型之後，您可以使用 SQL Server Management Studio (SSMS) 來管理角色。 若要進一步了解，請參閱[角色](../tabular-models/roles-ssas-tabular.md)。
...
這篇文章是表格式模型化教學課程中，應該依序完成的一部分。 執行工作之前在這一課，您應已完成上一課：[第 10 課：建立分割區](../tutorial-tabular-1400/as-lesson-10-create-partitions.md)。  
