---
title: 在 Windows 上安裝 SQL Server Java 語言延伸模組
titleSuffix: ''
description: 了解如何透過執行 SQL Server 安裝精靈，在 Windows 的 SQL Server 上安裝 Java 語言延伸模組功能。
author: cawrites
ms.author: chadam
ms.date: 09/17/2020
ms.topic: conceptual
...
ms.locfileid: "92155549"
---
# <a name="install-sql-server-language-java-extensions-on-windows"></a>在 Windows 上安裝 SQL Server 語言 Java 延伸模組
...
> [!NOTE]
> 此文章適用於在 Windows 上安裝 SQL Server 語言延伸模組。 針對 Linux，請參閱[在 Linux 上安裝 SQL Server 2019 語言延伸模組 (Java)](../../linux/sql-server-linux-setup-language-extensions-java.md)

<a name="prerequisites"></a> 

## <a name="pre-install-checklist"></a>預先安裝檢查清單

+ 如果您想要安裝語言延伸模組的支援，則需要 SQL Server 2019 安裝程式。
...
在 SQL Server 2019 候選版 1 中，有兩種方式可以透過 SQL Server 安裝並使用 Java：
...
1. 使用您慣用的 Java 發行版本，而不是預設的 Java Runtime。
    ...
    > [!NOTE]
    > 因為 Java 具有回溯相容性，舊版或許可運作，但候選版 1 版本的支援和測試版本是 Java 11。
...
4. 如果您在上一個步驟中選擇 [Java]  以安裝預設的 Java Runtime，將會顯示 [Java 安裝位置]  頁面。
...
2. 在 [控制台] 中，開啟 [系統及安全性]、開啟 [系統]，然後選取 [進階系統內容]。

3. 選取 [環境變數]。

4. 為 `JRE_HOME` 建立新的系統變數，其值為 JDK/JRE 路徑 (可在步驟 1 中找到)。

5. 重新啟動[啟動控制板](../concepts/extensibility-framework.md#launchpad)。
...
既然一切正常，您可能還需要最佳化伺服器以支援語言延伸模組。

### <a name="optimize-the-server-for-language-extensions"></a>針對語言延伸模組最佳化伺服器
