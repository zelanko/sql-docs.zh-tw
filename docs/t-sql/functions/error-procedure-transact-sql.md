...
在發生錯誤的預存程序 CATCH 區塊中呼叫時，`ERROR_PROCEDURE` 會傳回該預存程序的名稱。  
...
在巢狀 `CATCH` 區塊中，`ERROR_PROCEDURE` 會參考該 `CATCH` 區塊之 `CATCH` 區塊範圍特定的錯誤號碼。 例如，外部 TRY...CATCH 建構的 `CATCH` 區塊可能會有內部 `TRY...CATCH` 建構。 在該內部 `CATCH` 區塊內，`ERROR_PROCEDURE` 會傳回叫用內部 `CATCH` 區塊之錯誤的號碼。 如果 `ERROR_PROCEDURE` 是在外部 `CATCH` 區塊中執行，它會傳回叫用該外部 `CATCH` 區塊之錯誤的號碼。  
