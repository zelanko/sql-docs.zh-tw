 ...
 *expression* 為字元字串，或可以轉換為字元字串的[運算式](../../t-sql/language-elements/expressions-transact-sql.md)。 此運算式的長度必須少於 4,000 個字元。 日期和時間資料類型 (但不包括 datetime 和 smalldatetime) 不允許做為 ISDATE 的引數。
