...
## <a name="syntax"></a>語法  
**float** [ **(***n***)** ] 其中 *n* 是用來儲存科學記號標記法 **float** 數之尾數的位元數目；因此，其規定有效位數和儲存體大小。 若指定 *n*，則其值必須介於 **1** 和 **53** 之間。 *n* 的預設值為 **53**。
...
> [!NOTE]  
>  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 會將 *n* 當作兩個可能值的其中一個進行處理。 如果 **1**<=n<=**24**，便將 *n* 當作 **24** 來處理。 如果 **25**<=n<=**53**，便將 *n* 當作 **53** 來處理。  
