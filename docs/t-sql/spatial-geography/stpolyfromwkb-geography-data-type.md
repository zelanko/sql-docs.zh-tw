---
title: "STPolyFromWKB (geography 資料類型) |Microsoft 文件"
ms.custom: 
ms.date: 07/30/2017
...
傳回**geographyPolygon**從開放式地理空間協會 (OGC) 已知二進位 (well-known binary，WKB) 表示法的執行個體。
 ...
 *WKB_polygon*  
 是的 WKB 表示法**geographyPolygon**您想要傳回的執行個體。 *WKB_polygon*是**varbinary （max)**運算式。  
