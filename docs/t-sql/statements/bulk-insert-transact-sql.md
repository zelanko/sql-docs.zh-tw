 ...
 *schema_name*  
 這是資料表或檢視表結構描述的名稱。 如果執行大量匯入作業之使用者的預設結構描述，是指定之資料表或檢視表的結構描述，則 *schema_name* 為選擇性。 如果未指定 *schema*，且執行大量匯入作業之使用者的預設結構描述與指定的資料表或檢視表不同，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 就會傳回錯誤訊息，且會取消大量匯入作業。  
...
從 [!INCLUDE[ssSQLv14_md](../../includes/sssqlv14-md.md)] CTP 1.1 開始，BULK INSERT 可支援 CSV 格式。  
