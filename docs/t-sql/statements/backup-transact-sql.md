...
>  完整資料庫備份可對**主要**資料庫。  
 ...
 您可以將記錄備份還原至特定時間或備份中的交易藉由指定 WITH STOPAT、 STOPATMARK 或 STOPBEFOREMARK 中的您[RESTORE LOG](../../t-sql/statements/restore-statements-transact-sql.md)陳述式。  
...
\<file_or_filegroup > [ **，**... *n*  ] 只能搭配 BACKUP DATABASE，使用指定的資料庫檔案或檔案備份中包含的檔案群組，或指定唯讀檔案群組来包含在部分備份中。  
 ...
 *n*  
 ...
 READ_WRITE_FILEGROUPS [ **，**檔案群組 = { *logical_filegroup_name*| **@***logical_filegroup_name_var* }[ **,**...*n* ] ]  
...
>  根據預設，BACKUP DATABASE 會建立完整備份。  
 ...
 ENCRYPTION  
 用來指定備份的加密。 您可以指定加密演算法來加密備份，與指定 "NO_ENCRYPION" 不加密備份。 加密是有助於保護備份檔案的建議作法。 您可以指定的演算法清單包括：  
...
-   SERVER ASYMMETRIC KEY = Encryptor_Name  
    ...
    > [!WARNING]  
    >  加密 FILE_SNAPSHOT 引數搭配使用時，中繼資料檔案本身使用指定的加密演算法進行加密，系統會確認已完成資料庫的 TDE。 沒有其他加密就進行資料本身。 如果未加密的資料庫，或如果之前未完成加密發出 backup 陳述式，備份將會失敗。  
...
-   COPY_ONLY 選項搭配 BACKUP LOG 使用時，會建立*只複製記錄備份*，這不會截斷交易記錄檔。 僅複製記錄備份不會影響記錄檔鏈結，而且其他記錄備份的行為會如同僅複製備份並不存在。  
...
> [!NOTE]  
>  如果您使用 NOREWIND，則 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的執行個體會保有磁帶機的擁有權，直到在相同處理序中執行的 BACKUP 或 RESTORE 命令使用 REWIND 或 UNLOAD 選項，或是伺服器執行個體關閉為止。 保留磁帶的開啟狀態可以防止其他處理序存取這個磁帶。 如需如何顯示開啟的磁帶清單及關閉開啟的磁帶資訊，請參閱[備份裝置 &#40;SQL Server &#41;](../../relational-databases/backup-restore/backup-devices-sql-server.md).  
...
>  如果您不想要取得記錄備份，請使用簡單復原模式。 如需詳細資訊，請參閱[復原模式 &#40;SQL Server &#41;](../../relational-databases/backup-restore/recovery-models-sql-server.md)。  
 ...
 NO_TRUNCATE  
 指定不截斷記錄檔，並造成[!INCLUDE[ssDE](../../includes/ssde-md.md)]嘗試進行備份，不論資料庫的狀態。 因此，利用 NO_TRUNCATE 取得的備份可能會有不完整的中繼資料。 在資料庫已損毀的情況下，您可以利用這個選項來進行記錄的備份。  
  
 BACKUP LOG 的 NO_TRUNCATE 選項相當於同時指定 COPY_ONLY 和 CONTINUE_AFTER_ERROR。  
  
 當沒有 NO_TRUNCATE 選項時，資料庫必須處於 ONLINE 狀態。 如果資料庫處於 SUSPENDED 狀態，您可以指定 NO_TRUNCATE 來建立備份。 但是如果資料庫處於 OFFLINE 或 EMERGENCY 狀態，即使設定了 NO_TRUNCATE，也不允許 BACKUP。 資料庫狀態的相關資訊，請參閱[資料庫狀態](../../relational-databases/databases/database-states.md)。  
...
-   所有復原模式都支援完整和差異的資料備份。  
    ...
    |檔案或檔案群組|[檔案備份](../../relational-databases/backup-restore/full-file-backups-sql-server.md)涵蓋一個或多個檔案群組，而且僅與包含多個檔案群組的資料庫有關。 在簡單復原模式下，檔案備份基本上會限制用於唯讀的次要檔案群組。<br /><br /> 或者，每個檔案備份可以做為一系列的一個或多個基底[差異檔案備份](../../relational-databases/backup-restore/differential-backups-sql-server.md)。|  
...
> [!NOTE]  
>  BACKUP LOG WITH NO_LOG 和 WITH TRUNCATE_ONLY 選項已遭到停用。 如果您要使用完整復原模式或大量記錄復原模式，而且您必須從資料庫移除記錄備份鏈結，請切換到簡單復原模式。 如需詳細資訊，請參閱[檢視或變更資料庫的復原模式 &#40;SQL Server&#41;](../../relational-databases/backup-restore/view-or-change-the-recovery-model-of-a-database-sql-server.md)。  
...
#### <a name="backup-devices-in-a-striped-media-set-a-stripe-set"></a>等量媒體集 (等量集) 中的備份裝置  
 A*集等量分割*是一組磁碟檔案分割成區塊和固定順序散發資料。 等量集中所使用的備份裝置數目必須維持相同 (除非您使用 FORMAT 重新初始化媒體)。  
...
##  <a name="Additional_Considerations"></a>備份選項的相關的其他考量  
...
||NOINIT|INIT|  
|------|------------|----------|  
|NOSKIP|如果磁碟區包含有效的媒體標頭，則驗證如果有給定的 MEDIANAME，媒體名稱與其相符。 如果相符，則附加備份組，保留所有現有的備份組。<br /><br /> 如果磁碟區並未包含有效的媒體標頭，便會發生錯誤。|如果磁碟區包含有效的媒體標頭，則執行下列檢查：<br /><br /> -如果指定了 MEDIANAME，確認給定的媒體名稱符合媒體標頭的媒體 name.* *<br /><br /> -驗證已在媒體上的沒有未到期備份組。 如果有，則結束備份。<br /><br /> 如果通過這些檢查，則覆寫媒體中的任何備份組，只保留媒體標頭。<br /><br /> 當磁碟區並未包含有效媒體標頭時，如果有指定的 MEDIANAME 和 MEDIADESCRIPTION，則產生含有它們的媒體標頭。|  
|SKIP|如果磁碟區包含有效的媒體標頭，則附加備份組，保留所有現有的備份組。|如果磁碟區包含有效 * 媒體標頭，會覆寫任何備份組在媒體上，只保留媒體標頭。<br /><br /> 當媒體是空的時，如果有指定 MEDIANAME 和 MEDIADESCRIPTION，則產生含有它們的媒體標頭。|  
  
 * 有效性包括 MTF 版本號碼和其他標頭資訊。 如果不支援指定的版本，或它不是預期的值，就會發生錯誤。  
  
 * * 使用者必須屬於適當的固定資料庫或伺服器角色來執行備份作業。  
...
>  在舊版 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 中，無法還原較新的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]版本所建立的備份。  
 ...
 BACKUP 支援 RESTART 選項，以便提供與舊版 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 之間的回溯相容性。 但 RESTART 卻沒有任何作用。  
...
###  <a name="url"></a>我。 備份至 Microsoft Azure Blob 儲存體服務 
