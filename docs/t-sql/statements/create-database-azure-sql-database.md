...
指定資料庫的服務層。 可用的值為：'basic'、'standard'、'premium'、'GeneralPurpose' 和 'BusinessCritical'。 已移除 'premiumrs' 的支援。 如有疑問，請使用此電子郵件別名： premium-rs@microsoft.com。
...
若要在彈性資料庫集區中建立新資料庫，請將資料庫的 SERVICE_OBJECTIVE 設定為 ELASTIC_POOL 並提供集區的名稱。 如需詳細資訊，請參閱[建立和管理 SQL Database 彈性資料庫集區 (預覽)](https://azure.microsoft.com/documentation/articles/sql-database-elastic-pool-portal/)。  
...
如需引數和 `CREATE DATABASE` 陳述式的詳細資訊，請參閱 [CREATE DATABASE](../../t-sql/statements/create-database-sql-server-transact-sql.md)。  
