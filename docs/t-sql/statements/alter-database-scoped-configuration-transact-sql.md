...
您可以使用 max degree of parallelism 選項來限制要用於平行計畫執行的處理器數目。 SQL Server 會平行執行計畫的查詢、 索引資料定義語言 (DDL) 作業、 平行插入、 線上改變資料行、 平行的統計資料集合和靜態和索引鍵集驅動資料指標擴展。
...
只能設定為次要，當資料庫中，在主要伺服器上，並指出組態將會設定為主要。 如果組態中的主要變更，也就是次要資料庫上的值將會變更據以設定次要資料庫，無須值明確。 **主要**是次要資料庫的預設設定。  
...
此值才有效，當資料庫中主要、 次要資料庫上，並指定所有次要資料庫上的這個設定的值將會針對主要設定的值。 如果在主要伺服器上使用的組態[參數探測](../../relational-databases/query-processing-architecture-guide.md#ParamSniffing)次要資料庫上的值會變更的變更，因此而不需要設定次要資料庫值明確。 這是次要資料庫的預設設定。  
...
此值才有效，當資料庫中主要、 次要和指定的所有次要資料庫上的這個設定值設定為主要的值。 組態中的主要變更，也就是次要資料庫上的值變更時隨之而不需要設定次要資料庫值明確。 這是次要資料庫的預設設定。  
...
## <a name="general-remarks"></a>一般備註  
 ...
 執行此陳述式清除程序快取，在目前資料庫中，這表示所有查詢都需要重新編譯。  
...
**DacFx**  
 ...
 由於 ALTER DATABASE SCOPED CONFIGURATION 是 Azure SQL Database 和 SQL Server 開始，會影響資料庫結構描述，將匯出的結構描述 （不論有無資料） 會無法匯入較舊版本的 SQL Server 的 SQL Server 2016 中的新功能例如[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]或[!INCLUDE[ssSQLv14](../../includes/sssqlv14-md.md)]。 例如，若要匯出[DACPAC](https://msdn.microsoft.com/library/ee210546.aspx#Anchor_3)或[BACPAC](https://msdn.microsoft.com/library/ee210546.aspx#Anchor_4)從[!INCLUDE[ssSDS](../../includes/sssds-md.md)]或[!INCLUDE[ssSQL15](../../includes/sssql15-md.md)]使用這項新功能的資料庫將無法匯入到下層伺服器。  
...
這個範例會設定 PARAMETER_SNIFFING 次要資料庫，所以在地理複寫案例的主要資料庫上。  
...
設為 ON 的 QUERY_OPTIMIZER_HOTFIXES 地理複寫案例中是主要資料庫。  
...
這個範例會啟用第一次編譯批次時，儲存在快取已編譯的計畫虛設常式。
