---
title: "INSERT (TRANSACT-SQL) |Microsoft 文件"
ms.custom: 
ms.date: 08/10/2017
...
## <a name="arguments"></a>引數  
 ...
 指定定義在 INSERT 陳述式範圍內的暫存具名結果集，也稱為通用資料表運算式。 這個結果集是從 SELECT 陳述式衍生而來。 如需詳細資訊，請參閱[common_table_expression &#40; 與TRANSACT-SQL &#41;](../../t-sql/queries/with-common-table-expression-transact-sql.md).  
 ...
 **適用於**： [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
 ...
 這是資料表或檢視表所在之連結伺服器的名稱。 *server_name*可以指定為[連結的伺服器](../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md)名稱，或使用[OPENDATASOURCE](../../t-sql/functions/opendatasource-transact-sql.md)函式。  
  
 當*server_name*指定為連結的伺服器， *database_name*和*schema_name*所需。 當*server_name* OPENDATASOURCE，以指定*database_name*和*schema_name*可能不適用於所有資料來源和 OLE DB 功能受限於存取遠端物件的提供者。  
 ...
 *table_or view_name*  
 ...
 A[資料表](../../t-sql/data-types/table-transact-sql.md)變數，其在範圍內，可用來當做 INSERT 陳述式中的資料表來源。  
  
 所參考的檢視*table_or_view_name*必須能夠更新，而且參考檢視的 FROM 子句中的一個基底資料表。 例如，必須使用指向多資料表檢視的 INSERT *column_list*參考單一基底資料表的資料行。 如需可更新檢視的詳細資訊，請參閱[CREATE VIEW &#40;TRANSACT-SQL &#41;](../../t-sql/statements/create-view-transact-sql.md).  
  
 *rowset_function_limited*  
 **適用於**： [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
  
 可能是[OPENQUERY](../../t-sql/functions/openquery-transact-sql.md)或[OPENROWSET](../../t-sql/functions/openrowset-transact-sql.md)函式。 這些函數的使用方式受限於存取遠端物件之 OLE DB 提供者的功能。  
  
 使用 ( \<table_hint_limited > [...*n* ] )  
 指定目標資料表允許使用的一個或多個資料表提示。 WITH 關鍵字和括號都是必要的。  
  
 不允許使用 READPAST、NOLOCK 和 READUNCOMMITTED。 如需有關資料表提示的詳細資訊，請參閱[資料表提示 &#40;TRANSACT-SQL &#41;](../../t-sql/queries/hints-transact-sql-table.md).  
...
>  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 未來的版本將移除指定 INSERT 陳述式的目標資料表之 HOLDLOCK、SERIALIZABLE、READCOMMITTED、REPEATABLEREAD 或 UPDLOCK 提示的功能。 這些提示不會影響 INSERT 陳述式的效能。 請避免在新的開發工作中使用它們，並規劃修改目前在使用它們的應用程式。  
 ...
 (*column_list*)  
 這是要插入資料的一個或多個資料行所組成的清單。 *column_list*必須括在括號並以逗號分隔。  
  
 如果資料行不在*column_list*、[!INCLUDE[ssDE](../../includes/ssde-md.md)]必須要能夠提供值，根據資料行的定義; 否則無法載入資料列。 如果資料行符合下列條件，[!INCLUDE[ssDE](../../includes/ssde-md.md)] 會自動提供資料行值：  
  
-   具有 IDENTITY 屬性。 使用下一個累加識別值。  
  
-   有預設值。 使用資料行的預設值。  
  
-   具有**時間戳記**資料型別。 使用目前的時間戳記值。  
  
-   可為 Null。 使用 Null 值。  
  
-   是計算資料行。 使用計算的值。  
  
*column_list*明確值插入識別欄位，和 SET IDENTITY_INSERT 選項必須是 ON，資料表時，必須使用。  
  
OUTPUT 子句  
 在插入作業中，傳回插入的資料列。 這些結果可以傳回給處理應用程式或插入資料表或資料表變數，以便進一步處理。  
  
 [OUTPUT 子句](../../t-sql/queries/output-clause-transact-sql.md)參考本機資料分割的檢視、 分散式資料分割的檢視或遠端資料表的 DML 陳述式或 INSERT 陳述式含有不支援*execute_statement*. INSERT 陳述式含有不支援 OUTPUT INTO 子句\<dml_table_source > 子句。 
  
 VALUES  
 導入要插入的資料值清單。 必須是每個資料行中的一個資料值*column_list*，若指定，或在資料表中。 這份值清單必須括在括號中。  
  
 如果值清單中的值不在相同資料表中的資料行順序，或在資料表中，不需要每個資料行的值*column_list*必須明確指定儲存每個內送值的資料行使用。  
 ...
 DEFAULT  
 強制 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 載入定義給資料行的預設值。 如果資料行的預設值不存在，而且資料行允許 Null 值，就會插入 NULL。 資料行定義為**時間戳記**資料類型下, 一個時間戳記值會插入。 DEFAULT 對識別欄位無效。  
  
 *expression*  
 這是一個常數、變數或運算式。 此運算式不能包含 EXECUTE 陳述式。  
  
 參考 Unicode 字元資料類型時**nchar**， **nvarchar**，和**ntext**，'*運算式*' 前面應該要有大寫字母 ' N '。 如果沒有指定 'N'，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 會將字串轉換成對應至資料庫預設定序或資料行的字碼頁。 在此字碼頁中找不到的任何字元都會遺失。  
 ...
 *execute_statement*  
 ...
 如果*execute_statement*用搭配 INSERT、 每個結果集必須與在資料表或資料行相容*column_list*。  
  
 *execute_statement*可用來在同一部伺服器或遠端伺服器上執行預存程序。 執行遠端伺服器中的程序，而且結果集會傳回本機伺服器，且載入本機伺服器的資料表中。 在分散式交易中， *execute_statement*當連接啟用 multiple active result set (MARS) 時無法發出對回送連結伺服器。  
  
 如果*execute_statement*傳回的資料使用 READTEXT 陳述式中，每個 READTEXT 陳述式可以傳回最多 1 mb (1024 KB) 的資料。 *execute_statement*也可以搭配擴充程序。 *execute_statement*插入擴充程序的主執行緒所傳回的資料; 不過，不會插入主要執行緒以外之執行緒的輸出。  
 ...
 \<dml_table_source >  
 指定插入目標資料表中的資料列就是 INSERT、UPDATE、DELETE 或 MERGE 陳述式的 OUTPUT 子句所傳回的資料列 (可選擇由 WHERE 子句篩選)。 如果\<dml_table_source > 指定時，外部 INSERT 陳述式的目標必須符合下列限制： 
...
-   它不能參與合併式複寫或是異動複寫的可更新訂閱。  
 ...
 \<select_list >  
 這是逗號分隔的清單，可指定要插入之 OUTPUT 子句所傳回的資料行。 中的資料行\<select_list > 必須與在其中插入值的資料行相容。 \<select_list > 不能參考彙總函式或 TEXTPTR。 
  
> [!NOTE]  
>  選取清單中列出的任何變數是指其原始值，不論其中做的任何變更\<d >。  
 ...
 其中\<s >  
 這任何 WHERE 子句包含有效\<s > 的篩選所傳回的資料列\<d >。 如需詳細資訊，請參閱[搜尋條件 &#40;TRANSACT-SQL &#41;](../../t-sql/queries/search-condition-transact-sql.md). 在此內容中使用時\<s > 不能包含子查詢、 執行資料存取、 彙總函式、 TEXTPTR、 或全文檢索搜尋述詞的純量使用者定義的函數。 
...
**適用於**： [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
 ...
 由外部工具用來上傳二進位資料流。 此選項不適用於工具例如[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]、 SQLCMD、 OSQL 或資料存取應用程式開發介面例如[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]原生用戶端。  
 ...
 **適用於**： [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
 ...
 KILOBYTES_PER_BATCH = kilobytes_per_batch  
 指定的位元組 (KB) 的每個批次為資料的近似數目*kilobytes_per_batch*。 如需詳細資訊，請參閱 [BULK INSERT&#40;Transact-SQL&#41;](../../t-sql/statements/bulk-insert-transact-sql.md)。  
...
## <a name="remarks"></a>備註  
將資料插入 SQL 圖形資料表的特定資訊，請參閱[插入 （SQL 圖形）](../../t-sql/statements/insert-sql-graph.md)。 
...
#### <a name="using-insert-intoselect-to-bulk-import-data-with-minimal-logging"></a>使用 INSERT INTO…SELECT 搭配最低限度記錄來大量匯入資料  
 您可以使用`INSERT INTO <target_table> SELECT <columns> FROM <source_table>`有效率地傳送大量的資料列從一個資料表，例如暫存資料表，另一個資料表使用最低限度記錄。 最低限度記錄可以改善此陳述式的效能並且降低交易期間作業填滿可用交易記錄空間的可能性。  
...
-   TABLOCK 提示可以將插入作業的記錄數目減至最小。 資料庫的復原模式必須設定為簡單或大量記錄，而且目標資料表不得用於複寫。 如需詳細資訊，請參閱[Prerequisites for Minimal Logging 大量匯入](../../relational-databases/import-export/prerequisites-for-minimal-logging-in-bulk-import.md)。  
...
-   如果值正在載入具有資料行**char**， **varchar**，或**varbinary**資料類型、 填補或截斷尾端空白 (空格**char**和**varchar**，零**varbinary**) 取決於建立資料表時，資料行定義的 SET ANSI_PADDING 設定。 如需詳細資訊，請參閱 [SET ANSI_PADDING &#40;Transact-SQL&#41;](../../t-sql/statements/set-ansi-padding-transact-sql.md)。  
...
-   如果將空字串 (' ') 會載入至具有的資料行**varchar**或**文字**資料類型的預設作業是載入長度為零的字串。  
  
-   插入 null 值插入**文字**或**映像**資料行不會建立有效的文字指標，也不會預先配置 8KB 文字頁面。  
  
-   建立與資料行**uniqueidentifier**資料類型儲存特殊格式的 16 位元組二進位值。 不像識別欄位，[!INCLUDE[ssDE](../../includes/ssde-md.md)]不會自動產生資料行值**uniqueidentifier**資料型別。 插入作業期間，類型的變數資料**uniqueidentifier**和字串常數形式*xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx* (36 個字元，包括連字號，其中*x*是範圍 0-9 或 a-f 的十六進位數字) 可用於**uniqueidentifier**資料行。 例如，6F9619FF-8B86-D011-B42D-00C04FC964FF 是有效的值**uniqueidentifier**變數或資料行。 使用[newid （)](../../t-sql/functions/newid-transact-sql.md)函數來取得全域唯一識別碼 (GUID)。  
...
-   只要使用者定義型別支援從這個類型進行隱含或明確的轉換，便在 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 系統資料類型中提供一個值。 下列範例示範如何在使用者定義類型的資料行中插入值`Point`，由從字串進行明確的轉換。  
...
## <a name="interoperability"></a>互通性  
 定義資料表或檢視表之 INSERT 動作的 INSTEAD OF 觸發程序時，系統會執行觸發程序，而不是 INSERT 陳述式。 而不是觸發程序的相關資訊，請參閱[CREATE TRIGGER &#40;TRANSACT-SQL &#41;](../../t-sql/statements/create-trigger-transact-sql.md).  
  
## <a name="limitations-and-restrictions"></a>限制事項  
 當您將值插入遠端資料表時，如果並未完整指定所有資料行的所有值，您必須識別要插入指定值的資料行。  
  
 搭配 INSERT 使用 TOP 時，不會以任何順序排列參考的資料列，也不可以直接在這個陳述式中指定 ORDER BY 子句。 如果您需要使用 TOP 依有意義的時間順序插入資料列，您必須在 subselect 陳述式中指定 ORDER BY 子句一起使用頂端。 請參閱本主題稍後的＜範例＞一節。
 
使用的 INSERT 查詢選取搭配 ORDER BY 來擴展資料列保證識別值的計算方式，但不是在其中插入資料列的順序。    
  
## <a name="logging-behavior"></a>記錄行為  
 使用 OPENROWSET 函數搭配 BULK 關鍵字，或使用時，除了一律會完整記錄在 INSERT 陳述式`INSERT INTO <target_table> SELECT <columns> FROM <source_table>`。 這些作業可以進行最低限度記錄。 如需詳細資訊，請參閱本主題前面的＜大量載入資料的最佳做法＞一節。  
  
## <a name="security"></a>安全性  
 在連結的伺服器連接期間，傳送端伺服器會提供一個登入名稱與密碼來代表它本身，以連接到接收端伺服器。 這個連接運作，您必須建立使用連結的伺服器之間的登入對應[sp_addlinkedsrvlogin](../../relational-databases/system-stored-procedures/sp-addlinkedsrvlogin-transact-sql.md)。  
  
 當您使用 OPENROWSET(BULK…) 時，一定要了解 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 如何處理模擬。 如需詳細資訊，請參閱 < 安全性考量 > 中[使用 BULK INSERT 或 OPENROWSET &#40; 匯入大量資料BULK_ &#41;&#40;SQL Server &#41;](../../relational-databases/import-export/import-bulk-data-by-using-bulk-insert-or-openrowset-bulk-sql-server.md).  
...
|[資料列插入遠端資料表](#RemoteTables)|連結的伺服器 • OPENQUERY 資料列集函數 • OPENDATASOURCE 資料列集函數|  
|[大量載入資料，從資料表或資料檔案](#BulkLoad)|INSERT…SELECT • OPENROWSET 函數|  
...
#### <a name="a-inserting-a-single-row-of-data"></a>A. 插入單一資料列  
 下列範例會在 [!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)] 資料庫的 `Production.UnitMeasure` 資料表中插入一個資料列。 此資料表中的資料行為 `UnitMeasureCode`、`Name` 和 `ModifiedDate`。 因為所有資料行的值提供了在資料表中的資料行相同的順序列出，而且不需要指定資料行清單中的資料行名稱*。*  
...
#### <a name="b-inserting-multiple-rows-of-data"></a>B. 插入多個資料列  
 下列範例會使用[資料表值建構函式](../../t-sql/queries/table-value-constructor-transact-sql.md)至三個資料列插入`Production.UnitMeasure`資料表中[!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)]單一 INSERT 陳述式中的資料庫。 由於提供了所有資料行的值，而且依照資料表中資料行的相同順序來列出它們，因此，不需要在資料行清單中指定資料行名稱。  
...
#### <a name="c-inserting-data-that-is-not-in-the-same-order-as-the-table-columns"></a>C. 插入與資料表資料行順序不同的資料  
 下列範例會利用資料行清單來明確指定插入每個資料行的值。 中的資料行順序`Production.UnitMeasure`資料表中[!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)]資料庫是`UnitMeasureCode`， `Name`， `ModifiedDate`; 不過，資料行不會列出依該順序*column_list*。  
...
###  <a name="ColumnValues"></a>處理資料行值  
 本節中的範例示範的值插入資料行定義了 IDENTITY 屬性，預設值，或這類定義的資料類型方法**uniqueidentifier**或使用者定義類型資料行。  
  
#### <a name="d-inserting-data-into-a-table-with-columns-that-have-default-values"></a>D. 將資料插入包含有預設值之資料行的資料表  
 下列範例會顯示如何將資料列插入含有自動產生值或有預設值的資料行之資料表中。 `Column_1` 是一個計算資料行，它會將字串與插入 `column_2` 的值串連起來，自動產生某個值。 `Column_2` 會使用預設條件約束來定義。 如果此資料行並未指定值，就會使用預設值。 `Column_3`定義與**rowversion**資料型別，會自動產生唯一的遞增二進位數字。 `Column_4` 不會自動產生值。 如果未指定這個資料行的值，就會插入 NULL。 INSERT 陳述式會插入包含部分 (而非全部) 資料行值的資料列。 在最後一個 INSERT 陳述式中，並未指定任何資料行，只會使用 DEFAULT VALUES 子句插入預設值。  
...
#### <a name="f-inserting-data-into-a-uniqueidentifier-column-by-using-newid"></a>F. 利用 NEWID() 將資料插入 uniqueidentifier 資料行  
 下列範例會使用[NEWID](../../t-sql/functions/newid-transact-sql.md)（） 函數來取得的 GUID `column_2`。 不像識別欄位，[!INCLUDE[ssDE](../../includes/ssde-md.md)]不會自動產生資料行值[uniqueidentifier](../../t-sql/data-types/uniqueidentifier-transact-sql.md)資料型別，如所示，第二個`INSERT`陳述式。  
...
#### <a name="g-inserting-data-into-user-defined-type-columns"></a>G. 將資料插入使用者定義型別的資料行  
 下列 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式會將三個資料列插入 `PointValue` 資料表的 `Points` 資料行。 此資料行使用[CLR 使用者定義型別](../../relational-databases/clr-integration-database-objects-user-defined-types/clr-user-defined-types.md)(UDT)。 `Point` 資料類型包括公開為 UDT 屬性的 X 及 Y 整數值。 您必須使用 CAST 或 CONVERT 函數，將以逗號分隔的 X 及 Y 值轉換為 `Point` 類型。 前兩個陳述式使用 CONVERT 函數將字串值轉換成`Point`類型，而第三個陳述式則使用 CAST 函數。 如需詳細資訊，請參閱[操作 UDT 資料](../../relational-databases/clr-integration-database-objects-user-defined-types/working-with-user-defined-types-manipulating-udt-data.md)。  
...
#### <a name="h-using-the-select-and-execute-options-to-insert-data-from-other-tables"></a>H. 使用 SELECT 和 EXECUTE 選項插入其他資料表的資料  
 ...
 第一個 INSERT 陳述式使用 SELECT 陳述式從來源資料表衍生的資料 (`Employee`， `SalesPerson`，和`Person`) 中[!INCLUDE[ssSampleDBnormal](../../includes/sssampledbnormal-md.md)]資料庫中，結果集儲存在`EmployeeSales`資料表。 第二個 INSERT 陳述式會使用 EXECUTE 子句來呼叫包含 SELECT 陳述式的預存程序，而第三個 INSERT 陳述式會使用 EXECUTE 子句將 SELECT 陳述式當做常值字串來參考。  
...
#### <a name="k-inserting-data-by-specifying-a-view"></a>K. 指定檢視表以插入資料  
 下列範例會將檢視表名稱指定為目標物件；不過，新資料列會插入基礎基底資料表中。 `INSERT` 陳述式中的值順序必須符合檢視表的資料行順序。 如需詳細資訊，請參閱[修改資料透過檢視](../../relational-databases/views/modify-data-through-a-view.md)。  
...
###  <a name="RemoteTables"></a>資料列插入遠端資料表  
 本節中的範例將示範如何將資料列插入遠端目標資料表，使用[連結的伺服器](../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md)或[資料列集函數](../../t-sql/functions/rowset-functions-transact-sql.md)來參考遠端資料表。  
  
#### <a name="m-inserting-data-into-a-remote-table-by-using-a-linked-server"></a>M. 使用連結的伺服器將資料插入遠端資料表  
 下列範例會將資料列插入遠端資料表。 此範例一開始會使用建立遠端資料來源的連結[sp_addlinkedserver](../../relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql.md)。 連結的伺服器名稱`MyLinkServer`，然後指定表單中的四部分物件名稱的一部分*server.catalog.schema.object*。  
...
#### <a name="n-inserting-data-into-a-remote-table-by-using-the-openquery-function"></a>N. 使用 OPENQUERY 函數將資料插入遠端資料表  
 下列範例將資料列插入遠端資料表藉由指定[OPENQUERY](../../t-sql/functions/openquery-transact-sql.md)資料列集函數。 上一個範例所建立之連結的伺服器名稱會用於這個範例。  
...
#### <a name="o-inserting-data-into-a-remote-table-by-using-the-opendatasource-function"></a>O. 使用 OPENDATASOURCE 函數將資料插入遠端資料表  
 下列範例將資料列插入遠端資料表藉由指定[OPENDATASOURCE](../../t-sql/functions/opendatasource-transact-sql.md)資料列集函數。 使用格式指定有效的伺服器名稱的資料來源*server_name*或*\ 執行個體名稱*。  
...
#### <a name="p-inserting-into-an-external-table-created-using-polybase"></a>P. 使用 PolyBase 建立外部資料表中插入  
 將資料從 SQL Server 匯出至 Hadoop 或 Azure 儲存體。 首先，建立指向目的地檔案或目錄的外部資料表。 然後，使用 INSERT INTO 將資料從本機 SQL Server 資料表匯出至外部資料來源。 如果目的地檔案或目錄不存在，INSERT INTO 陳述式會加以建立，並將 SELECT 陳述式的結果以指定檔案格式匯出至指定位置。  如需詳細資訊，請參閱 [開始使用 PolyBase](../../relational-databases/polybase/get-started-with-polybase.md)。  
...
###  <a name="BulkLoad"></a>大量載入資料，從資料表或資料檔案  
...
#### <a name="r-using-the-openrowset-function-with-bulk-to-bulk-load-data-into-a-table"></a>R. 搭配 BULK 使用 OPENROWSET 函數，將資料大量載入資料表  
 下列範例會藉由指定 OPENROWSET 函數，將資料檔案中的資料列插入資料表。 將會指定 IGNORE_TRIGGERS 資料表提示來讓效能最佳化。 如需其他範例，請參閱[使用 BULK INSERT 或 OPENROWSET &#40; 匯入大量資料BULK_ &#41;&#40;SQL Server &#41;](../../relational-databases/import-export/import-bulk-data-by-using-bulk-insert-or-openrowset-bulk-sql-server.md).  
...
###  <a name="TableHints"></a>使用提示來覆寫查詢最佳化工具的預設行為  
 本節中的範例將示範如何使用[資料表提示](../../t-sql/queries/hints-transact-sql-table.md)處理 INSERT 陳述式時，暫時覆寫查詢最佳化工具的預設行為。  
...
###  <a name="CaptureResults"></a>擷取 INSERT 陳述式的結果  
 本節中的範例將示範如何使用[OUTPUT 子句](../../t-sql/queries/output-clause-transact-sql.md)傳回資訊，或是根據運算式，每個資料列 INSERT 陳述式所影響。 這些結果可以傳回給負責處理的應用程式，以便用在確認訊息、封存或其他這類應用程式需求等用途上。  
...
#### <a name="w-inserting-data-using-the-select-option"></a>W 使用選取的選項插入資料  
 下列範例會示範如何插入多個選取的選項搭配使用 INSERT 陳述式的資料列。 第一個`INSERT`陳述式會利用`SELECT`直接從來源資料表中，擷取資料的陳述式，然後將結果儲存至設定`EmployeeTitles`資料表。  
...
#### <a name="y-using-a-label-and-a-query-hint-with-the-insert-statement"></a>Y。 使用標籤和查詢提示搭配 INSERT 陳述式  
 此查詢會顯示搭配 INSERT 陳述式中使用的標籤以及查詢聯結提示的基本語法。 查詢提送到的控制節點之後[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]、 計算節點上執行，會在產生時套用雜湊聯結策略[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]查詢計劃。 如需有關聯結提示，以及如何使用 OPTION 子句的詳細資訊，請參閱[選項 (SQL Server PDW)](http://msdn.microsoft.com/en-us/72bbce98-305b-42fa-a19f-d89620621ecc)。  
...
## <a name="see-also"></a>另請參閱  
 ...
 [合併式 &#40;TRANSACT-SQL &#41;](../../t-sql/statements/merge-transact-sql.md)   
