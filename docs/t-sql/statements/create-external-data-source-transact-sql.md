...
建立 PolyBase 或彈性資料庫查詢的外部資料來源。 外部資料來源可用於建立連線並支援四個主要使用案例：
...
> [!NOTE]  
> 在 SQL Server (2016 或更新版本)、Azure SQL 資料倉儲和平行處理資料倉儲上支援 PolyBase。 只有在 Azure SQL Database v12 或更新版本上才支援[彈性查詢][intro_eq]。
...
指定資料來源的使用者定義名稱。 此名稱在 SQL Server、SQL Database (SQL DB) 和 SQL 資料倉儲 (SQL DW) 資料庫內必須是唯一的。 此名稱在平行處理資料倉儲 (PDW) 伺服器內必須是唯一的。
...
- 當外部資料來源為 Cloudera、Hortonworks、Azure Blob 儲存體、ADLS Gen 1 或 ADLS Gen 2 時，請使用 HADOOP。
...
如需示範如何建立外部資料來源 (其中 `TYPE` = `SHARD_MAP_MANAGER`) 的範例，請參閱[建立分區對應管理員外部資料來源](#f-create-a-shard-map-manager-external-data-source)

## <a name="permissions"></a>權限

需要 SQL Server、平行處理資料倉儲、SQL Database 和 SQL 資料倉儲的 CONTROL 權限。
...
## <a name="examples-sql-server-2016-and-parallel-data-warehouse"></a>範例:SQL Server (2016+) 和平行處理資料倉儲
...
## <a name="examples-sql-server-2016-sql-data-warehouse-and-parallel-data-warehouse"></a>範例:SQL Server (2016+)、SQL 資料倉儲和平行處理資料倉儲
...
## <a name="examples-sql-database"></a>範例:SQL Database

### <a name="f-create-a-shard-map-manager-external-data-source"></a>F. 建立分區對應管理員外部資料來源
...
### <a name="g-create-an-rdbms-external-data-source"></a>G. 建立 RDBMS 外部資料來源
...
## <a name="examples-sql-data-warehouse"></a>範例:SQL 資料倉儲

### <a name="h-create-external-data-source-to-reference-azure-data-lake-store-gen-1"></a>H. 建立參考 Azure Data Lake Store Gen 1 的外部資料來源
...
### <a name="i-create-external-data-source-to-reference-azure-data-lake-store-adls-gen-2"></a>I. 建立參考 Azure Data Lake Store (ADLS) Gen 2 的外部資料來源
