 ...
 *\<block_dml_operation>* 將套用 BLOCK 述詞的特定 DML 作業。 AFTER 可指定在 DML 作業 (INSERT 或 UPDATE) 執行之後，根據資料列的值來評估述詞。 BEFORE 可指定在 DML 作業 (UPDATE 或 DELETE) 執行之前，根據資料列的值來評估述詞。 如果沒有指定作業，則會將述詞套用至所有作業。  
