...
對 Parquet 或 ORC 檔案使用 CREATE EXTERNAL TABLE AS SELECT 將會造成錯誤，當資料中出現下列字元時，可能會包含拒絕的資料行：
...
若要使用包含這些字元的 CREATE EXTERNAL TABLE AS SELECT，您必須先對資料使用 CREATE EXTERNAL TABLE AS SELECT，以分隔文字檔，然後您即可使用外部工具，將其轉換成 Parquet 或 ORC。
