 ...
 *file_format_name*  
 ...
 RCFILE (搭配 SERDE_METHOD = *SERDE_method*) 指定的單欄式記錄的檔案格式 (RcFile)。 此選項需要您指定的 hive 控制檔的序列化和還原序列化程式 (SerDe) 方法。 如果您使用 Hadoop Hive/HiveQL 查詢 RC 檔，這項需求都相同。 請注意，SerDe method 會區分大小寫。
...
-   FORMAT_TYPE = RCFILE，SERDE_METHOD = 'org.apache.hadoop.hive.serde2.columnar.ColumnarSerDe'
 ...
 FIELD_TERMINATOR = *field_terminator*僅適用於分隔的文字檔。 欄位結束字元文字分隔檔案中指定一個或多個標記的每個欄位 （資料行） 結尾的字元。 預設值是管道字元 ꞌ | ꞌ。 如需保證支援，我們建議使用一或多個 ascii 字元。
...
-   STRING_DELIMITER = '0x7E0x7E'-兩個波狀符號 (例如，~ ~)
 ...
 日期\_格式 = *fieldinfo*指定分隔的文字檔案中可能會出現的所有日期和時間資料的自訂格式。 如果原始程式檔會使用預設的日期時間格式，不需要這個選項。 只有一個自訂的日期時間格式允許每個檔案。 您無法指定每個檔案的多個自訂日期時間格式。 不過，您可以使用多個日期時間格式，如果每個外部資料表定義中其各自的資料類型的預設格式。
...
-   年、 月和日可以有各種不同的格式和訂單。 下表僅顯示**ymd**格式。 月份可以有 1 或 2 的數字，或是 3 個字元。 日期可以有 1 或 2 位數。 年份可以有 2 或 4 位數。
...
-   若要指定每月為文字，請使用三個或多個字元。 1 或 2 個字元的月份會被視為數字。
...
-   字母 'zzz' 指定系統的目前的時區格式的時區位移 {+ |-} HH:ss]。
 ...
 USE_TYPE_DEFAULT = {TRUE |**FALSE** } 指定如何處理遺漏值中分隔的文字檔時 PolyBase 會從文字檔擷取資料。
  
 從文字檔擷取資料的外部資料表定義中的對應資料行的資料類型使用預設值儲存每個遺漏值時，則為 TRUE。 例如，取代遺漏值：  
...
-   1900-01-01 如果資料行是日期資料行。
 ...
 FALSE 存放所有遺漏值，則為 NULL。 會使用 NULL 這個字分隔的文字檔案中儲存任何 NULL 值會以字串 'NULL' 匯入。
  
   編碼 = {'UTF8' |'UTF16}' 中 Azure SQL 資料倉儲、 PolyBase 可以讀取 UTF8 及 UTF16 LE 編碼分隔的文字檔案。 在 SQL Server 和 PDW 中，PolyBase 不支援讀取 UTF16 編碼檔案。
  
 DATA_COMPRESSION = *data_compression_method*指定外部資料的資料壓縮方法。 如果沒有指定 DATA_COMPRESSION，預設為未壓縮的資料。
...
## <a name="general-remarks"></a>一般備註
 ...
 當其中一種壓縮格式儲存資料時，PolyBase 會傳回的資料記錄之前先解壓縮資料。
  
## <a name="limitations-and-restrictions"></a>限制事項
  
 Hadoop 的 LineRecordReader 必須支援資料列分隔符號分隔文字檔案中。 也就是說，它必須是 '\r'、 '\n'、 或 '\r\n'。 無法由使用者設定這些分隔符號。
 ...
 由於在外部資料表中的檔案數目的限制，我們建議您在根和外部檔案位置的子資料夾中儲存小於 30,000 檔案。 此外，我們建議您保持為較小的數字的根目錄底下的子資料夾的數目。 太多檔案參考時，可能是 Java 虛擬機器的記憶體不足例外狀況。
  
  當匯出至 Hadoop 或 Azure Blob 儲存體透過 PolyBase，只將資料匯出資料，沒有資料行 names(metadata) CREATE EXTERNAL TABLE 命令中所定義。
...
### <a name="a-create-a-delimitedtext-external-file-format"></a>A. 建立 DELIMITEDTEXT 外部檔案格式  
 這個範例會建立名為外部檔案格式*textdelimited1*文字分隔檔案。 格式所列出的選項\_選項所指定的應該使用縱線字元分隔檔案中的欄位 ' |'。 文字檔案也會自動使用 Gzip 轉碼器壓縮。 如果資料\_壓縮未指定，未壓縮的文字檔。
