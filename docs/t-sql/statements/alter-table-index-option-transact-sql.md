 ...
 **適用於**： [!INCLUDE[ssSQL14](../../includes/sssql14-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]。  
 ...
 A**交換器**或線上索引重建完成，因為此資料表沒有封鎖作業。 *WAIT_AT_LOW_PRIORITY*表示，如果**交換器**或線上索引重建作業無法立即完成，它將會等候。 作業會具有低優先順序鎖定，允許其他具有與 DDL 陳述式衝突之鎖定的作業執行。 省略**WAIT AT LOW PRIORITY**選項相當於`WAIT_AT_LOW_PRIORITY ( MAX_DURATION = 0 minutes, ABORT_AFTER_WAIT = NONE)`。  
  
MAX_DURATION =*時間*[**分鐘**]  
 等候時間 （以分鐘為單位的整數值），**交換器**或執行 DDL 命令時，將會等候必須取得的線上索引重建鎖定。 SWITCH 或線上索引重建作業會嘗試立即完成。 如果操作已遭到封鎖**MAX_DURATION**時間的其中一個**ABORT_AFTER_WAIT**會在執行動作。 **MAX_DURATION**時間永遠是以分鐘和 word**分鐘**可以省略。  
  
ABOUT_AFTER_WAIT = [**NONE** | **自助** | **封鎖**}]  
 無  
