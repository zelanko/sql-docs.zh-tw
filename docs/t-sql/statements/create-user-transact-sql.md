...
- 適用於 Azure AD 群組和 Azure AD 應用程式之 Azure AD 物件的 DisplayName。 如果您有「護士」安全性群組，則可以使用：  
 ...
 如需詳細資訊，請參閱[使用 Azure Active Directory 驗證連線到 SQL Database](https://azure.microsoft.com/documentation/articles/sql-database-aad-authentication)。  
...
##  <a name="syntax-summary"></a><a name="SyntaxSummary"></a> 語法摘要  
 **具有 master 登入的使用者**  
...
###  <a name="d-creating-and-using-a-user-without-a-login"></a><a name="withoutLogin"></a> D. 建立及使用不含登入的使用者  
...
### <a name="i-create-an-azure-ad-user-from-an-azure-ad-login-in-sql-database-managed-instance"></a>I. 在 SQL Database 受控執行個體中從 Azure AD 登入建立 Azure AD 使用者
 ...
 使用授與 `sysadmin` 角色的 Azure AD 登入來登入受控執行個體。 下列範例會從登入 bob@contoso.com 來建立 Azure AD 使用者 bob@contoso.com。 此登入是在 [CREATE LOGIN](/sql/t-sql/statements/create-login-transact-sql#examples) 範例中建立。
