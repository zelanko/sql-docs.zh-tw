...
MODIFY (BACKUP_STORAGE_REDUNDANCY **=** ['LOCAL' \| 'ZONE' \| 'GEO']) 會變更資料庫的時間點還原備份與長期保留備份 (如果已設定) 的儲存體備援。 這些變更會套用至所有未來建立的備份。 現有備份會繼續使用先前的設定。 
...
### <a name="h-update-the-backup-storage-redundancy-of-a-database"></a>H. 更新資料庫的備份儲存體備援
