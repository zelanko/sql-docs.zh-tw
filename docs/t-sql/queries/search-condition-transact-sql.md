...
> [!NOTE]  
>  非 Unicode 字串常數和變數使用對應至資料庫預設定序的字碼頁。 使用非 Unicode 字元資料時，程式碼頁的轉換可以發生，並參考非 Unicode 字元資料類型**char**， **varchar**，和**文字**。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]將非 Unicode 字串常數和變數轉換成對應至參考資料行的定序，或指定使用 COLLATE，如果該字碼頁對應至資料庫預設定序的字碼頁不同的字碼頁中。 在新的字碼頁中找不到任何字元將會轉譯成類似的字元，如果[自動調整的對應](http://www.unicode.org/Public/MAPPINGS/VENDORS/MICSFT/WindowsBestFit/)可以找到，否則就會轉換成預設的取代字元的"？"。  
>  
> 當使用多個字碼頁，字元常數可以在前面加上大寫字母 ' N '，和 Unicode 變數可用，以避免字碼頁轉換。  
