...
ms.openlocfilehash: 785dbfd706afce23726baac5f6e9c29c4fc389fa
ms.sourcegitcommit: e02c28b0b59531bb2e4f361d7f4950b21904fb74
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/02/2018
ms.locfileid: "39460012"
---
# <a name="top-transact-sql"></a>TOP (Transact-SQL)
[!INCLUDE[tsql-appliesto-ss2008-all-md](../../includes/tsql-appliesto-ss2008-all-md.md)]

  在 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 中，將查詢結果集中所傳回的資料列數限制為指定數目的資料列或是資料列的百分比。 當 TOP 與 ORDER BY 子句一起使用時，結果集會限制為前 *N* 個已排序資料列，否則會傳回前 *N* 個未定義順序的資料列。 使用這個子句以指定 SELECT 陳述式所傳回的資料列數目或受到 INSERT、UPDATE、MERGE 或 DELETE 陳述式所影響的資料列數目。  
 ...
 *expression*  
 這是指定要傳回的資料列數目之數值運算式。 如果已指定 PERCENT，*expression* 會隱含地轉換為 **float** 值；否則，它會轉換為 **bigint**。  
  
 PERCENT  
 指出查詢只從結果集中傳回前 *expression* % 的資料列。 含小數的值會無條件進位到下一個整數值。  
  
 WITH TIES  
 當您想要針對限制結果集中的最後一個位數傳回兩個或多個繫結的資料列時使用。 必須搭配 **ORDER BY** 子句使用。 **WITH TIES** 可能會導致傳回比 *expression* 中所指定之值更多的資料列。 例如，如果 *expression* 設定為 5，但 2 個額外的資料列符合資料列 5 中 **ORDER BY** 資料行的值，結果集就會包含 7 個資料列。  
...
-   在 SELECT 陳述式中，查詢最佳化工具在查詢最佳化期間會考量 TOP 或 FETCH 子句中的 *expression* 值。 因為 SET ROWCOUNT 是在執行查詢的陳述式之外使用，所以它的值不能在查詢計劃中列入考量。  
...
## <a name="examples-includesssdwfullincludessssdwfull-mdmd-and-includesspdwincludessspdw-mdmd"></a>範例：[!INCLUDE[ssSDWfull](../../includes/sssdwfull-md.md)] 和 [!INCLUDE[ssPDW](../../includes/sspdw-md.md)]  
 下列範例會傳回符合查詢準則的前 31 個資料列。 **ORDER BY** 子句用來確保 31 個傳回的資料列是根據 `LastName` 資料行字母順序的前 31 個資料列。  
