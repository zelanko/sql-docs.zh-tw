...
[!INCLUDE[tsql-appliesto-ss2008-all-md](../../includes/tsql-appliesto-ss2008-all-md.md)]
 ...
 KILL 可用於結束正常的連線，這樣就可在內部結束與指定之工作階段識別碼關聯的交易。 當 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 分散式交易協調器 (MS DTC) 在使用中時，此陳述式也可用以結束被遺棄和不確定的分散式交易。  
 ...
 *session ID*  
 這是要結束之處理序的工作階段識別碼。 *session ID* 是在建立連線時，指派給每個使用者連線的唯一整數 (**int**)。 在連線持續時間，工作階段識別碼值會繫結連線。 當連線結束時，會釋出這個整數值，它可以重新指派給新的連線。  
...
**適用於**：[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 至 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]
 ...
 識別分散式交易的工作單位識別碼 (UOW)。 *UOW* 是可以從 sys.dm_tran_locks 動態管理檢視的 request_owner_guid 資料行取得的 GUID。 您也可以從錯誤記錄檔或透過 MS DTC 監視器來取得 *UOW*。 如需有關監視分散式交易的詳細資訊，請參閱 MS DTC 文件。  
...
利用 @@SPID 來顯示目前工作階段的工作階段識別碼值。  
 ...
 當特定連線對資料庫資源有鎖定並封鎖另一個連線的進度時，封鎖連線的工作階段識別碼會出現在 `blocking_session_id` 的 `sys.dm_exec_requests` 資料行或是由 `blk` 傳回的 `sp_who` 資料行中。  
...
```  
 ...
 您可以重複使用不含 WITH STATUSONLY 選項的相同 KILL *session ID*|*UOW* 陳述式來取得相同的狀態報告，但建議您不要這樣做。 如果復原已完成，且在執行新的 KILL 陳述式之前，已將工作階段識別碼重新指派給新的工作，則重複使用 KILL *session ID* 陳述式可能會終止新的處理序。 指定 WITH STATUSONLY 可防止發生這個情況。  
