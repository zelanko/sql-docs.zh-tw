...
## <a name="factors-affecting-the-duration-of-checkpoint-operations"></a>影響檢查點作業持續時間的因素  
 一般而言，檢查點作業必須寫入的中途分頁數愈多，檢查點作業所需要的時間也會愈長。 為能將其他應用程式所受到的效能影響降到最低，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 預設會調整檢查點作業執行的寫入頻率。 降低寫入頻率會增加完成檢查點作業所需要的時間。 除非在 CHECKPOINT 命令中指定 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]checkpoint_duration*值，否則* 會將此策略應用於手動檢查點。  
