---
title: "STPointFromWKB (geometry 資料類型) |Microsoft 文件"
ms.custom: 
ms.date: 08/03/2017
...
傳回**geometryPoint**從開放式地理空間協會 (OGC) 已知二進位 (well-known binary，WKB) 表示法的執行個體。
 ...
 *WKB_point*  
 是的 WKB 表示法**geometryPoint**您想要傳回的執行個體。 *WKB_point*是**varbinary （max)**運算式。  
