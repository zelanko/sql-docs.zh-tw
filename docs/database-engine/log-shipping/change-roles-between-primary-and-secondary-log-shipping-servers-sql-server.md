---
title: 變更主要 & 次要記錄傳送角色
description: 了解如何設定您的次要資料庫以作為 SQL Server 記錄傳送解決方案的主要複本。
...
4.  如果您先前的記錄傳送組態已啟用記錄傳送監視，請將記錄傳送監視重新設定為監視新的記錄傳送組態。  將 threshold_alert_enabled 設定為 1 會指定當超過 restore_threshold 時將會引發警示。 執行下列命令，以資料庫的名稱取代 *database_name* ：  
