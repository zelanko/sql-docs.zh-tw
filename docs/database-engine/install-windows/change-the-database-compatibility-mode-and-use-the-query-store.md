...
helpviewer_keywords: 
  - "查詢計畫 [SQL Server], 移轉"
  - "升級 SQL Server, 移轉查詢計畫"
  - "計畫指南 [SQL Server], 移轉查詢計畫"
...
---
# 變更資料庫相容性模式並使用查詢存放區
  在 SQL Server 2016 中，某些變更只有在資料庫的 DATABASE_COMPATIBILITY 層級變更為 130 之後才會啟用。  有數種原因可以完成這項作業：  
  
-   因為升級是單向作業 (不可能降級檔案格式)，所以可以將啟用新功能區隔到資料庫內的個別作業。  可以將設定還原為先前的 DATABASE_COMPATIBILITY 層級。  新的模型可減少必須在中斷期間發生的事項數目。  
  
-   查詢處理器的變更會有複雜的影響。  即使系統的「良好」變更對大多數客戶可能不錯，但是可能會造成其他位置的重要查詢的無法接受衰退。  區隔此邏輯與升級程序，可讓功能 (例如查詢存放區) 快速地降低計畫選擇衰退，或甚至在實際執行伺服器中完全予以避免。  
...
>  如果使用者資料庫的相容性層級在升級前為 100 或更高層級，則在升級後仍會保持相同。 如果升級前的相容性層級為 90，則在升級後的資料庫中，相容性層級會設定為 100 (這是 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 所支援的最低相容性層級)。  
 ...
 啟用新查詢處理器功能的升級程序與產品的發行後服務模型有關。  其中某些修正程式是在追蹤旗標 4199 下發行。  需要修正程式的客戶可以選擇這些修正程式，而不會造成其他客戶的非預期衰退。  查詢處理器 Hotfix 的發行後服務模型記載在[這裡](https://support.microsoft.com/en-us/kb/974006)。 從 SQL Server 2016 開始，移到新的相容性層級表示不再需要 4199 追蹤旗標，因為現在預設會在最新相容性模式 (130) 中啟用那些修正程式。  因此，在升級程序期間，一定要確認在升級程序完成之後未啟用 4199。  
