...
---
# SQL Server 使用意見收集的本機稽核
## 簡介

Microsoft SQL Server 2016 (SQL Server) 包含一些啟用網際網路的功能，而這些功能可能會收集　貴用戶電腦或裝置的相關資訊 (「標準電腦資訊」) 並將此資訊傳送至 Microsoft。 [SQL Server 使用意見收集](http://support.microsoft.com/kb/3153756)的本機稽核元件會將服務收集的資料寫入至指定的資料夾，代表將傳送給 Microsoft 的資料 (記錄)。 本機稽核的目的是要讓客戶看到 Microsoft 以此功能收集的所有資料，以用於相容性、法規或隱私權驗證的理由。  

在 SQL Server 2016 CU2 中，本機稽核可以在 SQL Server Database Engine 和 Analysis Services (SSAS) 的執行個體層級設定。 SQL Server Integration Services (SSIS) 尚未啟用本機稽核。 安裝程式執行期間所安裝的其他 SQL Server 元件，以及在安裝程式執行之後下載或安裝的 SQL Server 工具，沒有使用意見收集的本機稽核功能。 
...
1. 建立登錄機碼設定來設定本機稽核目標目錄。 
    ...
    若為 Database Engine，於 *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL13.\<INSTANCENAME\>\\CPE* 建立機碼。 
    
    若為 Analysis Services，於 *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSAS13.\<INSTANCENAME\>\\CPE* 建立機碼。

### 取得 SQL Server CEIP 服務登入帳戶

執行下列步驟，以取得 SQL Server CEIP 遙測服務登入帳戶
 
1. 啟動**服務** - 按一下 [Windows] 按鈕並輸入 [services.msc]。 

2. 瀏覽至適當的服務。 例如，若為 Database Engine，便找出 **SQL Server CEIP 服務*執行個體名稱***。若為 Analysis Services，找出 **SQL Server Analysis Services CEIP *執行個體名稱** *。 
...
> 注意︰請在 SQL Server 安裝路徑以外設定本機稽核的目錄路徑，以避免允許稽核功能和修補作業造成潛在的 SQL Server 問題。
  ...
  ||設計決策|建議|  
  |------|-----------------|----------|  
  |![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|空間可用性 |針對大約 10 個資料庫的一般工作負載，請規劃每天每個執行個體大約 2 MB 的磁碟空間。|  
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|個別的目錄 | 為每個執行個體建立目錄。 例如，針對名為 `MSSQLSERVER` 的 SQL Server 執行個體，使用 *C:\\SQLCEIPAudit\\MSSQLSERVER\\DB\\*。 這可簡化檔案管理。
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|個別的資料夾 |針對每個服務使用特定的資料夾。 例如，針對給定的執行個體名稱，為資料庫引擎使用一個資料夾。 如果 SSAS 執行個體使用相同的執行個體名稱，請為 SSAS 建立個別的資料夾。 Database Engine 和 Analysis Services 執行個體設定為相同的資料夾將會造成所有本機稽核從兩個執行個體寫入相同的記錄檔。| 
|![Checkbox](../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|授與 SQL Server CIEP 遙測服務登入帳戶的權限|啟用 [列出資料夾內容]、[讀取] 和 [寫入] 存取給 SQL Server CEIP 遙測服務登入帳戶|


### 授與 SQL Server CIEP 遙測服務登入帳戶的權限
...
1. 瀏覽至適當的 CPE 路徑。 
    ...
    若為 Database Engine，使用 *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL13.\<INSTANCENAME\>\\CPE*。 
    
    若為 Analysis Services，使用 *HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Microsoft SQL Server\\MSSQL13.\<INSTANCENAME\>\\CPE*。
