...
# <a name="scm-services---change-the-password-of-the-accounts-used"></a>SCM 服務 - 變更所使用帳戶的密碼
 ...
 若要變更 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 驗證的密碼，請參閱 [密碼已過期](http://msdn.microsoft.com/library/9831b194-9ad5-47b0-8009-59c7aef4319b)。  
...
3.  在詳細資料窗格中，以滑鼠右鍵按一下 SQL Server (\<執行個體名稱>)，然後按一下屬性。  
  
4.  在 SQL Server (\<執行個體名稱>) 屬性 對話方塊的 登入 索引標籤上，針對列於 帳戶名稱 方塊的帳戶，在 密碼 和 確認密碼 方塊中鍵入新密碼，然後按一下確定。  
...
3.  在詳細資料窗格中，以滑鼠右鍵按一下 SQL Server Agent (\<執行個體名稱>)，然後按一下屬性。  
  
4.  在 SQL Server Agent (\<執行個體名稱>) 屬性 對話方塊的 登入 索引標籤上，針對列於 帳戶名稱 方塊的帳戶，在 密碼 和 確認密碼 方塊中鍵入新密碼，然後按一下確定。  
