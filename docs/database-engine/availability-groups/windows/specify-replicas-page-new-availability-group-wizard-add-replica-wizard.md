...
|索引標籤|簡短描述|  
|---------|-----------------------|  
|[複本](#ReplicasTab)|使用此索引標籤可指定將裝載或目前裝載次要複本的每個 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 執行個體。 請注意，您目前所連接的伺服器執行個體必須裝載主要複本。<br /><br /> 在 **[複本]** 索引標籤上完成指定所有複本，然後啟動其他索引標籤。|  
 ...
 **端點 URL**  
 顯示資料庫鏡像端點的實際或建議 URL。 若是建議的新端點，您可以變更此值。 如需這些 URL 格式的相關資訊，請參閱[在加入或修改可用性複本時指定端點 URL &#40;SQL Server&#41;](../../../database-engine/availability-groups/windows/specify endpoint url - adding or modifying availability replica.md)。  
