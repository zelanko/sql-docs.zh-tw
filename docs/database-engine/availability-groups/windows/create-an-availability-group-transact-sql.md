...
|準備次要資料庫|[BACKUP](../../../t-sql/statements/backup-transact-sql.md) 和 [RESTORE](../Topic/RESTORE%20\(Transact-SQL\).md)。|在裝載主要複本的伺服器執行個體上建立備份。<br /><br /> 使用 RESTORE WITH NORECOVERY，還原裝載次要複本之每個伺服器執行個體上的備份。|  
|將每一個次要資料庫加入可用性群組來啟動資料同步處理。|[ALTER DATABASE](../Topic/ALTER%20DATABASE%20SET%20HADR%20\(Transact-SQL\).md) *database_name* SET HADR AVAILABILITY GROUP = *群組名稱*|在裝載次要複本的每一個伺服器執行個體上執行。|  
...
1.  在您打算建立可用性群組的伺服器執行個體上 (這是 `COMPUTER01` 上名為 `AgHostInstance` 的執行個體)，建立名為 *dbm_endpoint* 的資料庫鏡像端點。 此端點使用通訊埠 7022。 請注意，可用性群組建立所在的伺服器執行個體將會裝載主要複本。  
...
2.  在要裝載次要複本的伺服器執行個體上 (這是 `COMPUTER02` 上的預設伺服器執行個體)，建立端點 *dbm_endpoint*。 此端點使用通訊埠 5022。  
...
-   **部落格：**  
     ...
     [Always On - HADRON Learning Series: Worker Pool Usage for HADRON Enabled Databases (AlwaysON - HADRON 學習系列：啟用 HADRON 之資料庫的背景工作集區使用方式)](http://blogs.msdn.com/b/psssql/archive/2012/05/17/Always%20On-hadron-learning-series-worker-pool-usage-for-hadron-enabled-databases.aspx)  
