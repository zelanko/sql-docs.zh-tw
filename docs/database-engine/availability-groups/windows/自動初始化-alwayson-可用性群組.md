...
manager: "jhubbard"
---
# 自動初始化 AlwaysOn 可用性群組
[!INCLUDE[tsql-appliesto-ss2016-xxxx-xxxx-xxx_md](../../../includes/tsql-appliesto-ss2016-xxxx-xxxx-xxx-md.md)]


 SQL Server 2016 引進對可用性群組進行自動植入。 當您使用自動植入建立可用性群組時，SQL Server 會自動為群組中的每個資料庫建立次要複本。 有了自動植入，您不再需要手動備份與還原次要複本。 若要啟用自動植入，請使用 T-SQL 建立可用性群組。
 
## 必要條件

自動植入要求參與可用性群組之每個 SQL Server 執行個體上的資料和記錄檔案路徑都必須相同。 
...
1. 建立端點。 每部伺服器都需要一個端點。 下列指令碼會建立針對接聽程式使用 TCP 通訊埠 5022 的端點。 設定 `<endpoint_name>` 和 `LISTENER_PORT` 以符合您的環境，然後執行下列指令碼：
...
1. 建立可用性群組。 下列指令碼會建立可用性群組。 更新的群組名稱、伺服器名稱和網域名稱的值，並在 SQL Server 的主要執行個體上執行。  
...
1. 將次要伺服器聯結至可用性群組，並將建立資料庫的權限授與可用性群組。 在 SQL Server 的次要執行個體上執行下列指令碼： 
...
您可以在現有的資料庫上設定自動植入。 下列命令會變更可用性群組以使用自動植入。 
...
這會強制資料庫視需要重新啟動植入。 例如，如果由於次要複本上的磁碟空間不足而導致植入失敗，您可以在增加可用空間之後，執行 `ALTER AVAILABILITY GROUP ... WITH (SEEDING_MODE=AUTOMATIC)` 以重新啟動植入。

## 停止自動植入

若要停止對可用性群組進行自動植入，請在裝載主要複本的執行個體上執行下列指令碼：
...
這會取消目前正在植入的任何複本，並防止 SQL Server 自動初始化此可用性群組中的任何複本。 但不會停止已初始化之任何複本的同步處理。 
