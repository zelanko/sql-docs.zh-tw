...
-   **開始之前：**  
     ...
     [建議閱讀資料](#RecommendedReading)  
  
-   **使用者入門：**  
  
     [設定 SQL Server 執行個體，以支援 AlwaysOn 可用性群組](#ConfigSI)  
  
     [建立及設定新的可用性群組](#ConfigAG)  
  
     [管理可用性群組、複本和資料庫](#ManageAGsEtc)  
  
     [監視可用性群組](#MonitorAGsEtc)  
...
|![Checkbox](../../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|**將接聽程式的 DNS 主機名稱提供給應用程式開發人員。**  開發人員需要在連接字串中指定這個 DNS 名稱，以便將連線要求導向可用性群組接聽程式。 如需詳細資訊，請參閱[可用性群組接聽程式、用戶端連線及應用程式容錯移轉 &#40;SQL Server&#41;](../../../database-engine/availability-groups/windows/listeners, client connectivity, application failover.md)。|[建立或設定可用性群組接聽程式 &#40;SQL Server&#41;](../../../database-engine/availability-groups/windows/create-or-configure-an-availability-group-listener-sql-server.md) 中的＜後續操作：建立可用性群組接聽程式之後＞|  
|![Checkbox](../../../database-engine/availability-groups/windows/media/checkboxemptycenterxtraspacetopandright.gif "Checkbox")|**設定執行備份作業的位置。**  如果您要在次要資料庫上執行備份，則必須建立備份作業指令碼，以便將自動備份喜好設定納入考量。 在裝載可用性群組之可用性複本的每一個伺服器執行個體上，為可用性群組中的每個資料庫建立指令碼。|[設定可用性複本的備份 &#40;SQL Server&#41;](../../../database-engine/availability-groups/windows/configure-backup-on-availability-replicas-sql-server.md) 中的＜後續操作：設定次要複本的備份之後＞|  
...
|加入或移除資料庫。|[加入資料庫](../../../database-engine/availability-groups/windows/add-a-database-to-an-availability-group-sql-server.md)<br /><br /> [移除次要資料庫](../../../database-engine/availability-groups/windows/remove-a-secondary-database-from-an-availability-group-sql-server.md)<br /><br /> [移除主要資料庫](../../../database-engine/availability-groups/windows/remove-a-primary-database-from-an-availability-group-sql-server.md)|  
...
-   **視訊 - AlwaysOn 簡介：**[Microsoft SQL Server Code-Named "Denali" AlwaysOn Series,Part 1: Introducing the Next Generation High Availability Solution](http://channel9.msdn.com/Events/TechEd/NorthAmerica/2011/DBI302) (Microsoft SQL Server 名稱代碼 "Denali" AlwaysOn 系列第一部分：新一代高可用性解決方案簡介)  
  
-   **視訊 - 深入探討 AlwaysOn：**[Microsoft SQL Server Code-Named "Denali" AlwaysOn Series,Part 2: Building a Mission-Critical High Availability Solution Using AlwaysOn](http://channel9.msdn.com/Events/TechEd/NorthAmerica/2011/DBI404) (Microsoft SQL Server 名稱代碼 "Denali" AlwaysOn 系列第二部分：使用 AlwaysOn 建立任務關鍵性高可用性解決方案)  
  
-   **技術白皮書：**[Microsoft SQL Server AlwaysOn Solutions Guide for High Availability and Disaster Recovery](http://go.microsoft.com/fwlink/?LinkId=227600) (Microsoft SQL Server AlwaysOn 高可用性和災害復原解決方案指南)  
  
-   **部落格：**[SQL Server AlwaysOn 團隊部落格：官方 SQL Server AlwaysOn 團隊部落格](http://blogs.msdn.com/b/sqlAlways%20On/)  
