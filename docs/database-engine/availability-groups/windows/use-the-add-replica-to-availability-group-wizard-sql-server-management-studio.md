---
title: "使用 [將複本加入至可用性群組] 精靈 (SQL Server Management Studio) | Microsoft Docs"
ms.custom: ""
ms.date: "05/17/2016"
...
---
# 使用 [將複本加入至可用性群組] 精靈 (SQL Server Management Studio)
  使用 [Add Replica to Availability Group Wizard (將複本加入可用性群組精靈)]，可協助您將新次要複本加入至現有的 AlwaysOn 可用性群組。  
...
-   **使用下列方法加入複本：**[將複本加入至可用性群組精靈 (SQL Server Management Studio)](#SSMSProcedure)  
...
7.  在 **[選取初始資料同步處理]** 頁面上，選擇您要如何建立新的次要資料庫並將它聯結至可用性群組。 選擇下列其中一個選項：  
    ...
    -   **Full**  
         ...
         在 [指定所有複本皆可存取的共用網路位置:] 欄位中，指定裝載複本之所有伺服器執行個體都有讀寫存取權限的備份共用。 記錄備份將是記錄備份鏈結的一部分。 請適當地儲存記錄備份檔案。  
...
8.  **[驗證]** 頁面會驗證您在此精靈中指定的值是否符合 [將複本加入至可用性群組] 精靈的需求。 若要進行變更，您可以按 **[上一步]** 返回先前的精靈頁面，以變更一個或多個值。 然後，按一下 [下一步] 返回 [驗證] 頁面，再按一下 [重新執行驗證]。  
...
10. [進度] 頁面會顯示建立可用性群組的步驟進度 (設定端點、建立可用性群組，並將次要複本加入群組中)。  
...
> [!IMPORTANT]  
>  加入複本之後，請參閱[將次要複本加入至可用性群組 &#40;SQL Server&#41;](../../../database-engine/availability-groups/windows/add-a-secondary-replica-to-an-availability-group-sql-server.md) 中的＜待處理：加入複本之後＞一節。  
