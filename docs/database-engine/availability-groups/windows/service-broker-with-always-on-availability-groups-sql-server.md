...
2.  **確定 Service Broker 端點存在而且設定正確。**  
    ...
    -   將所有主機伺服器執行個體的 Service Broker PORT 設定為相同的通訊埠編號。  
        ...
        > [!TIP]  
        >  若要檢視指定伺服器執行個體之 Service Broker 端點的通訊埠編號，請查詢 [sys.tcp_endpoints](../../../relational-databases/system-catalog-views/sys-tcp-endpoints-transact-sql.md) 目錄檢視的 **port** 資料行，其中 **type_desc** = 'SERVICE_BROKER'。  
...
4.  **確定 msdb 包含 AutoCreatedLocal 路由或特定服務的路由。**  
    ...
    >  根據預設，每個使用者資料庫 (包括 **msdb**) 都包含 **AutoCreatedLocal**路由。 此路由會比對所有服務名稱和 Broker 執行個體，並指定訊息應在目前執行個體內傳遞。 **AutoCreatedLocal** 的優先權低於明確指定與遠端執行個體通訊之特定服務的路由。  
     ...
     如需建立路由的相關資訊，請參閱 [Service Broker 路由範例](http://msdn.microsoft.com/library/ms166090\(SQL.105\).aspx) (在 [!INCLUDE[ssKilimanjaro](../../../includes/sskilimanjaro-md.md)] 版本的《線上叢書》中) 和 [CREATE ROUTE &#40;Transact-SQL&#41;](../../../t-sql/statements/create-route-transact-sql.md)。  
...
1.  **建立目標服務的路由。**  
    ...
    ```  
     ...
     如需詳細資訊，請參閱 [CREATE ROUTE &#40;Transact-SQL&#41;](../../../t-sql/statements/create-route-transact-sql.md)。  
