...
####  <a name="scopedconfiguration"></a> 資料庫範圍的組態
...
####  <a name="QueryOptimizer"></a> 查詢最佳化工具
...
####  <a name="StripedBackupToAzure"></a> Microsoft Azure Blob 儲存體的等量備份
...
####  <a name="FileSnapshotBackup"></a> Microsoft Azure Blob 儲存體的檔案快照備份
...
####  <a name="multipleTempDB"></a> TempDB 資料庫
 TempDB 有數個增強功能︰
...
####  <a name="DefaultDB"></a> 新的預設資料庫大小和自動成長值
...
###  <a name="AES"></a> 端點的 AES 加密
- 端點的預設加密已從 RC4 變成 AES。

####  <a name="newcredentialtype"></a> 新的認證類型
- 現在可以在資料庫層級建立認證 (除了先前可用的伺服器層級認證以外)。 如需詳細資訊，請參閱 [CREATE DATABASE SCOPED CREDENTIAL &#40;Transact-SQL&#41;](../t-sql/statements/create-database-scoped-credential-transact-sql.md)。


###  <a name="HighAvailability"></a> 高可用性增強功能
