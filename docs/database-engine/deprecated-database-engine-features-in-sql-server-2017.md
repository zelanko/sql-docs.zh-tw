...
|雜湊演算法|MD2、MD4、MD5、SHA 和 SHA1 演算法。 不應該使用這些。|使用 SHA2_256 或 SHA2_512。|已被取代的雜湊演算法||  
...
|[!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]|方案總管整合於 [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]||None|None|  
...
|追蹤管理物件|Microsoft.SqlServer.Management.Trace 命名空間 (包含 SQL Server 追蹤和重新執行物件的 API)|追蹤設定︰ <xref:Microsoft.SqlServer.Management.XEvent><br /><br /> 追蹤讀取︰ <xref:Microsoft.SqlServer.XEvent.Linq><br /><br /> 追蹤重新執行：None|||  
