...
description: 使用這個唯讀的資料庫鏡像監視器頁面，即可檢視所選資料庫之主體和鏡像伺服器執行個體的鏡像狀態。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
 ...
 **主體記錄 (** *\<time>* **)**  
 主體伺服器執行個體上記錄的狀態，以伺服器執行個體的當地時間為準，此時間會以 *\<time>* 表示。 會顯示下列參數：  
 ...
 **鏡像記錄 (** *\<time>* **)**  
 鏡像伺服器執行個體上的記錄狀態，以伺服器執行個體的當地時間為準，此時間會以 *\<time>* 表示。 會顯示下列參數：  
...
## <a name="remarks"></a>備註  
 **dbm_monitor** 固定資料庫角色的成員可以使用「資料庫鏡像監視器」或 **sp_dbmmonitorresults** 預存程序，檢視現有的鏡像狀態。 但是這些使用者無法更新狀態資料表。 它們必須透過 [資料庫鏡像監視器作業] 來定期更新狀態資料表。 若要了解所顯示狀態的存在時間，使用者可以查看 [主體記錄 ( _\<time>_ )] 和 [鏡像記錄 ( _\<time>_ )] 標籤中的時間。  
