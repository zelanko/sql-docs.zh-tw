...
##  <a name="1-create-a-table-report-and-dataset-from-the-table-or-matrix-wizard"></a><a name="Table"></a>1. 從資料表或矩陣 Wizard 建立資料表報表和資料集  
 從 [**消費者入門**] 對話方塊中，選擇共用資料來源、建立內嵌的資料集，並將資料顯示在資料表中。  
...
6.  在 [**選擇與資料來源的連接**] 頁面上，選取現有的資料來源，或流覽至報表伺服器並選取資料來源。 如果沒有資料來源可用，或無法存取報表伺服器，您可以改用內嵌資料來源。 如需詳細資訊，請參閱[教學課程：建立基本資料表報表 &#40;報表產生器&#41;](../reporting-services/tutorial-creating-a-basic-table-report-report-builder.md)。  
...
1.  在資料表中，以滑鼠右鍵按一下資料格下方的`[Sum(Sales)]`兩個數據格（顯示子類別銷售額的小計資料列），然後按一下 [**文字方塊屬性**]。  
  
2.  在 [**填滿**] 中，按一下 [**填滿色彩**] 選項旁邊的**fx**按鈕，然後在 [**設定 expression for： BackgroundColor** ] 欄位中輸入下列運算式：  
...
2.  在資料表中，以滑鼠右鍵按一下您在上一個程式中變更之資料格的資料行處理常式，指向 [**插入資料行**]，然後按一下 [**右**]。 新的資料行就會加入至此資料表。  
  
3.  在資料行標題中輸入**KPI** 。  
  
4.  在 [**插入**] 索引標籤的 [**資料區**] 群組中，按一下 [量測計]，**然後按一下資料表**外部的設計介面。 **[選取量測計類型]** 對話方塊隨即出現。  
  
5.  按一下 [**線性**]。 會選取第一個線性量測計類型 [**水準**]。  
...
9. 以滑鼠右鍵按一下量測計中的指標，然後按一下 [**指標屬性**]。  
  
10. 在 [**指標類型**] 中，選取 [**橫條**]。 這樣就會將指標從標記變更為橫條，以便在量測計加入至資料表時更明顯。  
  
11. 按一下 [**指標填滿**]。 在 [**次要色彩**] 中，選擇 [**黃色**]。 漸層填滿模式將從白色變更為黃色。  
...
16. (選擇性) 加入最大指針來處理溢位，使任何超過刻度最大值的值都永遠會指向最大指針：  
    ...
    3.  在 [**縮放 pin** ] 分類中，展開 [ **MaximumPin** ] 節點。  
  
    4.  將**Enable**屬性設定為`True`。 指針會顯示在標尺的最大值之後。  
...
5.  在 [**插入**] 索引標籤的 [**資料區**] 群組中，按兩下 [**指標]。**  
  
     [選取指標類型]**** 對話方塊隨即開啟。  
  
6.  按一下 [**圖形**]。 第一個圖形類型 [ **3 個流量燈（無框）** ] 已選取。  
...
10. 在 [值] 下拉式清單中，選取 **[Sum （Sales）]**，但不要變更任何其他選項。  
...
3.  （選擇性）以滑鼠右鍵按一下包含**產品銷售 KPI**的文字方塊，然後按一下 [**文字方塊屬性**]，再于 [字型] 索引標籤上選取不同的字型樣式、大小和色彩。  
...
## <a name="next-steps"></a>後續步驟  
 您已成功完成「將 KPI 加入至報表」教學課程。 如需詳細資訊，請參閱量測計（報表產生器）指標[&#40;報表產生器和 SSRS&#41;](report-design/indicators-report-builder-and-ssrs.md)。  
