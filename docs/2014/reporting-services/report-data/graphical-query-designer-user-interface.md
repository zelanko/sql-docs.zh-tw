...
-   **文字**支援關係[!INCLUDE[tsql](../../../includes/tsql-md.md)]資料庫資料來源的標準查詢文字，包括和 Oracle 的[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]資料處理延伸模組。  
  
-   **TableDirect**從指定的資料表中選取所有資料行。 例如，如果是名為 Customers 的資料表，這就等於 [!INCLUDE[tsql](../../../includes/tsql-md.md)] 陳述式 `SELECT * FROM Customers`。  
  
-   **StoredProcedure**支援對資料來源上預存程式的呼叫。 若要使用這個選項，您必須已經獲得資料來源的資料庫管理員授與對預存程序的 Execute 權限。  
...
|按鈕|描述|  
|------------|-----------------|  
|**當做文字編輯**|在以文字為基礎的查詢設計工具和圖形化查詢設計工具之間切換。|  
...
|![以 SQL 窗格按鈕確認 SQL](../media/rsqdicon-verifysql.gif "以 SQL 窗格按鈕確認 SQL")|檢查查詢文字的語法是否正確。|  
...
|![移除選取欄位上的篩選](../media/rsqdicon-removefilter.gif "移除選取欄位上的篩選")|針對 [圖表] 窗格中標示為有篩選（[![選取的篩選資料行] 旁的篩選圖形](../media/rsqdicon-filter.gif "在選取篩選條件資料行旁邊的篩選圖形")）的選取資料行，移除其篩選。|  
...
|![在圖表窗格中加入新資料表](../media/rsqdicon-addtable.gif "在圖表窗格中加入新資料表")|將資料來源中的新資料表加入至 [圖表] 窗格中。<br /><br /> **注意**當您加入新的資料表時，查詢設計工具會嘗試比對資料來源中的外鍵關聯性。 因此，在加入資料表之後，請確認以資料表之間的連結所表示的外部索引鍵關聯性是否正確。|  
...
## <a name="see-also"></a>另請參閱  
 ...
 [SQL Server &#40;SSRS&#41;的連線類型](sql-server-connection-type-ssrs.md)   
 [OLE DB &#40;SSRS&#41;的連線類型](ole-db-connection-type-ssrs.md)   
 [將資料加入報表 &#40;報表產生器和 SSRS&#41;](report-datasets-ssrs.md)   
