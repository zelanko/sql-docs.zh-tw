...
-   針對 SAP BW 3.5 和 7.0 支援 Package 20  
  
 對於 Windows 整合式驗證，提供者是針對下列系統所開發並測試。  
  
-   SAP Portals 6.40 支援 Package 20  
  
-   SAP 入口網站 7.0 支援封裝 11  
...
-   在設計模式中，將維度、成員、成員屬性以及關鍵數值拖曳到 [查詢] 窗格，或者直接在 [查詢] 窗格中輸入 MDX 文字。 請從 [導出成員] 窗格中將導出成員拖曳到 [資料] 窗格，以定義其他資料集欄位。  
 ...
 如需使用這個查詢設計工具的詳細資訊，請參閱 [SAP NetWeaver BI 查詢設計工具使用者介面 &#40;報表產生器&#41;](../sap-netweaver-bi-query-designer-user-interface-report-builder.md)。  
...
|`UniqueName`|`String`|傳回層級的完整名稱。 例如，`UniqueName`值可能是員工 *[0D_Company]。 [10D_Department]。[11]*.|  
  
 如需在運算式中使用欄位及欄位屬性的詳細資訊，請參閱[運算式中的內建集合 &#40;報表產生器及 SSRS&#41;](../report-design/built-in-collections-in-expressions-report-builder.md)。  
