...
ms.locfileid: "36030957"
---
# <a name="sql-server-parallel-data-warehouse-connection-type-ssrs"></a>SQL Server Parallel Data Warehouse 連接類型 (SSRS)
  [!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssDWCurrentFull](../../../includes/ssdwcurrentfull-md.md)] 是可擴充的資料倉儲應用裝置，提供效能和延展性，透過大量平行處理。 [!INCLUDE[ssDW](../../../includes/ssdw-md.md)] 使用[!INCLUDE[ssCurrent](../../../includes/sscurrent-md.md)]分散式處理和資料的儲存體的資料庫。  
 ...
 若要加入來自[!INCLUDE[ssDW](../../../includes/ssdw-md.md)]在報表中，您必須擁有類型之報表資料來源為基礎的資料集[!INCLUDE[msCoName](../../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Parallel Data Warehouse。 此內建資料來源類型根據[!INCLUDE[msCoName](../../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]平行處理資料倉儲資料延伸模組。 使用此資料來源類型可連接至 [!INCLUDE[ssDW](../../../includes/ssdw-md.md)] 並從中擷取資料。  
  
 此資料延伸模組支援多值參數、伺服器彙總，以及與連接字串分開管理的認證。  
  
 如需詳細資訊，請參閱 [SQL Server 2008 R2 平行資料倉儲](http://go.microsoft.com/fwlink/?LinkId=150895)網站。  
...
##  <a name="Connection"></a> 連接字串  
 當您連接到 [!INCLUDE[ssDW](../../../includes/ssdw-md.md)]時，是連接到 [!INCLUDE[ssDW](../../../includes/ssdw-md.md)] 應用裝置內的資料庫物件。 您會指定在查詢設計工具中使用的資料庫物件。 如果您未在連接字串中指定資料庫，則會連接到管理員指派給您的預設資料庫。 請洽詢資料庫管理員，以取得用來連接資料來源的連接資訊和認證。 下列連接字串範例指定範例資料庫， **CustomerSales**，請在[!INCLUDE[ssDW](../../../includes/ssdw-md.md)]應用裝置：  
  
```  
HOST=<IP address>; database= CustomerSales; port=<port>  
```  
  
 此外，您會使用 **[資料來源屬性]** 對話方塊來提供使用者和密碼等認證。 `User Id` 和 `Password` 選項會自動附加到連接字串，您不必隨連接字串一起輸入它們。 使用者介面也提供選項來指定中控制節點的 IP 位址[!INCLUDE[ssDW](../../../includes/ssdw-md.md)]設備與連接埠號碼。 通訊埠依預設為 17000。 管理員可設定此通訊埠，而您的連接字串可能會使用不同的通訊埠編號。  
...
-   輸入或貼上查詢。 使用以文字為基礎的查詢設計工具來輸入[!INCLUDE[DWsql](../../../includes/dwsql-md.md)]直接貼上從其他來源的輸入不能利用關聯式查詢設計工具中，建立複雜查詢，或是輸入以查詢為基礎的運算式的查詢文字的文字。  
...
-   [文字](#QueryText)  
 ...
 [!INCLUDE[DWsql](../../../includes/dwsql-md.md)] 支援子句、 關鍵字和彙總的查詢設計工具提供用於摘要資料。  
  
 [!INCLUDE[ssDW](../../../includes/ssdw-md.md)] 使用的圖形化查詢設計工具提供群組和彙總的內建支援，可協助您撰寫只擷取摘要資料的查詢。 [!INCLUDE[DWsql](../../../includes/dwsql-md.md)]語言功能包括： GROUP BY 子句、 DISTINCT 關鍵字以及如 SUM 和 COUNT 等彙總。 以文字為基礎的查詢設計工具提供的完整支援[!INCLUDE[DWsql](../../../includes/dwsql-md.md)]語言，包括群組和彙總。  
...
###  <a name="QueryText"></a> 使用 Text 查詢類型  
 在以文字為基礎的查詢設計工具中，您可以輸入 [!INCLUDE[DWsql](../../../includes/dwsql-md.md)] 命令來定義資料集內的資料。 您用來擷取資料的查詢[!INCLUDE[ssDW](../../../includes/ssdw-md.md)]是您用來擷取資料的執行個體從相同[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]內未執行[!INCLUDE[ssDW](../../../includes/ssdw-md.md)]應用程式。 例如，下列[!INCLUDE[DWsql](../../../includes/dwsql-md.md)]查詢會選取所有行銷助理員工的名字：  
...
```  
 ...
 根據預設，每個報表參數的資料類型為 Text，並具有自動建立的資料集，以提供可用值的下拉式清單。 建立報表參數後，您可能必須變更預設值。 如需詳細資訊，請參閱[報表參數 &#40;報表產生器和報表設計師&#41;](../report-design/report-parameters-report-builder-and-report-designer.md)。  
