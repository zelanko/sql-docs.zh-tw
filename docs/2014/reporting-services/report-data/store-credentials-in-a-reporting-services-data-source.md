...
##  <a name="bkmk_top"></a>預存認證的安全性原則需求  
 ![as_powerpivot_refresh_sss_set_key](https://docs.microsoft.com/analysis-services/analysis-services/media/as-powerpivot-refresh-sss-set-key.gif "as_powerpivot_refresh_sss_set_key")您在報表伺服器上針對下列其中一個安全性原則設定了您用於預存認證的帳戶，這是必要的。 建議您為您的環境選取具備需要的最低層級權限的原則。  
  
1.  **允許本機登**入。 如需詳細資訊，請參閱 [允許本機登入](https://technet.microsoft.com/library/cc756809\(v=WS.10\).aspx)。  
  
2.  **以批次工作登**入。 如需詳細資訊，請參閱 [以批次工作登入](https://technet.microsoft.com/library/cc755659\(v=ws.10\).aspx)。  
...
8.  按一下 [套用]  。  
     ...
     [針對預存認證](#bkmk_top)![使用的箭號圖示搭配 [回到頁首] 連結](../../2014-toc/media/uparrow16x16.gif "與 [回到頁首] 連結搭配使用的箭頭圖示")安全性原則需求  
  
##  <a name="bkmk_stored_credentials_data_source_sharepoint"></a>為報表特定的資料來源設定預存認證（SharePoint 模式）  
  
1.  流覽至包含報表的文件庫，然後按一下 [開啟] 功能表![文件庫 ssrs 專案的內容功能表](../media/ssrs-sharepoint-item-context-menu.png "適用於 SSRS 項目的文件庫內容功能表")。  
  
2.  按一下第二個開啟功能表 [![文件庫] [ssrs 專案] 內容功能表](../media/ssrs-sharepoint-item-context-menu.png "適用於 SSRS 項目的文件庫內容功能表")，然後按一下 [**管理資料來源**]。  
...
##  <a name="bkmk_stored_credentials_shared_data_source_native"></a>設定共用資料來源的預存認證（原生模式）  
...
4.  針對 [**連接字串**]，請指定報表伺服器用來連接到資料來源的連接字串。 
  [!INCLUDE[msCoName](../../../includes/msconame-md.md)] 建議您不要在連接字串中指定認證。  
...
##  <a name="bkmk_stored_credentials_shared_data_source_sharepoint"></a>設定共用資料來源的預存認證（SharePoint 模式）  
  
1.  在文件庫中，流覽至共用資料來源專案。![共用資料來源圖示](../media/hlp-16datasource.png "共用資料來源圖示")  
  
2.  針對 ssrs 專案按一下內容功能表![文件庫操作功能表](../media/ssrs-sharepoint-item-context-menu.png "適用於 SSRS 項目的文件庫內容功能表")，然後按一下第二個內容功能表 [檔程式庫] 內容![功能表中的 ssrs 專案](../media/ssrs-sharepoint-item-context-menu.png "適用於 SSRS 項目的文件庫內容功能表")。  
  
3.  按一下 [編輯資料來源定義] ****。  
  
4.  在 [**資料來源類型**] 清單中，指定用來處理資料來源中之資料的資料處理延伸模組。  
  
5.  針對 [**連接字串**]，請指定報表伺服器用來連接到資料來源的連接字串。 
  [!INCLUDE[msCoName](../../../includes/msconame-md.md)] 建議您不要在連接字串中指定認證。  
...
## <a name="see-also"></a>另請參閱  
 [指定報表資料來源的認證和連接資訊](../../integration-services/connection-manager/data-sources.md)   
