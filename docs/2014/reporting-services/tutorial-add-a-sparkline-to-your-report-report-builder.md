...
# <a name="tutorial-add-a-sparkline-to-your-report-report-builder"></a>教學課程：將走勢圖加入至報表 (報表產生器)
 ...
 本教學課程所建立的報表另有一個增強型版本，可從範例 [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] 報表產生器報表取得。 如需下載此範例報表及其他專案的詳細資訊，請參閱[報表產生器範例報表](https://go.microsoft.com/fwlink/?LinkId=184851)。 下圖顯示與您將要建立的報表相似的範例報表。  
  
 ![rs_SparklineMatrixTutorial](../../2014/tutorials/media/rs-sparklinematrixtutorial.gif "rs_SparklineMatrixTutorial")  
  
 影片[如何：在資料表中建立走勢圖（報表產生器影片）](https://technet.microsoft.com/bi/ff871942.aspx)說明如何建立包含走勢圖的類似報表。  
...
8.  在 [連接字串]**** 中，鍵入下列文字：  
     ...
     **資料來源 =\<servername>**  
     ...
     運算式\<servername> （例如 report001)）會指定安裝 SQL Server 資料庫引擎實例的電腦。 由於報表資料不是擷取自 SQL Server 資料庫，您不必加上資料庫的名稱。 指定之伺服器上的預設資料庫將用來剖析查詢。  
...
##  <a name="2-create-a-query-in-the-table-wizard"></a><a name="Query"></a>2. 在資料表 Wizard 中建立查詢  
...
2.  按一下 [當成**文字編輯**]。 以文字為基礎的查詢設計工具會顯示查詢窗格和結果窗格。  
...
10. 在 [**選擇版面**配置] 頁面的 [**選項**] 底下，確認已選取 [**顯示小計和總計**]。  
...
4.  在新的資料行中，以滑鼠右鍵按一下 [Product] 資料列，指向 [**插入**] 功能區索引標籤，然後按一下 [走勢**圖**]。  
  
5.  請確定已選取 [資料行] 資料**列**中的第一個走勢圖，然後按一下 **[確定]**。  
  
6.  按一下走勢圖以顯示 [圖表資料] 窗格。  
  
7.  按一下 [值] 方塊中的加號（+），然後按一下 [**銷售**]。  
  
     **Sales** 欄位內的值現在即為走勢圖的值。  
  
8.  按一下 [類別目錄群組] 方塊中的加號（+），然後按一下 [ **SalesDate**]。  
  
9. 按一下 [執行]**** 以預覽報表。  
  
     請注意資料表的每一列內都有走勢圖，但是圖表並不正確。 圖表中的橫條沒有彼此切齊。 第二個資料列只有四個橫條，而第一列有六個，因此前者的橫條比後者的橫條還要寬。 這樣您無法比較各產品每日的值， 這些橫條必須彼此貼齊。  
  
     同時請注意，每一列內最高的橫條都和該列等高。 這也會造成誤導，因為每個資料列的最大值不相等：預算 Movie Maker 的最大值為 $10400，但超薄數位的最大值為 $26576-超過兩倍以上。 然而，這兩列內最大值橫條的高度幾乎一樣， 這些橫條需要能夠隨著其他走勢圖縮放。  
  
     ![rs_SprklineMtrxUnaligndBars](../../2014/tutorials/media/rs-sprklinemtrxunaligndbars.gif "rs_SprklineMtrxUnaligndBars")  
  
##  <a name="4-align-the-sparklines-vertically-and-horizontally"></a><a name="AlignSparklines"></a>4. 垂直與水準對齊走勢圖  
 當走勢圖不全都使用相同的量值時，就難以閱讀走勢圖。 每一個走勢圖的水平和垂直軸都需要符合其餘的走勢圖。  
...
##  <a name="5-optional-format-data-as-currency"></a><a name="FormatCurrency"></a>5. （選擇性）將資料格式化為貨幣  
 根據預設，[ **Sales** ] 欄位的摘要資料會顯示一般數位。 格式化該欄位，將數字顯示為貨幣。 切換 [預留位置樣式]****，將格式化的文字方塊和預留位置文字顯示為範例值。  
  
#### <a name="to-format-a-currency-field"></a>格式化貨幣欄位  
  
1.  按一下 [**設計**]，切換至設計檢視。  
  
2.  在 [ **SalesDate** ] 資料行中，按一下第二個數據列中的資料格（在資料行標題資料列底下`[Sum(Sales)]`），然後拖曳以選取包含的所有資料格。  
...
##  <a name="6-optional-format-data-as-dates"></a><a name="FormatDates"></a>6. （選擇性）將資料格式化為日期  
...
3.  指向資料行控點之間的線條，使游標變成雙箭頭。 將資料行拖曳到所需的大小。 例如，展開 [**產品**] 的資料行，讓產品名稱顯示在一行上。  
