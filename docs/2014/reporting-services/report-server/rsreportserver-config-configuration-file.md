...
-   [一般組態設定 (rsreportserver.config)](#bkmk_generalconfiguration)  
  
-   [URLReservations （RSReportServer.config 檔）](#bkmk_URLReservations)  
  
-   [Authentication （RSReportServer.config 檔）](#bkmk_Authentication)  
  
-   [Service （RSReportServer.config 檔）](#bkmk_service)  
  
-   [傳遞 UI 延伸模組一般組態](#bkmk_ui)  
  
-   [延伸模組 （RSReportServer.config 檔） 原生模式](#bkmk_extensions)  
  
    -   [傳遞延伸模組一般組態](#bkmk_extensionsgeneral)  
  
        -   [檔案共用傳遞延伸模組組態設定](#bkmk_fileshare_extension)  
  
        -   [報表伺服器電子郵件延伸模組組態設定](#bkmk_email_extension)  
  
        -   [報表伺服器 SharePoint 文件庫延伸模組組態](#bkmk_documentlibrary_extension)  
  
        -   [NULL 傳遞延伸模組組態](#bkmk_null_extension)  
  
    -   [傳遞 UI 延伸模組一般組態](#bkmk_ui)  
  
    -   [轉譯延伸模組一般組態](#bkmk_rendering)  
  
    -   [資料延伸模組一般組態](#bkmk_data)  
  
    -   [語意查詢延伸模組一般組態](#bkmk_semantic)  
  
    -   [模型產生組態](#bkmk_model)  
  
    -   [安全性延伸模組組態](#bkmk_security)  
  
    -   [驗證延伸模組組態](#bkmk_authentication)  
...
-   [HTML 裝置資訊設定...]/../ html 裝置資訊-settings.md）  
...
|**AppID**|指定要用於 Bing Maps Web 服務的應用程式識別碼 (AppID)。 `(Default)` 會指定 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 預設 AppID。<br /><br /> 如需有關在報表中使用 Bing 地圖底圖的詳細資訊，請參閱 [其他使用規定](https://go.microsoft.com/fwlink/?LinkId=151371) 和 [隱私權聲明](https://go.microsoft.com/fwlink/?LinkId=151372)。<br /><br /> 除非您必須針對自己的 Bing Maps 授權合約指定自訂 AppID，否則請勿變更此值。 變更 AppID 時，您不需要重新啟動 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] ，即可讓變更生效。|  
