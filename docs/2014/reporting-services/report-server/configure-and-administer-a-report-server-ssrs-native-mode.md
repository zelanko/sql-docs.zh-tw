...
-   使用 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 來自訂伺服器屬性、啟用 [我的報表]、啟用追蹤記錄及設定網站範圍的預設值。 如需站台設定的詳細資訊，請參閱[Reporting Services 報表伺服器&#40;原生模式&#41;](reporting-services-report-server-native-mode.md) Management studio。 請注意，您可以建立並執行以程式設計方式設定伺服器屬性的指令碼。 如需詳細資訊，請參閱[指令碼部署和管理工作](../tools/script-deployment-and-administrative-tasks.md)和[報表伺服器系統屬性](../report-server-web-service/net-framework/reporting-services-properties-report-server-system-properties.md)。  
...
-   (選擇性) 修改組態檔來變更應用程式設定。 如需有關每個檔案和修改這些指導方針的詳細資訊，請參閱[Reporting Services 組態檔](reporting-services-configuration-files.md)。  
