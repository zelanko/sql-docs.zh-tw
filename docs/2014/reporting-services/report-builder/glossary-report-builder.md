...
|authorization|1.確認授與使用者之權限和存取權的作業。 2. 將特定資訊、服務或功能之存取權授與人員、電腦處理序或裝置的程序。|  
...
|concurrency|讓多位使用者同時存取和變更共用資料的程序。 SQL Server 利用鎖定，讓多位使用者同時存取和變更共用資料，且彼此不會發生衝突。|  
...
|schema|在 SQL-92 標準中，由單一使用者所擁有並且形成單一命名空間之資料庫物件的集合。 命名空間是一組不能有重複名稱的物件。|  
