...
|授權 (authorization)|1.確認授與使用者的存取權限與權限的作業。 2. 將特定資訊、服務或功能之存取權授與人員、電腦處理序或裝置的程序。|  
...
|批次 (batch)|1.一組 SQL 陳述式一起提交並以群組方式執行。  2. 在 Reporting Services 中，單一交易內所有 SOAP 方法呼叫的集合。|  
...
|資料來源 (data source)|1.在 ADO 和 OLE DB、 OLE DB 提供者所公開的資料來源的位置。 2. Cube 或維度之類物件的資料來源。 它也是存取來源資料所需要之資訊的規格。 它有時是指 ClassType clsDataSource 的物件。 <br />3.在 Reporting Services、 指定的資料來源類型、 連接字串和認證，可以單獨地儲存報表伺服器和報表專案之間共用或內嵌於報表定義 (.rdl) 檔案。|  
...
|資料集 (dataset)|1.在 OLE DB for OLAP 中，執行產生的多維度運算式 (MDX) SELECT 陳述式的多維度資料集。   <br />2.Reporting Services 中的具名規格，包括資料來源定義、 查詢定義和選擇性參數值、 導出的欄位，以及篩選和定序資訊做為報表定義 (.rdl) 檔的一部分。 一個 .rdl 檔案可以有多個資料集。|  
|DBCS|雙位元組字集|  
|預設|1.資料值、 選項設定、 定序，或由系統自動指派，如果使用者未指定值、 設定、 定序的名稱或名稱。 2. 另外，這也是在使用者未指定要採取之動作的情況下，在發生特定事件時，將自動採取的動作。|  
|預設資料庫 (default database)|使用者在登入 SQL Server 之後，立即連接的資料庫。|  
|預設執行個體|使用與安裝所在之電腦相同名稱的 SQL Server 執行個體。|  
|分隔符號 (delimiter)|1.TRANSACT-SQL，來指示的開始和結束的物件名稱字元中使用雙引號 ("") 或方括號 ([])。 2. 在 Integration Services 和 Reporting Services 中，用於分隔記錄、欄位或字串的字元。|  
...
|domain|1.在 Windows 安全性的電腦集合分組以檢視和系統管理用途而它們共用通用的安全資料庫。 2. 在關聯式資料庫中，資料行所允許的一組有效值。|  
|向下鑽研/向上鑽研 (drill down/drill up)|在最摘要性 (上) 到最詳細 (下) 的資料層級之間導覽。|  
|鑽研 (drill through)|1.在 Analysis Services，來擷取 cube 資料格資料的摘要詳細的資料。 2. 在 Reporting Services 中，指按一下主要鑽研報表中的超連結，開啟相關報表。|  
...
|實體 (entity)|1.在 Reporting Services 中，實體是模型項目，包括來源欄位、 角色、 資料夾與運算式，以熟悉的商務詞彙的邏輯集合。 2. 在 Master Data Services 中，實體就是模型內的物件。 每個實體都包含由屬性所定義的相關成員。|  
...
|ESRI .shp 檔 (ESRI .shp file)|Environmental Systems Research Institute，Inc.所開發的開放式規格(ESRI) 針對空間資料。|  
|ESRI 形狀檔 (ESRI Shapefile)|Environmental Systems Research Institute，Inc.所開發的開放式規格(ESRI) 針對空間資料。|  
...
|expression|1.在 SQL 中，符號和運算子會評估得出單一資料值的組合。 2. 在 Integration Services 中，會評估得出單一資料值的常值、常數、函數和運算子組合。|  
...
|filter|1.一組準則，以控制的一組記錄傳回為結果集。  2. 在全文檢索搜尋中，在給定指定的副檔名之後，從 varbinary(max) 或 image 資料行所儲存的檔案中擷取文字的篩選。|  
...
|HTML|請參閱其他詞彙：超文字標記語言 (HTML) (Hypertext Markup Language)|  
...
|IPsec|請參閱其他詞彙：網際網路通訊協定安全性 (Internet Protocol security)|  
...
|本機伺服器 (local server)|1.在 SQL Server 連接，應用程式的同一部電腦上執行的 SQL Server 的執行個體。 <br />2.在 TRANSACT-SQL 陳述式時解析參考資料庫物件，執行陳述式的 SQL Server 執行個體。 <br />3.在 SQL Server 分散式查詢中，執行分散式的查詢的 SQL Server 執行個體。 之後，本機伺服器便會存取查詢所參考的任何連結的伺服器。|  
...
|ODBC 資料來源|1.使用 ODBC 驅動程式可以存取的資料集的位置。 2. 包含 ODBC 應用程式連接到資料來源時所需要之所有連接資訊的預存定義。|  
...
|填補 (padding)|1.字串，通常會在最後一個純文字區塊很短時加入。 2. 在資料格中分配以便建立或維持特定大小的空間。|  
...
|樞紐 (pivot)|1.若要旋轉資料行及資料列，在交叉表格式資料瀏覽器中的資料行的資料列。 2. 從多維度資料結構的一組可用維度中選擇維度，以便顯示在交叉表格式結構的資料列和資料行中。|  
...
|遞迴階層 (recursive hierarchy)|1.來自單一報表資料集，其中在階層結構，例如組織階層內經理-員工關聯性的報告結構包含多個階層層級的資料階層架構。 2. 在 Master Data Services 中，包含遞迴關聯性的衍生階層。 當實體有基於實體本身的網域屬性時，就會有遞迴關聯性。|  
...
|關聯性 (relationship)|1.另一個資料表中的外部索引鍵的一個資料表中的主索引鍵的資料表之間的連結。 在資料庫圖表中，如果強制執行資料表之間的參考完整性，便用實線來表示關聯性線條，如果並未強制執行 INSERT 和 UPDATE 交易的參考完整性，便用虛線來表示。 關聯性線條的端點會顯示一個主索引鍵符號來表示主索引鍵到外部索引鍵的關聯性，它們也可能顯示一個無限符號來表示一對多關聯性的外部索引鍵端。 <br />2.在 Metadata Services 中，關聯性是一組物件，其中一個物件是原點，另一個物件是目的地之間的關聯。 每對後續物件的關聯都重複，因此，一個關聯性的目的地會成為下個關聯性的原點。 依照這個方式，關聯性鏈結會在整個資訊模型中，從一個物件延伸到下一個物件，資訊模型中的所有物件都會透過關聯性鏈結而關聯起來。|  
...
|角色 (role)|1.使用者或一組特定的權限授與對象的群組。  <br />2.在 Analysis Services 中，角色會使用 Windows 安全性帳戶來限制存取和權限的範圍，當使用者存取資料庫、 cube、 維度和資料採礦模型。  <br />3.在資料庫鏡像工作階段中，主體伺服器和鏡像伺服器執行互補的主體和鏡像角色。 另外，也可以選擇由第三個伺服器執行個體扮演見證的角色。|  
...
|資料列識別碼 (row identifier)|1.資料行或一組用來區別資料表中的資料列中的任何一個資料列的資料行中。 2. 在堆積中，資料列的指標。|  
...
|SMDL|請參閱其他詞彙：語義模型定義語言 (SMDL) (Semantic Model Definition Language (SMDL))|  
...
|SQL|請參閱其他詞彙：結構化查詢語言 (SQL) (Structured Query Language (SQL))|  
...
|資料表|1.關聯式資料庫中所建立的二維物件，其中包含資料列和資料行，會儲存實體的相關資料。 2. 在報表配置中以分欄格式顯示資料的資料區域。|  
...
|變數|1.在 Integration Services，會儲存可以在指令碼、 運算式和屬性運算式中用來設定資料行值及封裝物件屬性的值。 <br />2.已指派值的定義實體。 區域變數利用 DECLARE@localvariable 陳述式來定義，且會在利用 SELECT 或 SET@localvariable 陳述式來宣告區域變數的陳述式批次內指派初始值。|  
