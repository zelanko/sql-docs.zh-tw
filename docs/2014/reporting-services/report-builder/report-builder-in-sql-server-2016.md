...
##  <a name="JumpStartReptCreation"></a>快速入門報表建立  
...
-   **從資料表、矩陣或圖表 Wizard 開始**。 您可以選擇資料來源連接、拖放欄位以建立資料集查詢、選取配置和樣式，以及自訂報表。  
  
-   **從 [對應] 嚮導開始**，建立會針對地理或幾何背景顯示匯總資料的報表。 地圖資料可能是來自 [!INCLUDE[tsql](../../includes/tsql-md.md)] 查詢或環境系統研究協會 (Environmental Systems Research Institute, Inc.) 的空間資料。(ESRI) 形狀檔。 您也可以加入 [!INCLUDE[msCoName](../../../includes/msconame-md.md)] Bing 地圖底圖背景。  
  

  
##  <a name="DesignRept"></a>設計您的報表  
  
-   **建立含有資料表、矩陣、圖表和自由形式報表配置的報表。** 針對以資料行為基礎的資料建立資料表報表、針對摘要資料建立矩陣報表 (例如交叉分析或樞紐分析表報表)、針對地理資料建立圖表報表，以及針對其他資料建立自由形式的報表。 報表可以內嵌其他報表和圖表，連同動態網路架構應用程式的清單、圖形和控制項。  
...
-   **修改現有的報表。** 藉由使用報表產生器，您可以自訂及更新在報表設計師中[!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)]建立的報表。  
  
-   藉由篩選、分組和排序資料，或是加入公式或運算式來**修改您的資料**。  
  
-   **加入圖表、** 量測計、走勢圖和指標，以視覺化格式摘要資料，並一眼呈現大量的匯總資訊。  
  
-   **加入互動式功能**，例如檔引導模式、顯示/隱藏按鈕，以及子報表和鑽研報表的「鑽繪」連結。 您可以使用參數和篩選來篩選自訂檢視表的資料。  
  
-   **內嵌或參考影像**與其他資源，包括外部內容。  
  

  
##  <a name="ManageRpt"></a>管理您的報表  
  
-   將**報表的定義儲存**到您的電腦或報表伺服器，您可以在其中進行管理，並與其他人共用。  
  
-   當您開啟報表時，或在開啟報表之後，請**選擇呈現格式**。 您可以選取 Web 導向、頁面導向，以及桌面應用程式的格式。 格式包括 HTML、MHTML、PDF、XML、CSV、TIFF、Word 及 Excel。  
  
-   **設定訂閱。** 當您將報表發行至報表伺服器或處於 SharePoint 整合模式的報表伺服器之後，就可以將報表設定成在特定時間執行、建立報表記錄，以及設定電子郵件訂閱。  
  
-   使用 Reporting Services Atom 轉譯延伸模組，從報表**產生資料**摘要。  
