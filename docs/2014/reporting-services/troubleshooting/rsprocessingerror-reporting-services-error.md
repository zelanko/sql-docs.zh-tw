...
-   參數，可為 Null 的屬性設定為`False`偵測到 null 值的參數。  
...
### <a name="failed-to-load-expression-host-assembly"></a>無法載入運算式主機組件  
 自訂組件必須以強式名稱簽署，並設定 AllowPartiallyTrustedCallers 屬性。 如需詳細資訊，請參閱[使用 Custom Assemblies with Reports](../custom-assemblies/using-custom-assemblies-with-reports.md)和[Understanding Security Policies](../extensions/secure-development/understanding-security-policies.md)。  
...
### <a name="main-report-with-subreport-could-not-be-processed"></a>無法處理含有子報表的主報表  
 ...
 您必須變更主報表或子報表，如此所有報表才能由相同的報表處理器版本處理。 如需為何報表無法升級，請參閱[Upgrade Reports](../install-windows/upgrade-reports.md)。  
...
-   `Previous``RunningValue`或`RowNumber`彙總函式中的資料儲存格使用*\<報表項目類型 >* '*\<報表項目名稱 >*' 群組範圍，請參閱在資料行和資料列的*\<報表項目類型 >*。 所有的範圍參數`Previous`，`RunningValue`和`RowNumber`彙總函式內*\<報表項目類型 >* 資料列群組或資料行群組，但不能兩者都可以參考。  
