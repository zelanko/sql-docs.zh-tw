...
1.  啟動[報表管理員&#40;SSRS 原生模式&#41;](../../2014/reporting-services/report-manager-ssrs-native-mode.md)系統管理員權限，例如，以滑鼠右鍵按一下 Internet explorer 的圖示，然後按一下**系統管理員身分執行**。  
  
2.  瀏覽到包含 **Sales Orders** 報表的資料夾，然後在報表的內容功能表中，按一下 **[管理]**。  
  
     ![開啟 [報表] 內容功能表並選取管理](../../2014/tutorials/media/ssrs-tutorial-datadriven-manage-report.gif "開啟報表 內容功能表並選取管理")  
...
8.  按一下**當做 windows 認證連接到資料來源時使用**，然後按一下 **確定**。 如果您不想要使用網域帳戶 (例如，如果您使用[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]登入)，不要在上面按一下此核取方塊。  
  
9. 按一下 [測試連接]，確認您能夠連線到資料來源。  
  
10. 按一下 **[套用]**。  
  
11. 檢視報表以確認報表是以您指定的認證來執行。 若要檢視報表，請按一下  **檢視**  索引標籤。請注意，一旦報表開啟時，您必須選取員工姓名，然後按一下**檢視報告**來檢視報表 按鈕。  
...
6.  按一下 [確定] 。 參數會加入至**報表資料 窗格**和它看起來類似下列的影像：  
  
     ![新的參數加入至報表資料 窗格](../../2014/tutorials/media/ssrs-tutorial-datadriven-parameter.gif "新參數加入至 [報表資料] 窗格")  
  
7.  按一下**預覽**來執行報表 索引標籤。請注意位於報表頂端的參數輸入的方塊。 您可以：  
...
8.  請重新部署報表，讓下一課的訂閱組態能夠運用您在這一課所做的變更。 如需用於資料表教學課程之專案屬性的詳細資訊，請參閱[第 6 課：新增群組和總計 &#40;Reporting Services&#41;](../reporting-services/lesson-6-adding-grouping-and-totals-reporting-services.md) 的＜將報表發行至報表伺服器 (選擇性)＞一節。  
