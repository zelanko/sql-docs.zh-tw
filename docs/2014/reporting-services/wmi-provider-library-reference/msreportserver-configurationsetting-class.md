...
## <a name="example"></a>範例  
 若要執行下列程式碼，請新增您的伺服器名稱來取代每個 \<*伺服器名稱*>。 將此路徑更新為指向您的安裝位置 (如果不是預設值的話)。 下列程式碼範例會逐一中每一個屬性[MSReportServer_ConfigurationSetting](msreportserver-configurationsetting-class.md)類別，並列印每個屬性名稱和其值到主控台。  
