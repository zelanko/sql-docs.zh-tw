...
6.  在 [排列欄位] 頁面中執行下列操作：  
    ...
    2.  按一下箭號旁**sum （productid)**， **Sum(PurchaseOrderID)**， **Sum(PurchaseOrderDetailID)**， **Sum(OrderQty)**， **Sum(ReceivedQty)**， **Sum(RejectedQty)**，和**Sum(StockedQty)** 清除**總和**選取項目。  
