...
# <a name="tutorial-creating-a-matrix-report-report-builder"></a>教學課程：建立矩陣報表 (報表產生器)
 ...
 本教學課程即將建立的報表另有一個增強型版本，可從範例 [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] 報表產生器報表取得。 如需下載此範例報表及其他專案的詳細資訊，請參閱[報表產生器範例報表](https://go.microsoft.com/fwlink/?LinkId=184851)。  
...
2.  [從新增資料表或矩陣精靈組織資料、選擇配置和樣式](#Groups)  
...
##  <a name="1-create-a-matrix-report-and-dataset-from-the-new-table-or-matrix-wizard"></a><a name="CreateMatrix"></a>1. 從 [新增資料表或矩陣] Wizard 建立矩陣報表和資料集  
 從報表產生器的 [**消費者入門**] 對話方塊中，選擇共用資料來源、建立內嵌資料集，然後在矩陣中顯示資料。  
...
1.  按一下 **[開始]**、依序指向 **[程式集]** 和 **[Microsoft SQL Server 2012 報表產生器]**，然後按一下 **[報表產生器]**。  
    ...
    > [!NOTE]  
    >  **[使用者入門]** 對話方塊應會隨即出現。 如果不是，請從 [報表產生器] 按鈕按一下 [**新增**]。  
...
6.  在 [**選擇與資料來源的連接**] 頁面上，選取現有的資料來源，或流覽至報表伺服器，然後選取資料來源。 如果沒有資料來源可用，或無法存取報表伺服器，您可以改用內嵌資料來源。 如需建立內嵌資料來源的詳細資訊，請參閱[教學課程：建立基本資料表報表 &#40;報表產生器&#41;](../reporting-services/tutorial-creating-a-basic-table-report-report-builder.md)。  
...
##  <a name="2-organize-data-and-choose-layout-and-style-from-the-new-table-or-matrix-wizard"></a><a name="Groups"></a>2. 從 [新增資料表或矩陣] Wizard 組織資料並選擇版面配置和樣式  
...
4.  將 [產品] 拖曳至 [資料**行群組]，** 然後放在子類別底下。  
  
     欄位在 [資料**行群組**] 中列出的順序會定義群組階層。  
...
3.  在 **[主資料夾]** 索引標籤的 **[數值]** 群組中，按一下 **[貨幣]**。 資料格隨即變更為顯示格式化貨幣。  
     ...
     如果您的地區設定為 [英文 (美國)]，則預設範例文字會是 [**$12,345.00**]。 如果您看不到範例貨幣值，請按一下 [**數位**] 群組中的 [**預留位置樣式**]，然後按一下 [**範例值**]。  
  
4.  按一下包含 `[SalesDate]`的資料格。  
  
5.  在 [**數位**] 群組中，從下拉式清單選取 [**日期**]。  
...
##  <a name="4-add-adjacent-column-group"></a><a name="AdjacentGroup"></a>4. 加入連續的資料行群組  
...
> [!NOTE]  
>  如果資料中還有別的星期，報表也會納入其資料行。 每個資料行都有欄位`Sales`標題、和銷售總計（依地區）。  
  
##  <a name="5-change-column-widths"></a><a name="Width"></a>5. 變更資料行寬度  
...
##  <a name="6-merge-matrix-cells"></a><a name="MergeCells"></a>6. 合併矩陣資料格  
...
##  <a name="7-add-a-report-header-and-report-title"></a><a name="HeaderTitle"></a>7. 加入報表頁首和報表標題  
...
2.  按一下包含 [**按一下以加入標題**] 之報表主體頂端的文字方塊，然後按下 Delete 鍵。  
  
3.  在功能區的 [**插入**] 索引卷**標**上，按一下 [頁首]，然後按一下 [**新增標頭**]。  
...
6.  選取您輸入的文字，再按一下滑鼠右鍵，然後按一下 [**文字屬性**]。  
  
    > [!NOTE]  
    >  若要一次格式化整串文字，必須選取連續字元。  
  
7.  按一下 [**文字屬性**] 對話方塊中的 [**字型**]。  
  
8.  在 [**字型**] 清單中，選取 [ **Times New Roman**;在 [**大小**] 中選取 [ **24 pt**]，在 [**色彩**] 中選取 [**褐紫紅色**]，**並選取 [****樣式**  
...
##  <a name="8-save-the-report"></a><a name="Save"></a>8. 儲存報表  
...
##  <a name="9-optional-rotate-text-box-270-degrees"></a><a name="RotateTextBox"></a>9. （選擇性）旋轉文字方塊270度  
...
3.  在 [屬性] 窗格中，找出 WritingMode 屬性，並在其下拉式清單中選取 [ **Rotate270**]。  
  
     如果 [屬性] 窗格並未開啟，請按一下功能區的 [檢視]**** 索引標籤，然後選取 [屬性]****。  
  
4.  確認 [CanGrow] 屬性已設為`True`。  
