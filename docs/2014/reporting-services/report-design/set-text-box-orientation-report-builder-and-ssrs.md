...
# <a name="set-text-box-orientation-report-builder-and-ssrs"></a>設定文字方塊方向 (報表產生器及 SSRS)
 ...
 WritingMode 屬性，讓您用來指定文字方向，不適用於**文字方塊內容** 對話方塊。 您需要開啟 [屬性] 窗格，然後在該處設定屬性。 WritingMode 屬性可用的值為**水平**（閱讀文字從左到右）、**垂直**（閱讀文字由上的而下）、 **Rotate270** （閱讀文字由下至上）。 您必須手動調整資料行寬度及資料列高度，以便容納文字。  
...
4.  找出 writingmode 設定屬性在 [屬性] 窗格中，然後在下拉式清單中選取要套用至文字方塊中文字的方向。  
