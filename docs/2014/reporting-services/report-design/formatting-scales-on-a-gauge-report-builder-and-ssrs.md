...
# <a name="formatting-scales-on-a-gauge-report-builder-and-ssrs"></a>格式化量測計上的標尺 (報表產生器及 SSRS)
 ...
 ![量測計標尺的元素概觀](../media/scaleoverviewdiagram.gif "量測計標尺的元素概觀")  
...
##  <a name="DefiningMinMax"></a>定義尺規上的最小值、最大值和間隔  
...
2.  如果您未指定 Interval 的值，預設值會是 Auto。這表示應用程式會根據第一個步驟中所指定的最小和最大值，來計算等距的間隔數。 如果您有指定 [間隔] 的值，量測計將會計算最大值與最小值之間的差距，並將該數目除以 [間隔] 屬性中指定的值。  
...
##  <a name="SpecifyingScaleBar"></a>指定星形尺規上的尺規列寬度、半徑和角度  
...
##  <a name="PositioningLabels"></a>將標籤放置線上性或星形尺規上  
