...
```  
 ...
 若要參考自訂程式碼中的內建集合，包含內建參考`Report`物件：  
...
```  
 ...
 雖然自訂常數不會出現在 **[運算式]** 對話方塊的 **[常數]** 類別目錄中 (這個類別目錄只會顯示內建常數)，但是您可以從任何運算式加入自訂常數的參考，如下列範例所示。 在運算式中，將自訂常數會被視為`Variant`。  
