...
3.  在 [URL] 欄位中，輸入報表伺服器 Web 服務的完整路徑。  
    ...
    ```  
     ...
     這個 URL 包含部署報表伺服器 Web 服務的網域、包含服務的資料夾名稱，以及服務的探索檔名稱。 如需不同 URL 項目的完整描述，請參閱[存取 SOAP API](../accessing-the-soap-api.md)。  
  
     Web 服務提供的方法與屬性之描述會出現在左邊的 [瀏覽器] 窗格中。  
  
    > [!NOTE]  
    >  如需與報表伺服器 Web 服務建立關聯之項目的詳細資訊，請參閱[報表伺服器 Web 服務方法](../methods/report-server-web-service-methods.md)。  
