...
## <a name="passing-device-information-using-the-render-method"></a>使用轉譯方法傳遞裝置資訊  
 若要將裝置資訊設定傳遞至轉譯延伸模組，請使用 <xref:ReportExecution2005.ReportExecutionService.Render%2A> 方法。 例如，下列的 XML 字串可以傳遞至 <xref:ReportExecution2005.ReportExecutionService.Render%2A> 方法，以在轉譯為 HTML 時建立 HTML 片段。  
...
```  
 ...
 如需詳細資訊，請參閱[在 URL 中指定裝置資訊設定](../../specify-device-information-settings-in-a-url.md)。  
