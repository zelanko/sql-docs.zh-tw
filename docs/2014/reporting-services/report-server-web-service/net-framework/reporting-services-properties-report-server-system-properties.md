...
|SystemSnapshotLimit|針對報表所儲存之快照集的最大數目。 有效值為`-1`透過`2`，`147`，`483`，`647`。 如果值為`-1`，沒有任何快照集限制。|  
|SystemReportTimeout|在報表伺服器命名空間中管理之所有報表的預設報表處理逾時值 (以秒為單位)。 在報表層級可以覆寫這個值。 如果已設定此屬性，當指定的時間已過期時，報表伺服器就會嘗試停止處理報表。 有效值為`-1`透過`2`，`147`，`483`，`647`。 如果此值為 `-1`，命名空間中的報表就不會在處理期間逾時。 預設值是 `1800`。|  
|UseSessionCookies|指出報表伺服器與用戶端瀏覽器通訊時是否應該使用工作階段 Cookie。 預設值是 `true`。|  
|SessionTimeout|工作階段維持作用中狀態的時間長度 (以秒為單位)。 預設值是 `600`。|  
|EnableMyReports|指出 [我的報表] 功能是否已啟用。 值為`true`表示功能已啟用。|  
|MyReportsRole|在使用者之 [我的報表] 資料夾上建立安全性原則時所使用的角色名稱。 預設值是 `My Reports Role`。|  
|EnableExecutionLogging|指出報表執行記錄是否已啟用。 預設值是 `true`。|  
|ExecutionLogDaysKept|要將報表執行資訊保留在執行記錄中的天數。 這個屬性的有效值包括`0`透過`2`，`147`，`483`，`647`。 如果此值為 `0`，系統就不會從執行記錄資料表中刪除項目。 預設值是 `60`。|  
