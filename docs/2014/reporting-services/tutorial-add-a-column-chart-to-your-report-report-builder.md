...
##  <a name="Chart"></a> 1.從圖表精靈建立圖表報表  
 從**開始使用**對話方塊，請使用 [圖表精靈] 建立內嵌資料集，選擇共用的資料來源，並建立直條圖。  
...
3.  功能區上**首頁**索引標籤中，於**數目**群組中，按一下**貨幣** 按鈕。 這些軸標籤就會變更為顯示貨幣格式。  
  
4.  功能區上**首頁**索引標籤中，於**數目**群組中，按一下**減少小數位數**按鈕兩次，顯示四捨五入到最接近之美元的數字。  
  
5.  以滑鼠右鍵按一下垂直軸，然後按一下 **垂直軸屬性**。  
  
6.  按一下 **[數值]**。 請注意，**貨幣**中已選取**分類** 方塊中，並**小數位數**已經**0** （零）。  
  
7.  在 **中顯示的值**方塊中，按一下**千分位**。  
...
3.  以滑鼠右鍵按一下 **[sum （sales)]** 中的欄位**值**區域，，然後按一下**加入導出數列**。  
...
3.  型別**Sales Chart**，按下 ENTER，接著再輸入**年 1 月到 2009 年 12 月**，因此它看起來像這樣：  
  
     **銷售圖表**  
  
     **年 1 月到 2009 年 12 月**  
  
4.  選取**Sales Chart**，然後按一下**粗體**按鈕**字型**區段**首頁**功能區 索引標籤。  
  
5.  選取 **年 1 月到 2009 年 12 月**，然後在**字型**區段**首頁**索引標籤上，將字型大小設定為**10**。  
  
6.  （選擇性）您可能需要進行**Title**文字方塊的高度，才能容納兩行文字，當您按一下底端邊緣的中間時，拉下雙箭號上。  
