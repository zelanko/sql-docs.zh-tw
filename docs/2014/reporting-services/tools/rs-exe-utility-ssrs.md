 ...
 **-?**  
 ...
 `-p` *密碼*  
 (若`-u`指定) 指定要搭配使用的密碼`-u`引數。 此值區分大小寫。  
...
-   Exec2005  
 ...
 如果未指定值，則會使用 Mgmt2005 端點。 如需有關 SOAP 端點的詳細資訊，請參閱[報表伺服器 Web 服務端點](../report-server-web-service/methods/report-server-web-service-endpoints.md)。  
  
 `-l` *逾時*  
 (選擇性) 指定與伺服器的連接逾時之前所經過的秒數。預設值是 60 秒。 若未指定逾時值，則使用預設值。 `0` 的值指定連接永不逾時。  
  
 **-b**  
 (選擇性) 指定以批次方式執行指令碼檔案中的命令。 若有任何命令失敗，便會回復此批次。 有些命令無法批次處理，而會依平常方式執行。 只有在指令碼中發生未處理的例外狀況會導致批次復原。 如果指令碼處理例外狀況，並從正常地傳回`Main`，就會認可批次。 如果忽略此參數，則會執行此命令而不會建立批次。 如需詳細資訊，請參閱[Batching Methods](../report-server-web-service-net-framework-soap-headers/batching-methods.md)。  
 ...
 全域變數會使用給定的名稱來建立並設定為所提供的值。 比方說， **-v =**"`1`" **-v b =**"`2`"會產生名為的變數`a`值是"`1`"和變數**b**值是"`2`"。  
...
## <a name="remarks"></a>備註  
 您可以定義指令碼來設定系統屬性、發行報表等等。 您可以建立的指令碼包括 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] API 的任何方法。 如需方法和屬性可供您使用的詳細資訊，請參閱[報表伺服器 Web 服務](../report-server-web-service/report-server-web-service.md)。  
