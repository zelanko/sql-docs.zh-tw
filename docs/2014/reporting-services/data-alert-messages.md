...
1.  **代表**包含建立資料警示定義之人員的名稱。

2.  如果您在警示定義中提供了描述，該描述會顯示於 **代表**下方。

3.  **警示結果**會以表格格式顯示報表資料摘要中符合警示定義中所指定規則的資料列，或是顯示錯誤描述。 顯示的資料列數並無限制。

4.  [**移至報表**] 是建立警示定義時所依據之報表的連結。 如果因為報表移動或刪除而導致連結無效，則會顯示錯誤訊息。

5.  [**規則**] 會列出警示定義中的規則和子句。 這項資訊有助於驗證和了解警示結果，並且識別資料警示定義中您可能想要變更以縮小或擴大結果的規則。

6.  **報表參數**會列出執行報表時所使用的參數和參數值。 參數和參數值有助於了解警示結果。

7.  內容**值**會列出位於報表資料區域外之報表專案的名稱和值。 這些項目通常是文字方塊。 例如，含有常值的文字方塊 (像是報表的主旨或描述)。
...
## <a name="see-also"></a>另請參閱
 資料警示[設計](../../2014/reporting-services/data-alert-designer.md)工具[Reporting Services 資料警示](../ssms/agent/alerts.md)
