...
# <a name="troubleshoot-a-reporting-services-installation"></a>疑難排解 Reporting Services 安裝
 ...
 如果您遇到的問題在版本資訊中有描述，請參閱[線上版本](https://go.microsoft.com/fwlink/?linkid=236893)資訊。  
...
-   [您會看到錯誤訊息，指出未設定 URL](#bkmk_URL_not_configured)  
...
-   [您會看到 RS_SHP 不支援 PREPAREIMAGE 的錯誤訊息](#bkmk_RS_SHP_notsupported)  
  
###  <a name="bkmk_configmanager_notstart"></a>Reporting Services 組態管理員未啟動  
 **描述：** 這個問題是由中[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]的設計。 
  [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 現在的架構為 SharePoint 服務架構。 在 SharePoint 模式中設定及管理 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 時，不再需要組態管理員。  
 ...
 **描述：** 如果[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]成功安裝 sharepoint 模式和適用于 sharepoint [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 2010 的增益集之後，您在下列兩個功能表中看不到 [SQL Server Reporting Services]，表示[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]服務尚未註冊：  
...
###  <a name="bkmk_cmdlets_not_recognized"></a>Reporting Services PowerShell Cmdlet 無法使用，而且無法辨識命令  
...
-   執行適用於 SharePoint 產品的 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 增益集。 **rssharepoint.msi .msi**。  
...
###  <a name="bkmk_URL_not_configured"></a>您會看到錯誤訊息，指出未設定 URL  
 **描述：** 您會看到類似下面的錯誤訊息：  
 ...
 **詳細資訊：** 當您嘗試使用任何需要[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]服務連線的功能時，將會看到此錯誤訊息。 其中包括：  
...
###  <a name="bkmk_sharepoint_not_confiugred"></a>在已安裝 SharePoint 但尚未設定的電腦上，安裝程式會失敗  
 ...
 **詳細資訊：** 安裝[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]到現有的 sharepoint 安裝時，安裝程式會嘗試安裝並啟動[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] sharepoint 服務。 若尚未設定 SharePoint，服務安裝將會失敗並會導致安裝程式失敗。  
...
###  <a name="bkmk_reportbuilder_newreport_error"></a>當您嘗試建立新的報表產生器報表時，會看到一則錯誤訊息  
...
###  <a name="bkmk_RS_SHP_notsupported"></a>您會看到 RS_SHP 不支援 PREPAREIMAGE 的錯誤訊息  
 **描述：** 當您嘗試執行 PREPAREIMAGE 時[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] ，您會看到類似下面的錯誤訊息：  
  
 「執行 PREPAREIMAGE 動作時不支援指定的功能 'RS_SHP'，因為此動作不支援 SysPrep。 請移除與 SysPrep 不相容的功能，然後重新執行安裝程式。」  
  
 因應措施 **：** 沒有解決方法。 
  [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 不支援 SYSPREP (PREPAREIMAGE)。 
  [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 原生模式支援 SYSPREP。  
  
##  <a name="bkmk_tshoot_native"></a>針對原生模式安裝的問題進行疑難排解  
  
###  <a name="PerfCounters"></a>升級至 Windows Vista 或 Windows Server 2008 之後看不到效能計數器  
...
###  <a name="Default2005InstallBreaks2008"></a>在執行 SQL Server 2012Reporting Services 的電腦上，SQL Server 2005 Reporting Services 的預設實例安裝失敗  
 如果[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]您嘗試在已經執行實例的[!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]電腦上安裝的預設實例， [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]實例將無法安裝，並出現下列錯誤訊息：  
...
-   如果您必須在[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]電腦上以預設實例的形式執行，您必須在[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]實例之前安裝實例。  
  
-   [!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]如果實例不需要是預設實例，您可以在安裝[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] [!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]實例之後，將實例安裝為已命名的實例。  
  
###  <a name="WindowsAuthBreaksAfterUpgrade"></a>401-從 SQL Server 2005 升級至 SQL Server 2012 之後，使用 Windows 驗證時發生未經授權的錯誤  
 如果您從[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]升級到[!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]，而且對報表伺服器服務帳戶使用 NTLM 驗證與內建帳戶，當您在升級後存取報表伺服器或報表管理員時，可能會遇到 401-未經授權的錯誤。  
