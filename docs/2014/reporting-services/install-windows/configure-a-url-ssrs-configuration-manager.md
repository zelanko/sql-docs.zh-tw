...
-   設定進階的 URL 屬性，以定義其他 URL。  
 ...
 有關 URL 如何存儲和維護或互通性問題的詳細資訊，請參閱有關 URL[預留和註冊&#40;SSRS 組態管理員&#41;](about-url-reservations-and-registration-ssrs-configuration-manager.md)並[安裝報表服務和互聯網資訊服務並排&#40;SSRS 原生模式&#41;](install-reporting-and-internet-information-services-side-by-side.md)連線圖書。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 若要檢閱 Reporting Services 安裝中常用的 URL 範例，請參閱本主題的＜ [URL 範例](#URLExamples) ＞。  
...
4.  指定可唯一識別網路上之報表伺服器電腦的 IP 位址。 如果您想要指定主機標頭，或針對相同的應用程式執行個體定義其他 URL，您必須按一下 **[進階]**。 如需有關如何針對 URL 設定進階屬性的指示，請參閱本主題稍後的指示。 否則，請使用 **[Web 服務 URL]** 頁面，從下列值當中選取：  
    ...
    -   特定的 IP 位址也會出現在這個清單中。 IP 位址可以採用 IPv4 和 IPv6 格式。 *Nnn.nnn.nnn.nnn* 是電腦網路介面卡的 32 位元 IPv4 位址。 IPv6 位址為 128 位，8 個 4 位元組欄位由冒號\<分隔：首碼>：nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn：nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn：nnnnnnnnnnnnnnnnnnnnnn* *  
...
11. 此外，如果您的報表伺服器設定成使用 SharePoint 整合模式，請在 SharePoint 管理中心內設定報表伺服器 Web 服務 URL。 有關如何更新 SharePoint 管理中心中的報表伺服器 Web 服務 URL 的詳細資訊，請參閱[報表伺服器的配置和管理&#40;報表服務器共用點模式&#41;](../configure-administer-report-server-reporting-services-sharepoint-mode.md)和[報表服務報表服務器&#40;sharePoint 模式&#41;](../reporting-services-report-server-sharepoint-mode.md)。  
...
3.  指定虛擬目錄。 報表管理員會接聽與報表伺服器 Web 服務相同的 IP 位址和通訊埠。 如果您設定報表管理員指向不同的報表伺服器 Web 服務，您必須修改 RSReportServer.config 檔案中的報表管理員 URL 設定。 有關說明，請參閱在連線書籍中[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)][配置報表管理員&#40;原生模式&#41;。](../report-server/configure-web-portal.md)  
...
## <a name="see-also"></a>另請參閱  
 [報表服務組態管理員&#40;原生模式&#41;](../../sql-server/install/reporting-services-configuration-manager-native-mode.md)   
