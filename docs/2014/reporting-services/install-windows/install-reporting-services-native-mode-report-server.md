...
-   [什麼是預設設定？](#bkmk_whatisdefaultconfiguration)

-   [何時安裝原生模式的預設設定](#bkmk_whentoinstalldefaultconfig)

-   [需求](#bkmk_requirements)

-   [預設 URL 保留專案](#bkmk_defaultURLreservations)

-   [使用 SQL Server 安裝精靈安裝原生模式](#bkmk_installwithwizard)

-   [使用命令列安裝原生模式](#bkmk_commandline)

##  <a name="bkmk_whatisdefaultconfiguration"></a>什麼是預設設定？
...
-   
 ...
 此選項不適用於[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]或[!INCLUDE[ssBIDevStudioFull](../../includes/ssbidevstudiofull-md.md)]之類的共用功能，如果您想要安裝它們，必須將其指定為個別專案。
...
##  <a name="bkmk_whentoinstalldefaultconfig"></a>何時安裝原生模式的預設設定
...
-   [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]和[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)]必須一起安裝在相同的實例中。 
  [!INCLUDE[ssDE](../../includes/ssde-md.md)] 執行個體會主控安裝程式所建立及設定的報表伺服器資料庫。

-   用來執行安裝程式的使用者帳戶必須是本機管理員群組的成員，而且擁有在主控報表伺服器資料庫之 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 執行個體上存取和建立資料庫的權限。

-   安裝程式必須能夠使用預設值，以保留可提供報表伺服器和報表管理員之存取的 URL。 這些值包括連接埠 80、一個強式萬用字元，以及採用 **ReportServer_\<執行個體名稱******>** 和 **Reports_\<執行個體名稱******>** 格式的虛擬目錄名稱。
...
##  <a name="bkmk_defaultURLreservations"></a>預設 URL 保留專案
 URL 保留項目是由前置詞、主機名稱、通訊埠和虛擬目錄所組成：

|部分|描述|
|----------|-----------------|
|前置詞|預設前置詞是 HTTP。 如果您之前安裝了安全通訊端層 (SSL) 憑證，安裝程式將會嘗試建立使用 HTTPS 前置詞的 URL 保留項目。|
|主機名稱|預設主機名稱是強式萬用字元 (+)， 它會指定報表伺服器將會針對解析為電腦的任何主機名稱，接受指定埠上的任何 HTTP 要求，包括 HTTP://\<computername>/reportserver、 http://localhost/reportserver或 HTTP://\<IPAddress>/reportserver。|
...
|虛擬目錄|根據預設，會以報表伺服器 Web 服務的 ReportServer_\<*instance_name*> 格式建立虛擬目錄，\<*並 Reports_ instance_name> 的*報表管理員。 如果是報表伺服器 Web 服務，預設虛擬目錄會是 **reportserver**。 如果是報表管理員，預設虛擬目錄則為 **reports**。|
...
2.  在 **[特徵選取]** 頁面上，選取下列選項：
    ...
    -   除非已安裝 Database Engine 的實例，否則**資料庫引擎服務**。

    -   **Reporting Services 原生**。

    -   **管理工具-基本**。 管理工具並非必要，但建議您使用這些工具，除非您安裝了其他管理工具。 預設的設定選項會產生正常運作的報表伺服器，但您可能會想要在日後變更設定選項。 某些選項（例如「我的報表」）是透過來管理[!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)]
...
4.  在 **[Reporting Services 組態]** 頁面上選取 **[安裝和設定]**。
     ...
     ![SSRS 原生模式組態](../../../2014/sql-server/install/media/rs-setupconfiguration-native-with-circles.gif "SSRS 原生模式組態")
...
```
 ...
 如需詳細資訊和範例，請參閱[命令提示字元安裝 Reporting Services SharePoint 模式和原生模式](../../reporting-services/install-windows/install-reporting-services-at-the-command-prompt.md)，並[從命令提示字元安裝 SQL Server 2014](../../database-engine/install-windows/install-sql-server-from-the-command-prompt.md)

## <a name="see-also"></a>另請參閱
 [疑難排解 Reporting Services 安裝](../../reporting-services/install-windows/troubleshoot-a-reporting-services-installation.md)[請確認 REPORTING SERVICES 安裝](../../reporting-services/install-windows/verify-a-reporting-services-installation.md) [&#40;ssrs Configuration Manager&#41;](../../reporting-services/install-windows/configure-the-report-server-service-account-ssrs-configuration-manager.md)設定報表伺服器 Url &#40;Ssrs [Configuration Manager&#41;](../../reporting-services/install-windows/configure-report-server-urls-ssrs-configuration-manager.md)設定[報表伺服器資料庫連接 &#40;ssrs Configuration Manager](../../../2014/sql-server/install/configure-a-report-server-database-connection-ssrs-configuration-manager.md)&#41;[僅限檔案的安裝 &#40;](../../reporting-services/install-windows/files-only-installation-reporting-services.md) Reporting Services 將[報表伺服器初始化&#41;ssrs &#40;Configuration Manager](../../reporting-services/install-windows/ssrs-encryption-keys-initialize-a-report-server.md) [設定 SSL原生模式報表伺服器上的連接](../security/configure-ssl-connections-on-a-native-mode-report-server.md)[設定報表伺服器 URL &#40;SSRS Configuration Manager&#41;](../../reporting-services/install-windows/configure-report-server-urls-ssrs-configuration-manager.md) [設定 Windows 服務帳戶和許可權](../../database-engine/configure-windows/configure-windows-service-accounts-and-permissions.md)[快速開始安裝 SQL Server 2014](../../../2014/getting-started/quick-start-installation-of-sql-server-2014.md)
