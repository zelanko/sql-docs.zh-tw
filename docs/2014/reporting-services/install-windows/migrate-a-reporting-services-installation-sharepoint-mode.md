...
##  <a name="bkmk_prior_versions"></a>從 SQL Server 2012 之前的 Reporting Services SharePoint 模式版本遷移  
 在 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 中變更的 [!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]SharePoint 模式架構，包括服務應用程式資料庫結構描述。 如果您想要從之前[!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)]的版本遷移至 SharePoint 模式[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]，請先安裝 sharepoint 和[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] sharepoint 模式來建立新的 sharepoint 環境。 如需詳細資訊，請參閱[Reporting Services Sharepoint 模式安裝 &#40;sharepoint 2010 和 sharepoint 2013&#41;](../../reporting-services/install-windows/install-reporting-services-sharepoint-mode.md)。  
  
 新的 SharePoint 環境開始執行之後，可以在包含內容資料庫的資料庫層級選擇僅限內容移轉或完整移轉。  
  
###  <a name="bkmk_content_only_migration"></a>僅限內容的遷移  
 **僅限 Reporting Services 內容遷移：** 如果您想要將[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]內容複寫到新的伺服器陣列，則需要使用如**rs**等的工具將內容複寫到新的 SharePoint 安裝。 如需有關僅限內容移轉的詳細資訊，請參閱以下主題：  
  
-   ** [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] RSS 腳本：** 這些腳本可以在原生模式與 SharePoint 模式報表伺服器之間遷移內容和資源。 如需詳細資訊，請參閱[在報表伺服器之間遷移內容的範例 Reporting Services rs .exe](../tools/sample-reporting-services-rs-exe-script-to-copy-content-between-report-servers.md)腳本和將[內容從一個報表伺服器遷移至另一個的 Reporting Services rs .exe 腳本](https://azuresql.codeplex.com/releases/view/115207)。  
  
-   **Reporting Services 遷移工具：** 此工具可以將報表專案從原生模式伺服器複製到 SharePoint 模式伺服器。 如需詳細資訊，請參閱 [Reporting Services 移轉工具](https://www.microsoft.com/download/details.aspx?id=29560)。  
  
###  <a name="bkmk_full_migration"></a>完整遷移  
 **完整遷移：** 如果您要將 SharePoint 內容資料庫連同[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]目錄資料庫一起遷移至新的伺服器陣列，您可以遵循本主題摘要說明的一系列備份和還原選項。 在某些情況下，還原階段所使用的工具必須與備份階段所使用的工具不同。 例如，您可以使用[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] Configuration Manager 從舊版備份加密金鑰， [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]但是您必須使用 sharepoint 管理中心或 PowerShell 將加密金鑰還原到[!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] sharepoint 模式安裝。  
  
####  <a name="bkmk_databases"></a>您會在完成的遷移中看到的資料庫  
 ...
 **（\*）** 表格中顯示的範例名稱會遵循您在建立新的 ssrs 服務應用程式時 SSRS 所使用的命名慣例。 如果您從不同的伺服器移轉，您的目錄和 tempDB 所擁有的名稱將會來自原始安裝。  
...
##  <a name="bkmk_migrate_from_ctp"></a>從 SQL Server 2012 部署進行遷移  
...
-   [開始升級至 SharePoint 2013 （https://technet.microsoft.com/library/ee833948.aspx)](https://technet.microsoft.com/library/ee833948.aspx)）。  
  
-   [SharePoint 2013 升級程式的https://technet.microsoft.com/library/cc262483.aspx)總覽（](https://technet.microsoft.com/library/cc262483.aspx)。  
