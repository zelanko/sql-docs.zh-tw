...
> [!IMPORTANT]  
>  並非所有 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 版本都支援向外延展 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 如需詳細資訊，請[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]參閱[SQL Server 2014 版本所支援的功能](../../../2014/getting-started/features-supported-by-the-editions-of-sql-server-2014.md)一節。  
...
|執行 SharePoint 產品設定精靈，將伺服器加入至伺服器陣列。|1）在 [ **Microsoft sharepoint 2010 產品**] 程式群組中，按一下 [ **Microsoft sharepoint 2010 產品設定向導]**。<br /><br /> 2）在 [**連接到伺服器陣列]** 頁面上，選取 **[連接到現有的伺服器陣列]** ，然後按 **[下一步]**<br /><br /> 3）在 [**指定設定資料庫設定**] 頁面上，輸入用於現有伺服器陣列之資料庫伺服器的名稱，以及設定資料庫的名稱。 按 [下一步]  。<br />** \* \*重要\*事項**如果您看到類似下面的錯誤訊息，而且已確認您擁有許可權，請確認**SQL Server**中的 SQL Server 網路設定已啟用哪些通訊協定 Configuration Manager：「無法連線到資料庫伺服器」。 請確認資料庫存在、是 Sql Server，而且您擁有存取伺服器的適當許可權。」<br />** \* \*重要\*事項**如果您看到 [**伺服器陣列產品及修補狀態**] 頁面，您將需要先檢查頁面上的資訊，並使用所需的檔案補救伺服器，然後才能繼續將伺服器加入至伺服器陣列。<br /><br /> 4）在 [**指定伺服器陣列安全性設定**] 頁面上輸入您的伺服器陣列複雜密碼，然後按 **[下一步** 在確認頁面上，按 **[下一步]** 執行精靈。<br /><br /> 5）按 **[下一步]** 執行伺服器陣列設定**Wizard**。|  
|確認伺服器已加入至 SharePoint 伺服器陣列。|1) 在 SharePoint 管理中心內，按一下 [系統設定]**** 群組中的 [管理此伺服器陣列中的伺服器]****。<br /><br /> 2) 確認已加入新的伺服器，而且狀態正確。<br /><br /> 3）請注意，您看不到執行中的服務**SQL Server Reporting Services 服務**。 下一個步驟將會安裝此服務。<br /><br /> 4）若要從 WFE 角色中移除此伺服器，請按一下 [**管理伺服器上的服務**]，並停止 [ **Microsoft SharePoint Foundation Web 應用程式**服務]。|  
|安裝和設定 Reporting Services SharePoint 模式。|執行 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 安裝。 如需安裝[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] sharepoint 模式的詳細資訊，請參閱[安裝 SharePoint 2010 Reporting Services sharepoint 模式](../../../2014/sql-server/install/install-reporting-services-sharepoint-mode-for-sharepoint-2010.md)如果伺服器只會當做應用程式伺服器使用，而伺服器不會當做 WFE 使用，您就不需要在上選取**適用于 sharepoint 產品的 Reporting Services 增益集**：<br /><br /> 在 [**安裝程式角色**] 頁面上，選取 [ **SQL Server 功能安裝**]<br /><br /> [**特徵選取**] 頁面，選取 [ **Reporting Services-SharePoint** ]<br /><br /> -或-<br /><br /> [ **Reporting Services**設定] 頁面上，確認已針對 [ **Reporting Services SharePoint 模式]** 選取 [**只安裝**] 選項。|  
...
##  <a name="bkmk_additional"></a>其他設定  
 ...
 未啟用 Reporting Services Web 服務。 至少設定一個 Reporting Services SharePoint 服務的實例，讓\<iswebserviceenable>> 設為 true。 如需詳細資訊，請參閱[&#40;Rsreportserver.config 修改 Reporting Services 設定檔&#41;](../report-server/modify-a-reporting-services-configuration-file-rsreportserver-config.md)  
