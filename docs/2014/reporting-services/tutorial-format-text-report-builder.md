...
### <a name="set-up-the-report"></a>設定報表  
 ...
 2. [將欄位加入至報表設計介面 (先做錯，再用正確的方法)](#AddField)  
  
 3. [將資料表加入至報表 Design Surface](#AddTable)  
...
4.  確認連線類型為 Microsoft SQL Server，接著在 [連接字串]**** 方塊中鍵入 **Data Source = \<伺服器名稱>**  
    ...
    > [!NOTE]  
    >  運算式\<servername> （例如 report001)）會指定安裝 SQL Server 資料庫引擎實例的電腦。 本教學課程無須任何特定資料，您只需要連接到 [!INCLUDE[ssCurrent](../includes/sscurrent-md.md)] 資料庫。 如果您已有資料來源連接列於 [資料來源連接]**** 底下，就可以選取該連接並移至下一個程序「建立資料集」。 如需詳細資訊，請參閱[取得資料連線的替代方式 &#40;報表產生器&#41;](../reporting-services/alternative-ways-to-get-a-data-connection-report-builder.md)。  
...
1.  在 [**插入**] 功能表上，按一下 [**資料表**]，然後按一下 [**資料表嚮導]**。  
  
2.  在 [新增資料表或矩陣] wizard 的 [**選擇資料集**] 頁面上，按一下 [**選擇此報表或共用資料集中的現有資料集**]，然後按一下 [ **TextDataset （在此報表中）**]，再按 **[下一步]**。  
  
3.  在 [**排欄欄位**] 頁面上，將 [**區域**]、[ **linktext>**] 和 [**產品**] 欄位拖曳至 [**資料****列群組**]，將 [ **Sales** ] 欄位拖曳至 [值]，然後按一下 **[**  
  
4.  在 [**選擇**配置] 頁面上，清除 [**展開/** 折迭群組] 核取方塊，讓您可以看見整個資料表，然後按 **[下一步]**。  
  
5.  在 [**選擇樣式**] 頁面上，按一下 [**平板**電腦]，然後按一下 **[完成]**。  
...
9. 以滑鼠右鍵按一下包含`[LinkText]`的文字方塊，然後按一下 [**分割資料格**]。  
  
10. 選取`[LinkText]`資料格下方的空白資料格，然後按住 SHIFT 鍵並選取其右邊的兩個數據格： [ **Product** ] 資料行中的 [ **Total** ] `[Sum(Sales)]`資料格，以及 [ **Sales** ] 資料行中的資料格。  
...
6.  在 [**一般**] 頁面的 [**標記類型**] 底下，按一下 [ **Html-將 html 標記解讀為樣式**]。  
