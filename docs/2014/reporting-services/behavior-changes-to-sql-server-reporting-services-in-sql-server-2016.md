 ...
 **先前的行為：** 客戶可以開發 SOAP 用戶端程式代碼，直接與[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]端點通訊，並呼叫 GetReportServerConfigInfo （）。  
 ...
 **新的行為：**[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] Configuration Manager 不再支援 SharePoint 模式的報表伺服器。 您現在可以使用 SharePoint 管理中心完成 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] SharePoint 模式的組態設定，因此 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 組態管理員不再支援 SharePoint 模式。 組態管理員現在僅用於原生模式的報表伺服器。  
 ...
 **先前的行為：** 客戶會在[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] SharePoint 模式下安裝報表伺服器。 如果客戶想要將報表伺服器切換到原生模式，可以開啟 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 組態管理員切換到原生模式，方法是，建立新的原生模式資料庫，或連線至現有的原生模式資料庫。 客戶也可以使用 [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 組態管理員從 SharePoint 模式切換到原生模式。  
...
### <a name="secureconnectionlevel-property-in-the-reporting-services-wmi-provider-library"></a>Reporting Services WMI 提供者程式庫中的 SecureConnectionLevel 屬性  
 在的 WMI 提供者連結[!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]庫中 **，SecureConnectionLevel**屬性允許`0` `1`、、`2``3`、的值， `0`其表示任何 web 服務方法都不需要安全通訊端層（ssl）， `3`表示所有 web 服務方法都需要 ssl，而`1`和`2`則指出需要 ssl 的 web 服務方法子集。 在[!INCLUDE[ssKatmai](../includes/sskatmai-md.md)] [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]中，這些值將只有兩個可能的意義：  
...
-   正整數表示所有 Web 服務方法都需要 SSL。  
 ...
 這項變更會影響報表伺服器回應 Web 服務呼叫的方式。 <xref:ReportService2005.ReportingService2005.ListSecureMethods%2A>例如，如果**SecureConnectionLevel**設定為0， <xref:ReportService2005.ReportingService2005>且中的所有方法都設定為**** `1`、 `2`或`3`，則現在會傳回任何內容。  
