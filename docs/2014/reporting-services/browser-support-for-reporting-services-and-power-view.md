...
##  <a name="power-view-browser-scenarios"></a><a name="bkmk_powerview"></a>Power View 瀏覽器案例
...
|具有 Power View 工作表的 Excel 2013 活頁簿|**SharePoint Server：Excel Services 中的** [!INCLUDE[ssCrescent](../includes/sscrescent-md.md)] 。<br /><br /> **SharePoint Online （Office 365）：** [!INCLUDE[ssCrescent](../includes/sscrescent-md.md)]在 Excel Web 應用程式上。|請參閱[Excel Services 上的 Power View 或 SharePoint Online 上的 Excel Web 應用程式](#bkmk_powerview_on_ExcelServices)。|  
  
###  <a name="power-view-on-sharepoint-server-and-reporting-services-sharepoint-integrated-mode"></a><a name="bkmk_powerview_on_SSRS"></a>SharePoint Server 和 Reporting Services SharePoint 整合模式上的 Power View  
...
|**Internet Explorer 9**|不受支援|32 位元、64 位元|不受支援|32 位元、64 位元|32 位元、64 位元|不受支援|  
|**Internet Explorer 8**|不受支援|32 位元、64 位元|不受支援|32 位元、64 位元|32 位元、64 位元|不受支援|  
|**Mozilla Firefox （最新的公開發行版）**|32 位元|32 位元|32 位元|32 位元|32 位元|不受支援|  
...
###  <a name="power-view-on-excel-services-or-the-excel-web-app-on-sharepoint-online"></a><a name="bkmk_powerview_on_ExcelServices"></a>在 Excel Services 或 SharePoint Online 上的 Excel Web 應用程式上 Power View
...
|**Apple Safari （最新的公開發行版）**|不支援|不支援|不支援|不支援|不支援|32 位元、64 位元|  
|**Google Chrome （最新的公開發行版）**|32-位 **（\*）** 用於限定時間|32-位 **（\*）** 用於限定時間|32-位 **（\*）** 用於限定時間|32-位 **（\*）** 用於限定時間|32-位 **（\*）** 用於限定時間|不受支援|  
...
##  <a name="report-manager-browser-requirements-native-mode"></a><a name="bkmk_reportmanager"></a>報表管理員瀏覽器需求（原生模式）
...
##  <a name="browser-requirements-for-viewing-reports"></a><a name="bkmk_reportviewer"></a>查看報表的瀏覽器需求
...
|**Internet Explorer 9**|不受支援|32 位元、64 位元|不受支援|32 位元、64 位元|32 位元、64 位元|不支援|不支援|  
|**Internet Explorer 8**|不受支援|32 位元、64 位元|不受支援|32 位元、64 位元|32 位元、64 位元|不支援|不支援|  
...
###  <a name="authentication-requirements"></a><a name="bkmk_authentication"></a>驗證需求
...
##  <a name="browser-support-for-reportviewer-web-server-controls-in-visual-studio"></a><a name="bkmk_controls"></a>Visual Studio 中 ReportViewer Web 服務器控制項的瀏覽器支援
...
|||||||||  
|-|-|-|-|-|-|-|-|  
|**瀏覽器**|**Windows 8** 和 **Windows 8.1**|**Windows 7**|**Windows Server 2012**和**2012 R2**|**Windows Server 2008** 和 **2008 R2**|**Windows Server 2003**|**Mac OS X 10.6-10。9**|**注意事項**|  
...
|**Internet Explorer 9**|不受支援|是|不受支援|是|是|是|Internet Explorer 支援完整的 ReportViewer 功能集合。|  
|**Internet Explorer 8.0**|不受支援|是|不受支援|是|是<sup>1</sup>|不受支援|Internet Explorer 支援完整的 ReportViewer 功能集合。 <sup>1</sup>|  
|**Internet Explorer 7.0**|不受支援|是|不受支援|是|是<sup>1</sup>|不受支援|Internet Explorer 支援完整的 ReportViewer 功能集合。 <sup>1</sup>|  
|**Firefox （最新的公開發行版）**|是|是|是|是|是|不受支援|不支援列印和縮放。|  
|**Safari （最新的公開發行版）**|不支援|不支援|不支援|不支援|不支援|是|不支援列印和縮放。<br /><br /> 此瀏覽器已停用在參數化報表上選取日期所用的日曆控制項。 使用者必須手動輸入他們想要在參數提示區域中使用的日期。|  
|**Chrome （最新的公開發行版）**|是|是|是|是|是|不受支援|不支援列印和縮放。|  
