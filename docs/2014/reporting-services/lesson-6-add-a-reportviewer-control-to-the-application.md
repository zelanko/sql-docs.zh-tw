...
2.  從**AJAX 擴充功能**群組**工具箱** 視窗中，拖曳**ScriptManager**至設計介面的控制項。  
...
5.  在**選擇報表**方塊中，選取您建立父報表。  
     ...
     當您選取報表時，報表中所使用的資料來源執行個體會自動建立。 已產生程式碼來具現化每個 DataTable (及其[資料集](http://msdn.microsoft.com/library/system.data.dataset\(v=vs.100\).aspx)容器)。 [ObjectDataSource](http://msdn.microsoft.com/library/system.web.ui.webcontrols.objectdatasource\(v=vs.100\).aspx)控制項會加入至設計介面，對應每個報表中使用的資料來源。 此資料來源控制項會自動設定。  
  
     如果您使用 Microsoft Visual Studio 2012，請確定 ObjectDataSource 控制項結合完整專案命名空間的 dataset1，如果完整的名稱列示在**選擇您的商務物件**下拉式清單方塊 (例如 Projectnamespace.DataSet1TableAdapters.ProductTableAdapter)。 您可以存取清單方塊，以滑鼠右鍵按一下 [ObjectDataSource]，然後按一下**設定資料來源**。  
