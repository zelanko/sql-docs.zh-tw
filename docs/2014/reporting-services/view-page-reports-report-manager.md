...
## <a name="generating-data-feeds-from-report-data"></a>從報表資料產生資料摘要  
 您可以從報表產生資料摘要。 
  [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] Atom 轉譯延伸模組會產生兩份符合 Atom 規範的文件：列出報表所提供之資料摘要的 Atom 服務文件，以及列出包含報表資料之資料摘要的 Atom 服務文件。 
  [!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)] 會使用符合 Atom 1.0 規範的標準化格式來產生這些資料摘要，而且您可以使用取用符合 Atom 規範之資料摘要的應用程式來讀取與交換這些資料摘要。 例如， [!INCLUDE[ssGemini](../includes/ssgemini-md.md)] 用戶端可以取用從報表產生的資料摘要。  
