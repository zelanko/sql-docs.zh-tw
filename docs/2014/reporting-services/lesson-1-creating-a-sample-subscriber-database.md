...
## <a name="next-steps"></a>後續步驟  
 您已順利建立驅動報表散發作業的訂閱資料，且每位訂閱者的報表輸出都各不相同。 之後，您要修改將散發給各訂閱者之報表的資料來源屬性。 修改資料來源屬性是為了準備資料驅動訂閱所傳遞的報表。 您也會將報表設計修改為包含訂閱將搭配訂閱者資料使用的參數。 [第 2 課：修改報表資料來源屬性](lesson-2-modifying-the-report-data-source-properties.md)。  
