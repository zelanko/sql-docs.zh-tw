...
1.  在 [報表資料] 窗格中，以滑鼠右鍵*@StoreID*按一下參數，然後按一下 [**參數屬性**]。  
...
9. 預覽報表。  
     ...
     在報表檢視器工具列中，參數文字方塊現在是顯示** \<選取值>** 的下拉式清單。  
...
2.  在 [報表資料] 窗格中，以*@StoreID*滑鼠右鍵按一下 []，然後按一下 [**參數屬性**]。  
...
6.  在`equals`查詢中最後一行`IN`的[!INCLUDE[tsql](../includes/tsql-md.md)] `WHERE`子句中，將（=）變更為：  
