...
ms.locfileid: "37210148"
---
# <a name="canceling-commands-xmla"></a>取消命令 (XMLA)
  視發出命令之使用者的系統管理權限而定[取消](../xmla/xml-elements-commands/cancel-element-xmla.md)命令在 XML for Analysis (XMLA) 可以取消工作階段、 工作階段、 連接、 伺服器處理序或相關聯的工作階段上的命令或連接。  
...
## <a name="canceling-sessions"></a>取消工作階段  
 藉由指定明確的工作階段中的工作階段識別項[SessionID](../xmla/xml-elements-properties/id-element-xmla.md)屬性`Cancel`命令時，資料庫管理員或伺服器系統管理員可以取消工作階段，包括目前執行的命令. 資料庫管理員只能取消他或她擁有管理權限的資料庫之工作階段。  
...
## <a name="canceling-connections"></a>取消連接  
 藉由指定的連接識別項[ConnectionID](../xmla/xml-elements-properties/connectionid-element-xmla.md)屬性`Cancel`命令時，伺服器系統管理員可以取消所有與指定的連接，包括所有執行的命令，相關聯的工作階段和取消連接。  
...
## <a name="canceling-server-processes"></a>取消伺服器處理序  
 藉由指定伺服器處理序識別碼 (SPID)，在[SPID](../xmla/xml-elements-properties/spid-element-xmla.md)屬性`Cancel`命令時，伺服器系統管理員可以取消與指定 SPID 相關聯的命令。  
  
## <a name="canceling-associated-sessions-and-connections"></a>取消關聯的工作階段和連接。  
 您可以設定[CancelAssociated](../xmla/xml-elements-properties/cancelassociated-element-xmla.md)屬性設為 true，若要取消連接、 工作階段及連線、 工作階段中或在指定的 SPID 相關聯的命令`Cancel`命令。  
