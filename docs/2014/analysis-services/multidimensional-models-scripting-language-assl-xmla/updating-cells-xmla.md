...
## <a name="specifying-cells"></a>指定資料格  
 命令的 Cell 屬性包含要更新的資料格。 [Cell](https://docs.microsoft.com/bi-reference/xmla/xml-elements-properties/cell-element-xmla) `UpdateCells` 您可以使用資料格的序數，來識別 `Cell` 屬性中的每個資料格。 就概念[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]而言，cube 中的資料格數目就如同 cube*是一維*陣列，其中*p*是軸的數目。 資料格的處理順序是以資料列為主。 下圖顯示計算資料格序數的公式。  
