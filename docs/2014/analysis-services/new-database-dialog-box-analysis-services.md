...
# <a name="new-database-dialog-box-analysis-services"></a>新增資料庫對話方塊 (Analysis Services)
  使用 ** 中的 [新增資料庫]**[!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 對話方塊，即可建立空的新 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 資料庫。 在物件總管**** 中，以滑鼠右鍵按一下 ** 執行個體的 [資料庫]**[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 資料夾，然後選取 [新增資料庫]****，即可顯示[新增資料庫]**** 對話方塊。  
...
|**使用者名稱**|輸入選取的 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 資料庫所要使用之使用者帳戶的網域和名稱。 請使用下列格式：<br /><br /> **\\** * \<功能變數名稱* *>\<使用者帳戶名稱>*<br /><br /> 注意：唯有在選取了 [使用特定的使用者名稱和密碼]**** 之後，才會啟用此選項。|  
