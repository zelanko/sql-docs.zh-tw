...
4.  在 [**銷售**領域] 維度和量值群組的交集處，按一下資料格，然後 `Sales Quotas` 按一下瀏覽按鈕（**...**）。[**定義關聯**性] 對話方塊隨即開啟。  
...
11. 在 [**銷售**領域] 維度和 [**銷售配額 1** ] 量值群組的交集處，按一下資料格，然後按一下瀏覽按鈕（**...**）。[**定義關聯**性] 對話方塊隨即開啟。  
...
20. 在 **...** `Sales Quotas` 量值群組和維度交集的資料格中，按一下瀏覽按鈕（...）。 `Date`  
