...
## <a name="how-to-read-a-classification-matrix"></a>如何解讀分類矩陣  
 例如，假設您的目標是設計客戶忠誠度方案，並接著將客戶指派給適當的類別，以便您可以提供適當等級的獎勵。 您已實作的獎勵計畫-銅級、 silver 和 gold-三個層級，而且分發給客戶在試用版的階段。 您也設計了用於分析客戶及預測正確類別的模型。 現在，您將針對測試資料使用分類矩陣，以判斷模型在預測所有客戶之適當供應項目的精確度。  
...
3.  選取您要評估其精確度的資料行。 建立矩陣時，可以只選擇一個資料行，但是該資料行可包含多個值。  
     ...
     提示：它可能難以解譯分類矩陣，如果您的可預測資料行有許多要比較的資料行。  
...
-   如果您未使用的選項，來儲存測試集，以及您的結構或模型，您需要取得已基本上相同數目的資料行，以及相符的資料型別，模型中所使用的輸入的資料集。  
