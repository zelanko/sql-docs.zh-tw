...
|**資料來源模擬資訊**|與資料庫中的資料來源進行連接和互動時，請選取資料庫使用的模擬資訊。 有效值如下：<br /><br /> **ImpersonateAccount** （使用特定的 Windows 使用者名稱和密碼）。<br /><br /> **ImpersonateService** （使用服務帳戶）。<br /><br /> **ImpersonateCurrentUser** （使用目前使用者的認證）。<br /><br /> **預設值**（使用服務帳戶進行 MOLAP 作業，將目前使用者用於資料採礦）。<br /><br /> 雖然您可以在資料庫層級進行資料來源模擬設定，不過這樣做只會影響針對其模擬設定指定 **[繼承]** 的這些資料來源。 直接針對資料來源指定的模擬設定一定會覆寫在資料庫層級指定的任何設定。<br /><br /> 選擇模擬選項時，請考慮需要系統支援的作業類型。 某些作業 (例如處理) 無法執行|  
