---
title: 分類精靈 （資料採礦增益集的 Excel） |Microsoft 文件
ms.custom: ''
ms.date: 12/29/2017
...
3.  在**分類**頁面上，選擇您想要分類的資料行。  
     ...
     檢閱 在清單中，資料行**輸入資料行**，並取消選取任何資料行具有唯一值，而不適合建立模式，例如身分證號碼、 客戶名稱等等。 您也應該將基本上與可分類資料行重複的資料行移除。  
...
6.  在**完成**頁面、 提供您的資料集和模型的描述性名稱，然後設定下列選項可控制如何使用完成的模型：  
    ...
    -   **瀏覽模型**。 選取此選項時，只要精靈完成處理模型，它會開啟**瀏覽**可協助您瀏覽結果 視窗。 檢視器的內容取決於建立的模型類型。 如需詳細資訊，請參閱[瀏覽決策樹模型](browsing-a-decision-trees-model.md)和[瀏覽類神經網路模型](browsing-a-neural-network-model.md)。  
...
## <a name="more-about-classification-models"></a>深入了解分類模型  
 在**演算法參數**對話方塊中，您也可以選擇在 Analysis Services 中提供這些演算法中的分類方法：  
...
-   Microsoft 類神經網路  
 ...
 在**參數** 清單中，您可以變更進階的選項，您所選擇的演算法類型而定。 每個演算法的參數在《SQL Server 線上叢書》中有更詳細的說明。  
...
### <a name="requirements"></a>需求  
 若要使用**分類**精靈 中，您必須連接到[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]資料庫。 如需如何建立的連接資訊，請參閱[連接至來源資料&#40;適用於 Excel 的資料採礦用戶端&#41;](connect-to-source-data-data-mining-client-for-excel.md)。  
