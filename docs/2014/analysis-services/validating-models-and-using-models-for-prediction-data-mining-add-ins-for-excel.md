---
title: 驗證模型及使用模型進行預測 （資料採礦增益集的 Excel） |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
 ...
 **精確度圖表**精靈可協助您建立預測查詢，並藉由建立增益圖或散佈圖圖表來評估資料採礦模型的效能。 增益圖有助於將結構中幾乎相同的模型加以區分，以協助您判斷哪一個模型的預測能力最佳。  
 ...
 **交叉驗證**精靈可讓您指定要分割資料的摺疊數，並提供以統計方式描述這些交叉區段之間差異的交叉驗證報表。 根據此報表，您可以判斷模型是否在所有定型資料上都順利執行，或者可能對特定子集有所偏差。  
 ...
 **查詢**精靈是互動式的工具，可協助您建立預測查詢。 查詢是用來產生建議、未來預測等等的方式。  
 ...
 在**查詢**精靈，您挑選模型，並做為單一值或從資料表或範圍，然後提供輸入的資料，以及精靈會協助您選取要輸出資料行。 您也可以將函數加入至查詢，以產生機率分數和其他有用的統計資料。  
  
 [查詢&#40;SQL Server 資料採礦增益集&#41;](query-sql-server-data-mining-add-ins.md)  
  
## <a name="advanced-query-editor"></a>進階查詢編輯器  
 **進階查詢編輯器**是一組互動式對話方塊，可協助您建立各種 DMX 陳述式，任何項目從執行自訂查詢到建立及定型新模型、 刪除模型，或建立新的資料設定。  
