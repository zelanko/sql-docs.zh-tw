---
title: 設定預存的認證，PowerPivot 資料重新整理 (PowerPivot for SharePoint) |Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
 ...
 **[!INCLUDE[applies](../includes/applies-md.md)]**  SharePoint 2010  
 ...
 [設定資料重新整理任何 Windows 帳戶](#configAny)  
...
13. 在 [成員] 中，加入下列使用者和群組帳戶：  
    ...
    2.  加入 PowerPivot 服務應用程式的應用程式集區識別，讓它能夠在資料重新整理排程為執行時擷取認證。 若要檢視應用程式集區身分識別，請前往**管理服務應用程式**（這會選取該列） 的名稱，旁邊的空白處，即可選取 PowerPivot 服務應用程式，然後按一下 **屬性**. 出現在此頁面的安全性帳戶就是要當做目標應用程式成員加入的帳戶。  
...
13. 在 [成員] 中，加入下列使用者和群組帳戶：  
    ...
    2.  加入將使用目標應用程式存取其預存認證之每個 PowerPivot 服務應用程式的應用程式集區識別。 若要檢視識別，請前往**管理服務應用程式**（這會選取該列） 的名稱，旁邊的空白處，即可選取 PowerPivot 服務應用程式，然後按一下**屬性**。 出現在此頁面的安全性帳戶就是您要當做目標應用程式成員加入的帳戶。  
