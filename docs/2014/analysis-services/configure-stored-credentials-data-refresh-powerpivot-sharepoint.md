---
title: 設定預存的認證，PowerPivot 資料重新整理 (PowerPivot for SharePoint) |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
 ...
 **[!INCLUDE[applies](../includes/applies-md.md)]**  SharePoint 2010  
 ...
 本主題說明如何設定 SharePoint 2010 伺服器陣列中用於 PowerPivot 資料重新整理的使用者名稱和密碼。 您必須先啟用 Secure Store Service 並產生主要金鑰，然後才能使用這些步驟。 如需詳細資訊，請參閱[PowerPivot Data Refresh with SharePoint 2010](powerpivot-data-refresh-with-sharepoint-2010.md)  
  
 本主題包含下列幾節：  
  
 [設定資料重新整理的任何 Windows 帳戶](#configAny)  
  
 [設定預先定義的帳戶來存取外部或協力廠商資料來源](#config3rd)  
  
 如果您有設定，或使用資料重新整理的問題，請參閱[疑難排解 PowerPivot 資料重新整理](http://go.microsoft.com/fwlink/?LinkID=223279)可行的解決方案 TechNet wiki 上的頁面。  
  
##  <a name="configAny"></a> 設定資料重新整理的任何 Windows 帳戶  
...
13. 在 [成員] 中，加入下列使用者和群組帳戶：  
    ...
    2.  加入 PowerPivot 服務應用程式的應用程式集區識別，讓它能夠在資料重新整理排程為執行時擷取認證。 若要檢視應用程式集區身分識別，請移至**管理服務應用程式**（這會選取該列） 的名稱旁邊的空白處，即可選取 PowerPivot 服務應用程式，然後按一下  **屬性**. 出現在此頁面的安全性帳戶就是要當做目標應用程式成員加入的帳戶。  
  
    3.  加入將在資料重新整理排程中輸入此目標應用程式的 Windows 使用者和群組帳戶。  
  
14. 按一下 [確定] 。  
  
15. 選取您剛才建立的目標應用程式，按一下向下箭號並選取**設定認證。**  
...
###  <a name="bkmk_verify"></a> 步驟 4： 確認帳戶可用性資料重新整理組態頁面  
...
2.  確認**使用儲存在 Secure Store Service (SSS) 中的認證來登入的資料來源連接**選項中的資料重新整理組態頁面中，已啟用，然後輸入 目標應用程式的名稱。  
...
##  <a name="config3rd"></a> 設定預先定義的帳戶來存取外部或協力廠商資料來源  
 ...
 連接字串中必須包含使用者名稱及密碼，此步驟才能運作。 請注意，在連接字串中包含認證十分罕見，因此您在使用此選項時也會有所限制。 在大多數的情況下，您若是使用資料庫驗證來連接資料來源，則連接字串中將只會包含使用者識別碼及密碼。 如需如何檢查連接字串是否包含使用者 ID 和密碼的詳細資訊，請參閱中的 「 授與建立排程及存取外部資料的權限 」 一節[PowerPivot Data Refresh with SharePoint 2010](powerpivot-data-refresh-with-sharepoint-2010.md).  
...
10. 在**指定安全存放目標應用程式的認證欄位**頁面上，接受預設值，只有當資料來源使用 Windows 驗證。 否則，請選擇適用於資料來源的欄位類型，然後編輯欄位名稱以符合此類型。  
...
13. 在 [成員] 中，加入下列使用者和群組帳戶：  
    ...
    2.  加入將使用目標應用程式存取其預存認證之每個 PowerPivot 服務應用程式的應用程式集區識別。 若要檢視識別，移至**管理服務應用程式**（這會選取該列） 的名稱旁邊的空白處，即可選取 PowerPivot 服務應用程式，然後按一下 **屬性**。 出現在此頁面的安全性帳戶就是您要當做目標應用程式成員加入的帳戶。  
