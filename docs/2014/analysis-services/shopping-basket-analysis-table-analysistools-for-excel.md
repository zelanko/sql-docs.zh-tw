---
title: 購物籃分析 (適用於 Excel 的資料表 AnalysisTools) |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
 ...
 **購物籃分析**工具可協助您找出`associations`資料中。 關聯可以告訴您通常會同時購買的項目。 在資料採礦中，這項技術是知名的方法，稱為*購物籃分析*，用來分析資料集過大的客戶的購買行為。 商人可能會利用這項資訊，為客戶製造建議的相關產品，並將這些產品放在網頁上、目錄中或貨架上非常接近的地方來進行促銷。  
  
 若要使用購物籃分析，您要分析的項目必須透過交易識別碼產生關聯。 例如，如果要分析透過網站收到的所有訂單，每張訂單都會有一個與一或多個購買項目相關聯的訂單識別碼或交易識別碼。  
  
 當精靈完成分析資料時，它會建立兩個新的工作表，**購物籃項目群組**和**購物籃規則**。  
  
 **購物籃項目群組**工作表包含經常一起出現在交易中的項目清單。 這些常見的群組稱為*項目集*。 工作表也包含統計資料，例如*支援*和*提起*，以協助您了解的項目集重要性。 如果有價格資訊，此工作表也可以針對所有相關的項目建立值的總和，以提供交易總值的指標。  
  
 您可以在報表中篩選與排序資料行。 例如，您可能想要檢視只能搭配 2 或多個產品，這些項目集或排序的項目集**購物籃平均價值**。  
  
 **購物籃規則**工作表使用衍生自分析的統計資料來建立有關項目關聯方式的規則。 例如，規則可能是，如果客戶購買產品 A，則很可能購買產品 B。規則可以用來產生建議。 每個規則都有支援的統計資料來協助您評估規則的潛在強度，讓您可以只在規則超出特定機率臨界值時才提出建議。  
...
5.  按一下**進階**開啟**進階參數設定** 對話方塊。 增加的值**最小支援**以減少分組為項目集的產品數目。 增加**最小規則機率**篩選出很常見的項目集。  
 ...
 *最小支援*表示必須包含特定項目集視為重要項目集的交易數目。 例如，除非已經在至少 10 個交易中購買某個項目集，否則您可能不會有興趣。 有兩種方式來控制項目集重要性的臨界值，並同時使用**最小支援**參數。  
  
 **當是絕對值：** 輸入的數字，代表包含目標項目之交易的計數。 例如，如果您輸入 10，出現在至少 10 個購物籃中的任何項目集都會包含在結果中。  
  
 **以百分比表示：** 輸入整個集合的項目集的百分比表示的數字。 例如，如果您指定 10，系統會計算所有項目集，而且鎖定為目標的項目集必須至少顯示為項目集總數的 10%。 如果您有非常大的資料集，使用百分比 (而非計數) 可以協助您將重點放在最重要的項目群組上。  
 ...
 **購物籃分析**工具會建立兩個互補的報表。 第一個報表稱為**大量群組的項目會識別在分析期間**，提供的所有項目集找不到清單。 您可以使用 Microsoft Excel 中的新資料表工具來排序、篩選與瀏覽資料。  
  
 第二個報表稱為**購物籃規則**，告訴您哪種推斷可根據第一份報表中所列的項目集。 項目集的清單在瀏覽和了解資料時比較實用，而規則的清單則在進行預測與提出建議時比較實用。  
  
### <a name="shopping-basket-item-groups-report"></a>購物籃項目群組報表  
 此報表包含資料集中找出之所有可能項目組合的清單。 例如，如果您的交易資料包含每筆訂單的訂單，**購物籃分析**工具會計算的次數個別項目已排序，然後計算與其他項目，該項目的所有組合。  
...
|報表中的資料行|提供的資訊|  
|----------------------|-----------------------|  
|現有的項目|列出提出建議時所需的項目。<br /><br /> 在資料採礦中，這些項目據說是*左側*關聯規則。|  
|預測的項目|列出要建議的項目。<br /><br /> 在資料採礦中，這些項目據說是*右側*關聯規則。|  
|機率|顯示此規則正確的機率。|  
...
## <a name="related-tools"></a>相關工具  
 適用於 Excel 的資料採礦用戶端是一個獨立的增益集，它提供更進階的資料採礦功能，也包含執行關聯分析的精靈。 如需詳細資訊，請參閱[關聯精靈&#40;適用於 Excel 的資料採礦用戶端&#41;](associate-wizard-data-mining-client-for-excel.md)。  
