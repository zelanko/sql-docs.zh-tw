 ...
 **注意：** 確認已啟用資料收集的使用量。 若要確認，請移至 [SharePoint 管理中心] 的 **[監視]** 。 如需詳細資訊，請參閱 <<c0> [ 設定使用量資料收集的&#40;PowerPivot for SharePoint](configure-usage-data-collection-for-power-pivot-for-sharepoint.md)。</c0>  
...
|資料重新整理|PowerPivot 服務為已排程的資料更新所起始的資料重新整理作業。 在應用程式層級會為可操作的報表收集資料重新整理的使用量記錄，而且會反映在個別活頁簿的 [管理資料重新整理] 頁面。<br /><br /> **注意：** 針對[!INCLUDE[ssSQL11SP1](../../includes/sssql11sp1-md.md)]和 SharePoint 2013 部署而言，資料重新整理由 Excel Services 並不是 Analysis Services 伺服器。|資料分割 如果您啟用 PowerPivot 服務應用程式的資料重新整理，永遠都會收集資料重新整理使用量資料。|  
