---
title: 設定或修復 PowerPivot for SharePoint 2013 （PowerPivot 組態工具） |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
-   若要執行組態工具的一般需求[PowerPivot 組態工具](power-pivot-configuration-tools.md)。  
...
> [!TIP]  
>  如果組態工具掃描電腦並於左側窗格傳回空白的工作清單，就表示沒有偵測到任何需要設定的功能或設定。 若要修改 SharePoint 或 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 組態，請使用 Windows PowerShell 或 SharePoint 管理中心的管理頁面。 如需詳細資訊，請參閱[管理中心的 PowerPivot 伺服器管理和組態](power-pivot-server-administration-and-configuration-in-central-administration.md)。  
...
1.  上**啟動**功能表上，指向**所有程式**，按一下  [!INCLUDE[ssCurrentUI](../../includes/sscurrentui-md.md)]，按一下 **組態工具**，然後按一下  **PowerPivot for SharePoint 2013組態**。  
...
||[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] for Excel Services ([伺服器名稱]\ [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)])|使用者輸入|[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)]則需要啟用核心的 PowerPivot 功能的 Excel services 伺服器。 您在此頁面上輸入的伺服器名稱也會加入至 **[設定 PowerPivot 伺服器]** 頁面上的清單。|  
...
||檔案大小上限 (MB)|預設值 (如果需要)|預設值是 2047。 SharePoint 文件庫也有大小上限，而 [!INCLUDE[ssGemini](../../includes/ssgemini-md.md)] 設定不應超過文件庫設定。 如需詳細資訊，請參閱[設定檔案上傳大小上限&#40;PowerPivot for SharePoint&#41;](configure-maximum-file-upload-size-power-pivot-for-sharepoint.md)。|  
...
||無人看管帳戶的使用者名稱和密碼|預設值 (如果需要)|輸入目標應用程式用來執行無人看管之資料重新整理的 Windows 使用者帳戶認證。 如需詳細資訊，請參閱[的 SharePoint Server 2013 中使用自動的服務帳戶設定 Excel Services 資料重新整理](http://technet.microsoft.com/library/hh525344\(office.15\).aspx)(http://technet.microsoft.com/library/hh525344(office.15).aspx)。|  
...
## <a name="verify-the-configuration"></a>確認組態  
 請參閱 「 確認[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)]組態 」 一節[設定 PowerPivot 及部署方案&#40;SharePoint 2013&#41;](../instances/install-windows/configure-power-pivot-and-deploy-solutions-sharepoint-2013.md)。  
...
1.  在 SharePoint 2013 網站中，按一下 [設定] 圖示![SharePoint 設定](../media/as-sharepoint2013-settings-gear.gif "SharePoint 設定")，然後按一下 **站台設定**。  
...
-   [何謂 PowerPivot for Excel 2013 中的新](http://office.microsoft.com/excel-help/whats-new-in-powerpivot-in-excel-2013-HA102893837.aspx?CTT=1)(http://office.microsoft.com/excel-help/whats-new-in-powerpivot-in-excel-2013-HA102893837.aspx?CTT=1)。  
  
-   [在 Excel 2013 增益集中啟動 PowerPivot](http://office.microsoft.com/excel-help/start-powerpivot-in-excel-2013-add-in-HA102837097.aspx?CTT=5&origin=HA102837110) (http://office.microsoft.com/excel-help/start-powerpivot-in-excel-2013-add-in-HA102837097.aspx?CTT=5&origin=HA102837110)。  
