---
title: 將模型加入結構 （資料採礦增益集的 Excel） |Microsoft 文件
ms.custom: ''
ms.date: 12/29/2017
...
ms.locfileid: "36131609"
---
# <a name="add-model-to-structure-data-mining-add-ins-for-excel"></a>將模型加入結構 (適用於 Excel 的資料採礦增益集)
  ![將模型加入到結構按鈕](media/dmc-addmodel.gif "將模型加入結構按鈕")  
  
 當您按一下**將模型加入結構**，啟動精靈，可協助您建立新的採礦模型，以搭配現有的採礦結構。 這個選項可讓您比較以相同資料為基礎的模型，或是建立自訂的模型，因此會很有用。  
  
 如果 Analysis Services 執行個體尚未包含所需的資料，使用[Create Mining Structure &#40;SQL Server 資料採礦增益集&#41;](create-mining-structure-sql-server-data-mining-add-ins.md)精靈來設定採礦結構。 或者，您可以啟動其中一個模型精靈，然後將新模型加入由精靈所建立的結構中。  
  
 若要建立進階的模型使用精靈不支援的演算法，來建立採礦結構，並將模型使用**資料採礦進階查詢編輯器**。  
  
## <a name="add-a-new-model-to-an-existing-structure"></a>將新模型加入現有的結構  
  
1.  在**資料採礦**功能區中，按一下底下的箭頭**進階**，然後選取**將模型加入結構**。  
  
2.  在**選取結構**對話方塊方塊中，選擇包含您想要使用，然後按一下 的資料結構**下一步**。  
  
     **提示**： 如果您不確定哪個採礦結構包含所需的資料，使用**文件模型**精靈，以檢視資料行和資料的相關基本統計資料。  
...
6.  在**選取資料行**對話方塊中，檢閱資料行清單，如有必要，變更資料行的使用方式的其中一個這些值：  
    ...
    -   **索引鍵**。 每個模型至少需要一個索引鍵。 根據模型類型，您可能也可以選擇其他特殊索引鍵，例如 **[sequencekey]** 或**TimeKey**。  
  
    -   **請勿使用**。 表示即使在結構中也不應在模型內使用的資料。  
  
7.  按一下瀏覽 **（...）**  按鈕以開啟**設定資料行模型旗標** 對話方塊。  
     ...
     如果結構的資料行不適用於新演算法，您可以略過它們選取**不要使用**。  
  
8.  在**設定資料行模型旗標** 對話方塊中，檢閱或設定模型旗標，如果有的話。  
...
9. 在**完成**對話方塊方塊中，輸入名稱和新的採礦模型的描述。  
    ...
    -   使用從模型到來源資料的鑽研。  
         ...
         如需詳細資訊，請參閱[採礦模型上的鑽研](data-mining/drillthrough-on-mining-models.md)。  
...
|選項|註解|  
|------------|--------------|  
|**選取結構或模型**對話方塊|選擇現有的採礦結構，以做為建立新模型的基礎。  您挑選的結構必須位於目前的連接中。 如果沒有，請將連線變更為使用[連接至來源資料&#40;適用於 Excel 的資料採礦用戶端&#41;](connect-to-source-data-data-mining-client-for-excel.md)工具。|  
...
### <a name="requirements"></a>需求  
 若要建立資料採礦模型，您必須具有 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 執行個體的連接。 如需如何建立或變更連線的詳細資訊，請參閱[連接至來源資料&#40;適用於 Excel 的資料採礦用戶端&#41;](connect-to-source-data-data-mining-client-for-excel.md)。  
  
 如果您看不到所需的資料採礦結構，可能是該結構儲存在不同的執行個體或不同的 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 資料庫中。 如需如何變更為不同的資料採礦連接資訊，請參閱[連接到資料採礦伺服器](connect-to-a-data-mining-server.md)。  
