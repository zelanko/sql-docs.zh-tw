...
1.  使用「自動加總」功能建立並命名下列量值︰  
    ...
    |Internet Total Discount Amount|折扣量|Sum|=SUM([Discount Amount])|  
    |Internet Total Product Cost|產品總成本|Sum|=SUM([Total Product Cost])|  
    |Internet Total Sales|銷售量|Sum|=SUM([Sales Amount])|  
    |Internet Total Margin|Margin|Sum|=SUM([Margin])|  
    |Internet Total Tax Amt|稅額|Sum|=SUM([Tax Amt])|  
