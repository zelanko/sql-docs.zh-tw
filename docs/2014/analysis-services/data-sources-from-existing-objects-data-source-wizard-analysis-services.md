 ...
 **依據 Analysis Services 專案建立資料來源。**  
 選取即可建立參考 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 目前方案中另一個專案的新資料來源 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 。 新資料來源會從所選取專案的 `TargetServer` 和 `TargetDatabase` 屬性中取得設定。 建立、重新整理，或部署了使用新資料來源的專案之後，新的資料來源就會從您選取此選項時所指定的資料來源取得設定。  
