...
###  <a name="bkmk_remote"></a> 啟用遠端管理  
 您必須先啟用遠端管理和檔案共用，然後才能使用 Analysis Services PowerShell 搭配遠端 Analysis Services 執行個體。 下列錯誤表示防火牆組態問題：「RPC 伺服器無法使用。 (來自 HRESULT 的例外狀況: 0x800706BA)」。  
...
## <a name="see-also"></a>另請參閱  
 ...
 [管理表格式的模型使用 PowerShell （部落格）](http://go.microsoft.com/fwlink/?linkID=227685)   
 [設定 Internet Information Services 上的 HTTP 存取 Analysis Services &#40;IIS&#41; 8.0](instances/configure-http-access-to-analysis-services-on-iis-8-0.md)  
