...
|**輪詢方格**|包含 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 使用的輪詢查詢、處理查詢和資料表識別碼，以輪詢資料來源和識別物件之基礎資料表的變更。 方格包含下列資料行：<br /><br /> **輪詢查詢**： 輸入以識別該物件，變更輪詢間隔執行的單一查詢，或按一下省略符號按鈕 (**...**) 以開啟**建立輪詢查詢**對話方塊方塊中，並定義單一查詢。 如需詳細資訊，請參閱[建立輪詢查詢對話方塊 &#40;Analysis Services - 多維度資料&#41;](create-polling-query-dialog-box-analysis-services-multidimensional-data.md)。 如果選取 [啟用累加式更新]，則輪詢查詢應傳回會識別加入 [資料表] 中所識別資料表的最後一筆記錄的值。 如果未選取 [啟用累加式更新]，則輪詢查詢應傳回會識別資料表中之目前記錄數目的值。<br /><br /> **處理查詢**： 輸入從識別的資料表中擷取新記錄的輪詢間隔執行的查詢**表格**，或按一下省略符號按鈕 (**...**) 以開啟**建立處理查詢**對話方塊方塊中，並定義查詢。 如需詳細資訊，請參閱[建立處理查詢對話方塊 &#40;Analysis Services - 多維度資料&#41;](create-processing-query-dialog-box-analysis-services-multidimensional-data.md)。 應該參數化查詢以接受兩個參數—[輪詢查詢] 中之查詢所傳回的上一個值與 [輪詢查詢] 中之查詢所傳回的目前值—可用來識別和擷取輪詢期間加入的記錄。 請注意，只有當選取 [啟用累加式更新] 時，才能啟用此選項。<br /><br /> **資料表**： 輸入的識別碼依據資料表中的查詢**輪詢查詢**追蹤最後一筆記錄，或按一下省略符號按鈕 (**...**) 以開啟**尋找資料表** 對話方塊中，並選取資料表。 如需詳細資訊，請參閱[尋找資料表對話方塊 &#40;Analysis Services - 多維度資料&#41;](find-table-dialog-box-analysis-services-multidimensional-data.md)。|  
