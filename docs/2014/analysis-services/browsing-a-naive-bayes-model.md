...
##  <a name="BKMK_Tabs"></a> 瀏覽模型  
 ...
 如果您想要試驗貝氏機率分類檢視器，使用[分類精靈&#40;適用於 Excel 的資料採礦增益集&#41;](classify-wizard-data-mining-add-ins-for-excel.md)精靈在資料採礦功能區中，按一下 **進階**選項，然後變更演算法使用貝氏機率分類演算法  
  
 如需這些範例中，我們使用範例活頁簿中的來源資料和分組資料行， **Yearly Income**，五個收入群組，從**Very Low**至**非常高**。 然後，貝氏機率分類模型便分析了與每個收入類別目錄相互關聯的因數。  
  
###  <a name="bkmk_DepNet"></a> 相依性網路  
 您將使用第一個視窗是**相依性網路**。 它會概略顯示哪些輸入與選取的結果密切相關。  
  
 ![貝氏機率分類檢視器中的相依性網路](media/dm13-nb.gif "貝氏機率分類檢視器中的相依性網路")  
  
##### <a name="explore-the-dependency-network"></a>探索相依性網路  
  
1.  首先，請按一下目標結果**Yearly Income**，它會呈現為圖表中的節點。  
...
3.  篩選圖形之後，請按一下按鈕，**複製圖表檢視**。 然後，選取 Excel 中的工作表，並且按下 Ctrl+V。  
 ...
 **屬性設定檔**windows 會提供如何所有其他變數與相關的個別結果的視覺指示。  
  
##### <a name="explore-the-profiles"></a>探索設定檔  
  
1.  若要隱藏部分值，讓您能夠更輕易地比較結果，請按一下資料行標題並將它拖曳到其他資料行底下。  
  
     ![屬性在貝氏機率分類檢視器中的設定檔](media/dm13-nb-attprof.gif "屬性貝氏機率分類檢視器中的設定檔")  
  
2.  按一下任何資料格檢視中的值分佈**採礦圖例**。  
  
     因為與不同結果相關聯的屬性會以視覺效果方式顯示，所以您可以輕鬆地識別感興趣的相互關聯，例如各地區的收入分佈狀況。  
  
3.  若要取得的資料，此檢視，請按一下**複製至 Excel**。 系統就會在新的工作表中產生資料表，其中顯示個別屬性與結果之間的相互關聯。 在這個 Excel 資料表中，您可以輕鬆地隱藏或篩選資料行。  
 ...
 **屬性特性**檢視對於深入檢查特定結果變數和影響因數有用處。  
 ...
 ![屬性在貝氏機率分類檢視器中的特性](media/dm13-nb-viewer.gif "屬性貝氏機率分類檢視器中的特性")  
...
2.  按一下**複製至 Excel**來列出與選取之目標結果相關的所有屬性分數的新工作表中建立的資料表。  
  
 [回到頁首](#BKMK_Tabs)  
  
###  <a name="bkmk_AttDisc"></a> 屬性辨識  
 **屬性辨識**檢視有助於比較兩個結果或與所有其他結果的其中一個結果。  
  
 ![屬性辨識貝氏機率分類檢視器中](media/dm13-nb-attdisc.gif "屬性辨識貝氏機率分類檢視器中")  
  
##### <a name="explore-attribute-discrimination"></a>探索屬性辨識  
  
1.  使用的控制項，**值 1**和**值 2**來選取您想要比較的結果。  
  
     例如，在此模型中一些有趣的屬性中沒有的低收入群組，因此我們在第一個下拉式清單中選擇了最低收入群組，並選擇**所有其他狀態**第二個下拉式清單中。  
