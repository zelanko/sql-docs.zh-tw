...
# <a name="apply-prediction-functions-to-a-model"></a>將預測函數套用至模型
 ...
 在您選取模型之後，請將 [預測函數]** 加入查詢中。 匯入以瞭解預測函數用於許多用途-是，您可以預測值，但也可以取得相關的統計資料，以及用於產生預測的資訊。 預測函數可以傳回以下類型的值：  
...
9. 按一下 [切換到查詢文字檢視]**** 來檢閱產生的 DMX 陳述式，然後按一下 [切換到查詢結果檢視]**** 來查看預測查詢的資料行輸出。  
 ...
 如果您想要在語句中的其他位置新增其他子句（例如，如果您想要加入 WHERE 子句），則無法使用方格來加入它。您必須先切換到 DMX view。  
