 ...
 **快取定型案例** ：鑽研藉由擷取採礦結構中定型案例的相關資訊來運作。 這項資訊會在處理結構時快取。 因此，如果您選擇要清除所有快取的資料變更<xref:Microsoft.AnalysisServices.MiningStructureCacheMode>屬性設`ClearAfterProcessing`，鑽研將不會運作。  
