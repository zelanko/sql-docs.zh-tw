 ...
 **內容查詢**  
 ...
 [正在抓取模型的週期性提示](#bkmk_Query1)  
  
 [正在抓取 ARIMA 模型的方程式](#bkmk_Query2)  
  
 [正在抓取 ARTxp 模型的方程式](#bkmk_Query3)  
  
 **預測查詢**  
  
 [瞭解何時要取代和何時擴充時間序列資料](#bkmk_ReplaceExtend)  
  
 [使用 EXTEND_MODEL_CASES 進行預測](#bkmk_EXTEND)  
  
 [使用 REPLACE_MODEL_CASES 進行預測](#bkmk_REPLACE)  
...
###  <a name="bkmk_Query1"></a>範例查詢1：抓取模型的週期性提示  
...
###  <a name="bkmk_Query2"></a>範例查詢2：抓取 ARIMA 模型的方程式  
...
###  <a name="bkmk_Query3"></a>範例查詢3：抓取 ARTXP 模型的方程式  
...
###  <a name="bkmk_ReplaceExtend"></a>瞭解取代和擴充作業的行為  
...
-   **取代：** 當您取代資料數列中的資料時， [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]會保留定型的模型，但是會使用新的資料值來取代部分或所有現有的定型案例。 因此，定型資料的大小絕對不會改變，但是案例本身則會持續被更新的資料所取代。 如果您提供足夠的新資料，您可以使用完全新的序列來取代定型資料。  
...
###  <a name="bkmk_EXTEND"></a>使用 EXTEND_MODEL_CASES 進行預測  
...
-   在時間序列模型上建立 `PREDICTION JOIN`，並指定這三個月之新資料的來源。  
...
###  <a name="bkmk_REPLACE"></a>使用 REPLACE_MODEL_CASES 進行預測  
 ...
 例如，假設您在包含 1000 個資料列的案例資料集上定型您的模型， 然後您加入 100 列的新資料。 
  [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 會從定型集中卸除前 100 個資料列，並將 100 列的新資料加入一共 1000 個資料列的集合結尾。 如果您加入 1100 列的新資料，只會使用最近的 1000 個資料列。  
...
|-|-|  
|預測函數|使用量|  
|[DMX&#41;的 Lag &#40;](/sql/dmx/lag-dmx)|傳回目前案例的日期與定型集的最後日期之間的時間配量數目。<br /><br /> 這個函數的典型用法就是用來識別最近的定型案例，好讓您可以擷取有關案例的詳細資料。|  
|[DMX&#41;的 PredictNodeId &#40;](/sql/dmx/predictnodeid-dmx)|傳回指定之可預測資料行的節點識別碼。<br /><br /> 這個函數的典型用法就是用來識別產生特定預測值的節點，好讓您可以檢閱與此節點有關的案例，或擷取方程式和其他詳細資料。|  
|[DMX&#41;的 PredictStdev &#40;](/sql/dmx/predictstdev-dmx)|傳回指定之可預測資料行中預測的標準差。<br /><br /> 此函數會取代 INCLUDE_STATISTICS 引數，時間序列模型不支援這個引數。|  
|[DMX&#41;的 PredictVariance &#40;](/sql/dmx/predictvariance-dmx)|傳回指定之可預測資料行中預測的變異數。<br /><br /> 此函數會取代 INCLUDE_STATISTICS 引數，時間序列模型不支援這個引數。|  
|[DMX&#41;的 PredictTimeSeries &#40;](/sql/dmx/predicttimeseries-dmx)|傳回時間序列的預測記錄值或未來預測值。<br /><br /> 您也可以使用一般預測函數 [Predict &#40;DMX&#41;](/sql/dmx/predict-dmx) 來查詢時間序列模型。|  
