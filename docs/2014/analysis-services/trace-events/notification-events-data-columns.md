...
|EventSubclass|@shouldalert|@shouldalert|事件子類別提供有關每個事件類別的額外資訊。 以下是有效的子類別識別碼子類別名稱組：<br /><br /> 0：主動式快取開始<br />1：主動式快取結束<br />2：已啟動飛行記錄器<br />3：已停止飛行記錄器<br />4：已更新組態屬性<br />5：SQL 追蹤<br />6：已建立物件<br />7：已刪除物件<br />8：已改變物件<br />9：主動式快取輪詢開始<br />10：主動式快取輪詢結束<br />11：飛行記錄器快照集開始<br />12：飛行記錄器快照集結束<br />13：主動式快取：可通知物件已更新<br />14：延遲處理：開始處理<br />15：延遲處理：已完成處理<br />16：SessionOpened 事件開始<br />17：SessionOpened 事件結束<br />18：SessionClosing 事件開始<br />19：SessionClosing 事件結束<br />20：CubeOpened 事件開始<br />21：CubeOpened 事件結束<br />22：CubeClosing 事件開始<br />23：CubeClosing 事件結束<br />24：已要求交易中止|  
...
|IntegerData|10|@shouldalert|包含與通知事件相關聯的整數資料。 當 EventSubclass 資料行為 8 時，其值如下：<br /><br /> 1 = 已建立<br /><br /> 2 = 已刪除<br /><br /> 3 = 已變更物件的屬性<br /><br /> 4 = 已變更物件之子系的屬性<br /><br /> 6 = 已加入子系<br /><br /> 7 = 已刪除子系<br /><br /> 8 = 已完全處理物件<br /><br /> 9 = 已部份處理物件<br /><br /> 10 = 未處理物件<br /><br /> 11 = 完全最佳化物件<br /><br /> 12 = 部分最佳化物件<br /><br /> 13 = 物件未最佳化|  
|ObjectID|11|8|包含為其發出此通知的物件識別碼；這是一個字串值。|  
|ObjectType|12|@shouldalert|包含與通知事件相關聯的物件類型。|  
...
|ConnectionID|25|@shouldalert|包含與通知事件相關聯的唯一連接識別碼。|  
...
|SPID|41|@shouldalert|包含伺服器處理序識別碼 (SPID)，它會唯一識別與通知事件相關聯的使用者工作階段。 SPID 直接對應到 XMLA 使用的工作階段 GUID。|  
...
|---------------------|-------------------|---------------------|----------------------------|  
|EventClass|0|@shouldalert|事件類別用於將事件分類。|  
|EventSubclass|@shouldalert|@shouldalert|提供有關每個事件類別的額外資訊的特定使用者事件子類別。|  
|CurrentTime|2|5|包含通知事件的目前時間 (如果有的話)。 篩選所需的格式為 'YYYY-MM-DD' 與 'YYYY-MM-DD HH:MM:SS'。|  
|IntegerData|10|@shouldalert|特定的使用者定義事件資訊。|  
|ConnectionID|25|@shouldalert|包含與通知事件相關聯的唯一連接識別碼。|  
|DatabaseName|28|8|包含發生通知事件之資料庫的名稱。|  
|NTUserName|32|8|包含與通知事件相關聯的 Windows 使用者名稱。|  
|NTDomainName|33|8|使用者所隸屬的 Windows 網域。|  
