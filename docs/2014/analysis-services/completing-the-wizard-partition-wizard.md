---
title: 正在完成精靈 （資料分割精靈） |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
|**從現有的分割區複製彙總設計**|從量值群組的現有資料分割中，將彙總設計複製到新的資料分割。 按一下此選項，讓 [複製來源] 選項可以使用。 使用 [複製來源] 選項，即可選取要複製彙總設計的資料分割。<br /><br /> 請注意，可能會在未來的合併的資料分割必須有相同的資料表結構和彙總設計。 如果您可能在量值群組中將新的資料分割與現有的資料分割進行合併，則應將現有之資料分割的彙總設計複製到新的資料分割中。|  
