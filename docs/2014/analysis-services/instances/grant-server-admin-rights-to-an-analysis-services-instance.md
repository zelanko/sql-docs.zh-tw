---
title: 授與伺服器系統管理員權限 (Analysis Services) |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
2.  按一下 **[選取頁面]** 窗格中的 **[安全性]** ，然後按一下頁面底部的 **[加入]** ，以將一個或多個 Windows 使用者或群組加入至伺服器角色。  
     ...
     ![新增使用者對話方塊，在 management studio](../media/ssas-serveradminadd.png "新增在 management studio [使用者] 對話方塊")  
  
 在安裝期間，SQL Server 安裝程式會要求您至少指定一個使用者帳戶做為 Analysis Services 系統管理員。  
  
 依預設，也會將 Analysis Server 的管理權限授與本機 Administrators 群組的成員。 雖然本機群組未明確授與 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 伺服器管理員角色的成員資格，但本機管理員可以建立資料庫，新增使用者和權限，以及執行系統管理員可執行的其他工作。 這個行為是可設定的， 它由`BuiltinAdminsAreServerAdmins`伺服器屬性設定為**true**預設。 您可以在 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]中變更這個屬性。 如需詳細資訊，請參閱 [Security Properties](../server-properties/security-properties.md)。  
  
 您也可以使用分析管理物件 (AMO) 來管理伺服器角色。 如需詳細資訊，請參閱[使用分析管理物件 &#40;AMO&#41; 來開發](../multidimensional-models/analysis-management-objects/developing-with-analysis-management-objects-amo.md)。  
