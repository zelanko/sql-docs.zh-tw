 ...
 **本主題內容：**  
 ...
 [步驟2：設定基本 Analysis Services SharePoint 整合](#bkmk_config)  
  
 [步驟3：確認整合](#bkmk_verify)  
...
-   **後端服務：** 如果您使用 PowerPivot for Excel 建立包含分析資料的活頁簿，您必須擁有 PowerPivot for SharePoint，才能在伺服器環境中存取該資料。 您可以在已安裝 SharePoint Server 2013 的電腦或在沒有 SharePoint 軟體的另一部電腦上執行 SQL Server 安裝程式。 Analysis Services 沒有 SharePoint 的相依性。  
  
     **注意：** 本主題描述[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]伺服器和後端服務的安裝。  
...
###  <a name="bkmk_sqleditions"></a>SQL Server 版本需求  
 ...
 您可以在[SQL Server 2012 SP1 版本](ttp://go.microsoft.com/fwlink/?LinkID=248389)資訊（https://go.microsoft.com/fwlink/?LinkID=248389)）找到目前的版本資訊。  
...
8.  
  **[安裝安裝檔案]** 頁面會執行數分鐘。 檢閱任何規則警告或失敗規則，然後按 **[下一步]**。  
  
9. 如果您看到另一個 **[安裝程式支援規則]**，請檢閱任何警告，然後按 **[下一步]**。  
  
     **注意：** 由於已啟用 Windows 防火牆，因此您會看到開啟埠以啟用遠端存取的警告。  
  
10. 在 **[安裝程式角色]** 頁面上，選取 **[SQL Server PowerPivot for SharePoint]**。 此選項會安裝 SharePoint 模式的 Analysis Services。  
  
     您可選擇性地在安裝中加入一項 Database Engine 的執行個體。 在設定新的伺服器陣列時，您可能會新增資料庫引擎，而且需要資料庫伺服器來執行伺服器陣列的設定和內容資料庫。 此選項也會安裝 [!INCLUDE[ssManStudioFull](../../../includes/ssmanstudiofull-md.md)]。  
...
##  <a name="bkmk_config"></a>步驟2：設定基本 Analysis Services SharePoint 整合  
...
##  <a name="bkmk_verify"></a>步驟3：確認整合  
 下列步驟會逐步引導您建立和上傳新的活頁簿，以確認 Analysis Services 整合。 您將需要使用 SQL Server 資料庫才能完成這些步驟。  
  
1.  **注意：** 如果您已經有含交叉分析篩選器或篩選的先進活頁簿，您可以將它上傳至 SharePoint 文件庫，並確認您可以從文件庫視圖與交叉分析篩選器和篩選互動。  
...
13. 
  **[匯入資料]** 對話方塊將會出現。 選擇 **[樞紐分析表]**，然後按一下 **[確定]**。  
...
16. 將篩選或交叉分析篩選器加入至樞紐分析表。 **請勿略過此步驟**。 篩選或交叉分析篩選器是協助您確認 Analysis Services 安裝的元素。  
 ...
 **Web 前端（WFE）** 或仲介**層：** 若要在較大的 sharepoint [!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]伺服器陣列中使用 sharepoint 模式的伺服器，並將其他 PowerPivot 功能安裝到伺服器陣列中，請在每部 sharepoint 伺服器上執行安裝程式套件**sppowerpivot.msi。** spPowerPivot.msi 會安裝必要的資料提供者以及 PowerPivot for SharePoint 2013 組態工具。  
 ...
 **冗余和伺服器負載：** 在 SharePoint 模式下安裝第[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]二部或多部伺服器將會提供[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]伺服器功能的冗余。 其他伺服器也會分散伺服器的負載。 如需詳細資訊，請參閱下列：  
...
## <a name="see-also"></a>另請參閱  
 ...
 [安裝或卸載 PowerPivot for SharePoint 增益集 &#40;SharePoint 2013&#41;](https://docs.microsoft.com/analysis-services/instances/install-windows/install-or-uninstall-the-power-pivot-for-sharepoint-add-in-sharepoint-2013)   
 [&#40;SharePoint 2013&#41;升級活頁簿和排程的資料重新整理](https://docs.microsoft.com/analysis-services/instances/install-windows/upgrade-workbooks-and-scheduled-data-refresh-sharepoint-2013)  
