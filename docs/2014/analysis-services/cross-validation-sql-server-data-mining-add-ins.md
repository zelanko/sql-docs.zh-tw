 ...
 **K-fold Cross-validation**精靈，然後在每個折疊上建立新的模型、 上其他的摺疊，測試模型，然後報告模型的精確度。 完成時， **k-fold Cross-validation**精靈建立的報表，顯示針對每個摺疊，計量，並提供彙總模型的摘要。 這項資訊可用於判斷基礎資料對於模型的良好程度，或者用於比較根據相同資料建立的不同模型。  
...
1.  在 **精確度和驗證**的群組**Data Mining**功能區中，按一下 **交叉驗證**。  
...
4.  （選擇性） 設定要用於交叉驗證所輸入的數字的資料列數上限**最大資料列**文字方塊。  
...
7.  （選擇性） 指定要做為值**目標臨界值**中當做計算預測。 這個值會以機率的方式表示，也就是介於 0 和 1 之間的數字，其中 1 表示預測保證是正確的，0 則表示預測不可能正確，而 .5 則視為隨機猜測。  
...
#### <a name="lift"></a>增益  
 *提起*是與可能性相關聯的量值。 如果結果比較可能是當您使用比您什麼時候進行隨機猜測模型，模型即為提供*正增益*。 不過，如果模型進行比較不可能是隨機的預測，增益分數就*負數*。 因此，這項標準表示使用模型可以達到的改進程度，其分數越高越好。  
  
 系統會將增益當做實際預測機率與測試案例中臨界機率的比率計算。  
  
#### <a name="log-score"></a>對數分數  
 *對數分數*，也稱為*對數可能性分數*預測，表示轉換為對數刻度的兩個機率之間的比率。 由於機率會以小數表示，因此對數分數永遠為負數。 分數越接近 0，表示得分越高。  
