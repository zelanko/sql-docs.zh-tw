...
> [!NOTE]  
>  對於 [處理][!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]** 對話方塊中列出的物件，此對話方塊所指定的設定會覆寫從 ** 資料庫繼承的預設值。  
...
-   [**建立] 一律**會建立回寫資料表（如果不存在），或覆寫現有的回寫資料表（如果存在的話）。  
  
-   **使用現有**的回寫資料表（如果存在的話）。 如果回寫資料表不存在，則會發生錯誤。  
