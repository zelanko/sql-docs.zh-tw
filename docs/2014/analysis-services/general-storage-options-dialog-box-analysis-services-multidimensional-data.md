---
title: 一般 （儲存選項對話方塊） (Analysis Services-多維度資料) |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
|**無回應覆寫間隔**|設定在接收到針對物件的通知之後，主動式快取開始建立物件之新 MOLAP 影像的最長間隔與時間單位 (不論物件的目前活動情況)。 在到達此間隔後收到的通知，不會取消此間隔所觸發的 MOLAP 影像處理。<br /><br /> 注意：除非選取 [當資料變更時更新快取]，否則此選項會停用。 另外請注意，此選項不應該設定如果**儲存模式**設**HOLAP**。|  
|**卸除過期的快取**|指定在開始建立新的 MOLAP 快取與移除現有 MOLAP 快取之間的期間。<br /><br /> 注意：除非選取 [啟用主動式快取]，否則此選項會停用。 另外請注意，此選項不應該設定如果**儲存模式**設定為 HOLAP。|  
|**延遲**|指定在開始建立新 MOLAP 快取與移除現有 MOLAP 快取之間的期間之間隔及時間單位。<br /><br /> 注意：除非選取了 [卸除過期的快取]，否則此選項會停用。 另外請注意，此選項不應該設定如果**儲存模式**設**HOLAP**。|  
|**定期更新快取**|不論是否接收到通知，都會定期更新 MOLAP 影像。<br /><br /> 注意：除非選取 [啟用主動式快取]，否則此選項會停用。 另外請注意，此選項不應該設定如果**儲存模式**設**HOLAP**。|  
|**重建間隔**|選取間隔及時間單位，在建立了新的 MOLAP 影像之後，經過該時期 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 就會再次針對物件啟動 MOLAP 影像處理 (不論是否接收到通知)。 在到達此間隔後收到的通知，不會取消此間隔所觸發的 MOLAP 影像處理。<br /><br /> 注意：除非選取了 [定期更新快取]，否則此選項會停用。 另外請注意，此選項不應該設定如果**儲存模式**設**HOLAP**。|  
...
|**將設定套用至維度**|將儲存模式及主動式快取設定，套用到相關聯的維度。|  
  
## <a name="see-also"></a>另請參閱  
 [儲存選項對話方塊&#40;Analysis Services-多維度資料&#41;](storage-options-dialog-box-analysis-services-multidimensional-data.md)   
