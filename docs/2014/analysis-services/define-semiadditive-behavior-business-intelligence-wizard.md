---
title: 定義局部加總行為 （商業智慧精靈） |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
 ...
 **精靈偵測到\<維度名稱 > 帳戶維度包含局部加總的成員。伺服器將會彙總此維度，根據每一個帳戶類型指定的局部加總行為的成員。**  
 針對包含局部加總成員的帳戶維度，啟用局部加總行為。 選取此選項會將參考帳戶維度的量值群組中所有量值的彙總函式`ByAccount`。  
