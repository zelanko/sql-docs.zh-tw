---
title: 建立測試集 （資料採礦精靈） |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
# <a name="create-testing-set-data-mining-wizard"></a>建立測試集 (資料採礦精靈)
 ...
 如果您稍後想要變更的測試或定型資料量，您可以修改`HoldoutMaxCases`和`HoldoutMaxPercent`上使用資料採礦結構屬性**屬性**視窗。 不過，在進行變更後，必須重新處理採礦結構及所有相關聯的採礦模型。 同時適用下列限制：  
