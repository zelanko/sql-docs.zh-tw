...
3.  在**建立具名計算** 對話方塊中，輸入`SimpleDate`中**資料行名稱**方塊，然後輸入或複製並貼上下列`DATENAME`陳述式中的**運算式**方塊：  
...
4.  按一下**確定**，然後展開`Date`中**資料表**窗格。  
     ...
     `SimpleDate`具名的計算會出現在清單中 [Date] 資料表的資料行中並以圖示表示它是具名的計算。  
...
7.  向右捲動以檢閱 [Explore Date Table (瀏覽日期資料表)] 檢視中的最後一個資料行。  
     ...
     請注意，`SimpleDate`資料行會出現在資料來源檢視，可正確串連基礎資料來源，從幾個資料行不會修改原始資料來源。  
...
1.  針對 [!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)] 中的 [日期] 維度，開啟 [維度設計師]。 若要這樣做，請按兩下`Date`維度**維度**節點**方案總管 中**。  
...
2.  拖曳**日曆半年**屬性從**屬性**窗格將**\<新層級 >** 格**階層**窗格中，下方**日曆年度**層級。  
  
3.  拖曳**日曆季**屬性從**屬性**窗格將**\<新層級 >** 格**階層**窗格中，下方**日曆半年**層級。  
  
4.  拖曳**英文月份**屬性從**屬性**窗格將**\<新層級 >** 格**階層**窗格中，下方**日曆季**層級。  
  
5.  拖曳**日期索引鍵**屬性從**屬性**窗格將**\<新層級 >** 格**階層**窗格下方**英文月份**層級。  
  
6.  在**階層** 窗格中，以滑鼠右鍵按一下標題列**階層**階層、 按一下**重新命名**，然後輸入`Calendar Date`。  
  
7.  以滑鼠右鍵按一下內容功能表中，使用中`Calendar Date`階層中，重新命名**英文月份**層級`Calendar Month`，然後將重新命名**日期索引鍵**層級`Date`。  
...
1.  在**維度設計師**如`Date`維度中，按一下 [**屬性關聯性**] 索引標籤。  
...
3.  在**建立具名計算** 對話方塊中，輸入`MonthName`中**資料行名稱**方塊，然後輸入或複製並貼上下列陳述式中的**運算式**方塊：  
...
6.  在**建立具名計算** 對話方塊中，輸入`CalendarQuarterDesc`中**資料行名稱**方塊，然後輸入或複製並貼上下列 SQL 指令碼中的**運算式**方塊：  
...
9. 在**建立具名計算** 對話方塊中，輸入`CalendarSemesterDesc`中**資料行名稱**方塊，然後輸入或複製並貼上下列 SQL 指令碼中的**運算式**方塊：  
...
2.  您收到之後**成功地完成部署**訊息中，按一下**瀏覽器** 索引標籤**維度設計師**的`Date`維度，並然後按一下設計師工具列上的 重新連接 按鈕。  
...
1.  開啟**維度結構** 索引標籤，維度設計師`Date`維度，然後再選取**日曆半年**中**屬性**窗格。  
...
9. 在 [!INCLUDE[ssBIDevStudio](../includes/ssbidevstudio-md.md)] 的 [建立] 功能表上，按一下 [部署 Analysis Services 教學課程]。 當成功地完成部署之後時，按一下 [**瀏覽器**] 索引標籤，維度設計師`Date`維度。  
