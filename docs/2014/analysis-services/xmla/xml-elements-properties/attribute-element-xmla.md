...
# <a name="attribute-element-xmla"></a>Attribute 元素 (XMLA)
  定義或篩選中之屬性的成員父[插入](../xml-elements-commands/insert-element-xmla.md)，[更新](../xml-elements-commands/update-element-xmla.md)，或[卸除](../xml-elements-commands/drop-element-xmla.md)命令會執行。  
...
|-|-|  
|**上階或父系**|**子元素**|  
|[卸除](../xml-elements-commands/drop-element-xmla.md)，[其中](name-element-xmla.md)，[金鑰](keys-element-xmla.md)|  
...
## <a name="remarks"></a>備註  
 `Attribute`項目定義之屬性成員插入、 更新或刪除，分別由`Insert`， `Update`，或`Drop`命令。 因為只能針對單一屬性成員運作這些命令一次[屬性](attributes-element-xmla.md)集合`Insert`， `Update`，和`Drop`命令只能包含一個`Attribute`項目。 不過，`Attributes` 和 `Where` 命令之 `Drop` 元素的 `Update` 集合可以包含一個以上的 `Attribute` 元素，如此您就可以篩選要在可寫入維度中卸除或更新的屬性。  
