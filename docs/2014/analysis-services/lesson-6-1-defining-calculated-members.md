...
1.  開啟適用於 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 教學課程 Cube 的 Cube 設計師，然後按一下 [計算] 索引標籤。  
     ...
     ![Cube 設計師的 [計算]]索引標籤(../../2014/tutorials/media/l6-calculatedmembers-1.gif "Cube 設計師的 [計算]")索引標籤  
...
6.  在 [**運算式**] 方塊中, 于 [measure`+`] 後面輸入一個加號 () **。 [網際網路銷售-銷售量]** 。  
...
9. 在 [非空白行為] 清單中，勾選 [網際網路銷售 - 銷售量] 和 [轉售商銷售 - 銷售量] 的核取方塊，然後按一下 [確定]。  
     ...
     已![填入的計算運算式窗格]已(../../2014/tutorials/media/l6-calculatedmembers-03.gif "填入的計算運算式窗格")  
...
11. 在計算指令碼的 **Calculate;** 命令之後、剛新增的計算指令碼之前，新增一行，然後把下列文字新增到指令碼的那一行：  
    ...
    ```  
     ...
     [![計算運算式] 窗格中的腳本][(../../2014/tutorials/media/l6-calculatedmembers-04.gif "計算運算式] 窗格中的腳本")  
  
12. 在 [**計算**] 索引標籤的工具列上, 按一下 [**表單檢視**] `[Total Sales Amount]` , 確認已在 [**腳本召集人**] 窗格中選取, 然後按一下 [**新增匯出成員**]。  
...
1.  確認已在 [**腳本召集人**] 窗格中選取, 然後在 [**計算**] 索引標籤的工具列上, 按一下 [新增匯出成員]。 `[Total Product Cost]`  
...
17. 在計算腳本中, 緊接在`[Internet GPM]`計算之前加入新行, 然後將下列文字新增至腳本本身的行:  
    ...
    ```  
     ...
     [![計算運算式] 窗格中的新計算][(../../2014/tutorials/media/l6-calculatedmembers-05.gif "計算運算式] 窗格中的新計算")  
...
8.  將 [總毛利率]、[網際網路毛利率] 和 [轉售商毛利率] 量值新增至 [值] 區域中，然後檢閱結果。  
     ...
     ![顯示轉售商銷售的資料窗格](../../2014/tutorials/media/l6-calculatedmembers-7b.gif "顯示轉售商銷售的資料窗格")  
