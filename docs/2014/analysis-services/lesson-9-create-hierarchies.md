...
3.  在 階層名稱重新命名該階層輸入`Category`，然後按 ENTER 鍵。  
  
4.  在`Product`資料表中，按一下**Product Category Name**資料行，然後將它拖曳至`Category`階層，以及最上層的然後放開`Category`名稱。  
  
5.  在`Category`階層，以滑鼠右鍵按一下**Product Category Name**資料行，然後按一下 **重新命名**，然後輸入`Category`。  
  
    > [!NOTE]  
    >  重新命名階層中的資料行並不會重新命名資料表中的該資料行。 階層中的資料行只是資料表中資料行的表示。  
  
6.  在`Product`資料表中，以滑鼠右鍵按一下**Product Subcategory Name**資料行，然後在內容功能表，指向**將加入階層**，然後按一下  `Category`。  
  
7.  重新命名**Product Subcategory Name**至`Subcategory`。  
  
8.  使用按一下並拖曳，或使用**將加入階層**命令的內容功能表中，加入**模型名稱**和**產品名稱**（依順序） 的資料行，並將它們下方**Product Subcategory Name**資料行。 重新命名這些資料行`Model`和`Product`分別。  
