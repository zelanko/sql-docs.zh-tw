...
-   在範例資料模型中，只有一個可預測的結果-Bike Buyer-但是假設您擁有客戶是否購買服務方案，並想要一併預測的相關資訊。 在該情況下，您會將這些資料放入不同的資料行，並且在模型中加入兩個可預測的屬性。  
...
1.  以滑鼠右鍵按一下樹狀結構中包含所需資料的節點，然後選取下列其中一個選項：  
    ...
    -   **鑽研結構資料行**。 這個選項會取得屬於選取之節點的案例，並且將它們儲存至 Excel 中的資料表。 取得可在基礎的所有資訊時建置它，即使資料行的資料未用於模型中。 例如，您可能已經排除客戶地址和郵遞區號，因為這些欄位不適用於分析，但仍然保留在結構中。  
