...
# <a name="defining-the-unknown-member-and-null-processing-properties"></a>定義未知的成員和 Null 處理屬性
 ...
 在這個主題的工作中，您會從雪花資料表將產品類別目錄和產品子類別目錄屬性加入 [產品] 維度中，而且您將把雪花資料表加入 [!INCLUDE[ssSampleDBCoShort](../includes/sssampledbcoshort-md.md)] DW 資料來源檢視中。 接著，您將啟用 [產品] 維度的 [ **UnknownMember** ] `Assembly Components`屬性、指定作為 [ **UnknownMemberName** ] 屬性的值`Subcategory` 、 `Category`將和屬性與 [產品名稱] 屬性建立關聯，然後針對連結雪花資料表的成員索引鍵屬性定義自訂錯誤處理。  
...
8.  **在 [階層**] 清單中選取`All Products`[**產品類別目錄**]，展開，然後展開 [**元件**]。  
 ...
 若要修改上一個工作中所述的行為，您將啟用 Products 維度的**UnknownMember**屬性、設定**UnknownMemberName**屬性的值、將`Subcategory`和**模型名稱**屬性的**NullProcessing**屬性設定為**UnknownMember**、將`Category`屬性定義`Subcategory`為屬性的相關屬性，然後將**產品線**屬性定義為**模型名稱**屬性的相關屬性。 這些步驟將會使 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 在沒有 [SubcategoryKey]**** 資料行值的每一項產品中使用未知的成員名稱值，如下一項工作所示。  
...
12. 將 [NullProcessing]**** 屬性變更為 [UnknownMember]****。  
     ...
     由於這些變更，當在[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]處理期間遇到`Subcategory`屬性或**模型名稱**屬性的 null 值時，未知的成員值會取代為索引鍵值，而使用者定義階層將會正確地進行結構化。  
...
4.  展開`Assembly Components` `Category`層級的成員，然後展開`Assembly Components` `Subcategory`層級的成員。  
