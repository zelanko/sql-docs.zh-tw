...
---
# <a name="format_string-contents-mdx"></a>FORMAT_STRING 內容 (MDX)
  
  `FORMAT_STRING` 資料格屬性可格式化 `VALUE` 資料格屬性，建立 `FORMATTED_VALUE` 資料格屬性的值。 
  `FORMAT_STRING` 資料格屬性能處理原始的字串及原始數值，對該值套用格式運算式，以傳回格式化的 `FORMATTED_VALUE` 資料格屬性值。 下列資料表詳述了用來處理字串和數值的語法和格式字元。  
...
|**#**|代表會顯示數字或零的數字預留位置。<br /><br /> 如果運算式在格式字串中出現數位記號（**#**）的位置有一個數位，則格式化的值會顯示該數位。 否則，格式值就會在那個位置顯示一個零。<br /><br /> 數位記號（**#**）預留位置的運作方式就像零（**0**）數位預留位置，不同的是，如果數位的位數與格式運算式中小數點任一端的**#** 字元數相同或較少，就不會顯示前置和尾端零。|  
|**.**|代表可決定在小數點左、右要顯示幾位數字的小數預留位置。<br /><br /> 如果格式運算式在句號（**.**）左邊**#** 只包含數位記號（），小於1的數位就會以小數分隔符號開頭。 若要在小數點數字開頭顯示一個零，請使用零 (0) 做為小數點左邊的第一個數字預留位置。<br /><br /> 在格式化輸出中實際做為小數預留位置的字元，將依電腦系統所能辨識的數字格式而有所不同。<br /><br /> 注意：在某些地區設定中，是用逗點做為小數點。|  
...
|**:**|代表時間分隔符號，其會在格式化時間值的時候區隔時、分 和秒。<br /><br /> 注意：在某些地區設定中，可能是用其他字元來做為時間分隔符號。<br /><br /> 在格式化輸出中實際做為時間分隔符號使用的字元，是由電腦上的系統設定來決定。|  
...
|**- + $ ( )**|顯示實際字元。<br /><br /> 若要顯示其中一個列出的字元，請在該字元前面加**\\**上反斜線（），或以雙引號（**""**）括住字元。|  
|**\\**|顯示格示字串中的下一個字元。<br /><br /> 若要顯示有特殊意義的字元做為常值字元，請在字元**\\**前面加上一個反斜線（）。 反斜線本身不會被顯示。 使用反斜線就和使用雙引號把反斜線後面的字元括起來是一樣的。 若要顯示反斜線，請使用兩**\\**個反斜線（）。 無法顯示為實際字元的字元範例包括以下字元：<br /><br /> 日期格式及時間格式字元-**a**、 **c**、 **d**、 **h**、 **m**、 **n**、 **p**、 **q**、 **s**、 **t**、 **w**、 **y**、 **/** 和 **：**<br /><br /> 數值格式**#** 字元-、 **0**、 **%**、 **e**、 **e**、**逗號**和**句點**<br /><br /> **@** 字串格式字元-、 **&**、 **\<**、 **>** 和 **！**|  
...
|**dd**|將日期顯示為具有前置零的數位（01-31）。|  
|**ddd**|將日期顯示為縮寫（Sun-Sat）。|  
|**dddd**|將日期顯示為完整名稱（星期日-星期六）。|  
...
|**7，200**|將一年中的周數顯示為數字（1-54）。|  
|**分鐘**|將月份顯示為沒有前置零的數位（1-12）。<br /><br /> 如果 **m** 是直接跟隨在 **h** 或 **hh**後面，則顯示的將是「分」而不是「月」。|  
|**mm**|將月份顯示為有前置零的數位（01-12）。<br /><br /> 如果 **m** 是直接跟隨在 **h** 或 **hh**後面，則顯示的將是「分」而不是「月」。|  
|**mmm**|將月份顯示為縮寫（Jan-Dec）。|  
|**mmmm**|將月份顯示為完整月份名稱（月-12 月）。|  
|**問題解答**|將一年的季顯示為數字（1-4）。|  
|**y**|將一年中的日期顯示為數字（1-366）。|  
|**年月**|將年份顯示為兩位數的數位（00-99）。|  
|**yyyy**|將年份顯示為四位數數位（100-9999）。|  
|**h**|將小時顯示為沒有前置零的數位（0-23）。|  
|**hh**|將小時顯示為具有前置零的數位（00-23）。|  
|**n**|將分鐘顯示為沒有前置零的數位（0-59）。|  
|**.log**|將分鐘顯示為開頭為零的數位（00-59）。|  
|**今日**|將秒顯示為沒有前置零的數位（0-59）。|  
|**ss**|將秒顯示為開頭為零的數位（00-59）。|  
...
|**AMPM**|以電腦系統所定義的 AM 字串常值顯示午夜到正午的任何時間；以電腦系統所定義的 PM 字串常值顯示正午到午夜的任何時間。<br /><br /> 注意：使用 12 小時時鐘制。<br /><br /> **AMPM**可以是大寫或小寫，但是所顯示字串的大小寫會與電腦的系統設定所定義的字串相符。<br /><br /> 對於 Windows，預設的格式為 **AM/PM**。|  
...
## <a name="see-also"></a>另請參閱  
 [FORMATED_VALUE 上的語言和 FORMAT_STRING](mdx-cell-properties-formatted-value-property.md)   
