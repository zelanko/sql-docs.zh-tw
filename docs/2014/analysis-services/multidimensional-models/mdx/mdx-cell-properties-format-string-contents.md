...
|**#**|代表會顯示數字或零的數字預留位置。<br /><br /> 如果運算式在格式字串中出現數位記號（）的位置有一個數位 **#** ，則格式化的值會顯示該數位。 否則，格式值就會在那個位置顯示一個零。<br /><br /> 數位記號（ **#** ）預留位置的運作方式就像零（**0**）數位預留位置，不同的是，如果數位的位數與 **#** 格式運算式中小數點任一端的字元數相同或較少，就不會顯示前置和尾端零。|  
|**.**|代表可決定在小數點左、右要顯示幾位數字的小數預留位置。<br /><br /> 如果格式運算式在 **#** 句號（**.**）左邊只包含數位記號（），小於1的數位就會以小數分隔符號開頭。 若要在小數點數字開頭顯示一個零，請使用零 (0) 做為小數點左邊的第一個數字預留位置。<br /><br /> 在格式化輸出中實際做為小數預留位置的字元，將依電腦系統所能辨識的數字格式而有所不同。<br /><br /> 注意：在某些地區設定中，是用逗點做為小數點。|  
...
|**- + $ ( )**|顯示實際字元。<br /><br /> 若要顯示其中一個列出的字元，請在該字元前面加上反斜線（ **\\** ），或以雙引號（**""**）括住字元。|  
|**\\**|顯示格示字串中的下一個字元。<br /><br /> 若要顯示有特殊意義的字元做為常值字元，請在字元前面加上一個反斜線（ **\\** ）。 反斜線本身不會被顯示。 使用反斜線就和使用雙引號把反斜線後面的字元括起來是一樣的。 若要顯示反斜線，請使用兩個反斜線（ **\\\\** ）。 無法顯示為實際字元的字元範例包括以下字元：<br /><br /> 日期格式及時間格式字元-**a**、 **c**、 **d**、 **h**、 **m**、 **n**、 **p**、 **q**、 **s**、 **t**、 **w**、 **y**、 **/** 和 **：**<br /><br /> 數值格式字元- **#** 、 **0**、 **%** 、 **e**、 **e**、**逗號**和**句點**<br /><br /> 字串格式字元- **@** 、 **&** 、 **\<**, **>** 和 **！**|  
