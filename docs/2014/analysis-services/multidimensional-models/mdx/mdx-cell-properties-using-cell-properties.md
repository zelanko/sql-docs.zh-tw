...
|`FORMATTED_VALUE`|代表格式化的顯示的字元字串`VALUE`屬性。|  
|`LANGUAGE`|將會套用 `FORMAT_STRING` 的地區設定。 `LANGUAGE` 通常用於貨幣轉換。|  
|`UPDATEABLE`|此值代表是否可更新資料格。 此屬性可以有下列其中一個值：<br /><br /> `MD_MASK_ENABLED` (0x00000000) 可以更新資料格。<br /><br /> `MD_MASK_NOT_ENABLED` (0x10000000) 無法更新資料格。<br /><br /> `CELL_UPDATE_ENABLED` (0x00000001) 可以更新資料格集中的資料格。<br /><br /> `CELL_UPDATE_ENABLED_WITH_UPDATE` (0x00000002) 可以使用 update 陳述式更新資料格。 若要更新不可寫入的分葉資料格，則該次更新可能會失敗。<br /><br /> `CELL_UPDATE_NOT_ENABLED_FORMULA` (0x10000001) 儲存格不可更新，因為資料格都具有其座標; 之間的導出的成員儲存格已擷取一組在 where 子句。 即使公式會影響資料格的值 (或是啟用導出資料格)，還是會更新資料格。 在此案例中，因為計算將會影響到結果，所以此資料格最終的值可能不是更新的值。<br /><br /> `CELL_UPDATE_NOT_ENABLED_NONSUM_MEASURE` (0x10000002) 無法更新資料格，因為無法更新非總和量值 （計數、 最小值、 最大、 相異計數、 局部）。<br /><br /> `CELL_UPDATE_NOT_ENABLED_NACELL_VIRTUALCUBE` (0x10000003) 因為資料格不存在，因為它是量值的交集處與量值的量值群組不相關的維度成員，無法更新資料格。<br /><br /> `CELL_UPDATE_NOT_ENABLED_SECURE` (0x10000005) 因為資料格受到保護，無法更新資料格。<br /><br /> `CELL_UPDATE_NOT_ENABLED_CALCLEVEL` (0x10000006) 保留供未來使用。<br /><br /> `CELL_UPDATE_NOT_ENABLED_CANNOTUPDATE` (0x10000007) 因為內部原因，無法更新資料格。<br /><br /> `CELL_UPDATE_NOT_ENABLED_INVALIDDIMENSIONTYPE` (0x10000009) 因為採礦模型、 間接或資料採礦維度中不支援更新，無法更新資料格。|  
|`VALUE`|未格式化的資料格值。|  
  
 只有`CELL_ORDINAL`， `FORMATTED_VALUE`，和`VALUE`所需的資料格屬性。 `PROPERTIES` 結構描述資料列集中定義的所有內建或提供者特有的資料格屬性，包括它們的資料類型及提供者支援在內。 如需有關`PROPERTIES`結構描述資料列，請參閱[MDSCHEMA_PROPERTIES 資料列集](../../schema-rowsets/ole-db-olap/mdschema-properties-rowset.md)。  
  
 根據預設，如果`CELL PROPERTIES`不是關鍵字，傳回的資料格屬性是`VALUE`， `FORMATTED_VALUE`，和`CELL_ORDINAL`（依該順序）。 如果`CELL PROPERTIES`使用關鍵字，只明確陳述關鍵字的那些資料格屬性會傳回。  
...
```  
 ...
 傳回扁平化資料列集; 的 MDX 查詢不會傳回資料格屬性在此情況下，每個資料格都會如同只有`FORMATTED_VALUE`傳回資料格屬性。  
