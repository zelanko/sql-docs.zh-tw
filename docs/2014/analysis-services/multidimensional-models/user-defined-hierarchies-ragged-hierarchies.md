...
-   使用一般階層，但是在每個層級上設定 `HideMemberIf` 屬性來指定使用者是否可看到遺漏的層級。 設定時`HideMemberIf`，您也應該設定`MDXCompatibility`上連接字串，以覆寫預設導覽行為。 本主題將提供設定這些屬性的指示。  
...
##  <a name="bkmk_Mdx"></a> 設定 MDX 相容性以決定如何在用戶端應用程式中表示預留位置  
 在設定後`HideMemberIf`階層層級中，您也應該設定`MDX Compatibility`從用戶端應用程式傳送的連接字串中的屬性。 `MDX Compatibility` 設定會決定是否使用 `HideMemberIf`。  
