...
##  <a name="bkmk_Mdx"></a>設定 MDX 相容性以決定如何在用戶端應用程式中表示預留位置  
 在階層層級上設定 `HideMemberIf` 之後，您也應該在從用戶端應用程式傳送的連接字串中設定 `MDX Compatibility` 屬性。 
  `MDX Compatibility` 設定會決定是否使用 `HideMemberIf`。  
...
|**2**|隱藏預留位置的值 (Null 值或父層級的重複)，但是會顯示具有相關值的其他層級和節點。|就不完全階層而言，`MDX Compatibility`=2 通常會視為慣用設定。 
  [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 報表和某些協力廠商用戶端應用程式可以保留這項設定。|  
