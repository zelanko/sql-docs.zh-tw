...
# <a name="attributes-in-parent-child-hierarchies"></a>父子式階層中的屬性
 ...
 資料成員只可供含有父子式階層的維度使用，而且唯有父屬性允許時才會顯示出來。 您可以使用維度設計師來控制資料成員的可見性。 若要公開資料成員，設定`MembersWithData`屬性給父屬性，以`NonLeafDataVisible.`隱藏父屬性所包含的資料成員，設定`MembersWithData`屬性若要在父屬性上的`NonLeafDataHidden`。  
  
 這項設定不會覆寫非分葉成員的正常彙總行為；基於彙總用途，一律會將資料成員併入為子成員。 然而，可使用自訂積存公式來覆寫正常彙總行為。 多維度運算式 (MDX) [DataMember](/sql/mdx/datamember-mdx)函式可讓您存取相關聯的資料成員的值為何值`MembersWithData`屬性。  
  
 `MembersWithDataCaption` Parent 屬性會提供[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]與用來產生資料成員的成員名稱的命名範本。  
...
## <a name="using-data-members"></a>使用資料成員  
 ...
 甚至，每位銷售成員的個別佣金也會不同。 在此情況下，銷售經理的佣金計算方式會採用個別銷售毛額佔其銷售員產生之總銷售毛額的兩個不同比率來計算。 因此，非分葉成員可存取基礎事實資料表資料就變得非常重要。 MDX`DataMember`函式可以用來擷取個別銷售毛額 Sales Manager 1 成員，而且自訂積存運算式可用來從 Sales Manager 1 成員，提供 gross 彙總值中排除該資料成員銷售毛額，該成員相關聯。  
