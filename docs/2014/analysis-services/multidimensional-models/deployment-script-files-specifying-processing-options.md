...
-   **回寫資料表選項**如果已在[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]專案中啟用回寫，則此設定會定義如何處理回寫。 有三個回寫資料表選項：  
...
-   **交易式部署**此設定控制中繼資料變更與處理命令的部署是出現在單一交易或個別交易中。  
...
## <a name="modifying-the-processing-options-for-deployment"></a>修改部署的處理選項  
 不過，您可能需要使用不同于[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] \<*專案名稱*> d 檔案中儲存的處理選項，來部署專案。 例如，您可能需要完整地處理所有的物件、或使用預設處理選項處理、或不進行處理。 如果 Cube 或維度是可寫入的，您可以指定使用新的或現有的回寫資料表。  
