...
-   **交易式部署** 此設定控制中繼資料變更與處理命令的部署是發生於單一交易或個別交易中。  
    ...
    -   如果此選項為`True`（預設）、[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]部署中繼資料的所有變更，在單一交易中的所有處理程序命令。  
  
    -   如果此選項為`False`，[!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]部署中繼資料變更，在單一交易中，並將部署在其自己的交易中每個處理命令。  
