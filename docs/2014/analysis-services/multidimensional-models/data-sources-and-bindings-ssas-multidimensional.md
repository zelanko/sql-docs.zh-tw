...
-   每一個量值群組維度都有一組「資料粒度屬性」可定義此量值群組的資料粒度。 每一個屬性都必須繫結至包含此屬性索引鍵之事實資料表中的一或多個資料行  (如需有關資料粒度屬性的詳細資訊，請參閱本主題稍後的「量值群組資料粒度屬性」)。  
 ...
 預設資料來源必須定義於 DSV 中，藉此提供結構描述資訊，包括關聯性的詳細資料在內。 在資料分割層級上指定的任何其他資料表或查詢都不需要列在 DSV 中，但是它們的結構描述必須與針對量值群組定義之預測資料表的結構描述相同，或是至少必須包含量值或資料粒度屬性所使用的所有資料行。 根據量值的詳細繫結及資料粒度屬性無法在資料分割層級上覆寫，而且會假設它們是針對量值群組所定義的相同資料行。 因此，如果資料分割使用資料來源，事實上有不同的結構描述，`TableDefinition`中定義的資料分割必須與量值群組所使用的結構描述相同的結構描述產生的查詢。  
