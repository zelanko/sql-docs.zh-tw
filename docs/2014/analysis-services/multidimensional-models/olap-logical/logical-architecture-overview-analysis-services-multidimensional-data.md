...
## <a name="basic-architecture"></a>基本架構
 ...
 伺服器物件的[!INCLUDE[ssASnoversion](../../../includes/ssasnoversion-md.md)]每個實例在 AMO 中會被視為不同的伺服器物件;每個不同的實例都會透過<xref:Microsoft.AnalysisServices.Server>不同的連接連接到物件。 每一個伺服器物件都包含一個或多個資料來源、資料來源檢視和資料庫物件，以及組件和安全性角色。
