...
|`NullKeyNotAllowed`<br /><br /> 發生時`NullProcessing`  =  `Error`設定維度屬性，或用來唯一識別成員的屬性索引鍵資料行中存在 null 值。|`ReportAndContinue` 會記錄錯誤並將其算入計數。|`ReportAndStop` 會報告錯誤並立即停止處理，而無視於錯誤限制。<br /><br /> `IgnoreError` 既不會記錄錯誤，也未將錯誤算入計數，而只要錯誤計數低於上限，處理作業便會繼續。 觸發此錯誤的記錄預設會轉換成未知的成員，但是您可以設定 `KeyErrorAction` 屬性改為捨棄這些記錄。|  
|`NullKeyConvertedToUnknown`<br /><br /> Null 值接續轉換成未知的成員時發生。 設定`NullProcessing`  =  `ConvertToUnknown`維度屬性將會觸發此錯誤。|`IgnoreError` 既不會記錄錯誤，也未將錯誤算入計數，而只要錯誤計數低於上限，處理作業便會繼續。|如果您認為此錯誤屬於參考性質，請保留預設值。 否則，您可以選擇使用 `ReportAndContinue` 在 [處理] 視窗中報告錯誤並將其算入錯誤限制的錯誤計數。<br /><br /> `ReportAndStop` 會報告錯誤並立即停止處理，而無視於錯誤限制。|  
...
-   設定`NullProcessing` = `Error`以排除具有 null 值的記錄。 這將產生 `NullKeyNotAllowed` 錯誤，而此錯誤會記錄下來且其計數將算入索引鍵錯誤限制。 您可以設定錯誤組態屬性**不允許 Null 索引鍵**到`IgnoreError`來讓處理作業能夠繼續。  
...
##  <a name="bkmk_limit"></a> 變更錯誤限制或錯誤限制動作  
 ...
 您可以指定一旦達到錯誤限制時，要停止處理還是停止記錄。 例如，假設您設定錯誤限制為 100 且對應的動作是 `StopLogging`。 若發生第 101 個錯誤，處理作業仍會繼續，但將不再記錄錯誤或予以算入計數。 錯誤限制動作會指定為`KeyErrorLimitAction`中`ErrorConfiguration`中的屬性[!INCLUDE[ssBIDevStudio](../../includes/ssbidevstudio-md.md)]，或**錯誤動作**中的維度屬性 [錯誤組態] 索引標籤，cube 或量值群組中[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)].  
