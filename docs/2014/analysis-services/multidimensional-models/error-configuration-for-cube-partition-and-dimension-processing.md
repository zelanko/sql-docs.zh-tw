---
title: Cube、 分割區和維度處理 (SSAS-多維度) 的錯誤組態 |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
##  <a name="bkmk_exec"></a> 執行順序  
 伺服器執行的速度`NullProcessing`規則再`ErrorConfiguration`每一筆記錄的規則。 請務必了解這點，因為當索引鍵資料行中有兩筆以上的錯誤記錄為零時，Null 處理屬性若將 Null 轉換成零可能會接續引起索引鍵重複錯誤。  
...
-   轉換成未知的成員依預設，會發生因為`ConvertToUnknown`設定`KeyErrorAction`。 資料庫中會將配置為未知成員的記錄隔離，以玆證明曾有問題發生，供您在處理完成之後著手調查。  
  
     未知的成員會排除在查詢工作負載，但是它們會顯示在某些用戶端應用程式如果`UnknownMember`設**看得見**。  
  
     如果您想要追蹤已轉換成未知成員的 Null 數目，請修改 `NullKeyConvertedToUnknown` 屬性，以便由記錄檔或在 [處理] 視窗中報告這些錯誤。  
  
-   當您以手動方式設定時，就會發生捨棄`KeyErrorAction`屬性`DiscardRecord`。  
 ...
 具有強烈影響的錯誤 (例如，索引鍵欄位遺漏索引鍵或包含 Null 值) 均應快速予以解決。 根據預設，這些錯誤將遵守`ReportAndContinue`伺服器行為，伺服器會攔截錯誤、 將它加入至錯誤計數，然後再繼續處理的位置 （除了錯誤限制為零，以致處理作業立即停止）。  
...
|屬性|預設|其他值|  
|--------------|-------------|------------------|  
|`CalculationError`<br /><br /> 初始化錯誤組態時發生。|`IgnoreError` 不記錄也不會計算錯誤。只要錯誤計數低於上限，處理作業會繼續。|`ReportAndContinue` 記錄檔，然後計算錯誤。<br /><br /> `ReportAndStop` 報告錯誤並立即停止處理，而無視於錯誤限制。|  
|`KeyNotFound`<br /><br /> 事實資料表中的外部索引鍵在相關維度資料表中沒有相符的主索引鍵時發生 (例如，[銷售額] 事實資料表有一筆記錄，其產品識別碼不存在於 [產品] 維度資料表中)。 在資料分割處理或是雪花維度的維度處理期間，可能會發生此錯誤。|`ReportAndContinue` 記錄檔，然後計算錯誤。|`ReportAndStop` 報告錯誤並立即停止處理，而無視於錯誤限制。<br /><br /> `IgnoreError` 不記錄也不會計算錯誤。只要錯誤計數低於上限，處理作業會繼續。 觸發此錯誤的記錄預設會轉換成未知的成員，但是您可以變更 `KeyErrorAction` 屬性改為捨棄這些記錄。|  
|`KeyDuplicate`<br /><br /> 在維度中發現重複的屬性索引鍵時發生。 大多數情況下，具有重複的屬性索引鍵是可接受的，但此錯誤將讓您得知發生重複現象，從而可檢查維度的設計是否有缺陷而可能導致屬性之間的關聯性不一致。|`IgnoreError` 不記錄也不會計算錯誤。只要錯誤計數低於上限，處理作業會繼續。|`ReportAndContinue` 記錄檔，然後計算錯誤。<br /><br /> `ReportAndStop` 報告錯誤並立即停止處理，而無視於錯誤限制。|  
|`NullKeyNotAllowed`<br /><br /> 發生時`NullProcessing`  =  `Error`設定維度屬性，或用來唯一識別成員的屬性索引鍵資料行中存在 null 值。|`ReportAndContinue` 記錄檔，然後計算錯誤。|`ReportAndStop` 報告錯誤並立即停止處理，而無視於錯誤限制。<br /><br /> `IgnoreError` 不記錄也不會計算錯誤。只要錯誤計數低於上限，處理作業會繼續。 觸發此錯誤的記錄時，會轉換成未知的成員上，依預設，但是您可以設定`KeyErrorAction`改為捨棄這些屬性。|  
|`NullKeyConvertedToUnknown`<br /><br /> Null 值接續轉換成未知的成員時發生。 設定`NullProcessing`  =  `ConvertToUnknown`維度屬性將會觸發此錯誤。|`IgnoreError` 不記錄也不會計算錯誤。只要錯誤計數低於上限，處理作業會繼續。|如果您認為此錯誤屬於參考性質，請保留預設值。 否則，您可以選擇使用 `ReportAndContinue` 在 [處理] 視窗中報告錯誤並將其算入錯誤限制的錯誤計數。<br /><br /> `ReportAndStop` 報告錯誤並立即停止處理，而無視於錯誤限制。|  
...
##  <a name="bkmk_tools"></a> 在何處設定錯誤組態屬性  
 在 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 中於部署資料庫之後，或是在 [!INCLUDE[ssBIDevStudio](../../includes/ssbidevstudio-md.md)]的模型專案中，使用屬性頁。 這兩個工具所列出的屬性完全相同。 您也可以設定錯誤組態屬性中變更伺服器預設值為錯誤組態，然後在 msmdrsrv.ini 檔`Batch`和`Process`命令以執行指令碼作業執行的處理。  
...
1.  在 [方案總管] 中，按兩下任何維度或 Cube。 `ErrorConfiguration` 在下方窗格中，會出現在屬性內容。  
  
2.  或者，為只有單一維度，以滑鼠右鍵按一下方案總管 中的維度中，選擇`Process`，然後選擇 **變更設定**處理維度 對話方塊中。 [維度索引鍵錯誤] 索引標籤上隨即出現錯誤組態選項。  
  
##  <a name="bkmk_missing"></a> 遺漏索引鍵 (KeyNotFound)  
 遺漏索引鍵值的記錄一概無法加入至資料庫，即使忽略錯誤或錯誤限制不設限亦然。  
  
 伺服器會產生`KeyNotFound`資料分割在處理期間，當在事實資料表的記錄包含外部索引鍵值，但是該外部索引鍵沒有對應的記錄相關的維度資料表中的錯誤。 在處理相關維度或雪花維度資料表時，若某一維度的記錄所指定的外部索引鍵不存在於相關維度中，可能也會發生此錯誤。  
  
 一旦發生 `KeyNotFound` 錯誤，便會將違規的記錄配置為未知的成員。 此行為由**索引鍵動作**設為`ConvertToUnknown`，如此一來，您可以檢視配置的記錄做進一步調查。  
...
3.  在內容中，展開 **來源**檢視`NullProcessing`屬性。 此屬性依預設會設為 **[自動]** ，意指包含數值資料的欄位內凡是 Null 的 OLAP 項目都將轉換成零。  
  
4.  將值變更為`Error`以排除具有 null 值，避免 null 數值 （零） 轉換的任何記錄。 這項修改讓您避免重複的索引鍵錯誤相關的索引鍵的資料行中的多筆記錄，以及避開`KeyNotFound`零值的外部索引鍵相等相關的維度資料表中沒有主索引鍵時的錯誤。  
  
##  <a name="bkmk_nulldim"></a> 維度中的 Null 索引鍵  
 若要繼續處理在雪花維度中的外部索引鍵中發現 null 值時，先行處理 null 值藉由設定`NullProcessing`上`KeyColumn`的維度屬性。 如此將會捨棄或轉換記錄，避免可能發生 `KeyNotFound` 錯誤。  
  
 處理維度屬性的 Null 情況有兩種選項可用：  
  
-   設定`NullProcessing` = `UnknownMember`將具有 null 值的記錄配置為未知的成員。 這將產生 `NullKeyConvertedToUnknown` 錯誤，而依預設會忽略此錯誤。  
  
-   設定`NullProcessing` = `Error`排除具有 null 值的記錄。 這會產生`NullKeyNotAllowed`錯誤，並記錄和計數算入索引鍵錯誤限制。 您可以設定錯誤組態屬性**不允許 Null 索引鍵**至`IgnoreError`以便繼續處理。  
  
 Null 可能會對非索引鍵欄位構成問題，因為 MDX 查詢將視 Null 會解譯為零或空白而傳回不同的結果。 基於這個原因， [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 提供 Null 處理選項讓您可預先定義所需的轉換行為。 請參閱[定義未知的成員和 Null Processing Properties](../lesson-4-7-defining-the-unknown-member-and-null-processing-properties.md)和<xref:Microsoft.AnalysisServices.NullProcessing>如需詳細資訊。  
...
3.  在內容中，展開  **KeyColumns**檢視`NullProcessing`屬性。 此屬性依預設會設為 **[自動]** ，意指包含數值資料的欄位內凡是 Null 的項目都將轉換成零。 將值變更為`Error`或`UnknownMember`。  
  
     這項修改會移除觸發程序的基礎條件`KeyNotFound`捨棄或轉換記錄，它會檢查出有錯誤。  
  
     視錯誤組態而定，此等動作可能會造成將由系統報告和計數的錯誤。 您可能需要調整其他屬性，例如設定`KeyNotFound`至`ReportAndContinue`或`KeyErrorLimit`為非零值，可讓處理作業能夠繼續時，這些錯誤會報告和計數。  
  
##  <a name="bkmk_dupe"></a> 造成關聯性不一致的索引鍵重複狀況 (KeyDuplicate)  
 依預設，出現索引鍵重複狀況並不會停止處理作業，而是將忽略錯誤且資料庫會排除重複的記錄。  
  
 若要變更此行為，請設定`KeyDuplicate`至`ReportAndContinue`或`ReportAndStop`以回報錯誤。 接著，您可以藉由檢查錯誤，判斷維度的設計是否可能有缺陷。  
  
##  <a name="bkmk_limit"></a> 變更錯誤限制或錯誤限制動作  
 您可以藉著提高錯誤限制，允許處理期間發生更多次錯誤。 提高錯誤限制並無任何方針指引可循，適當的值會因您的實際狀況而不同。 錯誤限制會指定為`KeyErrorLimit`中`ErrorConfiguration`中的屬性[!INCLUDE[ssBIDevStudio](../../includes/ssbidevstudio-md.md)]，或為**錯誤數目**中的維度屬性 [錯誤組態] 索引標籤，cube 或量值群組中[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]。  
  
 您可以指定一旦達到錯誤限制時，要停止處理還是停止記錄。 例如，假設您將動作設定為`StopLogging`錯誤限制為 100。 若發生第 101 個錯誤，處理作業仍會繼續，但將不再記錄錯誤或予以算入計數。 錯誤限制動作會指定為`KeyErrorLimitAction`中`ErrorConfiguration`中的屬性[!INCLUDE[ssBIDevStudio](../../includes/ssbidevstudio-md.md)]，或為**錯誤動作**中的維度屬性 [錯誤組態] 索引標籤，cube 或量值群組中[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)].  
...
## <a name="see-also"></a>另請參閱  
 [記錄檔內容](../server-properties/log-properties.md)   
