...
|~ (波狀符號)|忽略成員的值。|  
 ...
 您可以使用 **UnaryOperatorColumn** 屬性指定具名計算，以傳回一元運算子作為屬性之所有成員的常值字元。 這可能和在具名計算中輸入常值字元 (例如 `'*'` ) 一樣簡單。 這會針對屬性的所有成員，使用乘法運算子 (星號 (*)) 來取代預設運算子 (加號 (+))。 如需詳細資訊，請參閱[在資料來源 View 中定義指名的計算 &#40;Analysis Services&#41;](define-named-calculations-in-a-data-source-view-analysis-services.md)。  
