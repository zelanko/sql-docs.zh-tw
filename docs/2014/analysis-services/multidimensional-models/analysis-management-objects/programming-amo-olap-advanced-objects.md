...
3.  將動作加入 Cube 集合並更新 Cube。 動作不是可更新的物件。  
 ...
 測試這個動作需要不同的程式應用程式。 您可以在 [!INCLUDE[ssBIDevStudioFull](../../../includes/ssbidevstudiofull-md.md)] 中測試動作。 首先，您必須安裝[!INCLUDE[ssRSnoversion](../../../includes/ssrsnoversion-md.md)]範例，請參閱[多維度模型物件處理](../processing-a-multidimensional-model-analysis-services.md)。  
...
|規格|描述|  
|-------------------|-----------------|  
|AggregationStorage|彙總的儲存類型。<br /><br /> 只適用於資料分割。 維度上它必須是 `Regular.`|  
...
|OnlineMode|當 MOLAP 影像是可用的。<br /><br /> 可以為 `Immediate` 或 `OnCacheComplete`。|  
