...
-   在 [**篩選**] 窗格的 [**維度**] 資料行中，按一下 **\<Select dimension>** 並從清單中選取維度，然後在 [階層] 資料 **\<Select hierarchy>** 行中按一下，並從清單中選取階層。 **Hierarchy**  
  
 指定階層之後，請指定運算子和篩選運算式。 下表描述運算子和篩選運算式。  
  
|運算子|篩選運算式|Description|  
|--------------|-----------------------|-----------------|  
...
|位於|一個或多個命名集|值必須在指定的命名集中<br /><br /> (僅支援屬性階層)。|  
