...
|**回寫資料表選項**|控制處理進行期間回寫資料表的處理方式。 此選項適用於 Cube 中的回寫分割區，並且使用下列選項：<br /><br /> **使用現有**的。 使用現有的回寫資料表。 這是預設值。<br /><br /> **Create**。 建立新的回寫資料表，如果資料表已存在，就會造成處理序失敗。<br /><br /> **永遠建立**。 即使已經有回寫資料表存在，還是會建立新的回寫資料表。 將刪除及取代現有的資料表。|  
