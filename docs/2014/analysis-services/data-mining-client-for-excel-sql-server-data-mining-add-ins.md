---
title: 適用於 Excel （SQL Server 資料採礦增益集） 的資料採礦用戶端 |Microsoft 文件
ms.custom: ''
ms.date: 12/29/2017
...
-   [檢視模型](#bkmk_ViewModels)  
     ...
     大部分工具所產生的圖表都可以直接儲存至 Excel。 使用[Excel 中瀏覽模型&#40;SQL Server 資料採礦增益集&#41;](browsing-models-in-excel-sql-server-data-mining-add-ins.md)工具來瀏覽模型。  
 ...
 **資料準備**群組包含以下精靈，協助您檢閱和清除資料在資料採礦工作的準備。 此外，大部分的精靈還可以讓您將資料分成定型集和測試集。  
...
-   選用的外部資料來源連接。 您可以使用可定義為 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 資料來源的任何類型資料來建立模型，或者是直接使用已存在 Excel 中的資料。  
 ...
 [瀏覽資料&#40;SQL Server 資料採礦增益集&#41;](explore-data-sql-server-data-mining-add-ins.md)  
 **瀏覽資料**精靈可協助您了解的類型和您的資料表中的資料量，藉由繪製的分佈和所選資料行，一次一個的值。  
  
 [範例資料&#40;SQL Server 資料採礦增益集&#41;](sample-data-sql-server-data-mining-add-ins.md)  
 為定型和測試模型而建立正確類型的資料是資料採礦的一個重要部分，但是如果沒有正確的工具，也是會乏味無趣。 **範例資料**精靈可讓您輕鬆地將分割成兩個群組的模型中，一個用於建立模型，另一個用於測試它所用的資料。 您可以使用隨機取樣或超取樣。  
  
 [預測計算器&#40;適用於 Excel 的資料表分析工具&#41;](prediction-calculator-table-analysis-tools-for-excel.md)  
 **移除極端值**精靈為您提供幾個工具來找出並適當地處理極端值。 它會顯示值的散發以及極端值和其他資料的關聯性，並讓您決定是要移除或變更極端值。  
  
 [預測計算器&#40;適用於 Excel 的資料表分析工具&#41;](prediction-calculator-table-analysis-tools-for-excel.md)  
 **重定標籤**精靈可協助您建立新標籤資料，使其更易於了解分析的結果。 例如，您可以使用更具描述性的名稱來重新命名某個資料範圍，或者可以從清單中選擇代表性的值。  
  
##  <a name="bkmk_Model"></a> 建立模型和分析  
 上的選項**資料模型化**工具列的區段可讓您從資料衍生模式; 群組資料列的資料根據屬性，或瀏覽關聯。 此工具功能區中的精靈是根據 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] 中提供之功能強大的資料採礦演算法。 這些精靈可讓您自訂此演算法的行為及使用各種資料來源，與適用於 Excel 的資料表分析工具中的類似工具不同。  
 ...
 **分類**精靈支援使用這些 Microsoft 資料採礦演算法： 決策樹演算法、 羅吉斯迴歸、 貝氏機率分類、 類神經網路。  
 ...
 **關聯**精靈可協助您建立資料採礦模型使用 Microsoft 關聯規則演算法，它會偵測經常共同發生的項目或事件。 這類關聯模型對於提出建議特別有用。  
  
 **關聯**精靈使用 Microsoft 關聯規則演算法。  
 ...
 **預測**精靈使用 Microsoft 時間序列演算法。  
 ...
 已經很熟悉資料採礦了嗎？ 您可以使用**進階**資料模型化選項，以建立自訂資料結構，並建立模型使用不包含在其他工具和精靈的自訂項目。  
  
##  <a name="bkmk_Validate"></a> 測試、 查詢和驗證模型  
 使用精靈在**精確度和驗證**工具列來使用業界標準的測試，以驗證您模型的精確度，以及評估用來建立模型的資料集的可行性。  
...
##  <a name="bkmk_ViewModels"></a> 檢視模型  
 ...
 [記錄採礦模型&#40;資料採礦 excel 增益集&#41;](documenting-mining-models-data-mining-add-ins-for-excel.md)  
...
##  <a name="bkmk_UsageMgmt"></a> 管理文件，及部署  
 ...
 按一下**追蹤**檢視 Excel 用戶端之間的互動的持續性擷取和[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]伺服器。 所有活動都會儲存為 DMX 或 XMLA 陳述式，好讓您可以針對資料採礦工作階段進行疑難排解，或是儲存資訊供日後使用。  
 ...
 **快速入門**  
 按一下**入門**按鈕以啟動組態精靈，引導您建立的執行個體連接的程序[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]，並取得來進行資料採礦所需的權限。  
  
 **說明**  
 **協助**下拉式功能表可提供連結，線上說明、 網站和組態精靈，以協助您完成安裝及啟動資料採礦。  
