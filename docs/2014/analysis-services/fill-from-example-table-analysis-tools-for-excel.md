---
title: 範例 （適用於 Excel 的資料表分析工具） 填滿 |Microsoft 文件
ms.custom: ''
ms.date: 12/29/2017
...
ms.locfileid: "36146636"
---
# <a name="fill-from-example-table-analysis-tools-for-excel"></a>根據範例填滿 (適用於 Excel 的資料表分析工具)
  ![根據範例填滿資料表分析工具中的按鈕](media/tat-fillex.gif "根據範例填滿資料表分析工具中的按鈕")  
  
 **根據範例填滿**工具可協助您建立新的資料行的資料是根據現有的值。  
  
 例如，假設您的資料包含**採購金額**資料行，**訂單數量**資料行，和**Premier 客戶**根據某些公式使用的資料行其他資料行。 如果**Premier 客戶**資料行包含許多空白資料列，您可以使用**採購金額**和**訂單數量**做為輸入，來推斷遺漏值的資料行。 此工具會分析資料中現有的模式連同您所輸入的範例，並且預測每個客戶將獲指派的分類。  
...
5.  按一下 **[執行]**。  
     ...
     分析完成時，此工具會建立新**模式**工作表，其中包含分析結果。 此報表會列出找到的規則或關鍵影響因數，並且顯示每個規則的機率。  
...
## <a name="understanding-the-pattern-report"></a>了解模式報表  
 當您執行**根據範例填滿**工具，建立報告可提供有關已偵測到之模式的詳細資訊。 這些模式會用於推斷新的資料值。  
...
|「資料行」|ReplTest1|喜好|相對影響|  
|------------|-----------|------------|---------------------|  
|StateProvinceCode|AB|>500 公里|80%|  
  
 這表示中的 AB 值**StateProvinceCode**強烈預測資料行的運輸距離 > 500 公里為單位。  
  
 通常，預測都會根據複雜度遠勝於此範例的模式，而且報表可能會針對每項預測包含許多規則資料列。 所有規則的效果都會組合以衍生出預測值。  
  
> [!NOTE]  
>  **相對影響**會顯示為灰色列。 該橫條越長，這條規則也就越有可能是填滿值的預測。  
  
 此工具也會將新的資料行加入至原始資料的資料表，名為\<資料行名稱 > 擴充。  
  
 如果原始的資料資料行包含某個值，該值便會複製到新資料行中。 不過，如果原始的資料行包含空白的儲存格，新資料行便會包含由精靈所預測的值。  
  
## <a name="related-tools-and-information"></a>相關工具和資訊  
 您也可以使用[瀏覽資料](explore-data-sql-server-data-mining-add-ins.md)提供 Excel 中，若要檢查的 Excel 資料行中值分佈的資料採礦用戶端精靈。 如需詳細資訊，請參閱[總管和清除資料](exploring-and-cleaning-data.md)。  
