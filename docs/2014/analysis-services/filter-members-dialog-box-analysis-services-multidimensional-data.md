...
# <a name="filter-members-dialog-box-analysis-services---multidimensional-data"></a>篩選成員對話方塊 (Analysis Services - 多維度資料)
  使用 ** 中的 [篩選成員]**[!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)] 對話方塊，即可在 [維度設計師]**** 的 [瀏覽器]**** 索引標籤中瀏覽維度時，依成員標題、成員名稱、成員唯一的名稱、索引鍵資料行的值或值資料行的值，篩選目前層級的維度成員。  
