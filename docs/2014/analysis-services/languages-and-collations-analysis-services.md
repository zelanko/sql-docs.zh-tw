...
-   [變更執行個體上的預設語言或定序](#bkmk_defaultLang)  
  
-   [變更 Cube 上的語言或定序](#bkmk_cube)  
  
-   [使用 XMLA 變更資料模型中的語言和定序](#bkmk_XMLA)  
  
-   [透過 EnableFast1033Locale 提高英文地區設定的效能](#bkmk_enablefast1033)  
...
-   **資料庫維度**： 雖然設計工具會公開`Language`和`Collation`資料庫維度，此物件上設定屬性的屬性不是很有用。 資料庫維度不會做為獨立物件使用，因此可能很難或不可能利用您定義的屬性。 在 Cube 中，維度一律會從其 Cube 父系繼承 `Language` 和 `Collation`， 並忽略您之前在獨立資料庫維度物件上設定的所有值。  
...
###  <a name="bkmk_lcid"></a> 語言屬性的值是地區設定識別碼 (LCID)  
 ...
 您可以使用十六進位或十進位格式設定 LCID。 一些有效值範例`Language`屬性包括：  
