...
# <a name="tabular-modeling-ssas-tabular"></a>表格式模型化 (SSAS 表格式)
 ...
 表格式模型支援兩種模式的資料存取：快取的模式與 DirectQuery 模式。 在快取模式中，您可以整合來自多個來源的資料，包括關聯式資料庫、資料摘要及一般文字檔。 在 DirectQuery 模式，您可以略過記憶體中模型，讓用戶端應用程式可以直接在 (SQL Server 關聯式) 來源中查詢資料。  
