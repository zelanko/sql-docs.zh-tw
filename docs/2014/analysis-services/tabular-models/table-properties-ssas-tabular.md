...
##  <a name="table-properties"></a><a name="bkmk_properties"></a>資料表屬性  
 **基本**  
  
|屬性|預設值|描述|  
|--------------|---------------------|-----------------|  
|**連接名稱**|\<connection name>|資料表的資料來源連接名稱。<br /><br /> 若要編輯連接，請按一下此按鈕。|  
|**隱含**|False|指定是否在報表用戶端欄位清單中隱藏資料表。|  
...
|**資料表名稱**|\<friendly name>|指定資料表的易記名稱。 資料表名稱可在使用 [資料表匯入精靈] 匯入資料表時指定，或在匯入後隨時指定。 模型中的資料表名稱可以不同於在來源的關聯資料表。 資料表易記名稱出現在報表用戶端應用程式欄位清單中以及 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]中的模型資料庫。|  
...
###  <a name="to-configure-table-property-settings"></a><a name="bkmk_config_prop"></a>若要設定資料表屬性設定  
