---
title: 從 PowerPivot 匯入（SSAS 表格式） |Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
>  可以加入至模型中內嵌資料的上限為 10,000 個資列列。 如果您從 PowerPivot 匯入模型並看到錯誤：「資料已截斷。 貼上的資料表不能包含超過10000個數據列」，您應該將內嵌資料移到另一個資料來源（例如 SQL Server 中的資料表）來修改 PowerPivot 模型，然後重新匯入。  
 ...
 如果工作空間資料庫位於本機 Analysis Services 執行個體上，您可以從 PowerPivot 活頁簿匯入中繼資料和資料。 中繼資料會從活頁簿複製，並用來建立表格式模型專案。 然後，資料會從活頁簿複製並儲存在專案的工作空間資料庫中（複製/貼上的資料除外，儲存在 model.bim 檔案中）。  
