...
-   資料列識別碼：指定只包含唯一值的資料行，允許該資料**行**做為內部群組索引鍵使用。

-   **保留唯一**資料列：指定哪些資料行提供應視為唯一的值，即使它們是重複專案（例如，員工名字和姓氏，適用于兩個以上的員工共用相同名稱的情況）。

-   [**預設標籤**]：指定哪個資料行提供顯示名稱來表示資料列資料（例如，員工記錄中的員工名稱）。

-   **預設影像**：指定哪個資料行提供代表資料列資料的影像（例如，員工記錄中的相片識別碼）。
...
##  <a name="bkmk_optimizeforlayout"></a>針對特定版面配置優化
