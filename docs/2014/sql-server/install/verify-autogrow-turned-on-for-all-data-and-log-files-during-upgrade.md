 ...
 **tempdb 資料和記錄檔**  
 ...
 在舊版的中 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ， **tempdb**資料庫是用來儲存下列物件：  
...
-   當您建立或重建索引時暫時排序的結果 (如果指定了 SORT_IN_TEMPDB 的話)。  
 ...
 其他物件也會使用**tempdb**資料庫。 下表列出 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 會導致**tempdb**資料和記錄檔產生額外磁碟空間需求的功能變更或新增專案。  
...
|暫存資料表和暫存變數中繼資料的快取|針對在中繼資料快取中快取的臨時表和暫存變數的每個中繼資料 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，會為**tempdb**配置兩個額外的頁面。<br /><br /> 如果預存程序或觸發程序建立暫存資料表或暫存變數，則在該程序或觸發程序完成執行時，將不會刪除暫存物件。 而是將暫存物件截斷為單一頁面，並在下次執行該程序或觸發程序時重複使用。|  
...
|大型物件 (LOB) 資料類型<br /><br /> LOB 變數與參數|資料類型 `varchar(max)` 、 `nvarchar(max)` 、 **Varbinary （max）文字**、 `ntext` `image,` 和 `xml` 是大型物件類型。<br /><br /> 在資料庫上啟用以資料列版本設定為基礎的交易隔離等級，並對大型物件進行修改時，會將已變更的 LOB 片段複製到**tempdb**中的版本存放區。<br /><br /> 定義為大型物件資料類型的參數會儲存在**tempdb**中。|  
