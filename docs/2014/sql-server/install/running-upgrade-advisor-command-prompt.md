...
|`Databases` 元素的 `SQLServer`|包含要分析之資料庫的清單。|選擇性的一次，每個`SQLServer`項目。 如果這個元素不存在，就會分析執行個體中的所有資料庫。|  
|`Database` 元素的 `SQLServer`|指定要分析之資料庫的名稱。|(必要) 如果 `Databases` 元素存在，就出現一或多次。 如果 `Database` 元素包含 "*" 值，就會分析執行個體中的所有資料庫。 沒有預設值。|  
|`TraceFiles`|包含要分析之追蹤檔案的清單。|選擇性的一次，每個`SQLServer`項目。|  
|`TraceFile`|指定要分析之追蹤檔案的路徑和名稱。|(必要) 如果 `TraceFiles` 元素存在，就出現一或多次。 沒有預設值。|  
|`BatchFiles`|包含要分析之批次檔案的清單。|選擇性的一次，每個`SQLServer`項目。|  
