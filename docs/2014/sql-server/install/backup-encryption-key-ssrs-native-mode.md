 ...
 **密碼**  
 請輸入可保護檔案的密碼，以防止未經授權的存取。 只有知道此密碼的使用者才能夠還原在檔案內鎖定的金鑰。 
  [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 會強制實施增強式密碼原則。 此密碼至少必須為 8 個字元，以及包含大小寫英數字元和至少一個符號字元的組合。  
