---
title: Upgrade Advisor How-to Topics&lt |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.suite: ''
ms.technology:
- database-engine
ms.tgt_pltfrm: ''
ms.topic: conceptual
helpviewer_keywords:
- SQL Server Upgrade Advisor, how-to topics
- Upgrade Advisor [SQL Server], how-to topics
ms.assetid: d73bf759-58f7-4789-af49-ac8ca94f913d
caps.latest.revision: 19
author: mashamsft
ms.author: mathoma
manager: craigg
ms.openlocfilehash: 009b42d22345e8b58abdc5f26092cc113da259b9
ms.sourcegitcommit: c18fadce27f330e1d4f36549414e5c84ba2f46c2
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 07/02/2018
ms.locfileid: "37323828"
---
# <a name="upgrade-advisor-how-to-topics"></a>Upgrade Advisor 使用說明主題
  此章節包含使用 Upgrade Advisor 的一般程序。  
  
## <a name="in-this-section"></a>本節內容  
 [如何：安裝升級建議程式](../../../2014/sql-server/install/how-to-install-upgrade-advisor.md)  
  
 [如何：啟動升級建議程式](../../../2014/sql-server/install/how-to-launch-upgrade-advisor.md)  
  
 [如何：執行升級建議程式分析精靈](../../../2014/sql-server/install/how-to-run-the-upgrade-advisor-analysis-wizard.md)  
  
 [如何：檢視升級建議程式報表](../../../2014/sql-server/install/how-to-view-an-upgrade-advisor-report.md)  
  
 [如何：篩選報表](../../../2014/sql-server/install/how-to-filter-reports.md)  
  
 [如何：匯出報表](../../../2014/sql-server/install/how-to-export-reports.md)  
  
## <a name="see-also"></a>另請參閱  
 [使用升級建議程式](../../../2014/sql-server/install/working-with-upgrade-advisor.md)  
  
  
