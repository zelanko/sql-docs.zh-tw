...
|安裝 SharePoint Server 2010 Enterprise Edition 或 Enterprise Evaluation Edition。|安裝 SharePoint 時，您可以選擇在完成安裝程式之後，不要執行 [SharePoint 2010 產品設定精靈]，以在稍後設定伺服器陣列。 等候設定伺服器陣列可讓您使用稍後步驟中[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]安裝的資料庫引擎實例，做為伺服器陣列的資料庫伺服器。 若要設定伺服器陣列，請使用 PowerPivot 組態工具。 此工具包括佈建尚未設定之伺服器陣列的動作。|  
