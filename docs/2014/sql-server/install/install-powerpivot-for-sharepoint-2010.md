...
|**[!INCLUDE[applies](../../includes/applies-md.md)]**  SharePoint 2010|  
 ...
 如需有關如何安裝指示[!INCLUDE[ssGemini](../../includes/ssgemini-md.md)]和[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]在相同伺服器上，請參閱[部署檢查清單： Reporting Services、 Power View 和 PowerPivot for SharePoint](deployment-checklist-reporting-services-power-view-power-pivot-for-sharepoint.md)。  
...
5.  您必須要有網域使用者帳戶，才能佈建 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)]。 在 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] 安裝中，Analysis Services 服務帳戶必須是可以從管理中心進行管理的網域使用者帳戶。 您輸入的帳戶和認證上**伺服器組態**這份文件中的步驟一部分的頁面。  
...
7.  [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] 執行個體不得為 SQL Server 容錯移轉叢集的一部分。 使用 SharePoint 產品的高可用性功能。 例如，Excel Services 會管理 PowerPivot for SharePoint 伺服器的負載平衡。 如需詳細資訊，請參閱[管理 Excel Services 資料模型設定 (SharePoint Server 2013)](http://technet.microsoft.com/library/jj219780.aspx) (http://technet.microsoft.com/library/jj219780.aspx)。  
...
7.  在**安裝規則** 頁面上，安裝程式會識別可能導致無法安裝的任何問題。 檢閱清單以判斷安裝程式是否已偵測到系統上的潛在問題。  
...
9. 您可選擇性地在安裝中加入一項 Database Engine 的執行個體。 若您正設定新的伺服器陣列，且需要資料庫伺服器以執行伺服器陣列的設定與內容資料庫，則可能會進行此作業。 若您加入 Database Engine，將會安裝為 PowerPivot 具名執行個體。 每當您需要指定 （例如，在伺服陣列組態精靈中如果您要使用該精靈設定伺服器陣列），這個執行個體的連接以此格式輸入資料庫名稱： <`servername`> \PowerPivot。  
  
     ![GMNI_SetupUI_FeatureRole](../../../2014/sql-server/install/media/gmni-setupui-featurerole.gif "GMNI_SetupUI_FeatureRole")  
  
10. 按 [下一步] 。  
  
11. 在**特徵選取**頁面上，將會安裝的功能的唯讀清單會顯示僅供參考之用。 您不能加入或移除預先為這個角色選取的項目。 按 [下一步] 。  
...
16. 若您正安裝 Database Engine，則隨即會出現 Database Engine 組態頁面。 在資料庫引擎組態] 中，按一下 [**加入目前使用者**授與您的使用者帳戶資料庫引擎執行個體上的系統管理員權限。 按一下**新增**新增其他帳戶。 按 [下一步] 。  
...
20. 按一下**下一步**每個其餘的頁面，直到您看到 [準備安裝] 頁面上。  
  
21. 按一下 **[安裝]**。  
  
> [!TIP]  
>  如果您需要對 SQL Server 安裝，請參閱[檢視與讀取 SQL Server Setup Log Files](../../database-engine/install-windows/view-and-read-sql-server-setup-log-files.md)。  
...
-   [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 版本的 MSOLAP 包含在 SQL Server 安裝程式中，因此只有在應用程式伺服器不是 PowerPivot 應用程式伺服器時，才需要明確安裝 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 版本的 MSOLAP。  
    ...
    > [!NOTE]  
    >  Excel Calculation Services 應用程式伺服器還需要的檔案執行個體**Microsoft.AnalysisServices.Xmla.dll**在全域組件。 若要將此 .dll 安裝在應用程式伺服器上，請安裝 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]。 選取上的 「 管理工具-完整**特徵選取**SQL Server 安裝程式精靈 頁面。  
...
##  <a name="bkmk_verify"></a> 步驟 4： 驗證安裝  
 在這個最後步驟中，您將驗證 SharePoint 2010 和 [!INCLUDE[ssGeminiShort](../../includes/ssgeminishort-md.md)] 都能完整運作。 如需指示，請參閱[Verify a PowerPivot for SharePoint 安裝](../../analysis-services/instances/install-windows/verify-a-power-pivot-for-sharepoint-installation.md)。  
