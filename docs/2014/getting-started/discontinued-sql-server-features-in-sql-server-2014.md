...
|[!INCLUDE[ssRSnoversion](../includes/ssrsnoversion-md.md)]-「排程事件和報表傳遞」和「Web 服務和 HTTP 存取」|編輯 RSReportServer.config 組態檔。|  
|命令列選項|這個版本不支援。|  
...
|USESYSDB|這個版本中沒有對等項目。|  
 ...
 <sup>2</sup>從 [!INCLUDE[ssKatmai](../includes/sskatmai-md.md)]開始，指定/Action = EditionUpgrade，在不使用原始安裝媒體的情況下，隨時將 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 的現有版本升級至不同的版本。 如需有關支援之版本與版別升級的詳細資訊，請參閱＜ [Supported Version and Edition Upgrades](../database-engine/install-windows/supported-version-and-edition-upgrades.md)＞。  
