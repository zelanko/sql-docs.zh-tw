...
3.  [活動監控] 畫面會在活動方格中顯示每個活動的相關資訊。 活動方格會顯示有關每個 DQS 活動的下列資訊：  
    ...
    |**作用中**|指出此活動目前是否使用中。 其值如下：<br /><br /> **Active**:目前執行的活動。<br /><br /> **結束**:活動已完成。<br /><br /> **終止**:活動具有已經使用活動監控 畫面來終止 DQS 系統管理員或使用者已取消活動時執行的個別功能區域中[!INCLUDE[ssDQSClient](../includes/ssdqsclient-md.md)]。|  
    ...
    |**目前的狀態**|指出活動的目前狀態。 活動狀態是由最後一個計算程序所決定。 其值如下：<br /><br /> **執行**:在計算程序正在執行。<br /><br /> **成功**:執行任何計算程序之前，要將狀態設定為**Succeeded**。 同樣地，在計算程序成功結束之後，狀態也設為 **[成功]**。<br /><br /> **無法**:無法在計算程序。<br /><br /> **停止**:在計算程序已停止。<br /><br /> <br /><br /> 注意：一個活動中可以有數個計算程序，例如多次執行探索程序 (在知識探索活動內部)。 因此，狀態可能會在活動存留期間變更許多次。|  
    |**DQKB**|用於活動的知識庫名稱。|  
...
1.  決定篩選準則：您想要根據活動方格內其中一個資料行的值 (以值為基礎)、根據日期範圍，還是根據這兩者篩選活動記錄。  
    ...
    1.  **值為基礎的篩選**:選取中的篩選準則**篩選依據**清單，然後再選取適當的值，以篩選所**值**清單。 在 **[篩選依據]** 清單中選取選項時， **[值]** 清單會依據可能值來更新。 您可以篩選活動記錄中的下列欄位：**作用中**，**型別**，**子類型**，**目前的狀態**， **DQKB**，以及**使用者**。  
  
    2.  **日期範圍為基礎的篩選**:選取適當的日期，在**日期**並**至今**日期控制項。 根據預設， **[開始日期]** 中顯示的日期是目前日期的兩天前，而 **[至今]** 中顯示的日期是目前日期。 篩選不是根據「開始」  和「至今」  日期執行，而是依照範圍執行。 這表示選取之日期範圍內執行的每個活動都會顯示。  
...
2.  下方窗格會在下列 2 個索引標籤底下顯示選取活動的活動詳細資料：  
    ...
    -   **活動步驟**:顯示方格之計算處理序 （活動步驟） 與所選活動相關聯。 此索引標籤底下的活動可能會顯示許多活動步驟。如果使用者執行了活動中相同的活動步驟許多次，就可能會發生這種情況。 例如，停止並重新啟動活動步驟。 此索引標籤底下的方格會顯示每個與活動相關聯的活動步驟的下列資訊：**型別**，**目前的狀態**，**開始時間**，**經過時間**，並**結束時間**。  
  
    -   **Profiler**:顯示目前和歷程記錄活動的程式碼剖析資訊。 若為目前活動，它會包含部分但一致的資訊。 當您將對應的活動詳細資料匯出至 Excel 檔案時，活動的分析資訊就會匯出至 Excel 檔案。 這項資訊會顯示在匯出之 Excel 檔案的 [分析工具 - 來源] 和 [分析工具 - 欄位] 工作表中。  
...
3.  此時，系統會提示您指定要儲存之 Excel 檔案的名稱和位置。 匯出的 Excel 檔案包含下列工作表：  
    ...
    |分析工具 - 來源|針對**清理**子型別，就會包含有關活動的下列資訊： 記錄、 正確的記錄、 已更正的記錄，以及無效的記錄。<br /><br /> 針對**知識探索**，**定義域管理**，**比對原則**，以及**比對**子類型，就會包含下列活動的相關資訊： 記錄、 總計值、 新的值，唯一的值和新的唯一值。|  
    |分析工具 - 欄位|針對**清理**並**SSIS 清理**子類型，就會包含有關活動的下列資訊： 欄位、 網域、 更正的值，建議的值、 完整性和精確度。<br /><br /> 針對**知識探索**，**定義域管理**，**比對原則**，以及**比對**子類型，就會包含下列活動的相關資訊： 欄位中，網域中，新的、 唯一的有效的網域，和完整性。|  
