...
##  <a name="FirstStep"></a> 第一個步驟：開始知識探索  
...
5.  按 **[下一步]** ，繼續前往知識探索精靈的 **[探索]** 頁面。 您也可以選取下列項目：  
    ...
    -   按一下 **[關閉]** ，返回 DQS 首頁並儲存工作。 系統會針對您封鎖知識庫，而且 **[開啟知識庫]** 畫面中知識庫資料表的知識庫狀態將會是 **[探索 - 對應]**。 按一下後**關閉**，以執行定義域管理] 活動中，您必須按一下 [**知識探索**從**開啟知識庫**畫面上，請繼續進行**知識庫管理：管理定義域詞彙**畫面上，按一下**完成**，然後按一下**是**發行知識庫或**否**來儲存知識庫工作並結束。  
...
2.  執行探索程序時，請檢查每一個探索步驟所顯示的完成狀態：**前置處理資料錄**，**正在執行定義域規則**，以及**執行探索**。 每一個階段都將顯示完成百分比和完成狀態。  
...
6.  按 **[下一步]** ，繼續前往知識探索精靈的 **[管理定義域值]** 頁面。 在此頁面上，您可以修改加入至知識庫定義域的知識。 您也可以選取下列項目：  
    ...
    -   按一下 **[關閉]** ，返回 DQS 首頁並儲存工作。 系統會針對您封鎖知識庫，而且 **[開啟知識庫]** 畫面中知識庫資料表的知識庫狀態將會是 **[探索 - 探索]**。 按一下後**關閉**，以執行定義域管理] 活動中，您必須按一下 [**知識探索**從**開啟知識庫**畫面上，請繼續進行**知識庫管理：管理定義域詞彙**畫面上，按一下**完成**，然後按一下**是**發行知識庫或**否**來儲存知識庫工作並結束。  
...
4.  若要變更值的狀態，請依照以下方式繼續進行：  
    ...
    1.  **將選取的定義域值設為更正**:若要將值的狀態從錯誤或無效變更為正確，選取值，然後**組選取的定義域值設為更正**（勾選） 從圖示列的向下箭號，或從 [類型] 下拉式清單。 如果錯誤或無效值與更正值群組在一起，請在作業之後刪除該值。  
  
    2.  **將選取的定義域值設為錯誤**:若要將值的狀態從更正或無效變更為錯誤，選取值，然後**將選取的定義域值設為錯誤**（十字符號） 圖示，將它從圖示列的向下箭號，或從 [類型] 下拉式清單。 您可以在 **[更正為]** 資料行中輸入更正，或是將它保留空白。  
  
    3.  **將選取的定義域值設為無效**:若要變更值的狀態從更正或錯誤不正確，選取值，然後**將選取的定義域值設為無效**（三角形） 圖示，將它從圖示列的向下箭號，或從 [類型] 下拉式清單。 您可以在 **[更正為]** 資料行中輸入更正，或是將它保留空白。  
  
    4.  **若要正確**:為錯誤或無效的值設定之後，輸入新值**更正為**資料行。 DQS 將會為取代值新增資料列、將它指定為正確的值，然後將兩個值群組在一起。 新的值將會顯示為前置值，前置值會以粗體顯示，而錯誤或無效的值則縮排。  
  
5.  若要將值指定為一組同義字，請選取多個正確值，然後依照以下方式繼續進行：  
  
    -   **將選取的定義域值設為同義字**:按一下即可將選取的值設為同義字。 DQS 會將其中一個值指定為將用來取代其他值的前置值。  
  
        > [!NOTE]  
        >  如果您選取群組中的兩個或多個值以及群組外的另一個值，然後將這些值設定為同義字，您會得到不正確的錯誤訊息。 在關閉錯誤訊息快顯視窗之後，這些值將會正確設定為同義字。  
  
    -   **中斷選取的同義字之間的關聯**:按一下 可復原同義字的指定。  
  
    -   **將選取的定義域值設為其群組的前置值**:未指定為前置值，群組中選取值，然後按一下來變更群組的前置值**將選取的定義域值設為其群組的前置值** 按鈕。  
  
6.  **拼字檢查**:如果您已啟用拼字檢查，在 [定義域屬性] 頁面中的，尋找具有波浪式紅色底線，表示拼字檢查有建議更正任何值。 以滑鼠右鍵按一下有底線的值，然後選取適用的更正。 值類型會變成 (或維持) 錯誤，而且更正將會加入至 **[更正為]** 資料行。 按一下向下箭號，以查看其他建議的更正。 手動輸入更正，將其加入至拼字檢查字典，而且能夠將其選取為更正。 如需相關資訊，請參閱 [Use the DQS Speller](../../2014/data-quality-services/use-the-dqs-speller.md) 及 [Set Domain Properties](../../2014/data-quality-services/set-domain-properties.md)。  
  
    > [!NOTE]  
    >  若要使用拼字檢查，您可以在 **[定義域屬性]** 頁面中啟用此功能，或者如果 **[定義域屬性]** 頁面中已停用此功能，您可以按一下 **[管理資料探索結果]** 頁面上的 **[啟用/停用拼字檢查]** 圖示，在此頁面上啟用此功能。  
  
7.  **加入新的定義域值**:加入網域中的新值，依序按一下**加入新的定義域值**按鈕，以在資料表結尾加入資料列。 在您輸入值之後，此資料列將會依字母順序重新定位。  
  
8.  **從 Excel 匯入定義域值**:Excel 試算表加入新的值，請按一下向下的箭號**匯入值**圖示，然後選取**從 Excel 匯入定義域值**。 輸入檔案名稱，並在適當情況下選取 **[使用第一個資料列做為標頭]** ，然後按一下 **[確定]**。 如需詳細資訊，請參閱 [將 Excel 檔案中的值匯入定義域中](../../2014/data-quality-services/import-values-from-an-excel-file-into-a-domain.md)。  
  
9. **匯入專案值**:資料品質專案加入新的值，請按一下向下的箭號**匯入值**圖示，然後選取**匯入專案值**。 輸入檔案名稱，並在適當情況下選取 **[使用第一個資料列做為標頭]** ，然後按一下 **[確定]**。 選取要匯入的值來自於哪一個專案，然後按一下 **[確定]**。 隨即顯示匯入的值。 按一下 **[完成]**。 如需詳細資訊，請參閱＜將專案值匯入定義域＞。  
  
10. **刪除選取的定義域值**:移除網域中的一或多個現有的值，以選取值，然後按一下**刪除選取的定義域值** 按鈕。 DQS_NULL 的項目無法刪除，所以如果您選擇多個要刪除的值，而且 DQS_NULL 的項目是其中一個，此作業將會失敗。  
...
12. 您也可以按一下以下項目：  
    ...
    -   按一下 **[上一步]** ，返回 **[探索]** 頁面。 按一下後**關閉**，以執行定義域管理] 活動中，您必須按一下 [**知識探索**從**開啟知識庫**畫面上，請繼續進行**知識庫管理：管理定義域詞彙**畫面上，按一下**完成**，然後按一下**是**發行知識庫或**否**來儲存知識庫工作並結束。  
...
-   **修正：** 這是值屬於定義域，而且沒有任何語法錯誤。 例如，"Chicago" 在城市定義域中是正確的。  
  
-   **錯誤：** 這是值屬於定義域，但不是正確值，這個值。 例如，"Shicago" (而非 "Chicago") 在城市定義域中是錯誤的。 DQS 如果在探索程序中偵測到某個值有語法錯誤和關聯的更正，則會將此值指定為錯誤。 語法錯誤包括拼字錯誤。  
  
-   **不是正確：** 這是不屬於網域，而且沒有更正的值。 例如，"12345" 值在城市定義域中是無效的。 當某個值未通過定義域規則時，DQS 會將它指定為無效的值。  
...
-   **總計值**:每個欄位以及總共找到的總計值數量  
  
-   **新的值**:自上次探索程序，以及其總計值百分比是新增多少值總計為每個欄位和所有對應的欄位  
  
-   **唯一值**:總計值為唯一的每個欄位和所有對應的欄位數目以及其總計值百分比  
  
-   **新的唯一值**:自上次探索程序，以及其總計值百分比是新增多少個唯一值的每個欄位和所有對應的欄位  
  
-   **在 定義域值中有效**:值的數量總計為每個欄位和所有對應的欄位有效，以及其總計值百分比  
...
-   **新**:新的值和新的值相較於欄位中的現有值的百分比數目  
  
-   **唯一**:中的欄位以及其總計百分比的唯一記錄的數目  
  
-   **在定義域中有效**:有效的定義域值和其總計百分比的數目  
  
-   **完整性**:完整的每一個來源欄位對應的比對的工作  
