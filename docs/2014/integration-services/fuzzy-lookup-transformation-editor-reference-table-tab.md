---
title: 模糊查閱轉換編輯器 （參考資料表索引標籤） |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
> [!NOTE]  
>  `Exhaustive`和`MaxMemoryUsage`模糊查閱轉換的屬性不適用於**模糊查閱轉換編輯器**，但是可以透過設定**進階編輯器**。 此外，大於 100 的值`MaxOutputMatchesPerInput`可以只在指定**進階編輯器**。 如需有關這些屬性的詳細資訊，請參閱＜ [Transformation Custom Properties](data-flow/transformations/transformation-custom-properties.md)＞的「模糊查閱轉換」一節。  
