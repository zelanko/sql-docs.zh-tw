...
# <a name="multiple-flat-files-connection-manager"></a>多個一般檔案連接管理員
 ...
 當您將「多個一般檔案」連接管理員加入封裝[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]時，會建立連線管理員，在執行時間解析為「多個一般檔案」連接、在「多個一般檔案」連接管理員上設定屬性，以及將「多`Connections`個一般檔案」連接管理員加入封裝的集合。  
