...
## <a name="configuration-the-smtp-connection-manager"></a>設定 SMTP 連接管理員  
 當您新增 SMTP 連接管理員加入封裝時，[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]建立連接管理員，將會解析為 SMTP 連接，在執行階段、 設定連接管理員屬性，以及將連接管理員加入`Connections`集合封裝。 `ConnectionManagerType`連接管理員的屬性設定為`SMTP`。  
