...
```
 ...
 在此範例中，腳本元件會根據您所設定的輸出名稱，為您產生**DirectRowTo \<OutputBufferX> **方法。 您可以使用類似的程式碼將錯誤資料列導向模擬的錯誤輸出。
...
2.  將來源的輸出或另一個轉換的輸出連接到 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 設計師中的新轉換元件。 此輸出應提供範例資料庫之**Person. Address**資料表中的資料 `AdventureWorks` ，其中包含**AddressID**和**City**資料行。
...
2.  將來源的輸出或另一個轉換的輸出連接到 [!INCLUDE[ssIS](../../includes/ssis-md.md)] 設計師中的新轉換元件。 這個輸出應該從範例資料庫的**Person. Address**資料表中提供資料 `AdventureWorks` ，其中至少包含**AddressID**和**City**資料行。
