...
### <a name="writing-your-custom-code"></a>撰寫您的自訂程式碼
 ...
 您可能也會想要覆寫 ProcessInput 方法本身，好讓您在輸入緩衝區內執行迴圈及針對每一個資料列呼叫 ProcessInputRow 之前或之後，可以執行其他初步或最終處理。 例如，本主題的其中一個程式碼範例會覆寫 ProcessInput，以計算特定城市的位址數目，做為 ProcessInputRow 迴圈的`.`資料列。此範例會在處理完所有資料列之後，將摘要值寫入第二個輸出。 此範例會完成 ProcessInput 中的輸出，因為呼叫 PostExecute 時輸出緩衝區將不再可用。
