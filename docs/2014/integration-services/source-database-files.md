...
## <a name="options"></a>選項。  
 **來源檔案**  
 在來源伺服器上，將會被傳送的資料庫檔案名稱。 **來源**檔案是唯讀的。  
  
 **源資料夾**  
 在來源伺服器上，要傳送之資料庫檔案所在的資料夾。 **源資料夾**是唯讀的。  
  
 **網路檔案共用**  
 在來源伺服器上，會從中傳送資料庫檔案的網路共用資料夾。 當您在離線模式中傳送資料庫時，請在 [**傳送資料庫工作編輯器**] 對話方塊的 [**資料庫**] 頁面中指定 [**為 [databaseoffline** For **Method** ]，以使用**網路檔案共用**。  
