...
## <a name="configuring-a-single-transaction"></a>設定單一交易  
 在這個選項中，封裝本身會起始單一交易。 您可以藉由將封裝的 TransactionOption 屬性設定為，來設定封裝來起始此交易 `Required` 。  
  
 接著，您可以在這個單一交易中編列特定工作和容器。 若要將工作或容器登記到交易中，您可以將該工作或容器的 TransactionOption 屬性設定為 `Supported` 。  
...
## <a name="configuring-multiple-transactions"></a>設定多個交易  
 在這個選項中，雖然封裝本身支援交易，但是不會啟動交易。 您可以藉由將封裝的 TransactionOption 屬性設定為，來設定封裝以支援交易 `Supported` 。  
  
 接著，您可以將封裝內部的所需工作和容器設定成起始或參與交易。 若要將工作或容器設定成起始交易，您可以將該工作或容器的 TransactionOption 屬性設為 `Required` 。  
