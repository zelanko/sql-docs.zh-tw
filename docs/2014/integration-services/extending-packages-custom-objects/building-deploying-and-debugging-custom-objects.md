...
##  <a name="top"></a>建立、部署和偵錯工具之自訂物件的步驟 Integration Services  
...
1.  [簽署](#signing)要使用強式名稱產生的元件。  
  
2.  [建立](#building)元件。  
  
3.  [](#deploying)藉由將元件移動或複製到適當[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]的資料夾來部署它。  
  
4.  將元件[安裝](#installing)在全域組件快取（GAC）中。  
  
     此物件會自動加入工具箱。  
  
5.  如有必要，請針對部署[進行疑難排解](#troubleshooting)。  
  
6.  [測試](#testing)和偵錯工具代碼。  
