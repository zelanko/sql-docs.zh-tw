...
## <a name="example"></a>範例  
 雖然 SQL Server 自訂連接管理員範例並不需要 `string` 類型之三個屬性的自訂持續性，不過下列程式碼所顯示的自訂程式碼範例，將需要保存連接管理員及其屬性。 包含此程式碼的類別必須實作 <xref:Microsoft.SqlServer.Dts.Runtime.IDTSComponentPersist> 介面。  
