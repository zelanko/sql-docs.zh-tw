...
ms.locfileid: "62768654"
---
# <a name="developing-custom-objects-for-integration-services"></a>開發 Integration Services 的自訂物件
  當隨附[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]的控制流程和資料流程物件無法完全滿足您的需求時，您可以自行開發許多類型的自訂物件，包括：  
  
-   **自訂**工作。  
  
-   **自訂連接管理員。** 連線至目前不支援的外部資料來源。  
  
-   **自訂記錄提供者。** 使用目前不支援的格式來記錄套件事件。  
  
-   **自訂枚舉器。** 支援反覆運算一組目前不支援其格式的物件或值。  
