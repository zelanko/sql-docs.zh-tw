...
### <a name="applying-the-dtslogprovider-attribute"></a>套用 DtsLogProvider 屬性  
 將 <xref:Microsoft.SqlServer.Dts.Runtime.DtsLogProviderAttribute> 屬性套用至您已建立的類別，以便將它識別為記錄提供者。 此屬性會提供記錄提供者的名稱和描述等設計階段資訊。 `DisplayName`和`Description`屬性的屬性會對應到**名稱**和`Description`中顯示的資料行**設定 SSIS 記錄**編輯器，以顯示時設定記錄中的封裝[!INCLUDE[ssBIDevStudioFull](../../../includes/ssbidevstudiofull-md.md)]。  
