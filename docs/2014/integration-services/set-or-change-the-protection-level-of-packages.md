...
# <a name="set-or-change-the-protection-level-of-packages"></a>設定或變更封裝的保護等級
 ...
 如需如何在封裝生命週期`ProtectionLevel`的不同階段判斷封裝適用之設定的詳細資訊，請參閱封裝[中的敏感性資料存取控制](security/access-control-for-sensitive-data-in-packages.md)。 如需 [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] 安全性功能的概觀，請參閱[安全性概觀 &#40;Integration Services&#41;](security/security-overview-integration-services.md)。  
...
1.  在`ProtectionLevel` [設定封裝的保護等級](security/access-control-for-sensitive-data-in-packages.md)主題中，檢查屬性的可用值，並決定您的封裝的適當值。  
...
5.  在 [屬性視窗的 [**安全**] 群組中，為`ProtectionLevel`屬性選取適當的值。  
...
2.  在[Dtutil 公用程式](dtutil-utility.md)主題`Encrypt`中，檢查選項的對應，並判斷要當做所選`ProtectionLevel`屬性值使用的適當整數。  
