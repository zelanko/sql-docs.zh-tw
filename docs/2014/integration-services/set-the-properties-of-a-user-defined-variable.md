---
title: 設定使用者定義變數的屬性 |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
-   變數的值可以由 Value 或 Expression 屬性設定。 根據預設，EvaluateAsExpression 屬性設定為`False`和 Value 屬性設定變數的值。 若要使用運算式來設定值，您必須先設 EvaluateAsExpression `True`，然後提供的運算式中的 Expression 屬性。 Value 屬性會自動設為運算式的評估結果。  
  
-   ValueType 屬性包含 Value 屬性中之值的資料類型。 運算式設定 Value 之後，ValueType 就會自動更新為與運算式之評估結果相容的資料類型。 例如，如果值包含 0 與 ValueType 屬性包含**Int32** ，然後設 getdate （） 的運算式值包含目前的日期和時間，ValueType 設定為`DateTime`。  
...
-   若要使用運算式設定變數值，先確認變數資料類型與運算式的評估結果相容，然後在提供的運算式`Expression`資料行**變數**視窗。 中的 EvaluateAsExpression 屬性**屬性**視窗會自動設為`True`。  
...
-   透過變數的 [變數] 視窗，可以存取 [運算式產生器] 對話方塊。 您可使用此工具建立、驗證和評估運算式。 如需詳細資訊，請參閱[運算式產生器](expressions/expression-builder.md)和 [Integration Services &#40;SSIS&#41; 運算式](expressions/integration-services-ssis-expressions.md)。  
 ...
 在這兩**變數**和**屬性**視窗中，如果您將運算式指派給變數，和`EvaluateAsExpression`設`True`，您無法變更變數資料類型。  
  
 **設定命名空間和名稱屬性**  
  
 值`Name`和`Namespace`Unicode Standard 2.0 中或底線 (_) 所定義的屬性必須以字母字元字母開頭。 後續的字元可以是 Unicode Standard 2.0 中定義的字母或數字，或是底線 (\_)。  
...
5.  在清單中，選取的變數，然後再更新中的值`Name`，**資料型別**， `Value`， `Namespace`，**引發變更事件**，**描述、** 和`Expression`資料行。  
