...
4.  確認值`AllowConfigurationChanges`屬性是**true**和 XML 包含`Package`的兩個封裝中，每個項目`MiscellaneousFile`的四個非封裝檔案中，每個項目和`ConfigurationFile`兩個 XML 組態檔的每個項目。  
