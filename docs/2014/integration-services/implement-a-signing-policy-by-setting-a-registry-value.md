...
# <a name="implement-a-signing-policy-by-setting-a-registry-value"></a>透過設定登錄值實作簽署原則
 ...
 本主題中的此程序說明如何將選擇性的 `BlockedSignatureStates` DWORD 值加入 HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Microsoft SQL Server\100\SSIS 登錄機碼。 
  `BlockedSignatureStates` 的資料值可指定當封裝包含不受信任的簽章、無效的簽章或未簽署時，是否應該加以封鎖。 對於用以簽署封裝的簽章狀態，`BlockedSignatureStates` 登錄值會使用下列定義：  
