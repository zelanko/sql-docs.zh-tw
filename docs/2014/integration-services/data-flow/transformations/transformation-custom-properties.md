...
# <a name="transformation-custom-properties"></a>轉換自訂屬性
 ...
 您可以使用屬性運算式來設定轉換的某些屬性。 如需詳細資訊，請參閱 [可以使用運算式設定的資料流程屬性](../../data-flow-properties-that-can-be-set-by-using-expressions.md)。  
...
|IsBig|布林|一個值，指出資料行包含大於 40 億的值，或有效位數超過雙精確度浮點數值的值。 此值可能是 0 或 1。 0 表示 IsBig`False`且資料行不包含大數值或精確值。 這個屬性的預設值為 1。|  
...
|ValidateExternalMetadata|布林|指出快取轉換是否在設計階段使用外部資料來源進行驗證。 如果此屬性設定為 `False`，就會在執行階段根據外部資料來源進行驗證。<br /><br /> 預設值，它`True`。|  
...
|屬性名稱|資料類型|描述|  
|-------------------|---------------|-----------------|  
|copyColumnId|Integer|`LineageID`複製來源的輸出資料行的輸入資料行。|  
...
|SourceInputColumnLineageId|Integer|`LineageID`來源的輸出資料行的輸入資料行。|  
...
|屬性|資料類型|描述|  
|--------------|---------------|-----------------|  
|AllowAppend|布林|一個值，指定轉換是否將資料附加至現有的檔案。 這個屬性的預設值是`False`。|  
|ForceTruncate|布林|一個值，指定轉換是否在寫入資料之前截斷現有的檔案。 這個屬性的預設值是`False`。|  
|FileDataColumnID|Integer|一個值，可識別包含轉換插入檔案中之資料的資料行。 擷取的資料行，這個屬性的值為**0**; 上檔案路徑資料行，此屬性包含`LineageID`擷取資料行。|  
...
|FileDataColumnID|Integer|一個值，可識別包含轉換插入資料流程中之資料的資料行。 要插入資料的資料行，此屬性有值為 0;其中包含來源檔案路徑的資料行，此屬性包含`LineageID`要插入資料的資料行。|  
...
|Exhaustive|布林|一個值，指定每個輸入資料錄是否會與其他每個輸入資料錄比較。 `True` 的值大部分用於偵錯目的。 這個屬性的預設值是`False`。<br /><br /> 注意：雖然您無法在模糊群組轉換編輯器中使用這個屬性，但是可以使用進階編輯器來設定這個屬性。|  
...
|ToBeCleaned|布林|一個值，指定此資料行是否用於識別重複項目。亦即，這個資料行是否為您分組所依據的資料行。 這個屬性的預設值是`False`。|  
...
##  <a name="flookup"></a> 模糊查閱轉換自訂屬性  
 ...
 下表描述的是模糊查閱轉換的自訂屬性。 所有的屬性，除了`ReferenceMetadataXML`是讀取/寫入。  
  
|屬性|資料類型|描述|  
|--------------|---------------|-----------------|  
|CopyReferenceTable|布林|指定是否應該針對模糊查閱索引建構和後續的查閱建立參考資料表的副本。 這個屬性的預設值是`True`。|  
...
|Exhaustive|布林|一個值，指定每個輸入資料錄是否會與其他每個輸入資料錄比較。 `True` 的值大部分用於偵錯目的。 這個屬性的預設值是`False`。<br /><br /> 注意：雖然您無法在模糊查閱轉換編輯器中使用這個屬性，但是可以使用進階編輯器來設定這個屬性。|  
...
|MaxMemoryUsage|Integer|查閱資料表的快取大小上限。 此屬性的預設值為 **0**，表示快取大小沒有任何限制。<br /><br /> 此屬性的值可以使用屬性運算式指定。<br /><br /> 注意：雖然您無法在模糊查閱轉換編輯器中使用這個屬性，但是可以使用進階編輯器來設定這個屬性。|  
|MaxOutputMatchesPerInput|Integer|轉換可以針對每個輸入資料列傳回的相符項目數上限。 這個屬性的預設值為 **1**。<br /><br /> 注意：您只能使用進階編輯器來指定大於 100 的值。|  
...
> [!NOTE]  
>  對於包含通過值的對應輸入資料行的輸出資料行，CopyFromReferenceColumn 是空的包含 SourceInputColumnLineageID`LineageID`對應輸入資料行。 若為包含查閱結果的輸出資料行，CopyFromReferenceColumn 會包含查閱資料行的名稱，而且 SourceInputColumnLineageID 是空的。  
...
##  <a name="lookup"></a> 查閱轉換自訂屬性  
 ...
 下表描述的是查閱轉換的自訂屬性。 所有的屬性，除了`ReferenceMetadataXML`是讀取/寫入。  
...
|MaxBuffersPerInput|Integer|您不再需要的值設定`MaxBuffersPerInput`屬性因為 Microsoft 已做出變更，降低合併聯結 」 轉換會耗用過多記憶體的風險。 這個問題有時候會發生在合併聯結的多個輸入以不平均的速率產生資料時。|  
|NumKeyColumns|Integer|聯結中使用的資料行數目。|  
|TreatNullsAsEqual|布林|一個值，指定轉換是否要將 Null 值當做相等值處理。 這個屬性的預設值是`True`。 如果此屬性值為 `False`，轉換處理 Null 值的方式就如同 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 一樣。|  
  
 下表描述的是合併聯結轉換之輸出資料行的自訂屬性。 所有屬性都是可讀寫的。  
  
|屬性名稱|資料類型|描述|  
|-------------------|---------------|-----------------|  
|InputColumnID|Integer|`LineageID`從中複製資料到這個輸出資料行的輸入資料行。|  
...
|屬性名稱|資料類型|描述|  
|-------------------|---------------|-----------------|  
|已選取|布林|指定取樣資料列要導向的目標輸出。 在選取的輸出中，選取設定為`True`，並在未選取的輸出中，選取設定為`False`。|  
...
|屬性|資料類型|描述|  
|--------------|---------------|-----------------|  
|PivotUsage|整數 (列舉)|其中一個資料集進行樞紐處理時，指定資料行的角色的下列值：<br /><br /> **0**： 資料行未經樞紐，且資料行值已傳送至轉換輸出。<br /><br /> **1**： 資料行是集索引鍵，可識別一個集中一個或多個資料列的一部分。 具有相同集索引鍵的所有輸入資料列會組合成一個輸出資料列。<br /><br /> **2**： 資料行是樞紐資料行。 至少應從每個資料行值建立一個資料行。<br /><br /> **3**： 此資料行的值會置於樞紐後建立的資料行中。|  
...
|SourceColumn|Integer|`LineageID`輸入資料行包含樞紐的值，則為-1。 值為 -1 表示此資料行不會用於樞紐作業中。|  
...
|EnableInferredMember|布林|一個值，指定是否要偵測推斷的成員更新。 這個屬性的預設值是`True`。|  
|FailOnFixedAttributeChange|布林|一個值，指定當含有固定屬性的資料列資料行變更或維度資料表中的查閱失敗時，轉換是否會失敗。 如果您預期傳入資料列會包含新的記錄，將此值設定為`True`讓轉換之後繼續該查閱失敗，因為轉換用來識別新記錄的失敗。 這個屬性的預設值是`False`。|  
|FailOnLookupFailure|布林|一個值，指定當現有資料錄的查閱失敗時，轉換是否會失敗。 這個屬性的預設值是`False`。|  
...
|屬性|資料類型|描述|  
|--------------|---------------|-----------------|  
|EliminateDuplicates|布林|指定轉換是否會從轉換輸出中移除重複的資料列。 這個屬性的預設值是`False`。|  
...
|屬性|資料類型|描述|  
|--------------|---------------|-----------------|  
|NewComparisonFlags|整數 (位元遮罩)|一個值，指定轉換如何比較資料行中的字串資料。 如需詳細資訊，請參閱 [Comparing String Data](../comparing-string-data.md)。|  
...
|isCaseSensitive|布林|一個值，指定在擷取名詞和名詞片語時是否要區分大小寫。 這個屬性的預設值是`False`。|  
|MaxLengthOfTerm|Integer|一個數值，指出詞彙的長度上限。 這個屬性只會套用至片語。 這個屬性的預設值為 **12**。|  
|NeedRefenceData|布林|一個值，指定轉換是否會使用儲存在參考資料表中的排除詞彙清單。 這個屬性的預設值是`False`。|  
...
|屬性|資料類型|描述|  
|--------------|---------------|-----------------|  
|isCaseSensitive|布林|一個值，指定區分大小寫的比較是否會套用至輸入資料行文字和查閱詞彙的相符項目。 這個屬性的預設值是`False`。|  
...
|屬性名稱|資料類型|描述|  
|-------------------|---------------|-----------------|  
|CustomLineageID|Integer|`LineageID`對應輸入資料行如果`InputColumnType`該資料行是 0 或 2。|  
...
> [!NOTE]  
>  本節仰賴於 [取消樞紐轉換](unpivot-transformation.md) 中所描述的取消樞紐狀況，來說明此處所描述之選項的使用方式。  
...
|屬性名稱|資料類型|描述|  
|-------------------|---------------|-----------------|  
|PivotKey|布林|指出是否在值`PivotKeyValue`寫入這個輸出資料行的輸入資料行的屬性。<br /><br /> 在 [取消樞紐轉換](unpivot-transformation.md)所描述的取消樞紐狀況中，樞紐值資料行名稱為 **Product** ，並且指定 Ham、Coke、Milk、Beer 和 Chips 資料行已取消樞紐的新 **Product** 資料行。|  
  
 取消樞紐轉換的輸入和輸出沒有任何自訂屬性。  
  
 如需詳細資訊，請參閱 [取消樞紐轉換](unpivot-transformation.md)。  
  
## <a name="see-also"></a>另請參閱  
 [Integration Services 轉換](integration-services-transformations.md)   
 [通用屬性](../../common-properties.md)   
 [路徑屬性](../../path-properties.md)   
 [可以使用運算式設定的資料流程屬性](../../data-flow-properties-that-can-be-set-by-using-expressions.md)  
