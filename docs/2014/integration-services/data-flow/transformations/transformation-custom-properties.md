...
|FileDataColumnID|整數|一個值，可識別包含轉換插入檔案中之資料的資料行。 在 [解壓縮] 資料行上，這個屬性的值為**0**。在 [檔案路徑] 資料行上，這個`LineageID`屬性包含 [解壓縮] 資料行的。|  
...
> [!NOTE]  
>  對於包含來自對應輸入資料行之傳遞值的輸出資料行，Copyfromreferencecolumn 就是空的， `LineageID`且 SourceInputColumnLineageID 包含對應輸入資料行的。 若為包含查閱結果的輸出資料行，CopyFromReferenceColumn 會包含查閱資料行的名稱，而且 SourceInputColumnLineageID 是空的。  
...
|屬性名稱|資料類型|描述|  
|-------------------|---------------|-----------------|  
|已選取|Boolean|指定取樣資料列要導向的目標輸出。 在選取的輸出上，選取的會`True`設定為，而在未選取的輸出上， `False`選取的會設定為。|  
