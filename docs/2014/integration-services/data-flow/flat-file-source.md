...
-   指定「一般檔案」來源是否將資料行中的零長度字串解譯為 Null 值。  
 ...
 輸出資料行也包含 UseBinaryFormat 屬性。 您可使用此屬性在檔案中實作二進位資料的支援，例如具有壓縮之十進位格式的資料。 根據預設 UseBinaryFormat 設定為`false`。 如果您想要使用一種二進位格式，設 UseBinaryFormat`true`上輸出資料行的資料類型和`DT_BYTES`。 當您執行這個動作時，一般檔案來源會略過資料轉換，並將資料直接傳遞到輸出資料行。 然後您就可以使用「衍生的資料行」或「資料轉換」等轉換，將 `DT_BYTES` 資料轉換成不同的資料類型，或者您可以在「指令碼」轉換中撰寫自訂指令碼來解譯資料。 您也可以撰寫自訂的資料流程元件來解譯資料。 如需詳細資訊，了解哪種資料類型，您可以轉型`DT_BYTES`若要查看[轉換&#40;SSIS 運算式&#41;](../expressions/cast-ssis-expression.md)。  
