...
|[OpenRowset]|String|當 AccessMode 為`OpenRowset`時，就是 Excel 目的地所存取之資料表或視圖的名稱。|  
|OpenRowsetVariable|String|當 AccessMode 為`OpenRowset from Variable`時，就是包含 Excel 目的地所存取之資料表或視圖名稱的變數名稱。|  
|SqlCommand|String|當 AccessMode 為`SQL Command`時，就是 Excel 目的地用來指定資料之目的地資料行的 transact-sql 語句。|  
