 ...
 **編輯 SQL**  
 使用預設的陳述式中**建立資料表 SQL 陳述式**對話方塊方塊中，或修改您的目的。 如果修改此陳述式，您還必須對資料表對應進行相關的變更。  
 ...
 **目的地**  
 指定在進行複製作業的期間，您是否想要忽略某資料行。 您可以藉由選取複製的資料行子集**\<忽略 >** 中此資料行，您想略過的資料行。 在對應資料行之前，您必須忽略不會加以對應的所有資料行。  
