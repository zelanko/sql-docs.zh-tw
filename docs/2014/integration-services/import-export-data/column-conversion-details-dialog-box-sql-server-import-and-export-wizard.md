...
1.  在 [ [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]匯入和匯出嚮導] 的 [**審核資料類型問題**] 頁面上，于 [**資料表**] 清單中選取資料表。  
  
2.  在 [**資料類型對應**] 清單中，按兩下包含您要查看其轉換詳細資料之資料行的資料列。  
  
 若要深入瞭解「 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]匯入和匯出嚮導」，請參閱[SQL Server 匯入和匯出嚮導](import-and-export-data-with-the-sql-server-import-and-export-wizard.md)。 若要瞭解啟動精靈的選項，以及成功執行嚮導所需的許可權，請參閱[執行 SQL Server 匯入和匯出嚮導](start-the-sql-server-import-and-export-wizard.md)。  
