...
### <a name="table-list"></a>資料表清單  
 上半部**檢閱資料類型問題**頁面是**資料表**列出從來源傳送到目的地資料表的清單。 下表將描述這份清單中的資料行。  
...
|**目的地**|顯示目的地資料表的名稱。|  
 ...
 若要檢視有關個別資料表的轉換資訊，請在此選取資料表**資料表**方格。 選取資料表的轉換資訊會顯示在此頁面下半部 **[資料類型對應]** 方格的資料行中。  
  
### <a name="data-type-mapping-list"></a>資料類型對應清單  
 下半部**檢閱資料類型問題**頁面是**資料類型對應**清單。 此方格會提供有關在 **[資料表]** 清單中選取之資料表內資料行的詳細轉換資訊。 下表將描述這份清單中的資料行。  
...
|**錯誤時**|指定精靈如何處理錯誤：<br /><br /> 使用**On Error （全域）** 設定。<br /><br /> 因錯誤而失敗，並停止匯入或匯出程序。<br /><br /> 忽略錯誤。|  
|**截斷時**|指定精靈如何處理截斷：<br /><br /> 使用**在截斷時 （全域）** 設定。<br /><br /> 因錯誤而失敗，並停止匯入或匯出程序。<br /><br /> 忽略截斷。|  
