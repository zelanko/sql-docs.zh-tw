...
4.  驗證及執行階段：執行，或如果不需要執行驗證封裝 **/validate**已指定選項。  
...
-   **/De [crypt]**_密碼_:  選擇性。 設定載入含密碼加密的封裝時，所用的解密密碼。  
...
-   **/Verifyb [uild]** _主要_[*; minor*[*; 建置*]]:選擇性。 根據驗證階段期間在 *major*、 *minor*及 *build* 引數指定的組建編號，來驗證封裝的組建編號。 如果發生不符的情形，將不會執行封裝。  
