...
-   `OdbcCommand` 需要位置參數。 參數的位置由命令文字中的問號 (?) 指出 (相較之下，`SqlCommand` 需要具名的參數)。  
 ...
 此範例使用 **AdventureWorks** 範例資料庫中的 **Person.Address** 資料表。 此範例會透過資料流程，傳遞這個資料表的第一個和第四個數據行，也就是**int * AddressID*** 和**Nvarchar （30） City**資料行。 在[開發特定類型的指令碼元件](../extending-packages-scripting-data-flow-script-component-types/developing-specific-types-of-script-components.md)主題中，這個相同的資料用於來源、轉換和目的地範例中。  
...
## <a name="see-also"></a>另請參閱  
 [以指令碼元件建立目的地](../extending-packages-scripting-data-flow-script-component-types/creating-a-destination-with-the-script-component.md)  
