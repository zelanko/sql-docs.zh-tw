...
# <a name="integration-services-ssis-variables"></a>Integration Services (SSIS) 變數
  變數會儲存封裝及其[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)]容器、工作和事件處理常式在執行時間可使用的值。 「指令碼」工作和「指令碼」元件中的指令碼也可以使用變數。 將工作和容器排序成工作流程的優先順序條件約束，可在其條件約束定義含有運算式時使用變數。  
...
>  您只能透過按一下 [變數]**** 視窗中的 [移動變數]**** 來變更此屬性設定。  
 ...
 不過，對於使用者定義的變數，系統會在符合**** 下列條件時`false` ，將 IncludeInDebugDump 選項重設為：  
