...
> [!NOTE]  
>  [ `CommandTimeout` **OLE DB 目的地編輯器**] 中無法使用 OLE DB 目的地的屬性，但可使用**進階編輯器**來設定。 此外，特定的快速載入選項僅適用于**進階編輯器**。 如需有關這些屬性的詳細資訊，請參閱＜ [OLE DB Custom Properties](data-flow/ole-db-custom-properties.md)＞的＜OLE DB 目的地＞一節。  
 ...
 **每批次的資料列**  
 指定批次中的資料列數目。 這個屬性的預設值為 **-1**，表示未指派任何值。  
