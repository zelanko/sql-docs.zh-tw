...
-   Oracle CDC 服務可使用 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]**AlwaysOn** 資料庫鏡像功能。 此支援要求 MSXDBCDC 與所有 CDC 資料庫位於相同的可用性群組中。 它也會要求 Oracle CDC 服務電腦系統管理員指定可用性群組的適當**AlwaysOn**連接資訊 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] （例如，連接屬性 `Failover_Partner and Network=dbmssocn` ）。 這樣可讓 CDC 服務在容錯移轉之後，自動繼續資料庫次要複寫的處理。  
