...
-   **時間戳記**:從狀態資料表讀取 CDC 狀態上次的 UTC 時間。  
  
-   **目前正在處理**:您監視這一節中的下列資訊。  
  
    -   **上次交易時間戳記**:上次交易寫入變更資料表的當地時間。  
  
    -   **上次變更時間戳記**:來源 Oracle 資料庫交易記錄中的 Oracle CDC 執行個體查看最近變更的當地時間。 這會提供有關 CDC 執行個體讀取 Oracle 交易記錄之目前延遲的資訊。  
  
    -   **交易記錄標頭 CN**:最近變更編號 (CN) 讀取 Oracle 交易記錄檔中。  
  
    -   **交易記錄結尾 CN**:變更復原或重新啟動 CDC 執行個體數目。 萬一重新啟動或發生任何其他類型的失敗 (包括叢集容錯移轉)，Oracle CDC 執行個體將會重新調整到這個位置。  
  
    -   **目前 CN**:上次變更編號 (SCN) 來源 Oracle 資料庫 （而不是交易記錄） 所示。  
  
    -   **使用中交易**:尚未決定 （認可/回復） 之來源 Oracle 交易正在處理 Oracle CDC 執行個體，並不是目前數目。  
  
    -   **暫存交易**:目前來源 Oracle 交易的數目以暫存[cdc.xdbcdc_staged_transactions](the-oracle-cdc-databases.md#BKMK_cdcxdbcdc_staged_transactions)資料表。  
  
-   **計數器**:您監視這一節中的下列資訊。  
  
    -   **完成的交易**:上次重設 CDC 執行個體之後完成的交易數目。 這不包括未包含相關資料表的交易。  
  
    -   **寫入的變更**:變更資料表的變更會寫入至 SQL Server 的數目。  
