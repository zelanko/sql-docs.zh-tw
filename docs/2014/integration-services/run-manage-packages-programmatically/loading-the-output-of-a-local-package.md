...
# <a name="loading-the-output-of-a-local-package"></a>載入本機封裝的輸出
  使用 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 將輸出儲存到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 目的地時，或使用 [!INCLUDE[vstecado](../../includes/vstecado-md.md)]System.IO** 命名空間，將輸出儲存到一般檔案目的地時，用戶端應用程式可以讀取 ** 套件的輸出。 但用戶端應用程式也可直接從記憶體讀取封裝的輸出，而無須在程序中間執行保存資料的步驟。 此解決方案的關鍵在於`Microsoft.SqlServer.Dts.DtsClient`命名空間，其中包含來自**system.web**命名空間的`IDbConnection`、 `IDbCommand`和**IDbDataParameter**介面的特製化執行。 預設會在 **%ProgramFiles%\Microsoft SQL Server\100\DTS\Binn** 中安裝 Microsoft.SqlServer.Dts.DtsClient.dll 組件。  
...
2.  在開發專案中，藉由尋找**DtsClient**的`Microsoft.SqlServer.Dts.DtsClient`元件來設定命名空間的參考。 此組件預設安裝在 **C:\Program Files\Microsoft SQL Server\100\DTS\Binn** 中。 使用 c # `Using`或[!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] `Imports`語句，將命名空間匯入您的程式碼中。  
...
5.  您可以選擇性地使用 `DtsDataParameter` 物件上的 `DtsCommand` 物件集合，間接地參數化封裝輸出，以便將值傳遞給定義在封裝中的變數。 您可以在封裝中使用這些變數做為查詢參數，或在運算式中加以運用，藉此影響傳回 DataReader 目的地的結果。 您必須在**DtsClient**命名空間的封裝中定義這些變數，才能將它們與用戶端`DtsDataParameter`應用程式中的物件搭配使用。 （您可能需要按一下 [**變數**] 視窗中的 [**選擇變數資料行**] 工具列按鈕，以顯示 [**命名空間**] 資料行）。在您的用戶端程式代碼中， `DtsDataParameter`當您`Parameters`將加入至`DtsCommand`的集合時，請省略變數名稱中的 DtsClient 命名空間參考。 例如：  
...
7.  將彙總轉換加入資料流程，然後將 OLE DB 來源的輸出連接到轉換。 開啟 [匯總轉換編輯器]，並將它設定為在所有輸入資料行（*）上執行「計算所有」作業，並使用別名 CustomerCount 輸出匯總值。  
...
2.  藉由流覽至`Microsoft.SqlServer.Dts.DtsClient` **%ProgramFiles%\Microsoft SQL Server\100\DTS\Binn**中相同名稱的元件，加入命名空間的參考。  
  
3.  將下列範例程式碼複製並貼入表單的程式碼模組。  
  
4.  視需要修改`dtexecArgs`變數的值，使其包含**dtexec**執行封裝所需的命令列參數。 範例程式碼會從檔案系統載入封裝。  
  
5.  視需要修改`dataReaderName`變數的值，使其包含封裝中 DataReader 目的地的名稱。  
  
6.  在表單上放置按鈕與文字方塊。 範例程式碼會`btnRun`使用做為按鈕的名稱，以及`txtResults`做為文字方塊的名稱。  
