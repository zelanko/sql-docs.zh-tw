...
2.  使用 BACKUP MASTER KEY Transact-SQL 陳述式，備份 SSISDB 資料庫的主要金鑰。 此金鑰儲存在您指定的檔案。 使用密碼來加密檔案中的主要金鑰密碼。  
     ...
     在下列範例中，主要金鑰是匯出至 `c:\temp directory\RCTestInstKey` 檔案。 
  `LS2Setup!` 密碼是用來加密主要金鑰。  
...
3.  使用 ** 中的 [備份資料庫]**[!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 對話方塊備份 SSISDB 資料庫。 如需詳細資訊，請參閱 [如何：備份資料庫 (SQL Server Management Studio)](https://go.microsoft.com/fwlink/?LinkId=231812)。  
...
3.  使用 ** 中的 [還原資料庫]**[!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 對話方塊，從備份還原 SSISDB 資料庫。 如需詳細資訊，請參閱下列主題。  
...
6.  使用 ** 中的 [登入屬性]**[!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] 對話方塊，將 SSISDB 使用者 ##MS_SSISServerCleanupJobUser## (SSISDB 資料庫) 對應至 ##MS_SSISServerCleanupJobLogin##。  
