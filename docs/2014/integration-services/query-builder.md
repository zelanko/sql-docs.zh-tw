...
-   使用**查詢的圖形標記法或使用 SQL 命令**[查詢產生器] 包含以圖形方式顯示查詢的窗格，以及顯示查詢之 SQL 文字的窗格。 您可使用圖形窗格或文字窗格。 「查詢產生器」會同步檢視，讓它們始終保持最新狀態。  
  
-   **聯結相關資料表**如果您將一個以上的資料表加入查詢中，查詢產生器會自動決定資料表的關聯方式，並建立適當的聯結命令。  
  
-   **查詢或更新資料庫**您可以使用 [查詢產生器] 來傳回資料，方法是使用 Transact-sql SELECT 語句，並建立可更新、加入或刪除資料庫中記錄的查詢。  
  
-   **立即觀看和編輯結果**您可以執行查詢並使用方格中的記錄集，讓您可以在資料庫中流覽和編輯記錄。  
