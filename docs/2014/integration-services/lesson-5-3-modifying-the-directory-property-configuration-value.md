...
2.  值變更**ConfiguredValue**項目比對的路徑`New Sample Data`您在上一個工作中建立的資料夾。 請勿用引號括住路徑。 如果`New Sample Data`資料夾位於您的磁碟機的根層級 (例如 c:\\)，則更新的 XML 應該類似以下的範例：  
     ...
     標題資訊`GeneratedBy`， `GeneratedFromPackageID`，並**GeneratedDate**將位於不同檔案，當然。 要注意的元素是 `Configuration` 元素。 `User::varFolderName` 變數的 `Value` 屬性現在將會包含 C:\New Sample Data。  
  
3.  儲存變更，然後關閉文字編輯器。  
  
## <a name="next-task-in-lesson"></a>本課程的下一項工作  
 [步驟 4：測試第 5 課的教學課程套件](../integration-services/lesson-5-4-testing-the-lesson-5-tutorial-package.md)  
