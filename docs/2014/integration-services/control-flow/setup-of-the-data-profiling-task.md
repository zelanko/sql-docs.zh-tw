...
|計算|這有助於識別|使用這個設定檔|  
|----------------|-------------------------|----------------------|  
|選取之資料行中所有不同的字串值長度，以及該資料表中每個長度所代表之資料列的百分比。|不正確**字串值**-例如，您分析了應該使用兩個字元做為美國州代碼的資料行，但發現長度超過兩個字元的值。|資料**行長度散發-** 適用于具有下列其中一種資料類型的資料行：<br /><br /> 字元資料類型：`char`、`nchar`、`varchar` 和 `nvarchar`|  
