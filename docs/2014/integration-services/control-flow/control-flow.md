...
# <a name="control-flow"></a>控制流程
  封裝由控制流程及選擇性的一個或多個資料流程所組成。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)][!INCLUDE[ssISnoversion](../../../includes/ssisnoversion-md.md)]提供三種不同類型的控制流程元素：提供封裝中結構的容器、提供功能的工作，以及將可執行檔、容器和工作連接到已排序控制流程的優先順序條件約束。  
...
## <a name="control-flow-implementation"></a>控制流程實作  
 ...
 ![具有封裝之控制流程設計師的螢幕擷取畫面](../media/samplecontrolflow.gif "具有封裝之控制流程設計師的螢幕擷取畫面")  
