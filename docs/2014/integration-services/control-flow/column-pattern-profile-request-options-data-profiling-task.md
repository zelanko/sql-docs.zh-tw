...
-   **符號**根據預設，**符號**清單包含下列字元： `,.;:-"'` ~ =&/@！？（） <> [] {} | # * ^% `. For example, if the symbols are "` （）-' "，值" （425） 123-4567 "已標記化為 [" （"，" 425 "，"） "，" 123 "，"-"，" 4567 "，"） "]。  
 ...
 **IsWildCard**  
 指定是否已選取 **（ \* ）** 萬用字元。 如果您已選取 **(\*)** 來分析所有資料行，這個選項會設定為 [True]****。 如果您已選取要分析的個別資料行，它就會設定為 **[False]** 。 此選項是唯讀的。  
