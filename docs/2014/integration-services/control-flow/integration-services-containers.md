...
|屬性|描述|  
|--------------|-----------------|  
|`DelayValidation`|布林值，指出是否延遲至執行階段才驗證容器。 這個屬性的預設值是`False`。<br /><br /> 如需詳細資訊，請參閱 <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.DelayValidation%2A>為止。|  
|`Description`|容器描述。 該屬性包含字串，但可能空白。<br /><br /> 如需詳細資訊，請參閱 <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.Description%2A>為止。|  
|`Disable`|布林值，指出容器是否執行。 這個屬性的預設值是`False`。<br /><br /> 如需詳細資訊，請參閱 <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.Disable%2A>為止。|  
...
|`IsolationLevel`|容器交易的隔離等級。 可能的值為 `Unspecified`、`Chaos`、`ReadUncommitted`、`ReadCommitted`、`RepeatableRead`、`Serializable` 和 `Snapshot`。 這個屬性的預設值是`Serializable`。 如需詳細資訊，請參閱 <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.IsolationLevel%2A>為止。|  
...
|`LoggingMode`|指定容器記錄行為的值。 值是`Disabled`， `Enabled`，和`UseParentSetting`。 這個屬性的預設值是`UseParentSetting`。 如需詳細資訊，請參閱 <xref:Microsoft.SqlServer.Dts.Runtime.DTSLoggingMode>為止。|  
...
|`Name`|容器的名稱。<br /><br /> 如需詳細資訊，請參閱 <xref:Microsoft.SqlServer.Dts.Runtime.DtsContainer.Name%2A>為止。|  
|`TransactionOption`|容器的交易式參與。 可能的值為 `NotSupported`、`Supported`、`Required`。 這個屬性的預設值是`Supported`。 如需詳細資訊，請參閱 <xref:Microsoft.SqlServer.Dts.Runtime.DTSTransactionOption>為止。|  
