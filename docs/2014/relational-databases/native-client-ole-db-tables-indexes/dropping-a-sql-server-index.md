...
# <a name="dropping-a-sql-server-index"></a>卸除 SQL Server 索引
 ...
 取用者會在 *pIndexID* 參數中，將索引名稱指定為 *uName* 聯集之 *pwszName* 成員中的 Unicode 字元字串。 
  *pIndexID* 的 *eKind* 成員必須是 DBKIND_NAME。 當[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] *pIndexID*為 Null 時，Native Client OLE DB 提供者不支援在資料表上卸載所有索引的 OLE DB 功能。 如果 *pIndexID* 為 Null，會傳回 E_INVALIDARG。  
