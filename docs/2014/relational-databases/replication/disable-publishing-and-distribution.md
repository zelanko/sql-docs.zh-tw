...
6.  呼叫 <xref:Microsoft.SqlServer.Replication.DistributionPublisher.Remove%2A> 方法。 `true`如果發行者和散發者位於不同的伺服器上，以及應該在散發者端卸載發行者，而不需要先確認發行集已不存在於發行者端，請傳遞的值為*force* 。  
