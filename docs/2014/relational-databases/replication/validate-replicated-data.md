 ...
 **若要驗證資料**  
 ...
 若要驗證訂閱中的所有發行項，請使用 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]、預存程序或 Replication Management Objects (RMO)。 如需詳細資訊，請參閱 [Validate Data at the Subscriber](validate-data-at-the-subscriber.md)。 若要驗證快照式和交易式發行集中的個別發行項，您必須使用預存程序。  
