...
-   參數化篩選允許將不同資料分割傳送到不同「訂閱者」，而無需建立多個發行集。 例如，資料表可進行篩選，以便給定銷售代表的資料僅複寫至該代表。 參數化篩選具有各種不同的選項，允許您修改篩選，以最佳化效能並使資料和應用程式需求的相符達到最佳。 如需詳細資訊，請參閱 [Parameterized Row Filters](parameterized-filters-parameterized-row-filters.md)。  
