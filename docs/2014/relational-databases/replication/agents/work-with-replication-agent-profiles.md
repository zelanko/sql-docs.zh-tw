...
3.  設定物件的下列屬性：  
    ...
    -   (選擇性) <xref:Microsoft.SqlServer.Replication.AgentProfile.Default%2A> - 如果依預設這個 `true` 的所有新代理程式作業都會使用這個設定檔，請將此屬性設定為 <xref:Microsoft.SqlServer.Replication.AgentType>。  
