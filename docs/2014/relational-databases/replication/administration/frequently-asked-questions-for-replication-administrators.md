...
### <a name="should-i-script-my-replication-configuration"></a>我是否應為我的複寫組態編寫指令碼？  
 是的。 編寫複寫組態的指令碼是複寫拓撲之任何損毀復原計劃的主要部分。 如需有關編寫指令碼的詳細資訊，請參閱＜ [Scripting Replication](../scripting-replication.md)＞。  
...
### <a name="does-replication-resume-if-a-connection-is-dropped"></a>如果連接斷開，則複寫是否會繼續？  
 是的。 如果連接斷開，複寫處理會從它停止的地方繼續。 如果您在不可靠的網路中使用合併式複寫，請考慮使用邏輯記錄，它可確保相關變更作為一個單位進行處理。 如需詳細資訊，請參閱[使用邏輯記錄分組相關資料列的變更](../merge/group-changes-to-related-rows-with-logical-records.md)。  
