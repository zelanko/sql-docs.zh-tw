...
4.  在散發者上，執行 [sp_changearticlecolumndatatype](/sql/relational-databases/system-stored-procedures/sp-changearticlecolumndatatype-transact-sql)。 指定「 ** \@發行者**」的「Oracle 發行者」的** \@名稱，** ** \@** 以及「發行集」、 ** \@ ** 「發行項」和「資料行」來定義已發行的資料行。 **針對\@類型**，指定要[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]對應的資料類型名稱，以及** \@長度** ** \@、有效**位數和** \@小**數位數（如果適用的話）。  
