...
4.  在散發者上，執行 [sp_changearticlecolumndatatype](/sql/relational-databases/system-stored-procedures/sp-changearticlecolumndatatype-transact-sql)。 針對 **\@publisher** 指定 Oracle 發行者的名稱，並指定 **\@publication**、 **\@article** 和 **\@column** 來定義已發佈的資料行。 針對 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]**type\@ 指定**  要對應的資料類型名稱，並指定適用的 **\@length**、 **\@precision** 和 **\@scale**。  
...
1.  在任何資料庫的散發者上，執行 [sp_helpdatatypemap](/sql/relational-databases/system-stored-procedures/sp-helpdatatypemap-transact-sql)。 針對  source_dbms **指定 \@ORACLE** 值，並指定篩選結果集所需的任何其他參數。  
