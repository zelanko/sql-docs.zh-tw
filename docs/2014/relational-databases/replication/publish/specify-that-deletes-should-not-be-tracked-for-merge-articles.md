---
title: 指定不應該為合併發行項 （複寫 TRANSACT-SQL 程式設計） 追蹤刪除 |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
1.  在發行集資料庫的發行者端，執行 [sp_addmergearticle &#40;Transact-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-addmergearticle-transact-sql)。 指定的值為`false`如**@delete_tracking**。 如需詳細資訊，請參閱 [定義發行項](define-an-article.md)。  
...
2.  如果步驟 1 的值是 **1**，請在發行集資料庫的發行者端執行 [sp_changemergearticle &#40;Transact-SQL&#41;](/sql/relational-databases/system-stored-procedures/sp-changemergearticle-transact-sql)。 指定的值為**delete_tracking**如**@property**，而值為`false`如**@value**。  
