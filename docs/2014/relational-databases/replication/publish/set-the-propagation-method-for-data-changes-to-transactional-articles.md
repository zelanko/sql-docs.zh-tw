...
##  <a name="using-sql-server-management-studio"></a><a name="SSMSProcedure"></a> 使用 SQL Server Management Studio  
 在 [發行項**屬性- \<Article> ** ] 對話方塊的 [**屬性**] 索引標籤上，指定傳播方法，這可以在 [新增發行集嚮導] 和 [**發行集屬性- \<Publication> ** ] 對話方塊中取得。 如需使用精靈及存取對話方塊的詳細資訊，請參閱[建立發行集](create-a-publication.md)和[檢視及修改發行集屬性](view-and-modify-publication-properties.md)。  
...
3.  在 [發行項**屬性- \<Article> ** ] 對話方塊的 [**屬性**] 索引標籤上，于 [**語句傳遞**] 區段中，使用 [**插入傳遞格式**]、[**更新傳遞格式**] 和 [**刪除傳遞格式**] 功能表，指定每個作業的傳播方法。  
...
2.  按一下 **[設定反白顯示資料表發行項的屬性]**。  
     ...
     在 [發行項** \<Article> 屬性-** ] 對話方塊的 [**屬性**] 索引標籤上，于 [**語句傳遞**] 區段中，從適當的 [傳遞格式] 功能表（**插入傳遞格式**、**更新傳遞格式**或**刪除傳遞格式**）選取呼叫語法，然後輸入要在**INSERT 預存**程式、**刪除預**存程式或**更新預存**程式中使用的程式名稱。 如需 CALL 語法的詳細資訊，請參閱[指定交易式發行項變更的傳播方式](../transactional/transactional-articles-specify-how-changes-are-propagated.md)中的＜預存程序的 Call 語法＞一節。  
...
1.  在發行集資料庫的發行者上，執行 [sp_addarticle](/sql/relational-databases/system-stored-procedures/sp-addarticle-transact-sql)。 指定發行項所屬的發行集** \@ 名稱、發行****項的名稱、用於 \@ ** ** \@ source_object**的資料庫物件，以及至少下列其中一個參數的**SQL**的值：  
...
1.  在發行集資料庫的發行者上，執行 [sp_addarticle](/sql/relational-databases/system-stored-procedures/sp-addarticle-transact-sql)。 指定發行項所屬的發行集** \@ 名稱、發行****項的名稱、用於 \@ ** ** \@ source_object**的資料庫物件，以及至少下列其中一個參數的值為**NONE** ：  
...
1.  在發行集資料庫的發行者上，執行 [sp_addarticle](/sql/relational-databases/system-stored-procedures/sp-addarticle-transact-sql)。 指定發行項所屬的發行集** \@ 名稱、發行****項的名稱、用於 \@ ** ** \@ source_object**的資料庫物件、包含值**0x02**的** \@ schema_option**位元遮罩值（可自動產生自訂預存程式），以及至少下列其中一個參數：  
