...
-   [產生 SQL 指令碼]  ，這可從   中 [複寫][!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 資料夾和所有子資料夾的操作功能表中使用。 此對話方塊可讓您撰寫 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體上所有複寫物件的指令碼。  
  
-   [**產生 SQL \<ObjectName> 腳本**]，這可從發行集和訂閱的內容功能表中取得。 此對話方塊可讓您編寫個別物件的指令碼。  
...
## <a name="generate-sql-script-objectname-options"></a>產生 SQL 腳本 \<ObjectName> 選項  
 **[若要建立或啟用元件]** 與 **[若要卸除或停用元件]**  
 指定指令碼是否應包含建立或卸除複寫物件的命令。 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 建議您使用對話方塊來建立一組啟用和停用元件的指令碼。  
