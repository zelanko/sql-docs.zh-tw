...
# <a name="specify-a-merge-subscription-type-and-conflict-resolution-priority-sql-server-management-studio"></a>指定合併訂閱類型和衝突解決優先權 (SQL Server Management Studio)
 ...
 建立訂用帳戶之後，就無法修改訂閱類型，但是您可以在 [**訂閱屬性- \<Publisher> ： \<PublicationDatabase> ** ] 對話方塊中修改伺服器訂閱類型的優先權。 如需有關存取這個對話方塊的詳細資訊，請參閱＜ [View and Modify Push Subscription Properties](view-and-modify-push-subscription-properties.md) ＞與＜ [View and Modify Pull Subscription Properties](view-and-modify-pull-subscription-properties.md)＞。  
...
1.  在 [**訂閱屬性- \<Publisher> ： \<PublicationDatabase> **在發行者] 上，為 [**優先順序**] 選項輸入值（0.00 到99.99）。  
