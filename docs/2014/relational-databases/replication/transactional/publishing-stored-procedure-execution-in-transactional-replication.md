...
> [!NOTE]  
>  依預設，使用 ALTER PROCEDURE 在「發行者」上所作的任何變更都將傳播到「訂閱者」。 若要防止發生這種情況，請在執行 ALTER PROCEDURE 之前停用結構描述變更的傳播。 如需結構描述變更的資訊，請參閱[對發行集資料庫進行結構描述變更](../publish/make-schema-changes-on-publication-databases.md)。  
