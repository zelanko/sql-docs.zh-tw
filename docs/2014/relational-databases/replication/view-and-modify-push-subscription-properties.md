...
5.  呼叫 <xref:Microsoft.SqlServer.Replication.ReplicationObject.LoadProperties%2A> 方法以取得物件的屬性。 如果此方法傳回`false`，或是在步驟 3 中的訂閱屬性定義不正確的訂用帳戶不存在。  
