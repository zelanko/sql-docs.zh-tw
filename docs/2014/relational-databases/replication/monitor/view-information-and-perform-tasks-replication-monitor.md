...
3.  若要查看訂閱的相關資訊，請按一下 [**訂閱監看清單**] 索引標籤。您也可以從這個索引標籤存取更詳細的資訊，並執行工作：    
...
4.  若要查看代理程式的相關資訊，請按一下 [**代理**程式] 索引標籤。您也可以在此索引標籤上存取更詳細的資訊並執行工作：    
...
-   針對佇列更新訂閱啟用之交易式發行集所使用的佇列讀取器代理程式。  
 ...
 若要查看此索引標籤上選項的詳細資訊，**請按一下功能表**欄上的 [說明]。 如需啟動複寫監視器的詳細資訊，請參閱[啟動複寫監視器](start-the-replication-monitor.md)。  
