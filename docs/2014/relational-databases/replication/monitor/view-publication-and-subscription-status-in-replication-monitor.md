...
-   訂閱過期  
     ...
     這個狀態值適用所有複寫類型。 如需相關資訊，請參閱 [Set Thresholds and Warnings in Replication Monitor](set-thresholds-and-warnings-in-replication-monitor.md)。  
  
-   效能嚴重不足  
  
     這個狀態值適用異動複寫和合併複寫。 如需詳細資訊，請參閱[使用複寫監視器監視效能](monitor-performance-with-replication-monitor.md)。  
  
-   長期執行合併  
  
     這個狀態值適用合併複寫。 如需詳細資訊，請參閱[使用複寫監視器監視效能](monitor-performance-with-replication-monitor.md)。  
