...
##  <a name="distqueries"></a> 分散式查詢和連結的伺服器  
 您可以使用 FILESTREAM 資料，透過分散式的查詢和連結的伺服器將它視為`varbinary(max)`資料。 在使用四部分名稱的分散式查詢中，即使此名稱參考本機伺服器，也無法使用 FILESTREAM **PathName()** 函數。 不過， **PathName()** 可在使用 **OPENQUERY()** 之傳遞查詢的內部查詢中使用。  
