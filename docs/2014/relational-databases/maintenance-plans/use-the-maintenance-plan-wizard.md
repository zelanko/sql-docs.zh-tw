...
# <a name="use-the-maintenance-plan-wizard"></a>使用維護計畫精靈
  本主題描述如何使用 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)]中的維護計畫精靈，建立單一伺服器或多伺服器維護計畫。 維護計畫嚮導會建立一個可定期執行[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]的維護計畫。 這樣可讓您依指定間隔執行各種資料庫管理工作，包括備份、資料庫完整性檢查，或資料庫統計資料更新。  
...
-   [在 SQL Server Management Studio 中使用維護計畫精靈建立維護計畫](#SSMSProcedure)  
...
-   
  **db_ssisadmin** 角色和 **dc_admin** 角色的成員可以將其權限提高為 **sysadmin**。 能夠提高權限是因為這些角色可以修改 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 封裝；這些封裝可藉由使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 的 **sysadmin** 安全性內容由 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行。 若要在執行維護計畫、資料收集組和[!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)]其他封裝時預防此權限提高，請將[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]執行封裝的 Agent 作業設為使用有限許可權的 proxy 帳戶，或是只將**sysadmin**成員新增至**db_ssisadmin**和**dc_admin**角色。  
...
1.  在 **[定義壓縮資料庫工作]** 頁面上，使用 `DBCC SHRINKDATABASE` 陳述式搭配 `NOTRUNCATE` 或 `TRUNCATEONLY` 選項建立嘗試縮減所選取資料庫大小的工作。 如需詳細資訊，請參閱 [DBCC SHRINKDATABASE &#40;Transact-SQL&#41;](/sql/t-sql/database-console-commands/dbcc-shrinkdatabase-transact-sql)。 完成時，按一下 [下一步]****。  
...
1.  在 [定義重新組織索引工作]On the **** 頁面上選取伺服器並移動其中的索引頁面，使其成為更有搜尋效率的順序。 此工作會使用 `ALTER INDEX ... REORGANIZE` 陳述式。 如需詳細資訊，請參閱[ALTER INDEX &#40;transact-sql&#41;](/sql/t-sql/statements/alter-index-transact-sql)。 完成時，按一下 [下一步]****。  
...
2.  在 [定義重建索引工作]**** 頁面上，選取要在其中重新建立多個索引的資料庫。 此工作會使用 `ALTER INDEX ... REBUILD PARTITION` 陳述式。 如需詳細資訊，請參閱[ALTER INDEX &#40;transact-sql&#41;](/sql/t-sql/statements/alter-index-transact-sql)。）完成時，按 **[下一步]**。  
