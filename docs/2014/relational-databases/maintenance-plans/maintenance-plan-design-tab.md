...
# <a name="maintenance-plan-design-tab"></a>維護計畫 (設計索引標籤)
  使用 [**維護計畫（設計**索引標籤）] 可指定維護計畫及其子計畫的屬性。 將工作從工具箱拖曳至計畫設計師。 在工作群組按一下滑鼠右鍵，即可建立分支執行路徑。 維護計畫會儲存為 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 封裝，然後由 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 作業執行。  
 ...
 **設計工具介面**  
 ...
 若要將維護工作加入計畫，請將工作的圖示從 [維護計畫工作]**** 工具箱拖曳到計畫設計介面，或按兩下工具箱中的工作，這會將該工作加入目前使用中的設計師介面。 如果看不到 [維護計畫工作]**** 工具箱，請從 ** [檢視]** 功能表選取 [工具箱][!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] ****。 在 [工具箱]**** 窗格中展開 [維護計畫工作]**** 節點。  
