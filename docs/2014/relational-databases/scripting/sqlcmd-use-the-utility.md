...
-   伺服器選項（**-S**），識別 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 要連接的實例 `sqlcmd` 。  
...
-   使用 Windows 驗證連接到預設執行個體，以互動方式執行 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式：  
    ...
    > [!NOTE]  
    >  在上述範例中，未指定 **-E** ，因為它是預設值，而且會 `sqlcmd` 使用 Windows 驗證連接到預設實例。  
...
## <a name="running-transact-sql-statements-interactively-by-using-sqlcmd"></a>使用 sqlcmd 以互動方式執行 Transact-SQL 陳述式  
 您可以使用 `sqlcmd` 公用程式，以互動方式在 [命令提示字元] 視窗中執行 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式。 若要使用以互動方式執行 [!INCLUDE[tsql](../../includes/tsql-md.md)] 語句 `sqlcmd` ，請執行公用程式，而不使用 **-q**、 **-q**、 **-Z**或 **-i**選項來指定任何輸入檔或查詢。 例如：  
 ...
 在 `sqlcmd` 提示字元中，您可以輸入 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式及 `sqlcmd` 命令 (例如 `GO` 及 `EXIT`)。 每個 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式會放在稱為陳述式快取的緩衝區中。 這些陳述式會在您輸入 `GO` 命令並按 ENTER 後，傳送至 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 若要結束 `sqlcmd` ，請在 `EXIT` `QUIT` 新行的開頭輸入或。  
  
 若要清除陳述式快取，請輸入 `:RESET`。 輸入 `^C` 會導致結束 `sqlcmd` 。 在發出 `^C` 命令後，也可以使用 `GO` 來停止執行陳述式快取。  
  
 [!INCLUDE[tsql](../../includes/tsql-md.md)]在互動式會話中輸入的語句，可以藉由輸入 **： ED**命令和提示來編輯 `sqlcmd` 。 此時會開啟編輯器，而在編輯過 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式並關閉編輯器之後，修訂的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式即顯示於命令視窗。 輸入 `GO` 以執行修訂 [!INCLUDE[tsql](../../includes/tsql-md.md)] 語句。  
...
## <a name="interactive-sqlcmd-example"></a>互動的 sqlcmd 範例  
 ...
 這表示您已經連接到 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 的執行個體，而且 `sqlcmd` 已經準備好要接受 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式與 `sqlcmd` 命令。 在 `1>` 後面的閃爍底線是 `sqlcmd` 提示字元，該字元標示將顯示您輸入之陳述式與命令的位置。 現在，輸入 `USE AdventureWorks2012` 並按 enter，然後輸入，然後 `GO` 按 enter 鍵。 [命令提示字元] 視窗的內容將如下所示：  
...
### <a name="f-using-sqlcmd-in-a-windows-script-file"></a>F. 在 Windows 指令碼檔案中使用 sqlcmd  
 `sqlcmd`之類的命令 `sqlcmd -i C:\InputFile.txt -o C:\OutputFile.txt,` 可以在 .bat 檔案中與 VBScript 一起執行。 在這種情況下，請不要使用互動式選項。 `sqlcmd` 必須安裝在執行 .bat 檔案的電腦上。  
...
### <a name="g-using-sqlcmd-to-set-encryption-on-azure-sql-database"></a>G. 使用 sqlcmd 設定 Azure SQL Database 的加密  
 `sqlcmd`可以在上與資料的連接上執行， [!INCLUDE[ssSDS](../../includes/sssds-md.md)] 以指定加密和憑證信任。 有兩個 ' sqlcmd ' ' 選項可用：  
