...
|------------------------|-----------------------------|--------------------|-----------------------|------------|  
|**Windows/中斷點**|無法使用|**中斷點**|CTRL+ALT+B|顯示您可以用來檢視和管理中斷點的 **[中斷點]** 視窗。|  
|**Windows/Watch/Watch1**|無法使用|**中斷點/監看式/Watch1**|CTRL+ALT+W、1|顯示 **[監看式 1]** 視窗。|  
|**Windows/Watch/Watch2**|無法使用|**中斷點/監看式/Watch2**|CTRL+ALT+W、2|顯示 **[監看式 2]** 視窗。|  
|**Windows/Watch/Watch3**|無法使用|**中斷點/監看式/Watch3**|CTRL+ALT+W、3|顯示 **[監看式 3]** 視窗。|  
|**Windows/Watch/Watch4**|無法使用|**中斷點/監看式/Watch4**|CTRL+ALT+W、4|顯示 **[監看式 4]** 視窗。|  
|**Windows/區域變數**|無法使用|**中斷點/區域變數**|CTRL+ALT+V、L|顯示 [**區域變數**] 視窗。|  
|**Windows/呼叫堆疊**|無法使用|**中斷點/呼叫堆疊**|CTRL+ALT+C|顯示 [**呼叫堆疊**] 視窗。|  
|**Windows/執行緒**|無法使用|**中斷點/執行緒**|CTRL+ALT+H|顯示 [**執行緒**] 視窗。|  
|**持續**|無法使用|**持續**|ALT+F5|執行到下一個中斷點。 除非您將焦點放在處於 debug 模式的 [查詢編輯器] 視窗，否則 [**繼續**] 不是作用中狀態。|  
|**開始調試**|無法使用|**開始調試**|ALT+F5|讓 [查詢編輯器] 視窗進入偵錯模式，並且執行到第一個中斷點。 如果您在處於偵錯模式的 [查詢編輯器] 視窗中放置焦點， **[開始偵錯]** 就會由 **[繼續]** 所取代。|  
|**全部中斷**|無法使用|**全部中斷**|CTRL+ALT+BREAK|
  [!INCLUDE[tsql](../../includes/tsql-md.md)] 偵錯工具不會使用這項功能。|  
|**停止調試**|無法使用|**停止調試**|SHIFT+F5|讓 [查詢編輯器] 視窗離開偵錯模式，並且返回一般模式。|  
|**卸離全部**|無法使用|無法使用|無法使用|停止偵錯模式，但在 [查詢編輯器] 視窗中執行其餘陳述式。|  
|**逐步執行**|無法使用|**逐步執行**|F11|執行下一個陳述式，而且如果下一個陳述式會執行預存程序、觸發程序或函數，就會在偵錯模式中開啟新的 [查詢編輯器] 視窗。|  
|**不進入一步**|無法使用|**不進入一步**|F10|與 **[逐步執行]** 相同，但是不會偵錯任何函數、預存程序或觸發程序。|  
|**跳出**|無法使用|**跳出**|SHIFT+F11|執行觸發程序、函數或預存程序中的其餘程式碼，但是不會針對任何中斷點暫停。 當控制權返回呼叫模組的程式碼時，就會繼續進行一般偵錯模式。|  
|無法使用|**執行至**滑鼠|無法使用|CTRL+F10|執行所有程式碼 (從上一個停止位置到目前的資料指標位置)，但是不會在任何中斷點上停止。|  
...
|無法使用|**新增監看式**|無法使用|無法使用|將選取的運算式加入至 **[監看式]** 視窗。|  
