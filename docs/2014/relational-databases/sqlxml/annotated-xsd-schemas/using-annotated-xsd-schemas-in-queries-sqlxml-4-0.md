...
```  
 ...
 範本也會指定兩個 XPath 查詢。 每個** \<xpath 查詢>** 元素會藉由指定`mapping-schema`屬性來唯一識別對應架構。  
  
 當您在範本中指定內嵌架構時，也`sql:is-mapping-schema`必須在** \<xsd： schema>** 元素上指定批註。 `sql:is-mapping-schema` 會接受布林值 (0 = false，1 = true)。 具有**sql： is-對應架構 = "1"** 的內嵌架構會被視為內嵌批註的架構，而且不會在 XML 檔中傳回。  
 ...
 除了在範本中指定`mapping-schema` ** \<sql： xpath-query>** 專案的屬性（當有 xpath 查詢時），或在 updategram 中的** \<updg： sync>** 元素上，您可以執行下列動作：  
  
-   在範本`mapping-schema`中的** \<根>** 元素（全域宣告）上指定屬性。 接著，這個對應結構描述會變成沒有明確 `mapping-schema` 註解之所有 XPath 和 Updategram 節點使用的預設結構描述。  
  
-   使用 ADO `mapping schema` 物件指定 `Command` 屬性。  
  
 `mapping-schema` **在\<xpath 查詢>** 或** \<updg： sync>** 元素上指定的屬性具有最高優先順序;ADO `Command`物件的優先順序最低。  
