...
ms.locfileid: "36023774"
---
# <a name="specify-field-length-by-using-bcp-sql-server"></a>使用 bcp 指定欄位長度 (SQL Server)
  欄位長度會指出以字元格式表現資料所需的最大字元數。 如果是以原生格式儲存資料的話，欄位長度眾所皆知；例如，`int` 資料類型會佔用 4 個位元組。 如果指出前置長度為 0 **bcp**命令會提示您輸入欄位長度、 預設的欄位長度，以及欄位長度對包含的資料檔案中儲存資料的影響`char`資料。  
...
## <a name="using-default-field-lengths"></a>使用預設的欄位長度  
 ...
 如果指定不正確的欄位長度，就會發生問題。 例如，如果複製數值資料，而對該資料來說指定的欄位長度太短， **bcp** 公用程式就會印出溢位訊息，而且不會複製該資料。 此外，如果您匯出`datetime`資料，並指定欄位長度少於 26 個位元組的字元字串中，為**bcp**公用程式會截斷的資料沒有錯誤訊息。  
  
> [!IMPORTANT]  
>  使用預設的大小選項時， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 預期會讀取整個字串。 在某些狀況下，使用預設欄位長度卻會導致發生「未預期的檔案結尾」錯誤。 一般而言，會發生此錯誤與`money`和`datetime`資料類型時只有部分預期的欄位，就會發生在資料檔案中; 例如，當`datetime`值*公釐*/*dd* / *yy*指定沒有時間元件，所以也，小於預期 24 個字元長度的`datetime`值`char`格式。 若要避免此種錯誤類型，請使用欄位結束字元或固定長度資料欄位，或指定其他的值以變更預設的欄位長度。  
...
|XML|0|  
 ...
 \*如需有關`decimal`和`numeric`資料類型，請參閱[decimal 和 numeric &#40;TRANSACT-SQL&#41;](/sql/t-sql/data-types/decimal-and-numeric-transact-sql)。  
...
|`timestamp`|8|  
 ...
 <sup>1</sup>如需有關`decimal`和`numeric`資料類型，請參閱[decimal 和 numeric &#40;TRANSACT-SQL&#41;](/sql/t-sql/data-types/decimal-and-numeric-transact-sql)。  
