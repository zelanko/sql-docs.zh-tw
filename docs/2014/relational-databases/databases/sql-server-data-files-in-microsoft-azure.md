...
-   在此功能的目前版本中，不支援將 `FileStream` 資料儲存在 Azure 儲存體中。 您可以將 `Filestream` 資料儲存在 Azure 儲存體整合的本機資料庫中，但無法使用 Azure 儲存體在電腦之間移動 Filestream 資料。 對於 `FileStream` 資料，我們建議您繼續使用傳統的技術，在不同的電腦之間移動與 Filestream 相關聯的檔案 (.mdf 和 .ldf)。  
...
-   您無法使用 Azure 儲存體功能中的 SQL Server 資料檔案，將記憶體內部 OLTP 資料儲存在 Azure Blob 中。 這是因為記憶體內部 OLTP 相依于 `FileStream`，而在目前版本的這項功能中，不支援將 `FileStream` 資料儲存在 Azure 儲存體中。  
