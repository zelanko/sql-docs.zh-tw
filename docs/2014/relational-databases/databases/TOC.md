...
# [Windows Azure 中的 SQL Server 資料檔案](sql-server-data-files-in-microsoft-azure.md)
...
# [管理在另一個伺服器執行個體上提供資料庫時所需的中繼資料](manage-metadata-when-making-a-database-available-on-another-server.md)
