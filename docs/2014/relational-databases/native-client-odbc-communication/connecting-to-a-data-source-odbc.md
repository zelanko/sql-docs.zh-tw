...
-   **SQLBrowseConnect**  
 ...
 如需有關連接到資料來源，包括各種連接字串可用的選項，請參閱[搭配 SQL Server Native Client 使用連接字串關鍵字](../native-client/applications/using-connection-string-keywords-with-sql-server-native-client.md)。  
 ...
 **SQLConnect**假設資料來源名稱、 使用者識別碼和密碼是足以連接至資料來源，而 ODBC 資料來源包含 ODBC 驅動程式進行連接時所需的所有其他資訊。 不同於[SQLDriverConnect](../native-client-odbc-api/sqldriverconnect.md)並[SQLBrowseConnect](../native-client-odbc-api/sqlbrowseconnect.md)， **SQLConnect**不會使用連接字串。  
