...
|DBPROP_INIT_ASYNCH|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client OLE DB 提供者支援非同步初始化。<br /><br /> 在 DBPROP_INIT_ASYNCH 屬性中設定 DBPROPVAL_ASYNCH_INITIALIZE 位元會使 **IDBInitialize::Initialize** 成為未封鎖的呼叫。 如需詳細資訊，請參閱 <<c0> [ 正在執行非同步作業](../native-client/features/performing-asynchronous-operations.md)。|  
...
|屬性識別碼|描述|  
|-----------------|-----------------|  
|SSPROP_AUTH_OLD_PASSWORD|類型：VT_BSTR<br /><br /> R/W:寫入<br /><br /> 預設：VT_EMPTY<br /><br /> 描述：目前或過期的密碼。 如需詳細資訊，請參閱 <<c0> [ 變更密碼以程式設計方式](../native-client/features/changing-passwords-programmatically.md)。|  
...
|SSPROP_INIT_MARSCONNECTION|類型：VT_BOOL<br /><br /> R/W:讀取/寫入<br /><br /> 預設：VARIANT_FALSE<br /><br /> 描述：指定連接是否已啟用多個作用中結果集 (MARS)。 在連接到資料庫之前，必須將此選項設定為 True。 如需詳細資訊，請參閱[使用 Multiple Active Result Sets &#40;MARS&#41;](../native-client/features/using-multiple-active-result-sets-mars.md)。|  
...
|SSPROP_INIT_NETWORKLIBRARY|類型：VT_BSTR<br /><br /> R/W:讀取/寫入<br /><br /> 描述：之網路程式庫 (DLL) 用來通訊的執行個體的名稱[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 名稱不得包含路徑或 .dll 副檔名。<br /><br /> 預設值可以使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 用戶端組態公用程式自訂。 **注意：** 這個屬性支援只有 TCP 和具名管道。 如果您搭配前置詞使用此屬性，結尾有雙前置詞時，會導致錯誤，因為此屬性用來在內部產生前置詞。|  
