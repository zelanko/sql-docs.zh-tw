...
ms.locfileid: "36146512"
---
# <a name="use-enhanced-date-and-time-features-ole-db"></a>使用增強型日期和時間功能 (OLE DB)
  此範例會示範如何使用在 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 中導入的日期/時間功能。 此範例會使用四個新日期和時間類型 (`date`，**時間**， `datetime2`，和`datetimeoffset`) 來執行具有參數的命令，並擷取資料列集結果。  
