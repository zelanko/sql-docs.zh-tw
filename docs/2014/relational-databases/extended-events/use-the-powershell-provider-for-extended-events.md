...
-   在物件總管中，展開執行個體名稱、展開 [管理]、以滑鼠右鍵按一下 [擴充事件]，然後按一下 [啟動 PowerShell]。 這樣就會在下列路徑中啟動 PowerShell：  
 ...
 若要檢視與工作階段相關聯的事件、其述詞和目標，您可以將目錄變更為工作階段名稱，然後檢視 events 或 targets 資料夾。 例如，若要檢視的事件及其述詞，與預設系統健康工作階段，從 PS sqlserver: \xevent 相關聯\\*ServerName*\\*InstanceName*\Sessions> > 路徑中，輸入`cd system_health\events,`按下 ENTER、 輸入`dir`，然後按 ENTER 鍵。  
...
-   指令碼必須執行從 PS SQLSERVER:\\> 提示字元 (輸入`sqlps`在命令提示字元)。  
