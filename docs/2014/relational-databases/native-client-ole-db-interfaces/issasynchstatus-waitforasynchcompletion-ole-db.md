...
> [!NOTE]  
>  當使用 OLE DB 服務元件時，可能會傳回 S_OK 時預期 DB_S_ASYNCHRONOUS，讓應用程式應該呼叫[issasynchstatus:: Getstatus](issasynchstatus-getstatus-ole-db.md)傳回 S_OK 或 DB_S_ASYNCHRONOUS 時，檢查是否完成。  
 ...
 此外，SSPROP_ISSAsynchStatus 屬性已加入到 DBPROPSET_SQLSERVERROWSET 屬性集。 支援的提供者[ISSAsynchStatus](issasynchstatus-ole-db.md)介面必須實作此屬性值為 VARIANT_TRUE。  
