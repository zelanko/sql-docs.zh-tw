...
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 備份作業會使用多個執行緒來最佳化 Windows Azure Blob 儲存體服務的資料傳輸。  不過，其效能取決於各種因素，例如 ISV 頻寬和資料庫的大小。 如果您計畫要備份內部部署 SQL Server 資料庫中的大型資料庫或檔案群組，建議您先進行一些輸送量測試。 [Windows Azure 儲存體 SLA 的](https://go.microsoft.com/fwlink/?LinkId=271619)有最大的處理時間，您可以納入考量的 blob。  
...
-   使用下列錯誤記錄來協助疑難排解備份錯誤：  
    ...
    -   您也可以藉由檢閱 Windows 事件記錄檔-在應用程式記錄檔，名稱為 'SQLBackupToUrl' 找到資訊。  
  
-   從壓縮備份還原時，您可能會看見下列錯誤：  
  
    -   **發生 SqlException 3284。嚴重性：16 狀態：5**  
        **訊息在裝置上的檔案標記 'https://mystorage.blob.core.windows.net/mycontainer/TestDbBackupSetNumber2_0.bak' 未對齊。請使用用來建立備份組的相同區塊大小，重新發出 Restore 陳述式：'65536' 類似可能值。**  
...
-   含有使用中租用的 Blob 導致備份期間發生錯誤：失敗的備份活動可能會產生含有使用中租用的 Blob。  
     ...
     **Backup to URL 收到遠端端點的例外狀況。例外狀況訊息：遠端伺服器傳回錯誤：(412) 目前 blob 上的租用，而且要求中指定任何租用識別碼**。  
...
2.  將組態檔放在 SQL Server 執行個體的 Binn 資料夾。 例如，如果我的 SQL Server 安裝在電腦的 C 磁碟機上，將設定檔置於這裡：*C:\Program Files\Microsoft SQL Server\MSSQL12。\<執行個體名稱 > \MSSQL\Binn*。  
