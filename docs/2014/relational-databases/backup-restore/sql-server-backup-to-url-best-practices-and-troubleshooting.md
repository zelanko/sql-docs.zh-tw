...
-   [使用 Azure Blob 儲存體服務的 SQL Server 備份及還原](sql-server-backup-and-restore-with-microsoft-azure-blob-storage-service.md)  
...
-   
  [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 備份作業會使用多個執行緒來最佳化 Azure Blob 儲存體服務的資料傳輸。  不過，其效能取決於各種因素，例如 ISV 頻寬和資料庫的大小。 如果您計畫要備份內部部署 SQL Server 資料庫中的大型資料庫或檔案群組，建議您先進行一些輸送量測試。 [Azure 儲存體 SLA](https://go.microsoft.com/fwlink/?LinkId=271619)具有 blob 的最長處理時間，您可以將其納入考慮。  
  
-   備份大型檔案時，務必遵循`WITH COMPRESSION`管理備份**一節中的建議使用 ** 選項。  
...
-   備份期間發生錯誤，因為 Blob 擁有使用中租用：失敗的備份活動可能會產生擁有使用中租用的 Blob。  
     ...
     **備份至 URL 收到來自遠端端點的例外狀況。例外狀況訊息：遠端伺服器傳回錯誤：（412） blob 上目前有租用，而且要求中未指定租用識別碼**。  
