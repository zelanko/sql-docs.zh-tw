...
# <a name="sql-server-backup-and-restore-with-azure-blob-storage-service"></a>使用 Azure Blob 儲存體服務的 SQL Server 備份及還原
  本主題介紹[Azure Blob 儲存體服務](http://www.windowsazure.com/develop/net/how-to-guides/blob-storage/)的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 備份和還原。 它也提供使用 Azure Blob 服務儲存 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 備份的優點摘要。  
...
-   目前，針對在 Azure 虛擬機器中執行的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 實例，您可以藉由建立連接的磁片來備份至 Azure Blob 儲存體服務。 不過，您可以連接到 Azure 虛擬機器的磁片數目有所限制。 超大執行個體的限制為 16 個磁碟，而較小執行個體的限制則更低。 藉由啟用直接備份至 Azure Blob 儲存體，您可以略過16個磁片限制。  
