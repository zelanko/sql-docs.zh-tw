 ...
 **Blob** ：任何類型和大小的檔案。 有兩種類型的 blob 可儲存在 Azure Blob 儲存體服務中：區塊和分頁 blob。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 備份會使用分頁 Blob 做為 Blob 類型。{2} 您可以使用下列 URL 格式來定址 blob： HTTPs://\<儲存體帳戶 >. net/\<容器 >/\<blob >  
