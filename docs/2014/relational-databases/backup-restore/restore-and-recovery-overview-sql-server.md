...
|檔案還原**\***|還原一個或多個損毀的唯讀檔案，而不還原整個資料庫。 唯有當資料庫至少有一個唯讀檔案群組時，才能使用檔案還原。|還原一個或多個檔案，而不還原整個資料庫。 可以在資料庫離線時，或在資料庫仍在線上時 (適用於某些 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]版本) 執行檔案還原。 在檔案還原期間，包含正在還原中之檔案的檔案群組一律為離線狀態。|  
 ...
 **\*** 僅適用于 Enterprise edition 的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]  
  
 **\*\*** 如需必要的條件，請參閱本主題稍後[的簡單復原模式下的還原限制](#RMsimpleScenarios)。  
