...
4.  在 **[一般]** 頁面上，使用 **[來源]** 區段指定要還原之備份組的來源和位置。 選取下列其中一個選項：  
    ...
    -   **[裝置]**  
         ...
         **[加入]**  
         新增備份裝置的位置**備份位置**清單。 依據您在 **[備份媒體]** 欄位中選取的媒體類型，按一下 **[加入]** 就會開啟下列其中一個對話方塊。  
...
9. 若要檢視或選取進階選項，在 **[選項]** 頁面的 **[還原選項]** 面板中，您可以選取下列任何選項 (如果情況適用)：  
    ...
    3.  如果選取的時間點需要 [還原前先進行結尾記錄備份]，則會予以選取。 您不需要修改這個設定，但是即使不需要，還是可以選擇備份記錄結尾。 在這裡的檔案名稱？ 如果在 **[一般]** 頁面上設定的第一個備份位於 Windows Azure 中，結尾記錄也會備份至相同的儲存體容器。  
