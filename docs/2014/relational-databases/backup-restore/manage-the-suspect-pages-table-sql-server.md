...
-   **自動更新 suspect_pages 資料表**  
     ...
     資料庫鏡像夥伴或 AlwaysOn 可用性複本會在嘗試從資料檔讀取頁面，但是因為以下其中一個原因而失敗之後，更新 **suspect_pages** 資料表。  
...
-   **資料庫管理員的維護角色**  
     ...
     若要監視 suspect_pages 資料表上的活動，您可以使用 [Database Suspect Data Page 事件類別](../event-classes/database-suspect-data-page-event-class.md)。 資料列有時會因為暫時性錯誤而加入 **suspect_pages** 資料表中。 但是，如果有許多資料列加入此資料表，則可能意味著 I/O 子系統發生問題。 如果您注意到突然有許多資料列加入到此資料表，我們建議您最好調查 I/O 子系統是否有問題。  
