 ...
 **請注意**資料表值函式只能執行透過資料存取[!INCLUDE[tsql](../../includes/tsql-md.md)]以查詢`InitMethod`方法，而不在`FillRow`方法。 如果執行 [!INCLUDE[tsql](../../includes/tsql-md.md)] 查詢，應該以 `InitMethod` 屬性 (Attribute) 的屬性 (Property) 來標記 `SqlFunction.DataAccess.Read`。  
