...
### <a name="measurements-in-spatial-data-types"></a>空間資料類型的度量
 ...
 在橢圓體 (或圓形地球) 系統中，會使用經緯度來提供座標。 但是，長度和區域通常會使用公尺和平方公尺來測量，但是這樣的測量可能取決於 `geography` 執行個體的空間參考識別碼 (SRID)。  資料類型最常見的度量單位是公尺。
...
```
  ...
  執行個體可以直接儲存圓弧線段 (2 2, 1 3, 0 2)，而  執行個體則必須將曲線轉換成許多較小的直線線段。
...
### <a name="polygon-and-curvepolygon-comparison"></a>Polygon 和 CurvePolygon 的比較
  執行個體可以在定義其外部與內部環形時，使用  和  執行個體。   執行個體無法使用圓弧線段類型： 和 。


## <a name="see-also"></a>另請參閱
 [空間資料 &#40;SQL Server&#41;](../spatial/spatial-data-sql-server.md) [Geometry 資料類型方法參考](/sql/t-sql/spatial-geometry/spatial-types-geometry-transact-sql) [geography 資料類型方法參考](/sql/t-sql/spatial-geography/spatial-types-geography) [STNumCurves &#40;Geometry 資料類型&#41;](/sql/t-sql/spatial-geometry/stnumcurves-geometry-data-type) [STNumCurves &#40;geography 資料類型&#41;](/sql/t-sql/spatial-geography/stnumcurves-geography-data-type) [STGeomFromText &#40;geometry 資料類型&#41;](/sql/t-sql/spatial-geometry/stgeomfromtext-geometry-data-type) [STGeomFromText &#40;geography 資料類型&#41;](/sql/t-sql/spatial-geography/stgeomfromtext-geography-data-type)
