...
-   `geography`資料類型方法需要兩個輸入`geography`執行個體，例如 stintersection （）、 stunion （）、 stdifference （） 和 stsymdifference （），將會傳回 null 方法的結果未納入單一半球時。 如果輸出超過單一半球，STBuffer() 也會傳回 null。  
...
```  
 ...
 `CompoundCurve` 執行個體同時允許`LineString`和`CircularString`元件因此只需要兩個點的圓形圖配量之直線線段為已知。  此程式碼範例會示範如何使用 `CompoundCurve` 來儲存相同的圖形：  
