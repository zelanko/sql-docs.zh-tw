...
### <a name="example-cast-a-string-value-to-typed-xml-and-assign-it-to-a-column"></a>範例將字串值轉換成具類型的 xml，並將它指派到的資料行  
 下列範例會轉換為字串變數，其中包含以 XML 片段`xml`資料類型，並將其儲存在`xml`類型資料行：  
...
```  
 ...
 下列插入作業會將隱含轉換的字串`xml`類型：  
  
```  
INSERT INTO T VALUES (3, @s)   
```  
  
 您可以明確地使用 cast （） 的字串`xml`類型：  
...
### <a name="example-convert-a-string-to-typed-xml-and-assign-it-to-a-variable"></a>範例將字串轉換成 xml 類型，並將它指派給變數  
...
```  
 ...
 在下列範例中，具型別的`xml`FOR XML 查詢的結果插入`xml`類型資料行：  
...
## <a name="using-bulk-load"></a>使用大量載入  
 增強的 [OPENROWSET (Transact-SQL)](/sql/t-sql/functions/openrowset-transact-sql) 功能，可讓您在資料庫中大量載入 XML 文件。 您可以大量載入 XML 執行個體，從檔案將`xml`類型資料行在資料庫中。 如需實用範例，請參閱[大量匯入與匯出 XML 文件的範例 &#40;SQL Server&#41;](../import-export/examples-of-bulk-import-and-export-of-xml-documents-sql-server.md)。 如需有關載入 XML 文件的詳細資訊，請參閱[載入 XML 資料](load-xml-data.md)。  
