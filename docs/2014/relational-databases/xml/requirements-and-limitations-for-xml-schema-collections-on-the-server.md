...
|**\<xsd:choice>**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]會拒絕具有沒有子系之物件的架構 **\<xsd:choice>** ，除非已使用值為零的**minOccurs**屬性來定義物件。|  
|**\<xsd:include>**|目前， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 不支援這個元素。 伺服器將會拒絕包含此元素的 XML 結構描述。<br /><br /> 作為解決方案，包含指示詞的 XML 架構 **\<xsd:include>** 可進行前置處理，以便將任何內含架構的內容複寫和合併成單一架構，以上傳至伺服器。 如需詳細資訊，請參閱 [前置處理結構描述以合併包含的結構描述](preprocess-a-schema-to-merge-included-schemas.md)。|  
...
|**xs： QName**|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]不支援使用 XML 架構限制元素的**xs： QName**衍生的類型。<br /><br /> [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]不支援**xs： QName**為成員元素的聯集類型。<br /><br /> 如需詳細資訊，請參閱 [xs:QName 類型](the-xs-qname-type.md)。|  
...
|ID 屬性|每個 XML 結構描述元件都可擁有識別碼屬性。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]會對識別碼類型的宣告強制唯一性 **\<xsd:attribute>** ，但不會儲存這些值。 **ID** 唯一性的強制範圍是 {CREATE &#124; ALTER} XML SCHEMA COLLECTION 陳述式。|  
...
|區域命名空間|必須針對元素明確指定區域命名空間 **\<xsd:any>** 。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 會拒絕使用空字串 ("") 作為命名空間屬性值的結構描述。 不過， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 會要求明確使用 "##local"，將不完整的元素或屬性指示成萬用字元的執行個體。|  
...
|[&#60;xsd:redefine&#62; 元素](the-xsd-redefine-element.md)|說明使用元素的限制 \<xsd:redefine> ，並說明因應措施。|  
|[xs:QName 類型](the-xs-qname-type.md)|描述有關 xs:QName 類型的限制。|  
|[&#60;xsd:simpleType&#62; 宣告的值](values-for-xsd-simpletype-declarations.md)|描述適用于宣告的限制 \<xsd:simpleType> 。|  
...
## <a name="see-also"></a>另請參閱  
 ...
 [授與 XML 架構集合的許可權](grant-permissions-on-an-xml-schema-collection.md)   
