...
|訊息文字|資料表錯誤：頁面 P_ID，其標頭顯示被配置給物件識別碼 O_ID，索引識別碼 I_ID，分割區識別碼 PN_ID，配置單位識別碼 A_ID （類型 TYPE），會配置另一個物件。|  
...
> [!CAUTION]  
>  若針對相符的 [MSSQLSERVER_2533](mssqlserver-2533-database-engine-error.md) 錯誤執行 REPAIR，則會先取消配置頁面再進行重建。  
