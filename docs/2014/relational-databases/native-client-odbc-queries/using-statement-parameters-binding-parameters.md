...
-   呼叫[SQLFreeStmt](../native-client-odbc-api/sqlfreestmt.md)具有*選項*參數設定為 SQL_RESET_PARAMS，釋出所有的參數繫結至陳述式控制代碼。  
...
-   當每一個個別參數繫結到自己的變數陣列時，就會完成資料行取向繫結。  
     ...
     資料行取向繫結由呼叫[SQLSetStmtAttr](../native-client-odbc-api/sqlsetstmtattr.md)具有*屬性*設為 SQL_ATTR_PARAM_BIND_TYPE 以及*ValuePtr*設為 SQL_PARAM_BIND_BY_COLUMN。  
