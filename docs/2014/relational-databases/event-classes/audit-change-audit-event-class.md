...
# <a name="audit-change-audit-event-class"></a>Audit Change Audit 事件類別
  每當進行審核追蹤修改時，就會發生**Audit Change Audit**事件類別。  
...
|**SessionLoginName**|**nvarchar**|引發工作階段之使用者的登入名稱。 例如，如果您使用 Login1 連接[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]到，並以 Login2 的身分執行語句，**則 sessionloginname 將**會顯示 Login1，而**LoginName**會顯示 Login2。 此資料行將同時顯示 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 和 Windows 登入。|64|是|  
