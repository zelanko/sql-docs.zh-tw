...
|**EventSubClass**|`int`|鎖定擴大的原因：<br /><br /> **0-LOCK_THRESHOLD**表示語句超過鎖定臨界值。<br /><br /> **1-MEMORY_THRESHOLD**表示語句超過記憶體臨界值。|21|是|  
...
|**LineNumber**|`int`|
  [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式的行號。|5|是|  
...
|**ObjectID**|`int`|已觸發鎖定擴大之資料表的系統指派識別碼。|22|是|  
|**ObjectID2**|`bigint`|相關物件或實體的識別碼。 (已觸發鎖定擴大的 HoBT 識別碼)。|56|是|  
|**投影**|`int`|
  [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式的起始位移。|61|是|  
