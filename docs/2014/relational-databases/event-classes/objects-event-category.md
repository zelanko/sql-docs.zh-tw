---
title: Objects 事件類別目錄 | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.technology: supportability
ms.topic: conceptual
topic_type:
- apiref
helpviewer_keywords:
- SQL Server event classes, Objects event category
- Objects event category [SQL Server]
- event classes [SQL Server], Objects event category
ms.assetid: acc1c7b0-4abf-42f8-b9a9-c383b715a735
author: stevestein
ms.author: sstein
manager: craigg
ms.openlocfilehash: 56a76c3675ceb3cf84c7812b4f9d95d61f9f830a
ms.sourcegitcommit: ceb7e1b9e29e02bb0c6ca400a36e0fa9cf010fca
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 12/03/2018
ms.locfileid: "52778428"
---
# <a name="objects-event-category"></a>Objects 事件類別目錄
  **Objects** 事件類別目錄包含一般物件事件。  
  
## <a name="in-this-section"></a>本節內容  
  
|主題|描述|  
|-----------|-----------------|  
|[Auto Stats 事件類別](auto-stats-event-class.md)|指出已自動更新索引和資料行統計資料。|  
|[Object:Altered 事件類別](object-altered-event-class.md)|指出已改變物件；例如透過 ALTER DATABASE 陳述式或 ALTER TABLE 陳述式。|  
|[Object:Created 事件類別](object-created-event-class.md)|指出已建立物件；例如透過 CREATE INDEX 陳述式、CREATE TABLE 陳述式或是 CREATE DATABASE 陳述式。|  
|[Object:Deleted 事件類別](object-deleted-event-class.md)|指出已刪除物件；例如透過 DROP INDEX 陳述式或 DROP TABLE 陳述式。|  
  
## <a name="see-also"></a>另請參閱  
 [擴充事件](../extended-events/extended-events.md)  
  
  
