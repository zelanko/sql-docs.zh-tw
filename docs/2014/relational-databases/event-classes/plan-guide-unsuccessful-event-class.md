...
-   計畫指南定義中的提示 (包括 USE PLAN 提示) 沒有成功套用至查詢或批次。 也就是說，已編譯的查詢計畫無法接受指定的提示，而且系統將不會使用此計畫指南來編譯計畫。  
 ...
 無效的計畫指南可能會導致引發此事件。 請使用 [sys.fn_validate_plan_guide](/sql/relational-databases/system-functions/sys-fn-validate-plan-guide-transact-sql) 函數來驗證查詢或批次所使用的計畫指南，然後更正此函數所回報的錯誤。  
