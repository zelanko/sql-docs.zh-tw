...
# <a name="showplan-all-for-query-compile-event-class"></a>Showplan All for Query Compile 事件類別
  當編譯 SQL 語句時[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，會發生「執行程式表 All for Query Compile」事件類別。 加入此事件類別，可識別 Showplan 運算子。 所包含的資訊，是 Showplan XML For Query Compile 事件類別中可用資訊的子集。  
