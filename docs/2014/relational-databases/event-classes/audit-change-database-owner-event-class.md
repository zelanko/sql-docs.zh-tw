...
# <a name="audit-change-database-owner-event-class"></a>Audit Change Database Owner 事件類別
  當您使用 ALTER AUTHORIZATION 語句來變更資料庫的擁有者，而且已檢查完成該動作所需的許可權時，就會發生**Audit Change Database Owner**事件類別。  
...
|**TargetLoginName**|**nvarchar**|對於目標為登入的動作，這是目標登入的名稱。|42|是|  
|**TargetLoginSid**|**image**|對於目標為登入的動作，這是目標登入的安全性識別碼 (SID)。|43|是|  
