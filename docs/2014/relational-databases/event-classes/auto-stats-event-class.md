...
|**EventSubClass**|**int**|事件子類別的類型：<br /><br /> 1：同步; 建立/更新的統計資料**TextData**資料行會指出哪些統計資料及其是否已建立或更新。<br /><br /> 2：非同步統計資料更新;工作排入佇列。<br /><br /> 3:非同步統計資料更新;啟動的工作。<br /><br /> 4:非同步統計資料更新;已完成的工作。|21|是|  
...
|**TextData**|**ntext**|此資料行的內容需視統計資料是採同步更新 (**EventSubClass** 1) 或非同步更新 (**EventSubClass** 2、3 或 4) 而定：<br /><br /> 1：列出哪些統計資料已更新/建立<br /><br /> 2、 3 或 4:NULL;**IndexID**資料行會填入已更新統計資料的索引/統計資料識別碼。|1|是|  
