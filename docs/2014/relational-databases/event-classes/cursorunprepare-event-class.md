...
# <a name="cursorunprepare-event-class"></a>CursorUnprepare 事件類別
  **CursorUnprepare**事件類別提供有關應用程式開發介面（API）資料指標中所發生之資料指標 unprepare 事件的資訊。 當捨棄執行計畫時， [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssDE](../../includes/ssde-md.md)]就會發生資料指標 unprepare 事件。  
...
|**DatabaseID**|**int**|由 USE 陳述式所指定的資料庫識別碼，或者如果沒有針對指定執行個體發出 USE 陳述式，則是預設的資料庫。 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)]如果在追蹤中捕捉到**ServerName**資料行，而且伺服器可供使用，則會顯示資料庫的名稱。 請使用 DB_ID 函數判斷資料庫的值。|3|是|  
|**DatabaseName**|**nvarchar**|正在執行使用者陳述式的資料庫名稱。|35|是|  
|**EventClass**|**int**|記錄的事件類型 = 77。|27|否|  
|**EventSequence**|**int**|
  **CursorUnprepare** 事件類別的批次順序。|51|否|  
|**GroupID**|**int**|SQL 追蹤事件引發所在之工作負載群組的識別碼。|66|是|  
|**圖**|**Int**|識別要取消準備的已準備控制代碼。|33|是|  
