...
# <a name="showplan-text-event-class"></a>Showplan Text 事件類別
  執行 SQL 語句時[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，會發生顯示計畫 Text 事件類別。 包含的資訊是 Showplan All、Showplan XML Statistics Profile 或 Showplan XML 事件類別中可用資訊的子集。  
