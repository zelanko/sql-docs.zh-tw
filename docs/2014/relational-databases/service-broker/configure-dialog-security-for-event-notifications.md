...
|[授與權限](/sql/t-sql/statements/grant-transact-sql) 來建立來源資料庫使用者的事件通知。 如需此權限的詳細資訊，請參閱 [CREATE EVENT NOTIFICATION &#40;Transact-SQL&#41;](/sql/t-sql/statements/create-event-notification-transact-sql)。|授與 REFERENCES 權限給現有的事件通知 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 合約上的目標資料庫使用者： `http://schemas.microsoft.com/SQL/Notifications/PostEventNotification`。|  
...
|來源伺服器|目標伺服器|  
|-------------------|-------------------|  
|[建立路由](/sql/t-sql/statements/create-route-transact-sql) 到目標服務，並指定目標資料庫的 Service Broker 識別碼和已同意的 TCP 通訊埠編號。|建立路由到來源服務，並指定來源資料庫的 Service Broker 識別碼和已同意的 TCP 通訊埠編號。 若要指定來源服務，請使用下列提供的服務： `http://schemas.microsoft.com/SQL/Notifications/EventNotificationService`。|  
