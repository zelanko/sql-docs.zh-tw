...
# <a name="sqlvariant-support-for-date-and-time-types"></a>日期和時間類型的 sql_variant 支援
 ...
 若為結果資料行，驅動程式會從變數轉換成日期/時間類型。 如需詳細資訊，請參閱[從 SQL 轉換成 C](datetime-data-type-conversions-from-sql-to-c.md)。繫結至 SQL_C_BINARY 時，緩衝區長度必須夠大，足以接收對應至 SQL 類型的結構。  
