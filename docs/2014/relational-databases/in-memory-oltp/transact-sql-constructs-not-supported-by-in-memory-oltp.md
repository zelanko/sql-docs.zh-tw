...
|功能|ANSI_PADDING OFF|工作階段選項`ANSI_PADDING`建立記憶體最佳化資料表或原生編譯的預存程序時，必須是 ON。 執行 CREATE 陳述式之前，請先執行 `SET ANSI_PADDING ON`。|  
...
|作業|以記憶體最佳化資料表做為 MERGE 的目標|記憶體最佳化資料表不可以是 `MERGE` 作業的目標。 使用`INSERT`， `UPDATE`，或`DELETE`陳述式改為。|  
...
|功能|EXECUTE WITH RECOMPILE|選項`WITH RECOMPILE`不支援原生編譯的預存程序。|  
...
|作業|ALTER AUTHORIZATION|不支援變更現有記憶體最佳化資料表或原生編譯預存程序的擁有者。 請卸除後再重新建立資料表或程序，以變更擁有權。|  
|運算子|OPENROWSET|不支援此運算子。 移除`OPENROWSET`從原生編譯預存程序。|  
|運算子|OPENQUERY|不支援此運算子。 移除`OPENQUERY`從原生編譯預存程序。|  
|運算子|OPENDATASOURCE|不支援此運算子。 移除`OPENDATASOURCE`從原生編譯預存程序。|  
|運算子|OPENXML|不支援此運算子。 移除`OPENXML`從原生編譯預存程序。|  
|運算子|CONTAINSTABLE|不支援此運算子。 移除`CONTAINSTABLE`從原生編譯預存程序。|  
|運算子|FREETEXTTABLE|不支援此運算子。 移除`FREETEXTTABLE`從原生編譯預存程序。|  
|功能|資料表值函式|資料表值函式不可從原生編譯預存程序參考。 此限制的可行因應措施是將資料表值函式內的邏輯加入程序主體中。|  
|運算子|CHANGETABLE|不支援此運算子。 移除`CHANGETABLE`從原生編譯預存程序。|  
|運算子|GOTO|不支援此運算子。 請使用其他程序性建構，例如 WHILE。|  
|運算子|EXECUTE、INSERT EXEC|不支援巢狀原生編譯預存程序。 所需的作業可以用內嵌方式指定為預存程序定義的一部分。|  
|運算子|OFFSET|不支援此運算子。 移除`OFFSET`從原生編譯預存程序。|  
...
|運算子|CONTAINS|不支援此運算子。 移除`CONTAINS`從原生編譯預存程序。|  
|運算子|FREETEXT|不支援此運算子。 移除`FREETEXT`從原生編譯預存程序。|  
|運算子|NOT|不支援此運算子。 移除`NOT`從原生編譯預存程序。 在某些情況下可以將 `NOT` 取代為不相等。 例如，`NOT a=b` 可以取代為 `a!=b`。|  
|運算子|TSEQUAL|不支援此運算子。 移除`TSEQUAL`從原生編譯預存程序。|  
|運算子|LIKE|不支援此運算子。 移除`LIKE`從原生編譯預存程序。|  
...
|運算元|TABLESAMPLE|不支援此運算子。 移除`TABLESAMPLE`從原生編譯預存程序。|  
|選項|RECOMPILE|原生編譯預存程序是在建立時編譯。 若要重新編譯原生編譯預存程序，請將它卸除再重新建立。 移除`RECOMPILE`從程序定義。|  
|選項|ENCRYPTION|不支援這個選項。 移除`ENCRYPTION`從程序定義。|  
|選項|FOR REPLICATION|不可針對複寫建立原生編譯預存程序。 已從程序定義中移除 `FOR REPLICATION`。|  
|選項|FOR XML|不支援這個選項。 移除`FOR XML`從原生編譯預存程序。|  
|選項|FOR BROWSE|不支援這個選項。 移除`FOR BROWSE`從原生編譯預存程序。|  
...
|功能|ROLLUP|`ROLLUP` 無法與原生編譯預存程序中的 `GROUP BY` 子句一起使用。 移除`ROLLUP`從程序定義。|  
|功能|CUBE|`CUBE` 無法與原生編譯預存程序中的 `GROUP BY` 子句一起使用。 移除`CUBE`從程序定義。|  
|功能|GROUPING SETS|`GROUPING SETS` 無法與原生編譯預存程序中的 `GROUP BY` 子句一起使用。 移除`GROUPING SETS`從程序定義。|  
