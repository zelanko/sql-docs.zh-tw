...
|功能|編號的預存程序|原生編譯預存程序不可編號。 請 `;` 從語句中移除該*數位* `CREATE PROCEDURE` 。|  
...
|函式|*Function*|原生編譯預存程序中不支援內建函數。 請從預存程序中移除函數。 如需所支援內建函數的詳細資訊，請參閱原生[編譯的預存程式](../in-memory-oltp/natively-compiled-stored-procedures.md)。|  
...
|函式|*Function*|不支援此函數。 請從原生編譯預存程序中將它移除。|  
