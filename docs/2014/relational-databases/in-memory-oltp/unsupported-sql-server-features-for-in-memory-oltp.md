...
-   容錯移轉叢集：做為一部分[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]AlwaysOn 供應項目，AlwaysOn 容錯移轉叢集執行個體是利用 Windows Server 容錯移轉叢集 (WSFC) 功能，可提供本機高可用性，透過在伺服器執行個體層級的容錯移轉叢集的備援性執行個體 (FCI)。 如需詳細資訊，請參閱 [AlwaysOn 容錯移轉叢集執行個體 (SQL Server)](../../sql-server/failover-clusters/windows/always-on-failover-cluster-instances-sql-server.md)。  
...
|複寫|訂閱者端記憶體最佳化資料表的異動複寫以外的複寫組態與參考記憶體最佳化資料表的資料表或檢視表不相容。 複寫使用 sync_mode = 'database snapshot' 不支援記憶體最佳化檔案群組時。 如需詳細資訊，請參閱[複寫至記憶體最佳化資料表訂閱者](../replication/replication-to-memory-optimized-table-subscribers.md)。|  
