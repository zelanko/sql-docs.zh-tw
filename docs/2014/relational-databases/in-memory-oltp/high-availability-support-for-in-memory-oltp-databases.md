...
-   **可讀取次要**   
    您可以存取與查詢次要複本上的記憶體最佳化資料表 (如果它已設定讀取權限)。 如需詳細資訊，請參閱[作用中次要複本：可讀取次要複本 （AlwaysOn 可用性群組）](../../database-engine/availability-groups/windows/active-secondaries-readable-secondary-replicas-always-on-availability-groups.md)。  
