...
## <a name="linked-servers-and-delegation"></a>連結的伺服器與委派  
 建立連結的伺服器，當`@provstr`的參數[sp_addlinkedserver](/sql/relational-databases/system-stored-procedures/sp-addlinkedserver-transact-sql)可用來指定伺服器和容錯移轉夥伴 Spn。 這麼做的優點是在用戶端連接字串中指定 Spn 相同：建立使用 Kerberos 驗證的連接更簡單，而且更可靠。  
