...
> [!NOTE]  
>  您也可以指定**varchar （max)**， **nvarchar （max)**，並**varbinary （max)** 做為預存程序、 函式的輸入和輸出參數類型的資料類型傳回型別或在[CAST 和 CONVERT](/sql/t-sql/functions/cast-and-convert-transact-sql)函式。  
