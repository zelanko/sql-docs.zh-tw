...
## <a name="ole-db-statement-metadata"></a>OLE DB 陳述式中繼資料  
 從 [!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)] 開始，提供新的 DBCOLUMNFLAGS 旗標值 DBCOLUMNFLAGS_SS_ISCOLUMNSET。 此值應該針對 `column_set` 值的資料行設定。 DBCOLUMNFLAGS 旗標可以透過 IColumnsInfo：： Getcolumnsinfo 所的*dwFlags*參數，以及 IColumnsRowset：： GetColumnsRowset 所傳回之資料列集的 DBCOLUMN_FLAGS 資料行來抓取。  
...
|類型或成員函數|描述|  
|-----------------------------|-----------------|  
|IColumnsInfo::GetColumnsInfo|已針對`column_set` *dwFlags*中的資料行設定新的 DBCOLUMNFLAGS 旗標值 DBCOLUMNFLAGS_SS_ISCOLUMNSET。<br /><br /> 
...
|ICommand::Execute|如果**使用\* select from** *table* ，它會傳回非 sparse `column_set`成員的所有資料行，再加上包含屬於 sparse `column_set`成員之所有非 Null 資料行值（如果有的話）的 XML 資料行。|  
