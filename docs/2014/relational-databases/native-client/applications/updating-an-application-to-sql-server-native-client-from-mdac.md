...
-   MDAC 連接字串允許**Trusted_Connection**關鍵字的布林`true`值（）。 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 連接字串必須使用`yes`或**否**。  
...
-   資料`varchar(max)`類型、、 `nvarchar(max)` `varbinary(max)`、 `xml`、 `udt`或其他大型物件類型的傳回值，不能傳回到早于[!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)]的用戶端版本。 如果您想要使用這些類型當做傳回值，則必須使用 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client。  
...
-   當 MDAC 應用程式連接到 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 時，[!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] 中導入的資料類型將會以 [!INCLUDE[ssVersion2000](../../../includes/ssversion2000-md.md)] 相容的資料類型形式出現，如下表所示。  
    ...
    |`xml`|`ntext`|  
     ...
     此類型對應會影響資料行中繼資料傳回的值。 例如， `text`資料行的大小上限為2147483647，但[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] native client ODBC 會將資料`varchar(max)`行的大小上限報告為 SQL_SS_LENGTH_UNLIMITED，而[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] native client OLE DB 會根據平臺，將`varchar(max)`資料行的大小上限報告為2147483647或-1。  
