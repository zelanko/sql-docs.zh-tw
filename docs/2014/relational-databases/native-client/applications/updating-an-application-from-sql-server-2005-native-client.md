...
|`sp_columns`預存程式現在會針對 IS_NullABLE 資料行傳回 **"no"** ，而不是 **"no** "。|從 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] Native Client 10.0 （ [!INCLUDE[ssKatmai](../../../includes/sskatmai-md.md)] ）開始， `sp_columns` 預存程式現在會針對 IS_NullABLE 資料行傳回 **"no"** ，而不是 **"no** "。|  
