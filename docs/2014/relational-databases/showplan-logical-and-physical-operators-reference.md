...
|無|`Asnyc Concat`|`Asnyc Concat`運算子只用於遠端查詢 （分散式查詢）。 它有 *n* 個子節點和一個父節點。 一般來說，某些子節點是參與分散式查詢的遠端電腦。 `Asnyc Concat` 問題`open()`同時呼叫至所有子系，並再將點陣圖套用到每個子節點。 針對每個是 1 的位元`Async Concat`父節點，視傳送的輸出資料列。|  
...
|無|`Broadcast`|`Broadcast` 有一個子節點並*n*父節點。 `Broadcast` 會依要求將其輸入資料列傳送至多位取用者。 每位取用者都會收到所有資料列。 例如，若所有取用者都是雜湊聯結的建立者，則會建立 *n* 份雜湊資料表。|  
|![建立雜湊運算子圖示](../../2014/database-engine/media/build-hash.gif "建立雜湊運算子圖示")|`Build Hash`|指示建立 xVelocity 記憶體最佳化的資料行存放區索引之批次雜湊資料表。|  
|無|`Cache`|`Cache` 是特製化的版本**多工緩衝處理**運算子。 它只儲存一列資料。 `Cache` 是邏輯運算子。 `Cache` 不會用於[!INCLUDE[ssCurrent](../includes/sscurrent-md.md)]。|  
...
|無|`Flow Distinct`|`Flow Distinct`邏輯運算子會掃描輸入，移除重複項目。 然而`Distinct`運算子會產生任何輸出之前就先取用所有輸入**相異**運算子會傳回每個資料列，因為它會取得一個輸入 （除非該資料列重複，在此情況下則會加以捨棄）。|  
|無|`Full Outer Join`|`Full Outer Join`邏輯運算子會傳回各列相聯結述詞，第一個 （上方） 輸入與第二個 （下方） 輸入每一列聯結。 它也會傳回以下資料列：<br /><br /> - 第一個輸入中與第二個輸入完全不符合者。<br /><br /> - 第二個輸入中與第一個輸入完全不符合者。<br /><br /> <br /><br /> 不包含相符值的輸入會以 Null 值傳回。 `Full Outer Join` 是邏輯運算子。|  
...
|![如果語言項目圖示](../../2014/database-engine/media/if-32x.gif "如果語言項目圖示")|`If`|`If`運算子會執行以運算式為基礎的條件式處理。 `If` 是語言元素。|  
|無|`Inner Join`|`Inner Join`邏輯運算子會傳回滿足第一個 （上方） 輸入與第二個 （下方） 輸入的聯結每個資料列。|  
...
|![合併間隔運算子圖示](../../2014/database-engine/media/merge-interval-32x.gif "合併間隔運算子圖示")|`Merge Interval`|`Merge Interval`運算子會合併多個的 （可能重疊） 間隔以產生最少且非重疊間隔，然後用來尋找索引項目。 這個運算子一般會出現一或多個上述`Compute Scalar`運算子上方`Constant Scan`運算子，後者會建構的間隔 （以資料列中的資料行代表），這個運算子所合併。 `Merge Interval` 是邏輯與實體運算子。|  
...
|![非叢集索引插入運算子圖示](../../2014/database-engine/media/nonclust-index-insert-32x.gif "非叢集索引插入運算子圖示")|`Index Insert`|`Index Insert`運算子會從其輸入中指定的非叢集索引中插入資料列`Argument`資料行。 `Argument`資料行也包含 SET:() 述詞，指出要設定每個資料行的值。 `Index Insert` 是實體運算子。|  
...
|無|`Parallelism`|`Parallelism`運算子會執行散發資料流、 蒐集資料流，並重新分割資料流的邏輯作業。 `Argument`資料行可以包含 PARTITION COLUMNS:() 述詞，以及被分割資料行的逗號分隔的清單。 `Argument`資料行也可以包含 ORDER BY:() 述詞，列出的排序次序分割期間要保留的資料行。 `Parallelism` 是實體運算子。<br /><br /> 注意： 如果查詢已經編譯為平行查詢，但在執行階段中，它執行以序列查詢，顯示計畫輸出由 SET STATISTICS XML 或使用產生**包括實際執行計畫**選項[!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]將不會包含`RunTimeInformation`項目`Parallelism`運算子。 SET STATISTICS PROFILE 輸出中實際的資料列計數和實際數目的執行會顯示為零`Parallelism`運算子。 其中一個條件發生時，這表示`Parallelism`運算子只用於在查詢編譯期間，而不是在執行階段查詢計畫。 請注意，如果伺服器上有大量的並行載入，平行查詢計畫有時會以序列方式執行。|  
...
|無|**部分彙總**|「部分彙總」用於平行計畫。 它將彙總函式套用至盡可能最多的輸入資料列，因此不需要寫入磁碟 (稱為「溢出」)。 `Hash Match` 是實作資料分割的彙總的只是實體運算子 (iterator)。 **部分彙總** 是邏輯運算子。|  
...
|![遠端查詢運算子圖示](../../2014/database-engine/media/remote-query-32x.gif "遠端查詢運算子圖示")|`Remote Query`|`Remote Query`運算子會送出的遠端來源的查詢。 傳送至遠端伺服器的查詢文字會出現在`Argument`資料行。 `Remote Query` 是邏輯與實體運算子。|  
...
|![RID 查閱運算子圖示](../../2014/database-engine/media/rid-nonclust-locate-32x.gif "RID 查閱運算子圖示")|`RID Lookup`|`RID Lookup` 在堆積上的書籤查閱使用提供的資料列識別碼 (RID)。 `Argument`資料行包含書籤標籤，用來查閱資料表中資料列查閱資料表的名稱中的資料列。 `RID Lookup` 一律都會伴隨 NESTED LOOP JOIN。 `RID Lookup` 是實體運算子。 如需書籤查閱的詳細資訊，請參閱 MSDN SQL Server 部落格中的[書籤查閱](http://go.microsoft.com/fwlink/?LinkId=132568)。|  
|無|`Right Anti Semi Join`|`Right Anti Semi Join`運算子會比對中的資料列的第一個 （上方） 輸入不存在時，輸出第二個 （下方） 輸入中的每個資料列。 相符的資料列會定義為滿足該述詞中的資料列`Argument`資料行 （如果沒有述詞存在，每個資料列是相符的資料列）。 `Right Anti Semi Join` 是邏輯運算子。|  
|無|`Right Outer Join`|`Right Outer Join`運算子會傳回每個資料列，皆滿足第二個 （下方） 輸入與每個相符資料列的第一個 （上方） 輸入聯結。 它也會傳回第二個輸入中與第一個輸入完全不相符的任何資料列，以 NULL 相聯結。 如果沒有聯結述詞存在於`Argument`資料行，每個資料列是相符的資料列。 `Right Outer Join` 是邏輯運算子。|  
...
|![資料流彙總運算子圖示](../../2014/database-engine/media/stream-aggregate-32x.gif "資料流彙總運算子圖示")|`Stream Aggregate`|`Stream Aggregate`運算子分組一個或多個資料行中的資料列，然後計算查詢所傳回的一或多個彙總的運算式。 這個運算子的輸出可稍後由查詢中的運算子參考，並/或傳回到用戶端。 `Stream Aggregate` 運算子需要其群組內的輸入項目依資料行排列。 如果資料因為前面的 `Sort` 運算子或因為已排序索引搜尋或掃描，而尚未進行排序，最佳化工具就會在這個運算子之前使用 `Sort` 運算子。 在 SHOWPLAN_ALL 陳述式或中的圖形化執行計畫[!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]，GROUP BY 述詞中的資料行會列在`Argument`直條圖和彙總運算式中所列**Defined Values**資料行。 `Stream Aggregate` 是實體運算子。|  
...
|![資料表插入運算子圖示](../../2014/database-engine/media/table-insert-32x.gif "資料表插入運算子圖示")|`Table Insert`|`Table Insert`運算子會從輸入將指定的資料表中插入資料列`Argument`查詢執行計畫的資料行。 `Argument`資料行也包含 SET:() 述詞，指出要設定每個資料行的值。 如果`Table Insert`沒有插入值的子系，則插入的資料列會從插入運算子本身取得。 `Table Insert` 是實體運算子。|  
