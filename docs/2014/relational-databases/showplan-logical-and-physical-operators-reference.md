...
|None|`Asnyc Concat`|`Asnyc Concat`運算子只用於遠端查詢 （分散式查詢）。 它有 *n* 個子節點和一個父節點。 一般來說，某些子節點是參與分散式查詢的遠端電腦。 `Asnyc Concat` 問題`open()`同時呼叫至所有子系，並再將點陣圖套用到每個子節點。 針對每個是 1 的位元`Async Concat`父節點，視傳送的輸出資料列。|  
...
|None|`Cache`|`Cache` 是特製化的版本**多工緩衝處理**運算子。 它只儲存一列資料。 `Cache` 是邏輯運算子。 `Cache` 不會用於[!INCLUDE[ssCurrent](../includes/sscurrent-md.md)]。|  
...
|None|`Flow Distinct`|`Flow Distinct`邏輯運算子會掃描輸入，移除重複項目。 然而`Distinct`運算子會產生任何輸出之前就先取用所有輸入**相異**運算子會傳回每個資料列，因為它會取得一個輸入 （除非該資料列重複，在此情況下則會加以捨棄）。|  
|None|`Full Outer Join`|`Full Outer Join`邏輯運算子會傳回各列相聯結述詞，第一個 （上方） 輸入與第二個 （下方） 輸入每一列聯結。 它也會傳回以下資料列：<br /><br /> - 第一個輸入中與第二個輸入完全不符合者。<br /><br /> - 第二個輸入中與第一個輸入完全不符合者。<br /><br /> <br /><br /> 不包含相符值的輸入會以 Null 值傳回。 `Full Outer Join` 是邏輯運算子。|  
...
|None|`Inner Join`|`Inner Join`邏輯運算子會傳回滿足第一個 （上方） 輸入與第二個 （下方） 輸入的聯結每個資料列。|  
...
|None|`Left Semi Join`|`Left Semi Join`運算子從第一個 （上方） 輸入傳回每一列第二個 （下方） 輸入中沒有相符的資料列。 如果沒有聯結述詞存在於`Argument`資料行，每個資料列是相符的資料列。 `Left Semi Join` 是邏輯運算子。|  
...
|None|`Parallelism`|`Parallelism`運算子會執行散發資料流、 蒐集資料流，並重新分割資料流的邏輯作業。 `Argument`資料行可以包含 PARTITION COLUMNS:() 述詞，以及被分割資料行的逗號分隔的清單。 `Argument`資料行也可以包含 ORDER BY:() 述詞，列出的排序次序分割期間要保留的資料行。 `Parallelism` 是實體運算子。<br /><br /> 注意： 如果查詢已經編譯為平行查詢，但在執行階段中，它執行以序列查詢，顯示計畫輸出由 SET STATISTICS XML 或使用產生**包括實際執行計畫**選項[!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)]將不會包含`RunTimeInformation`項目`Parallelism`運算子。 SET STATISTICS PROFILE 輸出中實際的資料列計數和實際數目的執行會顯示為零`Parallelism`運算子。 其中一個條件發生時，這表示`Parallelism`運算子只用於在查詢編譯期間，而不是在執行階段查詢計畫。 請注意，如果伺服器上有大量的並行載入，平行查詢計畫有時會以序列方式執行。|  
...
|None|`Right Anti Semi Join`|`Right Anti Semi Join`運算子會比對中的資料列的第一個 （上方） 輸入不存在時，輸出第二個 （下方） 輸入中的每個資料列。 相符的資料列會定義為滿足該述詞中的資料列`Argument`資料行 （如果沒有述詞存在，每個資料列是相符的資料列）。 `Right Anti Semi Join` 是邏輯運算子。|  
|None|`Right Outer Join`|`Right Outer Join`運算子會傳回每個資料列，皆滿足第二個 （下方） 輸入與每個相符資料列的第一個 （上方） 輸入聯結。 它也會傳回第二個輸入中與第一個輸入完全不相符的任何資料列，以 NULL 相聯結。 如果沒有聯結述詞存在於`Argument`資料行，每個資料列是相符的資料列。 `Right Outer Join` 是邏輯運算子。|  
