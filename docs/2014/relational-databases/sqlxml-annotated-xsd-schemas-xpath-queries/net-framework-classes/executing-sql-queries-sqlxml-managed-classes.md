...
3.  在命令提示字元中，執行 DocSample.exe。  
 ...
 您可以指定執行 Updategram (也是一個範本) 的範本 (如以下程式碼片段所示) 來插入客戶記錄，而不是指定 SQL 查詢當做命令文字。 您可以在檔案中指定範本和 Updategrams 並執行這些檔案。 如需詳細資訊，請參閱[使用 CommandText 屬性執行範本檔案](executing-template-files-by-using-the-commandtext-property.md)。  
...
> [!NOTE]  
>  您也可以使用傳回 XmlReader 物件 ExecuteXMLReadermethod。 如需詳細資訊，請參閱[使用 ExecuteXMLReader 方法執行 SQL 查詢](executing-sql-queries-by-using-the-executexmlreader-method.md)。  
