...
```  
 ...
 在查詢中， `child` 是軸而且 `OrderDetail` 是節點測試（如果**OrderDetail**是，則為 TRUE **\<element node>** ，因為 **\<element>** 節點是軸的主要節點 `child` ）。 對於所有 **\<OrderDetail>** 元素節點，會套用述詞中的測試，而且只會傳回滿足條件的節點。  
