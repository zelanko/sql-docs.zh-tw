...
```  
 ...
 在查詢中， `child` 是軸， `Customer` 而和 `Order` 是節點測試（如果 Customer 和 Order 是節點，則這些節點測試為 TRUE **\<element>** ，因為 **\<element>** 節點是軸的主要節點 `child` ）。 針對每個符合的節點 **\<Customer>** ，會將符合的節點 **\<Orders>** 加入到結果中。 只有 **\<Order>** 在結果集中才會傳回。  
