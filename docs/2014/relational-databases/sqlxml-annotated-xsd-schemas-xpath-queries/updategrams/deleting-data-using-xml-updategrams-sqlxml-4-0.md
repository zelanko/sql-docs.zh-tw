...
```  
 ...
 您可以省略**\<之後 >** 標記如果 updategram 會執行刪除作業。 如果您沒有指定選擇性`mapping-schema`屬性 **\<ElementName >** updategram 對應至資料庫資料表和子元素或屬性對應到資料表中的資料行中指定。  
...
1.  完整的範例 B （「 插入多筆記錄使用 updategram 」） 中[插入資料使用 XML Updategram &#40;SQLXML 4.0&#41;](inserting-data-using-xml-updategrams-sqlxml-4-0.md)。  
  
2.  將上述的 updategram 複製到記事本，並將它儲存為 Updategram-removeshifts.xml 相同資料夾中，與用來完成 （「 插入多筆記錄使用 updategram 」） 中[插入資料使用 XML Updategram &#40;SQLXML 4.0&#41;](inserting-data-using-xml-updategrams-sqlxml-4-0.md).  
