...
-   如果您指定 SchemaGen 屬性（例如，SchemaGen = true），就會建立對應架構中所識別的資料表。 但是，SchemaGen 不會在這些資料表上建立任何條件約束（例如主鍵/外鍵條件約束），但有一個例外狀況：如果在關聯性中組成主鍵的 XML 節點定義為具有識別碼的 XML 類型（也就是`type="xsd:ID"`針對 XSD），而且 SGUseID 屬性設定為 True 以進行 SchemaGen，則不只是從識別碼型別節點建立的主鍵，而是從對應架構關聯性建立主鍵/外鍵關聯性。  
