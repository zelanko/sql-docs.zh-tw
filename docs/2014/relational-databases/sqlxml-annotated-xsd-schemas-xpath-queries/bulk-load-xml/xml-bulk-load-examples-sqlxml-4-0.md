...
5.  執行 VBScript 程式碼。  
     ...
     必須指定的結構描述對應`sql:datatype`for **CustomerID**屬性時的值**CustomerID**做為包含大括號 （{和}），這類的 GUID 指定為：  
    ...
    ```  
     ...
     當`sql:datatype`指定用來識別資料行類型為`uniqueidentifier`，在大量載入作業會移除大括號 （{和}） 從**CustomerID**之前插入資料行中的值。  
...
## <a name="j-bulk-loading-in-xml-data-type-columns"></a>J. 大量載入到 xml 資料類型資料行  
 如果對應結構描述指定[xml 資料型別](/sql/t-sql/xml/xml-transact-sql)資料行使用`sql:datatype="xml"`註釋，XML 大量載入可以插入此資料行從來源文件中複製對應欄位的 XML 子元素。  
