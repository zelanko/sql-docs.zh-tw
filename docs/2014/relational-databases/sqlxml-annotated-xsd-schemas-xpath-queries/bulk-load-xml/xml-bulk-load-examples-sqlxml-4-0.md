...
5.  執行 VBScript 程式碼。  
     ...
     `sql:datatype`當**customerid**的值指定為包含大括弧（{和}）的 GUID 時，架構必須指定**customerid**屬性的對應，例如：  
    ...
    ```  
     ...
     當 `sql:datatype` 指定將資料行類型識別為時 `uniqueidentifier` ，大量載入作業會先從**CustomerID**值中移除大括弧（{和}），再將它插入資料行中。  
