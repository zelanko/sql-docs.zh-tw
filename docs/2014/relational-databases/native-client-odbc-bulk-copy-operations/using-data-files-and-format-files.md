...
ms.locfileid: "37417987"
---
# <a name="using-data-files-and-format-files"></a>使用資料檔案與格式檔案
  最簡單的大量複製程式會執行下列動作：  
  
1.  呼叫[bcp_init](../native-client-odbc-extensions-bulk-copy-functions/bcp-init.md)指定大量複製 （設定 BCP_OUT） 從資料表或檢視，以將資料檔。  
...
2.  呼叫[bcp_control](../native-client-odbc-extensions-bulk-copy-functions/bcp-control.md)具有*eOption*設定為 BCPHINTS 並*iValue*設為包含 TRANSACT-SQL 陳述式之 SQLTCHAR 字串的指標。  
...
3.  呼叫**bcp_exec**以便執行大量複製作業。  
 ...
 進行大量複製時從資料檔所使用格式檔案描述，請藉由呼叫讀取格式檔案[bcp_readfmt](../native-client-odbc-extensions-bulk-copy-functions/bcp-readfmt.md)之後**bcp_init**之前**bcp_exec**。  
