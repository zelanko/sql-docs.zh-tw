...
-   Db-library 應用程式呼叫**BCP_SETL**之前連線到啟用 dbprocess 的大量複製作業。 ODBC 應用程式改為呼叫[SQLSetConnectAttr](../native-client-odbc-api/sqlsetconnectattr.md)連線到啟用連接控制代碼上的大量作業之前：  
...
-   Db-library 針對指定的值[bcp_bind](../native-client-odbc-extensions-bulk-copy-functions/bcp-bind.md)*varlen*參數解譯方式與 ODBC **bcp_bind * * * cbData*參數。  
...
-   Db-library  **bcp_colfmt * * * file_collen*和 ODBC [bcp_colfmt](../native-client-odbc-extensions-bulk-copy-functions/bcp-colfmt.md)* cbUserData * 有相同的問題 **bcp_bind * * * varlen*和*cbData*先前所述的參數。 變更任何 Db-library *file_collen*規格為-1 為 SQL_VARLEN_DATA 以及任何*file_collen* 0 為 SQL_NULL_DATA 的規格。  
  
-   *IValue* ODBC 參數[bcp_control](../native-client-odbc-extensions-bulk-copy-functions/bcp-control.md)函式是 void 指標。 資料程式庫中*iValue*是整數。 值轉換為 ODBC *iValue* void *。  
