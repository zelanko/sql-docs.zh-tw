...
-   如果沒有指定 TABLOCK，則將批次大小限制為小於 1,000 資料列。  
 ...
 當大量複製中，從資料檔時，藉由呼叫指定批次大小**bcp_control**使用 BCPBATCH 選項，然後再呼叫[bcp_exec](../native-client-odbc-extensions-bulk-copy-functions/bcp-exec.md)。 當大量複製程式變數使用[bcp_bind](../native-client-odbc-extensions-bulk-copy-functions/bcp-bind.md)和[bcp_sendrow](../native-client-odbc-extensions-bulk-copy-functions/bcp-sendrow.md)，批次大小由呼叫[bcp_batch](../native-client-odbc-extensions-bulk-copy-functions/bcp-batch.md)之後呼叫[bcp_sendrow](../native-client-odbc-extensions-bulk-copy-functions/bcp-sendrow.md) *x*次，其中*x*批次中的資料列數。  
  
 除了指定交易大小外，批次也會影響資料列何時會透過網路傳送到伺服器。 大量複製函數通常會快取中的資料列**bcp_sendrow**直到網路封包填滿，然後再將完整的封包傳送到伺服器。 當應用程式呼叫**bcp_batch**，不過，目前的封包傳送到伺服器，不論它是否已滿。 使用很小的批次大小如果造成傳送許多部分填滿的封包到伺服器，則可能會使效能降低。 例如，呼叫**bcp_batch**之後每個**bcp_sendrow**會導致在不同的封包中傳送的每個資料列，而且除非資料列很大，會浪費每個封包中的空間。 雖然應用程式可以藉由呼叫變更大小，SQL Server 的網路封包的預設大小是 4 KB， [SQLSetConnectAttr](../native-client-odbc-api/sqlsetconnectattr.md)指定 SQL_ATTR_PACKET_SIZE 屬性。  
