...
```  
 ...
 應用程式可以使用[bcp_colptr](../native-client-odbc-extensions-bulk-copy-functions/bcp-colptr.md)函式，每當程式需要變更原本在所指定的資料變數的位址**bcp_bind** _pData_參數。 每當應用程式需要變更原本在 [bcp_bind](../native-client-odbc-extensions-bulk-copy-functions/bcp-collen.md) cbData **參數中指定的資料長度時，可以隨時使用**_bcp_collen_ 函數。  
