...
>  若是大數值類型，將會傳回包含 SQL_SS_LENGTH_UNLIMITED 值的所有長度參數。  
 ...
 適用于 ODBC 2。*x*應用程式不在*TableName*中使用`SQLColumns`萬用字元，會傳回名稱符合*tablename*且由目前使用者擁有之任何資料表的相關資訊。 如果目前使用者沒有任何資料表的名稱符合*tablename*參數， `SQLColumns`會傳回其他使用者所擁有的任何資料表的相關資訊，其中資料表名稱符合*tablename*參數。 適用于 ODBC 2。*x*應用程式使用萬用字元`SQLColumns` ，會傳回名稱符合*TableName*的所有資料表。 適用于 ODBC 3。*x*應用`SQLColumns`程式會傳回名稱符合*TableName*的所有資料表，不論擁有者或是否使用萬用字元。  
