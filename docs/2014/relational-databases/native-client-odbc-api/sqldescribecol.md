...
ms.locfileid: "36036588"
---
# <a name="sqldescribecol"></a>SQLDescribeCol
  對於執行的陳述式， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client ODBC 驅動程式不需要查詢伺服器，以描述結果集中的資料行。 在此情況下，`SQLDescribeCol`不會造成伺服器往返。 像[SQLColAttribute](sqlnumresultcols.md)，則呼叫`SQLDescribeCol`上備妥但未執行的陳述式會產生伺服器往返。  
 ...
 開始 database engine 中的增強功能[!INCLUDE[ssSQL11](../../includes/sssql11-md.md)]允許 SQLDescribeCol 以取得更精確的預期結果的描述。 這些更精確的結果，在舊版的 SQLDescribeCol 傳回的值可能會有所不同[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]。 如需詳細資訊，請參閱[中繼資料探索](../native-client/features/metadata-discovery.md)。  
