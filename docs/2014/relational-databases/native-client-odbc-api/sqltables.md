...
## <a name="sqltables-and-table-valued-parameters"></a>SQLTables 和資料表值參數  
 當 SQL_SOPT_SS_NAME_SCOPE 陳述式屬性具有值為 SQL_SS_NAME_SCOPE_TABLE_TYPE，而非 SQL_SS_NAME_SCOPE_TABLE 的預設值時，SQLTables 傳回資料表類型的相關資訊。 針對資料表中的型別資料行 4 SQLTables 所傳回的結果集傳回的 TABLE_TYPE 值是資料表類型。 如需有關 SQL_SOPT_SS_NAME_SCOPE 的詳細資訊，請參閱[SQLSetStmtAttr](sqlsetstmtattr.md)。  
