...
### <a name="example"></a>範例  
 這個範例示範如何擷取**點**資料當做未經處理位元組到位元組陣列，使用`SqlDataReader`。 該程式碼使用 `System.Text.StringBuilder`，將未經處理位元組轉換成要顯示於主控台視窗的字串表示。  
...
### <a name="example-using-getsqlbytes"></a>使用 GetSqlBytes 的範例  
 這個範例示範如何擷取**點**資料當做在單一作業中使用的未經處理位元組**GetSqlBytes**方法。 該程式碼使用 `StringBuilder`，將未經處理位元組轉換成要顯示於主控台視窗的字串表示。  
...
## <a name="using-udts-in-query-parameters"></a>在查詢參數中使用 UDT  
 設定 `SqlParameter` 物件的 `System.Data.SqlClient.SqlCommand` 時，可以使用 UDT 做為參數值。 對 `SqlDbType.Udt` 集合呼叫 `SqlParameter` 方法時，可使用 `Add` 物件的 `Parameters` 列舉型別表示參數是 UDT。 `UdtTypeName`屬性`SqlCommand`物件用來指定 UDT 的完整的名稱的資料庫中*database.schema_name.object_name*語法。 儘管並非必須如此，但使用完整名稱可消除程式碼的模稜兩可性。  
