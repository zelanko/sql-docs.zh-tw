...
### <a name="example"></a>範例  
 下列範例需要再建立一個包含 `Point` UDT 資料行及 `timestamp` 資料行的資料表。 兩個資料表都用來說明如何建立自訂命令物件來更新資料，以及如何使用 `timestamp` 資料行更新。 執行下列 [!INCLUDE[tsql](../../includes/tsql-md.md)] 陳述式，以建立第二個資料表，並在其中填入範例資料。  
