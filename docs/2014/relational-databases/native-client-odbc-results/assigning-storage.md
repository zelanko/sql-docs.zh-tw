...
# <a name="assigning-storage"></a>指派儲存體
 ...
 為了讓資料行的儲存區產生關聯，應用程式會呼叫[SQLBindCol](../native-client-odbc-api/sqlbindcol.md)並傳遞它：  
...
-   當每個資料行繫結至自己的變數陣列時，就會完成資料行取向繫結。  
     ...
     藉由呼叫[SQLSetStmtAttr](../native-client-odbc-api/sqlsetstmtattr.md)並將*屬性*設定為 SQL_ATTR_ROW_BIND_TYPE，並將*valueptr 是*設定為 SQL_BIND_BY_COLUMN 來指定資料行取向系結。 所有陣列的元素數目都必須相同。  
