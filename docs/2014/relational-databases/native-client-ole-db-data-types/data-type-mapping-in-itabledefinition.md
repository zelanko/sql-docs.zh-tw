...
|DBTYPE_UDT|**UDT**|下列資訊用於`DBCOLUMNDESC`結構由**itabledefinition:: Createtable**需要 UDT 資料行時：<br /><br /> -   *pwSzTypeName*會被忽略。<br />-   *rgPropertySets*必須包含`DBPROPSET_SQLSERVERCOLUMN`屬性設定為上一節中所述`DBPROPSET_SQLSERVERCOLUMN`，請在[Using User-Defined 類型](../native-client/features/using-user-defined-types.md)。|  
|DBTYPE_UI1|**tinyint**||  
