...
```  
 ...
 在 [!INCLUDE[ssVersion2005](../../../includes/ssversion2005-md.md)] 以前，預設結果集不能在相同連接上支援多個作用中陳述式。 在連接上執行 SQL 陳述式以後，伺服器要等到結果集中的所有資料列都已經處理過後，才能在該連接上接受來自用戶端的命令 (取消其餘結果集的要求除外)。 若要取消的部分處理的結果集的其餘部分，請呼叫[SQLCloseCursor](../../native-client-odbc-api/sqlclosecursor.md)或[SQLFreeStmt](../../native-client-odbc-api/sqlfreestmt.md)與*fOption*參數設定為 SQL_CLOSE。 若要完成的部分處理的結果集和測試的另一個結果集是否存在，請呼叫[SQLMoreResults](../../native-client-odbc-api/sqlmoreresults.md)。 如果 ODBC 應用程式嘗試連接控制代碼上的命令，完全處理完預設結果集之前，呼叫會產生 SQL_ERROR 並呼叫**SQLGetDiagRec**傳回：  
