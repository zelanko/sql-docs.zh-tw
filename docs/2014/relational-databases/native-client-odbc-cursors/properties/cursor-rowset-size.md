...
-   資料列取向的繫結  
 ...
 [SQLGetData](../../native-client-odbc-api/sqlgetdata.md)也可用來從區塊資料指標擷取資料行的資料。 因為**SQLGetData**一次處理一個資料列**SQLSetPos**必須將特定的資料列中資料列集設定為目前的資料列會在呼叫之前呼叫**SQLGetData**。  
