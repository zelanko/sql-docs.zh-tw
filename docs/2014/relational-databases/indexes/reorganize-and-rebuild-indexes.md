...
> [!TIP]
> 這些值提供概略的方針，讓您可判斷應該在 `ALTER INDEX REORGANIZE` 和 `ALTER INDEX REBUILD` 之間切換的時間點。 不過，實際的值可能隨各種狀況而異。 請務必嘗試不同的值，以判斷適合您環境的最佳臨界值。 例如, 如果指定的索引主要用於掃描工作, 則移除片段可以改善這些作業的效能。 對於主要用於搜尋作業的索引而言, 效能優勢較不明顯。 同樣地, 移除堆積中的片段 (沒有叢集索引的資料表) 對於非叢集索引掃描工作特別有用, 但在查閱作業中會有極大的影響。
...
-  重建唯一的叢集索引
-  重建非唯一的叢集索引
-  變更索引架構, 例如將資料分割配置套用至叢集索引, 或將叢集索引移至不同的檔案群組
