 ...
 **注意**每個資料表只允許一個叢集索引。 每個資料表只允許有一個 xVelocity 記憶體最佳化的資料行存放區索引。  
 ...
 **加入**  
 將資料行加入索引鍵。 從 [**從** * \<資料表名稱*選取資料行>] 對話方塊中，選取當您按一下 [**加入**] 時所顯示的資料表資料行。 如果是空間索引，在您選取一個資料行之後，此按鈕會呈暗灰色。  
 ...
 **內含資料行**  
 在非叢集索引中包含非索引鍵資料行。 此選項可藉由在非叢集索引的分葉層級中加入資料行當做非索引鍵資料行，以略過索引鍵大小總計的目前索引限制，以及參與索引鍵的最大資料行數目。 如需詳細資訊，請參閱[使用內含資料行建立索引](create-indexes-with-included-columns.md)  
 ...
 **資料分割配置**  
 在資料分割結構描述中儲存索引。 按一下 **[資料分割配置]** 以啟用下列方格。 預設清單選取項目是用來儲存資料表資料的資料分割配置。 當您在清單中選取不同的資料分割配置時，會更新方格中的資訊。 如需詳細資訊，請參閱＜ [Partitioned Tables and Indexes](../partitions/partitioned-tables-and-indexes.md)＞。  
...
### <a name="grids"></a>方格  
 此面板會顯示鑲嵌式配置之每一個層級上的方格密度。 密度會指定為 **[低]**、 **[中]** 或 **[高]**。 預設值是 **[中]**。 [**低**] 代表4x4 方格（16個數據格）、[**中**] 代表8x8 方格（64儲存格），而 [**高**] 代表16x16 方格（256個數據格）。 當選擇了 **[幾何自動方格]** 或 **[地理自動方格]** 鑲嵌選項時，就無法使用這些選項。  
