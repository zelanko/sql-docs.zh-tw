...
-   當資料表沒有已停用的叢集索引時，您可以在線上重建已停用的非叢集索引。 然而，如果您使用 ALTER INDEX REBUILD 或 CREATE INDEX WITH DROP_EXISTING 陳述式，則一定要以離線方式重建已停用的叢集索引。 如需線上索引作業的詳細資訊，請參閱 [線上執行索引作業](perform-index-operations-online.md)。  
...
-   如果叢集索引已停用， [DBCC CHECKDB](/sql/t-sql/database-console-commands/dbcc-checkdb-transact-sql) 便無法傳回基礎資料表的相關資訊；相反地，陳述式會報告該叢集索引已停用。 [DBCC INDEXDEFRAG](/sql/t-sql/database-console-commands/dbcc-indexdefrag-transact-sql) 不能用來重組已停用的索引：陳述式會失敗並出現錯誤訊息。 您可以使用 [DBCC DBREINDEX](/sql/t-sql/database-console-commands/dbcc-dbreindex-transact-sql) 來重建已停用的索引。  
