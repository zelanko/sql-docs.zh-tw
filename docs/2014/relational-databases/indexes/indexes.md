...
|columnstore|記憶體中的資料行存放區索引會使用資料行為基礎的資料儲存和資料行為基礎的查詢處理來儲存及管理資料。<br /><br /> 資料行存放區索引可在主要執行大量載入和唯讀查詢的資料倉儲工作負載中順利運作。 與傳統的資料列導向儲存相較之下，使用資料行存放區索引最高可達到 **10 倍查詢效能** 改善，與未壓縮資料大小相較之下，最高可達到 **7 倍資料壓縮** 。|[資料行存放區索引說明](columnstore-indexes-described.md)<br /><br /> [使用非叢集資料行存放區索引](../../database-engine/using-nonclustered-columnstore-indexes.md)<br /><br /> [使用叢集資料行存放區索引](../../database-engine/using-clustered-columnstore-indexes.md)|  
