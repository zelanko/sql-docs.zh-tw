...
-   鏡像伺服器必須具有相同 GUID 的稽核，才能讓資料庫稽核規格寫入稽核記錄。 您可以使用 [`=`*\<從來源伺服器 audit*> 建立 AUDIT WITH guid guid] 命令來設定此項。  
...
### <a name="auditing-administrators"></a>稽核系統管理員  
 系統會將`sysadmin`固定伺服器角色的成員識別為每個資料庫中的**dbo**使用者。 若要稽核系統管理員的動作，請稽核 **dbo** 使用者的動作。  
