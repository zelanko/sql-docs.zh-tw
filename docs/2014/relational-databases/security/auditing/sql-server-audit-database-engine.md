...
-   鏡像伺服器必須具有相同 GUID 的稽核，才能讓資料庫稽核規格寫入稽核記錄。 這可以透過使用 CREATE AUDIT WITH GUID 命令設定`=`*\<從來源伺服器稽核的 GUID*>。  
