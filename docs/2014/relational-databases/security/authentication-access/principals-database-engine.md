...
# <a name="principals-database-engine"></a>主體 (Database Engine)
  *主體*是可以要求[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]資源的實體。 主體就像其他 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 授權模型的元件一樣，可以階層方式安排。 主體的影響範圍視主體的定義範圍 (Windows、伺服器、資料庫)，以及主體是否可分割或者是一個集合而定。 「Windows 登入」是不可分割主體的一個範例，而「Windows 群組」則是主體為集合的範例。 每個主體都有一個安全性識別碼 (SID)。  
...
## <a name="information_schema-and-sys"></a>INFORMATION_SCHEMA 與 sys  
 每個資料庫都包含兩個在目錄檢視中顯示為使用者的實體：INFORMATION_SCHEMA 和 sys。 
  [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]需要這些實體。 它們並非主體，也不能被修改或卸除。  
...
-   [管理登入、使用者和架構的使用說明主題](managing-logins-users-and-schemas-how-to-topics.md)  
