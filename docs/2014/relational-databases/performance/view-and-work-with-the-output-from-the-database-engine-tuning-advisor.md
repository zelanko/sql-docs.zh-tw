 ...
 **詳細資料**  
 [建議的目標] 的描述。 可能的值包括 [叢集]、[索引視圖] 或 [空白], 表示非叢集索引。 此外還指出索引是否是唯一的。  
