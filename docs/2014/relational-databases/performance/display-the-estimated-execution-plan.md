...
4.  若要改變執行計畫的顯示，請以滑鼠右鍵按一下執行計畫，然後選取 [**放大**]、[**縮小**]、[**自訂顯示比例**] 或 [**縮放至適當比例**]。 [放大]**** 與 [縮小]**** 可讓您以固定量放大或縮小執行計畫。 [自訂顯示比例]**** 可讓您定義自己的顯示倍率，例如縮放至百分之 80。 [縮放至適當比例]**** 會放大執行計畫，以符合結果窗格的大小。  
