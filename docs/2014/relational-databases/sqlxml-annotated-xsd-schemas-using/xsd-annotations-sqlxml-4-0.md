...
|`sql:hide`|在產生的 XML 文件中，隱藏結構描述中指定的元素或屬性。|[使用 sql:hide 來隱藏元素和屬性](hiding-elements-and-attributes-by-using-sql-hide.md)|不支援|  
...
|`sql:inverse`|指示 updategram 邏輯反向解釋使用** \<sql： relationship>** 所指定的父子式關聯性。|[在 sql： relationship 上指定 sql：反向屬性 &#40;SQLXML 4.0&#41;](specifying-the-sql-inverse-attribute-on-sql-relationship-sqlxml-4-0.md)|不支援|  
...
|`sql:relationship`|指定 XML 元素之間的關聯性。 
  `parent`、`child`、`parent-key` 和 `child-key` 屬性可用來建立關聯性。|[使用 sql： relationship 指定關聯性 &#40;SQLXML 4.0&#41;](specifying-relationships-using-sql-relationship-sqlxml-4-0.md)|屬性名稱不同：<br /><br /> `key-relation`<br /><br /> `foreign-relation`<br /><br /> `key`<br /><br /> `foreign-key`|  
...
> [!NOTE]  
>  XSD 原生 `targetNamespace` 屬性會取代 `target-namespace` XDR 對應結構描述中推出的 [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)] 註解。  
