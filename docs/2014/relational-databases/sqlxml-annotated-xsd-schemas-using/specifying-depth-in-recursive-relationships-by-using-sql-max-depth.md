...
5.  建立和使用 SQLXML 4.0 測試指令碼 (Sqlxml4test.vbs) 以執行範本。 如需詳細資訊，請參閱[ADO to Execute SQLXML 4.0](../sqlxml/using-ado-to-execute-sqlxml-4-0-queries.md)。  
...
### <a name="recursive-elements"></a>遞迴元素  
 如果您同時在遞迴關聯性中的父元素和子元素上指定了 `sql:max-depth`，就會優先使用在父系上指定的 `sql:max-depth` 註解。 例如，在下列結構描述中，同時在父和子員工元素上指定了 `sql:max-depth` 註解。 在此情況下，`sql:max-depth=4`上指定 **\<Emp >** 父元素 （扮演監督者的角色），會優先。 `sql:max-depth`指定的子系上 **\<Emp >** 元素 （扮演監督的角色） 會被忽略。  
...
### <a name="nonrecursive-elements"></a>非遞迴元素  
 如果您在結構描述中不會導致任何遞迴的元素上指定了 `sql:max-depth` 註解，系統就會忽略此註解。 下列的結構描述，  **\<Emp >** 元素組成**\<常數 >** 子元素，其具有 **\<Emp >** 子項目。  
  
 在這個結構描述，`sql:max-depth`上指定註解**\<常數 >** 項目會被忽略，因為沒有任何遞迴之間 **\<Emp >** 父系和**\<常數 >** 子項目。 但是沒有之間的遞迴 **\<Emp >** 上階和 **\<Emp >** 子系。 此結構描述會同時在這兩個項目上指定 `sql:max-depth` 註解。 因此，`sql:max-depth`註釋所指定的上階 (**\<Emp >** 監督者角色中) 的優先順序。  
...
## <a name="complex-types-derived-by-restriction"></a>限制所衍生的複雜類型  
 如果您擁有所衍生的複雜型別**\<限制 >**，對應的基底複雜類型的元素不能指定`sql:max-depth`註解。 在這些情況下，您可以將 `sql:max-depth` 註解加入至衍生類型的元素。  
  
 另一方面，如果您擁有所衍生的複雜型別**\<副檔名 >**，可以指定對應的基底複雜類型的項目`sql:max-depth`註解。  
  
 例如，下列 XSD 結構描述會產生錯誤，因為在基底類型上指定了 `sql:max-depth` 註解。 由衍生類型上不支援此註解**\<限制 >** 從另一個型別。 若要修正這個問題，您必須變更此結構描述並且在衍生類型的元素上指定 `sql:max-depth` 註解。  
...
```  
 ...
 在此結構描述中，`sql:max-depth` 指定於 `CustomerBaseType` 複雜類型上。 結構描述也指定**\<客戶 >** 類型的項目`CustomerType`，其衍生自`CustomerBaseType`。 在這類結構描述上指定的 XPath 查詢將會產生錯誤，因為定義於限制基底類型中的元素不支援 `sql:max-depth`。  
