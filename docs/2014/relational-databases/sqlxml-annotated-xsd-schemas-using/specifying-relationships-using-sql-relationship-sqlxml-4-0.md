...
### <a name="c-specifying-the-relationship-annotation-on-an-attribute"></a>C. 在屬性上指定關聯性註解  
 此範例中的架構包含\<客戶> 元素，其中包含\<CustomerID> 子項目和 IDREFS 類型的 OrderIDList 屬性。 \<Customer> 元素會對應到 AdventureWorks 資料庫中的 customer 資料表。 根據預設，此對應的範圍會套用至所有子專案或屬性，除非`sql:relation`在子項目或屬性上指定，在此情況下，必須使用\<關聯性> 元素來定義適當的主鍵/外鍵關聯性。 同時，使用 `relation` 註解指定不同資料表的子元素或屬性也必須指定 `relationship` 註解。  
...
```  
 ...
 在架構中， ** \<Emp1>** 元素和** \<Emp2>** 元素都屬於類型。 `EmpType` 此類型`EmpType`描述** \<Order>** 子項目和對應** \<的 sql： relationship>**。 在這種情況下， ** \<sql： relationship>** 中沒有可使用**父**屬性識別的單一父系。 在此情況下，您不會在** \<sql： relationship>** 中指定**parent**屬性。**父**屬性資訊是從架構中的階層取得。  
