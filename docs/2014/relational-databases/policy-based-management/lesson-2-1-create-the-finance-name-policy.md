...
2.  在 **管理原則類別目錄**對話方塊的 **名稱**，型別`Finance`空白方塊中，然後清除**託管資料庫訂閱**。 [託管資料庫訂閱] 將會強制執行個體中的每一個資料庫訂閱屬於這個原則類別目錄的原則。 基於這個理由，只有 Finance 資料庫應該訂閱 Finance Name 原則。  
