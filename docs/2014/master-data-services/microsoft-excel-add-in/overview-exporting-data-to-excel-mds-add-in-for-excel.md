---
title: 載入資料 （MDS 增益集的 Excel） |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36135407"
---
# <a name="loading-data-mds-add-in-for-excel"></a>載入資料 (適用於 Excel 的 MDS 增益集)
  在[!INCLUDE[ssMDSshort](../../includes/ssmdsshort-md.md)] [!INCLUDE[ssMDSXLS](../../includes/ssmdsxls-md.md)]，您必須將資料載入 MDS 儲存機制從使用中的 Excel 工作表之前，您可以使用它。 當您使用資料完成之後，請將它發行至 MDS 儲存機制，讓其他使用者能夠共用資料。  
...
## <a name="filtering-data"></a>篩選資料  
 您可以在載入之前篩選資料，以便限制即將下載的資料量。 這包括選擇您想要載入的屬性 (資料行)、您想要顯示屬性的順序，以及您想要使用的成員 (資料列)。 如需詳細資訊，請參閱[載入之前篩選資料&#40;MDS 增益集的 Excel&#41;](filter-data-before-exporting-mds-add-in-for-excel.md)。  
...
|在下載資料之前變更資料行的順序。|[重新排列資料行&#40;MDS 增益集的 Excel&#41;](reorder-columns-mds-add-in-for-excel.md)|  
