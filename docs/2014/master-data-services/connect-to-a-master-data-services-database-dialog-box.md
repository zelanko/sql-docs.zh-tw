...
|**驗證類型**|選取連接到指定的 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 執行個體時所要使用的驗證類型。 您用來連接的認證會決定顯示在 [Master Data Services 資料庫]**** 下拉式清單中的資料庫。 驗證類型包括：<br /><br /> **目前使用者-整合式安全性**：使用整合式 Windows 驗證，透過目前 Windows 使用者帳戶的認證來連接。 
