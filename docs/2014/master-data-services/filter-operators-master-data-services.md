...
|**比對**|使用模糊查閱索引來篩選結果。<br /><br /> 使用 [相似度層級] 欄位來指定屬性值必須符合所指定之篩選準則的程度 (預設為 30%)。 在 [演算法] 清單方塊中選取下列其中一項：<br /><br /> **Levenshtein**:編輯數目為基礎的距離 (例如，新增或刪除) 所需某個字串要比對另一個字串。 這是預設值。 不需要任何額外的參數。<br /><br /> **Jaccard**:嘗試比對多個字串時最適合的索引。 此搜尋支援其他內含項目偏差的參數 (請參閱下面)。<br /><br /> **Jaro-winkler**:最適用於尋找重複人名的距離。 此方法比其他任何方法傳回的結果還多。 不支援內含項目偏差。<br /><br /> **最長通用序列**:但是可以分隔這些，模式中的字母依序出現的序列根據運作 （例如，"MSR"是"MaSteR"的子序列）。 此搜尋支援其他內含項目偏差的參數 (請參閱下面)。<br /><br /> <br /><br /> 針對 [Jaccard] 或 [最長的通用子序列] 演算法，新增 [內含項目偏差]。 [內含項目偏差] 是以介於 0 和 1 之間的小數位數百分比提供的長度臨界值，預設值為 .62。 臨界值較低時，會增加傳回的可能符合項目數目。|  
