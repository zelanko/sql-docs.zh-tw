...
> [!IMPORTANT]  
>  [!INCLUDE[ssNoteDepFutureAvoid](../includes/ssnotedepfutureavoid-md.md)]請改用 `sqlps` PowerShell 模組。 如需 `sqlps` 模組的詳細資訊，請參閱匯[入 SQLPS 模組](../database-engine/import-the-sqlps-module.md)。  
 ...
 **-Command**  
 指定 `sqlps` 公用程式要執行的命令。 除非同時指定 **-NoExit** ，否則 `sqlps` 公用程式會執行命令，然後結束。 請勿在 **-Command**之後指定任何其他參數，因為這些參數將會讀取成命令參數。  
