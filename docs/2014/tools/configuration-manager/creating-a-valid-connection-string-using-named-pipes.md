---
title: 建立有效的連接字串使用具名的管道 |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
-   輸入 **[管道名稱]**。 或者，您可以將**管道名稱**空白和[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]Configuration Manager 會在您指定之後完成適當的管道名稱**通訊協定**和**伺服器**  
  
-   指定 **[伺服器]**。 對於具名執行個體，您可以提供伺服器名稱與執行個體名稱。  
  
 在連接時， [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Native Client 元件會讀取伺服器、 通訊協定，，和管道名稱值從登錄中指定之別名名稱，並且格式建立管道名稱`np:\\<computer_name>\pipe\<pipename>`或`np:\\<IPAddress>\pipe\<pipename>`。具名的執行個體，預設管道名稱是`\\<computer_name>\pipe\MSSQL$<instance_name>\sql\query`。  
  
> [!NOTE]  
>  [!INCLUDE[msCoName](../../includes/msconame-md.md)] Windows 防火牆會關閉預設為通訊埠 445。 因為[!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]通訊透過連接埠 445，您必須重新開啟通訊埠如果[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]設定為接聽內送的用戶端連接使用具名的管道。 如需設定防火牆的相關資訊，請參閱《 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 線上叢書》中的＜如何：設定防火牆供 SQL Server 存取＞，或請檢閱您的防火牆文件集。  
