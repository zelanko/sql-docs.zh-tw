...
# <a name="tcp-ip-properties-ip-addresses-tab"></a>TCP IP 屬性 （IP 位址索引標籤）
 ...
 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 重新啟動後，變更就會生效。 啟動和停止 SQL Server Browser 服務的詳細資訊，請參閱操作說明：啟動及停止 SQL Server Browser 服務上線。  
 ...
 **[TCP 動態通訊埠]**  
 ...
 若要設定單一 IP 位址接聽多個通訊埠，還必須在 [TCP/IP 屬性]  對話方塊的 [通訊協定]  索引標籤上，將 [全部接聽]  參數設為 [否]  。 如需詳細資訊，請參閱 「 如何：設定 Database Engine 接聽多個 TCP 通訊埠 中[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]線上叢書 》。  
