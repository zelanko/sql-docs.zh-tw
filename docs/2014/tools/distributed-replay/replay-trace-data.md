...
# <a name="replay-trace-data"></a>重新執行追蹤資料
  備妥輸入追蹤資料之後，您[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]可以使用 Distributed Replay 功能啟動分散式重新執行。 如需詳細資訊，請參閱 [準備輸入追蹤資料](prepare-the-input-trace-data.md)。  
...
1.  **（選擇性）修改**重新執行設定：如果您想要修改重新執行設定（例如排序模式和各種調整值），您必須修改以 XML 為`<ReplayOptions>`基礎之重新執行設定檔`DReplay.exe.replay.config`的元素。 此外，您也可以修改 `<OutputOptions>` 元素來指定輸出設定，例如是否要記錄資料列計數。 如果您要修改重新執行組態檔，我們建議您修改複本，而不是原始版本。 若要修改設定，請執行下列步驟：  
    ...
    3.  當起始事件重新執行階段 (下一步) 時，使用 [重新執行]** 選項的 **config_file** 參數指定已修改組態檔的位置。  
     ...
     如需重新執行組態檔的詳細資訊，請參閱 [設定 Distributed Replay](configure-distributed-replay.md)。  
  
2.  起始**事件**重新執行時間：若要啟動 distributed replay，您必須以**replay**選項執行管理工具。 如需詳細資訊，請參閱[重新執行選項 &#40;Distributed Replay 管理工具&#41;](replay-option-distributed-replay-administration-tool.md)。  
...
3.  當分散式重新執行完成時，管理工具就會傳回摘要資訊。 如果您已指定 **-o** 選項，重新執行活動就已經儲存在每個用戶端的結果追蹤檔案中。 如需結果追蹤檔案的詳細資訊，請參閱 [檢閱重新執行結果](review-the-replay-results.md)。  
