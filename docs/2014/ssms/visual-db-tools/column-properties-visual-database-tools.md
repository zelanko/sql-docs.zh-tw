...
# <a name="column-properties-visual-database-tools"></a>資料行屬性 (Visual Database Tools)
  有兩個數據行的屬性集：您可以在資料表設計工具的 [資料**行屬性**] 索引標籤中看到的完整集合[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] （僅適用于資料庫），以及您可以在使用伺服器總管的屬性視窗中看到的子集。  
...
> [!NOTE]  
>  每個資料列的 `varchar(max)` 資料類型長度都會不同。 sp_help 會傳回（-1）做為資料`varchar(max)`行的長度。 
  [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 會顯示 -1 當作資料行大小。  
