---
title: 索引和索引鍵對話方塊（Visual Database Tools） |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
 ...
 **說明**  
 提供描述索引鍵或索引的位置。 若要寫入更詳細的描述，請按一下 [**描述**]，然後按一下屬性欄位右邊的省略號按鈕（**...**）。 如此便可提供較大的區域以寫入文字。  
...
-   **[是]** [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]會發出警告，忽略違規的傳入資料列，並嘗試插入其餘的資料列。  
 ...
 **已停用**  
 指示此索引是否停用。 這是唯讀屬性。 如果已在 Visual Database Tools 外部停用索引，則這個屬性只能設定為 [是]****。  
