...
> [!IMPORTANT]  
>  將 proxy 存取權授與**agentdatabaseroles 意義****的成員**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]之前，請先考慮安全性含意。 **SQLAgentReaderRole** 的成員自動為 **SQLAgentUserRole**的成員。 這表示 **SQLAgentReaderRole** 的成員可以存取所有授與 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SQLAgentUserRole **的** Agent Proxy，也可以使用這些 Proxy。  
