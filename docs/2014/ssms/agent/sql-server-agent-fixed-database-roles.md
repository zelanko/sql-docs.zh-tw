...
-   **SQLAgentOperatorRole**  
 ...
 當使用者不是這些角色的其中一個成員，而以 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 連接到 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 時，他們不會在物件總管中看見 [SQL Server Agent]**** 節點。 使用者必須是這些固定資料庫角色的其中一個成員，或是**系統管理員 (sysadmin)** 固定伺服器角色，才能使用 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent。  
 ...
 **SQLAgentUserRole**是[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 固定資料庫角色的最低許可權。 它只對操作員、本機作業和作業排程擁有權限。 
  **SQLAgentUserRole** 的成員只對他們所擁有的本機作業和作業排程擁有權限。 他們無法使用多伺服器作業 (主要和目標伺服器作業)，也無法變更作業擁有權來取得他們尚未擁有之作業的存取權。 **SQLAgentUserRole**成員只能在的 [**作業步驟屬性**] 對話方塊中，查看可用的 proxy 清單[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]。 
  **SQLAgentUserRole** 的成員在 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 物件總管中只能看見 [作業]**** 節點。  
  
> [!IMPORTANT]  
>  將 proxy 存取權授與**agentdatabaseroles 意義****的成員**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]之前，請先考慮安全性含意。 
  **SQLAgentReaderRole** 和 **SQLAgentOperatorRole** 自動為 **SQLAgentUserRole** 的成員。 這表示 **SQLAgentReaderRole** 和 **SQLAgentOperatorRole** 的成員可以存取所有授與 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SQLAgentUserRole **的** Agent Proxy，也可以使用這些 Proxy。  
 ...
 **SQLAgentReaderRole**包含所有**SQLAgentUserRole**許可權，以及查看可用的多伺服器作業清單、其屬性及其歷程記錄的許可權。 此角色的成員也可以檢視所有可用作業的清單，和作業排程及其屬性，而非只有他們擁有的作業和作業排程。 **SQLAgentReaderRole**成員無法變更作業擁有權來取得他們尚未擁有之作業的存取權。 
  **SQLAgentReaderRole** 的成員在 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 物件總管中只能看見 [作業]**** 節點。  
  
> [!IMPORTANT]  
>  將 proxy 存取權授與**agentdatabaseroles 意義****的成員**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]之前，請先考慮安全性含意。 
  **SQLAgentReaderRole** 的成員自動為 **SQLAgentUserRole** 的成員。 這表示 **SQLAgentReaderRole** 的成員可以存取所有授與 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SQLAgentUserRole **的** Agent Proxy，也可以使用這些 Proxy。  
  
 下表摘要出 **SQLAgentReaderRole** 對 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 物件所擁有的權限。  
  
|動作|操作員|本機作業|多伺服器作業|作業排程|Proxy|  
|------------|---------------|----------------|----------------------|-------------------|-------------|  
|建立/修改/刪除|否|是<sup>1</sup> （僅限擁有的作業）|否|是 (僅擁有的排程)|否|  
 ...
 **SQLAgentOperatorRole**是[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Agent 固定資料庫角色的最具許可權。 它包括 **SQLAgentUserRole** 和 **SQLAgentReaderRole**的所有權限。 此角色的成員也可以檢視操作員和 Proxy 的屬性，並列舉出伺服器上可用的 Proxy 和警示。  
  
 **SQLAgentOperatorRole**成員對本機作業和排程擁有額外的許可權。 他們可以執行、停止或啟動所有本機作業，也可以刪除伺服器上任何本機作業的作業記錄。 還可以啟用或停用伺服器上所有本機作業和排程。 若要啟用或停用本機作業或排程，此角色的成員必須使用預存程序 **sp_update_job** 和 **sp_update_schedule**。 **SQLAgentOperatorRole**的成員只能指定指定作業或排程名稱或識別碼的參數，以及** \@已啟用**的參數。 如果他們指定任何其他參數，執行這些預存程序會失敗。 **SQLAgentOperatorRole**成員無法變更作業擁有權來取得他們尚未擁有之作業的存取權。  
  
 
  **SQLAgentOperatorRole** 的成員可以看見 ** 物件總管中的 [作業]****、[警示]****、[操作員]**[!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 和 [Proxy]**** 節點。 只有 [錯誤記錄檔]**** 節點對此角色的成員是不可見的。  
  
> [!IMPORTANT]  
>  將 proxy 存取權授與**agentdatabaseroles 意義****的成員**[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]之前，請先考慮安全性含意。 
  **SQLAgentOperatorRole** 的成員自動為 **SQLAgentUserRole** 和 **SQLAgentReaderRole** 的成員。 這表示 **SQLAgentOperatorRole** 的成員可以存取所有授與 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] SQLAgentUserRole **或** SQLAgentReaderRole **的** Agent Proxy，也可以使用這些 Proxy。  
...
|------------|------------|---------------|----------------|----------------------|-------------------|-------------|  
|建立/修改/刪除|否|否|是<sup>2</sup> （僅限擁有的作業）|否|是 (僅擁有的排程)|否|  
...
|附加/卸離|不適用|不適用|不適用|不適用|是 (僅擁有的排程)|不適用|  
 ...
 <sup>3</sup> **SQLAgentOperatorRole**成員可以使用預存程式**sp_update_job**並指定** \@已啟用**和** \@job_id** （或** \@job_name**）參數的值，來啟用或停用不是由他們擁有的本機作業。 如果此角色的成員為此預存程序指定任何其他參數，執行程序會失敗。  
  
 <sup>4</sup> **SQLAgentOperatorRole**成員可以使用預存程式**sp_update_schedule**並指定** \@已啟用**和** \@schedule_id** （或** \@名稱**）參數的值，來啟用或停用不是由他們擁有的排程。 如果此角色的成員為此預存程序指定任何其他參數，執行程序會失敗。  
