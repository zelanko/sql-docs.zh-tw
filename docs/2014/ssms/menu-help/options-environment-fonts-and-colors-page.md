...
# <a name="options-environment-fonts-and-colors-page"></a>選項 (環境：字型和色彩頁面)
  [**選項**] 對話方塊可讓您針對中[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)]的各種使用者介面元素，建立自訂字型和色彩配置。 在 [工具]**** 功能表上按一下 [選項]****、展開 [環境]**** 資料夾，然後選取 [字型和色彩]****。  
...
|執行計畫|變更此專案的字型樣式、大小和色彩顯示設定，會影響[!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]和[!INCLUDE[ssEW](../../includes/ssew-md.md)]查詢之執行計畫中的文字外觀。|  
