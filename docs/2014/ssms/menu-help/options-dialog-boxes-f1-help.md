---
title: 選項對話方塊 F1 說明 | Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
ms.prod: sql-server-2014
ms.reviewer: ''
ms.technology: ssms
ms.topic: conceptual
helpviewer_keywords:
- Options [SQL Server Management Studio], dialog boxes
- Options [SQL Server Management Studio]
ms.assetid: b98d2631-530a-43b4-8491-f724f8e4a23a
author: stevestein
ms.author: sstein
ms.openlocfilehash: 0ca013ca12af9ee215fa5c3e4eefe7712ba02d68
ms.sourcegitcommit: 57f1d15c67113bbadd40861b886d6929aacd3467
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 06/18/2020
ms.locfileid: "84994824"
---
# <a name="options-dialog-boxes-f1-help"></a>選項對話方塊 F1 說明
  此章節包含在 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 中按一下 [工具] 功能表上的 [選項] 時，可用之對話方塊的 F1 說明。  
  
 [&#40;設計工具的選項-Analysis Services 設計工具-一般&#41;](options-designers-analysis-services-designers-general.md)  
  
 [選項 &#40;設計工具-維護計畫頁面&#41;](options-designers-maintenance-plans-page.md)  
  
 [&#40;設計工具的選項-資料表和資料庫設計工具頁面&#41;](options-designers-table-and-database-designers-page.md)  
  
 [選項 &#40;環境：字型和色彩頁面&#41;](options-environment-fonts-and-colors-page.md)  
  
 [選項 &#40;環境-一般頁面&#41;](../../integration-services/general-page-of-integration-services-designers-options.md)  
  
 [選項 &#40;環境-說明-一般頁面&#41;](options-environment-help-general-page.md)  
  
 [選項 &#40;環境-說明-線上頁面&#41;](options-environment-help-online-page.md)  
  
 [選項 &#40;環境-鍵盤頁面&#41;](options-environment-keyboard-page.md)  
  
 [&#40;查詢執行的選項-Analysis Server-一般頁面&#41;](../../database-engine/options-query-execution-analysis-server-general-page.md)  
  
 [&#40;查詢執行的選項： SQL Server： Advanced Page&#41;](../../database-engine/options-query-execution-sql-server-advanced-page.md)  
  
 [&#40;查詢執行的選項-SQL Server-ANSI 頁面&#41;](../../database-engine/options-query-execution-sql-server-ansi-page.md)  
  
 [&#40;查詢執行的選項-SQL Server-一般頁面&#41;](../../database-engine/options-query-execution-sql-server-general-page.md)  
  
 [&#40;查詢結果的選項-Analysis Server-一般頁面&#41;](../../database-engine/options-query-results-analysis-server-general-page.md)  
  
 [&#40;查詢結果的選項-SQL Server-一般頁面&#41;](../../database-engine/options-query-results-sql-server-general-page.md)  
  
 [&#40;查詢結果的選項-SQL Server 到格線的結果頁面&#41;](../../database-engine/options-query-results-sql-server-results-to-grid-page.md)  
  
 [&#40;查詢結果的選項-SQL Server 到文字的結果頁面&#41;](../../database-engine/options-query-results-sql-server-results-to-text-page.md)  
  
 [查詢結果和相依性服務頁面 &#40;的選項&#41;](../../database-engine/options-query-results-and-dependency-services-page.md)  
  
 [選項 &#40;文字編輯器-所有語言-一般頁面&#41;](../../database-engine/options-text-editor-all-languages-general-page.md)  
  
 [選項 &#40;文字編輯器-所有語言-索引標籤頁面&#41;](../../database-engine/options-text-editor-all-languages-tabs-page.md)  
  
 [選項 &#40;文字編輯器-副檔名頁面&#41;](../../database-engine/options-text-editor-file-extension-page.md)  
  
 [選項 &#40;文字編輯器-純文字-一般頁面&#41;](../../database-engine/options-text-editor-plain-text-general-page.md)  
  
 [選項 &#40;文字編輯器-純文字-定位點頁面&#41;](../../database-engine/options-text-editor-plain-text-tabs-page.md)  
  
 [&#40;文字編輯器的選項-XML-格式化頁面&#41;](../../database-engine/options-text-editor-xml-formatting-page.md)  
  
 [選項 &#40;文字編輯器-XML-一般頁面&#41;](../../database-engine/options-text-editor-xml-general-page.md)  
  
 [&#40;文字編輯器的選項-XML-其他頁面&#41;](../../database-engine/options-text-editor-xml-miscellaneous-page.md)  
  
 [選項 &#40;文字編輯器： XML：定位點頁面&#41;](../../database-engine/options-text-editor-xml-tabs-page.md)  
  
  
