...
# <a name="set-warning-thresholds"></a>設定警告臨界值
  使用此對話方塊可針對 [**資料庫鏡像監視器**] 對話方塊的導覽樹狀目錄中選取的資料庫，啟用及設定一個或多個警告臨界值。  
...
## <a name="options"></a>選項。  
 *伺服器實例及其連接狀態*  
 _系統_**\\**_INSTANCE_NAME_形式的夥伴伺服器實例名稱。 如果是預設伺服器執行個體，則只會顯示系統名稱。  
  
 這個欄位也會指出監視器目前是否已連接到這個伺服器執行個體。 可能的連接狀態如下：  
  
-   **未連接到**  *server_instance_name*  
  
-   **嘗試連接到**  *server_instance_name*  
 ...
 **連接**/**取消**  
 **[連接**/**取消**] 按鈕會與每個*伺服器實例和它的 [連接狀態*] 欄位相關聯。 這個按鈕的狀態是依據連接狀態而定：  
