 ...
 **主體記錄（** * \<時間>* **）**  
 主體伺服器實例上的記錄狀態，以伺服器實例的本地時間為限，以* \<時間>* 表示。 會顯示下列參數：  
 ...
 **鏡像記錄（** * \<時間>* **）**  
 鏡像伺服器實例上的記錄狀態，以伺服器實例的本地時間為限，以* \<時間>* 表示。 會顯示下列參數：  
...
## <a name="remarks"></a>備註  
 **dbm_monitor** 固定資料庫角色的成員可以使用「資料庫鏡像監視器」或 **sp_dbmmonitorresults** 預存程序，檢視現有的鏡像狀態。 但是這些使用者無法更新狀態資料表。 它們必須透過 [資料庫鏡像監視器作業]**** 來定期更新狀態資料表。 若要瞭解所顯示狀態的存在時間，使用者可以查看 [**主體記錄（***\<時間>***）** ] 和 [**鏡像記錄（***\<時間>***）** ] 標籤中的時間。  
