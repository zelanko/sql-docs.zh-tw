...
## <a name="failing-over-from-the-principal-to-the-mirror-database"></a>從主體資料庫容錯移轉至鏡像資料庫  
 下圖將說明當鏡像在具有自動容錯移轉的高安全性模式下執行時，記錄傳送與資料庫鏡像將如何搭配運作。 起初， **Server_A** 既是鏡像的主體伺服器，也是記錄傳送的主要伺服器。 **Server_B**是鏡像伺服器，而且也設定為目前非使用中的主伺服器。 **Server_C**和**Server_D**是記錄傳送的次要伺服器。 為盡可能提高記錄傳送工作階段的可用性，備份位置位於個別主機電腦的共用目錄上。  
