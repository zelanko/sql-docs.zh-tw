...
|**夥伴 (連接為)**|資料庫的容錯移轉夥伴名稱。 **主控台使用者的 Windows 驗證**或**登入 ' ***\<login name>*** ' 的 SQL Server 驗證**會顯示在括弧內。 如果之前已加入這個執行個體，則這是目前使用的驗證資訊；如果尚未將這個執行個體加入至監視器，則這是將要使用的驗證資訊。|  
