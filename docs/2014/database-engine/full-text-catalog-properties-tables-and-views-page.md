---
title: 全文檢索目錄屬性 （資料表和檢視頁面） |Microsoft 文件
ms.custom: ''
ms.date: 03/08/2017
...
|**斷詞工具的語言**|顯示斷詞工具的語言。|  
|**資料類型資料行**|列出資料行文件類型中所列的資料行的資料表中的資料行的名稱**可用的資料行**如果資料行是`varbinary(max)`或`image`資料行。|  
...
|**手動**|當修改、 加入或刪除索引資料中資料[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]追蹤變更。 **[手動]** 變更追蹤生效時，索引不會自動更新這些變更。 相反地，系統管理員可以手動套用變更使用[ALTER FULLTEXT INDEX...START UPDATE POPULATION](/sql/t-sql/statements/alter-fulltext-index-transact-sql)陳述式。|  
