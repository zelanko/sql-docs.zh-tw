...
-   以滑鼠右鍵按一下工作階段名稱，然後按一下 **[監看即時資料]** ，開始顯示追蹤資料。  
     ...
     若要在追蹤視窗中加入其他資料行，請按一下 [擴充事件] 工具列上的 **[選擇資料行]** 按鈕。 
  **[詳細資料]** 索引標籤會顯示選定事件的所有事件詳細資料。  
...
### <a name="group-results"></a>將結果分組  
 分組的結果就相當於 `GROUP BY` 中 [!INCLUDE[tsql](../includes/tsql-md.md)] 子句的功能。 目標資料表將顯示分成一組的資料，讓您展開或摺疊資料。  
