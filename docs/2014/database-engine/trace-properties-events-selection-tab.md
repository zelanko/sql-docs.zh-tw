...
## <a name="options"></a>選項。  
 **事件**資料行  
 選取或清除事件資料行中的核取方塊，來指定追蹤的事件。 **事件**會依事件類別目錄進行組織。 範本中所指定的事件類別會自動選取。 如需詳細資訊，請參閱 [SQL Server Event Class Reference](../relational-databases/event-classes/sql-server-event-class-reference.md)。  
