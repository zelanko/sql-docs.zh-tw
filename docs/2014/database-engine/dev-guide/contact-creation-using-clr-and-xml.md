---
title: 使用 CLR 及 XML 建立連絡人 |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
3.  在 c:\MySample 中，建立 `Contacts.vb` (適用於 Visual Basic 範例) 或 `Contacts.cs` (適用於 C# 範例) 並將適當的 Visual Basic 或 C# 範例程式碼 (下面) 複製到檔案中。  
...
10. 複製[!INCLUDE[tsql](../../includes/tsql-md.md)]測試命令指令碼至檔案，並將它儲存成`test.sql`範例目錄中。  
...
```  
 ...
 下列 [!INCLUDE[tsql](../../includes/tsql-md.md)] 會從資料庫中移除組件、預存程序和插入的資料。  
