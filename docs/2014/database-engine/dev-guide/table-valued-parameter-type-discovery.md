...
# <a name="table-valued-parameter-type-discovery"></a>資料表值參數類型探索
 ...
 ICommandWithParameters::GetParameterInfo 可針對大多數的參數類型來支援程序參數的類型資訊。 從開始[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]，隨著使用者定義型別和`xml`資料型別的引進，GetParameterInfo 方法並不足以達成此目的，因為無法透過 ICommandWithParameters 提供使用者定義型別資訊（名稱、架構和目錄）。 因此定義了新的介面 ISSCommandWithParameters 來提供擴充類型資訊。  
