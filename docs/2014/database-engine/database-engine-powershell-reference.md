...
|描述|指令程式|  
|-----------------|------------|  
|執行 TRANSACT-SQL 和 XQuery 指令碼，例如使用執行的指令碼`sqlcmd`公用程式。|[Invoke-Sqlcmd Cmdlet](../../2014/database-engine/invoke-sqlcmd-cmdlet.md)|  
...
### <a name="information-about-other-cmdlets"></a>其他 Cmdlet 的詳細資訊  
 `Encode-Sqlname`和`Decode-Sqlname`cmdlet 可協助您指定包含 PowerShell 路徑中不支援字元的 SQL Server 識別碼。 如需詳細資訊，請參閱 [SQL Server Identifiers in PowerShell](../powershell/sql-server-identifiers-in-powershell.md)。  
