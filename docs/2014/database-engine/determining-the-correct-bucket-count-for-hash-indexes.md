---
title: 判斷雜湊索引的正確值區計數 |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
# <a name="determining-the-correct-bucket-count-for-hash-indexes"></a>判斷雜湊索引的正確值區計數
 ...
 如需非叢集雜湊索引的詳細資訊，請參閱[雜湊索引](hash-indexes.md)和[使用記憶體最佳化資料表的索引指導方針](../relational-databases/in-memory-oltp/memory-optimized-tables.md)。  
  
 針對記憶體最佳化的資料表上的每個雜湊索引配置一個雜湊表。 配置的指定索引的雜湊表大小`BUCKET_COUNT`中的參數[CREATE TABLE &#40;TRANSACT-SQL&#41; ](/sql/t-sql/statements/create-table-transact-sql)或[CREATE TYPE &#40;TRANSACT-SQL&#41; ](/sql/t-sql/statements/create-type-transact-sql). 值區計數會於內部四捨五入進位至下一個二的乘冪。 例如，指定值區計數 300,000 將產生實際值區計數 524,288。  
...
## <a name="recommendations"></a>建議  
 ...
 您不一定能夠預測某個特定索引鍵可能擁有或將會擁有多少個值。 效能應該是可接受如果`BUCKET_COUNT`值是在 5 次的索引鍵值實際數目。  
...
## <a name="troubleshooting-the-bucket-count"></a>對值區計數進行疑難排解  
 若要疑難排解記憶體最佳化資料表中的值區計數問題，請使用[sys.dm_db_xtp_hash_index_stats &#40;TRANSACT-SQL&#41; ](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-xtp-hash-index-stats-transact-sql)取得關於空白貯體和資料列鏈結的長度的統計資料。 下列查詢可用來取得目前資料庫中所有雜湊索引的相關統計資料。 如果資料庫中有大型資料表，則查詢可能需要幾分鐘才能完成。  
 ...
 *avg_chain_length*  
 *avg_chain_length*指出雜湊值區中的資料列鏈結的平均長度。  
...
-   主索引鍵索引 (PK__SalesOrder…)：有 36% 的貯體是空的，這是理想的狀況。 另外，平均鏈結長度為 1，這也很理想。 不需要變更。  
 ...
 如需有關記憶體最佳化雜湊索引的問題疑難排解的詳細資訊，請參閱[常見效能問題疑難排解記憶體最佳化雜湊索引](../../2014/database-engine/troubleshooting-common-performance-problems-with-memory-optimized-hash-indexes.md)。  
...
-   值區計數越小，指派給單一貯體的值越多。 這樣會降低點查閱和插入的效能，因為[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]可能需要周遊單一貯體，以尋找搜尋述詞所指定的值中的數個值。  
...
### <a name="big-tables"></a>大型資料表  
 對於大型資料表而言，記憶體使用量可能成為問題。 例如，具有 250 萬個資料列的資料表具有 4 個雜湊索引，每個值區計數為十億，雜湊表的負擔是 4 個索引 * 1 10 億個貯體\*8 位元組 = 32 gb 的記憶體使用量。 如果針對每個索引選擇的值區計數為 2500 萬，則雜湊表的總負擔將會是 8 GB。 請注意，這是 8 位元組的記憶體使用量除了每個索引加入每個個別的資料列，也就是在此案例中的 8 gb (4 個索引\*8 個位元組\*250 萬列)。  
