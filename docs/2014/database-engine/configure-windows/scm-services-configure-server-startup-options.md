...
5.  重新啟動 [!INCLUDE[ssDE](../../includes/ssde-md.md)]。  
    ...
    > [!WARNING]  
    >  使用單一使用者模式啟動參數 方塊中完畢之後，請選取`-m`中的參數**現有參數**方塊，然後再按一下**移除**。 重新啟動 [!INCLUDE[ssDE](../../includes/ssde-md.md)]，將 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 還原為一般多使用者模式。  
