 ...
 **執行超時**  
 在 **[新增連接]** 對話方塊中設定預設值。 使用此微調方塊，指出 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 取消查詢之前要等候的秒數。 值為0表示無限期等候，或沒有超時。新安裝上的此值為0。  
  
 **批次分隔符號**  
 輸入用來將 [!INCLUDE[tsql](../includes/tsql-md.md)] 陳述式分隔成批次的文字。 預設為 GO。  
  
 **預設會以 SQLCMD 模式開啟新查詢**  
 選取此核取方塊，即可以 SQLCMD 模式開啟新查詢。 如需 SQLCMD 模式的詳細資訊，請參閱[使用查詢編輯器編輯 SQLCMD 腳本](../relational-databases/scripting/edit-sqlcmd-scripts-with-query-editor.md)。  
