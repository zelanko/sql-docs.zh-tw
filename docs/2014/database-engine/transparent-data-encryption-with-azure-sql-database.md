...
# <a name="transparent-data-encryption-with-azure-sql-database"></a>Azure SQL Database 的透明資料加密
 ...
 [!INCLUDE[ssSDSfull](../includes/sssdsfull-md.md)] 不支援 Azure 金鑰保存庫與 TDE 整合。 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 可以使用金鑰保存庫中的非對稱金鑰。 如需詳細資訊, [請參閱範例 A:使用 Key Vault](../relational-databases/security/encryption/extensible-key-management-using-azure-key-vault-sql-server.md#ExampleA)中的非對稱金鑰來透明資料加密。  
...
8.  在 [資料加密預覽] 刀鋒視窗中，將 [資料加密] 按鈕移至 [開啟]，然後按一下 [儲存] (位於頁面頂端)，以套用此設定。 [加密狀態] 會顯示透明資料加密的概略進度。  
     ...
     您也可以使用查詢工具 (例如 [!INCLUDE[ssSDS](../includes/sssds-md.md)] )，以具有 [!INCLUDE[ssManStudioFull](../includes/ssmanstudiofull-md.md)] VIEW DATABASE STATE **權限的資料庫使用者身分，連接到** 監視加密進度。 查詢`encryption_state` [sys.databases _database_encryption_keys](/sql/relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql)視圖的資料行。  
...
3.  若[!INCLUDE[ssSDS](../includes/sssds-md.md)]要監視的加密進度, 具有**VIEW database STATE**許可權的資料庫使用者可以查詢`encryption_state` [sys.databases _database_encryption_keys](/sql/relational-databases/system-dynamic-management-views/sys-dm-database-encryption-keys-transact-sql)視圖的資料行。  
...
##  <a name="Working"></a>使用上的 TDE 受保護資料庫[!INCLUDE[ssSDS](../includes/sssds-md.md)]  
