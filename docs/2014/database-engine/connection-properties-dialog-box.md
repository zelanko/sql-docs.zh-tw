...
## <a name="authentication"></a>驗證  
 檢視目前連接的驗證屬性。 驗證屬性是建立連接時的登入和驗證方法。 若要變更驗證屬性，請從 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]中斷連接，然後使用想要的連接選項，再次將物件總管連接到伺服器。  
