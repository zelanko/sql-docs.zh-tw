 ...
 **如果要執行 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 功能探索報告：**  
 ...
 您也可以從命令列產生探索報告。 執行"Setup.exe /Action = RunDiscovery"的命令提示字元，如果您將加入"/ q"上述命令列將不會顯示任何 UI，但仍會在 %programfiles%中建立報表\\[!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]\120\Setup Bootstrap\Log\\< 最後一個安裝程式工作階段\>。  
