...
```  
 ...
 PowerShell 環境不需要所有的 **sqlcmd** 參數。 例如，Windows PowerShell 會從 Cmdlet 格式化所有輸出，所以 **Invoke-Sqlcmd** 中不會實作指定格式化選項的 **sqlcmd**參數。 下表顯示**Invoke-sqlcmd**參數和**Sqlcmd**選項之間的關聯性：  
