 ...
 **伺服器執行個體**  
 ...
 如果 **[可用性複本]** 方格沒有列出您用來裝載次要複本的伺服器執行個體，按一下 **[加入複本]** 按鈕。 如果您在混合式 IT 環境設定可用性群組 (請參閱 [SQL Server 在 Windows Azure 虛擬機器的高可用性和災害復原](http://msdn.microsoft.com/library/windowsazure/jj870962.aspx))，您可以按一下 [加入 Azure 複本] 按鈕，以在 Windows Azure 中建立包含次要複本的虛擬機器。  
