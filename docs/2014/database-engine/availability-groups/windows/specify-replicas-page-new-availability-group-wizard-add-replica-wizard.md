---
title: 指定複本頁面 (新增可用性群組精靈：加入複本精靈） |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
> [!IMPORTANT]  
>  不會強制執行備份喜好設定。 這個喜好設定的解譯取決於您在給定可用性群組之資料庫的備份作業中所編寫的邏輯 (如果有的話)。 如需詳細資訊，請參閱[作用中次要複本：在次要複本 （AlwaysOn 可用性群組） 上備份](active-secondaries-backup-on-secondary-replicas-always-on-availability-groups.md)。  
