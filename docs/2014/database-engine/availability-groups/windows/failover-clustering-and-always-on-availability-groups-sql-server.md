...
|**使用 WSFC 叢集**|是|是|  
|**保護等級**|執行個體|資料庫|  
|**儲存體類型**|Shared|非共用<br /><br /> 請注意，雖然可用性群組中的複本不會共用儲存體，但是 FCI 裝載的複本會使用該 FCI 所需的共用儲存方案。 只有 FCI 內的節點共用儲存方案，可用性群組的複本之間不共用儲存方案。|  
...
##  <a name="restrictions-on-using-the-wsfc-failover-cluster-manager-with-availability-groups"></a><a name="FCMrestrictions"></a>搭配可用性群組使用 WSFC 容錯移轉叢集管理員的限制  
