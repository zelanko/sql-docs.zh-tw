---
title: 建立擴充的事件工作階段使用精靈 (在 [物件總管] 中) |Microsoft Docs
ms.custom: ''
ms.date: 06/13/2017
...
-   [建立事件工作階段](#BKMK_CreateEventSession)  
  
##  <a name="BKMK_Welcome"></a> 簡介  
 在 [簡介] 頁面上，執行下列動作：  
...
##  <a name="BKMK_SetSessionProperties"></a> 設定工作階段屬性  
...
##  <a name="BKMK_SelectEventsToCapture"></a> 選取要擷取的事件  
 在 [選取要擷取的事件] 頁面上，執行下列動作：  
  
1.  從 [事件程式庫] 選取您想要擷取的事件，然後按一下向右箭號。 您可以使用 Shift+按一下滑鼠左鍵或 Ctrl+按一下滑鼠左鍵，在 [事件程式庫] 中選取多個事件。  
  
     您可以從下拉式清單方塊中選取您想要搜尋事件的方式。 例如，您可以搜尋事件名稱或是事件名稱及其說明。 您可以在 [事件程式庫] 方塊中輸入您想要搜尋的文字，以搜尋資料表中的任何文字。 例如，如果您想要尋找 **lock_acquired** 事件，您可以輸入 **lock** 或 **lock acquire**。  
...
##  <a name="BKMK_CaptureGlobalFields"></a> 擷取全域欄位  
...
##  <a name="BKMK_SetSessionEventFilters"></a> 設定工作階段事件篩選  
...
##  <a name="BKMK_SpecifySessionDataOutput"></a> 指定工作階段資料存放區  
...
##  <a name="BKMK_CreateEventSession"></a> 建立事件工作階段  
