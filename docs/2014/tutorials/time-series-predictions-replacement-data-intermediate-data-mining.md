---
title: 時間序列預測使用取代資料 （中繼資料採礦教學課程） |Microsoft Docs
ms.custom: ''
ms.date: 04/27/2017
...
## <a name="performing-cross-prediction-with-a-time-series-model"></a>在時間序列模型上執行交叉預測  
 ...
 在 SQL Server 資料採礦，您必須執行交叉預測使用 REPLACE_MODEL_CASES 參數的引數的函式內[PredictTimeSeries &#40;DMX&#41;](/sql/dmx/predicttimeseries-dmx)。  
...
1.  在**方案總管**，以滑鼠右鍵按一下**採礦結構**，然後選取**New Mining Structure&lt**即可啟動資料採礦精靈。  
...
2.  在資料採礦精靈中，進行下列選擇：  
    ...
    -   針對**採礦結構名稱**，型別： `All Regions`  
  
    -   針對**採礦模型名稱**，型別： `All Regions`  
  
3.  處理新結構和新模型。  
  
#### <a name="to-build-the-prediction-query-and-map-the-replacement-data"></a>若要建立預測查詢及對應取代資料  
  
1.  如果模型尚未開啟，按兩下 AllRegions 結構，並在 [資料採礦設計師中，按一下**採礦模型預測**] 索引標籤。  
  
2.  在 [**採礦模型**] 窗格中，應該已選取 AllRegions 模型。 如果未選取，按一下**選取模型**，然後選取 AllRegions 模型。  
  
3.  在 **選取輸入資料表**窗格中，按一下**選取案例資料表**。  
  
4.  在 **選取表格**對話方塊中，變更資料來源為 T1000 Pacific Region，，，然後按一下**確定**。  
  
5.  以滑鼠右鍵按一下 採礦模型和輸入的資料之間的聯結線，然後選取**修改連接**。 將資料來源檢視中的資料對應至模型，如下所示：  
...
6.  現在，您將建立預測查詢。  
    ...
    1.  在方格中，按一下第一個空白資料列，底下**來源**，然後選取 AllRegions 採礦模型。  
  
    2.  針對**欄位**，選取區域。  
  
    3.  針對**別名**，型別**Model Used**。  
  
7.  接著在結果中加入另一個標籤，讓您得知預測是針對哪些數列。  
  
    1.  按一下空白的資料列，然後在**來源**，選取**自訂運算式**。  
  
    2.  在 **別名**資料行中輸入**ModelRegion**。  
  
    3.  在 **準則/引數**資料行中輸入`'T1000 Pacific'`。  
...
8.  現在，您將設定交叉預測函數。  
    ...
    5.  在 [**準則/引數**] 欄中，欄位名稱後面輸入下列文字： `,5, REPLACE_MODEL_CASES`  
