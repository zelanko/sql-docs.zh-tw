---
title: 建立預測的撥接中心模型 （中繼資料採礦教學課程） |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
6.  從 [採礦模型] 功能表中選取 [單一查詢]。  
     ...
     **單一查詢輸入** 對話方塊隨即出現，並對應至採礦模型中的資料行的資料行。  
  
7.  在**單一查詢輸入**對話方塊中，按 shift 鍵，資料列，然後選取*午夜*。  
  
8.  按一下 Lvl 2 Operators，與類型 資料列`6`。  
  
9. 在底部的下半部**採礦模型預測**索引標籤上，按一下方格中的第一個資料列。  
  
10. 在**來源**資料行中，按一下向下箭號，然後選取**預測函數**。 在**欄位**欄中，選取**PredictHistogram**。  
  
     您可以自動使用此預測函數的引數清單會出現在**準則/引數**方塊。  
  
11. 將 ServiceGrade 資料行中的資料行清單**採礦模型**窗格**準則/引數**方塊。  
  
     資料行的名稱會自動插入做為引數。 您可以選擇任何可預測的屬性資料行，將其拖曳至此文字方塊中。  
  
12. 按一下按鈕**切換到查詢結果檢視**，在預測查詢產生器上方角落。  
...
2.  在資料來源檢視精靈中，選取[!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)]當做資料來源，然後按一下**下一步**。  
  
3.  在**選取資料表和檢視**頁面上，按一下**下一步**但不選取任何資料表。  
  
4.  在**正在完成精靈**頁面上，輸入名稱， `Shifts`。  
  
     這個名稱會顯示在 [方案總管] 中，做為資料來源檢視的名稱。  
  
5.  以滑鼠右鍵按一下空的設計窗格，然後選取 **新增具名查詢**。  
  
6.  在**建立具名查詢**對話方塊中，針對**名稱**，型別`Shifts for Call Center`。  
...
8.  在 [設計] 窗格中，以滑鼠右鍵按一下資料表，Shifts for Call Center，然後選取**瀏覽資料**來預覽 T-SQL 查詢所傳回的資料。  
  
9. 以滑鼠右鍵按一下索引標籤上， **Shifts.dsv (Design)** ，然後按一下 **儲存**來儲存新的資料來源檢視定義。  
...
2.  在**採礦模型** 窗格中，按一下 **選取模型**，選擇 撥接中心-LR 從可用模型的清單。  
  
3.  從**採礦模型**功能表上，清除選項，**單一查詢**。 此時會出現一個警告，告知您將會失去單一查詢輸入。 按一下 [確定] 。  
  
     **單一查詢輸入**對話方塊取代**選取輸入資料表** 對話方塊。  
  
4.  按一下 [選取案例資料表] 。  
  
5.  在**選取資料表**對話方塊中，selectShifts 從資料來源的清單。 在**資料表/檢視表名稱**清單中，選取 排班為撥接中心 （它可能會自動選取），然後按一下**確定。**  
  
     **採礦模型預測**設計介面經過更新以顯示資料行中輸入的資料和模型中的名稱和資料類型會根據建立的對應。  
  
6.  其中一條聯結線，以滑鼠右鍵按一下，然後選取**修改連接**。  
...
1.  在下方方格中的一半**預測查詢產生器**，按一下下方的空白儲存格**來源，** ]，然後選取 [Shifts for Call Center。  
  
2.  在底下的空資料格**欄位**，選取 排班。  
  
3.  按一下方格中的下一個空行，然後重複上述的程序，為 [薪資類型] 加入另一個資料列。  
  
4.  在方格中，按下一個空行。 在**來源**欄中，選取**預測函數**。 在**欄位**欄中，選取**預測**。  
  
5.  將 ServiceGrade 資料行從**採礦模型**窗格向方格中，到**準則/引數**儲存格。 在**別名**欄位中，輸入**預測的服務等級**。  
  
6.  在方格中，按下一個空行。 在**來源**欄中，選取**預測函數**。 在**欄位**欄中，選取**PredictProbability**。  
  
7.  將 ServiceGrade 資料行從**採礦模型**窗格向方格中，到**準則/引數**儲存格。 在**別名**欄位中，輸入**機率**。  
...
-   [SQL Server 2005 資料採礦增益集用於 Office 2007](http://www.microsoft.com/sql/technologies/dm/addins.mspx)： 搜尋目標與假設狀況分析  
  
-   [SQL Server 2008 資料採礦增益集用於 Office 2007](http://go.microsoft.com/fwlink/?LinkID=117790)： 目標搜尋狀況分析、 假設狀況分析，以及預測計算器  
