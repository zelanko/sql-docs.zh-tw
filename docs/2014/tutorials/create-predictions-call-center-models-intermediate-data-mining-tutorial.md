---
title: 建立撥打電話中心模型的預測（元資料採礦教學課程） |Microsoft Docs
ms.custom: ''
ms.date: 03/06/2017
...
2.  在 [資料採礦設計師] 中，按一下 [**採礦模型預測**] 索引標籤。  
  
3.  在 [**採礦模型**] 窗格中，按一下 [**選取模型**]。  
  
4.  [**選取採礦模型**] 對話方塊會顯示一份採礦結構清單。 展開採礦結構，檢視與該結構相關聯之採礦模型的清單。  
  
5.  展開採礦結構 [撥接中心預設值]，然後選取類神經網路模型 [撥接中心 – LR]。  
  
6.  從 [採礦模型]**** 功能表中選取 [單一查詢]****。  
  
     [**單一查詢輸入**] 對話方塊隨即出現，其中的資料行會對應到「採礦模型」中的資料行。  
  
7.  在 [**單一查詢輸入**] 對話方塊中，按一下 [排班] 的資料列，然後選取 [*午夜*]。  
  
8.  按一下 [Lvl 2] 運算子的資料列， `6`然後輸入。  
  
9. 在 [**採礦模型預測**] 索引標籤的下半部，按一下方格中的第一個資料列。  
  
10. 在 [**源**資料行] 中，按一下向下箭號，然後選取 [**預測函數**]。 在 [**欄位**] 資料行中，選取 [ **PredictHistogram**]。  
  
     在 [**準則/引數**] 方塊中，您可以搭配此預測函數使用的引數清單會自動出現。  
  
11. 將 [ServiceGrade] 資料行從 [**採礦模型**] 窗格中的資料行清單拖曳到 [**準則/引數**] 方塊中。  
  
     資料行的名稱會自動插入做為引數。 您可以選擇任何可預測的屬性資料行，將其拖曳至此文字方塊中。  
  
12. 在預測查詢產生器的右上角，按一下 [**切換到查詢結果**] 的按鈕。  
...
2.  在 [資料來源 View wizard] 中[!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)] ，選取做為資料來源，然後按 **[下一步]**。  
  
3.  在 [**選取資料表和視圖**] 頁面上，按 **[下一步]** 而不選取任何資料表。  
  
4.  在 [**正在完成嚮導]** 頁面上，輸入名稱`Shifts`。  
...
8.  在設計窗格中，以滑鼠右鍵按一下資料表，再按 [撥接中心]，然後選取 [**流覽資料**] 來預覽 t-SQL 查詢所傳回的資料。  
  
9. 以滑鼠右鍵按一下索引標籤，再按 [**移至 dsv （設計）]，** 然後按一下 [**儲存**] 以儲存新的資料來源視圖定義。  
...
1.  在 [資料採礦設計師] 中，按一下 [**採礦模型預測**] 索引標籤。  
  
2.  在 [**採礦模型**] 窗格中，按一下 [**選取模型**]，然後從可用的模型清單中選擇 [撥接中心-LR]。  
  
3.  從 [**採礦模型**] 功能表中，清除 [**單一查詢**] 選項。 此時會出現一個警告，告知您將會失去單一查詢輸入。 按一下 [確定]  。  
  
     [**單一查詢輸入**] 對話方塊會取代為 [**選取輸入資料表**] 對話方塊。  
  
4.  按一下 [選取案例資料表] ****。  
  
5.  在 [**選取資料表**] 對話方塊中，從資料來源清單中 selectShifts。 在 [**資料表/視圖名稱**] 清單中，選取 [撥接中心的轉移（可能會自動選取）]，然後按一下 **[確定]。**  
  
     [**採礦模型預測**] 設計介面會更新，以顯示根據輸入資料和模型中的資料行名稱和資料類型所建立的對應。  
...
7.  按一下 [LevelTwoOperators] 旁邊的空資料格，然後選取 [**撥接中心 AvgOperators**]。  
  
8.  按一下 [呼叫] 旁的空白資料格，選取 [ **AvgCalls] 的 [移位**]。 然後按一下 [確定]****。  
  
##### <a name="to-create-the-predictions-for-each-shift"></a>若要建立每個排班的預測  
  
1.  在**預測查詢**產生器下半部的方格中，按一下 [**來源**] 底下的空白資料格，然後選取 [撥接中心的轉移]。  
  
2.  在 [**欄位**] 底下的空白資料格中，選取 [Shift]。  
  
3.  按一下方格中的下一個空行，然後重複上述的程序，為 [薪資類型] 加入另一個資料列。  
  
4.  在方格中，按下一個空行。 在 [**源**資料行] 中，選取 [**預測函數**]。 在 [**欄位**] 資料行中，選取 [**預測**]。  
  
5.  將 [ServiceGrade] 資料行從 [**採礦模型**] 窗格向下拖曳到方格中，然後拖曳至 [**準則/引數**] 資料格中。 在 [**別名**] 欄位中，輸入**預測的服務等級**。  
  
6.  在方格中，按下一個空行。 在 [**源**資料行] 中，選取 [**預測函數**]。 在 [**欄位**] 資料行中，選取 [ **[predictprobability]**]。  
  
7.  將 [ServiceGrade] 資料行從 [**採礦模型**] 窗格向下拖曳到方格中，然後拖曳至 [**準則/引數**] 資料格中。 在 [**別名**] 欄位中 **，輸入機率**。  
  
8.  按一下 [**切換到查詢結果檢視**] 以查看預測。  
...
-   [SQL Server 2005 適用于 Office 2007 的資料採礦增益集](https://www.microsoft.com/sql/technologies/dm/addins.mspx)：目標搜尋和 What If 案例分析  
  
-   [SQL Server 2008 適用于 Office 2007 的資料採礦增益集](https://go.microsoft.com/fwlink/?LinkID=117790)：目標搜尋案例分析、What If 案例分析，以及預測計算器  
...
## <a name="see-also"></a>另請參閱  
 [羅吉斯回歸模型查詢範例](../../2014/analysis-services/data-mining/logistic-regression-model-query-examples.md)   
 [Microsoft 羅吉斯回歸演算法](../../2014/analysis-services/data-mining/microsoft-logistic-regression-algorithm.md)   
