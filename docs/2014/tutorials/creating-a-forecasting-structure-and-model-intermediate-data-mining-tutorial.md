---
title: 建立預測結構和模型 （中繼資料採礦教學課程） |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
4.  在**建立資料採礦結構**頁面的 **您想要使用哪一種資料採礦技術？**，選取**Microsoft 時間序列**，然後按一下  **下一步**。  
  
5.  在**選取資料來源檢視**頁面的 **可用的資料來源檢視**，選取**SalesByRegion**。  
  
6.  按 [下一步] 。  
  
7.  在**指定資料表類型**頁面上，確認中的核取方塊**案例**vTimeSeries 資料表的資料行已選取，然後按一下**下一步**。  
...
9. 選取核取方塊，在**輸入**和**可預測**數量、 資料行，然後按一下資料行**下一步**。  
  
     藉由選取**可預測**，代表要對此資料行中的資料建立預測。 不過，由於您要根據過去的資料進行預測，因此您必須加入資料行做為輸入內容。  
  
10. 在頁面上**指定資料行的內容和資料類型**，檢閱所做的選擇。  
  
     ModelRegion 資料行指定為**金鑰**資料行和 ReportingDate 資料行自動指定為**Key Time**資料行。 每一種類型的索引鍵，您只能有一個。  
  
11. 按 [下一步] 。  
  
12. 在**正在完成精靈** 頁面上，針對**採礦結構名稱**，型別`Forecasting`。  
...
13. 在**採礦模型名稱**，型別`Forecasting`，然後按一下 **完成**。  
     ...
     資料採礦設計師會開啟以顯示`Forecasting`您剛才建立的採礦結構。  
