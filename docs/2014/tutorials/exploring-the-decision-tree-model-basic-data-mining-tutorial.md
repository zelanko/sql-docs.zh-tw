---
title: 探索決策樹模型（基本資料採礦教學課程） |Microsoft Docs
ms.custom: ''
ms.date: 04/27/2017
...
##  <a name="decision-tree-tab"></a><a name="Decision_Tree_Tab"></a>決策樹索引標籤  
 ...
 當您在判定`TM_Decision_Tree`樹檢視器中查看模型時，您可以在圖表的左側看到最重要的屬性。 「最重要」表示這些屬性對結果的影響最大。 沿著樹狀結構 (位於圖表右側) 愈往下的屬性其影響愈小。  
...
1.  按一下`Bike Buyer`節點以識別其相依性。  
     ...
     相依性網路`Bike Buyer`的中央節點表示在採礦模型中的可預測屬性。 圖形會反白顯示對可預測屬性有影響的任何相連節點。  
