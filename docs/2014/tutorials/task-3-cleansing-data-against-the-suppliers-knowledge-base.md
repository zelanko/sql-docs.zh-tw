...
2.  將清理程序完成時，檢閱**統計資料**中**Profiler**  索引標籤。來源統計資料提供處理，找到為正確、 DQS 更正的記錄、 已變更 DQS 所建議的記錄數目和無效的記錄數目的數字的記錄數目的記錄的數目。 在右邊的清單方塊中，您可以看到清理程序中每一個相關定義域的更正值、建議值及完整性 (資料存在的程度) 和精確度 (資料可用於預定用途的程度) 等值。  
