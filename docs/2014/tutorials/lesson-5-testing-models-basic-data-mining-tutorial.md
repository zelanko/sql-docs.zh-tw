---
title: 第 5 課： 測試模型 （基本資料採礦教學課程） |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
# <a name="lesson-5-testing-models-basic-data-mining-tutorial"></a>第 5 課：測試模型 (基本資料採礦教學課程)
 ...
 由於測試集中的資料已經包含自行車購買的已知值，所以可以輕鬆地判斷模型的預測是否正確。 執行效能最好的模型將供[!INCLUDE[ssSampleDBCoFull](../includes/sssampledbcofull-md.md)]行銷部門，以識別其目標郵寄活動的客戶。  
  
 在這一課，您將使用多種方法來驗證模型：  
  
1.  您要針對測試集進行預測，以查看模型對於已知結果有多精確。 您將使用*增益圖*測量模型的有效性。  
...
2.  您要針對篩選的資料子集測試模型。 您可以在同一個增益圖中比較多個模型。  
 ...
 如需有關如何建立的模型驗證一般情況下，請參閱[資料採礦概念](../../2014/analysis-services/data-mining/data-mining-concepts.md)。  
