 ...
 **模型**索引標籤[!INCLUDE[msCoName](../includes/msconame-md.md)]資料採礦設計師中的時間序列檢視器可讓您檢視以樹狀圖形表單中的預測模型。  
 ...
 從樹狀檢視的模型中可學到什麼？ 例如，讓我們來比較兩個模型，可在樹狀目錄中的數個層級，另一個有單一節點。  
...
-   當時間序列的樹狀圖形有多個分支時，這表示偵測到的時間序列太複雜，無法以單一方程式表示。 相反地，樹狀圖形中可以包含多個分支，加上造成樹狀結構的每個分支*分割*。 當樹狀目錄分割時，每個分支表示不同的時間區段，其中的趨勢可用單一方程式描述。  
     ...
     例如，如果您查看圖表圖形，並看到銷售量從 9 月某個時間，一直到年底假期中，您可以切換到**模型**檢視來查看確切的日期變更趨勢的地方。 在樹狀目錄中代表 「 9 月前 」 和 「 9 月後 」 的分支會包含不同的公式： 一個公式以數學方式描述分割前的銷售趨勢和另一個公式則描述 9 月到銷售趨勢年底假期。  
  
#### <a name="to-explore-the-decision-tree-for-a-time-series-model"></a>瀏覽時間序列模型的決策樹  
  
1.  在 [**樹狀**清單**模型**] 索引標籤的檢視器中，選取**T1000 Europe:數量**系列。  
...
3.  現在選取另一個要檢視的時間序列。 使用**樹狀**上列出**模型** 索引標籤的檢視器中，選取**M200 North America:數量**系列。  
...
-   **一般內容樹狀檢視器**:可讓您瀏覽及檢視所使用的資料數列的所有模型中一次，如果您建立了混合模型，這兩個 ARIMA 和 ARTXP 樹狀結構會顯示在相同的圖形。  
