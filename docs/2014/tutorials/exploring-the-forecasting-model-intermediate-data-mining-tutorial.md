---
title: 探索預測模型 （中繼資料採礦教學課程） |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
ms.locfileid: "36313036"
---
# <a name="exploring-the-forecasting-model-intermediate-data-mining-tutorial"></a>探索預測模型 (中繼資料採礦教學課程)
  既然您已建立預測的採礦模型，您可以瀏覽結果使用**採礦模型檢視器**資料採礦設計師索引標籤。 [!INCLUDE[msCoName](../includes/msconame-md.md)]時間序列檢視器包含兩個索引標籤：**圖表**和**模型**。  
...
1.  請嘗試變更的值**預測步驟**逐漸增加從**5**至**10**，然後再設回**6**。  
...
3.  請注意 X 軸的小數位數。 歷程記錄和預測資料的變更永遠是以百分比表示，但實際值會自動調整，以便將所有值放在圖形上。 因此，在比較模型時需要特別小心，不要只依賴視覺項目。 若要取得精確值或百分比增量和值的預測，將滑鼠暫時放在點線或實線，或按一下線條以檢視中的值**採礦圖例**。  
  
     **提示**： 如果**採礦圖例**不可見，切換至**模型**檢視，以滑鼠右鍵按一下任何節點，然後選取**顯示圖例**。  
 ...
 **模型** 索引標籤[!INCLUDE[msCoName](../includes/msconame-md.md)]資料採礦設計師中的時間序列檢視器可讓您檢視以樹狀圖形表單中的預測模型。  
 ...
 您可以選擇哪一種產品線、 地區和銷售相關數據您想要檢視所選取序列組合**樹狀**下拉式清單上的**模型** 索引標籤。  
  
 從樹狀檢視的模型中可學到什麼？ 舉例來說，假設要比較兩個模型，其中一個有數個樹狀層級，另一個有單一節點。  
  
-   當樹狀圖形包含單一節點時，這表示模型中的趨勢在一段時間後通常是同質性的。 您可以使用這個單一節點，標示為**所有**，若要檢視描述輸入的變數與結果之間的關聯性的公式。  
  
-   當時間序列的樹狀圖形有多個分支時，這表示偵測到的時間序列太複雜，無法以單一方程式表示。 相反地，樹狀圖形可能包含多個分支，每個分支以造成樹狀結構之條件標示*分割*。 當樹狀目錄分割時，每個分支表示不同的時間區段，其中的趨勢可用單一方程式描述。  
  
     例如，如果您查看圖表圖形，看到銷售量從 9 月某個時間並繼續進行到年底假期中，您可以切換到**模型**檢視來查看趨勢變更的確切日期。 樹狀目錄中代表「9 月前」和「9 月後」的分支會包含不同的公式：一個公式以數學方式描述分割前的銷售趨勢，另一個公式則描述 9 月到年底假期的銷售趨勢。  
  
#### <a name="to-explore-the-decision-tree-for-a-time-series-model"></a>瀏覽時間序列模型的決策樹  
  
1.  中**樹狀結構**清單上**模型** 索引標籤的檢視器中，選取**T1000 Europe: Amount**數列。  
  
     按一下節點標示為**所有**。  
  
     如**所有**出現的工具提示的節點包含的案例數目等資訊在完整的系列，以及時間序列方程式衍生自分析的資料。  
  
2.  如果**採礦圖例**不可見，以滑鼠右鍵按一下節點並選取**顯示圖例**。  
  
     **採礦圖例**提供的資訊相同的工具提示中。 如果有任何離散的獨立變數，您也會看到顯示變數分佈在節點中的長條圖。  
  
3.  現在選取另一個要檢視的時間序列。 使用**樹狀結構**清單**模型** 索引標籤的檢視器中，選取**M200 North America: Amount**數列。  
  
     現在包含樹狀圖形**所有**節點和兩個子節點。 您可以透過查看子節點上的標籤，了解趨勢線在哪個時間點變更。  
  
     對於每個子節點，在描述**採礦圖例**也包含樹狀結構的每個分支中的案例計數。  
  
 下列清單說明樹狀檢視器的一些其他功能：  
  
-   您可以變更由圖表中使用變數**背景**控制項。 根據預設，會較暗的節點包含更多的案例，因為值**背景**設**母體擴展**。 若要查看有多少案例只會在節點中，將滑鼠暫時放在某個節點上方，並檢視工具提示隨即出現，或按一下節點並檢視中的數字**節點圖例**視窗。  
...
##  <a name="bkmk_Content"></a> （選擇性）一般內容樹狀檢視器  
 除了時間序列的自訂檢視器[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]提供**MicrosoftGeneric 內容樹狀檢視器**用於所有資料採礦模型。 此檢視器具備一些優點：  
  
-   **Microsoft 時間序列檢視器**： 此檢視合併兩個演算法的結果。 雖然您可以分別檢視每個序列，但無法判斷各演算法結果如何結合。 此外，在此檢視中，工具提示和採礦圖例只顯示最重要的統計資料。  
  
-   **一般內容樹狀檢視器**： 可讓您瀏覽及檢視所使用的資料數列的所有模型中一次，如果您已經建立了混合模型，這兩個 ARIMA 和 ARTXP 樹狀結構會顯示在相同的圖表。  
...
1.  在**採礦模型檢視器**索引標籤上，選取**Microsoft 一般內容樹狀檢視器**從**檢視器**下拉式清單。  
  
2.  在**節點標題** 窗格中，按一下最頂端的 (All) 節點。  
  
3.  在**節點詳細資料** 窗格中，檢視 attribute_name 的值。  
  
     這個值顯示這個節點包含哪一個序列或產品與區域組合。 在 AdventureWorks 範例中，最頂端的節點屬於 M200 Europe 序列。  
  
4.  在**節點標題** 窗格中，找出第一個子節點的節點。  
  
     如果一個序列節點包含子系，樹狀 檢視上顯示的**模型**Microsoft 時間序列檢視器 索引標籤也會包含分支結構。  
  
5.  展開該節點，並按一下其中一個子節點。  
  
     結構描述的 NODE_DESCRIPTION 資料行包含造成樹狀結構分岔的條件。  
  
6.  在**節點標題** 窗格中，按一下最頂端的 ARIMA 節點，然後展開節點，直到所有子節點都會顯示。  
