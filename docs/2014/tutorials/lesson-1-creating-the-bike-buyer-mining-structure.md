...
```  
 ...
 您可以使用離散化函數，在\<[採礦結構資料行]>，使用下列語法來離散化連續資料行：  
...
2.  在 [**連接到伺服器**] 對話方塊的 [**伺服器類型**] 中，選取 [ **Analysis Services**]。 在 [**伺服器名稱**] `LocalHost`中，輸入，或輸入您想要[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]在此課程中連接的實例名稱。 按一下 [ **連接**]。  
  
3.  在**物件總管**中，以滑鼠右鍵按一下的[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]實例，指向 [追加**查詢**]，然後按一下 [ **DMX** ] 以開啟 [**查詢編輯器**] 和新的空白查詢。  
...
7.  在 [**另存**新檔] 對話方塊中，流覽至適當的資料夾，並`Bike Buyer Structure.dmx`將檔案命名為。  
