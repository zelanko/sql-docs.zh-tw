---
title: 工作 14： 將執行 SQL 工作，為 MDS 執行預存程序的控制流程 |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
2.  拖放**執行 SQL 」 工作**從**SSIS 工具箱**至**控制流程** 索引標籤。  
  
3.  以滑鼠右鍵按一下**執行 SQL 」 工作**中**控制流程**索引標籤，然後按一下**重新命名**。 型別**觸發預存程序將資料載入 MDS**按**ENTER**。  
  
4.  連接**接收、 清理、 比對和鑑定供應商資料**至**觸發程序預存程序將資料載入**使用綠色連接器。  
  
     ![連接到執行 SQL 工作](../../2014/tutorials/media/et-addingesqltasktocftorunthespformds-01.jpg "連接到執行 SQL 工作")  
  
5.  使用**變數**視窗中，使用下列設定加入兩個新的變數。 如果您沒有看到**變數**視窗中，按一下  **SSIS**功能表列上按一下**變數**。  
...
6.  按兩下**觸發將資料載入 MDS 的預存程序**。  
  
7.  在**執行 SQL 工作編輯器**對話方塊中，選取 **（本機）。MDS** (或**localhost。MDS**) 的**連接**。  
  
8.  型別**EXEC [stg]。 [udp_Supplier_Leaf]？，？，？** 如**SQL 陳述式**。 您可以使用 SQL Server Management Studio 來驗證此名稱。  
...
11. 選取**user:: versionname**從下拉式清單中，為**變數名稱**。  
  
12. 選取**NVARCHAR**如**資料型別**。  
...
15. 按一下**確定**關閉**執行 SQL 編輯器** 對話方塊。  
  
## <a name="next-step"></a>下一個步驟  
 [工作 15： 建置和執行 SSIS 專案](../../2014/tutorials/task-15-building-and-running-the-ssis-project.md)  
