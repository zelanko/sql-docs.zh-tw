---
title: 瀏覽貝氏機率分類模型 （基本資料採礦教學課程） |Microsoft 文件
ms.custom: ''
ms.date: 04/27/2017
...
1.  使用**採礦模型**清單頂端的**採礦模型檢視器**索引標籤，以切換至`TM_NaiveBayes`模型。  
  
2.  使用**檢視器**清單可切換到**Microsoft 貝氏機率分類檢視器**。  
  
3.  按一下`Bike Buyer`節點識別其相依性。  
...
5.  按兩下**Number Cars Owned** bike buyer 資料格中 （資料行標記為 1） 資料行。  
  
     **採礦圖例**顯示更詳細的檢視。  
  
  
##  <a name="AttributeCharacteristics"></a> 屬性特性  
 與**屬性特性**索引標籤上，您可以選取屬性和值來查看其他屬性的值會出現在選定的值案例的頻率。  
...
##  <a name="AttributeDiscrimination"></a> 屬性辨識  
 與**屬性辨識**索引標籤上，您可以調查自行車購買的兩個離散值與其他屬性值之間的關聯性。 因為`TM_NaiveBayes`模型有兩個狀態，1 和 0，您不必進行任何變更，向檢視器。  
