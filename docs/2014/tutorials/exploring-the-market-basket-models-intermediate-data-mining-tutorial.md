---
title: 探索購物籃模型 （中繼資料採礦教學課程） |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
# <a name="exploring-the-market-basket-models-intermediate-data-mining-tutorial"></a>探索購物籃模型 (中繼資料採礦教學課程)
  既然您已建立`Association`模型中，您可以瀏覽它使用[!INCLUDE[msCoName](../includes/msconame-md.md)]關聯檢視器中**採礦模型檢視器**資料採礦設計師索引標籤。 此教學課程會逐步引導您使用檢視器來探索項目之間的關聯性。 此檢視器可幫助您快速地查看哪些產品經常一起出現，並大概了解新興的模式。  
...
-   [項目集索引標籤](#bkmk_Itemsets)  
  
-   [規則索引標籤](#bkmk_Rules)  
  
-   [一般內容檢視](#bkmk_ContentViewer)  
  
 此教學課程中，您將上啟動**相依性網路**索引標籤，然後再使用**規則** 索引標籤和**項目集** 索引標籤，加深您了解關聯性顯示在檢視器中。 您也會使用**Microsoft 一般內容樹狀檢視器**來擷取個別規則或項目集的詳細統計資料。  
  
##  <a name="bkmk_DepNet"></a> 相依性網路索引標籤  
 與**相依性網路**索引標籤上，您可以調查的模型中的不同項目互動。 檢視器中的每一個節點都代表某個項目，而節點之間的線條則代表規則。 您可以透過選取某個節點，查看其他哪些節點會預測選取的項目，或目前的項目預測出哪些項目。 在某些情況下，項目之間會存在雙向關聯，表示它們通常會出現在同一項交易中。 您可以參考此索引標籤底部的色彩圖例來判斷關聯的方向。  
...
8.  選取**只顯示屬性名稱**的相依性網路 窗格頂端的下拉式清單。  
  
     圖表中的文字標籤會更新，只顯示模型名稱。  
  
 [回到頁首](#bkmk_DepNet)  
  
##  <a name="bkmk_Itemsets"></a> 項目集索引標籤  
 接下來，您將會深入了解此模型針對 Touring Tire 和 Touring Tire Tube 產品所產生的規則與項目集。 **項目集** 索引標籤會顯示三個重要資訊與相關項目集的[!INCLUDE[msCoName](../includes/msconame-md.md)]關聯演算法探索：  
...
-   **項目：** 的每個項目集內包含的項目清單。  
 ...
 演算法可能會產生許多項目集，這會因演算法參數的設定方式而不同。 檢視器中傳回的每個項目集都代表已賣出此項目的交易。 在頂端使用的控制項**項目集**索引標籤上，您可以篩選在檢視器顯示僅包含項目集指定的最小支援和項目集大小。  
...
2.  在**篩選項目集**方塊中，輸入`Touring Tire`，然後在方塊之外按一下。  
...
5.  清除**顯示完整名稱**核取方塊。  
 ...
 中的值**支援**資料行表示針對每個項目集的交易數目。 項目集的交易表示包含此項目集內所有項目的購買。  
...
1.  在項目集清單中，按一下 **項目集**資料行標題，依名稱排序。  
  
2.  找到的項目， `Touring Tire` （與任何第二個項目）。  
  
3.  以滑鼠右鍵按一下項目， `Touring Tire`，選取**鑽研**，然後選取**模型和結構資料行**。  
  
     **鑽研** 對話方塊隨即顯示當做這個項目集支援個別交易。  
  
4.  展開巢狀資料表 vAssocSeqLineItems，檢視交易中的實際購買清單。  
  
#### <a name="to-filter-itemsets-by-support-or-size"></a>若要依照支援或大小篩選項目集  
  
1.  清除可能在任何文字**篩選項目集**方塊。 您無法將文字篩選與數值篩選一起使用。  
  
2.  在**最小支援**方塊中輸入 100，然後再按一下 檢視器的背景。  
...
-   **規則：** 規則定義。 若為購物籃模型，規則會描述特定的項目組合。  
 ...
 每項規則都可用來根據其他項目的出現情況，預測這個項目在交易中的出現狀況。 就像在**項目集**索引標籤上，您可以篩選規則，以顯示最有趣的規則。 如果您正在使用沒有任何規則的採礦模型，您可能會想要變更演算法參數，以降低規則的機率臨界值。  
...
2.  在**篩選規則**方塊中，輸入`Mountain-200`。  
  
     清除**顯示完整名稱**核取方塊。  
  
3.  從**顯示**清單中，選取**只顯示屬性名稱**。  
  
     檢視器就會顯示含有文字的規則"`Mountain-200`"。 規則的機率會告訴您可能就是，當有`Mountain-200`自行車，該人員同時購買其他列出的產品。  
...
2.  選取**鑽研**，然後選取**僅模型資料行**，或**模型和結構資料行**。  
     ...
     **鑽研**對話方塊提供的規則，在頂端的窗格中，以及一份用做為規則支援資料的所有案例的摘要。  
  
 [回到頁首](#bkmk_DepNet)  
  
##  <a name="bkmk_ContentViewer"></a> 一般內容樹狀檢視器  
 此檢視器可用於所有模型，不論演算法或模型類型為何。 **Microsoft 一般內容樹狀檢視器**可從**檢視器**下拉式清單。  
...
1.  在**採礦模型檢視器**索引標籤上，選取**Microsoft 一般內容樹狀檢視器**從**檢視器**清單。  
...
3.  在 [節點詳細資料] 窗格中，檢閱 NODE_TYPE 和 NODE_DESCRIPTION 的值。  
 ...
 您也可以建立內容查詢，以取得有關規則的詳細統計資料。 如需有關採礦模型內容以及如何解譯它的詳細資訊，請參閱[關聯模型的採礦模型內容&#40;Analysis Services-資料採礦&#41;](../../2014/analysis-services/data-mining/mining-model-content-for-association-models-analysis-services-data-mining.md)。  
