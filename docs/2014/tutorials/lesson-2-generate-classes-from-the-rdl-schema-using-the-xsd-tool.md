---
title: 第 2 課： 使用 xsd 工具的 RDL 結構描述產生類別 |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
1.  開啟的執行個體[!INCLUDE[msCoName](../includes/msconame-md.md)]Internet Explorer （或相當的網頁瀏覽器） 瀏覽至下列 URL:  
  
    ```  
    http://schemas.microsoft.com/sqlserver/reporting/2010/01/reportdefinition/ReportDefinition.xsd  
    ```  
  
2.  一旦在瀏覽器中開啟 RDL 結構描述，瀏覽至**檔案**功能表，然後選取**存**。  
  
3.  瀏覽至建立 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 專案的位置，然後以 ReportDefinition.xsd 檔案名稱儲存結構描述。  
  
4.  在儲存檔案之後，開啟的執行個體[!INCLUDE[vs_dev10_long](../includes/vs-dev10-long-md.md)]命令提示字元。 若要開啟命令提示字元的執行個體，請按一下 開始 功能表，指向**所有程式**，指向  **Microsoft Visual Studio 2010**，指向  **Visual Studio Tools**按一下**Visual Studio 命令提示字元 (2010)**。  
...
7.  將 ReportDefinition.xsd 加入至您的專案。 從**專案**功能表上，按一下 **加入現有項目**。 瀏覽至 ReportDefinition.xsd 檔案的位置、 選取 ReportDefinition.xsd，然後按一下 **新增**。  
    ...
    > [!NOTE]  
    >  ReportDefinition.xsd 檔案加入專案之後，您會注意到**方案總管 中**，ReportDefinition.cs (.vb) 檔案不存在。 若要顯示檔案，請按一下 ReportDefinition.xsd 檔案旁邊的展開/摺疊按鈕。  
