...
# <a name="creating-a-solution-and-data-source-intermediate-data-mining-tutorial"></a>建立方案與資料來源 (中繼資料採礦教學課程)
 ...
 當您使用建立專案時，直到部署方案之前，方案都會儲存為本機檔案。 當您部署方案時，[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]求[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]伺服器專案屬性中指定，並建立新[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]與專案同名的資料庫。 根據預設，[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]會使用**localhost**針對新專案的執行個體。 如果您要使用具名執行個體，或者如果為預設執行個體指定了不同名稱，您必須將專案的部署資料庫屬性變更到您要建立資料採礦物件的位置。  
...
3.  確認 **[伺服器]** 的名稱為 **[localhost]**。 如果您要使用不同的執行個體，請輸入執行個體的名稱。 如果您使用的具名執行個體[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]，鍵入電腦名稱，然後執行個體名稱。 [!INCLUDE[clickOK](../includes/clickok-md.md)]  
...
1.  在 [方案總管] 中，以滑鼠右鍵按一下專案，然後選取 **[屬性]**。  
     ...
     在  [!INCLUDE[ssBIDevStudioFull](../includes/ssbidevstudiofull-md.md)]，請在**專案**功能表上，選取**屬性**。  
...
-   [建立資料來源&#40;基本資料採礦教學課程&#41;](../../2014/tutorials/creating-a-data-source-basic-data-mining-tutorial.md)  
 ...
 單一資料來源可以支援多個資料來源檢視，而且每一個資料來源檢視可以包含多個資料表。 不過，由於資料來源和資料來源檢視已部署至您[!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]資料庫與您建立最佳的作法是，您應該只資料表每個資料來源檢視中包含的資料採礦模型所需的每個資料採礦模型或模型群組。  
