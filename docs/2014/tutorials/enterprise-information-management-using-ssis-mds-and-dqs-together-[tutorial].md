---
title: 使用 SSIS、 MDS 和 DQS 一起 [教學課程] 管理企業資訊 |Microsoft 文件
ms.custom: ''
ms.date: 12/29/2017
...
-   SQL Server Master Data Services  
 ...
 SQL Server Integration Services (SSIS) 提供了功能強大而且可擴充的平台，透過全面性擷取、轉換和載入 (ETL) 解決方案從各種不同的來源整合資料，這個解決方案可支援商務工作流程、資料倉儲或主要資料管理。 請參閱[Integration Services 概觀](http://msdn.microsoft.com/library/ms141263\(SQL.105\).aspx)主題取得快速概觀和典型用法 SSIS。  
 ...
 請參閱[清理和比對主要資料的使用 EIM 技術](http://msdn.microsoft.com/library/hh403491.aspx)白皮書的全面性指引實作 EIM 解決方案一起使用這些 Microsoft EIM 技術和監看式[Enterprise資訊管理 (EIM): 結合使用 SSIS、 DQS 和 MDS](http://go.microsoft.com/fwlink/?LinkId=258672) EIM 案例的示範影片。  
...
1.  已安裝下列元件的 Microsoft SQL Server 2012。  
    ...
    4.  SQL Server Data Tools  
         ...
         請參閱[SQL Server 2012 安裝指南](http://msdn.microsoft.com/library/bb500469.aspx)如需有關安裝產品的詳細資訊。  
  
2.  [設定 MDS 使用 Master Data Services 組態管理員](http://msdn.microsoft.com/library/ee633884.aspx)  
  
     使用組態管理員建立及設定 Master Data Services 資料庫。 建立 MDS 資料庫之後，為 MDS 建立 web 應用程式中的網站 (例如： [ http://localhost/MDS ](http://localhost/MDS)) 並將 MDS 資料庫與 MDS web 應用程式產生關聯。 請注意，若要建立 MDS Web 應用程式，您的電腦上應該已安裝 IIS。 請參閱[Web 應用程式需求 (Master Data Services)](http://msdn.microsoft.com/library/ee633744.aspx)和[資料庫需求 (Master Data Services)](http://msdn.microsoft.com/library/ee633767.aspx)設定 MDS 資料庫與 web 應用程式的必要條件的詳細資料。  
  
3.  [安裝和設定 DQS 使用 Data Quality Server Installer](http://msdn.microsoft.com/library/hh231682.aspx)。 按一下**啟動**，按一下 **所有程式**，按一下  **Microsoft SQL Server 2014**，按一下  **Data Quality Services**，然後按一下  **Data Quality Server Installer**。  
  
4.  Microsoft Excel 2010 (慣用 32 位元版本)。  
  
5.  安裝**Master Data Services 增益集的 Excel** （32 位元或 64 位元根據您在電腦的 Excel 版本） 從[這裡](http://www.microsoft.com/download/details.aspx?id=29064)。 若要尋找您的電腦上安裝的 Excel 版本，請執行**Excel**，按一下 **檔案** 功能表，按一下 **協助**以查看右窗格中的版本。 請注意，您需要安裝 Visual Studio 2010 Tools for Office Runtime，才能安裝 Excel 增益集。  
  
6.  （選擇性）建立的帳戶具有[Windows Azure Marketplace](https://datamarket.azure.com/)。 其中一個教學課程中的工作必須要有**Azure Marketplace** (最初稱為**資料市場**) 帳戶。 如果您想要可以略過此工作，並繼續進行下一項工作。  
  
7.  下載 Suppliers.xls 檔案從[Microsoft Download Center](http://go.microsoft.com/fwlink/?LinkId=271504)。  
...
|[第 2 課： 清理供應商資料使用供應商知識庫](../../2014/tutorials/lesson-2-cleansing-supplier-data-using-the-suppliers-knowledge-base.md)|在這一課中，建立和執行 DQS 專案來清理 Excel 檔案中的供應商資料使用**供應商**您在第 1 課中建立的知識庫。|45|  
|[第 3 課： 比對資料來移除重複項從供應商清單](../../2014/tutorials/lesson-3-matching-data-to-remove-duplicates-from-supplier-list.md)|在這一課，您會建立 DQS 專案來執行比對活動，以識別及移除已清理之供應商清單中的重複項。|45|  
|[第 4 課： 在 MDS 中儲存供應商資料](../../2014/tutorials/lesson-4-storing-supplier-data-in-mds.md)|在這一課，您將資料上傳已清理和比對的供應商 Master Data Services (MDS) 使用**MDS 增益集的 Excel**。|45|  
