---
title: 建立時序群集採礦模型結構 （中繼資料採礦教學課程） |Microsoft 文件
ms.custom: ''
ms.date: 03/06/2017
...
4.  在**建立資料採礦結構**頁面上，確認選擇**建立採礦結構與採礦模型**已選取。 接下來，按一下下拉式清單選項，**您想要使用哪一種資料採礦技術？**，然後選取**Microsoft 時序群集**。 按 [下一步] 。  
  
     **選取資料來源檢視**頁面隨即出現。 在下**可用的資料來源檢視**，選取`Orders`。  
...
6.  在**指定資料表類型**頁面上，選取**案例**旁的核取方塊**vAssocSeqOrders**資料表，並選取**巢狀**核取方塊旁邊**vAssocSeqLineItems**資料表。 按 [下一步] 。  
  
    > [!NOTE]  
    >  如果發生錯誤，當您選取**案例**或**巢狀**核取方塊，它可能是資料來源檢視中的聯結不正確。 巢狀的資料表， **vAssocSeqLineItems**，必須連接至案例資料表、 **vAssocSeqOrders，** 由多對一聯結。 您可以用滑鼠右鍵按一下聯結線並反轉聯結的方向，藉以編輯關聯性。 如需詳細資訊，請參閱[建立或編輯關聯性對話方塊&#40;Analysis Services-多維度資料&#41;](../../2014/analysis-services/create-or-edit-relationship-dialog-box-analysis-services-multidimensional-data.md)。  
...
7.  在**指定培訓資料**頁面上，選擇使用的資料行在模型中，選取核取方塊，如下所示：  
    ...
    -   **LineNumber**選取`Key`和**輸入**核取方塊。  
         ...
         **LineNumber**欄位將用做為識別碼，巢狀資料表，或`Sequence Key`。 巢狀資料表的索引鍵永遠都必須用於輸入。  
  
    -   **模型**選取**輸入**和**可預測**核取方塊。  
  
     確認選取項目都正確無誤，然後按一下**下一步**。  
  
8.  在**指定資料行的內容和資料類型**頁面上，確認此方格包含資料行、 內容類型和下表中所顯示的資料類型，然後按一下**下一步**。  
...
10. 在**正在完成精靈** 頁面上，針對**採礦結構名稱**，型別`Sequence Clustering with Region`。  
  
11. 如**採礦模型名稱**，型別`Sequence Clustering with Region`。  
