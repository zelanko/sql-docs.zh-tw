---
title: 瀏覽撥接中心模型 （中繼資料採礦教學課程） |Microsoft 文件
ms.custom: ''
ms.date: 06/13/2017
...
-   [Microsoft 類神經網路檢視器](#bkmk_NNviewer) **:** 這個檢視器可用於**採礦模型檢視器** 索引標籤的資料採礦設計師中，而且設計來協助您試驗資料中的互動性。  
  
-   [Microsoft 一般內容樹狀檢視器](#bkmk_genviewer) **:** 這個標準的檢視器提供深入的詳細資料模式和統計資料的演算法所產生時，發現模型。  
  
##  <a name="bkmk_NNviewer"></a> Microsoft 類神經網路檢視器  
 此檢視器包含三個窗格：**輸入**，**輸出**，和**變數**。  
  
 使用**輸出** 窗格中，您可以選取不同的可預測的屬性或相依變數的值。 如果您的模型包含多個可預測屬性，您可以選取該屬性從**輸出屬性**清單。  
  
 **變數**窗格會比較您選擇依據提供之屬性或變數的兩個結果。 彩色列以視覺方式表示變數影響目標結果的強度。 您也可以檢視變數的增益分數。 增益分數會根據您所使用的採礦模型類型而有不同的計算方式，但在使用此屬性進行預測時，通常會告訴您模型的增進部分。  
  
 **輸入**窗格可讓您將影響因數加入至模型，以嘗試各種假設狀況。  
  
### <a name="using-the-output-pane"></a>使用輸出窗格  
 在此初始模型中，您可能有興趣想要查看各種因數如何影響服務等級。 若要這樣做，您可以從輸出屬性的清單中選取服務等級，然後再從下拉式清單中選取範圍，藉以比較不同的服務等級**值 1**和**值 2**。  
  
##### <a name="to-compare-lowest-and-highest-service-grades"></a>若要比較最低和最高的服務等級  
  
1.  如**值 1**，選取具有最低值的範圍。 例如，範圍 0-0-0.7 表示最低的放棄率，因此是最佳的服務等級。  
  
    > [!NOTE]  
    >  在此範圍中的實際值可能會隨著您設定模型的方式而有所不同。  
  
2.  如**值 2**，選取具有最高值範圍。 例如，值為 >=0.12 的範圍表示最高的放棄率，因此是最差的服務等級。 換句話說，在此排班期間打電話的客戶之中，有 12% 的客戶在與服務人員通話前，就會掛斷電話。  
...
1.  在**變數** 窗格中，將滑鼠暫時放在第一個資料列的彩色列上方。  
  
     這個彩色的列會顯示強烈程度`Average Time Per Issue`對於服務等級。 工具提示會針對變數和目標結果的每個組合，顯示整體分數、機率，以及增益分數。  
...
4.  在不同的 Excel 工作表，以滑鼠右鍵按一下任何資料格，然後選取**貼**。  
...
3.  如**值**，選取**PM1**。  
     ...
     **變數**窗格會更新以顯示排班變更時模型的影響。  
  
4.  在**輸入**] 窗格中，按一下底下的下一個空白資料列**屬性**，然後選取 [通話。 如**值**，選取表示大通話數的範圍。  
  
     新的輸入條件就會加入至清單中。 **變數**窗格會更新以顯示通話量時，最高時對特定排班之模型的影響。  
  
5.  繼續變更 [排班] 和 [通話] 的值，以尋找排班、通話量，以及服務等級之間任何有趣的相互關聯。  
  
    > [!NOTE]  
    >  若要清除**輸入**] 窗格，讓您可以使用不同的屬性，按一下 [**重新整理檢視器內容**。  
...
-   **值 1 的增益**和**值 2 的增益**： 分數表示使用這個特定變數來預測值 1] 和 [值 2 結果的影響。 分數越高，表示變數越能預測結果。  
 ...
 下表包含最重要之影響因數的一些範例值。 例如，**值 1 的機率**為 60.6%，**值 2 的機率**為 8.30%，這表示，44-70 分鐘的時間範圍中每個問題的平均時間時，60.6%的案例中班中最高服務等級 (值 1) 和 8.30%的案例時較差的服務等級 (值 2) 與 shift 鍵。  
...
|attribute|ReplTest1|喜好\<0.07|喜好 >= 0.12|  
|---------------|-----------|--------------------|----------------------|  
|每個問題的平均時間|89.087 120.000||分數： 100<br /><br /> Value1 的機率： 4.45%<br /><br /> 值 2 的機率： 51.94%<br /><br /> Value1 的增益： 0.19<br /><br /> 值 2 的增益： 1.94|  
...
##  <a name="bkmk_genviewer"></a> Microsoft 一般內容樹狀檢視器  
 此檢視器可用於在處理模型時，檢視由演算法所建立的更詳細資訊。 **MicrosoftGeneric 內容樹狀檢視器**將採礦模型表示為一系列的節點，其中每個節點代表所學習到有關定型資料的知識。 此檢視器可以搭配所有模型使用，但是節點的內容會隨著模型類型而有所不同。  
...
3.  在**節點標題** 窗格中，按一下名稱為 10000000000000000 的節點。  
...
```  
 ...
 您也可以使用 NODE_DISTRIBUTION 資料表中的節點階層與詳細資料來周遊類神經網路中的個別路徑，並檢視隱藏層的統計資料。 如需詳細資訊，請參閱[類神經網路模型查詢範例](../../2014/analysis-services/data-mining/neural-network-model-query-examples.md)。  
