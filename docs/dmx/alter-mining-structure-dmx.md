...
[!INCLUDE[ssas-appliesto-sqlas](../includes/ssas-appliesto-sqlas.md)]
  ...
  建立以現有採礦結構為基礎的新採礦模型。  當您使用**ALTER database**語句來建立新的採礦模型時, 該結構必須已經存在。 相反地, 當您使用語句時,[建立「挖掘&#40;模型&#41;DMX](../dmx/create-mining-model-dmx.md)」時, 您會建立模型, 並同時自動產生其基礎的「採礦結構」。  
...
|**MODEL_EXISTENCE_ONLY**|指出屬性資料行的值是否比屬性的存在更重要。|  
 ...
 您可以為一個資料行定義多個模型旗標。 如需有關如何使用模型旗標的詳細資訊, 請參閱[模型旗標&#40; &#41;DMX](../dmx/modeling-flags-dmx.md)。  
...
## <a name="filter-criteria-expressions"></a>篩選準則運算式  
 ...
 篩選準則運算式是經過簡化的 DMX 述詞，與 WHERE 子句類似。 篩選運算式限於使用基本數學運算子、純量和資料行名稱的公式。 EXISTS 運算子則是例外；如果子查詢至少傳回一個資料列，該運算子就會評估為 True。 您可以使用常見的邏輯運算子來結合述詞:AND、OR 和 NOT。  
  
 如需有關與採礦模型搭配使用之篩選的詳細資訊, 請參閱[Analysis Services 資料&#40;挖掘&#41;的採礦模型篩選](https://docs.microsoft.com/analysis-services/data-mining/filters-for-mining-models-analysis-services-data-mining)。  
...
## <a name="parameter-definition-list"></a>參數定義清單  
 您可以將演算法參數加入至參數清單，以調整模型的效能和功能。 可以使用的參數是依照您在 USING 子句中指定的演算法而定。 如需與每個演算法相關聯的參數清單, 請參閱[資料採礦&#40;演算法 Analysis Services-資料&#41;挖掘](https://docs.microsoft.com/analysis-services/data-mining/data-mining-algorithms-analysis-services-data-mining)。  
...
## <a name="example-2-add-a-filtered-model-to-a-structure"></a>範例 2：將篩選的模型加入至結構  
...
## <a name="example-3-add-a-filtered-model-to-a-structure-with-a-nested-table"></a>範例 3:將篩選的模型加入具有嵌套資料表的結構  
 下列範例將採礦模型加入至購物籃採礦結構的修改版本中。 範例中使用的採礦結構已經過修改, 以加入**區域**資料行 (其中包含用戶端區域的屬性) 和**收入群組**資料行 (使用**High**、適中的值來分類客戶收入)或**Low**。  
