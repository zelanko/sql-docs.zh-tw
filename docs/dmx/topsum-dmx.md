...
## <a name="remarks"></a>備註  
 **TopSum**函數會根據每個資料列的\<rank 運算式 > 引數的評估值, 以遞減的次序順序傳回最頂部的資料列, 讓\<次序運算式 > 值的總和至少是指定的\<總和 > 引數所指定的總計。 **TopSum**會在仍符合指定總和值的情況下, 傳回可能的最小元素數目。  
 ...
 **TopSum**函數的第一個引數是資料表資料行的名稱。 在此範例中, 會呼叫 Predict 函數並使用 INCLUDE_STATISTICS 引數來傳回嵌套的資料表。  
