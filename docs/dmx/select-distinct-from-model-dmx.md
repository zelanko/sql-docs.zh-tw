---
description: 從 &lt; 模型 &gt; (DMX) 選取相異
title: 從 &lt; 模型 &gt; (DMX) 選取相異 |Microsoft Docs
ms.date: 06/07/2018
ms.prod: sql
...
## <a name="remarks"></a>備註  
 **SELECT DISTINCT FROM**語句只適用于單一資料行或一組相關的資料行。 這個子句不能配合一組未關聯的資料行使用。  
 ...
 **SELECT DISTINCT FROM \<model> **語句的結果會根據資料行類型而有所不同。 下表說明支援的資料行類型與陳述式的輸出。  
