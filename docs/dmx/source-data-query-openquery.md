...
## <a name="remarks"></a>備註  
 **OPENQUERY** 提供更安全的方式來存取外部資料，方法是支援資料來源許可權。 因為連接字串儲存在資料來源中，所以管理員可以使用資料來源的屬性管理資料的存取。 如需資料來源的詳細資訊，請參閱 [&#40;SSAS-多維度&#41;支援的資料來源 ](https://docs.microsoft.com/analysis-services/multidimensional-models/supported-data-sources-ssas-multidimensional)。  
  
 您可以藉由查詢 **MDSCHEMA_INPUT_DATASOURCES** 架構資料列集，取得可在伺服器上使用的資料來源清單。 如需使用 **MDSCHEMA_INPUT_DATASOURCES**的詳細資訊，請參閱 MDSCHEMA_INPUT_DATASOURCES 資料列 [集](https://docs.microsoft.com/previous-versions/sql/sql-server-2012/ms126243(v=sql.110))。  
