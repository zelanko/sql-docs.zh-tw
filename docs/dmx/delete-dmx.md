...
|引數|結果|  
|---------------|------------|  
|從採礦結構*\<結構中刪除>*<br /><br /> 或<br /><br /> 從採礦結構*\<結構>* 刪除。CONTENT|在此採礦結構上執行 ProcessClear。 清除採礦結構及其相關聯的採礦模型中所有的內容。|  
|從採礦結構*\<結構>* 刪除。種|在此採礦結構上執行 ProcessClearStructureOnly。 清除採礦結構中所有的內容，但是相關聯的採礦模型則保持不變。 清除採礦結構之後，在相關聯之採礦模型上的鑽研將會失敗。|  
|從採礦模型*\<模型中刪除>*<br /><br /> 或<br /><br /> 從 [採礦模型*\<模型>* 中刪除。CONTENT|在採礦模型上執行 ProcessClear，但狀態值保持不變。 狀態值是資料行的可能狀態。 例如，性別資料行的狀態值為男性與女性。|  
