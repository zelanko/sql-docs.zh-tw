...
WideWorldImporters 被設計來展示的許多重要功能的 SQL Server，包括最新 SQL Server 2016 中引進的功能。 下表列出 SQL server 的功能。 每個資料列也會提供的功能如何用於 WideWorldImporters 的描述。  
&nbsp;

|SQL Server 功能或功能|用於 WideWorldImporters|
|:-------------------------------|:------------------------|
|時態表|有許多的時態表，包括所有的查閱樣式參考資料表和主要實體，例如 StockItems、 客戶及供應商。 使用時態表可讓以方便地追蹤這些實體的歷程記錄。|
|適用於 JSON 的 AJAX 呼叫|應用程式經常會使用 AJAX 呼叫來查詢這些資料表：人員、 客戶、 供應商和 StockItems。 呼叫以 JSON 格式傳回資料。 例如，請參閱預存程序`Website.SearchForCustomers`。|
...
|查詢存放區|在資料庫上啟用查詢存放區。 在之後執行一些查詢，請執行下列步驟：<br/><br/>1.在 Management Studio 中開啟的資料庫。<br/>2.開啟查詢存放區，也就是資料庫底下的節點。<br/>3.開啟報表*熱門資源取用查詢*。 看到查詢執行中，與您剛剛執行的查詢計畫。|
