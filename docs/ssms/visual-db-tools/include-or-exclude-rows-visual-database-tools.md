---
description: 包含或排除資料列 (Visual Database Tools)
title: 包含或排除資料列
...
執行查詢時， [!INCLUDE[ssDE](../../includes/ssde_md.md)] 會檢查並將搜尋條件套用到您正在搜尋之資料表中的每個資料列。 如果符合搜尋條件，該資料列將包含在查詢中。 例如，要在特定區域中尋找所有員工的搜尋條件可以是：  
