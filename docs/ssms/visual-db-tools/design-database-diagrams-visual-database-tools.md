---
description: 設計資料庫圖表 (Visual Database Tools)
title: 設計資料庫圖表
...
**資料列選取器** ：您可以按一下資料列選取器，選取資料表中的資料庫資料行。 如果該資料行位於資料表的主索引鍵，資料列選取器將顯示索引鍵符號。 如需主索引鍵的資訊，請參閱[使用索引鍵](../../relational-databases/tables/primary-and-foreign-key-constraints.md)。  
