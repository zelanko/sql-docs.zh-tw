...
| 錯誤 (bug) 修正 | 詳細資料 |
| :------ | :------ |
| **一般 SSMS：**&nbsp; &nbsp; 已修正在使用者識別碼隸屬於多個租用戶時防止進行多重要素驗證 (MFA) 的問題。 | &nbsp; |
| **一般 SSMS：**&nbsp; &nbsp; 已修正效能儀表板報表會報告在子報表中找不到的 PAGELATCH 和 PAGEIOLATCH 等候的問題。 | &nbsp; |
| **SSMS 編輯器：**&nbsp; &nbsp; 已修正有數個系統檢視和資料表值函式未正確以色彩標示的問題。 | &nbsp; |
| **物件總管：** 已在物件總管篩選中新增適當的逸出。 | 請參閱 [Azure 意見反應 36678803](https://feedback.azure.com/forums/908035/suggestions/36678803) \(英文\)。 |
| **SMO：**&nbsp; &nbsp; 已修正 `GetDbComparer` 會將所有 Azure 資料庫的定序不正確地預設為 CI 的問題。 | &nbsp; |
| **受控執行個體支援：**&nbsp; &nbsp; 已改善受控執行個體伺服器特定屬性的顯示。 | 這些屬性包括硬體世代、服務層級，以及已使用和已保留的儲存體。 |
| **Azure SQL Database：**&nbsp; &nbsp; 已修正服務等級目標 (SLO) 會硬式編碼而導致 SSMS 難以支援較新 SQL Azure SLO 的問題。 | 現在，使用者可以登入 Azure，並允許 SSMS 擷取所有適用的 SLO 資料。<br/>適用於 SLO 資料項目為版本和大小上限。 |
| **結果方格：**&nbsp; &nbsp; 已修正按一下方格時會導致「索引超出範圍」例外狀況的問題。 | &nbsp; |
| **結果方格：**&nbsp; &nbsp; 已修正方格結果背景色彩遭到忽略的問題。 | 請參閱 [Azure 意見反應 32895916](https://feedback.azure.com/forums/908035/suggestions/32895916) \(英文\)。 |
| **分析工具：**&nbsp; &nbsp; 已修正導致 SQL Profiler 無法在 Windows 7 SP1 上啟動的問題。 | &nbsp; |
| **執行程序表：**&nbsp; &nbsp; 在 `RunTimeCountersPerThread` 中新增有關實際執行 XML 方案的四個屬性。 | &bull; &nbsp; &nbsp; `HpcRowCount` - HPC 裝置所處理的資料列數。<br/><br/>&bull; &nbsp; &nbsp; `HpcKernelElapsedUs` - 等候使用中核心執行的已耗用時間。<br/><br/>&bull; &nbsp; &nbsp; `HpcHostToDeviceBytes` -從主機傳輸到裝置的位元組。<br/><br/>&bull; &nbsp; &nbsp; `HpcDeviceToHostBytes` -從裝置傳輸到主機的位元組。 |
| **資料層應用程式精靈：**&nbsp; &nbsp; 已修正使用者因為對伺服器的存取受到限制而無法匯入資料層應用程式 (.dacpac) 的問題。 | 例如，如果使用者無法存取相同伺服器上的所有資料庫，則他們可能無法匯入 .dacpac 檔案。 |
...
## <a name="180-preview---cumulative-changelog-through-rc1"></a>18.0 預覽 - 至 RC1 為止的累積變更記錄
