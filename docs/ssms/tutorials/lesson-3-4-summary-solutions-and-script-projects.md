---
title: "摘要：解決方案和指令碼專案 | Microsoft Docs"
ms.custom: 
ms.date: 03/14/2017
ms.prod: sql-server-2016
ms.reviewer: 
ms.suite: 
ms.technology:
- database-engine
ms.tgt_pltfrm: 
ms.topic: article
applies_to:
- SQL Server 2016
ms.assetid: 9421d20f-0c8b-48e2-8e97-31f3047b8177
caps.latest.revision: 30
author: BYHAM
ms.author: rickbyh
manager: jhubbard
ms.workload: Inactive
ms.translationtype: HT
ms.sourcegitcommit: 5db067d5a2fe5bbf9953484c9a999ed7b1fcddae
ms.openlocfilehash: 8cd6ab8e3a5c73d7e7500916c6a22c6c2896f8b2
ms.contentlocale: zh-tw
ms.lasthandoff: 07/31/2017

---
# <a name="lesson-3-4---summary---solutions-and-script-projects"></a>課程 3-4 - 摘要 - 解決方案和指令碼專案
[!INCLUDE[msCoName](../../includes/msconame-md.md)]  [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] Visual Studio 廣泛的開發人員功能，提供開發人員在企業層級程式碼管理所需要的功能。 您可以建立自訂範本來支援重複的工作，在集中式與協調環境中儲存和管理複雜指令碼專案。  
  
## <a name="return-to-sql-server-tools-tutorials-portal"></a>返回 SQL Server 工具教學課程入口網站  
[教學課程：SQL Server Management Studio](../../tools/sql-server-management-studio/tutorial-sql-server-management-studio.md)  
  
## <a name="see-also"></a>另請參閱  
[適用於 SQL Server 2016 的教學課程](../../sql-server/tutorials-for-sql-server-2016.md)  
[Database Engine 教學課程](../../relational-databases/database-engine-tutorials.md)  
[Transact-SQL](http://msdn.microsoft.com/library/66a6b7b1-de6a-4161-82bd-98ded486947b)  
  
  
  

