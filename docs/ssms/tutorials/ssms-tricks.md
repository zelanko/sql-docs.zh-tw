...
5. 以滑鼠右鍵按一下 [資料表]，然後選取 [篩選] > [篩選設定]：

    ![篩選設定](media/ssms-tricks/filtersettings.png)

6. 在 [篩選設定] 視窗中，您可以修改下列部分篩選設定：
    - 依名稱篩選： 
...
7. 若要清除篩選，以滑鼠右鍵按一下 [資料表]然後選取 [移除篩選]。
...
4. 以滑鼠右鍵按一下 [目前] 錯誤記錄檔，然後選取 [檢視 SQL Server 記錄檔]：
...
## <a name="determine-sql-server-name"></a>尋找您的 SQL Server 名稱
