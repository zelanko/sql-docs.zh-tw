---
Title: 教學課程：透過使用 SQL Server Management Studio 來連線至及查詢 SQL Server 執行個體
...
在本節中，您會在新建立的 TutorialDB 資料庫中建立資料表。 因為查詢編輯器仍在主要  資料庫的內容中，請執行下列步驟將連線內容切換至 *TutorialDB*： 
