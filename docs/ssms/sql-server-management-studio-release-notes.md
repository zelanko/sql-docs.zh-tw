...
1. **還原資料庫精靈會產生不正確的目的地資料庫檔案位置的路徑模式**這是已知的問題，發生於當 SSMS 連線到 Linux 伺服器時。 即使路徑看起來不正確/怪異，伺服器端仍可正確地處理，因此不會有任何功能上的問題。

2. **檔案瀏覽器問題**
    - 使用以 Windows 為基礎的 SQL Server 2017 CTP 2.0 執行個體時，如果伺服器有空的軟碟機，或有安裝由 Bitlocker 保護的固定式磁碟，則 SSMS 中的檔案瀏覽器 UI 可能會無法開啟。 
...
5. **SSMS 17.x 只能連線到 SQL Server 2017 Integration Services (SSIS 2017) 執行個體。**  
