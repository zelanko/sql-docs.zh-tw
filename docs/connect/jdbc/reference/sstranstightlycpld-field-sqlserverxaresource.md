...
## <a name="remarks"></a>Remarks  
 每一筆交易都是由 XA 分支交易識別碼 (XID) 和全域交易識別碼 (GTRID) 所識別。 為了讓應用程式使用緊密結合的 XA 交易 (這些交易擁有不同的 XID，但是有相同的 GTRID)，您必須在 XAResource.start 方法的 flags 參數上設定 [SSTRANSTIGHTLYCPLD](../../../connect/jdbc/reference/sstranstightlycpld-field-sqlserverxaresource.md)。 如需如何使用此旗標的詳細資訊, 請參閱[瞭解 XA 交易](../../../connect/jdbc/understanding-xa-transactions.md)。  
