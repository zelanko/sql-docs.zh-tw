...
本文章提供有關如何開發使用 Microsoft JDBC 驅動程式中的 Azure Active Directory 驗證功能，適用於 SQL Server 的 Java 應用程式的資訊。
...
若要針對 SQL Server 的 Microsoft JDBC 驅動程式支援 Azure Active Directory 驗證的連接屬性如下：
*   **驗證**： 使用這個屬性來指出要用於連線的 SQL 驗證方法。 可能的值為： 
    * **ActiveDirectoryMSI**
        * 支援的驅動程式版本之後**v7.2**，`authentication=ActiveDirectoryMSI`可用來連線到 Azure SQL Database/資料倉儲從 Azure 資源內啟用的 「 身分識別 」 支援。 （選擇性） **msiClientId**也可以指定連接/資料來源屬性，以及此驗證模式中，必須包含用戶端識別碼，用來取得的受控服務身分識別中**accessToken**來建立連線。
    * **ActiveDirectoryIntegrated**
        * 支援的驅動程式版本之後**v6.0**，`authentication=ActiveDirectoryIntegrated`可用來連線到 Azure SQL Database/資料倉儲使用整合式的驗證。 若要使用此驗證模式中，您需要建立內部部署 Active Directory Federation Services (ADFS) 與雲端中的 Azure Active Directory 的同盟。 它已設定完畢後，您可以連接至應用程式類別路徑，在 Windows 作業系統上，將原生程式庫 'sqljdbc_auth.dll'，或設定跨平台驗證支援的 Kerberos 票證。 您將能夠存取 Azure SQL DB/DW，而不需要您登入加入網域的電腦時提示輸入認證。
    * **ActiveDirectoryPassword**
        * 支援的驅動程式版本之後**v6.0**，`authentication=ActiveDirectoryPassword`可用來連線到 Azure SQL Database/資料倉儲使用 Azure AD 主體名稱和密碼。
    * **SqlPassword**
...
* 如果您使用**ActiveDirectoryPassword**驗證模式中，您需要[azure activedirectory-程式庫-針對-java](https://github.com/AzureAD/azure-activedirectory-library-for-java)和其相依性。 如需詳細資訊，請參閱 <<c0>  **使用 ActiveDirectoryPassword 驗證模式連接**一節。
* 如果您使用**ActiveDirectoryIntegrated**模式中，您需要 azure activedirectory-程式庫-針對-java 和其相依性。 如需詳細資訊，請參閱 <<c0>  **使用 ActiveDirectoryIntegrated 驗證模式連接**一節。

## <a name="connecting-using-activedirectorymsi-authentication-mode"></a>使用 ActiveDirectoryMSI 驗證模式進行連接
下列範例示範如何使用 `authentication=ActiveDirectoryMSI` 模式。 執行此範例從內 Azure 資源、 e、 g Azure 虛擬機器、 App Service 或同盟函式應用程式與 Azure Active Directory。
...
若要使用 ActiveDirectoryMSI 驗證模式範例：
...
Azure 虛擬機器上執行此範例會擷取來自的存取權杖_系統指派給受控身分識別_或是_使用者指派給受控身分識別_(如果**msiClientId**已指定），並建立連線，使用擷取的存取權杖。 如果建立連線，您應該會看到下列訊息：
...
下列範例示範如何使用 `authentication=ActiveDirectoryIntegrated` 模式。 在加入網域的電腦與 Azure Active Directory 同盟執行此範例。 代表您的 Azure AD 主體，或您所屬的群組，其中的自主的資料庫使用者必須存在於資料庫中，而且必須具有 CONNECT 權限。 
...
應用程式/服務可以從 Azure Active Directory 擷取存取權杖，然後使用它來連線到 Azure SQL Database/資料倉儲。

> [!NOTE] 
> **accessToken**只能設定使用 DriverManager 類別中的 getconnection （） 方法的屬性參數。 它不能在連接字串。

下列範例包含簡單的 Java 應用程式連接到 Azure SQL Database/資料倉儲使用存取權杖型驗證。 再建置及執行範例，請執行下列步驟：
