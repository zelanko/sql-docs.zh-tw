...
## <a name="compile-time"></a>編譯時間
 ...
 - `com.microsoft.azure:azure-keyvault` ： 永遠加密 Azure Key Vault 功能 （選擇性） azure Key Vault Provider
 - `com.microsoft.azure:azure-keyvault-webkey` ： 永遠加密 Azure Key Vault 功能 （選擇性） azure Key Vault Provider
 - `com.microsoft.azure:adal4j` ： 適用於 Java 的 Azure Active Directory 驗證功能和 Azure Key Vault 功能 （選擇性） azure Active Directory 程式庫
 - `com.microsoft.rest:client-runtime` ： 適用於 Java 的 Azure Active Directory 驗證功能和 Azure Key Vault 功能 （選擇性） azure Active Directory 程式庫
- `org.osgi:org.osgi.core`： OSGi Framework 支援 OSGi 核心程式庫。
- `org.osgi:org.osgi.compendium`： OSGi Framework 支援 OSGi 精品程式庫。
...
- JDBC 驅動程式版本 7.2.1-相依性版本： Azure key Vault （1.2.0 版）、 Azure 金鑰保存庫 Webkey （1.2.0 版）、 Adal4j （版本 1.6.3），用戶端-執行階段-針對-AutoRest (1.6.5) 和其相依性 ([範例應用程式](../../connect/jdbc/azure-key-vault-sample-version-7.0.md))
- JDBC 驅動程式版本 7.0.0-相依性版本： Azure key Vault （1.0.0 版）、 Adal4j （1.6.0 版），及其相依性 ([範例應用程式](../../connect/jdbc/azure-key-vault-sample-version-7.0.md))
...
- JDBC 驅動程式版本 7.2.1-相依性版本： Adal4j （版本 1.6.3），用戶端-執行階段-針對-AutoRest (1.6.5) 和其相依性
