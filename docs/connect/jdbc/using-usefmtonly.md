---
description: 透過 useFmtOnly 擷取 ParameterMetaData
...
```
> [!NOTE]  
>  此功能僅支援 `SELECT/INSERT/UPDATE/DELETE` 查詢。 查詢的開頭應為 4 個支援的關鍵字之一或一個[通用資料表運算式](../../t-sql/queries/with-common-table-expression-transact-sql.md)，後面接著其中一個支援的查詢。 不支援在通用資料表運算式內使用參數。
