...
[!INCLUDE[Driver_JDBC_Download](../../includes/driver_jdbc_download.md)]
 ...
 為了有效處理複雜陳述式，JDBC 驅動程式提供一些方法來查詢傳回的物件和資料，使應用程式可以正確處理它們。 處理複雜陳述式的關鍵在於 [SQLServerStatement](../../connect/jdbc/reference/execute-method-sqlserverstatement.md) 類別的 [execute](../../connect/jdbc/reference/sqlserverstatement-class.md) 方法。 此方法會傳回**布林值**。 若此值為 true，則從陳述式傳回的第一個結果是結果集。 當此值為 false，則傳回的第一個結果是更新計數。  
