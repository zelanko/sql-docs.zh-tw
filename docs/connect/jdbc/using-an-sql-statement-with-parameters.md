---
title: 搭配參數使用 SQL 語句 |Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
...
當您將值傳遞至 setter 方法時，您不但要指定用於 SQL 陳述式的實際值，也要指定 SQL 陳述式中參數的序數位置。 例如, 如果您的 SQL 語句包含單一參數, 其序數值將會是1。 如果陳述式包含兩個參數，則第一個序數值為 1，而第二個序數值會是 2。
