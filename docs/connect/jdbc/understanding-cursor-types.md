...
## <a name="cursor-conversion"></a>資料指標轉換  
 ...
 使用[!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)], 當您透過 TYPE_SCROLL_SENSITIVE 和 resultset. CONCUR_UPDATABLE 結果集更新資料時, 會擲回例外狀況並顯示訊息「資料指標為唯讀」。 這個例外狀況的發生是因為 [!INCLUDE[ssVersion2000](../../includes/ssversion2000-md.md)] 已經針對該結果集完成隱含資料指標轉換，而且沒有傳回已經要求的可更新資料指標。  
  
 若要解決這個問題，您可以執行下列其中一種解決方案：  
  
-   確保基礎資料表擁有主索引鍵。  
  
-   建立語句時, 請使用[SQLSERVERRESULTSET TYPE_SS_SCROLL_DYNAMIC](../../connect/jdbc/reference/type-ss-scroll-dynamic-field-sqlserverresultset.md)來取代 RESULTSET. TYPE_SCROLL_SENSITIVE。  
...
## <a name="cursor-updating"></a>資料指標更新  
 ...
 在呼叫更新\<類型 > 之後, get\<類型 > 就無法存取受影響的資料行, 直到呼叫 updateRow 或[cancelRowUpdates](../../connect/jdbc/reference/cancelrowupdates-method-sqlserverresultset.md)為止。 這樣可以避免使用不同於伺服器所傳回之類型更新資料行所產生的問題，而且後續的 getter 呼叫可能會叫用產生不正確結果的用戶端類型轉換。 get\<Type> 的呼叫將會擲回例外狀況訊息：「直到呼叫 updateRow() 或 cancelRowUpdates() 之後，才能存取已更新的資料行」。  
