...
Azure Key Vault 是存放和管理 Always Encrypted 資料行主要金鑰的方便選項 (尤其是當應用程式裝載在 Azure 時)。 Microsoft JDBC Driver for SQL Server 包含內建的提供者 SQLServerColumnEncryptionAzureKeyVaultProvider, 適用于具有 Azure Key Vault 儲存金鑰的應用程式。 此提供者的名稱是 AZURE_KEY_VAULT。 若要使用 Azure Key Vault 存放區提供者, 應用程式開發人員必須在 Azure Key Vault 中建立保存庫和金鑰, 並在 Azure Active Directory 中建立應用程式註冊。 已註冊的應用程式必須被授與 Get、解密、加密、解除包裝金鑰、包裝金鑰, 以及驗證針對與 Always Encrypted 所建立的金鑰保存庫所定義之存取原則的許可權。 如需有關如何設定金鑰保存庫和建立資料行主要金鑰的詳細資訊, 請參閱[Azure Key Vault-逐步](https://blogs.technet.microsoft.com/kv/2015/06/02/azure-key-vault-step-by-step/)解說和[在 Azure Key Vault 中建立資料行主要金鑰](../../relational-databases/security/encryption/create-and-store-column-master-keys-always-encrypted.md#creating-column-master-keys-in-azure-key-vault)。
...
針對此頁面上的範例, 如果您已使用 SQL Server Management Studio 建立 Windows 憑證存放區型資料行主要金鑰和資料行加密金鑰, 則用來重新建立的 T-sql 腳本可能會類似于此範例, 並具有自己的特定**KEY_PATH**和**ENCRYPTED_VALUE**:
...
強制加密功能會在使用 Always Encrypted 時, 強制加密參數。 如果使用強制加密但 SQL Server 向驅動程式告知參數不需要加密，使用該參數的查詢就會失敗。 此屬性會針對受到安全性攻擊危害的 SQL Server 提供額外的保護，此類 SQL Server 會將不正確的加密中繼資料提供給用戶端，而可能導致資料洩露。 SQLServerPreparedStatement 和 SQLServerCallableStatement 類別中的 set * 方法和 SQLServerResultSet 類別中\*的 update 方法會多載, 以接受布林值引數來指定強制加密設定。 如果這個引數的值為 false, 則驅動程式不會強制加密參數。 如果 [強制加密] 設定為 [true], 則只有在目的地資料行已加密, 而且在連接或語句上啟用 Always Encrypted 時, 才會傳送查詢參數。 使用此屬性可提供額外的安全性層級, 以確保驅動程式不會在預期加密時, 錯誤地將資料傳送到 SQL Server 做為純文字。
