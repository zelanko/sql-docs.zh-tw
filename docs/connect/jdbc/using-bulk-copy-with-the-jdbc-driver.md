...
> [!IMPORTANT]  
>  此範例不會執行，除非您已建立工作資料表中所述[資料表設定](../../ssms/download-sql-server-management-studio-ssms.md)取得它。  
...
4.  目的地中，請選取**一般檔案目的地**輸入**檔案名稱**與目的地，例如 C:\Test\TestBulkCSVExample.csv。 請檢查**格式**分隔**文字限定詞**為 none，並啟用**中第一個資料列的資料行名稱**，然後選取**下一步**  
...
### <a name="bulk-copy-with-always-encrypted-columns"></a>使用永遠加密資料行的大量複製  
 ...
 根據大量複製選項，以及加密的來源和目的地資料表 JDBC 驅動程式可能會以透明的方式解密和加密的資料，或它的型別可能會傳送加密的資料依。 例如，大量資料複製到未加密的資料行的加密資料行時驅動程式以透明的方式解密資料傳送到 SQL Server 之前。 同樣地複製大量資料時未加密的資料行 （或從 CSV 檔案） 來加密資料行，此驅動程式以透明的方式加密資料傳送到 SQL Server 之前。 如果同時在來源和目的地已加密，然後視**allowEncryptedValueModifications**大量複製選項時，會將資料解密和加密再傳送到 SQL Server 驅動程式會傳送資料。  
  
 如需詳細資訊，請參閱**allowEncryptedValueModifications**大量複製選項，和[使用一律加密與 JDBC 驅動程式](../../connect/jdbc/using-always-encrypted-with-the-jdbc-driver.md)。  
  
> [!IMPORTANT]  
>  Microsoft JDBC Driver 6.0 for SQL Server 複製大量資料時從 CSV 檔案加密的資料行的限制：  
>   
>  日期和時間類型支援只有 TRANSACT-SQL 預設字串常值格式  
>   
>  不支援 DATETIME 和 SMALLDATETIME 資料型別  
...
|建構函式|Description|  
|-----------------|-----------------|  
|SQLServerBulkCopy(Connection)|初始化使用指定的 SQLServerConnection 開啟執行個體的 SQLServerBulkCopy 類別的新執行個體。 如果此連接已啟用交易，將會在該交易內執行複製作業。|  
|SQLServerBulkCopy (字串 connectionURL)|初始化並開啟 SQLServerConnection 根據提供的 connectionURL 的新執行個體。 建構函式會使用 SQLServerConnection 初始化 SQLServerBulkCopy 類別的新執行個體。|  
  
|屬性|Description|  
|--------------|-----------------|  
|字串 DestinationTableName|該伺服器的目的地資料表名稱。<br /><br /> 如果呼叫 writeToServer 時尚未設定 DestinationTableName，便會擲回 SQLServerException。<br /><br /> DestinationTableName 是三部分名稱 (\<資料庫 >。\<d >。\<名稱 >)。 如果您要加以選擇，則可以用該資料表的資料庫和主控結構描述來限定該資料表名稱。 不過，如果該資料表名稱使用底線 ("_") 或任何其他特殊字元，您就必須使用括起來的括號逸出該名稱。 如需詳細資訊，請參閱《SQL Server 線上叢書》中的＜識別碼＞。|  
...
|Void addColumnMapping （sourceColumn 字串、 字串 destinationColumn）|加入新的資料行對應，方法是使用資料行名稱來指定來源和目的地資料行。|  
...
|字串 getDestinationTableName()|擷取目的地資料表的目前名稱。|  
...
|Void writeToServer(ResultSet sourceData)|所有資料列中提供的結果集複製到 SQLServerBulkCopy 物件的 DestinationTableName 屬性所指定的目的地資料表。|  
|Void writeToServer(RowSet sourceData)|將提供的資料列集之所有資料列複製到 SQLServerBulkCopy 物件的 DestinationTableName 屬性所指定的目的地資料表。|  
|Void writeToServer(ISQLServerBulkRecord sourceData)|複製 SQLServerBulkCopy 物件的 DestinationTableName 屬性所指定的目的資料表提供的 ISQLServerBulkRecord 實作中的所有資料列。|  
...
|建構函式|Description|  
|-----------------|-----------------|  
|SQLServerBulkCopyOptions()|初始化 SQLServerBulkCopyOptions 類別使用的所有設定預設的新執行個體。|  
  
 Getter 和 Setter 主要用於下列選項：  
  
|選項|Description|預設值|  
|------------|-----------------|-------------|  
|布林 CheckConstraints|在插入資料時檢查條件約束。|False - 不會檢查條件約束。|  
...
|布林 allowEncryptedValueModifications|此選項是使用 Microsoft JDBC Driver 6.0 （或更高） for SQL Server。<br /><br /> 指定時， **allowEncryptedValueModifications**可讓您無須解密資料的資料表或資料庫之間的加密資料的大量複製。 一般而言，應用程式會選取從加密資料行從一個資料表的資料而無須解密 （應用程式會連接到資料庫資料行加密設定關鍵字設定為停用） 的資料，然後會使用這個選項來大量插入資料，這仍屬於加密。 如需詳細資訊，請參閱[使用一律加密與 JDBC 驅動程式](../../connect/jdbc/using-always-encrypted-with-the-jdbc-driver.md)。<br /><br /> 指定時請特別小心**allowEncryptedValueModifications**這可能會導致這些資料庫損毀，因為驅動程式不會檢查是否資料是否確實加密，或是否正確加密使用相同的加密類型、 演算法和金鑰設成目標資料行。||  
  
 Getter 和 setter:  
  
|方法|Description|  
|-------------|-----------------|  
|布林 isCheckConstraints()|指出是否要或不在插入資料時檢查條件約束。|  
|Void setCheckConstraints(Boolean checkConstraints)|設定條件約束是否要先檢查在插入資料時。|  
|布林 isFireTriggers()|指出是否伺服器應該引發插入觸發程序被插入至資料庫的資料列。|  
|Void setFireTriggers(Boolean fireTriggers)|設定是否應該引發觸發程序的資料列插入資料庫設定伺服器。|  
|布林 isKeepIdentity()|指出要保留的任何來源識別值。|  
|Void setKeepIdentity(Boolean keepIdentity)|設定要保留識別值。|  
|布林 isKeepNulls()|指出是否要保留的設定預設值，不論目的資料表中的 null 值，或如果它們應取代為預設值 （如果適用的話）。|  
|Void setKeepNulls(Boolean keepNulls)|設定是否要保留的設定預設值，不論目的資料表中的 null 值，或如果它們應取代為預設值 （如果適用的話）。|  
|布林 isTableLock()|指出 SQLServerBulkCopy 是否應該在大量複製作業期間取得大量更新鎖定。|  
|Void setTableLock(Boolean tableLock)|設定是否 SQLServerBulkCopy 應該在大量複製作業期間取得大量更新鎖定。|  
|布林 isUseInternalTransaction()|指出是否會在交易內發生大量複製作業的每個批次。|  
|Void setUseInternalTranscation(Boolean useInternalTransaction)|是否在交易內執行大量複製作業的每個批次或未設定。|  
|Int getBatchSize()|取得每個批次中的資料列數目。 在每個批次結束時，批次中的資料列會傳送到伺服器|  
|Void setBatchSize(int batchSize)|設定每個批次中的資料列數目。 在每個批次的結尾，會將該批次中的資料列傳送到伺服器。|  
|Int getBulkCopyTimeout()|取得作業逾時前完成的秒數。|  
|Void setBulkCopyTimeout(int timeout)|設定作業逾時前完成的秒數。|  
|布林 isAllowEncryptedValueModifications()|指出 allowEncryptedValueModifications 設定是否要啟用或停用。|  
|void setAllowEncryptedValueModifications(boolean allowEncryptedValueModifications)|將設定 allowEncryptedValueModifications 設定是用於使用永遠加密資料行的大量複製。|  
...
|建構函式|Description|  
|-----------------|-----------------|  
|SQLServerBulkCSVFileRecord （fileToParse 字串、 字串編碼、 字串分隔符號，布林 firstLineIsColumnNamesSQLServerBulkCSVFileRecord （字串、 字串、 字串、 布林值）|初始化 SQLServerBulkCSVFileRecord 類別的新執行個體，這會以提供的分隔符號和編碼，剖析 fileToParse 的每一行。 如果 firstLineIsColumnNames 設為 True 時，檔案中的第一行會剖析為資料行名稱。  如果編碼為 NULL，將會使用預設編碼方式。|  
|SQLServerBulkCSVFileRecord (字串 fileToParse，字串編碼，布林 firstLineIsColumnNamesSQLServerBulkCSVFileRecord （String，String，布林值）|初始化 SQLServerBulkCSVFileRecord 類別的新執行個體，這會以逗點做為分隔符號和以提供的編碼，剖析 fileToParse 的每一行。 如果 firstLineIsColumnNames 設為 True 時，檔案中的第一行會剖析為資料行名稱。  如果編碼為 NULL，將會使用預設編碼方式。|  
...
|方法|Description|  
|------------|-----------------|  
|Void addColumnMetadata （int positionInFile 字串 columnName、 int jdbcType、 int、 整數位數 int 小數位數）|在該檔案中加入指定資料行的中繼資料。|  
