---
title: 使用大型資料
description: 了解如何在這些範例應用程式中，使用 JDBC Driver for SQL Server 中的大型資料類型。
ms.custom: ''
ms.date: 08/12/2019
ms.prod: sql
...
在 [!INCLUDE[msCoName](../../includes/msconame_md.md)][!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)] JDBC 驅動程式 1.2 版中，預設的緩衝模式為 "**full**"。 如果您的應用程式並未在連接屬性中或使用 [SQLServerStatement](reference/sqlserverstatement-class.md) 物件的 [setResponseBuffering](reference/setresponsebuffering-method-sqlserverstatement.md) 方法，將 "responseBuffering" 連接屬性設定為 "**adaptive**"，此驅動程式就會支援一次從伺服器中讀取完整的結果。 為了取得適應性緩衝行為，您的應用程式必須明確將 "responseBuffering" 連接屬性設定為 "**adaptive**"。

**adaptive** 值就是預設的緩衝模式，而且 JDBC 驅動程式會在必要時緩衝處理最少的可能資料。 如需使用自適性緩衝的詳細資訊，請參閱[使用自適性緩衝](using-adaptive-buffering.md)。
...
| 主題                                                                                                   | 描述                                                              |
| ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| [讀取大型資料範例](reading-large-data-sample.md)                                               | 描述如何使用 SQL 陳述式來擷取大數值資料。       |
| [使用預存程序讀取大型資料範例](reading-large-data-with-stored-procedures-sample.md) | 描述如何擷取大型 CallableStatement OUT 參數值。 |
| [更新大型資料範例](updating-large-data-sample.md)                                             | 描述如何更新資料庫中的大數值資料。                |
