...
description: 建立使用 OLE DB Driver for SQL Server 的 SQL Server 資料表
ms.custom: ''
ms.date: 06/14/2018
ms.prod: sql
...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
 ...
 OLE DB Driver for SQL Server 可以產生永久或暫存資料表的名稱。 當取用者將 *pTableID* 參數設定為 NULL，並將 *ppTableID* 設定為指向有效的 DBID\* 時，OLE DB Driver for SQL Server 會在 *ppTableID* 值所指向之 DBID 的 *uName* 聯集 *pwszName* 成員中，傳回所產生的資料表名稱。 若要建立 OLE DB Driver for SQL Server 命名的暫存資料表，取用者要將 OLE DB 資料表屬性 DBPROP_TBL_TEMPTABLE 包含在 *rgPropertySets* 參數中所參考的資料表屬性集內。 OLE DB Driver for SQL Server 命名的暫存資料表是本機。  
  
 如果 **pTableID** 參數的 *eKind* 成員不包含 DBKIND_NAME，*CreateTable* 會傳回 DB_E_BADTABLEID。  
  
## <a name="dbcolumndesc-usage"></a>DBCOLUMNDESC 用法  
 取用者可以使用 *pwszTypeName* 成員或 *wType* 成員來指出資料行資料類型。 如果取用者指定中的資料類型*pwszTypeName*，OLE DB Driver for SQL Server 會忽略值*wType*。  
 ...
 OLE DB Driver for SQL Server 可辨識中的 OLE DB 列舉之 DBTYPE 值的子集*wType*成員。 如需詳細資訊，請參閱 < [ITableDefinition 中的資料類型對應](../../oledb/ole-db-data-types/data-type-mapping-in-itabledefinition.md)。  
...
|DBPROP_COL_DESCRIPTION|R/W：讀取/寫入<br /><br /> 預設值：無<br /><br /> 描述：OLE DB Driver for SQL Server 不會實作 DBPROP_COL_DESCRIPTION 資料行屬性。<br /><br /> 當取用者嘗試寫入屬性值時，DBPROP 結構的 *dwStatus* 成員會傳回 DBPROPSTATUS_NOTSUPPORTED。<br /><br /> 將屬性設定不會構成嚴重錯誤 OLE DB driver for SQL Server。 如果其他所有參數值都有效，則會建立 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 資料表。|  
...
|屬性識別碼|Description|  
|-----------------|-----------------|  
|DBPROP_TBL_TEMPTABLE|R/W：讀取/寫入<br /><br /> 預設值： VARIANT_FALSE 描述： 根據預設，OLE DB Driver for SQL Server 會建立由取用者命名的資料表。 當 VARIANT_TRUE、 OLE DB Driver for SQL Server 會產生取用者的暫存資料表名稱。 取用者會將 **CreateTable** 的 *pTableID* 參數設定為 NULL。 *ppTableID* 參數必須包含有效的指標。|  
