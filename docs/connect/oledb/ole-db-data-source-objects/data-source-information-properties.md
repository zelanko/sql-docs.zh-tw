...
|SSPROP_UNICODELCID|輸入：VT_I4 R/W：讀取<br /><br /> 描述：Unicode 地區設定識別碼。<br /><br /> 這是用於 Unicode 資料排序的地區設定。|  
|SSPROP_UNICODECOMPARISONSTYLE|輸入：VT_I4 R/W：讀取<br /><br /> 描述：Unicode 比較樣式。<br /><br /> 用於 Unicode 資料排序的排序選項。|  
  
 在提供者特定的屬性集 DBPROPSET_SQLSERVERSTREAM 中，OLE DB Driver for SQL Server 會定義下列其他屬性。  
  
|屬性識別碼|描述|  
|-----------------|-----------------|  
|SSPROP_STREAM_XMLROOT|輸入：VT_BSTR R/W：讀取/寫入<br /><br /> 描述：FOR XML 查詢的結果可能不是語式正確的文件。 指定此屬性時，'select ... for XML' 查詢的結果會包裝在此屬性提供的根標記中，以傳回格式正確的 XML 文件。 如果在瀏覽器中執行查詢，載入結果時，它可能會使瀏覽器顯示剖析器錯誤。 為避免這個錯誤，SQL ISAPI 支援關鍵字 ROOT。 此關鍵字會對應至 SSPROP_STREAM_XMLROOT 屬性。|  
