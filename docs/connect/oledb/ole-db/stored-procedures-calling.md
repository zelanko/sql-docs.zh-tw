...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
  ...
  預存程序可以有零或多個參數。 它也可以傳回值。 使用 SQL Server 的 OLE DB 驅動程式，當預存程序的參數可傳遞的：  
...
>  搭配 OLE DB 使用具名參數呼叫 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 預存程序時，參數名稱開頭必須是 '\@' 字元。 這是一個 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 特定的限制。 SQL Server OLE DB 驅動程式會比 MDAC 更嚴格地強制執行這項限制。  
 ...
 呼叫暫存預存程序使用參數的命令必須先呼叫**icommandwithparameters:: Setparameterinfo**定義的參數資訊，可以成功準備命令之前。 這是因為暫存預存程序的內部名稱不同於用戶端所使用的外部名稱，且 MSOLEDBSQL 無法查詢系統資料表來判斷暫存預存程序的參數資訊。  
...
## <a name="methods-of-calling-a-stored-procedure"></a>呼叫預存程序的方法  
 執行中的預存程序時[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]，SQL Server 的 OLE DB 驅動程式支援:  
