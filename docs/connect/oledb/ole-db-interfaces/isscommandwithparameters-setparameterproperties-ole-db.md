 ...
 **SetParameterInfo**必須呼叫方法，然後再呼叫**isscommandwithparameters::** 方法。 呼叫 `SetParameterProperties(0, NULL)` 會清除所有指定的參數屬性，呼叫 `SetParameterInfo(0,NULL,NULL)` 則會清除所有的參數資訊，包括任何可能與參數相關聯的屬性。  
 ...
 呼叫**isscommandwithparameters::** 即可指定屬性的類型不是參數 DBTYPE_XML 或 DBTYPE_UDT 會傳回 DB_E_ERRORSOCCURRED 或 DB_S_ERRORSOCCURRED，並*dwStatus*所有 DBPROPs SSPARAMPROPS 中包含具有 DBPROPSTATUS_NOTSET 的該參數的欄位。 系統會周遊 SSPARAMPROPS 中包含的每個 DBPROPSET 的 DBPROP 陣列，以偵測 DB_E_ERRORSOCCURRED 或 DB_S_ERRORSOCCURRED 所參考的是哪一個參數。  
