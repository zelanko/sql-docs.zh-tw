...
|BCP_OPTION_FMTXML|用於指定所產生的格式檔案應該是 XML 格式。 根據預設，這個選項是關閉的，而且格式檔案會儲存為文字檔案。 XML 格式檔案提供更大的彈性，但是有一些條件約束。 例如，您不可以同時指定欄位的前置詞和結束字元，即使在更早版本中是可以這麼做的。<br /><br /> 注意:只有當同時安裝 [!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)] 工具與 OLE DB Driver for SQL Server 時，才支援 XML 格式檔案。|  
