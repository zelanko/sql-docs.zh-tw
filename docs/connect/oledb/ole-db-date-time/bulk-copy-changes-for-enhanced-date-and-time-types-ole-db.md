...
[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]
  ...
  本文說明在 SQL Server 的 OLE DB 驅動程式中, 支援大量複製功能的日期/時間增強功能。  
...
## <a name="character-data-files"></a>字元資料類型  
 在字元資料檔中, 日期和時間值會以資料類型的「資料格式: 字串和常值」一節中所述的方式來表示, 以支援 OLE DB 的[OLE DB 日期和時間改善](../../oledb/ole-db-date-time/data-type-support-for-ole-db-date-and-time-improvements.md)。  
...
## <a name="bcp-types-in-msoledbsqlh"></a>內含 msoledbsql.h 中的 BCP 類型  
 下列類型定義于內含 msoledbsql.h 中。 這些類型會以 OLE DB 中 IBCPSession:: BCPColFmt 的*eUserDataType*參數傳遞。  
  
|檔案儲存類型|主檔案資料類型|輸入內含 msoledbsql.h 以搭配 IBCPSession:: BCPColFmt 使用|ReplTest1|  
|-----------------------|-------------------------|-----------------------------------------------------------|-----------|  
 ...
 **OLE DB 注意**：下列轉換是由 IBCPSession 所執行。 IRowsetFastLoad 使用[從用戶端到伺服器執行的轉換](../../oledb/ole-db-date-time/conversions-performed-from-client-to-server.md)中所定義的 OLE DB 轉換。 請注意，datetime 值會捨入為一秒的 1/300，而 smalldatetime 值在執行以下所描述的用戶端轉換之後，會將其秒數設定為零。 Datetime 捨入會透過小時和分鐘 (但非日期) 傳播。  
...
|------------|-------------|  
|-|不支援轉換。<br />|  
|1|如果提供的資料無效, 就會公佈錯誤。 對於 datetimeoffset 值，即使沒有要求轉換為 UTC，此時間部分在轉換到 UTC 之後仍然必須在範圍內。 這是因為 TDS 和伺服器永遠會以 UTC 的 datetimeoffset 值，正規化時間。 因此，用戶端必須確認時間元件在轉換為 UTC 之後，仍然位於支援的範圍內。|  
|2|忽略時間元件。|  
...
|7|日期會設定為 1900-01-01。|  
|8|忽略時區位移。|  
|9|根據出現的第一個標點符號字元以及剩餘的元件是否存在，字串會經過剖析，並轉換為 date、datetime、datetimeoffset 或 time 值。 接著，在此程序針對來源類型探索本文結尾之資料表中的規則之後，此字串會轉換為目標類型。 如果無法剖析所提供的資料而不發生錯誤, 或任何元件部分超出允許的範圍, 或如果沒有從常數值型別轉換成目標型別, 就會公佈錯誤。 對於 datetime 和 Smalldatetime 參數, 如果年份超出這些類型支援的範圍, 就會公佈錯誤。<br /><br /> 對於 datetimeoffset，即使沒有要求轉換為 UTC，此值在轉換到 UTC 之後仍然必須在範圍內。 這是因為 TDS 和伺服器永遠會以 UTC 的 datetimeoffset 值，將時間正規化，因此用戶端必須在轉換成 UTC 之後，確認時間元件位於支援的範圍內。 如果值不在支援的 UTC 範圍內, 則會公佈錯誤。|  
|10|針對用戶端對伺服器的轉換, 如果截斷時發生資料遺失, 就會公佈錯誤。 如果此值落在伺服器使用之 UTC 範圍所代表的範圍外，也可能發生這個錯誤。 如果在從伺服器轉換為用戶端時發生秒或小數秒的截斷，只會有一個警告。|  
|11|針對用戶端對伺服器的轉換, 如果截斷時發生資料遺失, 就會公佈錯誤。|
