...
description: 了解 OLE DB Driver for SQL Server 如何透過 varchar(max)、varbinary(max) 與 nvarchar(max) 類型支援大數值資料類型。
ms.custom: ''
ms.date: 06/12/2018
ms.prod: sql
...
-   繫結為 DBTYPE_IUNKNOWN 並使用資料流。  
 ...
 在報告資料行的大小上限時，OLE DB Driver for SQL Server 將會報告：  
