...
> [!NOTE]  
>  預設不會啟用 MARS 功能。 若要使用 MARS 時，連線到[!INCLUDE[ssNoVersion](../../../includes/ssnoversion-md.md)]利用 OLE DB 驅動程式的 SQL Server，您必須特別啟用它的連接字串內。 如需詳細資訊，請參閱本主題稍後的 SQL Server 各節，OLE DB 驅動程式。  
...
## <a name="ole-db-driver-for-sql-server"></a>SQL Server 的 OLE DB 驅動程式  
 SQL Server OLE DB 驅動程式可透過 SSPROP_INIT_MARSCONNECTION 資料來源初始化屬性，這在 DBPROPSET_SQLSERVERDBINIT 屬性集中實作支援 MARS。 此外，新的連接字串關鍵字， **MarsConn**也已經加入。 它可接受**true**或**false**值;**false**是預設值。  
  
 資料來源屬性 DBPROP_MULTIPLECONNECTIONS 預設為 VARIANT_TRUE。 這表示，為了支援多個並行命令和資料列集物件，此提供者將會繁衍多個連接。 啟用 MARS 時，OLE DB 驅動程式的 SQL Server 可以支援多個命令和資料列集物件上的單一連接，因此 multiple_connections 為 variant_false 預設。  
...
### <a name="ole-db-driver-for-sql-server-example"></a>OLE DB 驅動程式，例如 SQL Server  
 在此範例中，資料來源物件建立使用 SQL Server 的 OLE DB 驅動程式，並啟用 MARS 時使用的 DBPROPSET_SQLSERVERDBINIT 屬性集建立工作階段物件以前。  
