---
title: 命令 | Microsoft Docs
description: OLE DB Driver for SQL Server 中的命令
ms.custom: ''
ms.date: 06/14/2018
ms.prod: sql
ms.prod_service: database-engine, sql-database, sql-data-warehouse, pdw
ms.reviewer: ''
ms.technology: connectivity
ms.topic: reference
helpviewer_keywords:
- OLE DB Driver for SQL Server, commands
- commands [OLE DB]
author: pmasl
ms.author: pelopes
ms.openlocfilehash: f6919c8355660ee2c1e27f2c0896ca189497b50d
ms.sourcegitcommit: b78f7ab9281f570b87f96991ebd9a095812cc546
ms.translationtype: HT
ms.contentlocale: zh-TW
ms.lasthandoff: 01/31/2020
ms.locfileid: "68016024"
---
# <a name="commands"></a>命令
[!INCLUDE[appliesto-ss-asdb-asdw-pdw-md](../../../includes/appliesto-ss-asdb-asdw-pdw-md.md)]

[!INCLUDE[Driver_OLEDB_Download](../../../includes/driver_oledb_download.md)]

  OLE DB Driver for SQL Server 會公開 **ICommand** 介面和命令物件，本節將會有完整說明：  
  
-   [命令語法](../../oledb/ole-db-commands/command-syntax.md)  
  
-   [命令參數](../../oledb/ole-db-commands/command-parameters.md)  
  
-   [準備命令](../../oledb/ole-db-commands/preparing-commands.md)  
  
-   [產生多個資料列集結果的命令](../../oledb/ole-db-commands/commands-generating-multiple-rowset-results.md)  
  
## <a name="see-also"></a>另請參閱  
 [OLE DB Driver for SQL Server 程式設計](../../oledb/ole-db/oledb-driver-for-sql-server-programming.md)  
  
  
