...
|CEKeystoreProvider<br />CEKeystoreName<br />CEKeystoreEncryptKey|字串|指定的路徑、 名稱和 「 一律加密 」 功能給自訂金鑰存放區提供者的加密金鑰。 必須設定所有三個值，才能進行連接時，適當地設定自訂金鑰存放區提供者。 如需詳細資訊，請參閱[PHP 應用程式中啟用一律加密](../../connect/php/enabling-always-encrypted-php-application.md)。|未設定值。|
...
|ColumnEncryption|**啟用**或**已停用**|指定是否啟用 「 永遠加密 」 功能。 如需詳細資訊，請參閱[PHP 應用程式中啟用一律加密](../../connect/php/enabling-always-encrypted-php-application.md)。|已停用|  
