...
## <a name="table-of-connection-options"></a>連接選項表
...
|Failover_Partner|String|指定在主要伺服器無法使用時所要使用的伺服器和資料庫鏡像執行個體 (如果已啟用和設定)。<br /><br />搭配使用 Failover_Partner 與 MultiSubnetFailover 時有一些限制。 如需詳細資訊，請參閱 <<c0> [ 支援高可用性、 災害復原](../../connect/php/php-driver-for-sql-server-support-for-high-availability-disaster-recovery.md)。|未設定值。|  
