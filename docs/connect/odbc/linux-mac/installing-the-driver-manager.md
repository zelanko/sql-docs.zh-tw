---
title: 安裝驅動程式管理員
description: 此文章包含在 Linux 和 macOS 上安裝 unixODBC 驅動程式管理員，以便與所有 Microsoft ODBC Driver for SQL Server 版本搭配使用的指示。
ms.custom: ''
ms.date: 02/15/2018
ms.prod: sql
...
當遵循下列文章中的指示，在 Linux 或 macOS 上安裝 Microsoft ODBC Driver 13、13.1 或 17 for SQL Server 時，套件管理系統會自動解決驅動程式管理員相依性：
...
您現在已可開始安裝驅動程式。 如需詳細資訊，請參閱 [Linux](../../../connect/odbc/linux-mac/installing-the-microsoft-odbc-driver-for-sql-server.md) 或 [macOS](../../../connect/odbc/linux-mac/install-microsoft-odbc-driver-sql-server-macos.md) 的 ODBC 驅動程式安裝指示。
