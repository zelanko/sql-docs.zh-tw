...
指定驗證的方法，並設定 Microsoft  進階用戶端項目，以及在設定資料來源時， Native Client ODBC 驅動程式用來連接至  的登入與密碼。

## <a name="options"></a>選項。

### <a name="with-integrated-windows-authentication"></a>整合式 Windows 驗證(&W)
...
### <a name="with-active-directory-integrated-authentication"></a>利用 Active Directory 整合式驗證(&I)

指定驅動程式會向使用 Azure Active Directory 的 SQL Server。 選取之後，不論伺服器目前的登入安全性模式為何，SQL Server 都會透過此資料來源，使用整合式登入安全性來建立連接。

### <a name="with-sql-server-authentication"></a>使用 SQL Server 驗證

指定驅動程式會向 SQL Server 使用的登入識別碼和密碼。

### <a name="with-active-directory-password-authentication"></a>Active Directory 密碼驗證

指定驅動程式會向使用 Azure Active Directory 登入識別碼和密碼的 SQL Server。

### <a name="with-active-directory-interactive-authentication"></a>Active Directory 互動式驗證
...
[資料來源精靈畫面 4](../../../connect/odbc/windows/dsn-wizard-1.md)

[資料來源精靈畫面 4](../../../connect/odbc/windows/dsn-wizard-3.md)
