...
*operation*  
要傳遞至 TM 的 XA 作業。 可能的作業會在 [xadefs](use-xa-with-dtc.md#xadefsh) 中定義。

*xid*  
交易分支識別碼。

*flags*  
與 TM 要求相關聯的旗標。 可能的值會在 [xadefs](use-xa-with-dtc.md#xadefsh) 中定義。

*status*  
從 TM 傳回狀態。 如需可能的傳回狀態，請參閱 [xadefs](use-xa-with-dtc.md#xadefsh) 標頭。
...
下列範例示範如何與 TM 通訊以進行 XA 交易，以及如何從用戶端應用程式執行不同的作業。 如果測試是針對 Microsoft SQL Server 執行，則必須正確設定 MS DTC，才能啟用 XA 交易。 您可以在 [xadefs](#xadefsh) 標頭檔中找到 XA 定義。 
