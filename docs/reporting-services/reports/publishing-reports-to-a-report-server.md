...
ms.topic: "article"
helpviewer_keywords: 
  - "實際環境 [Reporting Services]"
  - "報表專案 [Reporting Services]"
  - "偵錯組態 [Reporting Services]"
  - "報表發行 [Reporting Services]"
  - "發行報表 [Reporting Services]"
  - "報表屬性 [Reporting Services]"
  - "報表設計師 [Reporting Services], 部署報表"
  - "實際組態 [Reporting Services]"
  - "發行報表 [Reporting Services], 實際執行環境"
  - "DebugLocal 組態 [Reporting Services]"
  - "部署 [Reporting Services], 報表"
  - "報表設計師 [Reporting Services], 發行報表"
...
在 [建立] 功能表上，按一下 [部署 \<報表專案名稱>]。 或者，在方案總管中，以滑鼠右鍵按一下報表專案，然後按一下 [部署]。 您可以在 [輸出] 視窗中，檢視發行程序的狀態。  
...
在方案總管中，以滑鼠右鍵按一下報表，然後按一下 [部署]。 您可以在 [輸出] 視窗中，檢視發行程序的狀態。  
  
 當您發行報表時，也必須部署該報表所使用的共用資料來源。   
 如果您不要發行專案中的所有報表，可以只選擇發行單一報表。 若要這樣做，請選取部署報表的組態 (例如 Release 組態)，以滑鼠右鍵按一下報表，然後按一下 [部署]。  
