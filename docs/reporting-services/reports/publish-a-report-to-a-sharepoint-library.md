...
5.  (選擇性) 為 **TargetDataSourceFolder** 鍵入 SharePoint 文件庫或文件庫資料夾的 URL (例如，`https://TestServer/TestSite/Documents/DataSources`)。  
  
     如果您未指定值，則會使用 **TargetReportFolder** 值。  
  
6.  為 **TargetReportFolder** 鍵入文件庫或文件庫資料夾的 URL (例如，`https://TestServer/TestSite/Documents/Reports`)。  
  
7.  為 **TargetServerURL**輸入 SharePoint 頂層網站或子網站的 URL。 若未指定網站，則使用預設的最上層網站 (例如，`https://servername`、`https://servername/site` 或 `https://servername/site/subsite`)。  
