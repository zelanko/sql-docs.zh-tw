...
[!INCLUDE[ssrs-appliesto](../../includes/ssrs-appliesto.md)] [!INCLUDE[ssrs-appliesto-2016](../../includes/ssrs-appliesto-2016.md)] [!INCLUDE[ssrs-appliesto-not-pbirsi](../../includes/ssrs-appliesto-not-pbirs.md)] [!INCLUDE[ssrs-appliesto-sharepoint-2013-2016i](../../includes/ssrs-appliesto-sharepoint-2013-2016.md)]
  ...
  本主題中的程序會引導您完成 SharePoint 模式的 Reporting Services 單一伺服器安裝。 這些步驟包含執行 [SQL Server 安裝精靈]，以及使用 SharePoint 管理中心的設定工作。 本主題也可以用於更新現有的安裝，例如若要建立 Reporting Services 服務應用程式的個別程序。  
  
> [!NOTE]
> SQL Server 2016 之後已無法再使用 reporting Services 與 SharePoint 整合。
  
 如需將更多的 Reporting Services 伺服器加入至現有的伺服陣列的資訊，請參閱下列各項：  
...
##  <a name="bkmk_singleserver"></a>單一伺服器部署範例
 ...
 單一伺服器安裝對開發和測試案例很實用，但是不建議在實際執行環境中使用單一伺服器。 單一伺服器環境是指已安裝在同一部電腦上的 SharePoint 與 Reporting Services 元件的單一電腦。 本主題並未涵蓋具有多個 Reporting Services 伺服器的向外。  
  
 下圖說明單一伺服器的 Reporting Services 部署的一部分的元件。  
...
|**(2)**|Reporting Services 增益集適用於 SharePoint 產品提供使用者介面元件的 SharePoint 伺服器上。|  
...
##  <a name="bkmk_setupaccounts"></a> 安裝程式帳戶
 ...
 本章節描述的帳戶和權限，用於以 SharePoint 模式的 Reporting Services 主要部署步驟。  
  
 **安裝及註冊 Reporting Services 服務：**  
  
-   期間 （稱為 「 安裝程式 」 帳戶） 的安裝 Reporting Services SharePoint 模式中使用的目前帳戶必須具有本機電腦系統管理權限。 如果您要安裝 Reporting Services，安裝 SharePoint，並在 「 安裝程式 」 帳戶也是 SharePoint 伺服器陣列系統管理員群組的成員之後，Reporting Services 安裝將會註冊您的 Reporting Services 服務。 如果您安裝 Reporting Services，才能安裝 SharePoint，或者 「 安裝程式 」 帳戶不是伺服器陣列管理員群組的成員，會手動註冊服務。 請參閱＜ [步驟 2：註冊並啟動 Reporting Services SharePoint 服務](#bkmk_install_SSRS_sharedservice)。  
  
 **建立 Reporting Services 服務應用程式**  
  
-   在安裝及註冊 Reporting Services 服務，建立一或多個 Reporting Services 服務應用程式。 「SharePoint 伺服器陣列服務帳戶」需要暫時成為本機 Administrators 群組的成員，才能建立 Reporting Services 服務應用程式。 如需 SharePoint 2013 帳戶權限的詳細資訊，請參閱 [SharePoint 2013 中的帳戶權限及安全性設定](http://technet.microsoft.com/library/cc678863.aspx) (http://technet.microsoft.com/library/cc678863.aspx) 或者對於 SharePoint 2016，請參閱 [SharePoint 2016 中的帳戶權限及安全性設定](https://technet.microsoft.com/library/cc678863\(v=office.16\).aspx)。  
...
##  <a name="bkmk_install_SSRS"></a> 步驟 1：在 SharePoint 模式下安裝 Reporting Services 報表伺服器
 ...
 這個步驟會在 SharePoint 模式和 Reporting Services 增益集適用於 SharePoint 產品安裝 Reporting Services 報表伺服器。 根據電腦上已經安裝的元件而定，您可能不會看見下列步驟所描述的部分安裝頁面。  
 
 > [!IMPORTANT]
 > 對於 SharePoint 2016，將安裝 Reporting Services 的 SharePoint 伺服器上必須具備**自訂**伺服器角色。 Reporting Services 的部署將會成功，不是在 SharePoint 伺服器上**自訂**角色，但在下一個 SharePoint 維護期間，MinRole 將停止的 Reporting Services 服務因為它偵測到Reporting Services SharePoint 整合模式中不會指出支援任何其他 SharePoint 伺服器角色。 Reporting Services 服務應用程式僅支援**自訂**角色。
 ...
 ### <a name="apply-the-custom-server-role-to-a-sharepoint-2016-server"></a>適用於 SharePoint 2016 伺服器的自訂伺服器角色
...
10. 如果您看見 **[伺服器組態]** 頁面，請輸入適當的認證。 如果您想要使用 Reporting Services 資料警示或訂閱功能，您需要變更**啟動類型**供 SQL Server Agent**自動**。 根據電腦上已經安裝的元件而定，您可能不會看見 **[伺服器組態]** 頁面。  
  
     選取 **[下一步]**。  
  
11. 如果您選取 Database Engine 服務，您會看到**資料庫引擎組態**頁面上，將適當的帳戶加入至 SQL 管理員清單並選取**下一步**。  
  
12. 在 **[Reporting Services 組態]** 頁面上，您應該會看到已選取 **[只安裝]** 選項。 這個選項會安裝報表伺服器檔案，並不會設定 Reporting services 的 SharePoint 環境。  
  
    > [!NOTE]
    > 當 SQL Server 安裝完成時，請遵循本主題其他章節來設定 SharePoint 環境。 這包括安裝 Reporting Services 共用的服務，以及建立 Reporting Services 服務應用程式。  
...
> [!NOTE]
> 如果您要在現有的 SharePoint 伺服器陣列中進行安裝，則不需要完成本節中的步驟。 安裝 Reporting Services SharePoint 服務和一部分，本文件的上一節執行 SQL Server 安裝精靈時啟動。  
  
 以下是您為什麼需要手動註冊 Reporting Services 服務的常見原因。  
  
-   安裝 SharePoint 之前，您會安裝 Reporting Services SharePoint 模式。  
  
-   若要安裝 Reporting Services SharePoint 模式中，使用的帳戶不是 SharePoint 伺服器陣列管理員群組的成員。 如需詳細資訊，請參閱＜ [Setup accounts](#bkmk_setupaccounts)＞一節。  
...
6.  執行下列 PowerShell 命令以啟動服務，或參閱下列附註，以取得有關如何從 SharePoint 管理中心啟動服務的指示：  
    ...
    >
    > 表示您不在 Windows Powershell，而非 SharePoint 管理命令介面中，或未安裝 Reporting Services SharePoint 模式。 如需有關 Reporting Services 和 PowerShell 的詳細資訊，請參閱[Reporting Services SharePoint 模式的 PowerShell 指令程式](../../reporting-services/report-server-sharepoint/powershell-cmdlets-for-reporting-services-sharepoint-mode.md)。  
...
11. 服務應用程式的建立程序會需要數分鐘才能完成。 完成後，您會看到確認訊息及 **[提供訂閱和警示]** 頁面的連結。 如果您想要使用 Reporting Services 訂閱功能或資料警示功能，請完成提供步驟。 如需詳細資訊，請參閱 [Provision Subscriptions and Alerts for SSRS Service Applications](../../reporting-services/install-windows/provision-subscriptions-and-alerts-for-ssrs-service-applications.md)。  
  
 ![PowerShell 相關內容](../../analysis-services/instances/install-windows/media/rs-powershellicon.jpg "PowerShell 相關內容")上使用 PowerShell 建立 Reporting Services 服務應用程式的資訊，請參閱：  
...
##  <a name="bkmk_powerview"></a>步驟 4： 啟動 Power View 網站集合功能。
 ...
 [!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)]一項功能的 SQL Server 2016 Reporting Services 增益集的[!INCLUDE[msCoName](../../includes/msconame-md.md)]SharePoint 產品是網站集合功能。 針對根網站集合以及安裝 Reporting Services 增益集之後所建立的網站集合自動啟用此功能。 如果您打算使用 [!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)]，請確認此功能是否已啟用。  
  
 如果您在安裝 SharePoint server 之後安裝的 Reporting Services 增益集適用於 SharePoint 產品，然後報表伺服器整合功能和 Power View 整合功能將只會針對啟用根網站集合。 如果是其他網站集合，請手動啟用這些功能。  
...
-   建立名為"Reporting Services Application"Reporting Services 服務應用程式。  
...
此外，使用 Reporting Services 服務應用程式的應用程式集區安全性帳戶必須是 Analysis Services 伺服器上的系統管理員。
  
###  <a name="bkmk_provision_agent"></a>佈建訂閱及警示  
...
### <a name="configure-e-mail-for-ssrs-service-applications"></a>設定 SSRS 服務應用程式的電子郵件  
 Reporting Services 資料警示功能會以電子郵件訊息傳送警示。 若要傳送電子郵件，您可能需要設定 Reporting Services 服務應用程式，您可能需要修改服務應用程式的電子郵件傳遞延伸模組。 如果您打算使用的電子郵件傳遞延伸模組 Reporting Services 訂閱功能，是需要電子郵件設定。 如需詳細資訊，請參閱[Reporting Services 服務應用程式 &#40; 設定電子郵件SharePoint 2013 和 SharePoint 2016&#41;](http://msdn.microsoft.com/38fc34a6-aae7-4dde-9ad2-f1eee0c42a9f). 
  
### <a name="add-reporting-services-content-types-to-content-libraries"></a>將 Reporting Services 內容類型加入至內容庫  
 Reporting Services 提供預先定義的內容類型，可用來管理共用的資料來源 (.rsds) 檔、 報表模型 (.smdl) 和報表產生器報表定義 (.rdl) 檔案。 將 **[報表產生器報表]**、 **[報表模型]**和 **[報表資料來源]** 內容類型加入至文件庫會啟用 **[新增]** 命令，讓您能夠建立該類型的新文件。 如需詳細資訊，請參閱 [將 Reporting Services 內容類型加入至 SharePoint 文件庫](../../reporting-services/report-server-sharepoint/add-reporting-services-content-types-to-a-sharepoint-library.md)。  
...
##  <a name="bkmk_verify_installation"></a> 驗證安裝  
 以下是建議的步驟和程序確認 Reporting Services SharePoint 模式部署。  
...
-   在 SharePoint 文件庫中，建立只包含一個文字方塊，例如標題的基本 Reporting Services 報表。 此報表不包含任何資料來源或資料集。 目標是確認您可以開啟報表產生器、建立基本報表和預覽報表。  
