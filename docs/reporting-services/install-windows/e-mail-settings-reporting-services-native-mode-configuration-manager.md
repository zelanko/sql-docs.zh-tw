---
title: "電子郵件設定 - Reporting Services 原生模式 (組態管理員) | Microsoft Docs"
ms.custom: ""
ms.date: "06/01/2016"
...
---
# 電子郵件設定 - Reporting Services 原生模式 (組態管理員)
...
> [!NOTE] 預設電子郵件傳遞延伸模組不提供以數位方式，將外寄郵件訊息簽章或加密的支援。
...
> [!NOTE] 報表伺服器電子郵件設定是以 CDO 為基礎。 如果您想要取得有關特定設定的詳細資料，可以參考 CDO 產品文件集。

## <a name="rsconfigman"/>使用 Reporting Services 管理員設定報表伺服器電子郵件
...
> [!NOTE] 建議您透過 Reporting Services 組態管理員設定郵件伺服器。
...
7. 如果 `<SendUsing>` 設為 1 或 2，請將 `<SMTPAuthenticate>` 設為 **1** 值。
...
- **[SMTPServerPickupDirectory]** 設定為本機磁碟機上的資料夾。
  ...
  > [!NOTE] 如果是使用本機 SMTP 伺服器，請確定並未設定 SMTPServer。
...
[Modify a Reporting Services Configuration File (rsreportserver.config) (修改 Reporting Services 組態檔 (RSreportserver.config))](../../reporting-services/report-server/modify-a-reporting-services-configuration-file-rsreportserver-config.md)  
