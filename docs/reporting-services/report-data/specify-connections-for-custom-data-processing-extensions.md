...
-   支援 <xref:Microsoft.ReportingServices.DataProcessing.IDbConnection> 的自訂資料處理延伸模組  
  
-   支援 <xref:Microsoft.ReportingServices.DataProcessing.IDbConnectionExtension> 的自訂資料處理延伸模組  
  
> [!NOTE]  
>  請洽詢協力廠商提供者，找出自訂資料處理延伸模組的實作方式。  
  
## <a name="impersonation-and-custom-data-processing-extensions"></a>模擬與自訂資料處理延伸模組  
 如果您的自訂資料處理延伸模組會連接到資料來源使用模擬，您必須使用 Open 方法上<xref:Microsoft.ReportingServices.DataProcessing.IDbConnection>或<xref:Microsoft.ReportingServices.DataProcessing.IDbConnectionExtension>來提出要求的介面。 此外，您也可以先儲存使用者識別物件 (System.Security.Principal.WindowsIdentity)，然後在其他資料處理延伸模組 API 中重複使用它。  
...
## <a name="connections-for-idbconnection"></a>IDbConnection 的連接  
 如果您使用自訂資料處理延伸模組僅支援<xref:Microsoft.ReportingServices.DataProcessing.IDbConnection>，您必須以下列方式指定連接：  
...
## <a name="connections-for-idbconnectionextension"></a>IDbConnectionExtension 的連接  
 如果您使用自訂資料處理延伸模組和支援， <xref:Microsoft.ReportingServices.DataProcessing.IDbConnectionExtension>，您可以下列方式來指定連接：  
