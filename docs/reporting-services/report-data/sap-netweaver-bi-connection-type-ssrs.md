---
title: "SAP NetWeaver BI 連接類型 (SSRS) |Microsoft 文件"
ms.custom: 
ms.date: 03/17/2017
...
-   在設計模式中，將維度、成員、成員屬性以及關鍵數值拖曳到 [查詢] 窗格，或者直接在 [查詢] 窗格中輸入 MDX 文字。 請從 [導出成員] 窗格中將導出成員拖曳到 [資料] 窗格，以定義其他資料集欄位。  
 ...
 如需使用這個查詢設計工具的詳細資訊，請參閱 [SAP NetWeaver BI 查詢設計工具使用者介面 &#40;報表產生器&#41;](http://msdn.microsoft.com/library/8edda06d-1608-498b-bd50-10905e54f6ce)。  
...
### <a name="working-with-field-properties"></a>使用欄位屬性  
 在 [報表資料] 窗格中，並不會顯示擴充欄位屬性，因為您無法將項目拖曳至報表配置上。 相反地，將該屬性的父欄位拖曳至報表，然後將預設屬性從 **Value** 變更為想要使用的屬性。 比方說，如果欄位名稱**Calendar Year/Month Level 01**建立 MDX 查詢設計工具所卸除一個層級的中繼資料 窗格拖曳到查詢窗格，請自訂擴充屬性**長檔名**在運算式中使用下列語法：  
