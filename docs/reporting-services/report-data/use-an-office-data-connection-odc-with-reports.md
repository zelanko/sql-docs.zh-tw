...
-   認證可以設定為**無**，**預存**，或**整合**。 如果認證方法設為**預存**，報表伺服器將會提示使用者提供認證，而不是使用預存的認證。 報表伺服器無法依照 .odc 檔案的定義使用預存認證。  
...
-   您必須在 [!INCLUDE[msCoName](../../includes/msconame-md.md)] Office 2007 (舊版 .odc 與報表定義檔不相容) 中建立此 .odc 檔案。  
 ...
 現有的 .odc 檔案必須與報表與查詢完全相容，您才能使用該檔案。 如果發生需要大幅修改報表或 .odc 檔案的錯誤，則應為報表建立新的 .rsds 檔案。 如需如何建立使用.rsds 檔案的共用的資料來源的詳細資訊，請參閱[建立和管理共用資料來源 &#40;Reporting Services SharePoint 整合模式 &#41;](http://msdn.microsoft.com/library/2d3428e4-a810-4e66-a287-ff18e57fad76).  
...
7.  **連接至特定資料表**預設會選取核取方塊。 並且用來結取特定資料表中的資料。 報表伺服器會忽略 .odc 檔案中的所有查詢，因此不論您選取或清除這個核取方塊都不會有影響。 為報表擷取資料的查詢會包含在報表定義檔案中，而非外部檔案中。  
  
8.  當連線開啟時，您可以編輯屬性並將它匯出。 在**資料**索引標籤的**連線**群組中，按一下**屬性**，然後按一下 **連接屬性**連線名稱旁邊的按鈕。  
  
9. 在**定義**索引標籤上，按一下 **匯出連線檔案**。  
  
10. 輸入檔案的名稱，然後按一下**儲存**。 接著關閉應用程式和所有已開啟的檔案。  
...
5.  按一下**開啟**選取檔案，請按一下**確定**儲存選取項目。 新項目的屬性頁會自動開啟。  
  
6.  在 內容類型選取**報表資料來源**，然後按一下**確定**。  
  
7.  指向報表。  
  
8.  按一下向下箭號，然後選取**管理資料來源**。  
  
9. 按一下資料來源名稱。  
  
10. 如果報表使用自訂資料來源資訊，請按一下**共用**。  
  
11. 在**資料來源連結**，按一下瀏覽 (**...**) 按鈕。  
  
12. 選取剛才上傳的 .odc 檔案。  
  
13. 按一下**確定**以選取檔案，然後按一下**確定**以儲存變更。  
