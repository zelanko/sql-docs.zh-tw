...
Power BI 報表伺服器以及 SQL Server Reporting Services 2016 和更新版本都使用**受控 ODP.NET** 來撰寫編頁報表。 只有在使用 Oracle ODAC 驅動程式 12.2 和更新版本時，才需要執行下列步驟，因為預設會對新 Oracle 主目錄的非電腦全域組態安裝這些驅動程式。 這些步驟假設您已將 ODAC 18.x 檔案安裝至 c:\oracle64，且 Oracle.ManagedDataAccess.dll 和 Oracle.DataAccess.dll 的檔案版本為 4.122.18.3。
...
報表產生器使用**受控 ODP.NET** 來撰寫編頁報表。 只有在使用 Oracle ODAC 驅動程式 12.2 和更新版本時，才需要執行下列步驟，因為預設會對新 Oracle 主目錄的非電腦全域組態安裝這些驅動程式。 這些步驟假設您已將 ODAC 18.x 檔案安裝至 c:\oracle32，且 Oracle.ManagedDataAccess.dll 的檔案版本為 4.122.18.3。
...
Power BI 報表使用**非受控 ODP.NET**。 只有在使用 Oracle ODAC 驅動程式 12.2 和更新版本時，才需要執行下列步驟，因為預設會對新 Oracle 主目錄的非電腦全域組態安裝這些驅動程式。 這些步驟假設您已將 ODAC 18.x 檔案安裝至 c:\oracle64 (適用於 64 位元) 和 c:\oracle32 (適用於 32 位元)，且 Oracle.DataAccess.dll 的檔案版本為 4.122.18.3。 遵循這些步驟來註冊非受控 ODP.NET：
