...
1.  在新的資料表或矩陣或新增圖表精靈中上,**選擇資料集**頁面上，選取**建立資料集**，然後按一下 **下一步**。 **[選擇與資料來源的連接]** 頁面隨即開啟。  
...
4.  按一下 **[下一步]**。  
    ...
    如有需要，請輸入您的認證。 若要儲存在本機的認證，請選取**連接的儲存密碼**。 如果您未選取此選項，則會在每次執行報表時提示您提供認證  
...
3.  從**查看**下拉式清單中，選取**最近使用的網站和伺服器**。 在資料來源] 窗格中，按一下您伺服器的 URL，然後按一下 [**開啟**。  
  
    資料來源或模型的清單隨即出現。  
  
4.  或者，在**名稱**，輸入報表伺服器的 URL。 按一下 **[開啟]**。  
  
    報表產生器會連接到報表伺服器，並且在根資料夾中載入可用的資料來源。  
  
5.  瀏覽至包含您有足夠的權限，以連接至，選取資料來源，然後按一下 資料來源的資料夾**開啟**。  
...
8.  如果提示您提供使用者名稱和密碼，請輸入您的認證。 若要儲存在本機的認證，請選取**連接的儲存密碼**。  
