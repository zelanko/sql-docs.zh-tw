...
| 相關內容 | 設定鑽研連結的能力。 此連結可以是在入口網站或自訂 URL 上發行行動報表。 |
...
## <a name="configuring-related-content"></a>設定相關的內容

當您選擇**行動報表**，您可以在對話方塊中選擇的目的地。

   ![行動報表](media/rscreatekpi-related-content-mobile-report.png)

當您現在按一下入口網站中的 KPI 時，行動報表縮圖會顯示相關內容的下拉式清單底下。 按一下這個縮圖可以直接瀏覽您對此報表。

您也可以指定自訂 URL。 這項工作可以是任何項目： 網站、 SharePoint 網站，SSRS 報表 （這可讓您傳遞硬式編碼的參數） 的 URL。

![自訂 URL](media/rscreatekpi-related-content-custom-url.png)

當您現在按一下 KPI 時，URL 會顯示在相關內容。

您才可以新增一份行動報表或一個自訂的 URL。
