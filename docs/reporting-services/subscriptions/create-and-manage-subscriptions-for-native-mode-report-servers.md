---
title: "建立及管理原生模式報表伺服器的訂閱 |Microsoft 文件"
ms.custom: 
ms.date: 03/14/2017
...
8.  **路徑**： 輸入您要將報表傳遞的現有資料夾的通用命名慣例 (UNC) 路徑 (例如， \\ \\< servername\>\\< myreports 都\>)。 在路徑的開頭包含雙反斜線字元。 請勿在尾端指定反斜線。  
...
10. **認證**︰選取以使用檔案共用帳戶或特定的 Windows 使用者認證。 若報表管理員未設定檔案共用帳戶，則會停用 [使用檔案共用帳戶]  。 如需詳細資訊，請參閱[訂閱設定與檔案共用帳戶 &#40;組態管理員&#41;](../../reporting-services/install-windows/subscription-settings-and-a-file-share-account-configuration-manager.md)。 在**使用者名**和**密碼**文字方塊中，指定存取檔案共用，使用格式所需的認證*\<網域 >*\\*\<使用者名稱 >*使用者名稱。  
