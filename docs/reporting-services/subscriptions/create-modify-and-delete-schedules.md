...
description: 建立、修改和刪除 Reporting Services 共用排程。 若為原生模式，請使用入口網站或 Management Studio。 若為 SharePoint 模式，請使用服務應用程式。
ms.date: 06/11/2019
...
ms.locfileid: "80742267"
---
# <a name="create-modify-and-delete-schedules"></a>Create, Modify, and Delete Schedules
  使用此文章來了解如何建立、修改及刪除 [!INCLUDE[ssRSnoversion_md](../../includes/ssrsnoversion-md.md)] 共用排程。  若要管理原生模式的共用排程，請使用入口網站中的 [排程] 頁面或 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 中的 [共用排程] 資料夾。 如果是 SharePoint 模式，請使用 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 服務應用程式的管理頁面。

 您可以使用下列其中一種方法來判斷是否主動使用共用排程︰

-   **入口網站：** 在 [網站設定]  的 [排程]  索引標籤上，檢閱 [上次執行] 日期、[下次執行] 日期與 [狀態] 欄位中的值。 若因為排程已過期而不再執行，到期日就會顯示在 [狀態] 欄位中。 如需詳細資訊，請參閱 [Web portal (SSRS Native Mode)](../../reporting-services/web-portal-ssrs-native-mode.md)。

-   **SQL Server Management Studio：** 檢視指定共用排程的 [報表]  頁面。 此頁面會列出使用共用排程的所有報表和共用資料集。 如需詳細資訊，請參閱 [SQL Server Management Studio 中的 Reporting Services](../../reporting-services/tools/reporting-services-in-sql-server-management-studio-ssrs.md)。

-  **記錄：** 檢視報表執行記錄檔或追蹤記錄，以便判斷報表是否已在排程指定的時間執行。 如需詳細資訊，請參閱 [Reporting Services 記錄檔和來源](../../reporting-services/report-server/reporting-services-log-files-and-sources.md)。

## <a name="when-you-delete-a-shared-schedule"></a>當您刪除共用排程時
...
**注意：** 如果您不確定是否使用共用排程，請考慮在 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] (而非 Web 入口網站) 中刪除該排程。 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 提供的共用排程管理功能與報表管理員相同，但是它會提供額外的 [報表] 頁面，其中顯示每個使用此排程之報表的名稱。
 ...
 ## <a name="when-you-delete-a-report-specific-schedule"></a>當您刪除報表特定排程時
...
1. 在入口網站中，選取工具列中的 [設定]  ![ssrs_portal_settings_gear](../../reporting-services/subscriptions/media/ssrs-portal-settings-gear.png)。  

   >[!NOTE]  
   >如果無法使用 [設定]  ，您就沒有存取站台設定的權限。  

1. 從下拉式功能表中選取 [網站設定]  。
1. 選取 **[排程]** 索引標籤。
1. 選取 [+ 新增排程]  。 (若要修改現有的排程，請選取排程的名稱。)
1. 輸入排程的描述性名稱。
1. 選取 **[小時]** 、 **[天]** 、 **[週]** 或 **[月]** 。 按 **[一次]** ，即可建立只執行一次的排程。 當您指定排程的基礎時，其他選項會出現。
1. 選擇性地選取排程的開始日期。 預設值是目前的日期。 您也可以選擇較晚的日期，來延後排程的開始時間。
1. 選擇性地選取結束排程的日期。 排程會在此日期停止執行，但不會遭到刪除。
1. 選取執行排程的時間。
1. 選取 [確定]  。

### <a name="to-delete-a-shared-schedule-web-portal"></a>刪除共用排程 (入口網站)

1. 在入口網站中，選取工具列中的 [設定]  ![ssrs_portal_settings_gear](../../reporting-services/subscriptions/media/ssrs-portal-settings-gear.png)。
2. 從下拉式功能表中選取 [網站設定]  。
3. 選取 **[排程]** 索引標籤。
4. 選取要刪除之共用排程旁邊的核取方塊，然後選取 [刪除]  。
