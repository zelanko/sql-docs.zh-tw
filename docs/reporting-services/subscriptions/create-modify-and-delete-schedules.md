...
-   **入口網站：** 在 [站台設定] 的 [排程] 索引標籤上，檢閱 [上次執行] 日期、[下次執行] 日期與 [狀態] 欄位中的值。 若因為排程已過期而不再執行，到期日就會顯示在 [狀態] 欄位中。 如需詳細資訊，請參閱 [Web portal (SSRS Native Mode)](../../reporting-services/web-portal-ssrs-native-mode.md)。

-   **SQL Server Management Studio:** 檢視**報表**給定共用排程的頁面。 此頁面會列出使用共用排程的所有報表和共用資料集。 如需詳細資訊，請參閱 [SQL Server Management Studio 中的 Reporting Services](../../reporting-services/tools/reporting-services-in-sql-server-management-studio-ssrs.md)。
