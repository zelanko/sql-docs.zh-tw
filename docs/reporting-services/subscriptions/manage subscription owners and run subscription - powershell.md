---
title: "使用 PowerShell 變更及列出 Reporting Services 訂閱擁有者並執行訂閱 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
...
manager: "erikre"
---
# 使用 PowerShell 變更及列出 Reporting Services 訂閱擁有者並執行訂閱
...
-   觸發要執行的特訂訂閱時，才需要在最後一個指令碼中使用 [ReportingService2010.FireEvent](http://technet.microsoft.com/library/reportservice2010.reportingservice2010.fireevent.aspx) (機器翻譯) 方法。 若您不打算使用該指令碼，可以跳過 FireEvent 方法所需的權限需求。  
  
 **原生模式：**  
  
-   列出訂閱：(報表的 [ReportOperation Enumeration](../Topic/ReportOperation%20Enumeration.md)，且使用者是訂閱擁有者)，或是 ReadAnySubscription。  
...
-   列出訂閱：ManageAlerts 或 (報表的 [CreateAlerts](https://technet.microsoft.com/library/microsoft.sharepoint.spbasepermissions.aspx)，且使用者是訂閱擁有者，同時訂閱為定時的訂閱)。  
...
> [!TIP]  
>  若要以 SharePoint 模式確認網站 URL，請使用 SharePoint Cmdlet **Get-SPSite**。 如需詳細資訊，請參閱 [Get-SPSite](http://technet.microsoft.com/library/ff607950\(v=office.15\).aspx)。  
