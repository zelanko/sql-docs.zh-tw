...
2.  選取圖表頂端的 [圖表標題] 這幾個字，然後輸入：**2014 和 2015 年銷售額**。  
...
3.  在 [主資料夾] 索引標籤 > [數值] 群組 > [貨幣]。 水平軸標籤就會變更為貨幣。  
  
3.  (選擇性) 移除小數位數。 在 [貨幣] 按鈕附近按兩次 [減少小數位數] 按鈕。  
  
4.  以滑鼠右鍵按一下水平軸，然後按一下 [水平軸屬性]。  
  
5.  在 [數值] 索引標籤上，選取 [值的顯示單位: 千]。  
  
6.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  

8.  以滑鼠右鍵按一下水平軸，然後選取 [顯示軸標題]。
  
7.  在 [軸標題] 方塊中，輸入**以千為單位的銷售額**，然後按 Enter。  
...
6.  在 [運算式] 中，輸入 **[Sum(SalesYear2015)]**。 這樣會建立基礎運算式 `=Sum(Fields!SalesYear2015.Value)`，如果您按一下 [fx] 按鈕可以看到此運算式。  
  
7.  確認資料類型是 [文字]。  
  
8.  在 [運算子] 中，從下拉式清單選取 [Top N]。  
