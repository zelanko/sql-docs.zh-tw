---
title: "利用 rs.exe 公用程式與 Web 服務編寫指令碼 | Microsoft Docs"
ms.custom: ""
ms.date: "03/14/2017"
ms.prod: "sql-server-2016"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "reporting-services-sharepoint"
  - "reporting-services-native"
ms.tgt_pltfrm: ""
ms.topic: "article"
helpviewer_keywords: 
  - "Web 服務 [Reporting Services], 指令碼"
  - "rs 公用程式"
  - "XML Web 服務 [Reporting Services], 指令碼"
  - "報表伺服器 Web 服務, 指令碼"
  - "指令碼 [Reporting Services], Web 服務"
ms.assetid: 0ec5ac6e-b3cf-49cd-96f6-6b4b7dc29982
caps.latest.revision: 43
author: "guyinacube"
ms.author: "asaxton"
manager: "erikre"
caps.handback.revision: 43
---
# 利用 rs.exe 公用程式與 Web 服務編寫指令碼
  開發人員與報表伺服器管理員可以在報表伺服器上，透過使用 **rs** 公用程式 (RS.exe) 來執行作業。 利用這個公用程式，您可以使用以 [!INCLUDE[msCoName](../../includes/msconame-md.md)] [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)]撰寫的指令碼，透過程式設計方式管理報表伺服器。  
  
 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 指令碼可用於執行任何報表伺服器 Web 服務作業。 使用指令碼時，可以將安全性複製到伺服器上的多個報表、新增和刪除項目、將報表伺服器項目從一部伺服器複製到另外一部或多部伺服器。 如需指令碼環境的詳細資訊，請參閱[執行 Reporting Services 指令碼檔案](../../reporting-services/tools/run-a-reporting-services-script-file.md)。 指令碼檔案採用特定格式，並以 [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] .NET 撰寫。 如需詳細資訊，請參閱[格式化 Reporting Services 指令碼檔案](../../reporting-services/tools/format-a-reporting-services-script-file.md)。  
  
 如需指令碼範例，請參閱下列主題：  
  
 [Sample Reporting Services rs.exe Script to Copy Content between Report Servers](../../reporting-services/tools/sample-reporting-services-rs-exe-script-to-copy-content-between-report-servers.md)。  
  
 [SQL Server Reporting Services 產品範例](http://go.microsoft.com/fwlink/?LinkId=177889)。  
  
## 請參閱＜  
 [編寫部署和管理工作的指令碼](../../reporting-services/tools/script-deployment-and-administrative-tasks.md)   
 [報表伺服器 Web 服務](../../reporting-services/report-server-web-service/report-server-web-service.md)   
 [技術參考 &#40;SSRS&#41;](../../reporting-services/technical-reference-ssrs.md)   
 [RS.exe 公用程式 &#40;SSRS&#41;](../../reporting-services/tools/rs-exe-utility-ssrs.md)  
  
  