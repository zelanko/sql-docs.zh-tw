...
|設定報表伺服器電子郵件。|撰寫可對 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] WMI 提供者執行的自訂程式碼。 提供者支援電子郵件組態設定的子集。<br /><br /> 雖然 RSReportServer.config 檔案包含所有設定，但請不要以自動方式使用檔案。 亦即，請不要使用批次檔複製檔案到其他報表伺服器。 每個組態檔都包含適用於目前執行個體的值。 這些值對其他報表伺服器執行個體無效。<br /><br /> 如需有關設定的詳細資訊，請參閱[電子郵件設定-Reporting Services 原生模式 （組態管理員）](../install-windows/e-mail-settings-reporting-services-native-mode-configuration-manager.md)。|  
