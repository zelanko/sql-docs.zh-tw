...
4.  在 [選擇資料集] 頁面上按一下 [建立資料集]，然後按一下 [下一步]。  
...
3.  在 [主資料夾] 索引標籤 > [數值] 群組中，按一下 [貨幣] 按鈕。 這些軸標籤就會變更為顯示貨幣格式。  
  
4.  按兩次 [減少小數位數] 按鈕，以顯示四捨五入到最接近之美金的數字。  
  
5.  以滑鼠右鍵按一下垂直軸 > [垂直軸屬性]。  
  
6.  在 [數值] 索引標籤上，請注意，[類別目錄] 方塊中已選取 [貨幣]，而且 [小數位數] 已經是 [0] (零)。  
...
3.  以滑鼠右鍵按一下位於 [值] 區域中的 [Sum(Sales)] 欄位，然後按一下 [新增導出數列]。  
