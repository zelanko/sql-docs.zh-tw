...
4.  在**選擇資料集 頁面**，按一下 **建立資料集**，然後按一下 **下一步**。  
...
1.  在**選擇圖表類型**頁面上，直條圖是預設的圖表類型。 按一下 **[下一步]**。  
  
2.  在**排列圖表欄位**頁面上，將 [salesdate] 欄位**類別**。 類別目錄會顯示在水平軸上。  
  
3.  拖曳至 [銷售量] 欄位**值**。 **值**方塊會顯示 sum （sales），因為每個日期彙總銷售總計值的總和。 值會顯示在垂直軸上。  
...
3.  在**數目**索引標籤的**類別**，選取**日期**。  
  
5.  在**類型**方塊中，選取**31 Jan 2000**。  
  
6.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
7.  在 首頁 索引標籤中，按一下 **執行**預覽報表。  
...
## <a name="4-rotate-the-axis-labels-on-the-horizontal-axis"></a>4.旋轉軸標籤的水平軸  
...
5.  在**標籤**索引標籤，下方**變更軸標籤自動調整選項**，選取**停用自動調整**。  
...
3.  在下**配置及位置**，選取不同的位置。 例如，選取下層中間選項。  
  
    當圖例位於圖表的頂端或底部時，圖例的配置就會從垂直變更為水平。 您可以選取不同的版面配置中**配置**方塊。  
  
4.  [!INCLUDE[clickOK](../includes/clickok-md.md)]  
  
5.  (選擇性) 因為這個教學課程只有一個類別目錄，所以圖表不需要圖例。 若要移除它，以滑鼠右鍵按一下圖例 >**刪除圖例**。  
...
2.  選取的文字**圖表標題**在圖表的頂端，然後輸入**商店銷售訂單總計**。  
...
3.  在**首頁** 索引標籤 >**數目**群組中，按一下**貨幣** 按鈕。 這些軸標籤就會變更為顯示貨幣格式。  
  
4.  按一下**減少小數位數**按鈕兩次，顯示四捨五入到最接近之美元的數字。  
  
5.  以滑鼠右鍵按一下垂直軸 >**垂直軸屬性**。  
  
6.  在**數目**索引標籤上，請注意，**貨幣**中已選取**類別** 方塊中，和**小數位數**已經**0** （零）。  
  
7.  請檢查**值的顯示單位**。 **千分位**已選取。  
...
10. 取代文字中的**標題文字**欄位**銷售總計 （以千為單位）**。 您也可以指定各種有關如何格式化標題的選項。  
...
## <a name="8-show-all-the-labels-on-the-horizontal-x-axis"></a>8.水平 (x) 軸上顯示所有標籤
...
3.  以滑鼠右鍵按一下**[sum （sales)]**欄位**值**區域中，然後按一下 **加入導出數列**。  
...
5.  在**設定公式參數**，如**期間**，選取**4**。  
  
6.  在**框線**索引標籤的**線條寬度**，選取**3pt**。  
...
5.  選取**年 1 月到 12 月 2015年**，然後在**首頁** 索引標籤 >**字型**區段 > 將字型大小設定為**10**。  
  
6.  （選擇性）您可能需要進行**標題**高度，才能容納兩行文字的文字方塊。 當您按下邊緣的中間時，會下拉雙箭號。 而且，您可能需要拖曳圖表的頂端，讓標題不重疊。  
...
您已成功完成「將直條圖加入至報表」教學課程。 若要了解有關圖表的詳細資訊，請參閱[圖表 &#40;報表產生器及 SSRS &#41;](../reporting-services/report-design/charts-report-builder-and-ssrs.md)和[走勢圖和資料橫條 &#40;報表產生器及 SSRS &#41;](../reporting-services/report-design/sparklines-and-data-bars-report-builder-and-ssrs.md).  
