---
title: "使用共用資料集 (入口網站) | Microsoft Docs"
ms.custom: ""
ms.date: "12/09/2016"
...
> [!NOTE] 如果您在資料夾中沒有看到共用資料集，請確定您正在檢視資料集。 您可以從入口網站右上角的功能表列選取 [檢視]。 請確定已選取 [資料集]。  
...
> [!NOTE] 設定到期時間不會重新整理快取。 若沒有快取重新整理計劃，則會在資料集下次執行的時候重新整理資料。  
...
> [!NOTE] 必須正在執行 SQL Server Agent 才能建立快取重新整理計劃。  
