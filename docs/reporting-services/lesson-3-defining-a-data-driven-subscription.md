...
description: 使用 Reporting Services 入口網站的資料驅動訂閱頁面，連線至訂閱資料來源，並建置可擷取訂閱資料的查詢。
ms.date: 06/06/2019
...
**寫入模式**| 輸入值| 自動遞增    
**副檔名** |輸入值 |True
**使用者名稱** | 輸入值 | 輸入網域使用者帳戶。 輸入格式為：\<domain>\\\<account>。 使用者帳戶必須擁有您設定之路徑的權限。 
**密碼** | 輸入值 | 輸入密碼
**使用檔案共用帳戶** | 輸入值 | False
