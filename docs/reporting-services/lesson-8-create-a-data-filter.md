...
2.  選取 tablix 中的資料行標題，會出現在資料行標題上方的灰色資料格上按一下滑鼠右鍵，然後選取**Tablix 屬性**。  
  
3.  選取 **篩選**頁面，然後再選取**新增**。  
  
4.  在**運算式**欄位中，選取**ProductID**從下拉式清單。 這是要套用篩選的資料行。  
  
5.  選取 equal (**=**) 中的運算子**運算子**下拉式清單。  
  
6.  選取 [運算式] 按鈕旁邊**值**欄位中，選取**參數**中**類別**區域中，然後再按兩下**productid**中**值**區域。 **設定運算式對象： 值**欄位現在應該會包含運算式類似於**= 參數 ！ productid。值**。  
  
7.  選取**確定，**和**確定**中再次**Tablix 屬性** 對話方塊。  
...
2.  ReportViewer 控制項中，以滑鼠右鍵按一下，然後選取**屬性**。  
