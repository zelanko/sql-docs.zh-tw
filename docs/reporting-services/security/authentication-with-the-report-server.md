...
|RSWindowsBasic|基本|否|基本驗證定義在 HTTP 通訊協定中，而且只能用於驗證報表伺服器的 HTTP 要求。<br /><br /> 認證會以 base64 編碼的形式傳入 HTTP 要求中。 如果您使用基本驗證，請利用傳輸層安全性 (TLS) (先前稱為安全通訊端層 (SSL)) 來加密使用者帳戶資訊，然後再透過網路傳送該資訊。 SSL 提供了透過 HTTP TCP/IP 連接將連接要求從用戶端傳送到報表伺服器的加密通道。 如需詳細資訊，請參閱 [TechNet 網站上的](https://go.microsoft.com/fwlink/?LinkId=71123) 使用 SSL 加密機密資料 [!INCLUDE[msCoName](../../includes/msconame-md.md)] 。|  
