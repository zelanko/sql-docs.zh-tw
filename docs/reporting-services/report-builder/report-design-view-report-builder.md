---
title: "報表設計檢視 （報表產生器） |Microsoft 文件"
ms.custom: 
ms.date: 03/01/2017
...
-   報表主體的屬性，例如框線和填滿色彩，以滑鼠右鍵按一下設計介面上任何報表項目中，外部的白色區域，然後按一下**主體屬性**。  
  
-   頁首和頁尾的屬性，例如框線和填滿色彩，以滑鼠右鍵按一下設計介面，在頁首或頁尾區域中，任何報表項目中，外部的白色區域，然後按一下**標頭屬性**或**頁尾屬性**。  
  
-   報表本身，例如版面設定，以滑鼠右鍵按一下設計介面周圍的灰色區域，然後按一下屬性**報表屬性**。  
  
-   以滑鼠右鍵按一下它們，然後按一下報表項目的屬性**屬性**。  
  
 如需使用鍵盤在設計介面上操作項目的資訊，請參閱[鍵盤快速鍵 &#40;報表產生器 &#41;](../../reporting-services/report-builder/keyboard-shortcuts-report-builder.md)  
...
> [!NOTE]  
>  您可以使用**新增**按鈕，將新項目加入至 [報表資料] 窗格。 您可以從相同的資料來源或其他資料來源，將多個資料集加入至報表。 您可以從報表伺服器加入共用資料集。 若要從相同的資料來源加入新的資料集，以滑鼠右鍵按一下資料來源，然後**加入資料集**。  
...
##  <a name="ReptPartGallery"></a> 報表組件庫  
 ...
 按一下**報表組件**開啟報表組件庫的 [插入] 索引標籤上。 您可以在這裡搜尋報表組件，以加入報表中。 您可以依下列方式篩選報表組件：報表組件的全部或部分名稱、建立者、上次修改報表組件者、上次修改時間、儲存位置，或報表組件的類型。 例如，您可以依其中一個同事，搜尋上星期建立的所有圖表。  
...
##  <a name="RunMode"></a> 在執行模式中預覽報表  
 ...
 當您按一下**執行**，報表產生器連接到報表資料來源、 您的電腦上的資料會快取、 結合資料和配置，然後轉譯報表在 HTML 檢視器中的。 在您繼續設計報表時，可以視需要不斷執行報表。 當您對報表感到滿意時，就可以將報表儲存至報表伺服器，具有適當權限的其他個人可以在其中檢視您的報表。  
...
### <a name="running-a-report-with-parameters"></a>使用參數執行報表  
 當您執行報表時，系統會自動處理報表。 如果報表包含參數，所有參數都必須具有預設值，然後報表才能自動執行。 參數沒有預設值，當您執行報表時，如果您需要選擇參數的值，然後按一下**檢視報告**[執行] 索引標籤上。 如需詳細資訊，請參閱[報表參數 &#40;報表產生器和報表設計師&#41;](../../reporting-services/report-design/report-parameters-report-builder-and-report-designer.md)。  
  
### <a name="print-preview"></a>預覽列印  
 當您在執行模式中預覽報表時，它就類似於以 HTML 產生的報表。 這個預覽並不是 HTML，不過報表的配置及分頁與 HTML 的輸出相似。 您可以切換到預覽列印模式，以變更檢視來顯示列印的報表。 按一下**預覽列印**按鈕**執行** 索引標籤。 所顯示的報表就如同在實際頁面上所看到的。 這個檢視與影像和 PDF 轉譯延伸模組所產生的輸出類似。 預覽列印並不是影像或 PDF 檔案，不過報表的配置及分頁與這些格式的輸出相似。  
