---
title: "教學課程： 建立快速圖表報表離線 （報表產生器） |Microsoft 文件"
ms.custom: 
ms.date: 05/30/2017
...
### <a name="using-xml-data-for-this-tutorial"></a>在此教學課程中使用 XML 資料  
 您可以使用從本主題複製的 XML 資料，並將它貼入精靈中。 您不需要連線到[!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)]報表伺服器以原生模式或 SharePoint 整合模式，而且您不需要存取的 SQL Server 執行個體。  
...
### <a name="using-a-includetsqlincludestsql-mdmd-query-that-contains-data-for-this-tutorial"></a>針對此教學課程使用包含資料的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 查詢  
 您可以從本主題複製包含資料的查詢，並將它貼入精靈中。 您將需要能夠以唯讀方式存取任何資料庫的 SQL Server 和認證的執行個體名稱。 教學課程中的資料集查詢會使用常值的資料，但必須傳回中繼資料所需的報表資料集的 SQL Server 執行個體所處理的查詢。  
...
1.  從[Web 入口網站，或 SharePoint 整合模式中的報表伺服器，或從您的電腦](../../reporting-services/report-builder/start-report-builder.md) 啟動報表產生器 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 。  
     ...
     如果**入門**對話方塊沒有出現，請按一下**檔案** >**新增**。 [新報表或資料集] 對話方塊大部分的內容和 [使用者入門] 對話方塊相同。  
...
11. （選擇性）按一下**執行**按鈕 (**！**) 若要查看您的圖表將依據的資料。  
...
14. 在**排列圖表欄位**頁面中，按兩下**銷售**欄位**可用的欄位**方塊。  
  
     請注意，它會自動移至 [值] 方塊，因為它是數值。  
  
     ![報表產生器精靈排列欄位](../../reporting-services/report-builder/media/rb-wizarrangefields.png "報表產生器精靈排列欄位")  
  
15. 拖曳**FullName**欄位從**可用的欄位**方塊**類別**方塊 (或按兩下它，就會移至**類別**方塊)，然後按一下**下一步**。  
  
     [預覽] 頁面會顯示有代表性資料的新圓形圖。 圖例會顯示成 Full Name 1、Full Name 2，以此類推，而非銷售人員的名稱，而且圓形圖配量的大小也不正確。 這只是要讓您了解報表即將呈現的外觀而已。  
  
     ![報表產生器的新圖表預覽](../../reporting-services/report-builder/media/rb-newchartpreview.png "報表產生器] 的 [新圖表預覽")  
  
16. 按一下 **[完成]**。  
  
     現在，您會在 [設計檢視] 中看到新的圓形圖報表，仍有代表性的資料。  
  
     ![報表產生器在設計檢視中的新圓形圖](../../reporting-services/report-builder/media/rb-newpiedesign.png "報表產生器在設計檢視中新的圓形圖")  
  
17. 若要查看實際的圓形圖，請在功能區的 [主資料夾] 索引標籤上，按一下 [執行]。  
  
     ![報表產生器的新圖表執行](../../reporting-services/report-builder/media/rb-newchartrun.png "報表產生器新的圖表執行")  
...
11. 在**排列圖表欄位**頁面中，按兩下**銷售**欄位**可用的欄位**方塊。  
  
     請注意，它會自動移至 [值] 方塊，因為它是數值。  
  
12. 拖曳**FullName**欄位從**可用的欄位**方塊**類別**方塊 (或按兩下它，就會移至**類別**方塊)，然後按一下**下一步**。  
...
2. 選取，請在 [屬性] 窗格的標題變更**色彩**至**黑色**和**FontSize**至**12pt**。
...
3.  在**標示資料**方塊中，輸入**#PERCENT {P0}**。  
  
     **{P0}** 提供您沒有小數位數的百分比。 如果您只輸入**#PERCENT**，您的數字會有兩個小數位數。 **#PERCENT**是關鍵字，為您執行計算或函數，則還有許多其他。  
...
##  <a name="WhatsNext"></a> 下一步  
 ...
 若要進行其他教學課程，您需要 SQL Server 和認證的執行個體的名稱能夠以唯讀方式存取任何資料庫。 系統管理員可能也會為您進行該設定。  
