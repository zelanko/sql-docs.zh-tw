...
# 教學課程：離線建立快速圖表報表 (報表產生器)
  在本教學課程中，您會使用精靈在 [!INCLUDE[ssRBnoversion](../../includes/ssrbnoversion-md.md)] 的 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 分頁報表中建立圓形圖。 然後您會加入百分比，然後稍微修改圓形圖。 
...
## 必要條件  
 不論您使用的是 XML 資料或 [!INCLUDE[tsql](../../includes/tsql-md.md)] 查詢，都需要具備 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)][!INCLUDE[ssRBnoversion](../../includes/ssrbnoversion-md.md)] 的存取權。 您可以在原生模式或 SharePoint 整合模式中從 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 報表伺服器啟動 [!INCLUDE[ssRBnoversion](../../includes/ssrbnoversion-md.md)]，或者從 Microsoft 下載中心下載 [!INCLUDE[ssRBnoversion](../../includes/ssrbnoversion-md.md)]。 如需詳細資訊，請參閱[安裝報表產生器](../../reporting-services/install-windows/install-report-builder.md)。  
  
##  <a name="TwoWays"></a> 進行此教學課程的兩種方式  
  
-   [使用 XML 資料建立圓形圖](#CreatePieChartXML)  
  
-   [使用包含資料的 Transact\-SQL 查詢建立圓形圖](#CreatePieQueryData)  
  
### 在此教學課程中使用 XML 資料  
 您可以使用從本主題複製的 XML 資料，並將它貼入精靈中。 您不需要連接至原生模式或 SharePoint 整合模式中的 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 報表伺服器，也不需要存取 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 的執行個體。  
  
 [使用 XML 資料建立圓形圖](#CreatePieChartXML)  
  
### 針對此教學課程使用包含資料的 [!INCLUDE[tsql](../../includes/tsql-md.md)] 查詢  
...
1.  從 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] Web 入口網站，或 SharePoint 整合模式中的報表伺服器，或從您的電腦[啟動報表產生器](../../reporting-services/report-builder/start-report-builder.md)。  
...
15. 將 [FullName] 欄位從 [可用的欄位] 方塊拖曳至 [類別目錄] 方塊中\(或是按兩下該欄位，它就會移至 [類別目錄] 方塊\)，然後按一下 [下一步]。  
...
12. 將 [FullName] 欄位從 [可用的欄位] 方塊拖曳至 [類別目錄] 方塊中\(或是按兩下該欄位，它就會移至 [類別目錄] 方塊\)，然後按一下 [下一步]。  
...
3.  在 [標籤資料] 方塊中，輸入 **\#PERCENT{P0}**。  
  
     **{P0}** 提供您沒有小數位數的百分比。 如果您只輸入 **\#PERCENT**，數字將具有兩個小數位數。 **\#PERCENT** 是為您執行計算或函數的關鍵字；還有其他關鍵字可以使用。  
...
按一下 **[確定]**。     
 ...
 如需自訂圖表標籤和圖例的詳細資訊，請參閱 [在圓形圖上顯示百分比值 &#40;報表產生器及 SSRS&#41;](../../reporting-services/report-design/display-percentage-values-on-a-pie-chart-report-builder-and-ssrs.md) 和[變更圖例項目的文字 &#40;報表產生器及 SSRS&#41;](../../reporting-services/report-design/change-the-text-of-a-legend-item-report-builder-and-ssrs.md)。  
