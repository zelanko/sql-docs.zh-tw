...
description: 了解在報表產生器中，如何在報表定義中或報表發佈之後，設定參數並執行參數化報表。
ms.date: 03/07/2017
...
# <a name="set-parameters-on-a-published-report---sharepoint-integrated-mode"></a>在已發行的報表上設定參數 - SharePoint 整合模式
 ...
 這些屬性可以在報表定義中設定，或是在已經發行報表後設定。 雖然可在已發佈的報表中修改某些參數屬性來變更值及顯示屬性，但無法變更參數名稱或資料類型。 參數名稱與資料類型僅能透過報表定義中的報表作者變更。  
