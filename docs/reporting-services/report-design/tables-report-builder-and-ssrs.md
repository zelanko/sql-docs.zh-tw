---
title: "資料表 （報表產生器及 SSRS） |Microsoft 文件"
ms.custom: 
ms.date: 03/07/2017
...
>  您只能在報表產生器中使用此精靈。  
 ...
 資料列的資料列控制代碼會顯示詳細資料符號 (![3 平行線的詳細資料列與資料列控制代碼](../../reporting-services/report-design/media/rs-icontablix-detailsrow.gif "3 平行線的詳細資料列與資料列控制代碼"))。 若要在這些資料列上顯示資料，將欄位從 [報表資料] 窗格拖曳到標頭資料列或詳細資料列的資料表資料格中。 兩個資料列會同時填入。 若要加入其他資料行，將欄位拖曳到資料表中，直到您看到插入點為止。 將資料集欄位加入到資料表之後，您可以變更日期和貨幣的預設格式來控制它們顯示在報表中的方式。 下圖顯示包含這些欄位的資料表資料區域：Date、Order、Product、Qty 和 Line Total。  
  
 ![設計，具有在粗體資料行標頭資料表](../../reporting-services/report-design/media/rs-basictabledetailsformatteddesign.gif "設計中，資料表具有在粗體資料行標頭")  
...
### <a name="adding-totals-for-detail-data"></a>加入詳細資料的資料表  
 ...
 ![總計資料列加入資料表](../../reporting-services/report-design/media/rs-basictabledetailstotaldesign.gif "總計資料列加入資料表")  
  
 在預覽中，報表會針對資料集查詢結果集中的每個資料列顯示一次標頭資料列和詳細資料列，而且該報表會顯示總計資料列。 下圖顯示資料表的最後幾個資料列，包括總計資料列。  
  
 ![使用資料表和總計資料列預覽](../../reporting-services/report-design/media/rs-basictabledetailstotalpreview.gif "Preview 搭配資料表和總計資料列")  
...
##  <a name="RemovingHidingRows"></a> 移除或隱藏詳細資料列  
 ...
 ![設計檢視，資料表包含兩個群組，沒有詳細資料](../../reporting-services/report-design/media/rs-basictablegroupsdrilldownnodetailsdesign.gif "設計檢視，資料表包含兩個群組，沒有詳細資料")  
...
>  確認移除詳細資料資料列之後，每個資料格中的運算式都會在適當時指定彙總運算式。 如有必要，編輯運算式以指定所需的彙總函式。  
 ...
 ![預覽，具有群組的資料表，但沒有詳細資料列](../../reporting-services/report-design/media/rs-basictablegroupsnodetailspreview.gif "預覽，具有群組的資料表，但沒有詳細資料列")  
 ...
 ![預覽、 資料表與已展開向下鑽研節點](../../reporting-services/report-design/media/rs-basictablegroupsdrilldownpreview.gif "預覽，具有已展開向下鑽研節點的資料表")  
