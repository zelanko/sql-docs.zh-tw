---
title: "加入、 變更或刪除地圖或地圖圖層 （報表產生器及 SSRS） |Microsoft 文件"
ms.custom: 
ms.date: 03/01/2017
...
# <a name="add-change-or-delete-a-map-or-map-layer-report-builder-and-ssrs"></a>加入、變更或刪除地圖或地圖圖層 (報表產生器及 SSRS)
 ...
 加入、移除或變更圖層選項的一個最簡單的方法，就是使用地圖圖層精靈。 您也可以從 [地圖] 窗格手動變更選項。 若要顯示**對應**窗格中，按一下報表設計介面上的地圖。 下圖顯示此窗格的幾個部分：  
  
 ![rsMapLayerZone](../../reporting-services/report-design/media/rsmaplayerzone.gif "rsMapLayerZone")  
  
 地圖圖層是以它們顯示在 [地圖] 窗格中的順序，由下而上繪製。 在上圖中，圖格圖層會最先繪製，而多邊形圖層則是最後繪製。 稍後繪製的圖層可能會隱藏圖層上先前繪製的地圖元素。 您可以使用 [地圖] 窗格工具列上的方向鍵來變更圖層的順序。 若要顯示或隱藏圖層，請切換可見性圖示。 您可以在變更圖層透明度**可視性**頁面**圖層資料**屬性 對話方塊。  
...
-   執行地圖圖層精靈。 這個精靈可讓您變更您使用地圖圖層精靈所建立的圖層選項。 在 [地圖] 窗格中，以滑鼠右鍵按一下圖層，和在工具列上，按一下圖層精靈按鈕 (![rs_IconMapLayerWizard](../../reporting-services/media/rs-iconmaplayerwizard.gif "rs_IconMapLayerWizard"))。  
...
2.  在工具列上，按一下 **加入圖層**按鈕，然後從下拉式清單中，按一下您想要加入的圖層類型：**點**，**列**，或**多邊形**。  
  
    > [!NOTE]  
    >  雖然您可以手動加入地圖圖層並加以設定，我們建議您最好使用地圖圖層精靈來新增圖層。 若要啟動此精靈，從 [地圖] 窗格工具列，按一下圖層精靈按鈕 (![rs_IconMapLayerWizard](../../reporting-services/media/rs-iconmaplayerwizard.gif "rs_IconMapLayerWizard"))。  
  
3.  以滑鼠右鍵按一下圖層，然後按一下 [圖層資料]。  
  
4.  在**使用空間資料來自**，選取空間資料的來源。 這些選項會隨著您選取的項目而不同。  
  
     如果您想要從此圖層上的報表視覺化分析資料，請執行下列操作：  
  
    1.  按一下 **[分析資料]**。  
  
    2.  在**分析資料集**，按一下包含分析資料與符合欄位建立分析資料與空間資料之間的關聯性的資料集的名稱。  
...
3.  在**使用空間資料來自**，選取**資料內嵌在報表中**。  
  
4.  若要從現有的報表載入地圖元素，或建立 ESRI 檔案為基礎的地圖元素，請按一下**瀏覽**，指向檔案，然後按一下**開啟**。 地圖元素便會內嵌在此報表定義中。 您所指向的空間資料必須符合圖層類型。 例如，若是點圖層，您必須指向指定點座標組的空間資料。  
  
5.  在**空間欄位**，指定包含空間資料欄位的名稱。 您可能需要從空間資料來源決定此名稱。  
...
3.  在**使用空間資料來自**，選取**ESRI Shapefile 的連結**。  
  
4.  在**檔案名稱**，輸入 ESRI 形狀檔的位置，或按一下**瀏覽**以選取 ESRI 形狀檔。  
...
3.  在**使用空間資料來自**，選取**資料集中的空間欄位**。  
  
4.  在**資料集名稱**，按一下包含您想要之空間資料的報表中的資料集的名稱。  
...
4.  在**磚選項**，選取圖格樣式。 如果可以使用 Bing 地圖底圖，設計介面上的圖層會更新成您選取的樣式。  
...
1.  設定 [可見性] 選項來控制地圖圖層的顯示。  
    ...
    -   地圖圖層 窗格中，以滑鼠右鍵按一下圖層來選取它，並在地圖圖層 工具列中，按一下 屬性，開啟**地圖圖層屬性**。  
  
    -   按一下 **[可見性]**。  
  
    -   在 圖層可見性，選取 **顯示或隱藏依據縮放值**。  
