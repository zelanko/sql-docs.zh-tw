...
-   執行地圖圖層精靈。 這個精靈可讓您變更您使用地圖圖層精靈所建立的圖層選項。 在 [地圖] 窗格中，以滑鼠右鍵按一下圖層，和在工具列上，按一下 圖層精靈按鈕 (![rs_IconMapLayerWizard](../../reporting-services/media/rs-iconmaplayerwizard.gif "rs_IconMapLayerWizard"))。  
...
2.  在工具列上，按一下 [加入圖層] 按鈕，然後從下拉式清單中按一下您要加入的圖層類型：[點]、[線條] 或 [多邊形]。  
    ...
    > [!NOTE]  
    >  雖然您可以手動加入地圖圖層並加以設定，我們建議您最好使用地圖圖層精靈來新增圖層。 若要啟動此精靈，從 [地圖] 窗格工具列，按一下 圖層精靈按鈕 (![rs_IconMapLayerWizard](../../reporting-services/media/rs-iconmaplayerwizard.gif "rs_IconMapLayerWizard"))。  
