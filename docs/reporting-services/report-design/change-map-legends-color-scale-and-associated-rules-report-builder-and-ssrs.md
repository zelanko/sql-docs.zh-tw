...
description: 了解如何變更地圖圖例，以協助使用者在報表產生器中解讀地圖上的資料視覺效果。
ms.date: 03/07/2017
...
1.  在 [設計] 檢視中，以滑鼠右鍵按一下圖例，然後開啟 [ _\<report item>_ 屬性] 頁面。  
  
2.  在 [位置] 中，按一下指定讓圖例顯示在相對於檢視區的位置。  
  
3.  若要在檢視區外顯示圖例，請選取 [在檢視區外顯示 \<report item>]。  
...
2.  以滑鼠右鍵按一下其中包含所需資料的圖層，然後按一下 [ _\<map element type>_ 色彩規則]。  
...
3.  選取 [使用 \<rule type> 將資料視覺化] 選項。 若要使用分佈選項，您必須根據與圖層相關聯的分析資料，在 [分佈] 頁面上建立範圍。  
...
2.  以滑鼠右鍵按一下其中包含所需資料的圖層，然後按一下 [ _\<map element type>_ 規則]。  
  
3.  確認已選取 [使用 \<*rule type*> 將資料視覺化]。  
...
3.  選取要使用的色彩規則選項。 若要以地圖圖例或色階顯示項目，則必須選取其中一個 [使用 \<rule type> 將資料視覺化] 選項。  
