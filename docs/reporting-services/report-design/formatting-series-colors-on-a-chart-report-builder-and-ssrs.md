---
title: "格式化圖表 （報表產生器及 SSRS） 上的數列色彩 |Microsoft 文件"
ms.custom: 
ms.date: 03/03/2017
...
# <a name="formatting-series-colors-on-a-chart-report-builder-and-ssrs"></a>設定圖表上數列色彩的格式 (報表產生器及 SSRS)
 ...
 您可以選取新的調色盤，或從 [屬性] 窗格定義自訂調色盤。 如需詳細資訊，請參閱[定義色彩在圖表中使用的調色盤 &#40;報表產生器及 SSRS &#41;](../../reporting-services/report-design/define-colors-on-a-chart-using-a-palette-report-builder-and-ssrs.md).  
...
## <a name="using-custom-palettes"></a>使用自訂調色盤  
 如果要將自己的色彩套用至圖表，請使用自訂調色盤。 自訂調色盤可讓您將依照要在圖表上顯示色彩的順序，加入自己的色彩。 如果在設計階段時還不知道圖表中的序列數，則自訂調色盤特別有用。 如需詳細資訊，請參閱[定義色彩在圖表中使用的調色盤 &#40;報表產生器及 SSRS &#41;](../../reporting-services/report-design/define-colors-on-a-chart-using-a-palette-report-builder-and-ssrs.md).  
  
## <a name="using-a-color-fill-on-each-series"></a>在每個序列上使用色彩填滿  
 您也可以針對圖表上的每個序列指定色彩，在圖表上定義自己的色彩。 若要這樣做，請開啟**數列屬性**對話方塊及設定**色彩**屬性**填滿**。 這個方法會覆寫所有定義的調色盤。 一般來說，最好使用自訂調色盤來定義自己的色彩，因為可能要到處理報表之後，才會知道資料集中的序列數。  
