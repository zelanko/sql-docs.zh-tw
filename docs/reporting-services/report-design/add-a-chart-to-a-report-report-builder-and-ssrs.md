---
title: "將圖表加入至報表 (報表產生器及 SSRS) | Microsoft Docs"
ms.custom: ""
ms.date: "03/03/2017"
...
6.  將一或多個欄位加入到 [值] 區域。 這些資訊將會繪製在值軸上。  
  
7.  將群組欄位加入到 [類別群組] 區域。 當您將這個欄位加入至 [類別群組] 區域時，系統會自動為您建立群組欄位。 每一個群組都代表數列中一個資料點。  
  
8.  若要依據類別目錄摘要資料，請以滑鼠右鍵按一下資料欄位，再按一下 [數列屬性]。 在 [類別目錄] 方塊中，從下拉式清單選取類別目錄欄位。 [!INCLUDE[clickOK](../../includes/clickok-md.md)]  
