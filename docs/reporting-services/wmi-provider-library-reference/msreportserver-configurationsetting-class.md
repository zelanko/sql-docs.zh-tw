---
description: MSReportServer_ConfigurationSetting 類別
...
## <a name="example"></a>範例  
 若要執行下列程式碼，請新增您的伺服器名稱以取代每個 \<*servername*>。 將此路徑更新為指向您的安裝位置 (如果不是預設值的話)。 下列程式碼範例會逐一查看 *MSReportServer_ConfigurationSetting* 類別中的每個屬性，並且將每個屬性的名稱和值列印至主控台。  
