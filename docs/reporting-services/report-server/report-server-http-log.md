---
title: "報表伺服器 HTTP 記錄檔 |Microsoft 文件"
ms.custom: 
ms.date: 03/16/2017
...
|||  
|-|-|  
|檔案名稱|根據預設，檔案名稱是 ReportServerService_HTTP_\<時間戳記 >。 記錄檔。 您可以透過在 ReportingServicesService.exe.config 檔中修改 HttpTraceFileName 屬性，自訂檔案名稱的前置詞。 此時間戳記是以國際標準時間 (UTC) 為基礎。|  
|檔案位置|檔案位於 \Microsoft SQL Server\\*\<SQL Server 執行個體 >*services\logfiles。|  
