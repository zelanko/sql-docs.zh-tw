...
|SOAP 用戶端 Proxy||  
|UI 頁面||  
|Power View|已寫入 **LogClientTraceEvents** API 中的記錄項目。 這些項目來自於用戶端應用程式，包括適用於 [!INCLUDE[msCoName](../../includes/msconame-md.md)][!INCLUDE[SPS2010](../../includes/sps2010-md.md)] Enterprise Edition 的 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 增益集功能 [!INCLUDE[ssCrescent](../../includes/sscrescent-md.md)]。<br /><br /> 所有來自於 LogClientTraceEvents API 的記錄項目都會記錄在 “SQL Server Reporting Services” 的 **類別目錄** 和 “Power View” 的 **區域** 之下。<br /><br /> 使用 “Power View” 的區域所記錄的項目內容是由用戶端應用程式所決定。|  
|報表伺服器警示執行階段||  
|報表伺服器應用程式定義域管理員||  
|報表伺服器緩衝回應||  
|報表伺服器快取||  
...
##  <a name="bkmk_powershell"></a> 利用 PowerShell 檢視記錄檔  
 ![PowerShell 相關內容](../../analysis-services/instances/install-windows/media/rs-powershellicon.png "PowerShell 相關內容")您可以使用 PowerShell 從 ULS 記錄檔傳回與事件相關的 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 清單。 從 SharePoint 2010 管理命令介面輸入下列命令，從包含 "**sql server reporting services**" 的 ULS 記錄檔 UESQL11SPOINT-20110606-1530.log 傳回已篩選過的資料列清單：  
