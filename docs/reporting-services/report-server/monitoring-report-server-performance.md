...
helpviewer_keywords: 
  ...
  - "報表伺服器 [Reporting Services], 效能"
  - "計數器 [Reporting Services]"
  - "監視效能 [Reporting Services]"
  - "SQL Server Reporting Services, 效能"
...
-   監視已排程作業和報表傳遞的 **MSRS 2011 Windows 服務**和 **MSRS 2011 Windows 服務 SharePoint 模式**。 這些效能物件包含一組計數器集合，用來追蹤透過已排程的作業所起始的報表處理。 已排程的作業包括訂閱與傳遞、報表執行快照集，以及報表記錄。  
  
-   監視 HTTP 相關事件和記憶體管理的 **Reportserver:Service** 和 **ReportServerSharePoint:Service**。 這些計數器是 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 特有的，而且它們會追蹤報表伺服器的 HTTP 相關事件，例如要求、連接和登入嘗試。 這個效能物件也包含與記憶體管理相關的計數器。  
...
## 其他效能計數器  
 僅針對 **MSRS 2008 Web 服務**、**MSRS 2008 Windows 服務**和 **ReportServer:Service** 提供自訂的 [!INCLUDE[ssRSnoversion](../../includes/ssrsnoversion-md.md)] 效能計數器。 下列效能物件會提供報表伺服器的其他效能監視資料。  
