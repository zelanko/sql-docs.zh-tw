---
title: "第 4 課： 將資料表加入至報表 (Reporting Services) |Microsoft 文件"
ms.custom: 
ms.date: 05/23/2016
...
包含重複的基礎資料集中的資料列的項目稱為*資料區域*。 基本報表只有一個資料區域，但是您可以加入其他資料區域，例如，當您想要將圖表加入至表格式報表時。 加入資料區域之後，您可以將欄位加入到資料區域中。  
  
### <a name="to-add-a-table-data-region-and-fields-to-a-report-layout"></a>將資料表資料區域和欄位加入到報表配置中  
  
1.  在**工具箱**，按一下 **資料表**，然後按一下設計介面上，並拖曳滑鼠。 報表設計師會繪製一個資料表資料區域，而且在設計介面的中央包含三個資料行。 **工具箱**可能會顯示索引標籤上的左半部為**報表資料**窗格。 若要開啟**工具箱**，將指標移**工具箱** 索引標籤。 如果**工具箱**不是可見的**檢視**功能表上，按一下 **工具箱**。
  
     ![ssrs_ssdt_addtable](../reporting-services/media/ssrs-ssdt-addtable.png) 
  
  您也可以從設計介面中將資料表新增至報表。  以滑鼠右鍵按一下設計介面上，按一下**插入**，然後按一下 **資料表**。
2.  在**報表資料** 窗格中，展開資料集**AdventureWorksDataset**來顯示欄位。  
  
3.  拖曳*日期*欄位從**報表資料**窗格，即可在資料表中的第一個資料行。  
  
    當您將欄位放到第一個資料行時，會出現兩種情況。 首先，資料格會顯示欄位名稱，也稱為*欄位運算式*，括號括住： `[Date]`。 接著，資料行標頭值會自動加到標頭資料列中，就在欄位運算式的正上方。 依預設，這個資料行是欄位的名稱。 您可以選取標頭資料列文字，然後輸入新的名稱。  
  
4.  拖曳*順序*欄位從**報表資料**窗格，即可在資料表中的第二個資料行。  
...
![ssrs_ssdt_preview](../reporting-services/media/ssrs-ssdt-preview.png)  
    ...
    ![預覽，具有 5 個資料行資料表的詳細資料列](../reporting-services/media/rs-basictabledetailspreview.png "預覽，具有 5 個資料行資料表的詳細資料列")  
