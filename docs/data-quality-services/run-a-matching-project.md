...
3.  按一下 **[開啟資料品質專案]** ，在現有的資料品質專案中執行比對。 選取專案，然後按 **[下一步]**  (您可以按一下底下的專案或者 **最近的資料品質專案**。)如果您開啟已關閉的比對專案時，您將繼續進行比對專案活動已關閉的階段 (由 **狀態** 底下的專案名稱或專案資料表中資料行 **最近的資料品質專案**)。 如果您開啟已完成的比對專案時，您將會前往 **匯出** 頁面 （和您不能返回上一個螢幕擷取畫面）。  
...
4.  按一下 [ **加號 （+）** 控制項，將資料列加入至對應資料表或 **減號 （-）** 控制項移除資料列。  
  
5.  按一下 **[預覽資料來源]** 查看您選取之 SQL Server 資料表或檢視表中的資料，或者您選取的 Excel 工作表。  
  
6.  按一下 [ **檢視/選取複合定義域** 檢視知識庫中可用的複合定義域清單並選取適當的對應。  
...
1.  在 **比對] 頁面**, ，請選取 **重疊的叢集** 從下拉式清單，以便顯示樞紐記錄和隨後的記錄所有的叢集執行比對時，即使叢集的群組有共同的記錄。 選取 **[非重疊的叢集]** ，在執行比對時顯示與單一叢集擁有共同記錄的叢集。  
  
2.  按一下 [ **重新載入資料來源** （預設） 將資料從資料來源複製到暫存資料表，並重新建立索引時執行比對專案。 按一下 [ **先前的資料上執行** 執行比對專案，而不需要將資料複製到暫存資料表，並重新建立索引的資料。 當您初次執行比對專案，或是您在**[對應]** 頁面中變更對應，然後在隨後的快顯視窗中按一下 **[是]** 時， **[在先前的資料上執行]** 將會停用。 在這兩種情況下，您都必須重新建立索引。 如果比對專案尚未變更，就不需要重新建立索引。 針對之前的資料執行有助於提高效能。  
...
7.  如果您選取非樞紐記錄資料表中，按一下 **檢視詳細資料** 圖示 （或按兩下記錄），DQS 將會顯示 **符合分數詳細資料** 按兩下記錄及其樞紐記錄 （和其所有欄位中的值），會顯示快顯視窗之間，以及向下鑽研的相符分數分數的每個欄位的貢獻。 按兩下樞紐記錄將不會顯示此快顯視窗。  
  
8.  按一下 **[全部摺疊]** 圖示摺疊 **[比對結果]** 資料表中顯示的記錄，以便只包含樞紐記錄，而不是重複的記錄。 按一下 **[全部展開]** 展開 [比對結果] 資料表中顯示的記錄，以便包含所有重複的記錄。  
  
9. 若要拒絕比對結果中的記錄，請針對此記錄按一下 **[拒絕]** 核取方塊。  
  
10. 若要變更最低符合分數會決定相符的記錄必須具有要顯示的層級，請選取 **最小值。比對分數** 資料表右邊上方的圖示，然後輸入較高的數目。 最小符合分數預設為 80%。 為此活動按一下 **[重新整理]** ，變更資料表的內容。  
...
3.  選取核取方塊 **比對結果** 匯出比對結果 （請參閱上述說明），SQL Server 資料庫中的指定資料表或指定的.csv 或 Excel 檔案。 選取核取方塊 **生存結果** 生存結果 （請參閱上述說明） 匯出至 SQL Server 資料庫中的指定資料表或指定的.csv 或 Excel 檔案。  
...
7.  選取存活規則，如下所示：  
    ...
    -   選取 **記錄的樞紐分析表** （預設值），將存活記錄識別為 DQS 任意選擇的初始樞紐記錄。  
