...
2.  如果資料來源是 **[SQL Server]**，則請依照以下方式進行：  
    ...
    2.  在 **資料表/檢視表** 欄位中，選取資料表或檢視您想要分析，以建立知識庫。 這個資料表或檢視表應該是範例資料，而不是您執行資料清理或比對的整個來源資料庫。 文字方塊下拉式清單會列出可供選取之資料庫使用的資料表和檢視表。  
...
4.  在 **[對應]** 資料表中，將您想要執行知識探索的每一個來源資料行對應到知識庫中的定義域，如下所示：  
    ...
    1.  從下拉式清單中選取來源資料行建立對應 **來源資料行** 資料行以及空白資料列，然後從下拉式清單中選取網域 **網域** 相同的資料列，如果網域中的資料行。 如果定義域不存在，請按一下 **[建立定義域]** 或 **[建立複合定義域]** 來建立定義域。 如需相關資訊，請參閱 [Create a Domain Rule](../data-quality-services/create-a-domain-rule.md) 或 [Create a Composite Domain](../data-quality-services/create-a-composite-domain.md)。  
    ...
    3.  按一下 [ **檢視/選取複合定義域** 顯示已定義的複合定義域。 如果沒有已經定義的複合定義域，將無法使用此控制項。  
  
    4.  按一下 [ **預覽資料來源** 快顯視窗中顯示您在選取的資料來源中的所有資料 **資料表/檢視表** 或 **Excel 檔案** 文字方塊。  
  
5.  按 **[下一步]** ，繼續前往知識探索精靈的 **[探索]** 頁面。 您也可以選取下列項目：  
  
    -   按一下 **[取消]** ，結束知識探索活動，不儲存工作並返回 DQS 首頁。  
  
    -   按一下 **[關閉]** ，返回 DQS 首頁並儲存工作。 知識庫會鎖定，而且知識庫的知識基底資料表中的狀態 **開啟知識庫** 螢幕會變成 **探索-對應**。 在按一下 **[關閉]**之後，若要執行定義域管理活動，您必須按一下 **[開啟知識庫]** 畫面中的 **[知識探索]** ，進入 **[知識庫管理: 管理定義域詞彙]** 畫面，按一下 **[完成]**，然後按一下 **[是]** 發行知識庫，或按一下 **[否]** 儲存知識庫工作並結束。  
...
6.  按 **[下一步]** ，繼續前往知識探索精靈的 **[管理定義域值]** 頁面。 在此頁面上，您可以修改加入至知識庫定義域的知識。 您也可以選取下列項目：  
    ...
    -   按一下 **[關閉]** ，返回 DQS 首頁並儲存工作。 知識庫會鎖定，而且知識庫的知識基底資料表中的狀態 **開啟知識庫** 螢幕會變成 **探索-探索**。 在按一下 **[關閉]**之後，若要執行定義域管理活動，您必須按一下 **[開啟知識庫]** 畫面中的 **[知識探索]** ，進入 **[知識庫管理: 管理定義域詞彙]** 畫面，按一下 **[完成]**，然後按一下 **[是]** 發行知識庫，或按一下 **[否]** 儲存知識庫工作並結束。  
...
1.  在左邊的 **[定義域清單]** 窗格中，選取要設定定義域值的定義域。 您可以執行以下動作來修改顯示的值。  
    ...
    -   按一下 [ **顯示/隱藏定義域值變更記錄面板** 按鈕來顯示預覽快顯視窗底部的 [值表格，顯示最近變更網域值集合。  
  
2.  將 **[篩選]** 設定為 **[錯誤]**，尋找 Data Quality Services 已提議的任何更正。 確認此值事實上有錯誤，而且 **[更正為]** 資料行中的值是適當的。  
  
3.  將 **[篩選]** 設定為 **[所有值]** ，並確認值的狀態是適當的。 若要變更值的狀態，請選取值，以及 [ **將選取的定義域值設為更正** （核取） 按鈕， **將選取的定義域值設為錯誤** （十字符號） 按鈕，或 **將選取的定義域值設為無效** （三角形） 按鈕。  
  
4.  若要變更值的狀態，請依照以下方式繼續進行：  
  
    1.  **將選取的定義域值設為更正**︰ 若要更正錯誤或無效變更值的狀態，選取值，以及 [ **將選取的定義域值設為更正** （檢查） 從圖示列的向下箭號，或從 [類型] 下拉式清單。 如果錯誤或無效值與更正值群組在一起，請在作業之後刪除該值。  
  
    2.  **將選取的定義域值設為錯誤**︰ 若要變更值的狀態從更正或無效的錯誤，選取值，以及 [ **將選取的定義域值視為錯誤** （十字符號） 圖示從圖示列的向下箭號，或從 [類型] 下拉式清單。 您可以在 **[更正為]** 資料行中輸入更正，或是將它保留空白。  
  
    3.  **將選取的定義域值設為無效**︰ 若要變更值的狀態從更正或錯誤不正確，選取值，以及 [ **將選取的定義域值為無效** 從圖示列的向下箭號，或從 [類型] 下拉式清單 （三角形） 圖示。 您可以在 **[更正為]** 資料行中輸入更正，或是將它保留空白。  
...
6.  **拼字檢查**：如果您已經在 [定義域屬性] 頁面中啟用拼字檢查，請尋找具有波浪式紅色底線的任何值，表示拼字檢查有建議更正。 以滑鼠右鍵按一下有底線的值，然後選取適用的更正。 實值型別會變成 （或維持） 錯誤，而且更正將會加入至 **更正為** 資料行。 按一下向下箭號，以查看其他建議的更正。 手動輸入更正，將其加入至拼字檢查字典，而且能夠將其選取為更正。 如需相關資訊，請參閱 [Use the DQS Speller](../data-quality-services/use-the-dqs-speller.md) 及 [Set Domain Properties](../data-quality-services/set-domain-properties.md)。  
  
    > [!NOTE]  
    >  若要使用拼字檢查，您可以啟用在 **網域內容** ] 頁面上，或如果中已停用 **網域內容** ] 頁面上，您可以按一下 **啟用/停用拼字檢查** 圖示 **管理資料探索結果** 頁面，讓此頁面上。  
...
10. **刪除選取的定義域值**︰ 從網域移除一或多個現有的值，以選取值，然後按一下 **刪除選取的定義域值** ] 按鈕。 DQS_NULL 的項目無法刪除，所以如果您選擇多個要刪除的值，而且 DQS_NULL 的項目是其中一個，此作業將會失敗。  
...
##  <a name="FollowUp"></a> 後續操作：在執行知識探索之後  
 當您在電腦輔助的知識探索程序中將知識加入至知識庫之後，您可以使用此知識庫來立即清理專案，或者您可以先執行定義域管理，然後再執行清理。 如需有關資料清理或定義域管理的詳細資訊，請參閱 [資料清理](../data-quality-services/data-cleansing.md) 或 [管理定義域](../data-quality-services/managing-a-domain.md)。  
...
-   **無效：** 這個值不屬於定義域，而且沒有更正。 例如，“12345” 值在城市定義域中是無效的。 當某個值未通過定義域規則時，DQS 會將它指定為無效的值。  
 ...
 當您執行互動式資料清理中 **管理和檢視結果** 頁面 **清理** 活動，無效和錯誤的值會包含在 **無效** 索引標籤上 **管理和檢視結果** 頁面。  
  
##  <a name="Display"></a> 如何顯示適當的值  
 您可以依照以下方式修改顯示：  
  
-   **篩選** 在資料表中，您想要的結果選取中的狀態為基礎的狀態， **篩選** 下拉式清單。  
...
-   在 **前置處理記錄** 階段中，DQS 會將資料載入和索引。 這是依照逐一記錄或逐一批次的方式進行，所以記錄可以顯示進度。 在這個步驟的執行期間，可以產生大多數的分析資料，但是 **[在定義域中有效]** 值除外。  
