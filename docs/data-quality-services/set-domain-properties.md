...
##  <a name="Properties"></a>網域屬性  
  
###  <a name="Name"></a>功能變數名稱和描述  
...
###  <a name="Speller"></a>啟用拼寫器  
 如果資料類型是 **[字串]**，按一下此選項可為定義域啟用 DQS 拼字檢查。 拼字檢查只適用於資料類型為字串的定義域。 
  **[啟用拼字檢查]** 核取方塊只會針對與此核取方塊相關聯的單一定義域來啟用拼字檢查。 此核取方塊不適用於複合定義域。  
