...
1.  設定現有網域的網域管理活動開啟知識庫屬性 (請參閱 [開啟知識庫](../data-quality-services/open-a-knowledge-base.md))，然後選取適當的網域中 **網域** 清單。 預設會顯示 [定義域屬性] 頁面。  
  
2.  設定新網域上的屬性中所述，建立它之後 [建立網域](../data-quality-services/create-a-domain.md)。  
...
###  <a name="Type"></a> 資料類型  
 當您建立網域時，網域中選取下列資料類型的值的其中一個︰ **字串** （預設）、 **日期**, ，**整數**, ，或 **十進位**。 在您建立定義域之後，可以檢視資料類型，但是無法變更資料類型。 針對定義域所選取的資料類型會定義可對應至該定義域的來源資料類型。 在 DQS 中四種定義域資料類型的每個支援的資料類型的相關資訊，請參閱 [支援的 SQL Server 和 SSIS 資料類型，適用於 DQS 定義域](../data-quality-services/supported-sql-server-and-ssis-data-types-for-dqs-domains.md)。  
  
###  <a name="Leading"></a> 使用前置值  
 選取這個核取方塊，指定一組同義字中的前置值將會是輸出，而不是與其同義的值。 取消選取 **使用前置值** ，指定每一個同義字值是正確或更正形式的輸出，而且其群組的前置值不會被取代。  
  
###  <a name="Normalize"></a> 正在將字串標準化  
 如果資料類型是 **字串**, ，按一下以忽略來源資料進行資料品質處理 DQS 中的特殊字元。 DQS 內部會在資料載入至網域時，以 null 或空格取代特殊字元。 冒號、連字號、句號、雙引號或分號會由空格取代。 單引號會由 null 取代。 使用 null 會將字串的兩部分結合在一起。  
...
###  <a name="Format"></a> 設定輸出格式為  
 選取當定義域中的資料值為輸出時，將套用的格式。 此格式是選取的資料類型所特有，如下列清單所示。 選取 **無** 表示不會套用任何清單中的格式。  
...
###  <a name="Language"></a> 語言  
 如果資料類型是 **字串**, ，選取您想要使用拼字檢查作業與定義域產生關聯的語言。 這個選取項目只適用於拼字檢查，因為拼字檢查結果取決於使用中的語言。 此選取項目只適用於資料類型為字串的單一定義域。 對於複合定義域而言，語言屬性是不相關的。 複合定義域之每一個部分的語言都是由相關的單一定義域所決定。  
  
 預設語言為英文。 設定 **語言** 屬性 **其他** 停用定義域的拼字檢查。  
  
> [!TIP]  
>  如果您的語言不會列在 **語言** 下拉式清單中，您必須選取 **其他**。 這樣可確保 DQS 根據定義域中的可用知識 (定義域規則、定義域值、TBR、比對規則) 清理並刪除未列出之語言資料的重複項目。  
  
###  <a name="Speller"></a> 啟用拼字檢查  
 如果資料類型是 **字串**, ，按一下以啟用 DQS 拼字檢查網域。 拼字檢查只適用於資料類型為字串的定義域。  **啟用拼字檢查** 核取方塊會啟用拼字檢查只會針對單一核取方塊相關聯的定義域。 此核取方塊不適用於複合定義域。  
  
 拼字檢查會針對定義域中的值來建議語法和驗證更正。 如需詳細資訊，請參閱 [使用 DQS 拼字檢查](../data-quality-services/use-the-dqs-speller.md)。  
  
###  <a name="Syntax"></a> 停用語法錯誤演算法  
 如果資料類型是 **字串**, ，請選取以指定語法錯誤將不會識別在網域中的 dqs 在清理期間。 當識別該定義域的語法錯誤無關緊要時，請選取這個核取方塊。 例如，對序號識別語法錯誤可能無關緊要。 此控制項只適用於字串資料類型。 DQS 將不會檢查非字串資料類型的語法錯誤。  
