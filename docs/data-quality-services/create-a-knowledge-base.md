...
4.  在 **[建立知識庫來源]**中，選取要做為知識庫基礎的來源：  
    ...
    -   若要使用已經在 **上建立的知識庫或預設知識庫做為新知識庫的基礎，請選取** [現有知識庫] [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)]。 選取從知識庫 **選取知識庫** 下拉式清單中或按一下 **瀏覽** 顯示 **選取知識庫** 對話方塊中，選取現有的知識要做為新知識庫，然後按一下 **確定**。 當您從資料表中選取知識庫時，該知識庫的定義域和比對規則就會顯示在對話方塊的右窗格中。 您也可以選取 **DQS 資料** 知識庫，這是預設知識庫，其中包含常見的方塊外的網域和美國公司、 地址和合作對象資料相關的知識。  
...
##  <a name="FollowUp"></a> 後續操作：建立知識庫之後  
 建立知識庫之後，系統會顯示可用來執行知識探索的精靈、建立比對原則的精靈或是執行定義域管理的頁面。 如需知識探索、 定義域管理或比對原則的詳細資訊，請參閱 [執行知識探索](../data-quality-services/perform-knowledge-discovery.md), ，[管理定義域](../data-quality-services/managing-a-domain.md), ，或 [建立比對原則](../data-quality-services/create-a-matching-policy.md)。  
