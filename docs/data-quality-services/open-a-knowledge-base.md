...
##  <a name="Locked"></a> 如果知識庫已鎖定  
 第一個資料行中的鎖定圖示顯示知識庫是否已鎖定。 已鎖定的知識庫名稱將會是紅色字型。 特定使用者透過知識庫活動正在修改的知識庫會標示為已鎖定。 鎖定的知識庫無法由另一個使用者操作。 正在處理知識庫的使用者可以將它解除鎖定知識庫資料表的 [開啟知識庫] 頁面上按一下滑鼠右鍵，然後按一下 **Unlock**, ，或是發行。 當游標置於鎖定的知識庫上時，DQS 將會顯示一個提示，指示鎖定知識庫的人以及鎖定的時間。  
  
##  <a name="State"></a> 知識庫的狀態  
 狀態欄位會指示知識庫位於活動的哪一個階段。 如果您開啟知識庫，它將會開啟到這個階段。  
  
-   **\< 空白>**: [狀態] 欄位是空的知識，如果已發行知識庫，依序按一下 **發行** 在定義域管理活動中，按一下 **是-發行知識庫並結束**。  
...
-   **探索-對應**︰ 上關閉知識庫 **知識庫管理︰ 對應** 頁面。 知識庫已鎖定，無法使用 [定義域管理] 和 [比對] 活動。  
  
-   **探索-探索**︰ 上關閉知識庫 **知識庫管理︰ 分析** 頁面。 知識庫已鎖定，[定義域管理] 活動無法使用。  
