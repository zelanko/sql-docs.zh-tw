...
-   **資料清理︰** 修改、 移除或充實的資料不正確或不完整，使用電腦輔助和互動式程序。 如需詳細資訊，請參閱 [Data Cleansing](../data-quality-services/data-cleansing.md)。  
  
-   **比對︰** 識別語意重複以規則為基礎的程序，可讓您判斷構成符合項目，並執行刪除重複作業。 如需詳細資訊，請參閱 [Data Matching](../data-quality-services/data-matching.md)。  
  
-   **Reference Data Services** ：使用參考資料提供者的服務驗證資料的品質。 您可以使用參考資料服務，從 [Microsoft Azure Marketplace](http://azure.microsoft.com/marketplace/) 清理，驗證、 比對，以及豐富的資料。 如需詳細資訊，請參閱 [Reference Data Services in DQS](../data-quality-services/reference-data-services-in-dqs.md)。  
...
-   **知識庫︰** Data Quality Services 是知識驅動方案，以分析資料，根據您使用 DQS 建立的知識。 這可讓您建立資料品質程序，持續增強您資料的相關知識，並藉以提升資料的品質。  
...
-   根據知識庫中的知識而建議來源資料變更的 **資料品質專案** 。  
 ...
 如需詳細資訊，請參閱 [DQS 知識庫與定義域](../data-quality-services/dqs-knowledge-bases-and-domains.md) 和 [資料品質專案 #40; DQS & #41;](../data-quality-services/data-quality-projects-dqs.md)。  
...
##  <a name="Components"></a> DQS 元件  
 ...
 [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)] 會實作為您管理及監視 SQL Server Management Studio （DQS_MAIN、 DQS_PROJECTS 和 DQS_STAGING_DATA） 中的三個 SQL Server 目錄。 DQS_MAIN 包含 DQS 預存程序、DQS 引擎和已發行的知識庫。 DQS_PROJECTS 包含知識庫管理和 DQS 專案活動所需的資料。 DQS_STAGING_DATA 提供中繼暫存資料庫，您可以從中複製來源資料以執行 DQS 作業，然後匯出已處理的資料。  
