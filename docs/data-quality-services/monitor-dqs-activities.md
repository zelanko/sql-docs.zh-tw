...
3.  [活動監控] 畫面會在活動方格中顯示每個活動的相關資訊。 活動方格會顯示有關每個 DQS 活動的下列資訊：  
     ...
     **為**使用中：表示活動目前是否為作用中。 它具有下列值：  
  
    -   **現用**：活動目前正在執行。  
  
    -   已**結束**：活動已完成。  
  
    -   已**終止**： DQS 系統管理員已使用 [活動監控] 畫面來終止活動，或者在的個別功能區域中執行活動時，使用者已經取消活動[!INCLUDE[ssDQSClient](../includes/ssdqsclient-md.md)]。  
    ...
    -   **成功**：在計算進程執行之前，以及在計算程式成功結束之後，會設定此狀態。  
     ...
     **使用者**：起始活動之使用者的名稱，或處理活動的最後一個使用者（如果兩者不同）。  
  
     **活動開始時間**：活動的開始日期和時間  
  
     **經過時間**：自活動啟動後經過的時間。 以 HH:MM:SS 標記法顯示。  
