...
3.  [活動監控] 畫面會在活動方格中顯示每個活動的相關資訊。 活動方格會顯示有關每個 DQS 活動的下列資訊：  
     ...
     **為使用中**：指出此活動目前是否使用中。 其值如下：  
  
    -   **使用中**：活動目前正在執行。  
  
    -   **已結束**：活動已完成。  
  
    -   **已終止**：DQS 系統管理員已經使用 [活動監控] 畫面來終止活動，或在 [!INCLUDE[ssDQSClient](../includes/ssdqsclient-md.md)] 的個別功能區域中執行活動時，使用者已經取消活動。  
  
     **類型**：指出活動的類型。 **子類型** ：表示針對活動類型執行的特定工作流程。 下列活動類型會受到監視：  
     ...
     **目前狀態**：指出活動的目前狀態。 活動狀態是由最後一個計算程序所決定。 請注意，一個活動中可以有數個計算程序，例如多次執行探索程序 (在知識探索活動內部)。 因此，狀態可能會在活動存留期間變更許多次。  
