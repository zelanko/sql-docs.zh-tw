...
> [!NOTE]  
>  本主題不包含從命令列執行安裝程式的指示。 如需安裝 [!INCLUDE[ssDQSServer](../../includes/ssdqsserver-md.md)] 和用戶端之命令列選項的資訊，請參閱[從命令提示字元安裝 SQL Server 2016](../../database-engine/install-windows/install-sql-server-2016-from-the-command-prompt.md) 中的[功能參數](../../database-engine/install-windows/install-sql-server-2016-from-the-command-prompt.md#Feature)。  
...
>  儘管 Microsoft Excel 不是安裝 Data Quality Client 的必要條件，仍須將 Microsoft Excel 2003 安裝在 Data Quality Client 電腦上以執行各種用戶端應用程式中的作業，例如從 Excel 檔案匯入定義域值，或是針對知識探索、清理或比對活動對應 Excel 檔案中的來源資料。  
 ...
 如需安裝 [!INCLUDE[ssCurrent](../../includes/sscurrent-md.md)] 之最低系統需求的詳細資訊，請參閱[安裝 SQL Server 2016 的硬體與軟體需求](../../sql-server/install/hardware-and-software-requirements-for-installing-sql-server-2016.md)。  
...
2.  對使用者授與 DQS 角色：  
     ...
     若要使用 [!INCLUDE[ssDQSClient](../../includes/ssdqsclient-md.md)] 登入 [!INCLUDE[ssDQSServer](../../includes/ssdqsserver-md.md)]，使用者必須具備 DQS_MAIN 資料庫的下列三個角色之一：  
