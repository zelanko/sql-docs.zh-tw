...
|||  
|-|-|  
|針對包含多個未剖析之相關詞彙的資料欄位建立語意表示法|[Create a Composite Domain](../data-quality-services/create-a-composite-domain.md)|  
|當您將複雜資料對應至複合定義域時，除了針對分隔符號進行剖析之外，您也可以根據知識來剖析資料。 DQS 會先嘗試使用它對於單一定義域的知識，以判斷複雜字串的部分如何屬於單一定義域。|[Create a Composite Domain](../data-quality-services/create-a-composite-domain.md)|  
...
|識別值組合，好讓 DQS 可以報告其頻率。|[Use Value Relations in a Composite Domain](../data-quality-services/use-value-relations-in-a-composite-domain.md)|  
