...
-   精確相似度：如果兩筆不同記錄之相同欄位的值必須相同，這樣這些值才會被視為相符的話，請選取 **[精確]** 。 如果相同的話，該定義域的符合分數將會設定為 “100”，而且 DQS 會在規則中使用該分數與其他定義域的分數，以判斷彙總符合分數。 如果不相同的話，該定義域的符合分數將會設定為 “0”，而且規則的處理作業將會繼續前往下一個條件。 如果您為數值定義域設定比對規則，並選取 **[類似]**，您可以輸入百分比或整數形式的容錯值。 對於日期類型的網域，您可以輸入容錯為日、 月或年 （整數） 如果您選取 **類似**; 沒有日期定義域沒有百分比容錯值。 如果您選取 **[精確]**，就沒有這個選項。  
...
3.  按一下 **[開啟知識庫]** ，在現有的知識庫中建立或修改比對原則。 選取知識庫，並選取 **[比對原則]**，然後按 **[下一步]**。 您也可以按一下 **[最近使用的知識庫]**底下的知識庫。 如果您開啟已關閉時比對原則而且正在處理的知識庫，您將繼續進行比對原則活動已關閉的階段 (由 **狀態** 知識庫中知識庫資料表或底下的知識庫名稱資料行 **近期知識庫**)。 如果您開啟的知識庫包含比對原則而且已完成，您將會前往 **[比對原則]** 頁面。 如果您開啟的知識庫不包含比對原則而且已完成，您將會前往 **[對應]** 頁面。  
  
##  <a name="MatchingStage"></a> 對應階段  
 在對應階段中，您會識別建立比對原則所針對的資料來源，而且您會將來源資料行對應至定義域，好讓定義域可供比對原則活動使用。  
  
1.  在 **對應** ] 頁面上，建立原則的資料庫，保留 **資料來源** 為 **SQL Server**, ，選取您想要建立的原則中的資料庫 **資料庫**, ，然後選取資料表或檢視中的 **資料表/檢視表**。 來源資料庫必須與 [!INCLUDE[ssDQSServer](../includes/ssdqsserver-md.md)]位於相同的 SQL Server 執行個體上， 否則不會出現在下拉式清單中。  
...
5.  按一下 [ **加號 （+）** 控制項，將資料列加入至對應資料表或 **減號 （-）** 控制項移除資料列。  
  
6.  按一下 **[預覽資料來源]** 查看您選取之 SQL Server 資料表或檢視表中的資料，或者您選取的 Excel 工作表。  
  
7.  按一下 [ **檢視/選取複合定義域** 檢視知識庫中可用的複合定義域清單並選取適當的對應。  
...
3.  增加的值， **最小符合分數** 如果您想要讓比對規定更嚴格。 如需最低符合分數的詳細資訊，請參閱 [如何設定比對規則參數](#MatchingRules)。  
...
6.  如果兩筆不同記錄之相同欄位的兩個值即使不相同，也可以被視為相符的話，請針對 **[相似度]**選取 **[類似]** 。 如果兩筆不同記錄之相同欄位的兩個值必須相同才會被視為相符的話，請選取 **[精確]**  (如需詳細資訊，請參閱 [如何設定比對規則參數](#MatchingRules)。)  
...
10. 選取 **重疊的叢集** 從下拉式清單，以便顯示樞紐記錄和隨後的記錄所有的叢集執行比對時，即使叢集的群組有共同的記錄。 選取 **[非重疊的叢集]** ，在執行比對時顯示與單一叢集擁有共同記錄的叢集。  
  
11. 按一下 [ **重新載入資料來源** 從資料來源的資料複製到暫存資料表並重新建立索引時執行的比對原則。 按一下 [ **先前的資料上執行** 執行比對原則，而不需要將資料複製到暫存資料表，並重新建立索引的資料。 當您初次執行比對原則，或是您在**[對應]** 頁面中變更對應，然後在隨後的快顯視窗中按一下 **[是]** 時， **[在先前的資料上執行]** 將會停用。 在這兩種情況下，您都必須重新建立索引。 如果比對原則尚未變更，就不需要重新建立索引。 針對之前的資料執行有助於提高效能。  
...
13. 您可以使用 **[比對結果]** 資料表中的資料，如下所示：  
    ...
    -   在 **%下拉式方塊**, ，從下拉式清單中，以"5"的遞增量選取百分比。 符合分數大於或等於該百分比的所有資料列都將顯示在比對結果資料表中。  
  
    -   如果您按兩下比對結果資料表中的記錄，DQS 會顯示 **符合分數詳細資料** 快顯視窗會顯示樞紐記錄和來源記錄 （和其所有欄位中的值） 之間，以及向下鑽研記錄比對分數。 向下鑽研會顯示樞紐記錄和來源記錄之每一個欄位中的值，好讓您可以比較這些值，並顯示每一個欄位的符合分數佔兩筆記錄之整體符合分數的比重。  
  
14. 請在 **[分析工具]** 和 **[比對結果]** 索引標籤中檢視統計資料，以確保您會達成所需的結果。 如需詳細資訊，請參閱 [分析工具和結果索引標籤](#Tabs)。  
...
1.  在 **比對結果** 頁面上，選取 **重疊的叢集** 從下拉式清單，以便顯示樞紐記錄和隨後的記錄所有的叢集執行比對時，即使叢集的群組有共同的記錄。 選取 **[非重疊的叢集]** ，在執行比對時顯示與單一叢集擁有共同記錄的叢集。  
  
2.  按一下 [ **重新載入資料來源** 從資料來源的資料複製到暫存資料表並重新建立索引時執行的比對原則。 按一下 [ **先前的資料上執行** 執行比對原則，而不需要將資料複製到暫存資料表，並重新建立索引的資料。 當您初次執行比對原則，或是您在**[對應]** 頁面中變更對應，然後在隨後的快顯視窗中按一下 **[是]** 時， **[在先前的資料上執行]** 將會停用。 在這兩種情況下，您都必須重新建立索引。 如果比對原則尚未變更，就不需要重新建立索引。 針對之前的資料執行有助於提高效能。  
  
3.  按一下 **[啟動]** ，針對您已定義的所有規則執行比對程序。  **比對結果** 資料表會顯示記錄識別碼、 叢集號碼和資料行 (包括不在比對規則) 叢集中的每一筆記錄。 叢集中的前置記錄是隨機選取的 (您決定存活記錄上藉由選取存活規則 **匯出** 頁面上，當您執行比對專案。)叢集中的每一個額外資料列都被視為重複項目，結果資料表中會提供其符合分數 (相較於樞紐記錄)。  
...
8.  按一下 **[關閉]** 儲存工作，並返回 DQS 首頁。 知識庫的狀態將會顯示「比對原則 –」字串和目前的狀態。 如果您按下 **關閉** 當您在 **比對結果** ] 畫面中，狀態會顯示: 「 比對原則-結果 」。 如果您按一下 [關閉當您在 **比對原則** ] 畫面中，狀態會顯示: 「 比對原則-比對原則 」。 在按一下 **[關閉]**之後，若要執行 **[知識探索]** 活動，您必須返回 **[比對原則]** 活動，按一下 **[完成]**，然後按一下 **[是]** 發行知識庫，或按一下 **[否]** 儲存知識庫工作並結束。  
