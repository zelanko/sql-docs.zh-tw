...
若要管理 （查詢、 監視等） 您需要一項工具的資料庫。 有數個資料庫的工具。 您的資料庫可以執行在雲端中，在 Windows 中，或上時[Linux](../linux/sql-server-linux-overview.md)，不需要與資料庫相同的平台上執行您的工具。 

本文章提供可用的工具來處理 SQL 資料庫的相關資訊。 
...
|[mssql-cli](mssql-cli.md)|mssql cli 是互動式命令列工具來查詢 SQL Server。 **執行 Windows、 macOS 和 Linux 上的 mssql cli**|
| [Visual Studio Code](https://code.visualstudio.com/)| 安裝 Visual Studio 程式碼之後, 安裝[mssql 延伸](https://marketplace.visualstudio.com/items?itemName=ms-mssql.mssql)開發 Microsoft SQL Server、 Azure SQL Database 和 SQL 資料倉儲。 **在 Windows、 macOS 和 Linux 上執行 visual Studio Code**。|

## <a name="which-tool-should-i-choose"></a>我應該選擇哪一種工具？

- 管理 SQL Server 執行個體或資料庫，請在 Windows、 Linux 或 Mac 上的輕量級編輯器嗎？ 選擇 [[!INCLUDE[name-sos](../includes/name-sos.md)]](../sql-operations-studio/download.md)
...
- 您要查詢 SQL Server 命令列工具，特色 IntelliSense、 語法高光源，以及其他更多嗎？ Choose [mssql-cli](mssql-cli.md)
...
| 工具 | Description |
|:--|:--|
| [組態管理員](../tools/configuration-manager/sql-server-configuration-manager-help.md) | 使用 SQL Server 組態管理員來設定 SQL Server 服務和設定網路連接性。 在 Windows 上執行的 configuration Manager|
|[mssql-conf](../linux/sql-server-linux-configure-mssql-conf.md)|若要設定在 Linux 上執行的 SQL Server 使用 mssql conf。|
...
|[sqlagent90 應用程式](../tools/sqlagent90-application.md)|用來從命令提示字元啟動 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] Agent。|\<drive>:\Program Files\Microsoft SQL Server\\<*instance_name*>\MSSQL\Binn|  
