...
helpviewer_keywords: 
  ...
  - "匯出資料"
  - "資料列匯出 [SQL Server]"
  - "複製資料 [SQL Server], bcp 公用程式"
  - "命令提示字元公用程式 [SQL Server], bcp"
  - "資料表 [SQL Server], 匯入資料"
  - "資料行匯入 [SQL Server]"
  - "bcp 公用程式 [SQL Server], 命令選項"
  - "檔案匯出 [SQL Server]"
  - "大量複製 [SQL Server]"
  - "bcp 公用程式 [SQL Server], 關於 bcp 公用程式"
  - "資料表 [SQL Server], 匯出資料"
  - "資料列匯入 [SQL Server]"
  - "匯入資料, bcp 公用程式"
  - "檔案匯入 [SQL Server]"
...
[!INCLUDE[tsql-appliesto-ss2008-all_md](../includes/tsql-appliesto-ss2008-all-md.md)]
 ...
 ![主題連結圖示](../database-engine/configure-windows/media/topic-link.png "主題連結圖示") 如需用於 **bcp** 語法的語法慣例，請參閱 [Transact-SQL 語法慣例 &#40;Transact-SQL&#41;](../t-sql/language-elements/transact-sql-syntax-conventions-transact-sql.md)。  
...
**"** ***query*** **"**<a name="query"></a>
 是一個傳回結果集的 [!INCLUDE[tsql](../includes/tsql-md.md)] 查詢。 如果查詢傳回多個結果集，則只會將第一個結果集複製到資料檔案中，並會忽略接下來的結果集。 請利用雙引號括住查詢，利用單引號括住內嵌在查詢中的任何項目。 從查詢中複製大量資料時，也必須指定**queryout** 。  
 ...
 **-C** { **ACP** | **OEM** | **RAW** | *code_page* }<a name="C"></a>   
 指定資料檔案中之資料的字碼頁。 只有當資料包含字元值大於 127 或小於 32 的 **char**、**varchar** 或 **text** 資料行時，*code_page* 才會相關。  
 ...
 **-K** ***application_intent***<a name="K"></a>   
 宣告連接到伺服器時的應用程式工作負載類型。 唯一可能的值是 **ReadOnly**。 若未指定 **-K**，bcp 公用程式將不會支援在 AlwaysOn 可用性群組中連接次要複本。 如需詳細資訊，請參閱[使用中次要：可讀取的次要複本 &#40;AlwaysOn 可用性群組&#41;](../database-engine/availability-groups/windows/active-secondaries-readable-secondary-replicas-always-on-availability-groups.md)。  
...
Bcp 公用程式也可以從 [Microsoft SQL Server 2016 功能套件](https://www.microsoft.com/download/details.aspx?id=526760) 個別下載。  選取 `ENU\x64\MsSqlCmdLnUtils.msi` 或 `ENU\x86\MsSqlCmdLnUtils.msi`。
...
### B. 將資料表資料列複製到資料檔案中 (使用信任連接)  
下列範例說明 `WorlWideImporters.Warehouse.StockItemTransactions` 資料表上的 **out** 選項。
...
### D. 將檔案資料複製到資料表中  
下列範例會使用上方建立的檔案說明 `WorlWideImporters.Warehouse.StockItemTransactions_bcp` 資料表上的 **in** 選項。
...
### F. 將特定資料列複製到資料檔案中  
若要複製特定資料列，您可以使用 **queryout** 選項。 下列範例只會將 `WorlWideImporters.Application.People` 資料表中名稱為 `Amy Trefl` 之連絡人的資料列複製到 `Amy_Trefl_c.bcp` 資料檔案中。  注意︰**-d** 參數用來識別資料庫。
...
### H. 建立格式檔案  
下列範例會針對 `WorlWideImporters` 資料庫中的 `Warehouse.StockItemTransactions` 資料表來建立三個不同的格式檔案。  檢閱每個建立檔案的內容。
...
### I. 使用格式檔案以 bcp 進行大量匯入  
若要在將資料匯入 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 的執行個體時使用先前所建立的格式檔案，請搭配 **in** 選項使用 **-f** 參數。  例如，下列命令會利用先前建立的格式檔案 `StockItemTransactions_c.xml`，將 `StockItemTransactions_character.bcp` 資料檔案的內容大量複製到 `Warehouse.StockItemTransactions_bcp` 資料表的複本中。  注意︰**-L** 參數用來只匯入前 100 個記錄。
