...
helpviewer_keywords: 
  ...
  - "EXIT 命令"
  - "作業系統 [SQL Server], 命令"
  - "osql 公用程式 [SQL Server]"
  - "預存程序 [SQL Server], 命令提示字元"
  - "指令碼 [SQL Server], 命令提示字元"
  - "RESET 命令"
  - "GO 命令"
  - "命令提示字元公用程式 [SQL Server], osql"
  - "CTRL+C 命令"
...
>  可能的話，請使用 **-E** 選項 (信任連接)。  
 ...
 當以互動方式使用 **osql** 時，您可以利用 **:r***file_name*，將作業系統檔案讀到命令緩衝區中。 這會將 *file_name* 中的 SQL 指令碼當作單一批次直接傳給伺服器。  
  
> [!NOTE]  
>  當使用 **osql** 時，如果批次分隔字元 GO 出現在 SQL 指令碼檔案中，[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 會將其視為語法錯誤。  
  
## 插入註解  
 您可以在 **osql** 提交給 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 的 Transact-SQL 陳述式中併入註解。 可用的註解樣式有兩種：-- 和 /*...\*/。  
