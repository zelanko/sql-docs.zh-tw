...
helpviewer_keywords: 
  ...
  - "sqlmaint 公用程式"
  - "維護資料庫 [SQL Server]"
  - "備份 [SQL Server], sqlmaint 公用程式"
  - "命令提示字元公用程式 [SQL Server], sqlmaint"
  - "維護計畫 [SQL Server], 命令提示字元"
  - "正在備份 [SQL Server], sqlmaint 公用程式"
 ...
 *backup_path* 會指定備份目錄。 如果也指定了 **-UseDefDir**，便不需要 *backup_path*，如果同時指定這兩者，**-UseDefDir** 就會覆寫 backup_path。 備份可以放在目錄或磁帶裝置位址 (例如 \\\\.\TAPE0) 中。 資料庫備份的檔案名稱會依照下列方式自動產生：  
...
### B. 在計畫的所有資料庫中，利用 15% 取樣來更新統計資料。 另外，將到達 110 MB 的任何資料庫壓縮成只有 10% 的可用空間  
  
```  
sqlmaint -S MyServer -PlanName MyUserDBPlan -UpdOptiStats 15 -RmUnusedSpace 110 10  
```  
  
### C. 在預設的 x:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\MSSQL\Backup 目錄中，將計畫中的所有資料庫備份到它們的個別子目錄中。 另外，如果備份比 2 星期還要舊，便予以刪除  
  
```  
sqlmaint -S MyServer -PlanName MyUserDBPlan -BkUpDB -BkUpMedia DISK -UseDefDir -CrBkSubDir -DelBkUps 2weeks  
```  
  
### D. 將資料庫備份到預設的 x:\Program Files\Microsoft SQL Server\MSSQL13.MSSQLSERVER\MSSQL\Backup 目錄中。  
