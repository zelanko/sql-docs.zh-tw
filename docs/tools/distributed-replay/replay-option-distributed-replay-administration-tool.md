 ...
 **replay** 選項會起始事件重新執行階段，控制器在此階段中會分派重新執行資料給指定的用戶端、啟動分散式重新執行，並同步處理用戶端。 另外，參與重新執行的每個用戶端可以記錄重新執行活動，並在本機上儲存結果追蹤檔案。  
 ...
 ![主題連結圖示](../../database-engine/configure-windows/media/topic-link.png "主題連結圖示") 如需有關管理工具語法所使用之語法慣例的詳細資訊，請參閱 [Transact-SQL 語法慣例 &#40;Transact-SQL&#41;](../../t-sql/language-elements/transact-sql-syntax-conventions-transact-sql.md)。  
 ...
 **-s** *target_server*  
 ...
 如果在重新執行組態檔 `DReplay.exe.replay.config` 的 `<ReplayOptions>` 區段中指定了 `<Server>` 元素，則不需要 **-s** 參數。  
  
 如果使用了 **-s** 參數，系統會忽略重新執行組態檔之 `<ReplayOptions>` 區段中的 `<Server>` 元素。  
 ...
 **-c** *config_file*  
 ...
 如果要使用重新執行組態檔 `DReplay.exe.replay.config` 的預設值，則不需要 **-c** 參數。  
...
-   不需要 **-s** 參數，因為重新執行組態檔 `DReplay.exe.replay.config` 的 `<ReplayOptions>` 元素中，指定了 `<Server>` 元素。  
