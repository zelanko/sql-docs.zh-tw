...
ms.topic: "article"
helpviewer_keywords: 
  - "Service Broker, 執行階段報表"
  - "Service Broker, 命令提示字元公用程式"
  - "疑難排解 [Service Broker], 交談"
  - "疑難排解 [Service Broker], 組態"
  - "命令提示字元公用程式 [Service Broker]"
  - "Service Broker, 疑難排解"
  - "Service Broker, 組態報表"
  - "Service Broker, 工具"
  - "疑難排解 [Service Broker], 執行階段"
  - "交談 [Service Broker], 疑難排解"
  - "疑難排解 [Service Broker], ssbdiagnose 公用程式"
  - "工具 [Service Broker], ssbdiagnose"
  - "Service Broker, ssbdiagnose 公用程式"
  - "ssbdiagnose"
 ...
 **-TIMEOUT** *timeout_interval*  
 指定 **RUNTIME** 報表要執行的秒數。 如未指定 **-TIMEOUT**，執行階段報表就會無限期地執行。 **-TIMEOUT** 僅用於 **RUNTIME** 報表，不用在 **CONFIGURATION** 報表。 如果沒有指定 **-TIMEOUT**，或想要在逾時間隔到期之前結束執行階段報表，您可以使用 CTRL + C 結束 **ssbdiagnose**。 *timeout_interval* 必須是介於 1 和 2,147,483,647 之間的數字。  
 ...
 **-S** *server_name*[\\*instance_name*]  
 指定保存要分析之 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 服務的 [!INCLUDE[ssDE](../../includes/ssde-md.md)] 執行個體。  
