...
helpviewer_keywords: 
  - "issue 元素"
  - "XML 輸出檔格式 [ssbdiagnose], Issue 元素"
  - "ssbdiagnose"
...
|Attribute|描述|  
|---------------|-----------------|  
|**型別**|識別 Issue 元素所報告的問題類別：<br /><br /> **「診斷」** ：報告在您分析 [!INCLUDE[ssSB](../../includes/sssb-md.md)] 設定時發現的設定問題。<br /><br /> **「問題」** ：報告讓 **ssbdiagnose** 無法完成其分析的問題。 請更正問題並重新執行 **ssbdiagnose**。<br /><br /> **「事件」**：報告在您執行 **-RUNTIME** 檢查時發現的 [!INCLUDE[ssSqlProfiler](../../includes/sssqlprofiler-md.md)] 事件。 只有在您指定 **-SHOWEVENTS** 時才會報告事件。|  
|**代碼**|識別訊息的錯誤號碼。|  
