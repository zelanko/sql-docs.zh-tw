...
description: 了解如何連線到 [!INCLUDE[big-data-clusters-2019](../includes/ssbigdataclusters-ver15.md)] 的 SQL Server 主要執行個體和 HDFS/Spark 閘道。
author: MikeRayMSFT
ms.author: mikeray
...
1. 輸入 SQL 登入**使用者名稱**和**密碼**。 如果您使用 Windows 驗證，則不需要這麼做。

   > [!TIP]
   > 根據預設，在巨量資料叢集部署期間，將會停用 **SA** 使用者名稱。 在部署期間會佈建新的系統管理員使用者，其名稱會對應至 **AZDATA_USERNAME** 環境變數，且密碼會與 **AZDATA_PASSWORD** 環境變數對應。
