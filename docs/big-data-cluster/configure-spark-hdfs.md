...
在巨量資料叢集中，我們定義 3 項服務：`hdfs`、`spark`、`sql`。 每項服務都剛好對應到同命的組態類別。 所有閘道組態都會分在 `gateway` 類別。 
...
## <a name="configurations-via-cluster-profile"></a>Configurations via Cluster Profile
