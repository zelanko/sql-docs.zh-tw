...
當部署完成之後，且其存取權會經歷 Knox 時，會啟動 WebHDFS。 Knox 端點透過名為的 Kubernetes 服務公開**閘道 svc 外部**。  若要建立必要的 WebHDFS URL，來上傳/下載檔案，您需要**閘道 svc 外部**服務外部 IP 位址和您的叢集名稱。 您可以取得**閘道 svc 外部**服務外部 IP 位址執行下列命令：
