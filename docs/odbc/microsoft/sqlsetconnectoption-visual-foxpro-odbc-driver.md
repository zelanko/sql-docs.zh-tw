...
>  本主題包含 Visual FoxPro ODBC 驅動程式特有的資訊。 如需此函式的一般資訊，請參閱底下的適當主題[ODBC API 參考](../../odbc/reference/syntax/odbc-api-reference.md)。  
 ...
 設定管理許多層面的連線的選項。 此函式僅部分支援： 此驅動程式支援的所有值*fOption*引數，但不支援部分*vParam*值*fOption*引數SQL_TXN_ISOLATION。  
...
|SQL_TXN_ISOLATION|驅動程式可讓只 SQL_TXN_READ_COMMITTED。<br /><br /> 下列*vParam*s 不支援：<br /><br /> SQL_TXN_READ_UNCOMMITTED<br /><br /> SQL_TXN_REAPEATABLE_READ<br /><br /> SQL_TXN_SERIALIZABLE 的情況|  
