...
|SYNONYMCOLUMNS|當這個值為 true (1)、 SQLColumn （） API 呼叫傳回的資料行資訊。 否則，SQLColumn （） 傳回資料表和檢視表的資料的行。 未設定此值時，oracle ODBC 驅動程式提供存取速度。|@shouldalert|  
...
|StdDayOfWeek|會強制執行的 DAYOFWEEK 純量的 ODBC 標準。 依預設會開啟，但需要當地語系化的版本的使用者可以變更要使用 Oracle 所傳回的任何內容的行為。|@shouldalert|  
