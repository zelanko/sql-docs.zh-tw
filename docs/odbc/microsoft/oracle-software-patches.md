...
>  這項功能將會在未來的 Windows 版本中移除。 請避免在新的開發工作中使用這項功能，並規劃修改目前使用這項功能的應用程式。 請改用 Oracle 所提供的 ODBC 驅動程式。  
 ...
 需要 Oracle 伺服器產品及其用戶端元件的修補程式，才能正常運作數種 Microsoft 產品和技術，包括適用于 Oracle 的 Microsoft ODBC 驅動程式、Microsoft OLE DB Provider for Oracle、Internet Information Services （IIS）、元件服務（或 Microsoft 交易伺服器，如果您使用的是 Windows NT）等等。  
