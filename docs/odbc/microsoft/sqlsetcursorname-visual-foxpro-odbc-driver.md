...
>  本主題包含 Visual FoxPro ODBC 驅動程式特有的資訊。 如需此函數的一般資訊，請參閱[ODBC API 參考](../../odbc/reference/syntax/odbc-api-reference.md)底下的適當主題。  
 ...
 將資料指標名稱與現用語句控制碼（ *hstmt*）產生關聯。 **SQLSetCursorName**包含在 VISUAL FoxPro Odbc 驅動程式 API 中，因為它是核心層級 odbc API 功能的一部分;它不能與其他 API 函式一起使用，因為驅動程式不支援定點更新。  
  
 如需詳細資訊，請參閱 ODBC 程式設計*人員參考*中的[SQLSetCursorName](../../odbc/reference/syntax/sqlsetcursorname-function.md) 。
