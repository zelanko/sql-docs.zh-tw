...
新式具有螢幕為基礎的應用程式，在使用者捲動向前及向後的資料。 對於這類應用程式中，返回先前擷取的資料列是問題。 其中一個可能性是關閉並重新開啟資料指標直到資料指標所需的資料列，然後提取資料列。 另一個可能性是讀取結果集、 快取在本機，和實作應用程式中捲動。 這兩種可能性也僅適用於小型結果集，後者可能的原因是難以實作。 更好的解決方案是使用*捲動資料指標，* 這可以向後移動，並轉送結果集中。  
 ...
 向後移動的能力會引發問題不適用於順向資料指標： 應該可捲動資料指標偵測先前擷取的資料列所做的變更嗎？ 也就是說，它應該偵測更新、 刪除及新插入的資料列？  
  
 因為結果的定義設定，就會發生這個問題 — 符合特定準則的資料列集，無法敘述時檢查資料列，以查看是否符合該條件，或它不會敘述是否資料列必須包含相同的資料會擷取每一次。 先前省略能夠偵測資料列是否已插入或刪除，而後者可讓它們偵測更新的資料，可捲動資料指標。  
