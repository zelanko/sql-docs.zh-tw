...
## <a name="remarks"></a>備註  
 對於一律會傳回或接受字串或長度引數的 Unicode 函式，引數會當做字元數傳遞。 針對傳回伺服器資料之長度資訊的函式，顯示大小和精確度會以字元數來描述。 當長度（資料的傳輸大小）可以參考字串或非字串資料時，長度會以八位長度描述。 例如， **SQLGetInfoW**仍然會將長度視為位元組計數，但是**SQLExecDirectW**會使用字元數的計數。  
