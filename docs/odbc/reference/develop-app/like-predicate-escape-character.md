...
ms.locfileid: "47674996"
---
# <a name="like-predicate-escape-character"></a>LIKE 述詞逸出字元
在 **像**述詞，百分比符號 （%） 比對零個以上的任何字元和底線 (_) 會比對任何一個字元。 比對實際的百分比符號或底線中**像**述詞，逸出字元必須在前面的百分比符號或底線。 定義逸出序列**像**述詞逸出字元是：  
  
 **{逸出 '** *逸出字元* **'}**  
  
 何處*逸出字元*是資料來源所支援的任何字元。  
  
 如需類似的詳細資訊逸出序列，請參閱 <<c0> [ 逸出序列類似](../../../odbc/reference/appendixes/like-escape-sequence.md)附錄 c: SQL 文法中。  
