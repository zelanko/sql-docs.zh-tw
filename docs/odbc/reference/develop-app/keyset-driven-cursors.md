...
ms.locfileid: "47719436"
---
# <a name="keyset-driven-cursors"></a>索引鍵集導向的資料指標
索引鍵集驅動資料指標的靜態和動態資料指標之間在於它能夠偵測變更。 靜態資料指標，例如它不會永遠偵測變更的成員資格和順序的結果集。 動態資料指標，例如它沒有偵測到值的資料列的變更結果集中 （受限於 SQL_ATTR_TXN_ISOLATION 連接屬性所設定的交易隔離等級）。  
 ...
 當資料指標會嘗試擷取已刪除的資料列時，這個資料列會顯示為結果集內的 「 漏洞 」: 資料列的索引鍵存在於索引鍵集，但在結果集中的資料列不存在。 如果已更新資料列中的索引鍵值，資料列視為已刪除，然後插入，因此這類資料列也會顯示為結果集中的漏洞。 雖然索引鍵集驅動資料指標一律可以偵測到其他人所刪除的資料列，它可以選擇性地移除索引鍵的資料列會自行從刪除索引鍵集。 執行這項操作的索引鍵集驅動資料指標無法偵測到自己刪除。 特定的索引鍵集驅動資料指標是否偵測到它自己刪除透過 SQL_STATIC_SENSITIVITY 選項中會回報**SQLGetInfo**。  
