...
下一步是擷取結果，如下圖所示。  
 ...
 如果在 「 步驟 3:: 建置和執行 SQL 陳述式 」 中執行的陳述式已**選取 **陳述式或類別目錄函式，在應用程式第一次呼叫**SQLNumResultCols**來判斷資料行數目在結果集中。 此步驟不需要應用程式已經知道設定資料行，例如是硬式編碼的垂直或自訂應用程式中的 SQL 陳述式的結果數目。  
 ...
 應用程式立即返回 「 步驟 3:: 建置和執行 SQL 陳述式 」 執行另一個陳述式中相同的交易;或者繼續進行 「 步驟 5:: Commit Transaction"認可或回復交易。
