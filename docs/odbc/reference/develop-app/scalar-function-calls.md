 ...
 **{fn***純量函數* **}**   
 ...
 何處*純量函數*是其中一個所列出的函數[附錄 e： 純量函數](../../../odbc/reference/appendixes/appendix-e-scalar-functions.md)。 如需純量函式逸出序列的詳細資訊，請參閱[純量函式逸出序列](../../../odbc/reference/appendixes/scalar-function-escape-sequence.md)附錄 c: SQL 文法中。  
 ...
 **轉換 (** *value_exp* **，** *data_type * * *)**  
 ...
 其中*value_exp*資料行名稱，另一個純量函式或常值，結果並*data_type*符合關鍵字 **#define**所使用的名稱SQL 資料類型識別項中定義[附錄 d： 資料類型](../../../odbc/reference/appendixes/appendix-d-data-types.md)。 例如，下列 SQL 陳述式會使用**轉換**並確定函式的輸出**CURDATE**函式是日期類型，而不是時間戳記或字元資料：  
