 ...
 **總結**  
 **SQLCleanupConnectionPoolID** 會通知驅動程式集區識別碼已超時。只要與集區識別碼相關聯的集區中的所有連接都超時，集區識別碼就會超時。如需連接逾時的詳細資訊，請參閱 [Microsoft 資料存取元件中](/previous-versions/ms810829(v=msdn.10)) 的共用。  
