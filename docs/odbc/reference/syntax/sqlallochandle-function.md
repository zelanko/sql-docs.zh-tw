...
## <a name="allocating-an-environment-handle"></a>配置環境控制代碼  
 ...
 在驅動程式管理員的環境控制碼之下，如果已經有驅動程式的環境控制碼，則在進行連線時，不會在該驅動程式中呼叫具有 SQL_HANDLE_ENV *HandleType*的**SQLAllocHandle** ，而只會使用*HandleType*為 SQL_HANDLE_DBC 的**SQLAllocHandle** 。 驅動程式管理員的環境控制碼若未提供驅動程式的環境控制碼，則會在第一次連線時，于驅動程式中呼叫具有 HandleType SQL_HANDLE_ENV 的 SQLAllocHandle，以及 HandleType 為 SQL_HANDLE_DBC 的 SQLAllocHandle環境的控制碼已連接到驅動程式。  
