...
-   如果 *CharacterAttributePtr* 是固定長度資料類型的指標，則 *BufferLength* 必須是下列其中一項： SQL_IS_INTEGER、SQL_IS_UNINTEGER、SQL_SMALLINT 或 SQLUSMALLINT。  
  
 *StringLengthPtr*  
 出緩衝區的指標，此緩衝區會傳回位元組總數 (排除字元資料的 null 結束字元位元組) 可在 **CharacterAttributePtr*中傳回。  
  
 若為字元資料，如果可傳回的位元組數目大於或等於*BufferLength*，CharacterAttributePtr 中的描述元資訊 \* *CharacterAttributePtr*就會截斷為*BufferLength*減去 null 終止字元的長度，而且由驅動程式以 null 終止。  
 ...
 **SQLColAttribute**會傳回 \* *NumericAttributePtr*或 CharacterAttributePtr 中的資訊 \* * *。 整數資訊在 NumericAttributePtr 中會傳回 \* *NumericAttributePtr*為 SQLLEN 值; 所有其他格式的資訊都會在 \* *CharacterAttributePtr*中傳回。 當 NumericAttributePtr 中傳回信息 \* *NumericAttributePtr*時，驅動程式會忽略*CharacterAttributePtr*、 *BufferLength*和*StringLengthPtr*。 當 CharacterAttributePtr 中傳回信息時 \* * *，驅動程式會忽略*NumericAttributePtr*。  
 ...
 **SQLColAttribute** 會從 IRD 的描述項欄位傳回值。 使用語句控制碼呼叫函式，而不是使用描述項控制碼。 使用適當的 IRD 控制碼呼叫**SQLGetDescField** ，也可以取出**SQLColAttribute**針對本節中稍後列出之*FieldIdentifier*值所傳回的值。  
 ...
 ODBC 3。*x* 驅動程式必須傳回每個描述項欄位的值。 如果描述項欄位未套用至驅動程式或資料來源，除非另有說明，否則驅動程式會在 StringLengthPtr 中傳回0， \* *StringLengthPtr*或在 **CharacterAttributePtr*中傳回空字串。  
...
-   如果 *FieldIdentifier* 是 ODBC 3 中的新值。*x*，在 ODBC 2 中沒有對應的值。*x*，當 ODBC 3 時，它將不會對應。*x* 應用程式會在 ODBC 2 的 **SQLColAttribute** 呼叫中使用它。*x* 驅動程式，而呼叫會傳回 SQLSTATE HY091 (不正確描述項欄位識別碼) 。  
 ...
 下表列出 **SQLColAttribute**傳回的描述元類型。 *NumericAttributePtr*值的型別為**SQLLEN \* **。  
