...
|SQL_LONGVARCHAR|LONG VARCHAR|可變長度字元資料。 最大長度是與資料來源相關的。9|  
|SQL_WCHAR|WCHAR （*n*）|固定字串長度*n*的 Unicode 字元字串|  
|SQL_WVARCHAR|VARWCHAR （*n*）|Unicode 可變長度字元字串，最大字串長度為*n*|  
|SQL_WLONGVARCHAR|LONGWVARCHAR|Unicode 可變長度字元資料。 最大長度是與資料來源相關|  
...
|SQL_LONGVARBINARY|長 VARBINARY|變數長度二進位資料。 最大長度是與資料來源相關的。9|  
