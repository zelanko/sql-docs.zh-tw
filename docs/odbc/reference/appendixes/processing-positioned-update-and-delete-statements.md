...
-   如果在包含 timestamp 資料行的資料表資料行上執行定位的 update 或 delete 語句，而且每次執行更新時都會自動更新，則所有後續定位的 update 或 delete 語句都會在時間戳記資料行系結時失敗。 發生這種情況的原因是，資料指標程式庫所建立的搜尋 update 或 delete 語句將無法正確識別要更新的資料列。 時間戳記資料行之搜尋語句中的值，不符合 timestamp 資料行自動更新的值。
