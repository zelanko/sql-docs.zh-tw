...
-   **資料的位元組長度**-數字用於傳送到資料來源的 SQL 資料的位元組，zda bude 資料會被截斷傳送至資料來源之前。 針對字串資料，這不包括 null 結束字元的空間。  
  
-   **資料行的位元組長度**-儲存在資料來源的資料所需的位元組數目。  
  
-   **字元的位元組長度**-最大字元形式來顯示資料所需的位元組數。 這是針對每個 SQL 資料類型所定義[顯示大小](../../../odbc/reference/appendixes/display-size.md)，但字元位元組長度是以位元組為單位，而顯示大小是以字元為單位。  
  
-   **數字數目**-數字來表示數字的字元，包括負號、 小數點和指數 （如有需要）。  
...
-   [C 到 SQL:Numeric](../../../odbc/reference/appendixes/c-to-sql-numeric.md)  
  
-   [C 到 SQL:Bit](../../../odbc/reference/appendixes/c-to-sql-bit.md)  
  
-   [C 到 SQL:二進位檔](../../../odbc/reference/appendixes/c-to-sql-binary.md)  
