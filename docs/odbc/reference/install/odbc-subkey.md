...
|值|意義|  
|-----------|-------------|  
|追蹤|當應用程式使用 SQL_HANDLE_ENV 選項呼叫**SQLAllocHandle**時，如果追蹤值設定為1，則會針對呼叫應用程式啟用追蹤。<br /><br /> 當應用程式使用 SQL_HANDLE_ENV 選項呼叫**SQLAllocHandle**時，如果 Trace 關鍵字設為0，則會停用呼叫應用程式的追蹤。 這是預設值。<br /><br /> 應用程式可以使用 SQL_ATTR_TRACE 連接屬性來啟用或停用追蹤。 不過，這樣做並不會變更此值的資料。|  
