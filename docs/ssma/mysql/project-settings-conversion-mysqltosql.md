...
|**設定資料類型轉換**|指定 MySQL SET 資料類型應如何轉換，轉換成 NVARCHAR (L) /Convert 到 BINARY (L) <br /><br />**預設模式**：轉換成 NVARCHAR (L) <br /><br />**開放式模式**：轉換成 NVARCHAR (L) <br /><br />**完整模式**：轉換成 NVARCHAR (L) |  
...
|**零-不是 Null 的資料行**|指定如何處理指派，而不是零日期、零日期或無效日期/時間值的非 Null 資料行。<br /><br />**預設模式**： GETDATE ( # A1<br /><br />**開放式模式**： GETDATE ( # A1<br /><br />**完整模式**： GETDATE ( # A1|  
