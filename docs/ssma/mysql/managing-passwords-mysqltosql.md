...
1.  **純文字：** 在 [密碼] 節點的 [值] 屬性中輸入資料庫密碼。 它位於腳本檔案或伺服器連接檔案之 [伺服器] 區段的 [伺服器定義] 節點底下。  
    ...
    **加密的密碼：** 在此情況下，指定的密碼會以加密格式儲存在 ProtectedStorage. ssma 中的本機電腦上。  
    ...
    -   **保護密碼**  
        ...
        -   在命令`SSMAforMySQLConsole.exe`行上`-securepassword` ，使用並新增參數來執行，並在伺服器定義區段中傳遞包含密碼節點的伺服器連接或腳本檔案。  
    ...
    -   **移除加密的密碼**  
        ...
        在命令`SSMAforMySQLConsole.exe`行上`-securepassword`使用`-remove`和參數執行，傳遞伺服器識別碼，以從本機電腦上的受保護儲存體檔案移除加密的密碼。  
    ...
    -   **列出其密碼已加密的伺服器識別碼**  
        ...
        `SSMAforMySQLConsole.exe`使用在命令列`-securepassword`上`-list`執行並切換，以列出其密碼已加密的所有伺服器識別碼。  
