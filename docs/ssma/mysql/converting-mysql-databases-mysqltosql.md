...
|資料庫轉換|做為 MySQL 物件的資料庫不會直接轉換的 SSMA for MySQL。 MySQL 資料庫會被視為一個類似的結構描述名稱和實體的所有參數都都會在轉換期間遺失。 SSMA for MySQL 使用[對應至 SQL Server 結構描述 &#40; 的 MySQL 資料庫MySQLToSQL &#41;](../../ssma/mysql/mapping-mysql-databases-to-sql-server-schemas-mysqltosql.md)對應到適當的 SQL Server 資料庫/結構描述對 MySQL 資料庫中的物件。|  
...
-   您可以修改 MySQL 資料庫，以移除或修改有問題的程式碼中的物件。 若要更新的程式碼載入 SSMA，您必須更新的中繼資料。 如需詳細資訊，請參閱[連接到 MySQL &#40;MySQLToSQL &#41;](../../ssma/mysql/connecting-to-mysql-mysqltosql.md)  
