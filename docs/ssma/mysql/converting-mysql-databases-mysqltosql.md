---
title: 將 MySQL 資料庫轉換 (MySQLToSQL) |Microsoft Docs
...
|語句轉換|-每個 SQL 語句物件都可能包含單一 MySQL 語句 (例如 DDL、DML 和其他類型的語句) 或開始 .。。結束區塊。<br />-   多重**語句轉換：開始 .。。結束區塊轉換**SQL 語句也可以包含 BEGIN .。。END 區塊，如同程式、函數或觸發程式定義中的一個。 這些區塊的轉換方式應該和轉換單一 MySQL 語句物件時相同。|  
