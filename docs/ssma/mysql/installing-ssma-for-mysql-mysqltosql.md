...
適用于 MySQL 的 SQL Server 移轉小幫手（SSMA）包含一個用戶端應用程式，可讓您用來執行[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]從 MySQL 到或 SQL Azure 的遷移。 它也包含支援資料移轉的延伸模組套件，以及在您遷移的資料庫中使用 MySQL 系統函數的功能。  
