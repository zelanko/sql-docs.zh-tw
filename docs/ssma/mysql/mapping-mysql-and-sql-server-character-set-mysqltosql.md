---
title: 將 MySQL 和 SQL Server 字元集（MySQLToSQL）對應 |Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
...
-   至國家 SQL Server 字元類型（NCHAR/NVARCHAR），或  
  
-   至一般 SQL Server 字元類型（CHAR/VARCHAR）  
...
1.  **在根中繼資料節點層級上：** [字元集對應] 方格包含 [字元集] 方格，其中每個字元集都有個別的資料行。 方格的資料行為：  
    ...
    > [!IMPORTANT]  
    > 特定字元集的預設值在 CHAR/VARCHAR 或 NCHAR/NVARCHAR 後面會有前置詞 ' （default） '。  
    ...
    |gbk|簡體中文 GBK|NCHAR/NVARCHAR （預設值）|  
    ...
    |cp932|適用于 Windows 日文的 SJIS|NCHAR/NVARCHAR （預設值）|  
    |eucjpms|適用于 Windows 日文的 UJIS|NCHAR/NVARCHAR （預設值）|  
...
> -   在方格中，**繼承**的值是以 ' （CHAR/VARCHAR） ' 或 ' （NCHAR/NVARCHAR） ' 作為尾碼，視這個特定字元集繼承自父系的值而定。  
