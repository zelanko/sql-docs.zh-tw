...
1.  複製 SSMA for MySQL 延伸模組套件。*n*.Install.exe，其中*n*是組建編號，在執行的電腦上 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 。  
  
2.  按兩下 [適用于 MySQL 的 SSMA 擴充功能套件]。*n*.Install.exe。  
...
11. 若要將公用程式安裝到另一個實例 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，請選取 **[是]**，然後按 **[下一步]**。 或者，若要結束嚮導，請按一下 [**否**]。  
