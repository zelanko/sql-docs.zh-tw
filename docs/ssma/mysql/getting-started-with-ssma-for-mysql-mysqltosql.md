---
title: 適用于 MySQL 的 SSMA (MySQLToSQL) 的消費者入門 |Microsoft Docs
...
若要使用 SSMA，您必須先在可存取來源 MySQL 資料庫和目標實例 SQL Server 或 Azure SQL Database 的電腦上安裝 SSMA 用戶端程式。 然後，在執行 SSMA 用戶端程式的電腦上， (MySQL ODBC 5.1 驅動程式 (信任的) # A3 安裝 MySQL 提供者。 如需安裝指示，請參閱[安裝 SSMA For MySQL &#40;MySqlToSql&#41;](../../ssma/mysql/installing-ssma-for-mysql-mysqltosql.md)  
...
安裝並授權 SSMA 之後，您可以使用 SSMA 將 MySQL 資料庫遷移至 SQL Server 或 Azure SQL Database。 在開始之前，最好先熟悉 SSMA 的使用者介面。 下圖顯示 SSMA 的使用者介面，包括 [中繼資料流覽程式]、[中繼資料]、[工具列]、[輸出] 窗格和 [錯誤清單] 窗格：  
...
3.  成功連線之後，MySQL 架構會出現在 MySQL Metadata Explorer 中。 以滑鼠右鍵按一下 [MySQL 中繼資料瀏覽器中的物件] 來執行工作，例如建立可評估轉換為 SQL Server/Azure SQL Database 的報表。  
...
如果您已從 [新增專案] 對話方塊的 [遷移至] 下拉式清單中選取 [Azure SQL Database]，則必須連接到 Azure SQL Database。 成功連接之後，Azure SQL Database 資料庫的階層會出現在 Azure SQL Database Metadata Explorer 中。 將 MySQL 架構轉換成 Azure SQL Database 架構之後，請在 Azure SQL Database Metadata Explorer 中選取那些已轉換的架構，然後將架構與 Azure SQL Database 同步處理。  
  
當您使用 SQL Server 或 Azure SQL Database 同步處理已轉換的架構之後，您可以返回 MySQL 中繼資料瀏覽器，並將資料從 MySQL 架構遷移至 SQL Server 或 Azure SQL Database 資料庫。  
  
如需這些工作和如何執行這些工作的詳細資訊，請參閱[將 MySQL 資料庫遷移至 SQL Server Azure SQL Database &#40;MySQLToSql&#41;](../../ssma/mysql/migrating-mysql-databases-to-sql-server-azure-sql-db-mysqltosql.md)。  
...
SQL Server 或 Azure SQL Database 中繼資料 Explorer 會顯示 SQL Server 或 Azure SQL Database 實例的相關資訊。 當您連接到 SQL Server 或 Azure SQL Database 的實例時，SSMA 會抓取該實例的中繼資料，並將它儲存在專案檔中。  
  
您可以使用此中繼資料瀏覽器選取已轉換的 MySQL 資料庫物件，然後將這些物件與 SQL Server 或 Azure SQL Database 的實例同步處理。  
  
如需詳細資訊，請參閱[同步處理 (MySQL 至 SQL Server/Azure SQL Database) ](https://msdn.microsoft.com/ac993a6d-0283-4823-8793-6b217677dfa3)  
...
[專案] 工具列包含用來處理專案、連接到 MySQL，以及連接到 SQL Server 或 Azure SQL Database 的按鈕。 這些**按鈕與 [檔案] 功能表上**的命令類似。  
...
|-|-|  
|**Button**|**Function**|  
|**建立報表**|將選取的 MySQL 物件轉換成 SQL Server 或 Azure SQL Database 物件，然後建立報表來顯示轉換的成功程度。<br /><br />除非在 MySQL Metadata Explorer 中選取物件，否則會停用此命令。|  
|**轉換架構**|將選取的 MySQL 物件轉換成 SQL Server 或 Azure SQL Database 物件。<br /><br />除非在 MySQL Metadata Explorer 中選取物件，否則會停用此命令。|  
|**遷移資料**|將資料從 MySQL 資料庫遷移至 SQL Server 或 Azure SQL Database。 執行此命令之前，您必須先將 MySQL 架構轉換成 SQL Server 或 Azure SQL Database 架構，然後將物件載入 SQL Server 或 Azure SQL Database。<br /><br />除非在 MySQL Metadata Explorer 中選取物件，否則會停用此命令。|  
|**停止**|停止目前的進程。|  
