...
例如，MySQL 資料庫中特定資料表的一般資料移轉如下的階層： 資料庫-&gt;資料表。  
  
當指令碼檔案中的所有命令都執行成功時，SSMA 主控台應用程式結束，並將控制權傳回給使用者。 指令碼檔案的內容會更多或較少的靜態變數的資訊包含在[變數值的檔案](http://msdn.microsoft.com/en-us/1dc56a7b-8e3a-4576-ad4f-47050bf7e28a)或變數值的指令碼檔案中的個別區段中。  
