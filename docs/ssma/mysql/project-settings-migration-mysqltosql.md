...
-   若要指定所有的 SSMA 專案設定**工具**功能表上，選取**預設專案設定**，選取專案類型中的**移轉的目標版本**下拉式清單按一下您要存取的設定，**一般**左的窗格中，然後再按一下底部**移轉**。  
...
|詞彙|定義|  
|--------|--------------|  
|**批次大小**|指定批次資料移轉期間使用的大小。<br /><br />**預設模式**: 1000年<br /><br />**開放式模式**: 1000年<br /><br />**完整模式**: 1000年|  
...
|**保留識別**|指定 SSMA 是否要將資料加入至 SQL Server 時保存 MySQL 識別值。 值為 False 會識別值依目的地指派。<br /><br />**預設模式**: True<br /><br />**開放式模式**: True<br /><br />**完整模式**: True|  
...
|**資料表鎖定**|指定當它將資料加入資料表資料移轉期間，SSMA 是否鎖定資料表。 大量複製作業期間，會取得大量更新鎖定。 如果值為 False 時，鎖定資料列層級設定。<br /><br />**預設模式**: False<br /><br />**開放式模式**: False<br /><br />**完整模式**: False|  
  
### <a name="data-modification"></a>資料修改  
  
|詞彙|定義|  
|--------|--------------|  
|**無效的日期移轉**|指定如何移轉具有無效的日期類似 '2007年-04-23' 或' 2000年-06-31 10:00:00 ' 日期和日期時間格式。<br /><br />**預設模式**： 設定為 NULL<br /><br />**開放式模式**： 設定為 NULL<br /><br />**完整模式**： 設定為 NULL|  
|**時間的負值移轉**|指定如何移轉負數值，類似 '-30:11:00' 時間資料行中。<br /><br />**預設模式**： 設定為 NULL<br /><br />**開放式模式**： 設定為 NULL<br /><br />**完整模式**： 設定為 NULL|  
|**過去 24 小時內移轉的時間值**|指定如何移轉時間值的多個 ' 23: 59:59' 時間資料行中。<br /><br />**預設模式**： 設定為 NULL<br /><br />**開放式模式**： 設定為 NULL<br /><br />**完整模式**： 設定為 NULL|  
|**截斷才能容納到資料行的二進位值**|如果是，SSMA 會截斷從 MySQL 不適合在 SQL 資料表資料行的二進位值，並產生適當的錯誤訊息。 如果不是，資料列會導致錯誤<br /><br />**預設模式**： 否<br /><br />**開放式模式**： 否<br /><br />**完整模式**： 否|  
|**截斷才能容納到資料行的字元值**|SSMA 會截斷從 MySQL 不適合在 SQL 資料表資料行的字元值，並產生適當的錯誤訊息。<br /><br />**預設模式**： 否<br /><br />**開放式模式**： 否<br /><br />**完整模式**： 否|  
|**零日期移轉**|指定如何移轉零的日期，例如 '0000-00-00' 或' 0000-00-00 00:00:00 ' 日期和日期時間資料行中。<br /><br />**預設模式**： 設定為 NULL<br /><br />**開放式模式**： 設定為 NULL<br /><br />**完整模式**： 設定為 NULL|  
|**零的日期移轉**|指定如何移轉像零組件的日期 '2009年-01 00' 或' 2000年-00-00 11:00:00 ' 日期和日期時間資料行中。<br /><br />**預設模式**： 設定為 NULL<br /><br />**開放式模式**： 設定為 NULL<br /><br />**完整模式**： 設定為 NULL|  
