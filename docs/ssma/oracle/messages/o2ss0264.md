...
接著，我們會呼叫此程式，並將類型的變數傳遞給它， `SYS_REFCURSOR` 以取得資料指標：
...
若要解決此錯誤，您必須先使用 SSMA 將程式（ `P_CLOSE_REFCURSOR` ）轉換為 transact-sql，並在 sql 程式碼中進行下列修改：

1. 當 SSMA 將 Oracle 程式轉換成 Transact-sql 時，它會將型別轉換 `CURSOR (@emp_refcur)` 成 `varchar(8000)` 。 但是在 SQL server 中，我們可以使用下列方式，在參數中宣告 cursor 資料型別 `OUTPUT` ： `@emp_refcur Cursor Varying OUTPUT` 。

2. 此外，SSMA 也會 `@emp_refcur` 使用值來初始化變數（其類型為 `varchar(8000)` ） `NULL` 。 因此，在變更其型別之後，我們必須藉由批註語句來移除此初始化 `SET @emp_refcur = NULL` 。
