...
請考慮下列已建立程式的範例 `print_emp` ，我們會在其中呼叫另一個傳回記錄的函式 `get_employeeTbl` 。
...
當 SSMA 嘗試轉換上述主要程式碼時 `print_emp` ，它不會解析傳回記錄之被呼叫函式的作業， `get_emploreeTbl` 因此會產生下列錯誤訊息：

> O2SS0359：無法取得函式呼叫運算式之傳回類型的描述

## <a name="possible-remedies"></a>可能的解決辦法

上述錯誤的解決方案是在 SQL Server 中重寫程式碼。 由於 SQL Server 支援純量函數、內嵌資料表值函式和多重語句資料表值函式，因此您可以在 `@mytable` 所呼叫函式的 t-sql 程式碼內宣告臨時表。 在程式碼中，您會使用相同的商務邏輯來填滿此資料表，然後將此資料表傳回給呼叫環境。 在呼叫程式中，您也必須使用 table 變數來儲存所呼叫函數的傳回值（在我們的案例中為記錄集）。
