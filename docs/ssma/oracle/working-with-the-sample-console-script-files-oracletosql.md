---
description: 使用範例主控台指令檔 (OracleToSQL)
title: 使用範例主控台指令檔 (OracleToSQL) |Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
...
-   **ServersConnectionFileSample.xml：**  
    ...
    -   如需有關連接到來源和目標伺服器的詳細資訊，請參閱 [建立伺服器連接檔案 &#40;OracleToSQL&#41;](../../ssma/oracle/creating-the-server-connection-files-oracletosql.md) 。  
...
-   **VariableValueFileSample.xml：** 在範例主控台指令檔中使用的所有變數，都已 `ServersConnectionFileSample.xml` 在這個檔案中進行定序。 若要執行範例主控台腳本，使用者必須直接以使用者定義的值取代範例變數值，並將此檔案作為額外的命令列引數一起傳遞給腳本檔案。  
    ...
    如需變數值檔案的詳細資訊，請參閱 [建立變數值檔案 &#40;OracleToSQL&#41;](../../ssma/oracle/creating-variable-value-files-oracletosql.md)。  
...
-   **AssessmentReportGenerationSample.xml：** 此範例可讓使用者產生 xml 評量報告，讓使用者在開始轉換和遷移資料之前，可供使用者用來進行分析。  
    ...
    如需有關產生報表的詳細資訊，請參閱 [&#40;OracleToSQL&#41;產生報表 ](../../ssma/oracle/generating-reports-oracletosql.md)。  
