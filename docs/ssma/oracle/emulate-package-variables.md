...
若要儲存封裝變數，SSMA for Oracle 會使用位於特殊架構中的預存程式 `ssma_oracle` 和 `ssma_oracle.db_storage` 資料表。 此資料表會依 `SPID` （會話識別碼）和登入時間進行篩選。 這項篩選可讓您區分不同會話的變數。

在每個轉換的封裝程式開始時 `ssma_oracle.db_check_init_package` ，SSMA 會呼叫特殊程式，這會檢查封裝是否已初始化，並在需要時將它初始化。 每個初始化程式都會清除儲存體資料表，並設定每個封裝變數的預設值。
...
Oracle 會 `Get` `Set` 針對封裝變數使用和方法，以避免讓其他 subprograms 直接讀取和寫入。 如果需要在相同會話中的 subprogram 呼叫之間保留一些可用的變數，這些變數就會視為全域變數。

若要克服此範圍規則，SSMA for Oracle 會 `ssma_oracle.set_pv_varchar` 針對每個變數類型使用像是的預存程式。 若要存取這些變數，SSMA 會使用一組與交易無關的 `get_*` 和 `set_*` 程式和函數。
