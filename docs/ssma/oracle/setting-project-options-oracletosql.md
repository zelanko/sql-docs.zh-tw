...
對大多數使用者建議的預設模式。 開放式模式仍可繼續多個目前的 Oracle 語法，而且更方便閱讀。 不過，保留目前的語法可能不正確。 如果 Oracle 語法必須轉換為對等項目[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]語法，完整模式執行的最完整的轉換，但產生的程式碼可能會更難閱讀。 在 [自訂] 模式中，您可以設定選項。  
