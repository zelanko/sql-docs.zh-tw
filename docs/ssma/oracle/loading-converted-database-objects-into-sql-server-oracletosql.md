...
如果您想要將轉換的資料庫物件載入 SQL Server 而不進行修改，您可以讓 SSMA 直接建立或重新建立資料庫物件。 該方法既快速又簡單，但不允許自訂可 [!INCLUDE[tsql](../../includes/tsql-md.md)] 定義 SQL Server 物件的程式碼，而不是預存程式。  
  
如果您想要修改 [!INCLUDE[tsql](../../includes/tsql-md.md)] 用來建立物件的，或如果您想要更充分掌控物件的建立，請使用 SSMA 來建立腳本。 接著，您可以修改這些腳本、個別建立每個物件，甚至使用 SQL Server Agent 來排程建立這些物件。  
...
您可以在中執行腳本或個別語句 [!INCLUDE[ssManStudioFull](../../includes/ssmanstudiofull-md.md)] 。  
