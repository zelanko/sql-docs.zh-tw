...
在 DB2 中，您可以在 XML 資料行上建立索引，以便有效率地評估 XQuery 運算式，以改善 XML 檔查詢的效能。 您也可以根據 XML 檔中的屬性或值，定義 () 之類的條件約束。
...
> [!IMPORTANT]
> 在此情況下，將額外的資料行加入至資料表 (計算資料行) 會影響所有 `SELECT *` 查詢。 請確定您的應用程式明確列出語句中的所有資料行 `SELECT` ，或忽略可能傳回的任何額外資料行。
