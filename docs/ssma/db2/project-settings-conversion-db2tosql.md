...
[**專案設定**] 對話方塊的 [轉換] 頁面包含自訂 SSMA 如何將 DB2 語法轉換[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]成語法的設定。  
...
下列範例會顯示所產生[!INCLUDE[tsql](../../includes/tsql-md.md)]的：  
...
-   如果您選取 [**使用 MERGE 語句**]，SSMA 就會將合併語句[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]轉換成中的 MERGE 語句。  
...
-   如果您選取 **[是]**，SSMA 會產生在 DB2 中的參考動作，但您必須先進行手動變更，然後再[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]將條件約束載入至。 例如，您可以選擇 [沒有動作]，而不是 [設定 Null]。  
...
某些 DB2 函式定義為自發交易，或包含在中[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]不正確語句。 在這些情況下，SSMA 會建立程式和函式，這是程式的包裝函式。 轉換的函式會呼叫執行程式。  
...
SSMA 可以將許多鎖定資料表語句轉換成資料表提示。 SSMA 無法轉換包含 PARTITION、SUBPARTITION、 @dblink和 NOWAIT 子句的任何鎖定資料表語句，而且會將這類語句標記為轉換錯誤訊息。  
...
在 DB2 中，可以使用 OPEN-FOR 語句，將結果集傳回給 REF 資料指標類型的 subprogram OUT 參數。 在[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]中，預存程式會直接傳回 SELECT 語句的結果。  
...
SSMA 可以將 DB2 SUBSTR 函式呼叫轉換[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]成**substring**函式呼叫，視參數數目而定。 如果 SSMA 無法轉換 SUBSTR 函式呼叫，或不支援參數數目，則 SSMA 會將 SUBSTR 函式呼叫轉換為自訂的 SSMA 函式呼叫。  
  
-   如果您選取 **[是]**，SSMA 會將使用三個參數的 SUBSTR [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]函式呼叫轉換成**子字串**。 其他 SUBSTR 函式將會轉換以呼叫自訂 SSMA 函數。  
...
-   如果您選取 **[是]**，SSMA [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]會從子類型建立使用者定義型別，並將它用於這個子型別的每個變數。  
...
-   如果您選取 **[是]**，SSMA [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]會根據先前的清單建立同義字和直接物件參考。  
...
-   如果您選取 **[是**]，SSMA 會將 DB2 事務[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]處理語句轉換成語句。  
  
-   如果您選取 [**否**]，SSMA 會將交易處理語句標示為轉換錯誤。  
  
> [!NOTE]  
> DB2 會隱含開啟交易。 若要在上[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]模擬這種行為，您必須在想要開始交易的位置手動加入 BEGIN TRANSACTION 語句。 或者，您也可以在會話的開頭執行 SET IMPLICIT_TRANSACTIONS ON 命令。 SSMA 會在使用自發交易轉換副程式時，自動將 SET IMPLICIT_TRANSACTIONS 新增至。  
...
-   在[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]中，Null 值是已排序清單中的最低值。 在遞增清單中，會先出現 Null 值。  
...
如果包含 INTO 子句的 SELECT 語句不會傳回任何資料列，DB2 會引發 NO_DATA_FOUND 例外狀況。 如果語句傳回兩個或多個資料列，就會引發 TOO_MANY_ROWS 例外狀況。 如果資料列計數[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]不同，中已轉換的語句不會引發任何例外狀況。  
...
當 SSMA 在中[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]建立資料表時，它可以建立 ROWID 資料行。 遷移資料時，每個資料列都會取得 newid （）函數所產生的新 UNIQUEIDENTIFIER 值。  
  
-   如果您選取 **[是]**，則會在所有資料表上建立[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [ROWID] 資料行，並在您插入值時產生 guid。 如果您打算使用 SSMA 測試人員，請一律選擇 [**是]** 。  
...
當字串[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]串連包含 Null 值時，DB2 和會傳回不同的結果。 DB2 會將 Null 值視為空的字元集。 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]傳回 Null。  
  
-   如果您選取 **[是]**，SSMA 會將 DB2 串連字號（| |） [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]取代為串連字號（+）。 SSMA 也會檢查串連兩端的運算式是否有 Null 值。  
