---
title: 評估 DB2 架構以進行轉換 (DB2ToSQL) |Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
...
ms.locfileid: "87937417"
---
# <a name="assessing-db2-schemas-for-conversion-db2tosql"></a>評估 DB2 架構以進行轉換 (DB2ToSQL) 
...
當它建立此評量報告時，SSMA 會將選取的 DB2 資料庫物件轉換成 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 語法，然後顯示結果。  
...
4.  請檢查任何錯誤訊息，然後判斷您想要對導致轉換問題的物件採取什麼動作：  
    ...
    -   您可以從遷移中排除物件。 在 [ [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 中繼資料 explorer] 和 [Db2 中繼資料 explorer] 中，清除專案旁的核取方塊，然後再將物件載入 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 並從 DB2 遷移資料。  
