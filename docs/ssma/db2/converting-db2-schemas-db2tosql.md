...
|-----------|----------------------------|  
|資料型別|**SSMA 會對應下列下面所列以外的每個類型：**<br /><br />CLOB： 某些原生函式使用這種類型的工作不支援 (例如 CLOB_EMPTY())<br /><br />BLOB： 某些原生函式使用這種類型的工作不支援 (例如 BLOB_EMPTY())<br /><br />DBLOB： 某些原生函式使用這種類型的工作不支援 (例如 DBLOB_EMPTY())|  
|使用者定義類型|**SSMA 會將對應下列使用者定義：**<br /><br />不同的型別<br /><br />結構化型別<br /><br />SQL PL 資料型別 – 附註： 不支援弱式資料指標類型。|  
...
|WHILE 陳述式|對應。|  
|取得診斷陳述式|**SSMA 會對應取得的診斷，但有下列例外狀況：**<br /><br />ROW_COUNT – 對應。<br /><br />DB2_RETURN_STATUS – 對應。<br /><br />MESSAGE_TEXT – 對應。<br /><br />DB2_SQL_NESTING_LEVEL-未對應到 SQL Server 的語意<br /><br />DB2_TOKEN_STRING-未對應到 SQL Server 的語意|  
...
|變數|會對應。|  
|例外狀況、 處理常式和條件|**SSMA 會對應 」 例外狀況處理 」，但有下列例外狀況：**<br /><br />結束處理常式 – 對應。<br /><br />復原處理常式 – 對應。<br /><br />繼續處理常式，未對應。<br /><br />條件-它不會對應到 SQL server 的語意。|  
|動態 SQL|未對應。|  
|Aliases|會對應。|  
