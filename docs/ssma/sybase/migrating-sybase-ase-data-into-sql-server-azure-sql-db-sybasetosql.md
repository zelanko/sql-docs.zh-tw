---
title: 將 Sybase ASE 資料移轉至 SQL Server Azure SQL Database |Microsoft Docs
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.reviewer: ''
...
ms.locfileid: "87934678"
---
# <a name="migrating-sybase-ase-data-into-sql-server---azure-sql-database--sybasetosql"></a>將 Sybase ASE 資料移轉至 SQL Server Azure SQL Database (SybaseToSQL) 
當您成功地將 Sybase 自動調整伺服器企業 (ASE) 資料庫物件載入 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 或 Azure SQL Database 之後，您就可以將 ase 中的資料移轉至 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 或 Azure SQL Database。  
  
> [!IMPORTANT]  
> 如果所使用的引擎是伺服器端資料移轉引擎，則在您遷移資料之前，您必須在執行 SSMA 的電腦上安裝 SSMA for Sybase ASE Extension Pack 和 Sybase ASE 提供者。 SQL Server Agent 服務也必須正在執行。 如需有關如何安裝延伸模組套件的詳細資訊，請參閱[在 SQL Server (SybaseToSQL 上安裝 SSMA 元件) ](https://msdn.microsoft.com/5ad9e12c-2cdb-4dd2-8703-05a23242d19d)  
  
## <a name="setting-migration-options"></a>設定遷移選項  
將資料移轉至 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 或 Azure SQL Database 之前，請先參閱 [**專案設定**] 對話方塊中的專案遷移選項。  
  
-   藉由使用此對話方塊，您可以設定選項，例如遷移批次大小、資料表鎖定、條件約束檢查、null 值處理和識別值處理。 如需專案遷移設定的詳細資訊，請參閱[ (遷移)  (Sybase) 的專案設定](https://msdn.microsoft.com/82f8857f-7ab1-4738-ab6e-b1e95ea94924)。  
...
-   在伺服器端資料移轉期間，引擎位於目標資料庫上。 它是透過延伸模組套件進行安裝。 如需有關如何安裝延伸模組套件的詳細資訊，請參閱[在 SQL Server (SybaseToSQL 上安裝 SSMA 元件) ](https://msdn.microsoft.com/5ad9e12c-2cdb-4dd2-8703-05a23242d19d)  
  
-   若要在伺服器端起始遷移，請在 [**專案設定**] 對話方塊中選取 [**伺服器端資料移轉引擎**] 選項。  
  
> [!NOTE]  
> 當 Azure SQL Database 做為目標資料庫時，只允許**用戶端資料移轉**，且不支援伺服器端資料移轉。  
  
## <a name="migrating-data-to-sql-server-or-azure-sql-database"></a>將資料移轉至 SQL Server 或 Azure SQL Database  
遷移資料是大量載入作業，會將資料列從 ASE 資料表移至交易中 SQL Server 資料表。 在每個交易中載入 SQL Server 或 Azure SQL Database 的資料列數目，會在專案設定中設定。  
...
1.  驗證下列項目：  
    ...
    -   您已將已轉換的物件與目標資料庫同步處理 (SQL Server 或 Azure SQL Database) 。  
...
5.  在 [**連接到 SYBASE ASE** ] 對話方塊中，輸入連接認證，然後按一下 **[連接]**。 如需連接到 Sybase ASE 的詳細資訊，請參閱[連接到 sybase &#40;SybaseToSQL&#41;](../../ssma/sybase/connect-to-sybase-sybasetosql.md)  
    ...
    如果目標資料庫是 Azure SQL Database，請在 [**連接到 Azure SQL Database** ] 對話方塊中輸入連接認證，然後按一下 **[連接]**。 如需連接到 Azure SQL Database 的詳細資訊，請參閱[連接到 Azure SQL Database &#40;SybaseToSQL&#41;](../../ssma/sybase/connecting-to-azure-sql-db-sybasetosql.md)  
