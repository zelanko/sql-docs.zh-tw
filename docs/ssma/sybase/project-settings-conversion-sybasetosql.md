...
-   **使用 ISNULL 函數包裝：** 如果設定這個選項時，每個非常 'string_expression' 中串連會包裝與 ISNULL(string_expression) 和 Null 將會取代為空字串。  
