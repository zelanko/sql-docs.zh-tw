---
description: 檢視測試案例報表 (SybaseToSQL)
title: " (SybaseToSQL) 觀看測試案例報表 |Microsoft Docs"
ms.custom: ''
ms.date: 01/19/2017
ms.prod: sql
ms.reviewer: ''
...
ms.locfileid: "88319874"
---
# <a name="viewing-test-case-reports-sybasetosql"></a>檢視測試案例報表 (SybaseToSQL)
測試案例報表會顯示測試驗證結果和一般測試資訊。 如果發生測試失敗，也會顯示已驗證物件中任何不相符資料的相關資訊。  
...
**必要條件錯誤**  
顯示在 **必要條件** 步驟中發生的任何錯誤。 通常會略過它。  
  
**初始 化**  
將執行狀態顯示為 **成功** 或 **失敗**。  
  
**測試物件結果**  
 (成功或失敗) 的結果比較，以及 SSMA 測試人員在失敗時偵測到的不符情形。  
