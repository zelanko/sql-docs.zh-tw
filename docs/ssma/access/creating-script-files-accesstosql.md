---
title: 建立指令碼檔 (AccessToSQL) |Microsoft Docs
ms.prod: sql
ms.custom: ''
ms.date: 08/17/2017
ms.reviewer: ''
...
## <a name="configuring-access-console-settings"></a>設定存取主控台  
指令碼的組態會顯示在主控台指令碼檔案。  
...
2.  **資料移轉連線提供者：** 這會指定它，則被視為資料移轉來源/目標伺服器。  來源-使用-上次使用時，表示上次使用的來源伺服器用於資料移轉。 同樣地目標-使用-上次使用會指出上次使用之目標伺服器用於資料移轉。 使用者也可以使用屬性的來源伺服器或目標伺服器指定伺服器端 （來源或目標）。  
...
4.  **重新連線提供者：** 這可讓使用者設定的連接失敗時的重新連線設定。 這可以設定來源和目標伺服器。  
...
7.  **停止作業：** 如果使用者想要停止作業，然後在中間作業期間 **' Ctrl + C'** 可用快速鍵。 SSMA for Access 主控台將會等到作業完成，並終止主控台執行。  
  
    如果使用者想要執行立即停止，然後 **' Ctrl + C'** 快速鍵可以按下一次 SSMA 主控台應用程式突然終止。  
  
8.  **進度的提供者：** 會通知每個主控台命令的進度。 這會停用預設值。 進度報告的屬性組成：  
...
9. **記錄器的詳細資訊：** 設定記錄詳細資訊層級。 這等同於使用 UI 中的 [所有類別目錄] 選項。 根據預設，記錄檔的詳細資訊層級會是"error"。  
...
10. **覆寫已加密的密碼：** 如果為 'true'，伺服器連接檔案的伺服器定義區段中，或在指令碼檔案中，如果受保護的儲存體中儲存的加密的密碼存在的覆寫，也會指定純文字密碼。 如果未指定密碼以純文字，系統會提示使用者輸入密碼。  
...
-   **最大重新連接嘗試：** 已建立的連線逾時或因網路失敗而中斷，伺服器時需要重新連接。 最大值，便允許重新連線嘗試**5**重試次數之後，主控台會自動執行重新連線。 自動重新連線的設備可減少您在重新執行指令碼的工作。  
  
## <a name="server-connection-parameters"></a>伺服器的連線參數  
在指令碼檔案，或伺服器連線檔案中，可以定義伺服器的連接參數。 請參閱[建立伺服器連線檔案&#40;AccessToSQL&#41; ](../../ssma/access/creating-the-server-connection-files-accesstosql.md)一節以取得更多詳細資料。  
  
## <a name="script-commands"></a>指令碼命令  
指令碼檔案包含一連串的移轉工作流程命令，以 XML 格式。 SSMA 主控台應用程式處理順序的命令會顯示在指令碼檔案移轉。  
  
例如，典型的資料移轉的 Access 資料庫中特定資料表如下的階層：資料庫-&gt;資料表。  
  
指令碼檔案中的所有命令都執行成功時結束 SSMA 主控台應用程式，並將控制權傳回給使用者。 指令碼檔案的內容會更多或較少的靜態變數的資訊包含在[變數值檔案](creating-variable-value-files-accesstosql.md)或變數值的指令碼檔案內的個別區段中。  
...
指令碼命令的完整清單可在[執行 SSMA 主控台&#40;AccessToSQL&#41;](../../ssma/access/executing-the-ssma-console-accesstosql.md)  
  
## <a name="script-file-validation"></a>指令碼檔案驗證  
使用者可以輕鬆地驗證他/她指令碼檔案對結構描述定義檔 **'A2SSConsoleScriptSchema.xsd'** 可用 [結構描述] 資料夾中。  
  
## <a name="next-step"></a>下一步
操作主控台的下一個步驟是[建立變數值檔案&#40;AccessToSQL&#41;](../../ssma/access/creating-variable-value-files-accesstosql.md)。  
