...
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]移轉小幫手（SSMA）進行存取可讓您快速地將 Access 資料庫物件[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]轉換成或 AZURE sql db 物件、將產生的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]物件上傳至或 azure sql db，以及將[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]資料從存取權遷移至或 azure sql db。 如有需要，您也可以將存取資料表[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]連結到或 AZURE sql db 資料表，以便繼續使用現有的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]存取前端應用程式搭配或 azure sql db。  
  
本主題將介紹安裝程式，並協助您熟悉 SSMA 使用者介面。  
  
## <a name="installing-ssma"></a>安裝 SSMA  
若要使用 SSMA，您必須先在可存取您想要遷移之[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]資料庫的電腦，以及或 AZURE SQL DB 的目標實例上安裝 SSMA 用戶端程式。 如需安裝指示，請參閱[安裝 SQL Server 移轉小幫手以存取 &#40;AccessToSQL&#41;](../../ssma/access/installing-sql-server-migration-assistant-for-access-accesstosql.md)。  
...
- 將存取架構轉換[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]為或 AZURE SQL DB 架構。

您也可以使用工具列和功能表來執行這些工作。  
  
您也必須連接到的[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]實例。 成功連接之後，[中繼資料流覽[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]器] 中[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]會顯示資料庫的階層。 將存取架構轉換為[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]架構之後，您可以在 [中繼資料流覽[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]器] 中選取那些已轉換的架構[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]，然後將架構載入至。  
  
如果您已從 [新增專案] 對話方塊的 [遷移至] 下拉式清單中選取 [Azure SQL DB]，則必須連接到 Azure SQL DB。 成功連線之後，azure SQL DB 資料庫的階層會出現在 Azure SQL DB 中繼資料 Explorer 中。 將存取架構轉換成 Azure SQL DB 架構之後，您可以在 Azure SQL DB 中繼資料 Explorer 中選取那些已轉換的架構，然後將[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]架構載入至。  
  
將已轉換的架構載入[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]或 AZURE sql db 之後，您可以返回存取中繼資料瀏覽器，並將資料從[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Access 資料庫移轉至或 azure sql db 資料庫。 如有必要，您也可以將 Access 資料表[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]連結至或 AZURE SQL DB 資料表。  
...
-   選取要轉換的物件，並將物件[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]轉換成語法。 如需詳細資訊，請參閱[轉換 Access 資料庫物件](converting-access-database-objects-accesstosql.md)。  
  
-   選取要資料移轉的物件，並將這些物件的資料[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]遷移至。 如需詳細資訊，請參閱將[存取資料移轉至 SQL Server](migrating-access-data-into-sql-server-azure-sql-db-accesstosql.md)。  
  
-   連結和取消連結存取[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]和資料表。  
  
#### <a name="sql-server-or-azure-sql-db-metadata-explorer"></a>SQL Server 或 Azure SQL DB 中繼資料 Explorer  
[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]或 Azure SQL DB 中繼資料 Explorer 會顯示實例或 Azure [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] sql db 的相關資訊。 當您連接到[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]或 AZURE SQL DB 的實例時，SSMA 會抓取該實例的中繼資料，並將它儲存在專案檔中。  
  
您可以使用 SQL Server 或 Azure SQL DB 中繼資料瀏覽器來選取已轉換的 Access 資料庫物件，並將這些物件載入（同步[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]處理）至實例或 AZURE SQL db。  
  
如需詳細資訊，請參閱將已[轉換的資料庫物件載入 SQL Server](loading-converted-database-objects-into-sql-server-accesstosql.md)。  
  
### <a name="metadata"></a>中繼資料  
每個 [中繼資料 explorer] 的右邊是描述所選物件的索引標籤。 例如，如果您在 [存取中繼資料瀏覽器] 中選取資料表，則會出現四個索引標籤：**資料表**、**類型對應**、**屬性**和**資料**。 如果您在 [中繼資料[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]瀏覽器] 中選取資料表，則會出現三個索引標籤：**資料表**、 **SQL**和**資料**。  
  
大部分的中繼資料設定都是唯讀的。 不過，您可以改變下列中繼資料：  
  
-   在 [存取中繼資料 Explorer] 中，您可以改變型別對應。 請務必在建立報表或轉換架構之前進行這些變更。  
  
-   在[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] [中繼資料] Explorer 中，您可以在 [**資料表**] 索引標籤上修改資料表和索引屬性。在[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]將架構載入之前，請先進行這些變更。 如需詳細資訊，請參閱[轉換 Access 資料庫物件](converting-access-database-objects-accesstosql.md)。  
...
|功能表|描述|  
|--------|---------------|  
|**檔案**|包含適用于 [遷移嚮導] 的命令、使用專案、新增和移除 Access 資料庫檔案，以及[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)]連接到或 AZURE SQL DB。|  
