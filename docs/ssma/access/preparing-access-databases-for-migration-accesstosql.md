...
移轉至 Access 資料庫之前[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]，您必須判斷哪些資料庫来移轉，然後確定這些資料庫準備好進行移轉。  
...
[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]移轉小幫手 (SSMA) 的存取可以找到您的 Access 資料庫。 然後，您可以匯出至這些資料庫的相關中繼資料[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]。 如需如何匯出和查詢中繼資料的詳細資訊，請參閱[匯出存取清查](http://msdn.microsoft.com/en-us/7e1941fb-3d14-4265-aff6-c77a4026d0ed)。  
  
**請注意**並非所有的存取功能和設定或不支援，可以輕鬆地轉換成[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]。 在開始移轉資料庫之前，請參閱[不相容的存取功能](http://msdn.microsoft.com/en-us/99d45b9c-e3b9-4d56-8c25-b594b887ace1)。  
...
SSMA 無法移轉使用工作群組保護的資料庫。 您可以執行下列命令以移除 Access 資料庫中的工作群組的保護：  
...
5.  選取**群組**選項中，選取**使用者**群組，並請確定**管理**選取權限。  
...
移轉程式存取資料庫之前[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]，您應該先備份您要移轉的 Access 資料庫和[!INCLUDE[ssNoVersion](../../includes/ssnoversion_md.md)]您要移轉到其中的資料庫存取物件和資料。  
...
您也可能會想要文件屬性的存取資料庫，例如資料庫物件的檔案大小、 及權限的清單。 若要產生在 Access 中，這份文件上**工具**功能表上，指向**分析**，然後按一下 **文件產生器**。  
