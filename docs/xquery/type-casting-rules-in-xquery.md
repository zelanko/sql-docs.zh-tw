---
title: "中的類型轉換規則 XQuery |Microsoft 文件"
ms.custom: 
ms.date: 03/14/2017
...
-   不要使用明確轉換**轉換為**或型別建構函式 (例如， `xs:integer("5")`)。  
...
-   基本類型可以轉換成從該基本類型所衍生的任何類型。 例如，您可以轉換成**xs: decimal**至**xs: integer**，或從**xs: decimal**至**xs: long**。  
  
-   衍生類型可以轉換成類型階層中為其上階的任何類型，並可一直轉換至其內建基本基底類型。 例如，您可以轉換成**xs: token**至**xs: normalizedstring**或**xs: string**。  
  
-   如果其基本上階可轉換成目標類型，則衍生類型就可轉換成基本類型。 例如，您可以轉型**xs: integer**的衍生類型**xs: string**、 基本類型，因為**xs: decimal**， **xs: integer**的基本的上階可轉換成**xs: string**。  
  
-   如果原始類型的基本上階可以轉換成目標類型的基本上階，則衍生類型就可轉換成另一個衍生類型。 例如，您可以轉換成**xs: integer**至**xs: token**，因為您可以轉換成**xs: decimal**至**xs: string**。  
  
-   將使用者定義型別轉換成內建類型的規則與內建類型的規則相同。 例如，您可以定義**myInteger**類型衍生自**xs: integer**型別。 然後， **myInteger**可轉換成**xs: token**，因為**xs: decimal**可轉換成**xs: string**。  
  
 不支援下列種類的轉換：  
  
-   不允許對清單類型進行轉換。 這包括使用者定義的清單類型和內建清單類型例如**xs: idrefs**， **xs: entities**，和**xs: nmtokens**。  
  
-   轉型往或來自**xs: qname**不支援。  
  
-   **xs: notation**和持續時間，完全排序子類型**xdt: yearmonthduration**和**xdt: daytimeduration**，不支援。 因此不支援對這些類型的轉換。  
...
```  
 ...
 在此範例中，兩個值傳遞至 XQuery **min （)**函式都是不同類型。 因此，會執行隱含轉換，其中**整數**類型升級至**double**兩個**double**值進行比較。  
...
-   不允許字串類型的隱含轉換。 例如，如果兩個**字串**預期有型別，以及您要傳入**字串**和**語彙基元**，不會隱含轉換，並傳回錯誤。  
...
##### <a name="casting-a-value-to-a-string-or-untypedatomic-type"></a>將值轉換成字串或 untypedAtomic 類型。  
 轉換成 string 或 untypedAtomic 類型會將值轉換成其 XQuery 標準語彙表示法。 具體而言，這意謂著在輸入期間已遵守特定模式或其他條件約束的值，將不會根據該條件約束來表示。  這個問題，通知使用者[!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)]旗標，其中的類型條件約束可能會產生問題的這些類型載入結構描述集合時提供警告。  
