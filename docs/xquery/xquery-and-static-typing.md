...
### <a name="example-function-over-union-type"></a>範例聯集類型的函式  
 請考慮的項目定義 <`r`> 的聯集類型：  
...
### <a name="example-operator-over-union-type"></a>範例聯集類型的運算子  
 加法運算 ('+') 需要精確的運算元類型。 如此一來，運算式`(//r)[1] + 1`傳回靜態錯誤具有先前所述的類型定義項目的 <`r`>。 有一種解決方法，就是將它改寫成 `(//r)[1] cast as xs:int? +1`，其中 "?" 表示出現次數 0 或 1。 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 需要 "cast as" 含有 "?"，因為任何轉換都可能會因執行階段錯誤，而造成空的序列。  
