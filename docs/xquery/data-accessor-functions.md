...
## <a name="understanding-fndata-fnstring-and-text"></a>了解 fn:data()、fn:string() 與 text()  
 XQuery 具有函數**fn： data （）** ，可從節點解壓縮純量、具類型的值、節點測試**文字（）** 以傳回文位元組點，以及函式**fn： string （）** ，可傳回節點的字串值。 它們的用法很容易混淆。 以下是在 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 中正確使用它們的指導方針。 XML 實例\<存留期>12\</age> 用於說明。  
  
-   不具類型的 XML：路徑運算式 /age/text() 會傳回文字節點 "12"。 函數 fn:data(/age) 會傳回字串值 "12"，而 fn:string(/age) 也是。  
  
-   具類型的 XML：運算式/age/text （）會針對任何簡單具類型\<的 age> 專案，傳回靜態錯誤。 在另一方面，fn:data(/age) 會傳回整數 12。 fn:string(/age) 會產生字串 "12"。  
