...
-   **的界限空格** 原則設定為刪除。 因此，邊界空格不會保留**任何項目**和**屬性**建構函式在查詢中的。 例如：  
...
-   將針對下列各項，初始化前置詞和命名空間的繫結：  
    ...
    -   任何使用 WITH XMLNAMESPACES 定義的命名空間。 如需詳細資訊，請參閱[查詢使用 WITH XMLNAMESPACES 新增命名空間](../relational-databases/xml/add-namespaces-to-queries-with-with-xmlnamespaces.md))。  
  
    -   任何在查詢初構中定義的命名空間。 請注意，初構中的命名空間宣告可能覆寫 WITH XMLNAMESPACES 中的命名空間宣告。 例如，在下列查詢中，WITH XMLNAMESPACES 宣告將它繫結至命名空間前置詞 (pd) (`http://someURI`)。 不過，在 WHERE 子句中，查詢初構會覆寫繫結。  
...
-   如果查詢具型別的**xml**資料行或變數，資料行或變數相關聯的 XML 結構描述集合的元件會匯入至靜態內容。 如需詳細資訊，請參閱 [比較具類型的 XML 與不具類型的 XML](../relational-databases/xml/compare-typed-xml-to-untyped-xml.md)。  
  
-   對於所匯入之結構描述中的每一個不可部份完成類型，也可以在靜態內容中使用轉換函數。 下列範例會加以說明。 在此範例中，指定查詢針對具類型**xml**變數。 與此變數相關聯之 XML 結構描述集合會定義不可部份完成類型 myType。 對應於此類型的轉換函數**myType()**，靜態分析時仍可使用。 查詢運算式 (`ns:myType(0)`) 傳回 myType 的值。  
    ...
    ```  
     ...
     在下列範例中，轉換函數**int**運算式中指定內建 XML 類型。  
...
-   若為 XML 建構，只支援 Strip 建構模式。 這是預設值。 因此，建構的項目節點的類型為**xdt： 不具型別的**型別和屬性都屬於**xdt: untypedatomic**型別。  
...
-   與相關聯的定序**xml**使用的資料類型。 此定序一律設為 Unicode 字碼指標定序。  
  
## <a name="dynamic-context"></a>動態內容  
 動態內容是指在執行運算式時必須提供的資訊。 除了靜態內容之外，下列資訊也會初始化成為動態內容的一部份：  
  
-   運算式焦點如內容項目、內容位置和內容大小皆已初始化，如後文所示。 請注意，所有這些值可以覆寫[nodes （） 方法](../t-sql/xml/nodes-method-xml-data-type.md)。  
  
    -   **Xml**資料型別設定內容項目，這是要處理，文件節點的節點。  
...
-   **目前日期和時間**內容函數**fn:current-日期**， **fn:current-時間**，和**fn:current-dateTime**，不是支援。  
