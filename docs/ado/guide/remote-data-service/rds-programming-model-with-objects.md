...
-   指定要在伺服器上叫用的程式，並取得 (proxy) 從用戶端 (RDS 參考它的方法 [。](../../../ado/reference/rds-api/dataspace-object-rds.md)) 的空間。  
  
-   叫用伺服器程式。 將參數傳遞至識別資料來源的伺服器程式，以及要發出 (proxy 或 RDS 的命令 [。DataControl](../../../ado/reference/rds-api/datacontrol-object-rds.md)) 。  
