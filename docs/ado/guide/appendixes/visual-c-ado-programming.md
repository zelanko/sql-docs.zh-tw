 ...
 **#Import** Visual C++ 編譯器指示詞可簡化使用 ADO 方法和屬性的作業。 指示詞會採用包含類型程式庫的檔案名，例如 ADO （Msado15.dll），並產生包含 typedef 宣告的標頭檔、介面的智慧型指標，以及列舉的常數。 在類別中，每個介面都會封裝或包裝。  
 ...
 針對類別中的每個作業（也就是方法或屬性呼叫），有一個宣告可直接呼叫作業（也就是作業的「原始」形式），而一個宣告用於呼叫原始作業，而如果作業無法順利執行，則會擲回 COM 錯誤。 如果作業是屬性，通常會有一個編譯器指示詞，它會針對具有類似 Visual Basic 語法的運算建立替代語法。  
...
## <a name="com-specific-data-types"></a>COM 特有的資料類型  
 一般而言，您在 ADO API 參考中找到的任何 Visual Basic 資料類型都有 Visual C++ 對等的。 其中包括標準資料類型（例如不**帶正負**號的 char 用於 Visual Basic 的**位元組**）、**整數**的**Long** **short** **，以及 long** 。 查看語法 Indexesto，以確切查看給定方法或屬性的運算元所需的內容。  
