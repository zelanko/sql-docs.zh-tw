...
-   *值*是與您將會比較欄位值的值 (例如`'Smith'`， `#8/24/95#`， `12.345`，或`$50.00`)。 使用字串中使用單引號 （'） 與井字符號 (`#`) 的日期。 對於數字，您可以使用小數位數、 貨幣符號和科學記號標記法。 如果*運算子*是**像是**，*值*可以使用萬用字元。 只有星號 (\*) 和百分比符號 （%） 萬用字元所允許的字元，而且必須是字串中的最後一個字元。 *值*不可為 null。  
