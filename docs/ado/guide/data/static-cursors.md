...
ms.locfileid: "47654186"
---
# <a name="static-cursors"></a>靜態資料指標
靜態資料指標一律會顯示的結果集時的第一次開啟資料指標。 根據實作中，靜態資料指標都是唯讀或讀取/寫入，並提供向前及向後捲動。 靜態資料指標通常無法偵測成員資格、 順序或結果集資料指標開啟後的值所做的變更。 靜態資料指標可能會偵測到他們自己的更新、 刪除和插入，雖然不需要執行這項操作。  
  
 靜態資料指標永遠不會偵測其他更新、 刪除和插入。 例如，假設在靜態資料指標提取資料列，以及另一個應用程式，然後更新該資料列。 如果應用程式 refetches 靜態資料指標的資料列，則它所看到的值會是不變，即使另一個應用程式所做的變更。 支援所有類型的捲動，但提供者可能會或可能不支援書籤。  
