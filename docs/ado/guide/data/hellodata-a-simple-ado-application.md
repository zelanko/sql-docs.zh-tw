...
1.  建立參考 ADO 程式庫的新標準 EXE Visual Basic 專案。 如需詳細資訊，請參閱 [參考 ADO 程式庫](../referencing-the-ado-libraries.md)。  
...
5.  最後，複製 [HelloData 程式碼](./hellodata-code.md)中所列的程式碼，並將它貼入表單的 [程式碼編輯器] 視窗中。 按 **F5** 以執行程式碼。  
  
> [!NOTE]
>  在下列範例中，和整個指南中，會使用密碼為 "123aBc" 的使用者識別碼 "MyId" 來對伺服器進行驗證。 您應以伺服器的有效登入認證取代這些值。 此外，請將 "MySQLServer" 值取代為您伺服器的名稱。  
  
 如需程式碼的詳細說明，請參閱 [HelloData 上的批註](./comments-on-hellodata.md)。  
