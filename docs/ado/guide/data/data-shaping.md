---
title: "資料成形 |Microsoft 文件"
ms.prod: sql-non-specified
ms.prod_service: drivers
ms.service: 
ms.component: guide
ms.technology:
- drivers
ms.custom: 
ms.date: 01/19/2017
ms.reviewer: 
ms.suite: sql
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- data shaping [ADO]
ms.assetid: 62bd7dc9-45b5-4ca9-8b52-457325e0ce9e
caps.latest.revision: 9
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.workload: Inactive
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 1ad3b257166308fe2c5a4bd94da5427dc0ca2649
ms.contentlocale: zh-tw
ms.lasthandoff: 09/09/2017

---
# <a name="data-shaping"></a>塑造資料
此章節包含下列主題。  
  
-   [資料成形概觀](../../../ado/guide/data/data-shaping-overview.md)  
  
-   [資料 ShapingExample](../../../ado/guide/data/data-shaping-example.md)  
  
-   [才可遏制](../../../ado/guide/data/reshaping.md)  
  
-   [身為下下層的彙總](../../../ado/guide/data/grandchild-aggregates.md)  
  
-   [參數化的命令與中介計算命令](../../../ado/guide/data/parameterized-commands-with-intervening-compute-commands.md)  
  
-   [保存的階層式資料錄集](../../../ado/guide/data/persisting-hierarchical-recordsets.md)  
  
-   [提供者所需的資料成形](../../../ado/guide/data/required-providers-for-data-shaping.md)  
  
-   [在一般的圖形命令](../../../ado/guide/data/shape-commands-in-general.md)  
  
-   [圖形 APPEND 子句](../../../ado/guide/data/shape-append-clause.md)  
  
-   [圖形 COMPUTE 子句](../../../ado/guide/data/shape-compute-clause.md)  
  
-   [Fabricating 階層式資料錄集](../../../ado/guide/data/fabricating-hierarchical-recordsets.md)  
  
-   [存取資料列中的階層式資料錄集](../../../ado/guide/data/accessing-rows-in-a-hierarchical-recordset.md)  
  
-   [型式圖形文法](../../../ado/guide/data/formal-shape-grammar.md)  
  
-   [Visual Basic 應用程式函式](../../../ado/guide/data/visual-basic-for-applications-functions.md)

