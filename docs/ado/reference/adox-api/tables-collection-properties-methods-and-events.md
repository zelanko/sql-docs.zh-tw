---
title: "資料表集合屬性、 方法和事件 |Microsoft 文件"
ms.prod: sql-non-specified
ms.technology:
- drivers
ms.custom: 
ms.date: 01/19/2017
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
helpviewer_keywords:
- Tables collection [ADOX], members
ms.assetid: 98101a52-53a6-4287-a8d9-2a7c76c3e0b9
caps.latest.revision: 11
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 6432ccf66838640bf02021d188d5b62d07658e15
ms.contentlocale: zh-tw
ms.lasthandoff: 09/09/2017

---
# <a name="tables-collection-properties-methods-and-events"></a>資料表集合屬性、 方法和事件
## <a name="properties"></a>屬性  
 [Count 屬性](../../../ado/reference/ado-api/count-property-ado.md)  
  
 [項目屬性](../../../ado/reference/ado-api/item-property-ado.md)  
  
## <a name="methods"></a>方法  
 [Append 方法 （資料表）](../../../ado/reference/adox-api/append-method-adox-tables.md)  
  
 [Delete 方法 （集合）](../../../ado/reference/adox-api/delete-method-adox-collections.md)  
  
 [重新整理方法](../../../ado/reference/ado-api/refresh-method-ado.md)  
  
## <a name="events"></a>事件  
 無。  
  
## <a name="see-also"></a>另請參閱  
 [資料表集合 (ADOX)](../../../ado/reference/adox-api/tables-collection-adox.md)
