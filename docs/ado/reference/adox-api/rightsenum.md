...
|**adRightDelete**|65536 (& H 10000)|使用者或群組已刪除的資料，從物件的權限。 物件，例如**資料表**，使用者有權從記錄中刪除資料值。|  
...
|**adRightMaximumAllowed**|33554432 (& H 2000000)|使用者或群組具有權限提供者所允許的最大數目。 特定權限會提供者而不同。|  
...
|**adRightReadDesign**|1024 (& H400)|使用者或群組具有讀取物件設計的權限。|  
|**adRightReadPermissions**|131072 （& H20000）|使用者或群組可以檢視，但不是變更，請在目錄中物件的特定權限。|  
|**adRightReference**|8192 (& H2000)|使用者或群組具有參考物件的權限。|  
|**adRightUpdate**|1073741824 （& H40000000）|使用者或群組已更新之物件的權限。 物件，例如**資料表**，使用者已更新資料表中的資料的權限。|  
|**adRightWithGrant**|4096 (& H1000)|使用者或群組具有權限授與權限的物件。|  
|**adRightWriteDesign**|2048 (& H800)|使用者或群組有權修改物件的設計。|  
