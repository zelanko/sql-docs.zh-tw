---
title: 型別屬性 （資料表） (ADOX) |Microsoft 文件
ms.prod: sql
ms.prod_service: drivers
ms.service: ''
ms.component: ado
ms.technology:
- drivers
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.suite: sql
ms.tgt_pltfrm: ''
ms.topic: conceptual
apitype: COM
f1_keywords:
- _Table::Type
- _Table::GetType
- _Table::get_Type
helpviewer_keywords:
- Type property [ADOX]
ms.assetid: 7b6e14bb-fb69-4d74-aaca-f5d380f4d887
caps.latest.revision: 11
author: MightyPen
ms.author: genemi
manager: craigg
ms.workload: Inactive
ms.openlocfilehash: ccff8b905f65f4bb1225f66289be777d095be3c1
ms.sourcegitcommit: bb044a48a6af9b9d8edb178dc8c8bd5658b9ff68
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/18/2018
---
# <a name="type-property-table-adox"></a>型別屬性 （資料表） (ADOX)
指出資料表類型。  
  
## <a name="return-values"></a>傳回值  
 傳回**字串**值，指定的類型; 例如"TABLE"、"系統 TABLE"或"全域暫存 」 的資料表。  
  
## <a name="remarks"></a>備註  
 此屬性是唯讀的。  
  
## <a name="applies-to"></a>適用於  
 [Table 物件 (ADOX)](../../../ado/reference/adox-api/table-object-adox.md)  
  
## <a name="see-also"></a>另請參閱  
 [目錄 ActiveConnection 屬性範例 (VB)](../../../ado/reference/adox-api/catalog-activeconnection-property-example-vb.md)   
 [連接關閉方法，資料表類型屬性範例 (VB)](../../../ado/reference/adox-api/connection-close-method-table-type-property-example-vb.md)   
 [型別屬性 （資料行） (ADOX)](../../../ado/reference/adox-api/type-property-column-adox.md)   
 [型別屬性 (Key) (ADOX)](../../../ado/reference/adox-api/type-property-key-adox.md)   
 [Table 物件 (ADOX)](../../../ado/reference/adox-api/table-object-adox.md)
