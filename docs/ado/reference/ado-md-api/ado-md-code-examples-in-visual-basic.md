---
title: "在 Visual Basic 中的 ADO MD 程式碼範例 |Microsoft 文件"
ms.prod: sql-non-specified
ms.technology:
- drivers
ms.custom: 
ms.date: 01/19/2017
ms.reviewer: 
ms.suite: 
ms.tgt_pltfrm: 
ms.topic: article
dev_langs:
- VB
helpviewer_keywords:
- ADO MD code examples, Visual Basic
ms.assetid: bfb52e8c-cb06-47a0-931c-03b8bdb1a05a
caps.latest.revision: 9
author: MightyPen
ms.author: genemi
manager: jhubbard
ms.translationtype: MT
ms.sourcegitcommit: f7e6274d77a9cdd4de6cbcaef559ca99f77b3608
ms.openlocfilehash: 6cc3b9cfb294610b39bf0a09fe704dcff325ee2a
ms.contentlocale: zh-tw
ms.lasthandoff: 09/09/2017

---
# <a name="ado-md-code-examples-in-visual-basic"></a>在 Visual Basic 中的 ADO MD 程式碼範例
下列範例會要求 Microsoft Visual Basic 版本 5.0 Service Pack 3 或 Visual Basic 6.0 版。  
  
> [!NOTE]
>  貼上整個程式碼範例中，從開頭至結尾，您的程式碼編輯器中。 如果使用部分的範例，或遺失段落格式時，此範例可能無法正確執行。  
  
-   [資料格集範例](../../../ado/reference/ado-md-api/cellset-example-vb.md)  
  
-   [類別目錄的範例](../../../ado/reference/ado-md-api/catalog-example-vb.md)
