---
title: 連接物件屬性、 方法和事件 |Microsoft 文件
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.suite: sql
ms.tgt_pltfrm: ''
ms.topic: conceptual
helpviewer_keywords:
- Connection object [ADO], members
ms.assetid: f571b74d-b796-4009-9c66-6a36ab995a2a
caps.latest.revision: 5
author: MightyPen
ms.author: genemi
manager: craigg
ms.openlocfilehash: a414ffc7fea1a269024a56dedd7221c2f5c5fda1
ms.sourcegitcommit: 62826c291db93c9017ae219f75c3cfeb8140bf06
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 06/11/2018
ms.locfileid: "35276947"
---
# <a name="connection-object-properties-methods-and-events"></a>連接物件屬性、 方法和事件
## <a name="propertiescollections"></a>/ 屬性的集合  
 [屬性的內容](../../../ado/reference/ado-api/attributes-property-ado.md)  
  
 [CommandTimeout 屬性](../../../ado/reference/ado-api/commandtimeout-property-ado.md)  
  
 [ConnectionString 屬性](../../../ado/reference/ado-api/connectionstring-property-ado.md)  
  
 [ConnectionTimeout 屬性](../../../ado/reference/ado-api/connectiontimeout-property-ado.md)  
  
 [CursorLocation 屬性](../../../ado/reference/ado-api/cursorlocation-property-ado.md)  
  
 [DefaultDatabase 屬性](../../../ado/reference/ado-api/defaultdatabase-property.md)  
  
 [錯誤集合](../../../ado/reference/ado-api/errors-collection-ado.md)  
  
 [IsolationLevel 屬性](../../../ado/reference/ado-api/isolationlevel-property.md)  
  
 [模式屬性](../../../ado/reference/ado-api/mode-property-ado.md)  
  
 [屬性集合](../../../ado/reference/ado-api/properties-collection-ado.md)  
  
 [提供者屬性](../../../ado/reference/ado-api/provider-property-ado.md)  
  
 [State 屬性](../../../ado/reference/ado-api/state-property-ado.md)  
  
 [Version 屬性](../../../ado/reference/ado-api/version-property-ado.md)  
  
## <a name="methods"></a>方法  
 [BeginTrans、 CommitTrans 和 RollbackTrans 方法](../../../ado/reference/ado-api/begintrans-committrans-and-rollbacktrans-methods-ado.md)  
  
 [Cancel 方法](../../../ado/reference/ado-api/cancel-method-ado.md)  
  
 [Close 方法](../../../ado/reference/ado-api/close-method-ado.md)  
  
 [Execute 方法 (ADO Connection)](../../../ado/reference/ado-api/execute-method-ado-connection.md)  
  
 [Open 方法 (ADO Connection)](../../../ado/reference/ado-api/open-method-ado-connection.md)  
  
 [OpenSchema 方法](../../../ado/reference/ado-api/openschema-method.md)  
  
## <a name="events"></a>事件  
 [BeginTransComplete、 CommitTransComplete 和 RollbackTransComplete 事件](../../../ado/reference/ado-api/begintranscomplete-committranscomplete-and-rollbacktranscomplete-events-ado.md)  
  
 [ConnectComplete 並中斷連線事件](../../../ado/reference/ado-api/connectcomplete-and-disconnect-events-ado.md)  
  
 [ExecuteComplete 事件](../../../ado/reference/ado-api/executecomplete-event-ado.md)  
  
 [InfoMessage 事件](../../../ado/reference/ado-api/infomessage-event-ado.md)  
  
 [WillConnect 事件](../../../ado/reference/ado-api/willconnect-event-ado.md)  
  
 [WillExecute 事件](../../../ado/reference/ado-api/willexecute-event-ado.md)  
  
## <a name="see-also"></a>另請參閱  
 [Connection 物件 (ADO)](../../../ado/reference/ado-api/connection-object-ado.md)
