---
title: ADO 程式碼範例 |Microsoft Docs
ms.prod: sql
ms.prod_service: connectivity
ms.technology: connectivity
ms.custom: ''
ms.date: 01/19/2017
ms.reviewer: ''
ms.topic: conceptual
helpviewer_keywords:
- ADO code examples
- ADO, code examples
ms.assetid: eb16724f-6fca-4873-b005-68626a8999b2
author: MightyPen
ms.author: genemi
ms.openlocfilehash: e46b5b081529d57456d1f6e8a96b057a8fc9c774
ms.sourcegitcommit: e042272a38fb646df05152c676e5cbeae3f9cd13
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 04/27/2020
ms.locfileid: "67921117"
---
# <a name="ado-code-examples"></a>ADO 程式碼範例
使用下列程式碼範例，瞭解如何使用 ADO 物件、方法、屬性和事件。  
  
> [!NOTE]
>  將整個程式碼範例貼到您的程式碼編輯器中。 如果使用部分範例或遺失段落格式，則此範例可能無法正確執行。  
  
-   [Visual Basic 中的 ADO 程式碼範例](../../../ado/reference/ado-api/ado-code-examples-in-visual-basic.md)  
  
-   [VBScript 中的 ADO 程式碼範例](../../../ado/reference/ado-api/ado-code-examples-vbscript.md)  
  
-   [Visual C++ 中的 ADO 程式碼範例](../../../ado/reference/ado-api/ado-code-examples-in-visual-c.md)  
  
-   [JScript 中的 ADO 程式碼範例](../../../ado/reference/ado-api/ado-code-examples-in-microsoft-jscript.md)
