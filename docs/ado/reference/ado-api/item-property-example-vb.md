...
這個範例示範 [Item](./item-property-ado.md) 屬性如何存取集合的成員。 此範例會使用參數化命令開啟***Pubs***資料庫的***作者***資料表。  
 ...
 針對資料庫發出的命令中的參數會依照索引和名稱從 [命令](./command-object-ado.md) 物件的 [Parameters](./parameters-collection-ado.md) 集合存取。 然後，會根據索引和名稱，從該物件的[fields](./fields-collection-ado.md)集合存取傳回之[記錄集](./recordset-object-ado.md)的欄位。  
