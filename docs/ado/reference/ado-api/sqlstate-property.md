...
## <a name="remarks"></a>備註  
 使用**SQLState**屬性可讀取在處理 SQL 語句期間發生錯誤時，提供者所傳回的五個字元錯誤碼。 例如，搭配 Microsoft SQL Server 資料庫使用適用于 ODBC 的 Microsoft OLE DB 提供者時，SQL 狀態錯誤碼會源自 ODBC，這取決於 ODBC 的特定錯誤，或源自 Microsoft SQL Server 的錯誤，然後對應至 ODBC出錯. 這些錯誤碼記載在 ANSI SQL 標準中，但可能會因不同的資料來源而以不同的方式執行。  
