...
## <a name="remarks"></a>備註  
 使用[連接](../../../ado/reference/ado-api/connection-object-ado.md)物件或[命令](../../../ado/reference/ado-api/command-object-ado.md)物件上的**CommandTimeout**屬性，以允許取消[執行](../../../ado/reference/ado-api/execute-method-ado-command.md)方法呼叫，因為網路流量或伺服器使用繁重的延遲。 如果在 **CommandTimeout** 屬性中設定的間隔在命令完成執行之前已經過，則會發生錯誤，且 ADO 會取消命令。 如果您將屬性設定為零，則 ADO 會無限期等候，直到執行完成為止。 請確定您撰寫程式碼的提供者和資料來源支援 **CommandTimeout** 功能。  
