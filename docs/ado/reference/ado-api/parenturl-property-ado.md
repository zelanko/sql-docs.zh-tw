...
ms.locfileid: "88773237"
---
# <a name="parenturl-property-ado"></a>ParentURL 屬性 (ADO)
指出指向目前**記錄**物件之父[記錄](./record-object-ado.md)的絕對 URL 字串。  
  
## <a name="return-value"></a>傳回值  
 傳回 **字串** 值，表示父 **記錄**的 URL。  
  
## <a name="remarks"></a>備註  
 **ParentURL**屬性取決於用來開啟**記錄**物件的來源。 例如，您可以使用包含[ActiveConnection](./activeconnection-property-ado.md)屬性所參考目錄之相對路徑名稱的來源來開啟**記錄**。  
...
> [!NOTE]
>  只有檔來源提供者（例如 [Microsoft OLE DB Provider For Internet 發佈](../../guide/appendixes/microsoft-ole-db-provider-for-internet-publishing.md)）支援此屬性。 如需詳細資訊，請參閱 [記錄和提供者提供的欄位](../../guide/data/records-and-provider-supplied-fields.md)。  
