...
## <a name="a"></a>A  
 ...
 alias  
 您授與資料行或運算式中的 SQL SELECT 陳述式，通常較短或更有意義的替代名稱。 比方說，BobSales 是下列的 SELECT 陳述式中的別名: 「 從 SalesDB BobSales 為選取了 wr 銷售額 」。 別名可用來以動態方式將 DataControl 物件上的控制項繫結中的資料行。  
...
## <a name="c"></a>c  
 ...
 COM 元件  
 二進位檔案，例如.dll、.ocx，以及一些.exe 檔案 —，支援 COM 標準提供的物件。 這類檔案包含一或多個 class factory，COM 類別、 登錄項目機制，載入程式碼，以及等等的程式碼。  
