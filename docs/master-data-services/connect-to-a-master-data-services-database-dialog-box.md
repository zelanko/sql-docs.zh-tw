...
|**密碼**|指定已與使用者帳戶建立關聯的密碼：<br /><br /> 當 [驗證類型] 為 [目前使用者 - 整合式安全性]，[密碼] 方塊會是唯讀的，而且將會使用指定的 Windows 使用者帳戶認證來連接。<br /><br /> 當 [驗證類型] 為 [SQL Server 帳戶]，將會啟用 [密碼] 方塊，而且您必須指定與特定使用者帳戶相關聯的密碼。|  
|**Connect**|使用指定的認證連接到 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 執行個體。|  
