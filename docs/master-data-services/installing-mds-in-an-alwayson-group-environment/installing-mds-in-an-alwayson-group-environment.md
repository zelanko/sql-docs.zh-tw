...
本文說明 Always On 可用性群組設定上所裝載之 Master Data Service （MDS）的解決方案。 本文說明如何在 SQL 2016 Always On 可用性群組（AG）上安裝和設定 SQL 2016 Master Data Services。 此解決方案的主要目的為提升 SQL Server 資料庫所裝載 MDS 後端資料的高可用性及災害復原。
...
這篇文章中所述的這個解決方案是建置於 AG 的上層，其中每個資料庫都有多個同步或非同步複本。 只有一個複本可接受交易 (接受使用者要求)。 這是主要複本。
...
此外，解決方案會使用 **MDS-HA1** 和 **MDS-HA2** 這兩個 VM 來裝載兩個複本。 只要 AG 支援，MDS 就不會限制您可以使用多少複本。
...
- 如何使用 SQL Server Always On 容錯移轉叢集實例（FCI）來支援 MDS 後端上的高可用性（HA）。 SQL Server 容錯移轉叢集是不同的 HA 解決方案並由 SQL Server 正式支援，而且可以與 MDS 搭配運作。

- 如何使用 FCI 和 AG 的混合式方案來支援 MDS 後端上的 HA。 混合式解決方案確實可以與 MDS 搭配運作。

## <a name="design-consideration"></a>設計考量

[圖 1] 顯示了大部分用於 AG 的一般設定。 在主要資料中心內，有兩個含同步認可關聯性的複本，而且兩個複本都有 VOTE 權限。 這主要用來改善主要複本失敗時的 HA。
...
3. 選取 [容錯移轉叢集]**** 核取方塊，然後按一下 [下一步]**** 完成安裝。 請參閱圖 2。
   ...
   如果系統要求您確認是否要**新增容錯移轉叢集所需的功能**，請按一下 [**新增功能**]。 請參閱圖 3。
...
Always On 有兩個功能可提供 MDS 的高可用性和嚴重損壞修復，這兩者都是建置於 WSFC 之上。
...
在所有 SQL Server 實例中啟用 AG 功能之後，您可以在一個節點上建立包含 MDS 資料庫的新 AG。
...
3. 在 [**物件總管**中，展開 [ **Always On 高可用性**] 資料夾，然後按一下 [**新增可用性群組嚮導]** 來啟動 [**新增可用性群組嚮導]**。 請參閱圖 13。
...
8. 在 [指定複本]**** 頁面上，按一下 [接聽程式]**** 索引標籤，然後執行下列動作。 請參閱圖 18。
   ...
   > [!NOTE]
   > （選擇性）您可以選擇 [靜態 IP] 做為**網路模式**，然後輸入靜態 ip。 您也可以輸入 1433 以外的連接埠。
...
2. 在**物件總管**中，展開 [ **Always On 高可用性**] 資料夾，以滑鼠右鍵按一下您剛剛在[建立可用性群組](#create-an-availability-group)一節中建立的 AG，然後按一下 [**顯示儀表板**]。 請參閱圖 20。 新 AG 和其複本的狀態即會顯示。
...
如需有關 Always On 可用性群組的詳細資訊，請參閱[SQL Server 2016 Always On 可用性群組](../../database-engine/availability-groups/windows/always-on-availability-groups-sql-server.md)。
...
6. 如果您先前已建立暫存資料庫（請參閱[建立可用性群組](#create-an-availability-group)一節）來建立 AG，則應該卸載暫存資料庫
