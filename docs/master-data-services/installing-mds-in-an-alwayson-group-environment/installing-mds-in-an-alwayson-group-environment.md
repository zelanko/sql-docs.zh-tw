---
title: 高可用性與災害復原
description: 在 Always On 可用性群組上安裝和設定 SQL Master Data Services，以改善後端資料的高可用性和嚴重損壞修復。
...
本文描述 Always On 可用性群組設定上所裝載之 Master Data Service (MDS) 的解決方案。 本文說明如何在 SQL 2016 Always On 可用性群組 (AG) 上安裝和設定 SQL 2016 Master Data Services。 此解決方案的主要目的為提升 SQL Server 資料庫所裝載 MDS 後端資料的高可用性及災害復原。

## <a name="introduction"></a>簡介

本文描述 Always On 可用性群組設定上所裝載之 Master Data Service (MDS) 的解決方案。 本文說明如何在 SQL 2016 Always On 可用性群組 (AG) 上安裝和設定 SQL 2016 MDS。 此解決方案的主要目的為提升 SQL Server 資料庫所裝載 MDS 後端資料的高可用性及災害復原。
...
本文所述的這個解決方案建置於 AG 之上，其中每個資料庫都有多個同步或非同步複本。 只有一個複本可接受交易 (接受使用者要求)。 這是主要複本。
...
此外，解決方案會使用 **MDS-HA1** 和 **MDS-HA2** 這兩個 VM 來裝載兩個複本。 只要 AG 支援它，MDS 就不會限制您可以使用的複本數目。

本文假設您已具備 Windows Server、Windows Server 容錯移轉叢集、Ag 和 SQL Server MDS 的基本知識。
...
- 如何使用 SQL Server Always On 容錯移轉叢集實例 (FCI) ，以支援 MDS 後端上的 HA (高可用性。 SQL Server 容錯移轉叢集是不同的 HA 解決方案並由 SQL Server 正式支援，而且可以與 MDS 搭配運作。

- 如何使用 FCI 和 AG 的混合式解決方案，以支援 MDS 後端上的 HA。 混合式解決方案確實可以與 MDS 搭配運作。

## <a name="design-consideration"></a>設計考量

[圖 1] 顯示在 AG 中通常使用的一般設定。 在主要資料中心內，有兩個含同步認可關聯性的複本，而且兩個複本都有 VOTE 權限。 這主要用來改善主要複本失敗時的 HA。

在災害復原資料中心內，具有的次要複本包含與主要複本的非同步認可關聯性。 此資料中心通常位在與主要資料中心不同的地區。 次要複本沒有 VOTE 權限。

這項設定可用來在主要資料中心發生損毀（例如火災、地震等）時達到復原。設定以相對較低的成本達成 HA 和嚴重損壞修復。
...
2. [建立 Windows Server 容錯移轉](#create-a-windows-server-failover-cluster)叢集。

如上節中的圖 1 所示，本文所述的解決方案包含 Windows Server 容錯移轉叢集 (WSFC)。 我們需要設定 WSFC，因為 Ag 相依于 WFSC，以進行失敗偵測和容錯移轉。
...
3. 選取 [容錯移轉叢集]**** 核取方塊，然後按一下 [下一步]**** 完成安裝。 請參閱圖 2。
   ...
   如果系統要求您確認 **新增容錯移轉叢集所需的功能**，請按一下 [ **新增功能**]。 請參閱圖 3。
...
### <a name="create-a-windows-server-failover-cluster"></a>建立 Windows Server 容錯移轉叢集
...
- 請確定您有適當的權限可在 Active Directory 中設定 WSFC。 如果有任何問題，請參閱 [Failover Cluster Step-by-Step Guide: Configure Accounts in Active Directory](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc731002(v=ws.10)) (容錯移轉叢集逐步指南：在 Active Directory 中設定帳戶)。

如需 WSFC 的詳細資訊，請參閱 [Failover Clusters](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc732488(v=ws.10)) (容錯移轉叢集)。

## <a name="sql-server-always-on-availability-group"></a>SQL Server Always On 可用性群組

本節涵蓋下列工作。

1. [啟用 SQL Server Always On 可用性群組](#enable-sql-server-always-on-availability-groups-on-every-sql-server-instance)。

2. [建立可用性群組](#create-an-availability-group)。

3. [驗證和測試可用性群組](#validation-and-test-the-availability-group)。

Always On 有兩項功能可提供 MDS 的高可用性和嚴重損壞修復，兩者都是以 WSFC 為基礎。

- Always On 可用性群組 (AG) 

- Always On 容錯移轉叢集實例 (FCI) 。

AG 提供資料庫層級的可用性。 AG (一組使用者資料庫) 和其虛擬網路名稱會註冊為 WSFC 中的資源。
...
### <a name="enable-sql-server-always-on-availability-groups-on-every-sql-server-instance"></a>在每個 SQL Server 實例上啟用 SQL Server Always On 可用性群組
...
2. 在 **SQL Server (MSSQLSERVER) ** **屬性** ] 對話方塊中，按一下 [ **Always On 高可用性** ] 索引標籤，然後選取 [ **啟用 Always On 可用性群組** ] 核取方塊。 在 [Windows 容錯移轉叢集名稱]**** 文字方塊中顯示值時，請按一下 [確定]**** 繼續。 請參閱圖 11。

   ![啟用 Always On 可用性群組選項](media/Fig11_EnableAlwaysOn.png)
...
在所有 SQL Server 實例中啟用 AG 功能之後，您可以建立新的 AG，其中包含一個節點上的 MDS 資料庫。
...
3. 在 **物件總管**中，展開 [ **Always On 高可用性** ] 資料夾，然後按一下 [ **新增可用性群組嚮導]** ，以啟動 [ **新增可用性群組] wizard**。 請參閱圖 13。
...
8. 在 [指定複本]**** 頁面上，按一下 [接聽程式]**** 索引標籤，然後執行下列動作。 請參閱圖 18。
   ...
   > [!NOTE]
   > （選擇性）您可以選擇 [靜態 IP] 作為 [ **網路模式** ]，然後輸入靜態 ip。 您也可以輸入 1433 以外的連接埠。
...
2. 在 **物件總管**中，展開 [ **Always On 高可用性** ] 資料夾，以滑鼠右鍵按一下您剛才在 [ [建立可用性群組](#create-an-availability-group) ] 區段中建立的 AG，然後按一下 [ **顯示儀表板**]。 請參閱圖 20。 新 AG 和其複本的狀態即會顯示。
...
如需 Always On 可用性群組的詳細資訊，請參閱 [SQL Server 2016 Always On 可用性群組](../../database-engine/availability-groups/windows/always-on-availability-groups-sql-server.md)。
...
4. 完成 [建立資料庫精靈]**** ****。 如需詳細資訊，請參閱 [Master Data Services 安裝和組態](../master-data-services-installation-and-configuration.md)。

5. 按一下 [Master Data Service 組態管理員]**** 中的 [Web 應用程式]**** 設定 Web 應用程式，然後按一下 [套用]**** 以將設定套用至 MDS。 請參閱圖 23。 如需詳細資訊，請參閱 [Master Data Services 安裝和組態](../master-data-services-installation-and-configuration.md)。
...
6. 如果您先前已建立暫存資料庫 (請參閱 [create a Availability Group](#create-an-availability-group)) 來建立 AG，然後您應該卸載暫存資料庫。

   如需 Master Data Service 的詳細資訊，請參閱 [Master Data Services](../master-data-services-overview-mds.md)。

## <a name="conclusion"></a>結論

在此白皮書中，我們已瞭解如何設定 Master Data Services 後端資料庫，並將其設定為 AG 的一部分。 此組態提供 Master Data Services 後端資料庫的高可用性和災害復原。 若要執行這種設定，您必須安裝和設定 Windows Server 容錯移轉叢集、AG 及 Master Data Services。
