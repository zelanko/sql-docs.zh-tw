---
title: 從資料表匯入資料
description: 從資料表匯入資料，並對模型進行大量資料變更。 您可以使用這個程式來新增、更新和刪除 Master Data Services 資料庫中的資料。
ms.custom: ''
ms.date: 03/14/2017
ms.prod: sql
ms.prod_service: mds
...
-   您必須具有將資料插入 stg. 中的許可權。 \<name>_Leaf，stg.。 \<name>_Consolidated，stg.。 \<name>資料庫中的 _Relationship 資料表 [!INCLUDE[ssMDSshort](../includes/ssmdsshort-md.md)] 。  
  
-   您必須擁有在 \<name> 資料庫中執行 stg.udp_ _Leaf、stg.. udp \_ \<name> _Consolidated 或 stg.. udp \_ \<name> _Relationship 預存 [!INCLUDE[ssMDSshort](../includes/ssmdsshort-md.md)] 程式的許可權。  
...
1.  準備要匯入 [!INCLUDE[ssMDSshort](../includes/ssmdsshort-md.md)] 資料庫之適當暫存資料表中的成員，這包括為必要欄位提供值。 如需暫存資料表的概觀，請參閱[概觀︰從資料表匯入資料 &#40;Master Data Services&#41;](../master-data-services/overview-importing-data-from-tables-master-data-services.md)  
    ...
    -   若為分葉成員，則會 stg. 資料表。 \<name>_Leaf，其中 \<name> 參考對應的實體。 如需必要欄位的資訊，請參閱[分葉成員暫存資料表 &#40;Master Data Services&#41;](../master-data-services/leaf-member-staging-table-master-data-services.md)  
  
    -   若為合併成員，則會 stg. 資料表。 \<name>_Consolidated。 如需必要欄位的資訊，請參閱[合併成員暫存資料表 &#40;Master Data Services&#41;](../master-data-services/consolidated-member-staging-table-master-data-services.md)。  
  
    -   若要在明確階層中移動成員的位置，則會 stg. 資料表。 \<name>_Relationship。 如需必要欄位的資訊，請參閱[合併成員暫存資料表 &#40;Master Data Services&#41;](../master-data-services/relationship-staging-table-master-data-services.md)。  
