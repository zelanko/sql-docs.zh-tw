---
title: "建立網站對話方塊 （Master Data Services 組態管理員） |Microsoft 文件"
ms.custom: 
ms.date: 03/20/2017
...
|**主機標頭**|選擇性的值。 輸入主機標頭名稱。 當您想要將主機名稱 (也稱為網域名稱) 指派給使用單一 IP 位址或通訊埠的電腦時，請使用這個選項。 當您指定主機名稱時，用戶端必須使用該名稱而非 IP 位址來存取網站。 當您設定主機名稱時，要等到 DNS 伺服器擁有該主機名稱的項目之後，您才可以在 Web 瀏覽器中開啟網站。<br /><br /> 例如，如果您想讓使用者存取您的網站`http://www.contoso.com/`，您必須指定 www.contoso.com 當做主機名稱和 DNS 伺服器必須擁有它的項目。<br /><br /> 如果您的網站在內部網路上使用，您不必指定主機名稱，如果使用者輸入的伺服器名稱瀏覽器，例如`http://server_name`。 不過，如果您環境中的 DNS 伺服器設定為儲存此 Web 伺服器的其他名稱，您可以建立每個主機名稱的個別繫結，好讓使用者可以使用 DNS 伺服器所儲存的其他名稱。 如果您必須為網站設定一個以上的主機名稱，請使用 IIS 工具 (如 IIS 管理員) 來加入其他網站繫結。|  
...
[Master Data Services 的安裝和設定](../master-data-services/master-data-services-installation-and-configuration.md) [Web 應用程式需求 &#40;Master Data services&#41;](../master-data-services/install-windows/web-application-requirements-master-data-services.md)   
