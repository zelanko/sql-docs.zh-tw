---
title: "建立自訂工作流程 (Master Data Services) |Microsoft 文件"
ms.custom: 
ms.date: 03/14/2017
...
### <a name="create-the-workflow-handler-assembly"></a>建立工作流程處理常式組件  
 自訂工作流程是一個實作 <xref:Microsoft.MasterDataServices.WorkflowTypeExtender.IWorkflowTypeExtender> 介面 .NET 類別庫組件。 SQL Server MDS 工作流程整合服務會呼叫 <xref:Microsoft.MasterDataServices.WorkflowTypeExtender.IWorkflowTypeExtender.StartWorkflow%2A> 方法來執行您的程式碼。 如需範例實作的程式碼<xref:Microsoft.MasterDataServices.WorkflowTypeExtender.IWorkflowTypeExtender.StartWorkflow%2A>，請參閱[自訂工作流程的範例 &#40;Master Data services&#41;](../../master-data-services/develop/create-a-custom-workflow-example.md).  
  
 請遵循以下步驟，使用 Visual Studio 2010 建立 SQL Server MDS 工作流程整合服務可以呼叫的組件，以處理自訂工作流程：  
  
1.  在 Visual Studio 2010 中，建立新**類別庫**會使用您所選擇的語言的專案。 若要建立 C# 類別庫，請選取**Visual C# \Windows**專案類型，然後選取**類別庫**範本。 輸入您專案的名稱，例如**MDSWorkflowTest**，然後按一下**確定**。  
  
2.  加入 Microsoft.MasterDataServices.WorkflowTypeExtender.dll 的參考。 這個組件位於\<安裝資料夾 > \Master Data Services\WebApplication\bin。  
...
6.  將您的組件複製到 SQL Server MDS 工作流程整合服務可執行檔的位置中名為 Microsoft.MasterDataServices.Workflow.exe，\<安裝資料夾 > \Master Data Services\WebApplication\bin。  
...
1.  尋找 Microsoft.MasterDataServices.Workflow.exe.config 中的\<安裝資料夾 > \Master Data Services\WebApplication\bin。  
...
3.  在 “ConnectionString” 設定下方加入 “WorkflowTypeExtenders” 設定，讓標記名稱與您的工作流程處理常式組件產生關聯。 例如：  
    ...
    ```  
     ...
     內部文字\<值 > 標記的形式是\<工作流程標記 > =\<組件限定工作流程類型名稱 >。 \<工作流程標記 > 是您用來識別工作流程處理常式組件，當您建立的商務規則中的名稱[!INCLUDE[ssMDSmdm](../../includes/ssmdsmdm-md.md)]。 \<組件限定工作流程類型名稱 > 命名空間限定您的工作流程類別名稱，後面接著一個逗號，後面接著組件的顯示名稱。 如果您的組件具備強式名稱，也必須包含版本資訊及其 PublicKeyToken。 您可以包含多個\<設定 > 標記，如果您已經建立不同類型的工作流程的多個工作流程處理常式。  
  
> [!NOTE]  
>  根據您伺服器的組態，可以在嘗試儲存 Microsoft.MasterDataServices.Workflow.exe.config 檔案時，看到「存取遭到拒絕」錯誤。 如果發生這個情形，請暫時停用伺服器上的 [使用者帳戶控制 (UAC)]。 若要這樣做，開啟 [控制台] 中，按一下**系統及安全性**。 在下**行動作業中心**，按一下 **變更使用者帳戶控制設定**。 在**使用者帳戶控制設定** 對話方塊中，長條滑動到底部，以便永遠不會通知您。 重新啟動電腦，然後重複以上的步驟以編輯您的組態檔。 儲存此檔案之後，將您的 UAC 設定重設為預設層級。  
...
3.  授與 mds_workflow_service 使用者權限以執行工作流程處理常式組件。 若要這樣做，請將 mds_workflow_service user 加入至**安全性** 索引標籤**屬性**工作流程的處理常式組件和授與 mds_workflow_service 使用者讀取和執行權限。  
  
4.  授與 mds_workflow_service 使用者權限以執行 SQL Server MDS 工作流程整合服務可執行檔。 若要這樣做，請將 mds_workflow_service user 加入至**安全性** 索引標籤**屬性**的 Microsoft.MasterDataServices.Workflow.exe，請在\<安裝資料夾 > \Master Data Services\WebApplication\bin 和授與 mds_workflow_service 使用者讀取和執行權限。  
  
5.  使用名稱為 InstallUtil.exe 的 .NET 安裝公用程式，安裝 SQL Server MDS 工作流程整合服務。 InstallUtil.exe 可以在.NET 安裝資料夾，例如 C:\Windows\Microsoft.NET\Framework\v4.0.30319\\。 在提升權限的命令提示字元下輸入以下內容，安裝 SQL Server MDS 工作流程整合服務：  
...
6.  使用 Services 嵌入式管理單元啟動 SQL Server MDS 工作流程整合服務。 若要這樣做，請尋找 [服務] 嵌入式管理單元中的 SQL Server MDS 工作流程整合服務、 加以選取，然後按一下**啟動**連結。  
...
1.  中的商務規則編輯器[!INCLUDE[ssMDSmdm](../../includes/ssmdsmdm-md.md)]、 指定您的商務規則，拖曳的條件之後**啟動工作流程**動作從**外部動作**清單**然後**窗格的**動作**標籤。  
  
2.  在**編輯動作**窗格，請在**工作流程類型**方塊中，輸入識別您的工作流程處理常式組件的標記。 這是您在組態檔中，為組件指定的標記，例如 TEST。  
  
3.  （選擇性） 選取**包含成員資料**核取方塊。 選擇此核取方塊以便在 XML 中包含傳遞至工作流程處理常式的屬性名稱和值。  
  
4.  在**工作流程網站**方塊中，輸入網站的名稱。 這可能不適用於您的自訂工作流程，但是可用於加入的內容。  
  
5.  在**工作流程名稱**方塊中，輸入從 Visual Studio 工作流程的名稱。 這可能不適用於您的自訂工作流程，但是可用於加入的內容。  
...
### <a name="view-the-service-broker-queue"></a>檢視 Service Broker 佇列  
 包含當做工作流程一部分傳遞之主資料的 Service Broker 佇列為：mdm.microsoft/mdm/queue/externalaction。 佇列位在**物件總管] 中**SQL Management Studio 的 [Service Broker 節點下的[!INCLUDE[ssMDSshort](../../includes/ssmdsshort-md.md)]資料庫。 請注意，如果服務已正確清除佇列，此佇列將是空的。  
