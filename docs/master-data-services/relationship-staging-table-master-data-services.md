...
|Column Name|描述|ReplTest1|  
|-----------------|-----------------|-----------|  
|**ID**|自動指派的識別碼。|請勿在此欄位中輸入值。 如果尚未處理批次，這個欄位是空白。|  
|**RelationshipType**|[區分]<br /><br /> 所設定的關聯性類型。|可能的值為：<br /><br /> **1**：父系<br /><br /> **2**：同層級 (在相同層級)|  
|**ImportStatus_ID**|[區分]<br /><br /> 匯入程序的狀態。|可能的值為：<br /><br /> **0**：您用來指定記錄已備妥，可供暫存。<br /><br /> **1**：自動指派，表示記錄的暫存處理序已成功。<br /><br /> **2**：自動指派，表示記錄的暫存處理序已失敗。|  
...
|**HierarchyName**|[區分]<br /><br /> 明確階層名稱。 每一個合併成員只能屬於一個階層。||  
|**ParentCode**|[區分]<br /><br /> 若是父子式關聯性，即為分葉子成員或合併子成員之父系的合併成員代碼。<br /><br /> 若是同層級關聯性，即為其中一個同層級的代碼。||  
|**ChildCode**|[區分]<br /><br /> 若是父子式關聯性，即為子系的合併成員或分葉成員代碼。<br /><br /> 若是同層級關聯性，即為其中一個同層級的代碼。||  
|**[排序順序]**|選擇性<br /><br /> 一個整數，代表此成員相對於父系底下其他成員的順序。 每個子成員都應該有一個唯一識別碼。||  
