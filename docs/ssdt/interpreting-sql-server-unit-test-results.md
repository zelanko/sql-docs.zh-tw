...
如需如何變更 [測試結果]  視窗顯示內容的詳細資訊，請參閱[如何：在測試視窗中新增或移除資料行 (Visual Studio 2010)](https://msdn.microsoft.com/library/ms182508(VS.100).aspx) 或[如何：在測試視窗中新增或移除資料行 (Visual Studio 2012)](https://msdn.microsoft.com/library/ms182508.aspx)。  
...
如需如何儲存測試結果、重複使用測試結果以及將測試結果與小組共用的詳細資訊，請參閱[如何：在 Visual Studio 2010 中儲存和開啟測試結果](https://msdn.microsoft.com/library/ms404662(VS.100).aspx)或[如何：在 Visual Studio 2012 中儲存和開啟測試結果](https://msdn.microsoft.com/library/ms404662.aspx)。  
