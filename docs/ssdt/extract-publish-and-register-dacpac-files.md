---
title: 擷取、發行及註冊 .dacpac 檔案 | Microsoft Docs
ms.custom:
- SSDT
ms.date: 02/09/2017
ms.prod: sql
...
您可以從資料庫擷取 .dacpac。 擷取會從即時 SQL Server 或 Windows Azure SQL Database 建立資料庫快照集檔案 (.dacpac)，不僅包含資料庫結構描述，可能還包含使用者資料表中的資料。  
