...
SSDT 可以回溯相容，亦即您可以隨時使用 [最新的 SSDT](https://msdn.microsoft.com/library/mt204009.aspx) 設計及部署要在舊版 SQL Server 上執行的資料庫、模型、報表及封裝。 您也可以使用以下列列出的任何一個先前發行的版本。  
  
> [!NOTE]  
> 過去會使用 Visual Studio Shell 來建立已用其他各種名稱發行的 SQL Server 內容類型，包括 **SQL Server Data Tools**、 **SQL Server Data Tools - Business Intelligence**以及 **Business Intelligence Development Studio**。 舊版隨附不同的專案範本集。 若要在一個 SSDT 中取得所有專案範本，您需要使用 [最新版本](https://msdn.microsoft.com/library/mt204009.aspx)。 否則，您可能會需要安裝多個舊版本才能取得 SQL Server 中使用的所有範本。  每個 Visual Studio 版本只會安裝一個 Shell；安裝第二個 SSDT 只會新增缺少的範本。  
...
-   **適用於 Visual Studio 2010 的 SSDT** 已無法再使用，因此，請選擇較新的版本。 較新版本的 SSDT 會與您現有的 Visual Studio 2010 安裝並存執行。 SSDT 無須符合您電腦上之 Visual Studio 的完整產品版本。  
...
[下載 SSDT-BI for Visual Studio 2013](https://www.microsoft.com/download/details.aspx?id=42313)：SQL Server 2014、SQL Server 2012、SQL Server 2008 與 2008 R2  
  
[下載 SSDT-BI for Visual Studio 2012](https://www.microsoft.com/download/details.aspx?id=36843)：SQL Server 2014、SQL Server 2012、SQL Server 2008 與 2008 R2  
  
Business Intelligence Development Studio (BIDS) -BIDS 是透過 SQL Server 安裝程式安裝。 沒有 Web 下載：SQL Server 2008 與 2008 R2  
  
對於 SQL Server 2012 或 2014，您可以使用 **適用於 Visual Studio 2012 的 SSDT-BI** 或 **SSDT-BI f或 Visual Studio 2013**。 唯一的差別是兩個 Visual Studio 版本的不同。  
