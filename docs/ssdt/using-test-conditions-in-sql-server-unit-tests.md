...
您可以建立 SQL Server 單元測試，測試不應成功的行為。 這些預期的失敗有時稱為負面測試。 一些範例如下：  
...
#### <a name="to-specify-an-expected-checksum"></a>指定預期的總和檢查碼  
...
4.  在 [組態] 屬性中，按一下瀏覽 ([…]) 按鈕。  
    ...
    [TestConditionName 的設定] 對話方塊隨即出現。  
  
5.  指定要測試的資料庫連接。 如需詳細資訊，請參閱[如何：建立資料庫連接](http://msdn.microsoft.com/library/aa833420(VS.100).aspx)。  
...
8.  如果結果符合預期的測試結果，請按一下 [確定]。 否則請修改 Transact\-SQL 主體並重複步驟 6、7 和 8，直到結果符合預期為止。  
    ...
    測試條件的 [值] 欄會顯示預期的結構描述資訊。 例如，它可能會是「必須是: 2 個資料表」。  
