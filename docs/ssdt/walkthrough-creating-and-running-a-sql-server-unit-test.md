...
#### <a name="to-create-a-script-from-which-you-can-import-a-schema"></a>建立可從中匯入結構描述的指令碼  
...
#### <a name="to-import-a-database-schema-from-a-script"></a>從指令碼匯入資料庫結構描述  
  
1.  在 [專案] 功能表上，按一下 [匯入]，然後按一下 [指令碼 (\*.sql)]。  
  
2.  在閱讀過歡迎頁面之後，按一下 [下一步]。  
...
#### <a name="to-examine-the-resulting-project"></a>檢查結果專案  
...
3.  選取所有五個預存程序的核取方塊：**Sales.uspCancelOrder**、**Sales.uspFillOrder**、**Sales.uspNewCustomer**、**Sales.uspPlaceNewOrder** 與 **Sales.uspShowOrderDetails**。  
  
4.  在 [專案] 下拉式清單中，選取 [建立新的 Visual C# 測試專案]。  
  
5.  接受專案名稱和類別名稱的預設名稱，然後按一下 [確定]。  
  
6.  在測試組態對話方塊中的 [使用下列資料連接來執行單元測試]，指定連接到您稍早在此逐步解說部署的資料庫。 例如，如果使用預設部署位置 (即 LocalDB)，則按一下 [新增連接] 以指定 **(LocalDB)\Projects**。 然後，選擇資料庫的名稱。 接著，按一下 [確定] 以關閉 [連接屬性] 對話方塊。  
...
8.  在 [資料庫專案] 中，按一下 **SimpleUnitTestDB.sqlproj**。  
...
3.  在 [測試條件] 窗格中，按一下 [結果不明] 測試條件，然後按一下 [刪除測試條件]。  
  
4.  在 [測試條件] 窗格中，按一下清單中的 [純量值]，然後按一下 [加入測試條件]。  
  
5.  在 [屬性] 視窗中，將 [需要的值] 屬性設定為 100。  
...
4.  在 [測試條件] 窗格中，按一下清單中的 [預期的結構描述]，然後按一下 [加入測試條件] 。  
...
13. 在 [測試條件] 窗格中，按一下清單中的 [資料總和檢查碼]，然後按一下 [加入測試條件]。  
...
#### <a name="to-correct-the-error-in-salesuspfillorder"></a>更正 Sales.uspFillOrder 的錯誤  
...
#### <a name="to-update-the-stored-procedure"></a>更新預存程序  
...
3.  在 測試條件  窗格，按一下 結果不明 測試條件，然後按一下 刪除測試條件  圖示。  
  
4.  在 測試條件 窗格中，按一下清單中的 純量值，然後按一下 加入測試條件  圖示。  
  
5.  在 [屬性] 視窗中，將 [需要的值] 屬性設定為 0。  
...
#### <a name="to-modify-the-code-for-the-unit-test"></a>修改單元測試的程式碼  
...
2.  在程式碼編輯器，巡覽至 Sales_uspCancelOrderTest 方法。 修改方法的屬性以符合下列程式碼：  
    ...
    > [!IMPORTANT]  
    > 目前 Visual Studio 2012 不支援 ExpectedSqlException 屬性。 如需解決此問題的詳細資訊，請參閱[無法執行「預期的失敗」資料庫單元測試](http://social.msdn.microsoft.com/Forums/en-US/ssdt/thread/e74e06ad-e3c9-4cb0-97ad-a6f235a52345) \(英文\)。  
