...
5. 按下 **Ctrl + N** 開啟新的編輯器。
6. 先前的步驟顯示如何輕鬆地取得正確格式的資料，以便完成呼叫 *setCustomer* 程序。 您可以查看下列程式碼：使用相同 JSON 格式與新的客戶詳細資料，讓我們能測試 setCustomer 預存程序。 陳述式包含參數宣告和執行新的 Get 與 Set 程序語法。 您可以貼上從上一章節複製的資料，並編輯它與下列範例相同，或簡單地貼上下列陳述式到查詢編輯器中。
...
7. 按下**F5**鍵執行指令碼。 指令碼將新增客戶資訊，並以 JSON 格式傳回新的客戶資訊。 按一下結果以開啟的格式化檢視。
