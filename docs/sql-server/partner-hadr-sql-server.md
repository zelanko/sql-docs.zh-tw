...
|![hpe][4] |**HPE Serviceguard**<br>使用 HPE Serviceguard for Linux (SGLX)，可在不同距離的實體和虛擬環境間發現大量的基礎結構和應用程式錯誤，以防止在 Linux ® 上重要的 SQL Server 2017 工作負載發生規劃和未規劃的停機。 HPE SGL X 是 Beta 程式的一部分，會將內容相關性監視和復原選項，提供給容錯移轉叢集執行個體和永遠可用的可用性群組 SQL Server 工作負載。 HPE SGLX 讓運作時間最大化，同時仍顧及資料完整性和效能。<br><br>Linux 上的 SQL Server 2017 - RedHat 7.3、7.4、SuSE 12 SP2、SP3|[網站][hpe_website]<br>[資料工作表][hpe]<br>[下載][hpe_download]<br>[Twitter][hpe_twitter]
