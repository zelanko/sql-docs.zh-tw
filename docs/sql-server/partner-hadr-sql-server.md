...
|![NEC][7]|**NEC**<br>ExpressCluster 是全方位且完全自動化的高可用性和災害復原解決方案，可處理所有主要失敗，包括在實體電腦上或在內部部署或雲端環境中虛擬機器上執行之 SQL Server 和相關應用程式的硬體、軟體、網路和站台失敗。<br><br>Microsoft SQL Server：2005 或更新版本；所有版本 |[網站][necec_website]<br>[資料工作表][necec_datasheet]<br>[影片][necec_youtube]<br>[下載][necec_download]|
...
若要深入了解其他合作夥伴，請參閱[監視][mon_partners]、[管理夥伴][management_partners]及[開發夥伴][dev_partners]。
