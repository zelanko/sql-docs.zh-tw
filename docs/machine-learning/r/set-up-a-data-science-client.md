...
如果您的程式碼需要 SQL Server 預設不會安裝的套件，請向資料庫管理員安排以搭配執行個體安裝那些套件。 SQL Server 是安全的環境，因此會限制可以安裝套件的位置。 如需詳細資訊，請參閱[在 SQL Server 上安裝新的 R 套件](../package-management/install-additional-r-packages-on-sql-server.md)。
