...
SQL Server Trusted Launchpad 服務支援 R 和 Python 外部指令碼執行。 

包括設定問題或變更，或是遺失網路通訊協定等等的許多問題都可能會使啟動控制板無法啟動。 本文提供許多問題的疑難排解指南。 如果您發現我們沒注意到的問題，可以將問題張貼至[機器學習服務伺服器論壇](https://social.msdn.microsoft.com/Forums/home?category=MicrosoftR).\(英文\)。
