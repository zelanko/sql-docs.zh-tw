...
此項目有一部分已經完成。  [SqlPackage.exe](../tools/sqlpackage-download.md) 現已在適用於 Windows、macOS 及 Linux 的 .NET Core 上提供。  SQL 專案 (SSDT) 功能已在 Azure Data Studio 的 [SQL Database Projects 延伸模組](extensions/sql-database-project-extension.md)中啟用。
...
是。 Azure Synapse Analytics 的 Azure Data Studio 支援和 Azure SQL 受控執行個體及 SQL Server 2019 巨量資料，目前皆處於預覽階段。
