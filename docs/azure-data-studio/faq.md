...
Azure Data Studio 為一個新的開源，跨平台的桌面環境，提供資料專業人員，在 Windows、MacOS 和 Linux 上使用內部和雲端資料平台的 Azure 資料系列服務。 先前以預覽名稱 SQL Operations Studio 發行，Azure Data Studio 提供現代化編輯器體驗，具有閃電般快速的 IntelliSense，程式碼片段，原始檔控制整合和整合式終端機。 它在設計時考慮了資料庫平台使用者，內建查詢結果集合和可自定義的儀表板。

研究顯示，在使用 SQL Server Management Studio 時與任何其他工作相比，使用者花費更多時間在查詢編輯上。 基於這個理由，Azure Data Studio 深度專注於這個功能，並以可選用擴充套件加入產品以提供額外體驗。 這允許每位使用者依據他們最常使用的工作流程來自訂他們的環境。
