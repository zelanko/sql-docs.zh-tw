---
description: MDX 陳述式參考 (MDX)
title: Mdx 語句參考 (MDX) |Microsoft Docs
ms.date: 06/04/2018
ms.prod: sql
ms.technology: analysis-services
ms.custom: mdx
ms.topic: reference
ms.author: owend
ms.reviewer: owend
author: minewiskan
ms.openlocfilehash: bc16426b29deda2419f861a5456a2807f37648b8
ms.sourcegitcommit: e700497f962e4c2274df16d9e651059b42ff1a10
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 08/17/2020
ms.locfileid: "88500501"
---
# <a name="mdx-statement-reference-mdx"></a>MDX 陳述式參考 (MDX)


  多維度運算式 (MDX) 陳述式共分成三類，如下表所述：  
  
## <a name="in-this-section"></a>本節內容  
  
|主題|描述|  
|-----------|-----------------|  
|[MDX 指令碼陳述式 &#40;MDX&#41;](../mdx/mdx-scripting-statements-mdx.md)|包含 MDX 指令碼陳述式管理 MDX 指令碼內的查詢內容、範圍與流程控制的資訊。|  
|[Mdx 資料定義語句 &#40;MDX&#41;](../mdx/mdx-data-definition-statements-mdx.md)|包含 MDX 資料定義陳述式建立、卸除和管理多維度物件的資訊。|  
|[Mdx 資料動作陳述式 &#40;MDX&#41;](../mdx/mdx-data-manipulation-statements-mdx.md)|包含 MDX資料管理陳述式從多維度物件擷取和管理資料的資訊。|  
  
## <a name="see-also"></a>另請參閱  
 [MDX 語言參考 &#40;MDX&#41;](../mdx/mdx-language-reference-mdx.md)  
  
  
