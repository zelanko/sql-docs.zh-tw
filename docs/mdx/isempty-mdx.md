...
## <a name="remarks"></a>備註  
 **IsEmpty**函式會傳回 **，則為 true**如果評估的運算式是空的資料格的值。 否則，此函數會傳回**false**。  
  
> [!NOTE]  
>  成員的預設屬性就是成員的值。  
  
 **IsEmpty**函式是因為空白資料格值中具有特殊意義，可靠地測試空白資料格的唯一方式[!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)]。  
