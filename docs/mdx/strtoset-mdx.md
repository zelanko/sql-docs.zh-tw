...
ms.locfileid: "88386774"
---
# <a name="strtoset-mdx"></a>StrToSet (MDX)


  傳回多維度運算式所指定的集合， (MDX) 格式的字串。  
...
## <a name="remarks"></a>備註  
 **StrToSet**函數會傳回字串運算式中指定的集合。 **StrToSet**函數通常會搭配使用者自訂函數使用，以將外部函式的集合規格傳回至 mdx 語句，或在 mdx 查詢參數化時使用。  
  
-   使用限制旗標時，集合規格必須包含限定或未限定成員名稱，或包含以大括弧括住的限定或未限定成員名稱的一組元組 {} 。 這個旗標用於降低遭到由指定字串發動資料隱碼攻擊的風險。 如果所提供的字串不能直接解析成限定或未限定成員名稱，會出現下列錯誤：「違反了 STRTOSET 函數中 CONSTRAINED 旗標所加諸的限制。」  
...
## <a name="examples"></a>範例  
 下列範例會使用 **StrToSet** 函數傳回省份屬性階層的成員集合。 集合規格提供有效 MDX 集合運算式。  
