---
title: Analysis Services 語言參考 |Microsoft Docs
ms.date: 06/04/2018
ms.prod: sql
ms.technology: analysis-services
ms.custom: mdx
ms.topic: reference
ms.author: owend
ms.reviewer: owend
author: minewiskan
manager: kfile
ms.openlocfilehash: 045bbc74fcd626059b0442026d13f7d84b53c24f
ms.sourcegitcommit: a6949111461eda0cc9a71689f86b517de3c5d4c1
ms.translationtype: MT
ms.contentlocale: zh-TW
ms.lasthandoff: 06/19/2019
ms.locfileid: "67263323"
---
# <a name="analysis-services-language-reference"></a>Analysis Services 語言參考


  [Data Analysis Expressions (DAX) 參考](/dax/data-analysis-expressions-dax-reference)  
  
 [多維度運算式 &#40;MDX&#41; 參考](../mdx/multidimensional-expressions-mdx-reference.md)  
  
 [資料採礦延伸模組 &#40;DMX&#41; 參考](../dmx/data-mining-extensions-dmx-reference.md)  
  
 [MDX 和 DAX 中的 VBA 函式](../mdx/vba-functions-in-mdx-and-dax.md)  
  
## <a name="see-also"></a>另請參閱  
 [Power pivot for SharePoint 的 PowerShell 參考](../analysis-services/powershell/powershell-reference-for-power-pivot-for-sharepoint.md)   
 [Analysis Services PowerShell 參考](../analysis-services/powershell/analysis-services-powershell-reference.md)   
 [資料採礦預存程序 &#40;Analysis Services - 資料採礦&#41;](../analysis-services/data-mining/data-mining-stored-procedures-analysis-services-data-mining.md)  
  
  







