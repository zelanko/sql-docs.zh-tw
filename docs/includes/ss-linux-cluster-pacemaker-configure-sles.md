2. 在所有叢集節點上，建立檔案以儲存 SQL Server 使用者名稱和密碼，以供 Pacemaker 登入使用。 下列命令會建立並填入這個檔案：
...
3. 啟用並啟動 `pcsd` 服務和 Pacemaker。 這將會允許節點在重新開機後重新加入叢集。 在所有節點上執行下列命令。
...
4. 安裝 SQL Server 的 FCI 資源代理程式。 在所有節點上執行下列命令。 
