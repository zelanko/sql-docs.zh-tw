...
SQL Server 2017 CTP 1.4 將 `sequence_number` 加入到 `sys.availability_groups` 以解決此問題。 `sequence_number` 是依序遞增的 BIGINT，表示本機 AG 複本相對於系統其餘部分的最新更新狀態。 執行容錯移轉、新增/移除複本以及其他 AG 作業都會更新這個數字。 這個數字會針對主要複本更新，然後推送到次要複本。 因此最新的次要複本會有與主要複本相同的數字。
